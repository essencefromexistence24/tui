...
/// The default Dx user directory (`~/Dx/tui`, canonicalized) used when
...
/// `~/Dx/tui/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// Per-user config directory: `$GROK_HOME` or `~/Dx/tui`. Created if needed.
...
mod tests {
    ...
    fn default_grok_home_has_no_verbatim_prefix() {
        ...
        assert!(home.ends_with(std::path::Path::new("Dx").join("tui")));
    }
