...
use tracing::error;
use fb_macro::try_format;
use fb_shared::{Id, RoCell};
...
impl Client {
	...
	pub async fn shot(kind: &str, receiver: Id, body: &str) -> Result<()> {
		...
		if version.as_deref() != Some(EmberHi::version()) {
			bail!(
				"Incompatible file-browser version (helper {}, shell {}). Restart all related DX TUI file-browser processes after upgrading.",
				EmberHi::version(),
	...
	/// sent by other file-browser instances:
	...
	pub async fn draw(kinds: HashSet<&str>) -> Result<()> {
		...
		let mut lines = make(&kinds).await.context("No running DX TUI file-browser instance found")?;
		loop {
