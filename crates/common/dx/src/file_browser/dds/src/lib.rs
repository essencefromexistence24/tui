...
fb_macro::mod_pub!(ember spark);

fb_macro::mod_flat!(client payload pubsub pump sendable server state stream);

pub fn init() {
	let (tx, rx) = tokio::sync::mpsc::unbounded_channel();

	// Client
	ID.init(fb_boot::ARGS.client_id.unwrap_or(fb_shared::Id::unique()));
