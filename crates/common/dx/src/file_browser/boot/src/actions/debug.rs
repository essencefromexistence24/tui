...
use regex::Regex;
use fb_config::{THEME, YAZI};
use fb_emulator::Mux;
use fb_fs::Xdg;
use fb_shared::timestamp_us;
...
impl Actions {
	pub(super) fn debug() -> Result<String, std::fmt::Error> {
		...
		writeln!(s, "    File browser     : {}", Self::config_state("yazi.toml"))?;
		...
		writeln!(s, "    Brand.from_env      : {:?}", fb_emulator::Brand::from_env())?;
		writeln!(s, "    Emulator.detect     : {:?}", &*fb_emulator::EMULATOR)?;

		writeln!(s, "\nAdapter")?;
		writeln!(s, "    Adapter.matches    : {:?}", fb_adapter::ADAPTOR)?;
		writeln!(s, "    Dimension.available: {:?}", fb_emulator::Dimension::available())?;
		...
		writeln!(s, "\nSSH")?;
		writeln!(s, "    shared.in_ssh_connection: {}", fb_shared::in_ssh_connection())?;

		writeln!(s, "\nWSL")?;
		writeln!(s, "    WSL: {:?}", fb_adapter::WSL)?;
		...
		writeln!(s, "    fb_config_HOME   : {:?}", env::var_os("fb_config_HOME"))?;
		...
		writeln!(s, "\nMultiplexers")?;
		writeln!(s, "    TMUX               : {}", fb_emulator::TMUX)?;
		...
		writeln!(
			s,
			"\n\nEnable logging to inspect DX TUI file-browser runtime errors."
		)?;
