...
use fb_boot::BOOT;
use fb_fs::provider::local::Local;
use fb_macro::plugin_preset as preset;
use fb_shared::{BytesExt, LOG_LEVEL, RoCell};
...
impl Loader {
	...
	pub fn compatible_or_error(id: &str, chunk: &Chunk) -> Result<()> {
		...
		bail!(
			"Plugin `{id}` requires at least DX TUI file browser {}, but your current version is {}.",
			chunk.since,
			fb_boot::actions::Actions::version()
		);
	}
