...
fn main() {
	...
	// live under crates/dx-gui/resources/windows/ — this build script
	...
	#[cfg(target_os = "windows")]
	{
		...
		if ico_path.exists() {
			...
			res.set("ProductName", "Dx TUI Editor");
			res.set("FileDescription", "Dx TUI Editor — fast terminal-based text editor");
			res.set("CompanyName", "Dx");
			res.set("LegalCopyright", "Licensed under GPL-2.0");
			res.set("OriginalFilename", "dx.exe");
			res.set("InternalName", "dx");
			...
			if std::env::var("CARGO_FEATURE_GUI").is_ok() {
				let manifest = "../dx-gui/resources/windows/dx.manifest";
				if Path::new(manifest).exists() {
					res.set_manifest_file(manifest);
				} else {
					eprintln!("Warning: gui feature enabled but manifest missing at {}", manifest);
				}
			}
...
fn collect_theme_files(
	...
	for entry in entries {
		...
		if path.is_dir() {
			...
			let new_pack =
				if pack.is_empty() { dir_name.to_string() } else { format!("{}/{}", pack, dir_name) };
...
fn generate_syntax_packdump() -> Result<(), Box<dyn std::error::Error>> {
	...
	let grammar_files: &[(&str, &str)] = &[
		...
		("src/grammars/git-commit.sublime-syntax", "Git Commit Message"),
		("src/grammars/gitignore.sublime-syntax", "Gitignore"),
		("src/grammars/gitconfig.sublime-syntax", "Git Config"),
		("src/grammars/gitattributes.sublime-syntax", "Git Attributes"),
		...
		("src/grammars/autohotkey/AutoHotkey.sublime-syntax", "AutoHotkey"),
	...
	for (path, name) in grammar_files {
		let content =
			fs::read_to_string(path).unwrap_or_else(|e| panic!("Failed to read grammar {}: {}", path, e));
