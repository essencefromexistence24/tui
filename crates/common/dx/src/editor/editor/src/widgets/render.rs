...
use dx_core::text_property::{InlineOverlay, OffsetUnit, TextPropertyEntry};
use serde_json::json;
...
fn collect_tabbable(spec: &WidgetSpec, out: &mut Vec<String>) {
	match spec {
		WidgetSpec::Button { key: Some(k), disabled, focusable, .. }
		...
		WidgetSpec::List { key: Some(k), focusable, .. } if !k.is_empty() && *focusable => {
			out.push(k.clone());
		}
		_ => {}
	}
...
fn render_collected(
	...
	match spec {
		...
		WidgetSpec::DualList { options, included, excluded, label, focused, visible_rows, key } => {
			collect_dual_list(
				options,
				included,
				excluded,
		...
		WidgetSpec::Button { label, focused, intent, key, disabled, .. } => {
			collect_button(label, *focused, *intent, key.as_deref(), *disabled, focus_key)
		}
		...
		WidgetSpec::WindowEmbed { window_id, rows, .. } => {
...
fn collect_row(
	...
	let has_blocks = row_pieces.iter().any(|p| matches!(p, RowPiece::Block { .. }));
	if has_blocks {
		zip_row_blocks(
			row_pieces,
			panel_width,
	...
	CollectedOutput { entries, hits, focus_cursor, embeds, overlays, scroll_regions }
}
...
fn allocate_row_child_widths(children: &[WidgetSpec], panel_width: u32) -> Vec<u32> {
	let block_indices: Vec<usize> =
		children.iter().enumerate().filter(|(_, c)| predicts_block(c)).map(|(i, _)| i).collect();
...
fn assemble_inline_row(
	...
	let inline_natural: usize = pieces
		...
		.sum();
	let flex_count = pieces.iter().filter(|p| matches!(p, RowPiece::Flex)).count();
	...
	for child in children {
		...
		if is_overlay {
			...
			for (i, e) in child_out.entries.into_iter().enumerate() {
				overlays.push(OverlayRow { buffer_row: row_offset + i as u32, entry: e });
			}
...
fn collect_toggle(
	...
	let (mut entry, chip_range) = if label_first {
		render_toggle_form(checked, indeterminate, label, is_focused, label_width, panel_width)
	} else {
...
fn collect_number(
	...
	let RenderedNumber { mut entry, value_range } = rendered;
...
fn collect_dropdown(
	...
	let (cur, state_open) = match key {
		Some(k) if !k.is_empty() => match prev.get(k) {
			Some(WidgetInstanceState::Dropdown { selected_index, open }) => {
	...
	let cur = if options.is_empty() { 0 } else { cur.clamp(0, options.len() as i32 - 1) };
	...
	if let Some(k) = key {
		if !k.is_empty() {
			next_state.insert(k.to_string(), WidgetInstanceState::Dropdown { selected_index: cur, open });
		}
	}

	let RenderedDropdown { mut entry, button_range, option_rows, scroll_offset } =
		render_dropdown(options, cur, label, is_focused, label_width, open, spec_scroll);
...
fn render_list_cards(
	...
	for item_spec in item_specs.iter() {
		...
		let card_entries = render_collected(item_spec, prev, &mut scratch, focus_key, width).entries;
...
fn plan_list_layout(
	...
	let total = if use_specs { item_specs.len() as u32 } else { items_len as u32 };
	...
	let (prev_scroll, prev_sel, user_scrolled) = list_key
		...
		.and_then(|s| match s {
			WidgetInstanceState::List { scroll_offset, selected_index, user_scrolled, .. } => {
	...
	let visible_items = if use_specs { (avail_rows / item_height).max(1) } else { avail_rows };
...
fn effective_text_field_width(
	...
	let label_overhead = if label.is_empty() { 0u32 } else { label.chars().count() as u32 + 1 };
	let marker_reserve = if marker_gutter_enabled() { 2 } else { 0 };
	panel_width.saturating_sub(label_overhead).saturating_sub(3).saturating_sub(marker_reserve).max(1)
}
...
fn emit_completion_overlays(
	...
	completions: &[dx_core::api::CompletionItem],
	visible_rows: u32,
	panel_width: u32,
	selected_idx: usize,
	navigated: bool,
	...
	out
		.overlays
		.push(OverlayRow { buffer_row: anchor, entry: render_completion_bottom_border(popup_total) });
	scroll
}
...
fn push_block_caret_overlay(entry: &mut TextPropertyEntry, byte: usize) {
	...
	let (start, end) = if b < text.len() {
		...
		let last_start =
			text.char_indices().last().map(|(i, _)| i).unwrap_or(text.len().saturating_sub(1));
		(last_start, text.len())
	} else {
		return;
	};
	entry.inline_overlays.push(InlineOverlay {
		...
		style: OverlayOptions { reversed: true, ..Default::default() },
		properties: Default::default(),
		unit: OffsetUnit::Byte,
	});
}
...
fn render_widget_text(
	...
	let effective_visible_rows =
		if completions_visible_rows == 0 { 5u32 } else { completions_visible_rows };
	...
	let mut prev_completions: Vec<dx_core::api::CompletionItem> = Vec::new();
	...
	match key.filter(|k| !k.is_empty()).and_then(|k| prev.get(k)) {
		...
		_ => {
			...
			let seed =
				if cursor_byte < 0 { value.len() } else { (cursor_byte as usize).min(value.len()) };
	...
	let effective_cursor = if is_focused { effective_cursor_byte } else { -1 };
	...
	let effective_field_width =
		effective_text_field_width(full_width, multiline, effective_label, panel_width, field_width);
	...
	if multiline {
		...
		{
			out.focus_cursor = Some(FocusCursor { buffer_row, byte_in_row: byte_in_row as u32 });
		}
		...
		if let Some(byte_in_row) = cursor_in_row {
			out.focus_cursor = Some(FocusCursor { buffer_row: 0, byte_in_row: byte_in_row as u32 });
			// Modal surfaces paint the caret as a REVERSED cell in the
			// row itself (no hardware cursor over a modal).
			if block_caret {
...
fn render_widget_tree(
	...
	let (prev_scroll, prev_sel, prev_expanded) = match prev_state {
		Some(WidgetInstanceState::Tree { scroll_offset, selected_index, expanded_keys }) => {
	...
	for (i, node) in nodes.iter().enumerate() {
		...
		let is_open =
			if node.has_children { !key.is_empty() && prev_expanded.contains(&key) } else { true };
...
fn render_completion_item(
	...
	if is_history {
		...
		inline_overlays.push(InlineOverlay {
			...
			style: OverlayOptions { italic: true, ..Default::default() },
			properties: Default::default(),
			unit: OffsetUnit::Byte,
		});
	}
...
fn completion_scrollbar_glyph(
	...
	if visible_row >= thumb_top && visible_row < thumb_top + thumb_size { Some('█') } else { None }
}
...
fn wrap_in_side_border(mut child: TextPropertyEntry, inner_width: usize) -> TextPropertyEntry {
	...
	if cur_cols < inner_width {
		...
		let byte_cutoff = indices.get(inner_width).copied().unwrap_or(child.text.len());
...
/// `crates/dx-editor/plugins/search_replace.ts:535–541`,
...
pub fn render_number(
	...
	match &edit {
		None => {
			...
			text.push_str(&format!("{:>width$} ", value_str, width = NUMBER_CELL_MIN_WIDTH));
		}
		Some(e) => {
	...
	RenderedNumber { entry, value_range: (cell_start, cell_end) }
}
...
pub fn render_toggle_form(
	...
	let label_cell = if label_width == 0 {
		...
		if lw == 0 { label.to_string() } else { fit_label(label, lw) }
	};
	let mut text = String::new();
	text.push_str(marker);
...
pub fn render_dropdown(
	...
	let max_option_len = options.iter().map(|s| s.chars().count()).max().unwrap_or(10);
	...
	if open {
		let indent = marker.len()
			+ if label.is_empty() { 0 } else { pad_label(label, label_width as usize).len() + 2 };
		for (row_i, opt) in options.iter().skip(scroll).take(visible).enumerate() {
			...
			let style = if idx == selected {
				...
				OverlayOptions { bg: Some(OverlayColorSpec::theme_key(KEY_INPUT_BG)), ..Default::default() }
			};
...
pub fn dual_label<'a>(options: &'a [DualListOption], value: &'a str) -> &'a str {
	options.iter().find(|o| o.value == value).map(|o| o.label.as_str()).unwrap_or(value)
}
...
pub fn dual_sanitize_included(options: &[DualListOption], included: &[String]) -> Vec<String> {
	included.iter().filter(|v| options.iter().any(|o| &o.value == *v)).cloned().collect()
}
...
fn dual_col_width(panel_width: u32) -> usize {
	...
	let width = if panel_width == u32::MAX { 40 } else { panel_width };
	...
	let (included, active_included, mut avail_cur, mut incl_cur) = match key {
		Some(k) if !k.is_empty() => match prev.get(k) {
			Some(WidgetInstanceState::DualList {
				...
				(included.clone(), *active_included, *available_cursor as usize, *included_cursor as usize)
			}
	...
	for i in 0..body_rows {
		...
		if is_focused {
			let (hs, he) = if active_included {
				if right_val.is_some() && i == incl_cur { (right_start, right_end) } else { (0, 0) }
...
pub fn render_tree_row(node: &TreeNode, expanded: bool, checkable: bool) -> RenderedTreeRow {
	...
	let mut text = String::with_capacity(
		indent_cols + disclosure_glyph.len() + separator.len() + checkbox_extra + node.text.text.len(),
	);
	...
	let disclosure_range = if node.has_children { Some((disc_start, disc_end)) } else { None };
	let entry = TextPropertyEntry {
		text,
	...
	RenderedTreeRow { entry, disclosure_range, checkbox_range }
}
...
pub fn render_text_input(
	...
	let cursor_byte_in_entry =
		if focused { cursor_in_inner.map(|c| inner_byte_start + c) } else { None };
...
pub fn render_text_area(
		...
		if focused {
			if let Some(((sl, sc), (el, ec))) = selection_lc {
				if line_idx >= sl && line_idx <= el {
					let line_text_len = if line_idx < lines.len() { lines[line_idx].len() } else { 0 };
	...
	RenderedTextArea { entries, scroll_row: scroll_row as u32, cursor_buffer_row, cursor_byte_in_row }
}
...
fn assemble_wrapped_row(
	...
	for piece in pieces {
		let RowPiece::Inline { mut entry, hits: child_hits, .. } = piece else {
...
fn pad_or_truncate_cols(text: &mut String, cols: usize) {
	...
	if cur < cols {
		...
		let cutoff = text.char_indices().nth(cols).map(|(i, _)| i).unwrap_or(text.len());
...
mod tests {
	...
	fn render_no_focus(
		spec: &WidgetSpec,
		prev: &HashMap<String, WidgetInstanceState>,
	) -> (Vec<TextPropertyEntry>, Vec<HitArea>, HashMap<String, WidgetInstanceState>) {
	...
	fn form_toggle_chip_stays_visible_on_narrow_panel() {
		...
		assert!(w <= panel as usize, "row must fit the panel ({w} > {panel}): {:?}", entry.text);
	...
	fn hint_bar_renders_entries_with_key_overlays() {
		let entries = vec![
			HintEntry { keys: "Tab".into(), label: "next".into() },
			HintEntry { keys: "Esc".into(), label: "close".into() },
		];
	...
	fn hint_bar_omits_label_when_empty() {
		let entries = vec![HintEntry { keys: "?".into(), label: "".into() }];
		let entry = render_hint_bar(&entries);
	...
	fn col_stacks_children_top_to_bottom() {
		let spec = WidgetSpec::Col {
			children: vec![
				WidgetSpec::HintBar {
					entries: vec![HintEntry { keys: "A".into(), label: "alpha".into() }],
					key: None,
				},
				WidgetSpec::HintBar {
					entries: vec![HintEntry { keys: "B".into(), label: "beta".into() }],
					key: None,
				},
			],
			key: None,
	...
	fn raw_passes_through_unchanged() {
		let spec = WidgetSpec::Raw { entries: vec![TextPropertyEntry::text("hello")], key: None };
		let (out, hits, _state) = render_no_focus(&spec, &HashMap::new());
		assert_eq!(out.len(), 1);
	...
	fn button_primary_unfocused_is_bold_help_key_fg_with_no_bg() {
		...
		assert!(style.bold);
		assert_eq!(style.fg.as_ref().and_then(|c| c.as_theme_key()), Some("ui.help_key_fg"),);
	...
	fn button_focused_overrides_with_popup_selection_keys() {
		...
		let style = &entry.inline_overlays[0].style;
		assert_eq!(style.fg.as_ref().and_then(|c| c.as_theme_key()), Some("ui.popup_selection_fg"));
		assert_eq!(style.bg.as_ref().and_then(|c| c.as_theme_key()), Some("ui.popup_selection_bg"));
		assert!(style.bold);
	}
	...
	fn row_collapses_inline_children_with_shifted_overlays() {
		let spec = WidgetSpec::Row {
			...
			children: vec![
				WidgetSpec::HintBar {
					entries: vec![HintEntry { keys: "Tab".into(), label: "x".into() }],
					key: None,
				},
				WidgetSpec::HintBar {
					entries: vec![HintEntry { keys: "Esc".into(), label: "y".into() }],
					key: None,
				},
			],
			key: None,
	...
	fn disabled_button_uses_menu_disabled_fg_overlay() {
		...
		assert!(!style.bold, "disabled buttons drop the intent's bold emphasis");
	...
	fn collect_tabbable_visits_widgets_with_keys_in_declaration_order() {
		let spec = WidgetSpec::Col {
			children: vec![
				WidgetSpec::HintBar { entries: vec![], key: Some("hb".into()) },
				WidgetSpec::Row {
					wrap: false,
					children: vec![
						WidgetSpec::Toggle {
	...
	fn focused_widget_renders_with_focused_styling() {
		...
		let focused_overlay = entry
			.inline_overlays
			.iter()
			.find(|o| o.style.bg.as_ref().and_then(|c| c.as_theme_key()) == Some("ui.popup_selection_bg"))
	...
	fn no_tabbables_yields_empty_focus_key() {
		let spec = WidgetSpec::Col {
			children: vec![WidgetSpec::HintBar { entries: vec![], key: None }],
			key: None,
		};
		let out = render_spec(&spec, &HashMap::new(), "", u32::MAX);
	...
	fn list_item_specs_render_multirow_cards_in_item_units() {
		...
		let card = |body: &str| WidgetSpec::LabeledSection {
			label: String::new(),
			child: Box::new(WidgetSpec::Raw { entries: vec![TextPropertyEntry::text(body)], key: None }),
			width_pct: None,
			key: None,
		};
		let spec = WidgetSpec::List {
		...
		assert!(hits[0..3].iter().all(|h| h.payload["index"] == 0 && h.widget_key == "a"));
		assert!(hits[3..6].iter().all(|h| h.payload["index"] == 1 && h.widget_key == "b"));
	...
	fn list_applies_selection_bg_to_selected_row() {
		let spec = WidgetSpec::List {
			items: vec![TextPropertyEntry::text("first"), TextPropertyEntry::text("second")],
			item_specs: vec![],
	...
	fn list_inside_col_offsets_hit_rows_by_preceding_lines() {
		let spec = WidgetSpec::Col {
			children: vec![
				WidgetSpec::HintBar {
					entries: vec![HintEntry { keys: "h".into(), label: "header".into() }],
					key: None,
				},
				WidgetSpec::List {
					items: vec![TextPropertyEntry::text("row0"), TextPropertyEntry::text("row1")],
					item_specs: vec![],
					item_keys: vec!["a".into(), "b".into()],
					selected_index: -1,
					visible_rows: 10,
	...
	fn make_list(selected: i32, visible: u32, total: usize, key: Option<&str>) -> WidgetSpec {
		let items = (0..total).map(|i| TextPropertyEntry::text(format!("row{}", i))).collect();
	...
	fn list_without_key_does_not_persist_state() {
		...
		assert!(state.is_empty(), "Lists without a `key` opt out of state preservation");
	}
	...
	fn text_input_unfocused_skips_selection_overlay() {
		...
		// shows no highlight.
		let entry = render_text_input("hello", -1, Some((0, 5)), false, "", None, 0, 0, false).entry;
	...
	fn text_input_unfocused_empty_shows_placeholder_in_muted() {
		let entry = render_text_input("", -1, None, false, "", Some("type here"), 0, 0, false).entry;
	...
	fn text_input_field_width_head_truncates_long_value() {
		...
		// chars + `…`; no pad space.
		let r =
			render_text_input("0123456789abcdefghijklmnopqrst", 30, None, false, "", None, 0, 10, false);
	...
	fn tnode(text: &str, depth: u32, has_children: bool) -> TreeNode {
		TreeNode { text: TextPropertyEntry::text(text), depth, has_children, checked: None }
	}
	...
	fn tree_row_shifts_plugin_overlays_by_prefix() {
		...
		node.text.inline_overlays.push(InlineOverlay {
			...
			style: OverlayOptions { bold: true, ..Default::default() },
			properties: Default::default(),
			unit: OffsetUnit::Byte,
		});
		...
		let plugin_overlay =
			r.entry.inline_overlays.iter().find(|o| o.style.bold).expect("bold overlay carried through");
	...
	fn tree_row_renders_unchecked_glyph_for_leaf() {
		...
		let r = render_tree_row(&node, false, true);
		let (cb_start, cb_end) = r.checkbox_range.expect("checkbox range for leaf with checked: Some");
	...
	fn tree_node_pad_to_chars_pads_text_before_prefix_offset_shift() {
		...
		assert!(trimmed.ends_with("x    "), "row should end with the padded body, got {trimmed:?}");
	}
	...
	fn tree_node_truncate_to_chars_cuts_body_before_prefix_offset_shift() {
		...
		assert!(trimmed.ends_with("abc..."), "row should end with truncated body, got {trimmed:?}");
	}
	...
	fn tree_node_char_unit_overlay_resolves_against_padded_text_and_shifts_by_prefix() {
		...
		let entry = &entries[0];
		let bold =
			entry.inline_overlays.iter().find(|o| o.style.bold).expect("bold overlay carried through");
	...
	fn tree_node_segments_concatenate_into_row_text_with_per_segment_overlays() {
		...
		node.text.segments = vec![
			dx_core::text_property::StyledSegment {
				text: "AB".to_string(),
				style: None,
				overlays: vec![],
			},
			...
			dx_core::text_property::StyledSegment {
				text: "CD".to_string(),
				style: Some(OverlayOptions { bold: true, ..Default::default() }),
				overlays: vec![],
			},
		];
		let spec = make_tree(vec![node], vec!["x"], -1, 10, vec![], Some("T"));
	...
	fn tree_node_segments_with_pad_pad_after_concatenation() {
		let mut node = tnode("", 0, false);
		node.text.segments = vec![dx_core::text_property::StyledSegment {
			text: "ab".to_string(),
			style: None,
			overlays: vec![],
		}];
	...
	fn tree_hits_carry_tree_spec_key_and_per_item_key_in_payload() {
		let spec =
			make_tree(vec![tnode("only", 0, false)], vec!["only-key"], -1, 10, vec![], Some("matchTree"));
		let (_entries, hits, _state) = render_no_focus(&spec, &HashMap::new());
	...
	fn tree_tabbable_keys_include_tree_with_key() {
		let spec = WidgetSpec::Col {
			children: vec![
				...
				make_tree(vec![tnode("a", 0, false)], vec!["a"], -1, 10, vec![], Some("tree")),
			],
			key: None,
		};
		let mut tabbable = Vec::new();
	...
	fn zip_row_blocks_keeps_overlays_on_char_boundaries() {
		...
		let spec = WidgetSpec::Row { wrap: false, children: vec![left, right], key: None };
		let out = render_spec(&spec, &HashMap::new(), "", 40);
	...
	fn labeled_section_text_full_width_fills_inner_area() {
		...
		assert!(middle.text.contains("[ab        ]"), "actual: {:?}", middle.text);
	}
	...
	fn number_editing_shows_buffer_selection_and_caret() {
		let r = render_number(
			...
			false,
			0,
			Some(NumberEdit { text: "750", cursor: 3, sel_start: 0, sel_end: 3 }),
		);
		assert_eq!(r.entry.text, "Size: [750 ]");
		// Selection bg over the digits + a REVERSED caret cell.
		assert!(r.entry.inline_overlays.iter().any(|o| o.style.bg.is_some() && !o.style.reversed));
	...
	fn dropdown_open_renders_inline_option_rows() {
		let r =
			render_dropdown(&["Red".into(), "Green".into(), "Blue".into()], 1, "Color", true, 0, true, 0);
		assert!(r.entry.text.contains("▲"));
	...
	fn dropdown_emits_button_and_option_hit_areas() {
		...
		let (_out, hits, _state) = render_no_focus(&spec, &HashMap::new());
		let toggles: Vec<_> = hits.iter().filter(|h| h.event_type == "dropdown_toggle").collect();
		let selects: Vec<_> = hits.iter().filter(|h| h.event_type == "dropdown_select").collect();
	...
	fn dropdown_instance_state_overrides_spec() {
		...
		let mut prev = HashMap::new();
		prev.insert("d".to_string(), WidgetInstanceState::Dropdown { selected_index: 2, open: false });
		let r = render_spec(&spec, &prev, "", u32::MAX);
		assert!(
	...
	fn dropdown_open_emits_inline_option_rows() {
		...
		// the value button.
		let mut prev = HashMap::new();
		prev.insert("d".to_string(), WidgetInstanceState::Dropdown { selected_index: 1, open: true });
		...
		let selects: Vec<_> = out.hits.iter().filter(|h| h.event_type == "dropdown_select").collect();
	...
	fn dropdown_popup_closes_when_unfocused() {
		...
		let mut prev = HashMap::new();
		prev.insert("d".to_string(), WidgetInstanceState::Dropdown { selected_index: 0, open: true });
	...
	fn opts(pairs: &[(&str, &str)]) -> Vec<DualListOption> {
		pairs
			.iter()
			.map(|(v, l)| DualListOption { value: v.to_string(), label: l.to_string() })
			.collect()
	}
