...
impl SplitRenderer {
	...
	pub fn render_content(
		...
		Vec<(crate::model::event::ContainerId, SplitDirection, u16, u16, u16)>,
	) {
...
mod tests {
	...
	use super::orchestration::render_line::{
		LastLineEnd, LineRenderInput, LineRenderOutput, render_view_lines,
	};
	...
	fn render_output_for(content: &str, cursor_pos: usize) -> (LineRenderOutput, usize, bool, usize) {
	...
	fn render_output_for_with_options(
		...
		let _ = state.buffer.populate_line_cache(viewport.top_byte, visible_count);
		...
		(output, state.buffer.len(), content.ends_with('\n'), selection.primary_cursor_position)
	}
	...
	fn viewport_end_is_bounded_for_overlong_single_line() {
		...
		let _ = state.buffer.populate_line_cache(viewport_start, visible_count);
	...
	fn indentation_guide_render_for_space_indents_but_not_root_lines() {
		...
		assert_eq!(rendered_line_text(&output, 2), "▏   ▏   let grandchild = 2;");
	...
	fn indentation_guide_resume_after_lower_indent_line() {
		let (output, _, _, _) =
			render_output_for_with_indentation_guide("        before\n    lower\n        after\n", 0, 0);
	...
	fn indentation_guide_render_for_tabs() {
		let (output, _, _, _) = render_output_for_with_indentation_guide("\tchild\n\t\tgrand\n", 0, 0);
	...
	fn test_folding_hides_lines_and_adds_placeholder() {
		...
		assert!(lines.iter().any(|l| l.contains("header") && l.contains("...")));
	}
	...
	fn fold_indicator_lands_on_header_not_blank_line_after_it() {
		...
		assert!(indicators.contains_key(&header_byte), "fold marker should be on the function header");
		assert!(!indicators.contains_key(&blank_byte), "fold marker must not be on the blank line");
	}
	...
	fn test_fold_indicators_collapsed_and_expanded() {
		...
		assert_eq!(indicators.get(&line1_byte).map(|i| i.collapsed), Some(false));
	}
	...
	fn last_line_end_tracks_trailing_newline() {
		...
		assert_eq!(
			...
			Some(LastLineEnd { pos: (3, 0), terminated_with_newline: true })
		);
	}
	...
	fn last_line_end_tracks_no_trailing_newline() {
		...
		assert_eq!(
			output.0.last_line_end,
			Some(LastLineEnd { pos: (3, 0), terminated_with_newline: false })
		);
	}
	...
	use dx_core::api::ViewTokenWireKind;
	...
	fn test_build_base_tokens_crlf_multiple_lines() {
		...
		let tokens =
			SplitRenderer::build_base_tokens_for_hook(&mut buffer, 0, 80, 10, false, LineEnding::CRLF);
	...
	fn test_build_base_tokens_lf_mode_for_comparison() {
		...
		let tokens =
			SplitRenderer::build_base_tokens_for_hook(&mut buffer, 0, 80, 10, false, LineEnding::LF);
	...
	fn test_apply_wrapping_transform_breaks_long_lines() {
		...
		let tokens = vec![
			...
			ViewTokenWire { kind: ViewTokenWireKind::Newline, source_offset: Some(25_000), style: None },
		];
		...
		let break_count = wrapped.iter().filter(|t| matches!(t.kind, ViewTokenWireKind::Break)).count();
		...
		assert_eq!(total_chars, 25_000, "Total character count should be preserved after wrapping");
	}
	...
	/// into a single `Text`).
	fn tokenize_for_wrap(text: &str) -> Vec<dx_core::api::ViewTokenWire> {
		use dx_core::api::{ViewTokenWire, ViewTokenWireKind};
	...
	fn rows_from_wrapped(wrapped: &[dx_core::api::ViewTokenWire]) -> Vec<String> {
		use dx_core::api::ViewTokenWireKind;
		let mut rows: Vec<String> = vec![String::new()];
		for tok in wrapped {
			match &tok.kind {
	...
	fn issue_1363_single_word_row_falls_back() {
		...
		let tokens = vec![
			...
			ViewTokenWire { source_offset: Some(8), kind: ViewTokenWireKind::Space, style: None },
			ViewTokenWire {
		...
		let rows = rows_from_wrapped(&wrapped);
		for row in &rows {
			assert!(row.chars().count() <= 8, "row {:?} exceeds eff_width 8 in fallback case", row,);
		}
	}
	...
	fn test_apply_wrapping_transform_preserves_short_lines() {
		...
		let tokens = vec![
			...
			ViewTokenWire { kind: ViewTokenWireKind::Newline, source_offset: Some(100), style: None },
		];
		...
		assert_eq!(break_count, 0, "Short lines should not have any breaks, got {}", break_count);
	...
	fn test_large_single_line_sequential_data_preserved() {
		...
		for i in 1..=num_markers {
			...
			assert!(reconstructed.contains(&marker), "Missing marker {} after pipeline", marker);
		}
	...
	fn line_has_current_line_bg(output: &LineRenderOutput, line_idx: usize) -> bool {
		...
		if let Some(line) = output.lines.get(line_idx) {
			line.spans.iter().any(|span| span.style.bg == Some(current_line_bg))
		} else {
			false
		}
	}
	...
	fn wrap_str_to_width_matches_apply_wrapping_transform() {
		...
		for &(text, wrap_width) in cases {
			...
			let helper_strings: Vec<&str> = helper_chunks.iter().map(|r| &text[r.clone()]).collect();
			...
			assert_eq!(
				transform_strings.iter().map(String::as_str).collect::<Vec<_>>(),
				helper_strings,
