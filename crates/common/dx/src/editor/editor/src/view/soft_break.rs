...
impl SoftBreakManager {
	...
	pub fn new() -> Self {
		Self { breaks: Vec::new(), marker_to_idx: HashMap::new(), version: 0 }
	}
	...
	pub fn add(
		...
		self.breaks.push(SoftBreakPoint { namespace, marker_id, indent });
	...
	pub fn remove_in_range(&mut self, start: usize, end: usize, marker_list: &mut MarkerList) {
		...
		let mut to_remove: Vec<usize> = hits
			...
			.filter_map(
				|(mid, pos, _)| {
					if *pos < end { self.marker_to_idx.get(mid).copied() } else { None }
				},
			)
			.collect();
	...
	pub fn query_viewport(
		...
		let mut results: Vec<(usize, u16)> = self
			...
			.filter_map(|b| {
				...
				if pos >= start && pos < end { Some((pos, b.indent)) } else { None }
			})
			.collect();
	...
	fn check_invariants(&self) {
		assert_eq!(self.marker_to_idx.len(), self.breaks.len(), "marker_to_idx size != breaks size");
...
mod tests {
	...
	fn test_soft_break_remove_in_range_keeps_only_outside() {
		...
		let kept: Vec<_> =
			manager.query_viewport(0, 1000, &marker_list).into_iter().map(|(p, _)| p).collect();
	...
	fn perf_full_buffer_rebuild_pass() {
		...
		let break_byte =
			|line: usize, k: usize| -> usize { line * LINE_BYTES + k * (LINE_BYTES / BREAKS_PER_LINE) };
		...
		let still_present = manager.query_viewport(0, LINES * LINE_BYTES, &marker_list).len();
