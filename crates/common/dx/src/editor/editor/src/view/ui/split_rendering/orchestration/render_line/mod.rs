...
use super::super::gutter::{LeftMarginContext, render_left_margin};
...
use super::tail_fill::{TailFillInput, resolve_tail_fill};
use cells::{CellPassInput, render_line_cells};
use trailing::{PostRowAccumulator, PostRowContext, fill_eof_rows, render_implicit_trailing_line};
...
use crate::view::ui::view_pipeline::{LineStart, ViewLine, should_show_line_number};
...
impl ActiveIndentationGuide {
	...
	fn for_view_lines(
		...
		let target_byte = indent_folding::find_line_start_byte(&state.buffer, primary_cursor_position);
		let (header_byte, body_start_byte, body_end_byte) = indent_folding::find_fold_range_at_byte(
			&state.buffer,
			target_byte,
			tab_size,
			max_scan_bytes,
		...
		let header_line = state.buffer.get_line(state.buffer.get_line_number(header_byte))?;
		...
		Some(Self { column, first_line_idx, last_line_idx })
	}
...
fn view_line_indent(line: &ViewLine, tab_size: usize) -> Option<(usize, bool)> {
	if line.source_start_byte.is_none()
		|| matches!(line.line_start, LineStart::AfterBreak | LineStart::AfterInjectedNewline)
	{
		return None;
	}
	...
	fn columns_for(
		...
		match view_line_indent(line, tab_size) {
			...
			None if matches!(line.line_start, LineStart::AfterBreak) => {
				let pad_width = line.char_source_bytes.iter().take_while(|b| b.is_none()).count();
...
fn next_content_indent(
	...
	view_lines.get(start_idx..).into_iter().flatten().find_map(|line| {
		match view_line_indent(line, tab_size) {
...
fn append_blank_line_guides(
	...
	let Some(last_col) =
		guide_columns.iter().copied().filter(|&c| c >= content_visual_end && c >= left_col).max()
	else {
		return;
	};
	...
	for col in start..=end_col {
		text.push(if guide_columns.contains(&col) { glyph } else { ' ' });
	}
...
pub(crate) fn render_view_lines(input: LineRenderInput<'_>) -> LineRenderOutput {
	...
	let guide_glyph_char = {
		...
		if crate::primitives::display_width::char_width(c) != 1 { '▏' } else { c }
	};
	...
	let mut overlay_sweep =
		OverlayActiveSet::new(&decorations.viewport_overlays, &decorations.overlay_position_index);
	...
	loop {
		...
		let line_start_byte: Option<usize> = if !is_continuation {
			line_char_source_bytes.iter().find_map(|opt| *opt).or_else(|| {
		...
		if !is_continuation {
			is_on_cursor_line =
				line_start_byte.is_some_and(|b| b >= cursor_line_start_byte && b < cursor_line_end_byte);
		}
		...
		let gutter_num = if let Some(byte) = line_start_byte {
			let n = if byte_offset_mode { byte } else { state.buffer.get_line_number(byte) };
		...
		if let Some(scanner) = guide_column_scanner.as_mut() {
			...
			let next_indent = match view_line_indent(current_view_line, tab_size) {
				Some((_, false)) => next_content_indent(view_lines, current_view_line_idx + 1, tab_size),
				_ => None,
			};
			scanner.columns_for(current_view_line, tab_size, next_indent, &mut guide_columns_buf);
		} else {
		...
		if state.buffer_settings.virtual_space.block_beyond_eol()
			...
			&& !matches!(line_start_type, LineStart::AfterBreak | LineStart::AfterInjectedNewline)
		{
			...
			if let Some((sel_start, sel_end)) = overlap {
				...
				let sel_len = (sel_end + 1 - sel_start).min(content_cols.saturating_sub(rendered_cols));
				if sel_len > 0 {
					push_span_with_map(
						&mut line_spans,
						&mut line_view_map,
		...
		if !line_has_newline {
			...
			let seg_y =
				(line_spans.is_empty() || !line_wrap || line_content.is_empty()).then_some(current_row);
		...
		if cursor_line_active && remaining_cols > 0 {
			line_spans
				.push(Span::styled(" ".repeat(remaining_cols), Style::default().bg(theme.current_line_bg)));
		}

		// For virtual rows (no source bytes), inherit from previous row
		let prev_line_end_byte =
			view_line_mappings.last().map(|prev: &ViewLineMapping| prev.line_end_byte).unwrap_or(0);
		...
		let end_x = if line_was_empty { gutter_width as u16 } else { last_visible_x.saturating_add(1) };
		...
		if !is_iterator_trailing_empty {
			last_line_end =
				Some(LastLineEnd { pos: (end_x, current_row), terminated_with_newline: line_has_newline });
		}
	...
	LineRenderOutput {
		...
		cursor: cursor.found.then_some((cursor.x.min(render_area.width.saturating_sub(1)), cursor.y)),
		last_line_end,
...
fn cursor_indicator_style(theme: &Theme, is_active: bool) -> Style {
	if is_active {
		Style::default().fg(theme.editor_fg).bg(theme.editor_bg).add_modifier(Modifier::REVERSED)
	} else {
		Style::default().fg(theme.editor_fg).bg(theme.inactive_cursor)
	}
}
...
fn place_line_end_cursor(
	...
	let after_last_char_buf_pos = line_char_source_bytes.get(after_last_char_idx).copied().flatten();
	...
	let virtual_pad = input.selection.virtual_cols_at.get(&end_byte).copied().unwrap_or(0);
	...
	let should_add_indicator =
		if input.is_active { input.software_cursor_only || !is_primary_at_end } else { true };
...
fn append_inline_diagnostic(
	...
	if padding > 0 {
		let pad_style =
			if cursor_line_active { Style::default().bg(current_line_bg) } else { Style::default() };
		push_span_with_map(line_spans, line_view_map, " ".repeat(padding), pad_style, None);
	...
	push_span_with_map(line_spans, line_view_map, display, effective_diag_style, None);
...
fn build_view_line_mapping(
	...
	let line_end_byte = if view_line.ends_with_newline {
		...
		view_line.char_source_bytes.iter().rev().find_map(|m| *m).unwrap_or(prev_line_end_byte)
	} else if let Some((char_idx, &Some(last_byte_start))) =
		view_line.char_source_bytes.iter().enumerate().rev().find(|(_, m)| m.is_some())
	{
