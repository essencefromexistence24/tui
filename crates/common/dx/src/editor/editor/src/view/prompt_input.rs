			...
			KeyCode::Up => {
				...
				if !self.suggestions.is_empty() {
					...
					if let Some(selected) = self.selected_suggestion {
						...
						if matches!(self.prompt_type, crate::view::prompt::PromptType::SelectTheme { .. }) {
						...
						if matches!(self.prompt_type, crate::view::prompt::PromptType::Plugin { .. }) {
							ctx.defer(DeferredAction::PromptSelectionChanged { selected_index: new_selected });
						}
					}
				} else {
				...
				if let Some(selected) = self.selected_suggestion {
					if let Some(suggestion) = self.suggestions.get(selected) {
						if !suggestion.disabled {
							...
							if matches!(self.prompt_type, crate::view::prompt::PromptType::QuickOpen) {
								let prefix =
									self.input.chars().next().filter(|c| *c == '>' || *c == '#' || *c == ':');
	...
	fn handle_ctrl_key(&mut self, c: char, ctx: &mut InputContext) -> InputResult {
		match c {
			...
			'c' => {
				...
				ctx.defer(DeferredAction::ExecuteAction(crate::input::keybindings::Action::PromptCopy));
				InputResult::Consumed
			}
			'x' => {
				// Cut - defer to Editor for clipboard access
				ctx.defer(DeferredAction::ExecuteAction(crate::input::keybindings::Action::PromptCut));
				InputResult::Consumed
			}
			'v' => {
				// Paste - defer to Editor for clipboard access
				ctx.defer(DeferredAction::ExecuteAction(crate::input::keybindings::Action::PromptPaste));
				InputResult::Consumed
			}
			'k' => {
...
mod tests {
	...
	fn test_prompt_ctrl_p_dispatch_returns_ignored() {
		...
		assert_eq!(result, InputResult::Ignored, "dispatch_input should return Ignored for Ctrl+P");
	}
}
