...
impl SuggestionsRenderer {
	...
	pub fn render_with_hover(
		...
		let block = Block::default()
			.borders(if with_border { Borders::ALL } else { Borders::NONE })
		...
		for (idx, suggestion) in visible_suggestions.iter().enumerate() {
			...
			lines.push(render_row(suggestion, is_selected, is_hovered, &layout, available_width, theme));
		}
...
impl ColumnLayout {
	fn compute(visible_suggestions: &[Suggestion], available_width: usize) -> Self {
		...
		let actual_max_name_width =
			visible_suggestions.iter().map(|s| str_width(&s.text)).max().unwrap_or(0);
	...
	fn fixed_columns_width(&self) -> usize {
		self.left_margin
			...
			+ self.column_spacing
			+ if self.has_keybinding { self.keybinding_column_width + self.column_spacing } else { 0 }
	}

	/// Width the source column reserves at the right edge of the row.
	fn source_reserved(&self) -> usize {
		if self.has_source { self.column_spacing + self.source_column_width } else { 0 }
	}
}
...
fn render_row(
	...
	push_description_column(&mut spans, suggestion, layout, available_width, base_style, theme);

	if layout.has_source {
		push_source_column(&mut spans, suggestion, layout, base_style, theme, is_selected, is_hovered);
	}
	...
	if current_width < available_width {
		spans.push(Span::styled(" ".repeat(available_width.saturating_sub(current_width)), base_style));
	}
...
fn push_keybinding_column(
	...
	let style = keybinding_style(theme, base_style, suggestion.disabled, is_selected, is_hovered);
...
fn push_description_column(
	...
	let desc_width = available_width.saturating_sub(fixed).saturating_sub(source_reserved);
...
fn push_source_column(
	...
	let style = source_style(theme, base_style, suggestion.disabled, is_selected, is_hovered);
...
fn row_base_style(
	...
	if disabled {
		...
		let bg = if is_selected { theme.suggestion_selected_bg } else { theme.suggestion_bg };
		Style::default().fg(Color::DarkGray).bg(bg).add_modifier(Modifier::DIM)
	} else if is_selected {
		Style::default().fg(theme.popup_selection_fg).bg(theme.suggestion_selected_bg)
	} else if is_hovered {
...
fn keybinding_style(
	...
	if disabled {
		...
		Style::default().fg(theme.help_key_fg).bg(theme.suggestion_selected_bg)
	} else if is_hovered {
		Style::default().fg(theme.help_key_fg).bg(theme.menu_hover_bg)
	} else {
...
fn source_style(
	...
	if disabled {
		...
		Style::default().fg(theme.line_number_fg).bg(theme.menu_hover_bg).add_modifier(Modifier::DIM)
	} else {
		Style::default().fg(theme.line_number_fg).bg(theme.suggestion_bg).add_modifier(Modifier::DIM)
	}
}
...
fn overlay_spec_color(
	spec: &dx_core::api::OverlayColorSpec,
	theme: &crate::view::theme::Theme,
) -> Option<Color> {
...
fn styled_span_style(
	base: Style,
	style: Option<&dx_core::api::OverlayOptions>,
	theme: &crate::view::theme::Theme,
) -> Style {
	let mut s = base;
...
mod tests {
	...
	fn test_suggestion_description_truncation_with_multibyte_utf8() {
		...
		let mut prompt = Prompt::new("Test: ".to_string(), crate::view::prompt::PromptType::QuickOpen);
	...
	fn test_truncation_preserves_valid_utf8() {
		...
		for description in test_cases {
			...
			let mut prompt =
				Prompt::new("Test: ".to_string(), crate::view::prompt::PromptType::QuickOpen);
			prompt.suggestions = vec![suggestion];
			...
			for width in 20..100 {
				...
				assert!(result.is_ok(), "Panic at width {} with description: {}", width, description);
			}
		}
	}
}
