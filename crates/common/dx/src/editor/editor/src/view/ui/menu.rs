...
use crate::config::{Menu, MenuItem, MenuItemExt, generate_dynamic_items};
...
pub use dx_core::menu::MenuContext;
...
impl MenuState {
	...
	pub fn open_submenu(&mut self, menus: &[Menu]) -> bool {
		...
		if let Some(item) = items.get(highlighted) {
			match item {
				MenuItem::Submenu { items: submenu_items, .. } if !submenu_items.is_empty() => {
	...
	pub fn get_current_items_cloned(&self, menu: &Menu) -> Option<Vec<MenuItem>> {
		...
		let mut items: Vec<MenuItem> =
			menu.items.iter().map(|i| i.expand_dynamic(&self.themes_dir)).collect();
...
impl MenuRenderer {
	...
	pub fn render(
		...
		if let Some(r) = rec.as_deref_mut() {
			r.run(area.x, area.y, area.width, Some("ui.menu_fg"), Some("ui.menu_bg"), "Menu Bar");
		}
		...
		for (idx, menu) in all_menus.iter().enumerate() {
			...
			layout.menu_areas.push((idx, Rect::new(current_x, area.y, label_width, 1)));
			...
			if let Some(r) = rec.as_deref_mut() {
				...
				r.run(current_x, area.y, label_width, Some(fg), Some(bg), "Menu Bar");
			}

			// Check for mnemonic character (Alt+letter keybinding)
			let mnemonic =
				if mnemonics_enabled { keybindings.find_menu_mnemonic(&menu.label) } else { None };
			...
			if let Some(mnemonic_char) = mnemonic {
				...
				for c in menu.label.chars() {
					if !found && c.to_ascii_lowercase() == mnemonic_char {
						...
						spans.push(Span::styled(c.to_string(), base_style.add_modifier(Modifier::UNDERLINED)));
						found = true;
					} else {
	...
	fn render_dropdown_chain(
		...
		for depth in 0..=menu_state.submenu_path.len() {
			...
			let highlighted_item =
				if is_deepest { menu_state.highlighted_item } else { Some(menu_state.submenu_path[depth]) };
	...
	fn fit_dropdown_area(
		...
		Rect { x: adjusted_x, y, width, height }
	}
	...
	fn record_dropdown_item_run(
		...
		let (fg, bg) = match item {
			...
			_ if is_highlighted || has_open_submenu => ("ui.menu_highlight_fg", "ui.menu_highlight_bg"),
			_ => ("ui.menu_dropdown_fg", "ui.menu_dropdown_bg"),
		};
		rec.run(item_area.x, item_area.y, item_area.width, Some(fg), Some(bg), "Menu Dropdown");
	}
		...
		match item {
			MenuItem::Action { label, action, checkbox, .. } => {
				let style = if !enabled {
					Style::default().fg(theme.menu_disabled_fg).bg(theme.menu_disabled_bg)
				} else if is_highlighted {
					Style::default().fg(theme.menu_highlight_fg).bg(theme.menu_highlight_bg)
				} else if is_hovered {
					Style::default().fg(theme.menu_hover_fg).bg(theme.menu_hover_bg)
				} else {
					Style::default().fg(theme.menu_dropdown_fg).bg(theme.menu_dropdown_bg)
				};

				let keybinding = keybindings
					.find_keybinding_for_action(action, crate::input::keybindings::KeyContext::Normal)
					.unwrap_or_default();

				let checkbox_icon = if checkbox.is_some() {
					if is_checkbox_checked(checkbox, context) { "☑ " } else { "☐ " }
				} else {
					""
				};
				...
				let text = if keybinding.is_empty() {
					...
					let padding_needed = content_width
						.saturating_sub(checkbox_width + label_display_width + keybinding_display_width + 2);
					format!(" {}{}{} {}", checkbox_icon, label, " ".repeat(padding_needed), keybinding)
				};
			...
			MenuItem::Separator { .. } => {
				...
				Line::from(vec![Span::styled(
					format!(" {separator}"),
					Style::default().fg(theme.menu_separator_fg).bg(theme.menu_dropdown_bg),
				)])
			}
			...
			MenuItem::Label { info } => {
				...
				let style = Style::default().fg(theme.menu_disabled_fg).bg(theme.menu_dropdown_bg);
				let info_display_width = str_width(info);
				let padding_needed = content_width.saturating_sub(info_display_width);
				Line::from(vec![Span::styled(format!(" {}{}", info, " ".repeat(padding_needed)), style)])
			}
		}
	}
}
...
mod tests {
	...
	fn test_menu_item_when_requires_selection() {
		...
		assert!(state.get_highlighted_action(std::slice::from_ref(&select_menu)).is_none());
	...
	fn test_menu_layout_hit_test() {
		...
		assert_eq!(layout.hit_test(25, 3), Some(MenuHit::SubmenuItem { depth: 1, index: 0 }));
	...
	fn test_dropdown_position_skips_hidden_menus() {
		...
		assert_eq!(x_help_hidden, 7, "Help dropdown should be at x=7 when Explorer is hidden");
		...
		assert_eq!(x_help_visible, 18, "Help dropdown should be at x=18 when Explorer is visible");
	}
	...
	fn test_dropdown_position_with_multiple_hidden_menus() {
		let menus = vec![
			...
			Menu { id: None, label: "D".to_string(), items: vec![], when: None },
		];
