...
use super::markdown::{StyledLine, parse_markdown, wrap_styled_lines, wrap_text_lines};

pub mod input;
use super::ui::scrollbar::{ScrollbarColors, ScrollbarState, render_scrollbar};
...
pub enum PopupResolver {
	...
	SettingsSaveError { layer: crate::config_io::ConfigLayer },
}
...
pub enum PopupContent {
	...
	List { items: Vec<PopupListItem>, selected: usize },
...
impl PopupListItem {
	pub fn new(text: String) -> Self {
		Self { text, detail: None, icon: None, data: None, disabled: false }
	}
...
impl Popup {
	...
	pub fn render_title(&self) -> Option<String> {
		let hint_label = if !self.focused {
			let hint = self.focus_key_hint.clone().unwrap_or_else(|| "Alt+T".to_string());
	...
	fn wrapped_item_count(&self) -> usize {
		...
		let wrap_width =
			(self.width as usize).saturating_sub(border_width).saturating_sub(scrollbar_width);
		...
		match &self.content {
			...
			PopupContent::Markdown(styled_lines) => wrap_styled_lines(styled_lines, wrap_width).len(),
	...
	pub fn start_selection(&mut self, line: usize, col: usize) {
		self.text_selection = Some(PopupTextSelection { start: (line, col), end: (line, col) });
	}
	...
	pub fn has_selection(&self) -> bool {
		if let Some(sel) = &self.text_selection { sel.start != sel.end } else { false }
	}
	...
	fn content_wrap_width(&self) -> usize {
		...
		if needs_scrollbar { conservative_width } else { inner_width as usize }
	}
	...
	fn get_text_lines(&self) -> Vec<String> {
		...
		match &self.content {
			...
			PopupContent::Markdown(styled_lines) => {
				if wrap_width > 0 {
					wrap_styled_lines(styled_lines, wrap_width).iter().map(|sl| sl.plain_text()).collect()
				} else {
	...
	pub fn link_at_position(&self, relative_col: usize, relative_row: usize) -> Option<String> {
		...
		let content_width =
			self.width.saturating_sub(border_width).saturating_sub(scrollbar_reserved) as usize;
	...
	fn content_height_for_width(&self, popup_width: u16) -> u16 {
		...
		let content_width =
			popup_width.saturating_sub(border_width).saturating_sub(scrollbar_reserved) as usize;
	...
	pub fn calculate_area(&self, terminal_area: Rect, cursor_pos: Option<(u16, u16)>) -> Rect {
		match self.position {
			PopupPosition::AtCursor | PopupPosition::BelowCursor | PopupPosition::AboveCursor => {
				...
				let x = if cursor_x + width > right { right.saturating_sub(width) } else { cursor_x }
					.max(terminal_area.x);
			...
			PopupPosition::Fixed { x, y } => {
				...
				let right = terminal_area.x + terminal_area.width;
				let bottom = terminal_area.y + terminal_area.height;
				let x = if x + width > right { right.saturating_sub(width) } else { x };
				let y = if y + height > bottom { bottom.saturating_sub(height) } else { y };
				Rect { x, y, width, height }
			}
			...
			PopupPosition::CenteredOverlay { width_pct, height_pct } => {
			...
			PopupPosition::BottomRight => {
				...
				let y = terminal_area.y + terminal_area.height.saturating_sub(height).saturating_sub(2);
				Rect { x, y, width, height }
			}
	...
	pub fn render_with_hover(
		...
		if let Some(desc) = &self.description {
			...
			for (i, line) in wrapped_desc.iter().enumerate() {
				...
				let line_area =
					Rect { x: inner_area.x, y: inner_area.y + i as u16, width: inner_area.width, height: 1 };
					...
					.map(|(line_idx, styled_line)| {
						...
						let spans: Vec<Span> = styled_line
							...
							.flat_map(|s| {
								...
								let span_width = unicode_width::UnicodeWidthStr::width(s.text.as_str());
			...
			PopupContent::List { items, selected } => {
				let list_items: Vec<ListItem> = items
					...
					.map(|(idx, item)| {
						...
						if item.disabled {
							text_style = text_style.fg(theme.help_separator_fg).add_modifier(Modifier::DIM);
						}
						...
						if is_selected {
							if let Some(ref hint) = self.accept_key_hint {
								...
								if used_width + hint_len + 1 < available {
									...
									spans.push(Span::styled(hint_text, Style::default().fg(theme.help_separator_fg)));
								}
							}
						}
						...
						let row_style = if is_selected {
							...
							Style::default().bg(theme.menu_hover_bg).fg(theme.menu_hover_fg)
						} else {
...
impl PopupManager {
	...
	pub fn is_completion_popup(&self) -> bool {
		self.top().map(|p| p.kind == PopupKind::Completion).unwrap_or(false)
	}

	/// Check if the topmost popup is a hover popup
	pub fn is_hover_popup(&self) -> bool {
		self.top().map(|p| p.kind == PopupKind::Hover).unwrap_or(false)
	}

	/// Check if the topmost popup is an action popup
	pub fn is_action_popup(&self) -> bool {
		self.top().map(|p| p.kind == PopupKind::Action).unwrap_or(false)
	}
...
mod tests {
	...
	fn test_popup_area_calculation() {
		let theme = crate::view::theme::Theme::load_builtin(theme::THEME_DARK).unwrap();
		let terminal_area = Rect { x: 0, y: 0, width: 100, height: 50 };

		let popup = Popup::text(vec!["test".to_string()], &theme).with_width(30).with_max_height(10);
	...
	fn test_popup_area_respects_left_dock_offset() {
		...
		let chrome = Rect { x: 34, y: 0, width: 120 - 34, height: 40 };

		let popup = Popup::text(vec!["test".to_string()], &theme).with_width(30).with_max_height(10);

		// Centered: centred within the chrome area, never under the dock.
		let centered =
			popup.clone().with_position(PopupPosition::Centered).calculate_area(chrome, None);
		...
		let overlay = popup
			.clone()
			.with_position(PopupPosition::CenteredOverlay { width_pct: 50, height_pct: 50 })
		...
		let br = popup.clone().with_position(PopupPosition::BottomRight).calculate_area(chrome, None);
	...
	fn hyperlink_overlay_chunks_pairs() {
		...
		assert!(first.contains("Pl"), "first chunk should contain 'Pl', got {first:?}");
		assert!(second.contains("ay"), "second chunk should contain 'ay', got {second:?}");
	}
	...
	fn test_popup_text_selection() {
		...
		let mut popup = Popup::text(
			vec!["Line 0: Hello".to_string(), "Line 1: World".to_string(), "Line 2: Test".to_string()],
			&theme,
		);
	...
	fn test_popup_text_selection_contains() {
		let sel = PopupTextSelection { start: (1, 5), end: (2, 10) };
	...
	fn test_popup_text_selection_normalized() {
		...
		let sel_backward = PopupTextSelection { start: (2, 10), end: (1, 5) };
