...
/// authority Dx currently supports (SSH today; containers;
	...
	pub fn label(&self) -> String {
		match self {
			...
			Self::Connected { label } => {
				label.as_deref().filter(|s| !s.is_empty()).unwrap_or("Connected").to_string()
			}
...
pub fn truncate_path(path: &Path, max_len: usize) -> TruncatedPath {
	...
	if components.is_empty() {
		return TruncatedPath { prefix: sep.to_string(), truncated: false, suffix: String::new(), sep };
	}
	...
	let prefix_count =
		if !leading_sep && is_drive(components[0]) { 2 } else { 1 }.min(components.len());
	let sep_str = sep.to_string();
	let prefix = {
		let joined = components[..prefix_count].join(&sep_str);
		if leading_sep { format!("{}{}", sep, joined) } else { joined }
	};
	...
	if available_for_suffix < 5 || components.len() <= prefix_count {
		...
		return TruncatedPath { prefix: String::new(), truncated: false, suffix: truncated_path, sep };
	}
	...
	TruncatedPath { prefix, truncated: true, suffix, sep }
}
...
fn truncate_to_width(s: &str, max_width: usize) -> String {
	...
	if truncate_at == 0 {
		return if max_width >= 3 { "...".to_string() } else { s.chars().take(max_width).collect() };
	}
	let mut w = 0;
...
fn format_cursor_position(line: usize, col: usize, line_count: usize) -> String {
	...
	if text.len() < min_width { format!("{text:<min_width$}") } else { text }
}
...
impl StatusBarRenderer {
	...
	pub fn render_prompt(
		...
		if let Some((sel_start, sel_end)) = prompt.selection_range() {
			...
			if sel_start < sel_end {
				...
				let selection_style =
					Style::default().fg(theme.prompt_selection_fg).bg(theme.prompt_selection_bg);
	...
	pub fn render_file_open_prompt(
		...
		let base_style = Style::default().fg(theme.prompt_fg).bg(theme.prompt_bg);
		let dir_style = Style::default().fg(theme.help_separator_fg).bg(theme.prompt_bg);
		// Style for the [...] ellipsis - use a more visible color
		let ellipsis_style = Style::default().fg(theme.menu_highlight_fg).bg(theme.prompt_bg);
		...
		let truncated = if total_len > threshold {
			...
			let available_for_path = threshold.saturating_sub(prefix_len).saturating_sub(input_len);
		...
		if truncated.truncated {
			...
			spans.push(Span::styled(format!("{}[...]", truncated.sep), ellipsis_style));
	...
	fn render_element(
		...
		match element {
			StatusBarElement::Filename => {
				let modified = if ctx.state.buffer.is_modified() { " [+]" } else { "" };
				let read_only_indicator = if ctx.read_only { " [RO]" } else { "" };
				let remote_disconnected =
					ctx.remote_connection.map(|conn| conn.contains("(Disconnected)")).unwrap_or(false);
				...
				let session_prefix =
					ctx.session_name.map(|name| format!("[{}] ", name)).unwrap_or_default();
				let display_name = ctx.display_name;
				let text =
					format!("{session_prefix}{remote_prefix}{display_name}{modified}{read_only_indicator}");
				let kind =
					if remote_disconnected { ElementKind::RemoteDisconnected } else { ElementKind::Normal };
				Some(RenderedElement { text, kind, token_key: None })
			}
			...
			StatusBarElement::Cursor => {
				...
				Some(RenderedElement { text, kind: ElementKind::Normal, token_key: None })
			}
			StatusBarElement::CursorCompact => {
				if !ctx.state.show_cursors {
					return None;
			...
			StatusBarElement::Diagnostics => {
				...
				Some(RenderedElement { text: parts.join(" "), kind: ElementKind::Normal, token_key: None })
			}
			StatusBarElement::CursorCount => {
			...
			StatusBarElement::Chord => {
				...
				let chord_str = ctx
					.chord_state
					.iter()
					.map(|(code, modifiers)| crate::input::keybindings::format_keybinding(code, modifiers))
					.collect::<Vec<_>>()
					.join(" ");
				Some(RenderedElement {
			...
			StatusBarElement::Language => {
				...
				Some(RenderedElement { text, kind: ElementKind::Language, token_key: None })
			}
			...
			StatusBarElement::Clock => {
				...
				Some(RenderedElement { text, kind: ElementKind::Clock, token_key: None })
			}
			StatusBarElement::RemoteIndicator => {
				...
				let (text, state) = if let Some(over) = ctx.remote_state_override {
					...
					("Disconnected".to_string(), RemoteIndicatorState::Disconnected)
				} else {
				...
				Some(RenderedElement { text, kind: ElementKind::RemoteIndicator(state), token_key: None })
			}
			StatusBarElement::WorkspaceTrust => {
				...
				.to_string();
				Some(RenderedElement { text, kind: ElementKind::WorkspaceTrust(level), token_key: None })
			}
			StatusBarElement::CustomToken(key) => {
				ctx.dynamic_status_bar_elements.get(key).map(|value| RenderedElement {
					text: value.clone(),
	...
	fn element_style(
		...
		match kind {
			ElementKind::Normal | ElementKind::Messages | ElementKind::Clock => {
				Style::default().fg(theme.status_bar_fg).bg(theme.status_bar_bg)
			}
			ElementKind::RemoteDisconnected => {
				Style::default().fg(theme.status_error_indicator_fg).bg(theme.status_error_indicator_bg)
			}
			...
			ElementKind::ReadOnly => {
				let mut style = Style::default().fg(theme.status_bar_fg).bg(theme.status_bar_bg);
				if is_hovering {
					style = style.add_modifier(Modifier::UNDERLINED);
				}
				style
			...
			ElementKind::Lsp => {
				...
				let (fg, bg) = match lsp_state {
					LspIndicatorState::Error => (theme.diagnostic_error_fg, theme.diagnostic_error_bg),
					LspIndicatorState::Off => {
						(theme.status_lsp_actionable_fg, theme.status_lsp_actionable_bg)
					}
			...
			ElementKind::Update => {
				Style::default().fg(theme.menu_highlight_fg).bg(theme.menu_dropdown_bg)
			}
			...
			ElementKind::Palette => {
				Style::default().fg(theme.status_palette_fg).bg(theme.status_palette_bg)
			}
			ElementKind::Custom => Style::default().fg(theme.status_bar_fg).bg(theme.status_bar_bg),
			ElementKind::RemoteIndicator(state) => {
				let (fg, bg) = match state {
					...
					RemoteIndicatorState::FailedAttach | RemoteIndicatorState::Disconnected => {
						(theme.status_error_indicator_fg, theme.status_error_indicator_bg)
					}
	...
	fn element_keys(kind: ElementKind, lsp_state: LspIndicatorState) -> (&'static str, &'static str) {
		match kind {
			ElementKind::Normal
			...
			ElementKind::RemoteDisconnected => {
				("ui.status_error_indicator_fg", "ui.status_error_indicator_bg")
			}
			ElementKind::Lsp => match lsp_state {
				LspIndicatorState::Error => ("diagnostic.error_fg", "diagnostic.error_bg"),
				LspIndicatorState::Off => ("ui.status_lsp_actionable_fg", "ui.status_lsp_actionable_bg"),
			...
			ElementKind::WarningBadge => {
				("ui.status_warning_indicator_fg", "ui.status_warning_indicator_bg")
			}
	...
	fn update_layout_for_element(
		...
		if kind == ElementKind::Custom {
			if let Some(key) = token_key {
				layout.plugin_token_areas.insert(key.to_string(), (row, start_col, end_col));
			}
		}
	}
	...
	fn element_spans(
		...
		let is_hovering = Self::clickable_for_kind(rendered.kind).is_some_and(|c| Some(c) == hovered);
		let base_style = Style::default().fg(theme.status_bar_fg).bg(theme.status_bar_bg);
		...
		let style = Self::element_style(rendered.kind, theme, is_hovering, warning_level, lsp_state);
		...
		if rendered.kind == ElementKind::Clock {
			...
			spans.push(Span::styled(":".to_string(), style.add_modifier(Modifier::SLOW_BLINK)));
	...
	fn render_side(
		...
		rendered
			...
			.map(|r| {
				...
				let (spans, width) = Self::element_spans(&r, theme, hovered, warning_level, lsp_state);
	...
	fn render_status(
		...
		let base_style = Style::default().fg(ctx.theme.status_bar_fg).bg(ctx.theme.status_bar_bg);
		...
		let separator_style =
			Style::default().fg(ctx.theme.status_separator_fg).bg(ctx.theme.status_separator_bg);
		...
		for (idx, (item_spans, width, kind, token_key)) in left_items.into_iter().enumerate() {
			...
			if width <= remaining {
				...
				let seg_text =
					(!draw).then(|| item_spans.iter().map(|s| s.content.as_ref()).collect::<String>());
				spans.extend(item_spans);
				used_left += width;

				if let Some(r) = rec.as_deref_mut() {
					let (fg, bg) = Self::element_keys(kind, lsp_state);
					r.run(area.x + start_col as u16, area.y, width as u16, Some(fg), Some(bg), "Status Bar");
				}
		...
		if narrow {
			if used_left < available_width {
				spans.push(Span::styled(" ".repeat(available_width - used_left), base_style));
			}
			if draw {
		...
		for (idx, (item_spans, width, kind, token_key)) in right_items.into_iter().enumerate() {
			...
			if let Some(r) = rec.as_deref_mut() {
				let (fg, bg) = Self::element_keys(kind, lsp_state);
				r.run(current_col, area.y, width as u16, Some(fg), Some(bg), "Status Bar");
			}
			Self::update_layout_for_element(
				&mut layout,
				kind,
	...
	pub fn render_search_options(
		...
		let mut layout = SearchOptionsLayout { row: area.y, ..Default::default() };

		// Use menu dropdown background (dark gray) for the options bar
		let base_style = Style::default().fg(theme.menu_dropdown_fg).bg(theme.menu_dropdown_bg);

		// Style for hovered options - use menu hover colors
		let hover_style = Style::default().fg(theme.menu_hover_fg).bg(theme.menu_hover_bg);
		...
		let get_shortcut = |action: &crate::input::keybindings::Action| -> Option<String> {
			keybindings
				.get_keybinding_for_action(action, crate::input::keybindings::KeyContext::SearchPrompt)
				.or_else(|| {
					keybindings
		...
		let active_style = Style::default().fg(theme.menu_active_fg).bg(theme.menu_active_bg);

		// Style for keyboard shortcuts - use theme color for consistency
		let shortcut_style = Style::default().fg(theme.help_separator_fg).bg(theme.menu_dropdown_bg);

		// Hovered shortcut style
		let hover_shortcut_style = Style::default().fg(theme.menu_hover_fg).bg(theme.menu_hover_bg);
		...
		let case_shortcut_text =
			case_shortcut.as_ref().map(|s| format!(" ({})", s)).unwrap_or_default();
		let case_full_width = str_width(&case_label) + str_width(&case_shortcut_text);

		spans.push(Span::styled(case_label, get_checkbox_style(case_hovered, case_sensitive)));
		if !case_shortcut_text.is_empty() {
			spans.push(Span::styled(
				case_shortcut_text,
				if case_hovered { hover_shortcut_style } else { shortcut_style },
			));
		}
		...
		let word_shortcut_text =
			word_shortcut.as_ref().map(|s| format!(" ({})", s)).unwrap_or_default();
		let word_full_width = str_width(&word_label) + str_width(&word_shortcut_text);

		spans.push(Span::styled(word_label, get_checkbox_style(word_hovered, whole_word)));
		if !word_shortcut_text.is_empty() {
			spans.push(Span::styled(
				word_shortcut_text,
				if word_hovered { hover_shortcut_style } else { shortcut_style },
			));
		}
		...
		let regex_shortcut_text =
			regex_shortcut.as_ref().map(|s| format!(" ({})", s)).unwrap_or_default();
		let regex_full_width = str_width(&regex_label) + str_width(&regex_shortcut_text);

		spans.push(Span::styled(regex_label, get_checkbox_style(regex_hovered, use_regex)));
		if !regex_shortcut_text.is_empty() {
			spans.push(Span::styled(
				regex_shortcut_text,
				if regex_hovered { hover_shortcut_style } else { shortcut_style },
			));
		}
		...
		if let Some(confirm_value) = confirm_each {
			...
			let confirm_shortcut_text =
				confirm_shortcut.as_ref().map(|s| format!(" ({})", s)).unwrap_or_default();
			let confirm_full_width = str_width(&confirm_label) + str_width(&confirm_shortcut_text);

			spans.push(Span::styled(confirm_label, get_checkbox_style(confirm_hovered, confirm_value)));
			if !confirm_shortcut_text.is_empty() {
				spans.push(Span::styled(
					confirm_shortcut_text,
					if confirm_hovered { hover_shortcut_style } else { shortcut_style },
				));
			}
		...
		if current_width < available_width {
			spans
				.push(Span::styled(" ".repeat(available_width.saturating_sub(current_width)), base_style));
		}
...
mod tests {
	...
	fn test_truncate_path_long_path() {
		...
		assert!(result.suffix.contains("project_root"), "Suffix should contain project_root");
	}
	...
	fn test_truncate_path_windows_backslashes() {
		...
		assert!(shown.starts_with(r"C:\Users"), "keeps drive + first dir: {shown}");
		assert!(shown.contains(r"\[...]\"), "uses a backslash ellipsis: {shown}");
	...
	fn test_remote_indicator_override_deserializes_kind_tags() {
		...
		let cases: &[(&str, RemoteIndicatorOverride)] = &[
			...
			(
				...
				RemoteIndicatorOverride::Connecting { label: Some("Building".into()) },
			),
			(r#"{"kind":"connecting"}"#, RemoteIndicatorOverride::Connecting { label: None }),
			(
				r#"{"kind":"connected","label":"Container:abc"}"#,
				RemoteIndicatorOverride::Connected { label: Some("Container:abc".into()) },
			),
			(
				r#"{"kind":"failed_attach","error":"exit 1"}"#,
				RemoteIndicatorOverride::FailedAttach { error: Some("exit 1".into()) },
			),
			(
				r#"{"kind":"disconnected","label":"Container:abc"}"#,
				RemoteIndicatorOverride::Disconnected { label: Some("Container:abc".into()) },
			),
		];
		for (json, expected) in cases {
			let parsed: RemoteIndicatorOverride =
	...
	fn test_remote_indicator_override_labels() {
		...
		let connecting_labeled = RemoteIndicatorOverride::Connecting { label: Some("Building".into()) };
		assert!(
		...
		let failed_detail = RemoteIndicatorOverride::FailedAttach { error: Some("exit 1".into()) };
		assert!(
	...
	fn test_remote_indicator_override_state_projection() {
		assert_eq!(RemoteIndicatorOverride::Local.state(), RemoteIndicatorState::Local);
		assert_eq!(
	...
	fn test_cursor_position_preserves_natural_number_text() {
		...
		assert!(text.ends_with(' '), "expected trailing padding, got {text:?}");
	}
	...
	fn test_cursor_position_compact_preserves_natural_text() {
		...
		assert!(text.starts_with("1:1"), "expected text to start with natural numbers, got {text:?}");
	}
	...
	fn test_cursor_column_counts_chars_not_bytes() {
		...
		assert_eq!(col, 5, "cursor at 'r' should be column 5, not byte offset 6");
	...
	fn test_cursor_column_counts_grapheme_clusters() {
		...
		let col = cursor_column(&mut buf, line_start + 3);
		assert_eq!(col, 1, "accented 'e' is one grapheme; 'x' should be column 1, not 2");
	}
