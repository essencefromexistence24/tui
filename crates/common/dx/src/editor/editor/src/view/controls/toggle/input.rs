...
impl ToggleState {
	...
	pub fn handle_mouse(&mut self, event: MouseEvent, layout: &ToggleLayout) -> Option<ToggleEvent> {
		if !self.is_enabled() {
			return None;
		}
...
mod tests {
	...
	fn make_layout() -> ToggleLayout {
		ToggleLayout { checkbox_area: Rect::new(8, 0, 3, 1), full_area: Rect::new(0, 0, 11, 1) }
	}
