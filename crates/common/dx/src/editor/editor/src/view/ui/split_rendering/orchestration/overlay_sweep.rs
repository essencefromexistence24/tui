...
impl<'a> OverlayActiveSet<'a> {
	pub(super) fn new(overlays: &'a [(Overlay, Range<usize>)], position_index: &'a [usize]) -> Self {
		Self {
			overlays,
			position_index,
			active: Vec::new(),
	...
	pub(super) fn enter_row(&mut self, wrap_continuation: bool) {
		...
		if wrap_continuation {
			self.row_touched.extend(self.active.iter().map(|(_, idx, _)| *idx));
		}
	}
	...
	pub(super) fn advance_to(&mut self, bp: usize) {
		...
		while self.next_pos < self.position_index.len() {
			...
			if range.end > bp {
				...
				let pos =
					self.active.iter().position(|(_, _, o)| o.priority > pri).unwrap_or(self.active.len());
		...
		if dirty {
			...
			self.active_refs.extend(self.active.iter().map(|(_, _, o)| *o));
		}
