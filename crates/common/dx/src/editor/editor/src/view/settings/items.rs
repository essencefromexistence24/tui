...
fn value_as_string_array(
	...
	current.and_then(from).or_else(|| default.and_then(from)).unwrap_or_default()
}
...
fn build_dual_list_state(
	...
	let mut all_options: Vec<(String, String)> =
		options.iter().map(|o| (o.value.clone(), o.name.clone())).collect();
	...
	DualListState::new(&schema.name, all_options).with_included(included).with_excluded(excluded)
}
...
impl SettingControl {
	...
	pub fn is_composite(&self) -> bool {
		matches!(self, Self::TextList(_) | Self::DualList(_) | Self::Map(_) | Self::ObjectArray(_))
	}
...
impl SettingItem {
	...
	pub fn layout_box(&self, width: u16, style: &ItemBoxStyle) -> ItemBox {
		ItemBox {
			section_header_rows: if self.is_section_start { style.section_header_rows } else { 0 },
...
pub fn clean_description(name: &str, description: Option<&str>) -> Option<String> {
	...
	let filler_words: HashSet<&str> = [
		...
		"whether", "if", "this", "that", "with", "from", "by", "as", "at", "show", "enable", "disable",
		"set", "use", "allow", "default", "true", "false",
	]
	.into_iter()
	.collect();
	...
	let desc_words: Vec<&str> =
		desc.split(|c: char| !c.is_alphanumeric()).filter(|w| !w.is_empty()).collect();
	...
	fn focus_regions(&self, width: u16) -> Vec<FocusRegion> {
		...
		match &self.control {
			...
			SettingControl::TextList(state) => {
				...
				// Label row
				regions.push(FocusRegion { id: 0, y_offset: label_y, height: 1 });
				// Each item row (id = 1 + row_index)
				for i in 0..state.items.len() {
					regions.push(FocusRegion { id: 1 + i, y_offset: label_y + 1 + i as u16, height: 1 });
				}
				// Add-new row
			...
			SettingControl::Map(state) => {
				...
				// Label row
				regions.push(FocusRegion { id: 0, y_offset: y, height: 1 });
				y += 1;
				...
				for (i, (_, v)) in state.entries.iter().enumerate() {
					...
					regions.push(FocusRegion { id: 1 + i, y_offset: y, height: entry_height });
					y += entry_height;
				}

				// Add-new row
				regions.push(FocusRegion { id: 1 + state.entries.len(), y_offset: y, height: 1 });
				regions
			}
			...
			_ => {
				vec![FocusRegion { id: 0, y_offset: label_y, height: plan.content_rows() }]
			}
		}
	}
}
...
pub fn build_pages(
	...
	let ctx = BuildContext { config_value, layer_sources, target_layer, available_status_bar_tokens };
...
fn build_page(category: &SettingCategory, ctx: &BuildContext) -> SettingsPage {
	let mut items: Vec<SettingItem> =
		category.settings.iter().flat_map(|s| expand_or_build(s, ctx)).collect();
	...
	for (idx, item) in items.iter_mut().enumerate() {
		...
		if is_new_section {
			if let Some(name) = item.section.clone() {
				sections.push(SectionInfo { name, first_item_index: idx });
			}
		}
		prev_section = item.section.as_ref();
	}

	let subpages = category.subcategories.iter().map(|sub| build_page(sub, ctx)).collect();
	...
	if let SettingType::Object { properties } = &schema.setting_type {
		let all_native = !properties.is_empty()
			&& properties.iter().all(|child| {
				!matches!(child.setting_type, SettingType::Object { .. } | SettingType::Complex)
			});
		if all_native {
...
pub fn build_item(schema: &SettingSchema, ctx: &BuildContext) -> SettingItem {
	...
	let control = match &schema.setting_type {
			...
			let current = if current_value.map(|v| v.is_null()).unwrap_or(false) {
				...
				current_value
					...
					.or_else(|| {
						...
						if default.is_null() { Some("") } else { default.as_str() }
					})
					.unwrap_or("")
			};
		...
		SettingType::IntegerArray => {
			...
			let state = TextListState::new(&schema.name).with_items(items).with_integer_mode();
			SettingControl::TextList(state)
		}
		...
		SettingType::Map { value_schema, display_field, no_add } => {
		...
		SettingType::ObjectArray { item_schema, display_field } => {
	...
	let layer_source = ctx.layer_sources.get(&schema.path).copied().unwrap_or(ConfigLayer::System);
		...
		SettingType::Boolean => {
			...
			let inherited = schema.nullable
				&& current_value
					.map(|v| v.is_null())
					.unwrap_or(schema.default.as_ref().map(|d| d.is_null()).unwrap_or(true));
			SettingControl::Toggle(ToggleState::new(checked, &schema.name).with_inherited(inherited))
		}
	...
	match control {
		...
		SettingControl::DualList(state) => {
			let arr: Vec<serde_json::Value> =
				state.included.iter().map(|s| serde_json::Value::String(s.clone())).collect();
			serde_json::Value::Array(arr)
		}
...
mod tests {
	...
	fn test_build_number_item() {
		let schema = SettingSchema {
			...
			description: None,
			setting_type: SettingType::Integer { minimum: Some(1), maximum: Some(16) },
