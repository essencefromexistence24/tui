...
pub fn search_settings(pages: &[SettingsPage], query: &str) -> Vec<SearchResult> {
	...
	results.sort_by(|a, b| b.score.cmp(&a.score).then_with(|| a.item.name.cmp(&b.item.name)));
			...
			for (entry_idx, (key, value)) in map_state.entries.iter().enumerate() {
				...
				if key_score > 0 {
					results.push(SearchResult {
						...
						deep_match: Some(DeepMatch::MapKey { key: key.clone(), entry_index: entry_idx }),
					});
			...
			for (list_idx, text) in list_state.items.iter().enumerate() {
				...
				if score > 0 {
					results.push(SearchResult {
						...
						description_matches: Vec::new(),
						deep_match: Some(DeepMatch::TextListItem { text: text.clone(), item_index: list_idx }),
					});
				}
			}
		}
	...
	match value {
		serde_json::Value::String(s) => {
			...
			if score > 0 {
				...
				let display_name =
					if field_name.is_empty() { s.clone() } else { format!("{}: {}", field_name, s) };
				results.push(SearchResult {
					page_index,
					item_index,
					item: item.clone(),
...
pub fn matches_query(item: &SettingItem, query: &str) -> bool {
	...
	item.name.to_lowercase().contains(&query_lower)
		|| item.description.as_ref().map(|d| d.to_lowercase().contains(&query_lower)).unwrap_or(false)
...
mod tests {
	...
	fn test_search_settings_description_match() {
		let pages = vec![make_page(
			"Editor",
			vec![make_item("Tab Size", Some("Number of spaces per tab character"), "/tab_size")],
		)];
	...
	fn test_search_settings_path_match() {
		let pages = vec![make_page("Editor", vec![make_item("Tab Size", None, "/editor/tab_size")])];
	...
	fn test_matching_categories() {
		let pages = vec![
			make_page("Editor", vec![make_item("Line Numbers", None, "/line_numbers")]),
	...
	fn test_search_map_key_no_duplicate_nested_values() {
		...
		let pages = vec![make_page(
			...
			vec![make_map_item(
				"Languages",
				"/languages",
				vec![("bash".to_string(), serde_json::json!({"grammar": "bash", "files": ["1.bash"]}))],
			)],
		)];
		...
		assert_eq!(
			...
			deep_results.len(),
			deep_results.iter().map(|r| &r.deep_match).collect::<Vec<_>>()
		);
		assert!(
	...
	fn test_search_map_nested_value() {
		...
		let deep_results: Vec<_> = results
			...
			.collect();
		assert!(!deep_results.is_empty(), "Should find nested value 'rust-analyzer'");
	}
