...
impl LineWrapCache {
	pub fn with_byte_budget(byte_budget: usize) -> Self {
		...
		Self { map: HashMap::new(), order: VecDeque::new(), byte_budget, current_bytes: 0 }
	}
...
pub fn char_position_in_layout(layout: &[ViewLine], char_pos_in_line: usize) -> (usize, usize) {
	...
	for (i, line) in layout.iter().enumerate() {
		let source_chars_in_row = line.char_source_bytes.iter().filter(|b| b.is_some()).count();
...
pub fn compute_line_layout(
	...
	if is_compose && !state.soft_breaks.is_empty() {
		let sb = state.soft_breaks.query_viewport(line_start, line_end, &state.marker_list);
		if !sb.is_empty() {
	...
	if is_compose && !state.conceals.is_empty() {
		let cr = state.conceals.query_viewport(line_start, line_end, &state.marker_list);
		if !cr.is_empty() {
...
pub fn count_visual_rows_for_text_with_soft_breaks(
	...
	if soft_breaks_in_line.is_empty() {
		return count_visual_rows_for_text(line_text, effective_width, gutter_width, hanging_indent);
	}
	...
	for &(pos, indent) in soft_breaks_in_line {
		...
		let consumed = line_text[rel..].chars().next().map(|c| c.len_utf8()).unwrap_or(0);
...
mod tests {
	...
	fn pipeline_inputs_version_changes_when_any_source_changes() {
		...
		assert_ne!(a, pipeline_inputs_version(101, 5, 3, 7), "buffer bump changes version");
		assert_ne!(a, pipeline_inputs_version(100, 6, 3, 7), "soft-break bump changes version");
		assert_ne!(a, pipeline_inputs_version(100, 5, 4, 7), "conceal bump changes version");
		assert_ne!(a, pipeline_inputs_version(100, 5, 3, 8), "virtual-text bump changes version");
	}
	...
	fn stale_soft_break_inside_multibyte_char_does_not_panic() {
		...
		let rows = count_visual_rows_for_text_with_soft_breaks(text, line_start, &breaks, 80, 6, false);
		assert!(rows >= 1);
	}
	...
	fn soft_break_row_count_properties() {
		...
		for _iter in 0..2000 {
			...
			let text: String = (0..n_pieces).map(|_| palette[next() % palette.len()]).collect();
			...
			let breaks: Vec<(usize, u16)> = (0..n_breaks)
				.map(|_| {
					...
					let indent = if next() % 10 == 0 { 500 } else { (next() % 12) as u16 };
					(pos, indent)
				})
				.collect();
			...
			assert_eq!(rows, again, "non-deterministic: text={text:?} breaks={breaks:?}");
	...
	fn shadow_agreement_pure_primitive() {
		...
		for step in 0..400usize {
			...
			assert!(real.current_bytes() <= real.byte_budget(), "cache exceeded byte budget");
		}
	}
	...
	fn version_bump_makes_old_entry_unreachable() {
		...
		let key_v1 = LineWrapKey { pipeline_inputs_version: 101, ..key_v0 };
		assert!(
		...
		assert_eq!(cache.get(&key_v0).map(|v| v.len()), Some(5), "v0 entry preserved until evicted");
	}
	...
	fn every_key_dimension_separates_entries() {
		...
		for (i, v) in variations.iter().enumerate() {
			...
			assert!(cache.get(v).is_none(), "variation {i} unexpectedly hit base entry");
