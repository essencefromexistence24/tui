...
impl ThemeInfo {
	...
	pub fn new(name: impl Into<String>, pack: impl Into<String>) -> Self {
		let name = name.into();
		let pack = pack.into();
		let key = if pack.is_empty() { name.clone() } else { format!("{}/{}", pack, name) };
		Self { name, pack, key }
	}
	...
	pub fn with_key(
		...
		Self { name: name.into(), pack: pack.into(), key: key.into() }
	}

	/// Get display name showing pack if present
	pub fn display_name(&self) -> String {
		if self.pack.is_empty() { self.name.clone() } else { format!("{} ({})", self.name, self.pack) }
	}
}
...
pub fn brighten_color(color: Color, amount: u8) -> Color {
	if let Some((r, g, b)) = color_to_rgb(color) {
		Color::Rgb(r.saturating_add(amount), g.saturating_add(amount), b.saturating_add(amount))
	} else {
		color
	}
}
...
pub fn shade_toward_contrast(color: Color, amount: u8) -> Color {
	if let Some((r, g, b)) = color_to_rgb(color) {
		...
		if avg < 128 {
			...
			Color::Rgb(r.saturating_sub(amount), g.saturating_sub(amount), b.saturating_sub(amount))
		}
	} else {
		color
	}
...
pub trait TokenColorExt {
	...
	fn from_ratatui(color: Color) -> Option<dx_core::api::TokenColor>;
}

impl TokenColorExt for dx_core::api::TokenColor {
	fn to_ratatui(&self, theme: &Theme) -> Color {
		use dx_core::api::TokenColor;
		match self {
	...
	fn from_ratatui(color: Color) -> Option<dx_core::api::TokenColor> {
		...
		match color {
			...
			other => Some(TokenColor::Named(token_color_named_from_ratatui(other).to_string())),
		}
	}
}
...
impl From<ThemeFile> for Theme {
	fn from(file: ThemeFile) -> Self {
		Self {
			...
			diff_remove_collision_fg: file.editor.diff_remove_collision_fg.clone().map(|c| c.into()),
			diff_modify_collision_fg: file.editor.diff_modify_collision_fg.clone().map(|c| c.into()),
...
fn apply_theme_overrides(theme: &mut Theme, theme_file: &ThemeFile, raw: &serde_json::Value) {
	...
	if raw.get("editor").and_then(|v| v.as_object()).is_some_and(|editor| {
		editor.contains_key("whitespace_indicator_fg") && !editor.contains_key("indentation_guide_fg")
	}) {
...
impl Theme {
	...
	pub fn is_light(&self) -> bool {
		color_to_rgb(self.editor_bg).map(|(r, g, b)| relative_luminance(r, g, b) > 0.5).unwrap_or(false)
	}
	...
	pub fn from_json(json: &str) -> Result<Self, String> {
		...
		let theme_file: ThemeFile =
			serde_json::from_value(raw.clone()).map_err(|e| format!("Failed to parse theme: {}", e))?;
...
pub fn get_builtin_themes() -> serde_json::Value {
	...
	for theme in BUILTIN_THEMES {
		map.insert(theme.name.to_string(), serde_json::Value::String(theme.json.to_string()));
	}
...
mod tests {
	...
	fn test_load_builtin_theme() {
		...
		assert!(terminal.semantic_highlight_modifier.contains(Modifier::BOLD));
	}
	...
	fn test_modifier_for_bg_key_lookup() {
		...
		assert!(terminal.modifier_for_bg_key("editor.selection_bg").contains(Modifier::REVERSED));
		assert!(terminal.modifier_for_bg_key("ui.semantic_highlight_bg").contains(Modifier::BOLD));
		// Unknown / unmapped keys yield empty so we don't accidentally
		// tint other UI regions.
		assert!(terminal.modifier_for_bg_key("ui.popup_selection_bg").is_empty());
	...
	fn test_modifier_round_trip_via_theme_file() {
		...
		assert_eq!(round_tripped.semantic_highlight_modifier, original.semantic_highlight_modifier);
	}
	...
	fn test_minimal_user_theme_from_issue_1281_loads() {
		// Verbatim from https://github.com/anomalyco/dx/issues/1281
		let json = r#"{
  ...
  "syntax": {
	...
	fn test_extends_unknown_builtin_errors_with_helpful_message() {
		...
		assert!(err.contains("no-such-theme"), "error should quote the bad value, got: {}", err);
		assert!(
	...
	fn test_theme_without_name_still_errors() {
		...
		assert!(err.contains("name"), "error should mention the missing `name` field, got: {}", err);
	}
	...
	fn test_file_status_colors_resolve_via_theme_key() {
		...
		assert_eq!(theme.resolve_theme_key("ui.file_status_added_fg"), Some(Color::Rgb(80, 250, 123)));
		assert_eq!(
	...
	fn override_colors_writes_known_keys_and_drops_unknowns() {
		...
		assert_eq!(theme.resolve_theme_key("ui.status_bar_fg"), Some(Color::Rgb(1, 2, 3)));
	}
	...
	fn resolve_theme_key_mut_matches_resolve_theme_key_domain() {
		...
		for key in probe {
			assert!(theme.resolve_theme_key(key).is_some(), "reader lost key {key}");
			assert!(theme.resolve_theme_key_mut(key).is_some(), "mutator missing key {key}");
		}
	}
	...
	fn is_color_leaf(v: &serde_json::Value) -> bool {
		v.is_string()
			|| v.as_array().is_some_and(|a| a.len() == 3 && a.iter().all(serde_json::Value::is_number))
	}
	...
	fn color_keys_round_trip_through_the_same_field_and_section() {
		...
		let pairs: Vec<(String, Color)> =
			keys.iter().enumerate().map(|(i, (s, f))| (format!("{s}.{f}"), sentinel(i))).collect();
		...
		for (i, (s, f)) in keys.iter().enumerate() {
			...
			let color: Color = serde_json::from_value::<ColorDef>(leaf.clone())
				...
				.into();
			assert_eq!(color, sentinel(i), "`{s}.{f}` serialized back to the wrong field or section");
		}
	...
	fn test_all_builtin_themes_define_visible_occurrence_highlight() {
		for builtin in BUILTIN_THEMES {
			...
			if theme.modifier_for_bg_key("ui.semantic_highlight_bg").is_empty() {
				assert_ne!(
