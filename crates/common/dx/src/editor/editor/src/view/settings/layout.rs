...
impl SettingsLayout {
	...
	pub fn add_item(
		...
		self.items.push(ItemLayout { index, path, area, control, inherit_button });
	}

	/// Add a search result to the layout
	pub fn add_search_result(&mut self, page_index: usize, item_index: usize, area: Rect) {
		self.search_results.push(SearchResultLayout { page_index, item_index, area });
	}
				...
				match &item.control {
					...
					ControlLayoutInfo::Dropdown { button_area, option_areas, scroll_offset } => {
						...
						for (i, area) in option_areas.iter().enumerate() {
							if point_in_rect(*area, x, y) {
								return Some(SettingsHit::ControlDropdownOption(item.index, scroll_offset + i));
							}
						}
					...
					ControlLayoutInfo::DualList(dual_layout) => {
						...
						if let Some(hit) = dual_layout.hit_test(x, y) {
							return Some(match hit {
								...
								DualListHit::AddButton => SettingsHit::ControlDualListAdd(item.index),
								DualListHit::RemoveButton => SettingsHit::ControlDualListRemove(item.index),
								DualListHit::MoveUpButton => SettingsHit::ControlDualListMoveUp(item.index),
								DualListHit::MoveDownButton => SettingsHit::ControlDualListMoveDown(item.index),
							});
						}
					}
...
mod tests {
	...
	fn test_hit_test_number_value_area() {
		...
		assert_eq!(layout.hit_test(45, 10), Some(SettingsHit::ControlNumberValue(0)));
		assert_eq!(layout.hit_test(51, 10), Some(SettingsHit::ControlDecrement(0)));
		assert_eq!(layout.hit_test(55, 10), Some(SettingsHit::ControlIncrement(0)));
	}
}
