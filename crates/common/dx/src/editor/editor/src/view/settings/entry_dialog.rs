...
use super::items::{
	ItemBoxStyle, SettingControl, SettingItem, build_item_from_value, control_to_value,
};
...
pub fn layout_field_action_buttons(
	...
	let widths: Vec<u16> = buttons.iter().map(|(_, label)| label.chars().count() as u16).collect();
...
impl EntryDialogState {
	...
	pub fn from_schema(
		...
		let title =
			if is_new { format!("Add {}", schema.name) } else { format!("Edit {}", schema.name) };
	...
	pub fn entry_path(&self) -> String {
		...
		if key.is_empty() { self.map_path.clone() } else { format!("{}/{}", self.map_path, key) }
	}
	...
	fn reset_distinct_default(&self, idx: usize) -> Option<Value> {
		...
		let resettable =
			is_simple_field_control(&item.control) || matches!(item.control, SettingControl::Json(_));
	...
	fn field_inherits(&self, idx: usize) -> bool {
		self
			.items
			.get(idx)
			.map(|item| self.inheritable_fields.contains(item.path.trim_start_matches('/')))
			.unwrap_or(false)
	}
	...
	pub fn field_action_buttons(&self, idx: usize) -> Vec<(FieldAction, String)> {
		...
		if self.reset_distinct_default(idx).is_some() {
			buttons.push((FieldAction::Reset, format!("[{}]", t!("settings.btn_reset"))));
		}
	...
	pub fn current_item(&self) -> Option<&SettingItem> {
		if self.focus_on_buttons { None } else { self.items.get(self.selected_item) }
	}

	/// Get currently selected item mutably
	pub fn current_item_mut(&mut self) -> Option<&mut SettingItem> {
		if self.focus_on_buttons { None } else { self.items.get_mut(self.selected_item) }
	}
		...
		if self.focus_on_buttons {
			if self.focused_button > 0 {
				...
				if self.first_editable_index < self.items.len() {
					...
					self.field_button_focus = self.field_focusable_count(self.selected_item).checked_sub(1);
				}
			}
		} else if let Some(i) = self.field_button_focus {
	...
	fn init_composite_focus(&mut self, from_above: bool) {
		if let Some(item) = self.items.get_mut(self.selected_item) {
			match &mut item.control {
				...
				SettingControl::ObjectArray(state) => {
					if from_above {
						state.focused_index = if state.bindings.is_empty() { None } else { Some(0) };
					} else {
				...
				SettingControl::TextList(state) => {
					if from_above {
						state.focused_item = if state.items.is_empty() { None } else { Some(0) };
					} else {
						// Coming from below: start at add-new
	...
	pub fn toggle_focus_region_direction(&mut self, forward: bool) {
		...
		if self.focus_on_buttons {
			...
			// Move to buttons
			self.focus_on_buttons = true;
			self.focused_button = if forward { 0 } else { self.button_count().saturating_sub(1) };
		}
	...
	pub fn update_focus_states(&mut self) {
		for (idx, item) in self.items.iter_mut().enumerate() {
			...
			let state =
				if !self.focus_on_buttons && idx == self.selected_item && self.field_button_focus.is_none()
				{
					FocusState::Focused
				} else {
					FocusState::Normal
	...
	pub fn total_content_height(&self) -> usize {
		let items_height: usize = self
			...
			.iter()
			.map(|item| {
				let section_h = if item.is_section_start { Self::SECTION_HEADER_HEIGHT } else { 0 };
	...
	pub fn selected_item_offset(&self) -> usize {
		...
		let own_section_h = self
			...
			.map(|item| if item.is_section_start { Self::SECTION_HEADER_HEIGHT } else { 0 })
			.unwrap_or(0);
...
mod tests {
	...
	fn select_field(dialog: &mut EntryDialogState, path: &str) -> usize {
		let idx = dialog.items.iter().position(|it| it.path == path).expect("field should exist");
	...
	fn esc_reverts_number_field() {
		let schema =
			prop_schema("/tab_size", "Tab Size", SettingType::Integer { minimum: None, maximum: None });
		let mut dialog = EntryDialogState::from_schema(
			"k".to_string(),
