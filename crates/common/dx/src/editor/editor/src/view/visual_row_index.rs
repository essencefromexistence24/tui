...
use crate::view::line_wrap_cache::{
	CacheViewMode, LineWrapKey, WrapGeometry, count_visual_rows_for_text,
	count_visual_rows_for_text_with_soft_breaks, pipeline_inputs_version,
};
...
impl VisualRowIndex {
	...
	pub fn line_row_count(&self, line_idx: usize) -> u32 {
		let next = self.prefix_sums.get(line_idx + 1).copied().unwrap_or_else(|| self.total_rows());
...
pub fn ensure_built(state: &mut EditorState, key: &VisualRowIndexKey) {
	...
	if line_count == 0 {
		...
		// every call.
		state.visual_row_index =
			VisualRowIndex { key: Some(*key), prefix_sums: vec![0], line_starts: vec![0] };
		return;
	}
	...
	let soft_break_pairs: Vec<(usize, u16)> = if state.soft_breaks.is_empty() {
		...
		state.soft_breaks.query_viewport(0, buffer_len + 1, &state.marker_list)
	};
	...
	for line_idx in 0..line_count {
		let line_start = state.buffer.line_start_offset(line_idx).unwrap_or(buffer_len);
		let line_end = if line_idx + 1 < line_count {
			state.buffer.line_start_offset(line_idx + 1).unwrap_or(buffer_len)
		} else {
			buffer_len
		};
	...
	state.visual_row_index = VisualRowIndex { key: Some(*key), prefix_sums, line_starts };
}
