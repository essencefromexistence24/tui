...
pub(super) fn render_left_margin(
	...
	if let Some((glyph, color)) = ctx.virtual_gutter_glyph {
		...
		.is_some_and(|k| ctx.fold_indicators.contains_key(&k) && !ctx.line_indicators.contains_key(&k))
	{
		...
		push_span_with_map(line_spans, line_view_map, indicator.symbol.clone(), style, None);
	} else {
		let mut style = Style::default();
		if let Some(bg) = indicator_bg {
			style = style.bg(bg);
	...
	if ctx.is_continuation {
		...
		let rendered_text =
			format!("{:>width$}", ctx.gutter_num, width = ctx.state.margins.left_config.width);
		...
		let rendered_text =
			format!("{:>width$}", display_num, width = ctx.state.margins.left_config.width);
		let mut margin_style = if is_cursor_line {
			Style::default().fg(ctx.theme.editor_fg)
		} else {
			Style::default().fg(ctx.theme.line_number_fg)
