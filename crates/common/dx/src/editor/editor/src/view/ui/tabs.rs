...
pub fn scroll_to_show_tab(
	...
	let max_offset = if total_width > max_width { max_offset_with_indicator } else { 0 };
	...
	// indicator-aware max.
	let effective_max = if result > 0 { max_offset } else { max_offset_no_indicator };
	...
	for t in tab_targets.iter() {
		match t {
			TabTarget::Buffer(id) => {
				...
				let is_terminal =
					meta.and_then(|m| m.virtual_mode()).map(|mode| mode == "terminal").unwrap_or(false);
...
pub fn calculate_tab_widths(
	...
	let resolved_names =
		resolve_tab_names(tab_targets, buffers, buffer_metadata, composite_buffers, group_names);
		...
		let modified = match t {
			TabTarget::Buffer(id) => {
				if composite_buffers.contains_key(id) {
					...
					if state.buffer.is_modified() { "*" } else { "" }
				} else {
					""
				}
			}
...
fn tab_styles(
	...
	let mut base_style = if is_active {
		let fg = if is_active_split { theme.tab_active_fg } else { theme.tab_inactive_fg };
		let bg = if is_active_split { theme.tab_active_bg } else { theme.tab_inactive_bg };
		Style::default().fg(fg).bg(bg).add_modifier(Modifier::BOLD)
	} else if is_hovered_name {
		// Non-active tab with name hovered - use hover background
		Style::default().fg(theme.tab_inactive_fg).bg(theme.tab_hover_bg)
	} else {
	...
	let close_style =
		if is_hovered_close { base_style.fg(theme.tab_close_hover_fg) } else { base_style };
...
type TabSpanLayout = (Vec<(Span<'static>, usize)>, Vec<(usize, usize, usize)>, Vec<TabTarget>);
...
fn build_tab_spans(
	...
	for t in tab_targets.iter() {
		...
		let modified = match t {
			TabTarget::Buffer(id) if !composite_buffers.contains_key(id) => {
				buffers.get(id).filter(|state| state.buffer.is_modified()).map(|_| "*").unwrap_or("")
			}
			_ => "",
		};
		let binary_indicator = match t {
		...
		let (base_style, close_style) =
			tab_styles(is_active, is_active_split, is_hovered_name, is_hovered_close, is_preview, theme);
...
fn build_visible_line(
	...
	if show_left {
		current_spans
			.push(Span::styled(SCROLL_INDICATOR_LEFT, Style::default().bg(theme.tab_separator_bg)));
		rendered_width += SCROLL_INDICATOR_WIDTH;
	}

	let right_reserve = if show_right { SCROLL_INDICATOR_WIDTH } else { 0 };
	for (mut span, width) in all_tab_spans.into_iter() {
		...
		if rendered_width + visible_chars_in_span > max_width.saturating_sub(right_reserve) {
			let remaining_width = max_width.saturating_sub(rendered_width).saturating_sub(right_reserve);
			let truncated_content =
				span.content.chars().skip(skip_chars_count).take(remaining_width).collect::<String>();
	...
	if show_right && rendered_width < max_width {
		current_spans
			.push(Span::styled(SCROLL_INDICATOR_RIGHT, Style::default().bg(theme.tab_separator_bg)));
		rendered_width += SCROLL_INDICATOR_WIDTH;
	}
...
fn map_tab_hit_areas(
	...
	for (idx, target) in rendered_targets.iter().enumerate() {
		...
		if let Some(r) = rec.as_deref_mut() {
			...
			r.run(screen_start, area.y, tab_width, Some(fg), Some(bg), "Tab Bar");
		}
...
impl TabsRenderer {
	...
	pub fn render_for_split(
		...
		if let Some(r) = rec.as_deref_mut() {
			r.run(area.x, area.y, area.width, None, Some("ui.tab_separator_bg"), "Tab Bar");
		}
		let resolved_names =
			resolve_tab_names(tab_targets, buffers, buffer_metadata, composite_buffers, group_names);
		...
		for (tab_idx, chunk) in all_tab_spans.chunks(spans_per_tab).enumerate() {
			...
			if separator_offset > 0 {
				...
				tab_ranges[tab_idx] =
					(start + separator_offset, end + separator_offset, close_start + separator_offset);
			}
			...
			if tab_idx < rendered_targets.len().saturating_sub(1) {
				final_spans.push((Span::styled(" ", Style::default().bg(theme.tab_separator_bg)), 1));
				separator_offset += 1;
			}
		}
		...
		let available =
			max_width.saturating_sub((show_left as usize + show_right as usize) * SCROLL_INDICATOR_WIDTH);

		// Phase 4: clip the spans to the viewport and paint the bar.
		let (current_spans, right_indicator_x) =
			build_visible_line(final_spans, area, offset, max_width, show_left, show_right, theme);
		...
		if let Some((plus_logical_start, plus_logical_end)) = inline_plus_range {
			...
			if plus_logical_end > visible_start && plus_logical_start < visible_end {
				let screen_start = if plus_logical_start >= visible_start {
					area.x + left_indicator_offset as u16 + (plus_logical_start - visible_start) as u16
				} else {
					area.x + left_indicator_offset as u16
				};
				let screen_end = if plus_logical_end <= visible_end {
					area.x + left_indicator_offset as u16 + (plus_logical_end - visible_start) as u16
				} else {
