...
impl ScrollSyncGroup {
	...
	pub fn new(id: ScrollSyncGroupId, left_split: SplitId, right_split: SplitId) -> Self {
		Self {
			...
			right_split,
			scroll_line: 0,
			anchors: vec![SyncAnchor { left_line: 0, right_line: 0 }],
		}
	}
	...
	pub fn set_anchors(&mut self, anchors: Vec<SyncAnchor>) {
		...
		if self.anchors.is_empty() {
			self.anchors.push(SyncAnchor { left_line: 0, right_line: 0 });
		}
	}
	...
	pub fn left_to_right_line(&self, left_line: usize) -> usize {
		...
		let anchor =
			self.anchors.iter().rfind(|a| a.left_line <= left_line).unwrap_or(&self.anchors[0]);
	...
	pub fn right_to_left_line(&self, right_line: usize) -> usize {
		...
		let anchor =
			self.anchors.iter().rfind(|a| a.right_line <= right_line).unwrap_or(&self.anchors[0]);
	...
	pub fn scroll_line_for_split(&self, split_id: SplitId) -> usize {
		if split_id == self.left_split { self.left_scroll_line() } else { self.right_scroll_line() }
	}
}
...
mod tests {
	...
	fn test_left_to_right_line_simple() {
		...
		group.set_anchors(vec![
			SyncAnchor { left_line: 0, right_line: 0 },
			SyncAnchor { left_line: 10, right_line: 10 },
			SyncAnchor { left_line: 20, right_line: 25 }, // Right has 5 extra lines
		]);
	...
	fn test_right_to_left_line() {
		...
		group.set_anchors(vec![
			SyncAnchor { left_line: 0, right_line: 0 },
			SyncAnchor { left_line: 10, right_line: 15 }, // Right has 5 extra lines
		]);
	...
	fn test_scroll_delta() {
		...
		group.set_anchors(vec![
			SyncAnchor { left_line: 0, right_line: 0 },
			SyncAnchor { left_line: 50, right_line: 60 },
		]);
