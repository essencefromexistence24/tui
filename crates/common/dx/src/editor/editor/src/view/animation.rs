...
impl SlideIn {
	pub fn new(from: Edge, duration: Duration) -> Self {
		Self { from, duration, after: None, before: None }
	}
...
impl FrameEffect for SlideIn {
	...
	fn apply(&mut self, buf: &mut Buffer, area: Rect, elapsed: Duration) -> EffectStatus {
		...
		for dy in 0..area.height {
			for dx in 0..area.width {
				...
				let before_cell = if let Some(before) = before {
					...
					{
						let idx = before_src_dy as usize * area.width as usize + before_src_dx as usize;
		...
		if t >= 1.0 { EffectStatus::Done } else { EffectStatus::Running }
	}
}
	...
	fn apply(&mut self, buf: &mut Buffer, area: Rect, elapsed: Duration) -> EffectStatus {
		...
		for dy in 0..area.height {
			for dx in 0..area.width {
				...
				if let (Some(new_rgb), Some(old_rgb)) = (color_to_rgb(cell.fg), color_to_rgb(old.fg)) {
					if new_rgb != old_rgb {
						cell.set_fg(blend_rgb(new_rgb, old_rgb, eased));
					}
				}
				if let (Some(new_rgb), Some(old_rgb)) = (color_to_rgb(cell.bg), color_to_rgb(old.bg)) {
					if new_rgb != old_rgb {
...
impl WaveEffect {
	...
	fn init(&mut self, buf: &Buffer, area: Rect) {
		...
		let fill_bg =
			bg_counts.into_iter().max_by_key(|&(_, n)| n).map(|(c, _)| c).unwrap_or(Color::Reset);
		...
		for dy in 0..area.height {
			...
			let close_run =
				|run: &mut Vec<usize>, particles: &mut Vec<WaveParticle>, words: &mut Vec<Word>| {
					if run.is_empty() {
						return;
					}
					let wid = words.len();
	...
	fn undulation(x: f32, t: f32, lp: f32, amp: f32) -> f32 {
		amp
			* (Self::A1 * (Self::K1 * x + Self::W1 * t + lp).sin()
	...
	fn paint_water(&self, buf: &mut Buffer, elapsed: Duration, level: f32, amp: f32) {
		...
		for layer in 1..=Self::LAYERS {
			...
			for dx in 0..area.width {
				let x = dx as f32;
				let surf = Self::surface_at(x, t, water_top_mean, amp);
				let ly = water_top_mean - Self::undulation(x, t, lp, amp) - Self::swing(t, amp) + base;
				...
				if let Some(dst) = buf.cell_mut((area.x + dx, area.y + row as u16)) {
					let gi = ((x * 0.4 + t * 1.2 + layer as f32).floor() as i64).rem_euclid(3) as usize;
					dst.set_symbol(CREST[gi]);
					dst.set_fg(fg);
				}
			}
...
fn blend_rgb(fg: (u8, u8, u8), bg: (u8, u8, u8), alpha: f32) -> Color {
	...
	let mix = |f: u8, b: u8| -> u8 {
		((f as f32) * a + (b as f32) * (1.0 - a)).round().clamp(0.0, 255.0) as u8
	};
...
impl AnimationRunner {
	pub fn new() -> Self {
		Self { next_id: 1, active: Vec::new(), total_started: 0, last_frame: None }
	}
	...
	pub fn start_with_id(&mut self, id: AnimationId, area: Rect, kind: AnimationKind) {
		...
		let now = Instant::now();
		let (effect, delay, duration): (Box<dyn FrameEffect + Send>, Duration, Duration) = match kind {
			AnimationKind::SlideIn { from, duration, delay } => {
				(Box::new(SlideIn::new(from, duration)), delay, duration)
			}
			AnimationKind::CursorJump { from, to, duration, cursor_color, bg_color } => (
				Box::new(CursorJump::new(from, to, duration, cursor_color, bg_color)),
				Duration::ZERO,
				duration,
			),
			AnimationKind::ColorTransition { duration } => {
				(Box::new(ColorTransition::new(duration)), Duration::ZERO, duration)
			}
			AnimationKind::Wave { duration } => {
				(Box::new(WaveEffect::new(duration)), Duration::ZERO, duration)
			}
		};
	...
	pub fn is_active(&self) -> bool {
		self.active.iter().any(|e| e.status == EffectStatus::Running)
	}
...
mod tests {
	...
	fn runner_caches_last_frame_for_push_transition() {
		...
		assert!(seen_old, "at least one OLD cell should still be visible mid-transition");
		let _ = id;
	}
	...
	fn runner_is_active_flips_after_duration() {
		...
		runner.apply_all(&mut buf);
		assert!(!runner.is_active(), "runner should have no active effects after duration elapses");
	}
	...
	fn cursor_jump_head_uses_cursor_color() {
		...
		assert!(found_full_cursor, "head cell should be painted with the full cursor color");
	}
	...
	fn cursor_jump_trail_fades_toward_bg() {
		...
		assert!(blended_count > 0, "at least one trail cell should be a blend between cursor and bg");
	}
	...
	fn cursor_jump_through_runner() {
		...
		runner.apply_all(&mut buf);
		assert!(!runner.is_active(), "cursor jump should complete after duration");
		let _ = id;
	}
	...
	fn color_transition_starts_at_old_colors() {
		...
		paint_colors(&mut old, area, Color::Rgb(200, 100, 0), Color::Rgb(10, 20, 30));
		let mut new = make_buf(3, 2);
		paint_colors(&mut new, area, Color::Rgb(0, 100, 200), Color::Rgb(90, 80, 70));
	...
	fn color_transition_blends_mid_flight() {
		...
		paint_colors(&mut new, area, Color::Rgb(0, 0, 0), Color::Rgb(255, 255, 255));
	...
	fn color_transition_leaves_unresolvable_colors_alone() {
		...
		let cell = new.cell((0, 0)).unwrap();
		assert_eq!(cell.fg, Color::Rgb(10, 10, 10), "old fg was Reset — no blend");
	...
	fn color_transition_through_runner_uses_cached_frame() {
		...
		paint_colors(&mut frame1, area, Color::Rgb(255, 0, 0), Color::Rgb(0, 0, 255));
		runner.apply_all(&mut frame1);
		...
		runner.start(area, AnimationKind::ColorTransition { duration: Duration::from_secs(3600) });
		runner.capture_before_all();
		let mut frame2 = make_buf(3, 2);
		paint_colors(&mut frame2, area, Color::Rgb(0, 255, 0), Color::Rgb(255, 255, 0));
		runner.apply_all(&mut frame2);
		assert!(runner.is_active());
	...
	fn wave_snapshots_ink_and_disturbs_content() {
		...
		assert!(non_a > 0, "wave should have displaced content / drawn crest glyphs");
	}
	...
	fn wave_through_runner_is_active_then_finishes() {
		...
		let mut runner = AnimationRunner::new();
		runner.start(area, AnimationKind::Wave { duration: Duration::from_millis(60) });
		assert!(runner.is_active());
