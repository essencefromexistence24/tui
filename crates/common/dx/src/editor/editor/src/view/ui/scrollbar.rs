...
impl ScrollbarState {
	...
	pub fn new(total_items: usize, visible_items: usize, scroll_offset: usize) -> Self {
		Self { total_items, visible_items, scroll_offset }
	}
	...
	pub fn thumb_geometry(&self, track_height: usize) -> (usize, usize) {
		...
		let thumb_size_raw =
			((self.visible_items as f64 / self.total_items as f64) * track_height as f64).ceil() as usize;
...
impl ScrollbarMouse {
	...
	pub fn press(&mut self, state: ScrollbarState, track: Rect, col: u16, row: u16) -> Option<usize> {
		...
		self.drag = Some(ScrollbarDrag { start_row: click_row, start_offset: new_offset });
		Some(new_offset)
	}
...
impl Default for ScrollbarColors {
	fn default() -> Self {
		Self { track: Color::DarkGray, thumb: Color::Gray }
	}
}
...
impl ScrollbarColors {
	...
	pub fn inactive() -> Self {
		Self { track: Color::Black, thumb: Color::DarkGray }
	}

	/// Create from theme colors
	pub fn from_theme(theme: &crate::view::theme::Theme) -> Self {
		Self { track: theme.scrollbar_track_fg, thumb: theme.scrollbar_thumb_fg }
	}

	/// Create from theme colors with hover
	pub fn from_theme_hover(theme: &crate::view::theme::Theme) -> Self {
		Self { track: theme.scrollbar_track_hover_fg, thumb: theme.scrollbar_thumb_hover_fg }
	}
}
...
pub fn render_scrollbar_with_hover(
	...
	let thumb_color = if is_thumb_hovered { Color::White } else { colors.thumb };
...
mod tests {
	...
	fn test_drag_to_offset_press_anywhere_on_thumb_no_jump() {
		...
		for row_in_thumb in thumb_top..(thumb_top + thumb_size) {
			...
			assert_eq!(new_offset, 75, "press at thumb row {row_in_thumb} should not move the viewport");
		}
	}
	...
	fn test_drag_to_offset_follows_cursor_down() {
		...
		assert!(dragged > 0, "drag down should increase offset, got {dragged}");
	}
	...
	fn test_offset_for_thumb_top_round_trip() {
		...
		let cases = [(200_usize, 50_usize, 20_usize), (1000, 30, 25), (50, 10, 15)];
	...
	fn test_offset_for_thumb_top_clamps_to_max() {
		...
		// wrap or overshoot.
		assert_eq!(state.offset_for_thumb_top(track, max_thumb_top + 100), 200 - 50);
	}
	...
	fn test_mouse_press_on_track_recenters_thumb() {
		...
		assert!(got_top.abs_diff(want_top) <= 1, "thumb landed at {got_top}, expected ~{want_top}");
	}
