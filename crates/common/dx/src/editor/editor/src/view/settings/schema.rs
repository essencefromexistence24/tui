...
pub enum SettingType {
	...
	DualList { options: Vec<EnumOption>, sibling_path: Option<String> },
...
pub fn parse_schema(schema_json: &str) -> Result<Vec<SettingCategory>, serde_json::Error> {
	...
	for (name, prop) in sorted_props {
		...
		let is_nullable = prop.any_of.as_ref().is_some_and(|variants| {
			variants
				.iter()
				.any(|v| v.schema_type.as_ref().map(|t| t.primary() == Some("null")).unwrap_or(false))
		});
		...
		if prop.standalone_category {
			...
			// concatenate them.
			let description = prop.description.clone().or_else(|| resolved.description.clone());
			categories.push(SettingCategory {
				name: display_name,
				path: path.clone(),
				description,
...
fn parse_setting(
	...
	let nullable = resolved.schema_type.as_ref().map(|t| t.contains_null()).unwrap_or(false)
	...
	SettingSchema {
		...
		order,
		nullable,
		enum_from: schema.enum_from.clone().or_else(|| resolved.enum_from.clone()),
		dual_list_sibling: schema
			.dual_list_sibling
			.clone()
			.or_else(|| resolved.dual_list_sibling.clone()),
		dynamically_extendable_status_bar_elements: schema.dynamically_extendable_status_bar_elements
...
fn determine_type(
	...
	if let Some(ref ref_path) = schema.ref_path {
		if let Some(options) = enum_values_map.get(ref_path) {
			if !options.is_empty() {
				return SettingType::Enum { options: options.clone() };
			}
		}
	}
		...
		let options: Vec<EnumOption> = values
			...
			.filter_map(|v| {
				if v.is_null() {
					...
					v.as_str().map(|s| EnumOption { name: s.to_string(), value: s.to_string() })
				}
			})
			.collect();
		if !options.is_empty() {
		...
		Some("array") => {
			...
			if let Some(ref items) = resolved.items {
				...
				if items.ref_path.is_some() {
					...
					let item_schema = parse_setting("item", "", item_resolved, defs, enum_values_map);
					...
					if matches!(item_schema.setting_type, SettingType::Object { .. }) {
						...
						return SettingType::ObjectArray { item_schema: Box::new(item_schema), display_field };
					}
				}
			}
			SettingType::Complex
			...
			if let Some(ref add_props) = resolved.additional_properties {
				match add_props {
					AdditionalProperties::Schema(schema_box) => {
						...
						let value_schema = parse_setting("value", "", inner_resolved, defs, enum_values_map);
...
fn resolve_ref<'a>(schema: &'a RawSchema, defs: &'a HashMap<String, RawSchema>) -> &'a RawSchema {
	...
	if let Some(ref variants) = schema.any_of {
		for variant in variants {
			let is_null =
				variant.schema_type.as_ref().map(|t| t.primary() == Some("null")).unwrap_or(false);
			if !is_null {
...
fn i18n_name(path: &str, fallback_name: &str) -> String {
	...
	if *translated == key { humanize_name(fallback_name) } else { translated.to_string() }
}
...
"##;
		...
		let fallback_cat =
			categories.iter().find(|c| c.path == "/fallback").expect("fallback should be a category");
		assert_eq!(fallback_cat.settings.len(), 3);

		// Verify sub-fields are properly typed
		let grammar = fallback_cat.settings.iter().find(|s| s.name == "Grammar").unwrap();
		assert!(matches!(grammar.setting_type, SettingType::String));

		let auto_indent = fallback_cat.settings.iter().find(|s| s.name == "Auto Indent").unwrap();
	...
	fn test_indentation_guide_generated_schema_renders_as_enum() {
		...
		let editor =
			categories.iter().find(|c| c.path == "/editor").expect("editor category should exist");
		...
		match &indentation_guide.setting_type {
			...
			other => panic!("expected indentation_guide to render as enum, got {:?}", other),
		}
	}
	...
	fn test_enum_from_parsed_from_schema() {
		...
		let theme =
			general.settings.iter().find(|s| s.name == "Theme").expect("should have Theme setting");
	...
	fn test_dual_list_parsed_from_schema() {
		...
		let categories = parse_schema(schema_json).unwrap();
		let general = &categories[0];
		let tags = general.settings.iter().find(|s| s.path == "/tags").expect("tags setting");
