			...
			KeyCode::Char(c) if ctrl => {
				match c {
					...
					'v' => {
						// Paste
						ctx
							.defer(DeferredAction::ExecuteAction(crate::input::keybindings::Action::PromptPaste));
						InputResult::Consumed
					}
					'k' => {
...
mod tests {
	...
	fn test_navigation_keys() {
		...
		assert!(
			ctx.deferred_actions.iter().any(|a| matches!(a, DeferredAction::FileBrowserSelectPrev))
		);
	}
	...
	fn test_character_input_updates_filter() {
		...
		assert!(
			ctx.deferred_actions.iter().any(|a| matches!(a, DeferredAction::FileBrowserUpdateFilter))
		);
	}
	...
	fn test_backspace_empty_goes_parent() {
		...
		assert!(ctx.deferred_actions.iter().any(|a| matches!(a, DeferredAction::FileBrowserGoParent)));
	}
	...
	fn test_enter_confirms() {
		...
		assert!(ctx.deferred_actions.iter().any(|a| matches!(a, DeferredAction::FileBrowserConfirm)));
	}
