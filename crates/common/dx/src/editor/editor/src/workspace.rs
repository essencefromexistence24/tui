...
//! ## Storage
//!
//! Workspaces are stored in `$XDG_DATA_HOME/dx/workspaces/{encoded_path}.json`
...
pub struct FileExplorerState {
	...
	#[serde(alias = "width_percent", default = "crate::config::default_explorer_width_value")]
...
/// at `$XDG_DATA_HOME/dx/file_states/{encoded_path}.json`.
...
/// `$XDG_DATA_HOME/dx/file_states/{encoded_path}.json` to avoid conflicts
	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			Self::WorkdirMismatch { expected, found } => {
				write!(f, "Working directory mismatch: expected {:?}, found {:?}", expected, found)
			}
			WorkspaceError::VersionTooNew { version, max_supported } => {
				write!(f, "Workspace version {} is newer than supported (max: {})", version, max_supported)
			}
		}
	}
}
...
impl Workspace {
	...
	pub fn load(working_dir: &Path) -> Result<Option<Workspace>, WorkspaceError> {
		...
		let expected = working_dir.canonicalize().unwrap_or_else(|_| working_dir.to_path_buf());
		let found =
			workspace.working_dir.canonicalize().unwrap_or_else(|_| workspace.working_dir.clone());

		if expected != found {
			tracing::warn!("Workspace working_dir mismatch: expected {:?}, found {:?}", expected, found);
	...
	pub fn has_no_preservable_content(&self) -> bool {
		self.external_files.is_empty()
			...
			&& self
				.split_states
				.values()
				.all(|s| s.open_tabs.iter().all(|t| matches!(t, SerializedTabRef::Terminal(_))))
	}
	...
	pub fn touch(&mut self) {
		self.saved_at = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs();
	}
}
...
mod tests {
	...
	fn test_workspace_config_overrides_with_values() {
		let overrides = WorkspaceConfigOverrides { line_wrap: Some(false), ..Default::default() };
		let json = serde_json::to_string(&overrides).unwrap();
	...
	fn test_file_state_serialization() {
		let file_state = SerializedFileState {
			cursor: SerializedCursor { position: 1234, anchor: Some(1000), sticky_column: 15 },
			additional_cursors: vec![SerializedCursor { position: 5000, anchor: None, sticky_column: 0 }],
			scroll: SerializedScroll { top_byte: 500, top_view_line_offset: 2, left_column: 10 },
	...
	fn test_bookmark_serialization() {
		...
		bookmarks
			.insert('a', SerializedBookmark { file_path: PathBuf::from("src/main.rs"), position: 1234 });
		bookmarks
			.insert('b', SerializedBookmark { file_path: PathBuf::from("src/lib.rs"), position: 5678 });
	...
	fn test_full_workspace_round_trip() {
		...
		workspace
			.bookmarks
			.insert('m', SerializedBookmark { file_path: PathBuf::from("src/main.rs"), position: 100 });
	...
	fn test_workspace_file_save_load() {
		...
		let temp_dir = std::env::temp_dir().join("dx_workspace_test");
		...
		workspace
			.bookmarks
			.insert('x', SerializedBookmark { file_path: PathBuf::from("test.txt"), position: 42 });
	...
	fn test_file_explorer_state_percent_round_trip() {
		let state = FileExplorerState {
			...
			side: crate::config::FileExplorerSide::Left,
			expanded_dirs: vec![PathBuf::from("src"), PathBuf::from("src/app"), PathBuf::from("tests")],
			scroll_offset: 5,
			show_hidden: true,
			show_gitignored: false,
		};
