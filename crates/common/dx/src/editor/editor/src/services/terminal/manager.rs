...
pub struct TerminalManager {
	...
	/// [`dx_core::WindowTerminalId`] — to stay unambiguous once they
...
impl TerminalManager {
	...
	/// it.
	pub fn new(window_id: dx_core::WindowId) -> Self {
		Self { window_id, terminals: HashMap::new(), next_id: 0, async_bridge: None }
	}
	...
	fn build_terminal(
		...
		let master_writer =
			pty_pair.master.take_writer().map_err(|e| format!("Failed to get PTY writer: {}", e))?;
		let reader =
			pty_pair.master.try_clone_reader().map_err(|e| format!("Failed to get PTY reader: {}", e))?;
		...
		// windows). See `dx_core::WindowTerminalId`.
		let wt_id = dx_core::WindowTerminalId::new(self.window_id, id);
	...
	pub fn cleanup_dead(&mut self) -> Vec<TerminalId> {
		let dead: Vec<TerminalId> =
			self.terminals.iter().filter(|(_, h)| !h.is_alive()).map(|(id, _)| *id).collect();
...
/// cwd in their own args, so both cwd and the local `DX_SESSION`
...
fn build_shell_command(
	...
	let TerminalWrapper { command: shell, args: cmd_args, manages_cwd: skip_cwd } = terminal_wrapper;
	...
	// control vars below, so TERM/DX_SESSION win over any same-named key
	// (issue #2355).
	...
	backing_path
		.and_then(|p| {
			...
			if existing_has_content {
				...
				std::fs::OpenOptions::new().create(true).write(true).truncate(true).open(p).ok()
			}
		})
		.map(std::io::BufWriter::new)
}
...
fn spawn_wait_thread(
	...
	async_bridge: Option<AsyncBridge>,
	wt_id: dx_core::WindowTerminalId,
	...
	thread::spawn(move || {
		...
		if let Some(bridge) = &async_bridge {
			#[allow(clippy::let_underscore_must_use)]
			let _ = bridge.sender().send(crate::services::async_bridge::AsyncMessage::TerminalExited {
				terminal: wt_id,
				exit_code,
			});
		}
		...
		loop {
			match command_rx.recv() {
				...
				Ok(TerminalCommand::Resize { cols, rows }) => {
					if let Err(e) = pty_master.resize(PtySize { rows, cols, pixel_width: 0, pixel_height: 0 })
					{
...
impl ReaderLoop {
	...
	fn run(mut self) {
		...
		loop {
			match self.reader.read(&mut buf) {
				Ok(0) => {
					// EOF - process exited.
					tracing::info!("Terminal {:?} EOF after {} total bytes", self.terminal_id, total_bytes);
					break;
				}
				Ok(n) => {
					total_bytes += n;
	...
	fn process_output(&mut self, bytes: &[u8]) {
		...
		for response in state.drain_pty_write_queue() {
			tracing::debug!("Terminal {:?} sending PTY response: {:?}", self.terminal_id, response);
			// Receiver may be dropped if the writer thread exited.
			#[allow(clippy::let_underscore_must_use)]
			let _ = self.response_tx.send(TerminalCommand::Write(response.into_bytes()));
		}
	...
	fn notify_redraw(&self) {
		if let Some(bridge) = &self.async_bridge {
			...
			let _ = bridge
				.sender()
				.send(crate::services::async_bridge::AsyncMessage::TerminalOutput { terminal: self.wt_id });
		}
	}
}
	...
	#[cfg(windows)]
	{
		...
		match prefix.kind() {
			...
			Prefix::VerbatimUNC(server, share) => {
				rebuilt.push(format!(r"\\{}\{}\", server.to_string_lossy(), share.to_string_lossy()));
			}
			_ => return Cow::Borrowed(path),
		}
...
mod tests {
	...
	fn terminal_ids_collide_across_windows_but_window_disambiguates() {
		use dx_core::{WindowId, WindowTerminalId};
		...
		assert_ne!(a0, b0, "same local terminal id in different windows must be distinct globally");
	}
