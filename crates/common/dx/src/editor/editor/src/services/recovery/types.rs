...
impl RecoveryChunk {
	...
	pub fn new(offset: usize, original_len: usize, content: Vec<u8>) -> Self {
		Self { offset, original_len, content }
	}
	...
	pub fn to_meta(&self) -> ChunkMeta {
		ChunkMeta { offset: self.offset, original_len: self.original_len, size: self.content.len() }
	}
}
...
impl ChunkedRecoveryData {
	...
	pub fn new(original_size: usize, final_size: usize, chunks: Vec<RecoveryChunk>) -> Self {
		Self { original_size, chunks, final_size }
	}
...
impl RecoveryMetadata {
	...
	pub fn format_description(&self) -> String {
		if self.original_file_size > 0 {
			format!("{} chunks, {} bytes original", self.chunk_count, self.original_file_size)
		} else {
...
impl SessionInfo {
	...
	pub fn new() -> Self {
		...
		Self { pid: std::process::id(), started_at: now, working_dir: std::env::current_dir().ok() }
	}
...
pub enum RecoveryResult {
	...
	Recovered { original_path: Option<PathBuf>, content: Vec<u8> },
	...
	RecoveredChunks { original_path: PathBuf, chunks: Vec<RecoveryChunk> },
...
impl InplaceWriteRecovery {
	...
	pub fn new(dest_path: PathBuf, temp_path: PathBuf, uid: u32, gid: u32, mode: u32) -> Self {
		...
		Self { dest_path, temp_path, uid, gid, mode, started_at: now, pid: std::process::id() }
	}
