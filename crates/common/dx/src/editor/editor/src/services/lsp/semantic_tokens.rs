...
pub fn is_semantic_token_overlay(overlay: &crate::view::overlay::Overlay) -> bool {
	overlay.namespace.as_ref().map(|ns| ns.as_str() == SEMANTIC_TOKENS_NAMESPACE).unwrap_or(false)
}
...
pub fn apply_semantic_tokens_range_to_state(
	...
	state.overlays.replace_range_in_namespace(&ns, &range, new_overlays, &mut state.marker_list);
	true
}
...
mod tests {
	...
	use crate::view::theme::{THEME_DARK, Theme};
	...
	fn semantic_token_overlays_shift_on_insert() {
		...
		state.apply(
			&mut cursors,
			&Event::Insert { position: 0, text: "abc".to_string(), cursor_id: CursorId::UNDO_SENTINEL },
		);
