...
use crate::services::remote::protocol::{
	PatchOp, RemoteDirEntry, RemoteMetadata, append_params, count_lf_params, decode_base64,
	ls_params, patch_params, read_params, stat_params, sudo_write_params, truncate_params,
	write_params,
};
...
impl RemoteFileSystem {
	...
	pub fn new(channel: Arc<AgentChannel>, connection_string: String) -> Self {
		Self { channel, connection_string }
	}
	...
	fn parse_temp_dir_from_info(info: Option<&serde_json::Value>) -> PathBuf {
		info
			.and_then(|r| r.get("temp_dir").and_then(|v| v.as_str()).map(PathBuf::from))
	...
	fn convert_metadata(rm: &RemoteMetadata, name: &str) -> FileMetadata {
		let modified =
			if rm.mtime > 0 { Some(UNIX_EPOCH + Duration::from_secs(rm.mtime as u64)) } else { None };
		...
		#[cfg(unix)]
		let is_readonly = {
			let (euid, user_groups) = crate::model::filesystem::StdFileSystem::current_user_groups();
	...
	fn convert_dir_entry(re: &RemoteDirEntry) -> DirEntry {
		...
		let modified =
			if re.mtime > 0 { Some(UNIX_EPOCH + Duration::from_secs(re.mtime as u64)) } else { None };
...
impl FileSystem for RemoteFileSystem {
	fn read_file(&self, path: &Path) -> io::Result<Vec<u8>> {
		...
		for chunk in data_chunks {
			if let Some(b64) = chunk.get("data").and_then(|v| v.as_str()) {
				let decoded =
					decode_base64(b64).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
	...
	fn read_range(&self, path: &Path, offset: u64, len: usize) -> io::Result<Vec<u8>> {
		...
		let agent_reported_size = result.get("size").and_then(|v| v.as_u64()).map(|s| s as usize);
	...
	fn count_line_feeds_in_range(&self, path: &Path, offset: u64, len: usize) -> io::Result<usize> {
		...
		result.get("count").and_then(|v| v.as_u64()).map(|c| c as usize).ok_or_else(|| {
			io::Error::new(io::ErrorKind::InvalidData, "missing count in count_lf response")
		})
	}
	...
	fn create_file(&self, path: &Path) -> io::Result<Box<dyn FileWriter>> {
		...
		Ok(Box::new(RemoteFileWriter::new(self.channel.clone(), path.to_path_buf())))
	}
	...
	fn open_file_for_append(&self, path: &Path) -> io::Result<Box<dyn FileWriter>> {
		...
		Ok(Box::new(AppendingRemoteFileWriter::new(self.channel.clone(), path.to_path_buf())))
	}
	...
	fn write_patched(&self, src_path: &Path, dst_path: &Path, ops: &[WriteOp]) -> io::Result<()> {
		...
		let dst_param = if src_path == dst_path { None } else { Some(dst_str.as_ref()) };
	...
	fn rename(&self, from: &Path, to: &Path) -> io::Result<()> {
		...
		self.channel.request_blocking("mv", params).map_err(Self::to_io_error)?;
		Ok(())
	}
	...
	fn copy(&self, from: &Path, to: &Path) -> io::Result<u64> {
		...
		let result = self.channel.request_blocking("cp", params).map_err(Self::to_io_error)?;
	...
	fn remove_file(&self, path: &Path) -> io::Result<()> {
		...
		self.channel.request_blocking("rm", params).map_err(Self::to_io_error)?;
		Ok(())
	}

	fn remove_dir(&self, path: &Path) -> io::Result<()> {
		let params = serde_json::json!({"path": path.to_string_lossy()});
		self.channel.request_blocking("rmdir", params).map_err(Self::to_io_error)?;
		Ok(())
	}
	...
	fn is_file(&self, path: &Path) -> io::Result<bool> {
		...
		Ok(result.get("file").and_then(|v| v.as_bool()).unwrap_or(false))
	}
	...
	fn set_permissions(&self, path: &Path, permissions: &FilePermissions) -> io::Result<()> {
		#[cfg(unix)]
		{
			...
			self.channel.request_blocking("chmod", params).map_err(Self::to_io_error)?;
		}
		#[cfg(not(unix))]
		{
	...
	fn read_dir(&self, path: &Path) -> io::Result<Vec<DirEntry>> {
		let path_str = path.to_string_lossy();
		let result =
			self.channel.request_blocking("ls", ls_params(&path_str)).map_err(Self::to_io_error)?;
	...
	fn create_dir(&self, path: &Path) -> io::Result<()> {
		let params = serde_json::json!({"path": path.to_string_lossy()});
		self.channel.request_blocking("mkdir", params).map_err(Self::to_io_error)?;
		Ok(())
	}
	...
	fn canonicalize(&self, path: &Path) -> io::Result<PathBuf> {
		let params = serde_json::json!({"path": path.to_string_lossy()});
		let result = self.channel.request_blocking("realpath", params).map_err(Self::to_io_error)?;

		let canonical = result
			.get("path")
			.and_then(|v| v.as_str())
			.ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "missing path in response"))?;
	...
	fn home_dir(&self) -> io::Result<PathBuf> {
		let result =
			self.channel.request_blocking("info", serde_json::json!({})).map_err(Self::to_io_error)?;

		let home = result
			.get("home")
			.and_then(|v| v.as_str())
			.ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "missing home in response"))?;
	...
	fn unique_temp_path(&self, dest_path: &Path) -> PathBuf {
		...
		let temp_dir = Self::parse_temp_dir_from_info(
			self.channel.request_blocking("info", serde_json::json!({})).ok().as_ref(),
		);
	...
	fn search_file(
		...
		let result = self.channel.request_blocking("search_file", params).map_err(Self::to_io_error)?;

		cursor.offset = result.get("next_offset").and_then(|v| v.as_u64()).unwrap_or(0) as usize;
		cursor.running_line = result.get("running_line").and_then(|v| v.as_u64()).unwrap_or(1) as usize;
	...
	fn sudo_write(&self, path: &Path, data: &[u8], mode: u32, uid: u32, gid: u32) -> io::Result<()> {
		...
		self
			.channel
			.request_blocking("sudo_write", sudo_write_params(&path_str, data, mode, uid, gid))
			.map_err(Self::to_io_error)?;
		Ok(())
	}
	...
	fn walk_files(
		...
		let (mut data_rx, result_rx) =
			self.channel.request_streaming_blocking("walk_files", params).map_err(Self::to_io_error)?;
...
impl RemoteFileWriter {
	fn new(channel: Arc<AgentChannel>, path: PathBuf) -> Self {
		Self { channel, path, buffer: Vec::new() }
	}
}
