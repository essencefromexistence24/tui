...
fn is_termux() -> bool {
	...
	std::env::var_os("PREFIX").map(|p| p.to_string_lossy().contains("com.termux")).unwrap_or(false)
}
...
fn termux_clipboard_get() -> Option<String> {
	...
	let output = std::process::Command::new("termux-clipboard-get").output().ok()?;
...
fn termux_clipboard_set(text: &str) -> bool {
	...
	let mut child = match Command::new("termux-clipboard-set").stdin(Stdio::piped()).spawn() {
		Ok(child) => child,
		Err(e) => {
...
impl Clipboard {
	...
	pub fn paste(&mut self) -> Option<String> {
		...
		if self.internal.is_empty() { None } else { Some(self.internal.clone()) }
	}
...
mod tests {
	...
	fn test_clipboard_config_disables_osc52() {
		let mut clipboard = Clipboard::new();
		let config = crate::config::ClipboardConfig { use_osc52: false, use_system_clipboard: true };
	...
	fn test_clipboard_config_disables_system() {
		let mut clipboard = Clipboard::new();
		let config = crate::config::ClipboardConfig { use_osc52: true, use_system_clipboard: false };
		clipboard.apply_config(&config);
		assert!(clipboard.use_osc52);
		assert!(!clipboard.use_system_clipboard);
	}
	...
	fn test_clipboard_internal_only_mode() {
		let mut clipboard = Clipboard::new();
		let config = crate::config::ClipboardConfig { use_osc52: false, use_system_clipboard: false };
		clipboard.apply_config(&config);
