...
impl ConnectionParams {
	...
	pub fn parse(s: &str) -> Option<Self> {
		...
		let (user_host, port) = if let Some((uh, p)) = s.rsplit_once(':') {
			if let Ok(port) = p.parse::<u16>() { (uh, Some(port)) } else { (s, None) }
		} else {
			(s, None)
		};
		...
		Some(Self { user, host: host.to_string(), port, identity_file: None, extra_args: Vec::new() })
	}
...
impl SshConnection {
	...
	pub async fn connect(params: ConnectionParams) -> Result<Self, SshError> {
		...
		let bootstrap = format!("python3 -u -c \"import sys;exec(sys.stdin.read({}))\"", agent_len);
		cmd.arg(bootstrap);
		...
		let ready: AgentResponse = serde_json::from_str(&ready_line).map_err(|e| {
			SshError::AgentStartFailed(format!("invalid ready message '{}': {}", ready_line.trim(), e))
		})?;

		if !ready.is_ready() {
			return Err(SshError::AgentStartFailed("agent did not send ready message".to_string()));
		}
		...
		Ok(Self { process: child, channel, params })
	}
...
pub fn spawn_reconnect_task(
	...
	spawn_reconnect_task_with(channel, connect_fn, ReconnectConfig::default(), "SSH remote")
}
	...
	let hint = match status {
		Ok(Ok(status)) => {
			match status.code() {
				...
				// that means `python3` is missing on the remote. Dx's remote
				...
				Some(127) => format!(
					"Python 3 was not found on the remote host {}. \
                     Dx's remote support requires python3 on the remote — \
                     install it there, then reconnect",
					params
				),
				Some(code) => {
					format!("SSH process exited with code {} while connecting to {}", code, params)
				}
				None => format!("SSH process was killed by a signal while connecting to {}", params),
			}
		}
		...
		Err(_) => {
			...
			format!("SSH process did not exit in time while connecting to {}", params)
		}
	};
...
async fn read_ssh_stderr(stderr: Option<ChildStderr>) -> Option<String> {
	...
	#[allow(clippy::let_underscore_must_use)]
	let _ =
		tokio::time::timeout(std::time::Duration::from_secs(2), stderr.read_to_string(&mut buf)).await;
	buf.trim().lines().map(str::trim).rfind(|line| !line.is_empty()).map(str::to_string)
}
...
async fn establish_ssh_transport(
	params: &ConnectionParams,
) -> Result<(BufReader<tokio::process::ChildStdout>, tokio::process::ChildStdin, Child), SshError> {
	let mut cmd = Command::new("ssh");
...
pub async fn spawn_local_agent_with_capacity(
	...
	Ok(std::sync::Arc::new(AgentChannel::with_capacity(reader, stdin, data_channel_capacity)))
}
...
#[doc(hidden)]
pub async fn spawn_local_agent_transport()
-> Result<(tokio::io::BufReader<tokio::process::ChildStdout>, tokio::process::ChildStdin), SshError>
{
	use tokio::process::Command as TokioCommand;
...
mod tests {
	...
	async fn heartbeat_keeps_channel_warm_and_exits_on_drop() {
		...
		assert!(channel.is_connected(), "channel stays connected while heartbeat pings");
		assert!(
