...
pub async fn bootstrap_agent(
	...
	let ready: AgentResponse = serde_json::from_str(&ready_line).map_err(|e| {
		TransportError::AgentStartFailed(format!("invalid ready message '{}': {e}", ready_line.trim()))
	})?;
	if !ready.is_ready() {
		return Err(TransportError::AgentStartFailed("agent did not send ready message".to_string()));
	}
...
impl KubeConnection {
	...
	pub async fn connect(target: KubeTarget) -> Result<Self, TransportError> {
		...
		Ok(Self { process: child, channel, display: transport.display(), heartbeat })
	}
...
mod tests {
	...
	fn argv_orders_flags_namespace_pod_then_command() {
		...
		assert_eq!(
			argv,
			vec!["--context", "k3d-dev", "exec", "-i", "-n", "dev", "dx-7c9f", "--", "python3", "-u",]
		);
	}
	...
	fn argv_includes_container_when_set() {
		...
		let pod = argv.iter().position(|a| a == "dx-7c9f").unwrap();
	...
	fn bootstrap_pycode_reads_exact_agent_length() {
		...
		assert_eq!(code, format!("import sys;exec(sys.stdin.read({}))", AGENT_SOURCE.len()));
	...
	async fn kube_reconnect_task_spawns_and_aborts_cleanly() {
		...
		let channel = crate::services::remote::spawn_local_agent().await.expect("spawn local agent");
	...
	fn build_command_pipes_stdio_and_targets_kubectl() {
		...
		assert_eq!(KubectlExecTransport::new(t).display(), "k8s:k3d-dev/dev/fresh-7c9f");
	}
}
