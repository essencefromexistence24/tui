...
use dx_core::services::PluginServiceBridge;
use std::any::Any;
...
impl PluginServiceBridge for EditorServiceBridge {
	...
	fn register_command(&self, command: dx_core::command::Command) {
		// Convert dx_core::command::Command to crate::input::commands::Command
		...
		self.command_registry.read().unwrap().register(editor_command);
	}
	...
	fn unregister_commands_by_prefix(&self, prefix: &str) {
		self.command_registry.read().unwrap().unregister_by_prefix(prefix);
	}

	fn unregister_commands_by_plugin(&self, plugin_name: &str) {
		self.command_registry.read().unwrap().unregister_by_plugin(plugin_name);
	}
