...
impl AgentChannel {
		...
		W: AsyncWrite + Unpin + Send + 'static,
	{
		...
		tokio::spawn(Self::write_task(Box::new(writer), write_rx, new_writer_rx, connected_write));

		// Spawn read task (lives for the lifetime of the AgentChannel)
		let pending_read = pending.clone();
		let connected_read = connected.clone();
		tokio::spawn(Self::read_task(Box::new(reader), new_reader_rx, pending_read, connected_read));
	...
	where
		...
		W: AsyncWrite + Unpin + Send + 'static,
	{
		self.runtime_handle.block_on(self.replace_transport(reader, writer));
	...
	pub fn set_request_timeout(&self, timeout: Duration) {
		self.request_timeout_ms.store(timeout.as_millis() as u64, Ordering::SeqCst);
	}
	...
	pub async fn request(
		...
		let result = tokio::time::timeout(timeout, async {
			...
			result_rx.await.map_err(|_| ChannelError::ChannelClosed)?.map_err(ChannelError::Remote)
		})
		.await;
	...
	pub async fn request_streaming(
		...
		(mpsc::Receiver<serde_json::Value>, oneshot::Receiver<Result<serde_json::Value, String>>),
		ChannelError,
	> {
		...
		self.write_tx.send(req.to_json_line()).await.map_err(|_| ChannelError::ChannelClosed)?;
	...
	pub async fn request_with_data(
		...
		match tokio::time::timeout(idle_timeout, result_rx).await {
			Ok(result) => {
				let result =
					result.map_err(|_| ChannelError::ChannelClosed)?.map_err(ChannelError::Remote)?;
				Ok((data, result))
			}
			Err(_elapsed) => {
				warn!("streaming request stalled awaiting result after {:?}", idle_timeout);
				self.connected.store(false, Ordering::SeqCst);
				Err(ChannelError::Timeout)
			}
		}
	...
	pub fn request_with_data_blocking(
		...
		self.runtime_handle.block_on(self.request_with_data(method, params))
	}
	...
	pub fn request_streaming_blocking(
		...
		ChannelError,
	> {
		self.runtime_handle.block_on(self.request_streaming(method, params))
	}
