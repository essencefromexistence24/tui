...
impl WorkspaceTrust {
	...
	pub fn decide(&self, command: &str, cwd: Option<&str>) -> SpawnDecision {
		match self.level() {
			...
			TrustLevel::Blocked => {
				SpawnDecision::Deny("workspace trust is set to Blocked — no processes may run".to_string())
			}
...
impl TrustStore {
	...
	pub fn for_project_dir(project_state_dir: &Path) -> Self {
		Self { path: project_state_dir.join("trust.json") }
	}
	...
	fn recorded_level(&self) -> Option<TrustLevel> {
		...
		serde_json::from_str::<StoredTrust>(&text).ok().map(|s| s.level)
	}
...
pub fn executable_content_markers(root: &Path) -> Vec<String> {
	...
	if root.join(".devcontainer").join("devcontainer.json").is_file()
...
mod tests {
	...
	fn blocked_denies_everything() {
		...
		assert!(matches!(t.decide("/usr/bin/git", None), SpawnDecision::Deny(_)));
	}
	...
	fn restricted_blocks_absolute_path_inside_workspace() {
		let t = trust("/home/u/proj", TrustLevel::Restricted);
		assert!(matches!(t.decide("/home/u/proj/.venv/bin/python", None), SpawnDecision::Deny(_)));
	}
	...
	fn restricted_blocks_relative_path_resolving_into_workspace() {
		...
		assert!(matches!(t.decide("./.venv/bin/python", Some("/home/u/proj")), SpawnDecision::Deny(_)));
	...
	fn restricted_allows_relative_path_escaping_workspace() {
		...
		assert_eq!(t.decide("../evil", Some("/home/u/proj")), SpawnDecision::Allow);
	}
	...
	fn restricted_does_not_confuse_sibling_prefix() {
		...
		let t = trust("/home/u/proj", TrustLevel::Restricted);
		assert_eq!(t.decide("/home/u/proj-evil/bin/x", None), SpawnDecision::Allow);
	}
	...
	fn set_level_persists_through_store() {
		...
		assert_eq!(TrustStore::for_project_dir(&proj_dir).level(), TrustLevel::Trusted);
	}
	...
	fn set_store_adopts_new_projects_persisted_level() {
		...
		TrustStore::for_project_dir(&b).record(TrustLevel::Blocked).unwrap();
	...
	fn set_store_to_undecided_project_resets_to_default() {
		...
		TrustStore::for_project_dir(&a).record(TrustLevel::Trusted).unwrap();
		let wt = WorkspaceTrust::new_persistent(
			Some(a.clone()),
			TrustLevel::Trusted,
			TrustStore::for_project_dir(&a),
	...
	fn executable_content_markers_lists_what_triggered() {
		...
		for expected in [".envrc", "mise.toml", ".venv", "devcontainer.json", "App.csproj"] {
			assert!(markers.iter().any(|m| m == expected), "expected '{expected}' in {markers:?}");
		}
	...
	use crate::config::{EnvKind, default_env_detectors};
