...
/// Dx sessions are open across different projects.
///
/// Format:
/// - With project: `<display_name> — <project_name> — Dx TUI Editor`
/// - Without project: `<display_name> — Dx TUI Editor`
///
...
pub fn build_window_title(display_name: &str, project_name: Option<&str>) -> String {
	match project_name {
		Some(p) if !p.is_empty() => {
			format!("{} \u{2014} {} \u{2014} Dx TUI Editor", display_name, p)
		}
		_ => format!("{} \u{2014} Dx TUI Editor", display_name),
	}
}
...
mod tests {
	...
	fn preserves_ordinary_text() {
		assert_eq!(sanitize_title("foo/bar.rs — Dx TUI Editor"), "foo/bar.rs — Dx TUI Editor");
	}
	...
	fn title_includes_project_when_provided() {
		assert_eq!(
			...
			"foo/bar.rs \u{2014} my-project \u{2014} Dx TUI Editor"
		);
	}

	#[test]
	fn title_omits_project_when_none() {
		assert_eq!(build_window_title("foo/bar.rs", None), "foo/bar.rs \u{2014} Dx TUI Editor");
	}

	#[test]
	fn title_omits_project_when_empty_string() {
		assert_eq!(build_window_title("foo/bar.rs", Some("")), "foo/bar.rs \u{2014} Dx TUI Editor");
	}
	...
	fn title_handles_virtual_buffer_names() {
		assert_eq!(
			...
			"[No Name] \u{2014} my-project \u{2014} Dx TUI Editor"
		);
	}
}
