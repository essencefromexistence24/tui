...
impl FileWatcherManager {
	pub fn new() -> Self {
		Self { watcher: None, handles: HashMap::new() }
	}
	...
	pub fn watch(
		...
		let watcher = self.watcher.as_mut().expect("just constructed above if missing");
		watcher.watch(path, mode).map_err(|e| format!("watchPath({}): {}", path.display(), e))?;
...
fn matches_handle(watch_path: &Path, recursive: RecursiveMode, event_path: &Path) -> bool {
	match recursive {
		...
		RecursiveMode::NonRecursive => {
			...
			event_path == watch_path || event_path.parent().map(|p| p == watch_path).unwrap_or(false)
		}
	}
}
...
mod tests {
	...
	fn matches_handle_respects_recursive_mode() {
		...
		assert!(matches_handle(root, RecursiveMode::Recursive, Path::new("/repo/src/lib.rs")));
		assert!(matches_handle(root, RecursiveMode::NonRecursive, Path::new("/repo/lib.rs")));
		assert!(!matches_handle(root, RecursiveMode::NonRecursive, Path::new("/repo/src/lib.rs")));
		assert!(!matches_handle(root, RecursiveMode::Recursive, Path::new("/other/file.rs")));
	}
	...
	fn kind_classification_covers_main_variants() {
		use notify::event::*;
		assert!(matches!(classify_kind(&EventKind::Create(CreateKind::File)), PathChangeKind::Create));
		assert!(matches!(classify_kind(&EventKind::Remove(RemoveKind::File)), PathChangeKind::Delete));
		assert!(matches!(
