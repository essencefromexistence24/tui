...
impl DockerExecSpawner {
	pub(crate) fn with_env(
		...
		Self { container_id, user, workspace, base_env, trust }
	}
	...
	/// calls.
	#[cfg(test)]
	pub(crate) fn new(container_id: String, user: Option<String>, workspace: Option<String>) -> Self {
		Self::with_env(
			container_id,
			user,
			workspace,
...
impl DockerExecSpawner {
	...
	fn build_exec_args(
		...
		let chosen_cwd: Option<String> =
			cwd.map(|p| p.to_string_lossy().into_owned()).or_else(|| self.workspace.clone());
...
impl ProcessSpawner for DockerExecSpawner {
	...
	async fn spawn_to_file(
		...
		Err(SpawnError::Process("stdoutTo is not supported for docker-exec processes".to_string()))
	}
}
...
impl DockerLongRunningSpawner {
	pub(crate) fn with_env(
		...
		Self { inner: DockerExecSpawner::with_env(container_id, user, workspace, base_env, trust) }
	}
...
impl LongRunningSpawner for DockerLongRunningSpawner {
	async fn spawn_stdio(
		...
		gate(&self.inner.trust, command, cwd.map(|p| p.to_string_lossy()).as_deref())?;
...
mod tests {
	...
	fn build_exec_args_base_env_lands_before_container_id() {
		...
		let lang_pos = args.iter().position(|a| a == "LANG=C.UTF-8").expect("LANG env injected");
	...
	fn build_exec_args_extra_env_appended_after_base_env() {
		...
		assert!(base_idx < override_idx, "base env precedes per-call env so the call-site value wins");
	}
	...
	fn docker_long_running_spawn_injects_env_before_container_id() {
		...
		let sp = DockerLongRunningSpawner::new("abc".into(), Some("vscode".into()), Some("/ws".into()));
