...
use super::provider::{
	...
	case_mismatch_penalty, is_word_grapheme_for_lang, smart_case_matches,
};
...
fn scan_for_candidates(
	...
	let mut scored: Vec<(usize, &str)> = words
		.iter()
		.filter(|(_, w)| {
			w.len() > ctx.prefix.len() && smart_case_matches(w, &ctx.prefix, ctx.prefix_has_uppercase)
		})
		.map(|(off, w)| {
	...
	fn smart_case_lowercase_prefix_matches_all() {
		...
		match result {
			ProviderResult::Ready(candidates) => {
				...
				let http_req = candidates.iter().find(|c| c.label == "http_request").unwrap();
