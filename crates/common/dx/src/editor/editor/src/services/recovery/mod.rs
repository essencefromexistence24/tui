...
//!
//! This module provides Emacs-style file recovery for the Dx editor.
...
//!
//! ```rust,ignore
//! use dx::services::recovery::RecoveryService;
//!
//! // On startup
...
pub use types::{
	ChunkMeta, ChunkedRecoveryData, ChunkedRecoveryIndex, InplaceWriteRecovery, MAX_CHUNK_SIZE,
	RecoveryChunk, RecoveryEntry, RecoveryMetadata, RecoveryResult, SessionInfo, generate_buffer_id,
	path_hash,
};
...
impl RecoveryService {
	...
	pub fn end_session_preserving(&mut self, preserve_ids: &[String]) -> io::Result<()> {
		...
		if preserve_ids.is_empty() {
			...
			for entry in entries {
				if !preserve_ids.contains(&entry.id) && self.storage.delete_recovery(&entry.id).is_ok() {
					cleaned += 1;
				}
			}
			tracing::info!(
	...
	pub fn save_buffer(
		...
		self.last_save_times.insert(buffer_id.to_string(), Instant::now());
	...
	pub fn load_recovery_with_original(
		...
		Ok(RecoveryResult::Recovered { original_path: Some(original_file.to_path_buf()), content })
	}
...
mod tests {
	...
	fn test_save_and_recover() {
		...
		service.save_buffer(&id, chunks, Some(path), None, Some(1), 0, content.len()).unwrap();
		...
		match result {
			RecoveryResult::Recovered { original_path, content: loaded } => {
	...
	fn test_disabled_service() {
		...
		service.save_buffer("test", chunks, None, None, None, 0, 7).unwrap();
	}
	...
	fn test_load_recovery_returns_chunks_for_large_files() {
		...
		match result {
			RecoveryResult::RecoveredChunks { original_path: path, chunks } => {
