...
use crate::server::capture_backend::{
	CaptureBackend, terminal_setup_sequences, terminal_teardown_sequences,
};
...
use crate::server::protocol::{
	ClientControl, PROTOCOL_VERSION, ServerControl, ServerHello, TermSize, VersionMismatch,
};
...
pub struct EditorServerConfig {
	...
	/// Whether to auto-load ~/.config/dx/init.ts (requires `plugins_enabled`).
...
impl EditorServer {
	...
	pub fn run(&mut self) -> io::Result<()> {
		...
		loop {
			...
			if !input_events.is_empty() {
				tracing::debug!("[server] process_clients returned {} events", input_events.len());
			}
			...
			if let Some(ref mut editor) = self.editor {
				if editor.should_quit() {
					...
					if pending_authority.is_some() || restart_dir.is_some() {
						...
						if let Err(e) = self.rebuild_editor(restart_dir, pending_authority, pending_keepalive) {
			...
			let detach_requested = self.editor.as_ref().map(|e| e.should_detach()).unwrap_or(false);
			if detach_requested {
				...
				if let Some(idx) = self.last_input_client.take() {
					if idx < self.clients.len() {
						...
						let quit_msg =
							serde_json::to_string(&ServerControl::Quit { reason: "Detached".to_string() })
								.unwrap_or_default();
						// Best-effort: client may already be disconnected
						#[allow(clippy::let_underscore_must_use)]
			...
			let suspend_requested =
				self.editor.as_mut().map(|e| e.take_suspend_request()).unwrap_or(false);
			if suspend_requested {
				if let Some(idx) = self.last_input_client {
					if idx < self.clients.len() {
						...
						let suspend_msg =
							serde_json::to_string(&ServerControl::SuspendClient).unwrap_or_default();
						// Best-effort: client may already be disconnected
						#[allow(clippy::let_underscore_must_use)]
				...
				for wait_id in editor.take_completed_waits() {
					if let Some(client_id) = self.waiting_clients.remove(&wait_id) {
						...
						if let Some(client) = self.clients.iter_mut().find(|c| c.id == client_id) {
							let msg = serde_json::to_string(&ServerControl::WaitComplete).unwrap_or_default();
							#[allow(clippy::let_underscore_must_use)]
	...
	pub fn initialize_editor(&mut self) -> io::Result<()> {
		...
		tracing::info!("Editor initialized with size {}x{}", self.term_size.cols, self.term_size.rows);
	...
	pub(crate) fn rebuild_editor(
		...
		if let Some(dir) = new_working_dir {
			...
			self.workspace_trust.set_root(Some(self.config.working_dir.clone()));
		...
		if let Some(auth) = new_authority {
			tracing::info!("Rebuild: installing authority with label {:?}", auth.display_label);
		...
		tracing::info!("Rebuild: complete, {} clients kept attached", self.clients.len());
	...
	fn handle_new_connection(
		...
		let session_id = self
			...
			.session_name
			.clone()
			.unwrap_or_else(|| crate::workspace::encode_path_for_filename(&self.config.working_dir));
	...
	fn process_clients(&mut self) -> io::Result<(Vec<Event>, bool, Option<usize>)> {
		...
		for (idx, client) in self.clients.iter_mut().enumerate() {
			...
			match client.conn.read_data(&mut buf) {
				...
				Ok(n) => {
					tracing::debug!("[server] Client {} read {} bytes from data socket", client.id, n);
					let events = client.input_parser.parse(&buf[..n]);
					tracing::debug!("[server] Client {} parsed {} events", client.id, events.len());
					if !events.is_empty() {
		...
		if !control_messages.is_empty() {
			tracing::debug!("[server] Processing {} control messages", control_messages.len());
		}
				...
				ClientControl::OpenWindow { path } => {
					if let Some(ref mut editor) = self.editor {
						...
						if path.is_absolute() {
							...
							tracing::warn!("OpenWindow rejected: path must be absolute: {:?}", path);
						}
					}
				}
	...
	fn handle_event(&mut self, event: Event) -> io::Result<bool> {
		...
		match event {
			...
			Event::Mouse(mouse_event) => {
				editor.handle_mouse(mouse_event).map_err(|e| io::Error::other(e.to_string()))
			}
			Event::Resize(w, h) => {
				editor.resize(w, h);
				Ok(true)
	...
	fn render_and_broadcast(&mut self) -> io::Result<()> {
		...
		terminal.draw(|frame| editor.render(frame)).map_err(|e| io::Error::other(e.to_string()))?;
	...
	fn disconnect_all_clients(&mut self, reason: &str) -> io::Result<()> {
		...
		for client in &mut self.clients {
			...
			#[allow(clippy::let_underscore_must_use)]
			let _ = client.data_writer.try_write(&teardown);
			let quit_msg = serde_json::to_string(&ServerControl::Quit { reason: reason.to_string() })
				.unwrap_or_default();
			// Best-effort: client may already be disconnected
			#[allow(clippy::let_underscore_must_use)]
			let _ = client.conn.write_control(&quit_msg);
...
mod wave_dismiss_tests {
	...
	fn server_with_editor(prefix: &str) -> EditorServer {
		...
		let config = EditorServerConfig {
			...
			workspace_trust: Arc::new(crate::services::workspace_trust::WorkspaceTrust::permissive()),
	...
	fn key_press_dismisses_wave_in_daemon_mode() {
		...
		let consumed = server
			.handle_event(Event::Key(KeyEvent::new(KeyCode::Char('a'), KeyModifiers::empty())))
			.expect("handle_event");
	...
	fn mouse_event_dismisses_wave_in_daemon_mode() {
		...
		assert!(
			!server.editor().expect("editor present").wave_animation_active(),
