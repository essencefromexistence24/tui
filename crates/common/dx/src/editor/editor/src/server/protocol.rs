...
pub enum ClientControl {
	...
	/// Used when a `fresh <dir>` is invoked from inside Dx's own
...
mod tests {
	...
	fn test_open_window_roundtrip() {
		let msg = ClientControl::OpenWindow { path: "/home/user/project".to_string() };
	...
	fn test_control_messages_use_snake_case_tags() {
		let resize = ClientControl::Resize { cols: 100, rows: 50 };
	...
	fn test_truecolor_detection() {
		...
		// truecolor
		hello.env.insert("COLORTERM".to_string(), Some("truecolor".to_string()));
		assert!(hello.supports_truecolor());

		// 24bit
		hello.env.insert("COLORTERM".to_string(), Some("24bit".to_string()));
		assert!(hello.supports_truecolor());
	}
	...
	fn test_all_client_control_variants_serialize() {
		let variants: Vec<ClientControl> = vec![
			...
			ClientControl::Resize { cols: 100, rows: 50 },
			ClientControl::Ping,
	...
	fn test_all_server_control_variants_serialize() {
		let variants: Vec<ServerControl> = vec![
			...
			ServerControl::Pong,
			ServerControl::SetTitle { title: "Test".to_string() },
			ServerControl::Bell,
			ServerControl::Quit { reason: "test".to_string() },
			ServerControl::Error { message: "error".to_string() },
