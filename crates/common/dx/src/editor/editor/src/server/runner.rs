		...
		for (idx, client) in self.clients.iter_mut().enumerate() {
			...
			match client.conn.read_data(&mut buf) {
				...
				Ok(n) => {
					...
					if !events.is_empty() {
						tracing::trace!("Client {} sent {} bytes -> {} events", client.id, n, events.len());
	...
	fn handle_control_message(&mut self, client_idx: usize, msg: ClientControl) -> io::Result<()> {
		...
		match msg {
			...
			ClientControl::OpenFiles { .. } => {
				...
				tracing::warn!("Client {} sent OpenFiles but no editor is running", client.id);
			}
			ClientControl::OpenWindow { .. } => {
				// This runner doesn't have an editor, so we can't open windows
				tracing::warn!("Client {} sent OpenWindow but no editor is running", client.id);
			}
		}
		Ok(())
	}
