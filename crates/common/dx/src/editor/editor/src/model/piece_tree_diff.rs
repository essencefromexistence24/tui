...
pub fn diff_piece_trees(before: &Arc<PieceTreeNode>, after: &Arc<PieceTreeNode>) -> PieceTreeDiff {
	...
	if Arc::ptr_eq(before, after) {
		return PieceTreeDiff { equal: true, byte_ranges: Vec::new(), nodes_visited: 1 };
	}
	...
	if span_slices_equal(&before_spans, &after_spans) {
		return PieceTreeDiff { equal: true, byte_ranges: Vec::new(), nodes_visited };
	}
	...
	PieceTreeDiff { equal: false, byte_ranges: ranges, nodes_visited }
}

/// Total bytes stored under a node (without calling private methods).
fn node_bytes(node: &PieceTreeNode) -> usize {
	match node {
		PieceTreeNode::Internal { left_bytes, right, .. } => left_bytes + node_bytes(right),
...
fn diff_collect_leaves(
	...
	*nodes_visited += 2; // counting both before and after nodes
	...
	match (before.as_ref(), after.as_ref()) {
		...
		(
			PieceTreeNode::Internal { left: b_left, right: b_right, .. },
			PieceTreeNode::Internal { left: a_left, right: a_right, .. },
		) => {
			diff_collect_leaves(
				b_left,
				a_left,
	...
	*nodes_visited += 1;
	match node.as_ref() {
		...
		PieceTreeNode::Leaf { location, offset, bytes, line_feed_cnt } => {
			...
			out.push(Span { leaf, doc_offset: *doc_offset });
			*doc_offset += bytes;
		}
	}
}
...
fn common_suffix_bytes(before: &[Span], after: &[Span], prefix_bytes: usize) -> usize {
	let total_before = before.last().map(|s| s.doc_offset + s.leaf.bytes).unwrap_or(0);
	let total_after = after.last().map(|s| s.doc_offset + s.leaf.bytes).unwrap_or(0);
...
fn collect_diff_ranges(
	...
	let doc_end = after.last().map(|s| s.doc_offset + s.leaf.bytes).unwrap_or(0);
...
mod tests {
	...
	fn diff_after_path_copy_insert_at_eof() {
		...
		let chunk_size = 1000;
		let total = 10_000;
		let content: Vec<u8> = (0..total).map(|i| if i % 100 == 99 { b'\n' } else { b'A' }).collect();
		...
		after_tree.insert(insert_offset, BufferLocation::Added(1), 0, 5, Some(0), &[buf, insert_buf]);
