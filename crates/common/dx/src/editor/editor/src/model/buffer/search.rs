...
impl ChunkedSearchState {
	...
	pub fn progress_percent(&self) -> usize {
		(self.scanned_bytes * 100).checked_div(self.total_bytes).unwrap_or(100)
	}
}
...
pub(crate) enum SearchRegion {
	...
	Unloaded { file_offset: usize, bytes: usize, doc_offset: usize },
...
impl HybridSearchPlan {
	...
	pub fn execute(
		...
		for region in &self.regions {
			...
			match region {
				SearchRegion::Unloaded { file_offset, bytes, doc_offset: region_doc_offset } => {
					// Boundary overlap: prev_tail + start of unloaded region
					if !prev_tail.is_empty() {
						let overlap_read = (*bytes).min(overlap_size);
						if let Ok(head) = fs.read_range(&self.file_path, *file_offset as u64, overlap_read) {
					...
					let mut cursor =
						FileSearchCursor::for_range(*file_offset, *file_offset + *bytes, running_line);
					while !cursor.done && all_matches.len() < max_matches {
						let mut batch = fs.search_file(&self.file_path, pattern, &opts_bounded, &mut cursor)?;
					...
					if *bytes >= overlap_size {
						...
						prev_tail =
							fs.read_range(&self.file_path, tail_off as u64, overlap_size).unwrap_or_default();
					} else {
						prev_tail =
							fs.read_range(&self.file_path, *file_offset as u64, *bytes).unwrap_or_default();
					}
				}
				SearchRegion::Loaded { data, doc_offset: region_doc_offset } => {
					...
					let buf_doc_offset =
						if overlap_len > 0 { *region_doc_offset - overlap_len } else { *region_doc_offset };

					let newlines_in_overlap =
						search_buf[..overlap_len].iter().filter(|&&b| b == b'\n').count();
...
pub(crate) fn search_boundary_overlap(
	...
	for m in regex.find_iter(&buf) {
		...
		if m.start() < overlap_len && m.end() > overlap_len {
			...
			line_at += buf[counted_to..m.start()].iter().filter(|&&b| b == b'\n').count();
			counted_to = m.start();

			let line_start =
				buf[..m.start()].iter().rposition(|&b| b == b'\n').map(|p| p + 1).unwrap_or(0);
