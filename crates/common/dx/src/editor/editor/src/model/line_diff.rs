...
pub fn diff_lines(saved: &[u8], current: &[u8]) -> LineDiff {
	...
	if saved == current {
		return LineDiff { equal: true, changed_lines: vec![], changes: vec![] };
	}
	...
	LineDiff { equal: changed_lines.is_empty() && changes.is_empty(), changed_lines, changes }
}
...
fn longest_common_subsequence(saved: &[&[u8]], current: &[&[u8]]) -> Vec<LineMatch> {
	...
	while i > 0 && j > 0 {
		if saved[i - 1] == current[j - 1] {
			lcs.push(LineMatch { saved_idx: i - 1, current_idx: j - 1 });
			i -= 1;
			j -= 1;
...
fn classify_change(
	...
	if has_match_before && has_match_after {
		...
		if current_len > saved_len { ChangeType::Inserted } else { ChangeType::Modified }
	} else if has_match_before && !has_match_after {
