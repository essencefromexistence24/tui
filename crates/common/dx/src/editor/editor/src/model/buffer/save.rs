...
impl WriteRecipe {
	...
	pub(crate) fn to_write_ops(&self) -> Vec<WriteOp<'_>> {
		self
			...
			.map(|action| match action {
				RecipeAction::Copy { offset, len } => WriteOp::Copy { offset: *offset, len: *len },
				RecipeAction::Insert { index } => WriteOp::Insert { data: &self.insert_data[*index] },
			})
			.collect()
	}
	...
	pub(crate) fn has_copy_ops(&self) -> bool {
		self.actions.iter().any(|a| matches!(a, RecipeAction::Copy { .. }))
	}
...
pub(super) fn build_write_recipe(
	...
	for piece_view in piece_tree.iter_pieces_in_range(0, total) {
		...
		let buffer = buffers.get(buffer_id).ok_or_else(|| {
			io::Error::new(io::ErrorKind::InvalidData, format!("Buffer {} not found", buffer_id))
		})?;

		match &buffer.data {
			// Unloaded buffer: can use Copy if same source file, else load and send
			BufferData::Unloaded { file_path, file_offset, .. } => {
				...
				if can_copy {
					...
					actions.push(RecipeAction::Copy { offset: src_offset, len: piece_view.bytes as u64 });
					continue;
				}
	...
	Ok(WriteRecipe { src_path: src_path_for_copy.map(|p| p.to_path_buf()), insert_data, actions })
}
...
pub(super) fn create_recovery_temp_file(
	...
	// Get recovery directory: $XDG_DATA_HOME/dx/recovery or ~/.local/share/dx/recovery
	...
	let temp_name = format!(".inplace-{}-{}-{}.tmp", file_name.to_string_lossy(), pid, timestamp);
...
pub(super) fn write_inplace_recovery_meta(
	...
	dest_path: &Path,
	temp_path: &Path,
	_original_metadata: &Option<FileMetadata>,
) -> io::Result<()> {
	#[cfg(unix)]
	let (uid, gid, mode) = _original_metadata
		.as_ref()
		.map(|m| {
			(
				m.uid.unwrap_or(0),
...
pub(super) fn save_with_inplace_write(
	...
	#[allow(clippy::let_underscore_must_use)]
	let _ =
		write_inplace_recovery_meta(fs, &recovery_meta_path, dest_path, &temp_path, &original_metadata);
