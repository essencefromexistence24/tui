...
pub use dx_core::api::{OverlayColorSpec, OverlayOptions};
pub use dx_core::overlay::{OverlayHandle, OverlayNamespace};
pub use dx_core::{BufferId, ContainerId, CursorId, LeafId, SplitDirection, SplitId};
...
pub enum PopupContentData {
	Text(Vec<String>),
	List { items: Vec<PopupListItemData>, selected: usize },
}
	...
	pub fn inverse(&self) -> Option<Self> {
		match self {
			Self::Insert { position, text, .. } => {
				...
				Some(Self::Delete { range, deleted_text: text.clone(), cursor_id: CursorId::UNDO_SENTINEL })
			}
			Self::Delete { range, deleted_text, .. } => Some(Self::Insert {
				position: range.start,
				text: deleted_text.clone(),
				cursor_id: CursorId::UNDO_SENTINEL,
			}),
			Self::Batch { events, description } => {
				// Invert all events in the batch in reverse order
				let inverted: Option<Vec<Self>> = events.iter().rev().map(|e| e.inverse()).collect();
			...
			Self::AddCursor { cursor_id, position, anchor } => {
				// To undo adding a cursor, we remove it (store its state for redo)
				Some(Self::RemoveCursor { cursor_id: *cursor_id, position: *position, anchor: *anchor })
			}
			Self::RemoveCursor { cursor_id, position, anchor } => {
				// To undo removing a cursor, we add it back
				Some(Self::AddCursor { cursor_id: *cursor_id, position: *position, anchor: *anchor })
			}
			Self::MoveCursor {
				cursor_id,
				old_position,
			...
			Self::Scroll { line_offset } => Some(Self::Scroll { line_offset: -line_offset }),
			...
			Self::BulkEdit {
				...
				let inverted_edits: Vec<(usize, usize, usize)> =
					edits.iter().map(|(pos, del_len, ins_len)| (*pos, *ins_len, *del_len)).collect();
...
impl EventLog {
	...
	pub fn is_at_saved_position(&self) -> bool {
		match self.saved_at_index {
			...
			Some(saved_idx) => {
				...
				self.entries[start..end].iter().all(|entry| !entry.event.modifies_buffer())
			}
		}
	}
	...
	pub fn last_event(&self) -> Option<&Event> {
		if self.current_index > 0 { Some(&self.entries[self.current_index - 1].event) } else { None }
	}
...
mod tests {
	...
	fn test_event_log_append() {
		let mut log = EventLog::new();
		let event = Event::Insert { position: 0, text: "hello".to_string(), cursor_id: CursorId(0) };
	...
	fn test_undo_redo() {
		...
		log.append(Event::Insert { position: 0, text: "a".to_string(), cursor_id: CursorId(0) });

		log.append(Event::Insert { position: 1, text: "b".to_string(), cursor_id: CursorId(0) });
	...
	fn test_event_inverse() {
		let insert = Event::Insert { position: 5, text: "hello".to_string(), cursor_id: CursorId(0) };
	...
	fn test_truncate_on_new_event_after_undo() {
		...
		log.append(Event::Insert { position: 1, text: "c".to_string(), cursor_id: CursorId(0) });
	...
	fn test_navigation_after_undo_preserves_redo() {
		...
		assert!(log.can_redo(), "Navigation after undo should preserve redo history");

		// Redo should still work
		let redo_events = log.redo();
		assert!(!redo_events.is_empty(), "Redo should return events after navigation");
	}
	...
	fn test_undo_group_reverts_and_reapplies_atomically() {
		...
		for (i, ch) in ['a', 'b', 'c'].into_iter().enumerate() {
			log.append(Event::Insert { position: i, text: ch.to_string(), cursor_id: CursorId(0) });
		}
		log.end_undo_group();
		...
		let redo_events = log.redo();
		assert_eq!(redo_events.len(), 3, "all three inserts reapply in one redo");
		assert_eq!(log.current_index(), 3);
		assert!(!log.can_redo());
	}
	...
	fn test_ungrouped_edit_after_group_undoes_separately() {
		...
		log.begin_undo_group();
		log.append(Event::Insert { position: 0, text: "ab".to_string(), cursor_id: CursorId(0) });
		log.append(Event::Insert { position: 2, text: "cd".to_string(), cursor_id: CursorId(0) });
		log.end_undo_group();

		// Standalone insert (not in any group).
		log.append(Event::Insert { position: 4, text: "Z".to_string(), cursor_id: CursorId(0) });
		assert_eq!(log.current_index(), 3);
	...
	fn test_consecutive_groups_undo_independently() {
		...
		for _ in 0..2 {
			log.begin_undo_group();
			log.append(Event::Insert { position: 0, text: "xy".to_string(), cursor_id: CursorId(0) });
			log.append(Event::Insert { position: 2, text: "zw".to_string(), cursor_id: CursorId(0) });
			log.end_undo_group();
		}
	...
	fn test_write_action_after_undo_clears_redo() {
		...
		log.append(Event::Insert { position: 0, text: "b".to_string(), cursor_id: CursorId(0) });
		assert!(!log.can_redo(), "Write action after undo should clear redo history");
	}
	...
	fn test_is_at_saved_position_after_truncate() {
		...
		for i in 0..150 {
			log.append(Event::Insert { position: i, text: "x".to_string(), cursor_id: CursorId(0) });
		}
		...
		log.append(Event::Insert { position: 0, text: "NEW".to_string(), cursor_id: CursorId(0) });
		...
		assert!(!result, "Should not be at saved position after undo + new edit");
	}
}
