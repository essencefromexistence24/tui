...
impl MarkerList {
	...
	pub fn new() -> Self {
		Self { tree: IntervalTree::new(), _affinity_map: HashMap::new() }
	}
	...
	pub fn create_line_anchor(
		...
		let tree_id =
			self.tree.insert_line_anchor(start as u64, end as u64, estimated_line, confidence);
		MarkerId(tree_id)
	}
	...
	pub fn get_line_anchor_info(
		...
		match marker.marker_type {
			crate::model::marker_tree::MarkerType::LineAnchor { estimated_line, confidence } => {
	...
	pub fn query_line_anchors(
		...
		self
			...
			.into_iter()
			.filter_map(|m| {
				if let crate::model::marker_tree::MarkerType::LineAnchor { estimated_line, .. } =
					m.marker_type
				{
					Some((MarkerId(m.id), m.interval.start as usize, m.interval.end as usize, estimated_line))
				} else {
					None
				}
			})
...
mod tests {
	...
	mod property_tests {
		...
		fn arb_edit_op(max_buffer_size: usize) -> impl Strategy<Value = EditOp> {
			prop_oneof![
				(0..=max_buffer_size, 1..=50usize)
					.prop_map(|(pos, len)| EditOp::Insert { position: pos, length: len }),
				(0..=max_buffer_size, 1..=20usize)
					.prop_map(|(pos, len)| EditOp::Delete { position: pos, length: len }),
			]
		}
