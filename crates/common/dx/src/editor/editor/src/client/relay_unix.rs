...
use super::{ClientExitReason, get_terminal_size};
	...
	loop {
		...
		if resize_flag.swap(false, Ordering::SeqCst) {
			if let Ok(size) = get_terminal_size() {
				let resize_msg =
					serde_json::to_string(&ClientControl::Resize { cols: size.cols, rows: size.rows })
						.unwrap();
		...
		if fds[2].revents().map(|r| r.contains(PollFlags::POLLIN)).unwrap_or(false) {
			...
			while let Some(pos) = ctrl_buf.iter().position(|&b| b == b'\n') {
				...
				if let Ok(ctrl) = serde_json::from_str::<crate::server::protocol::ServerControl>(msg) {
					match ctrl {
