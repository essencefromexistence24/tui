	...
	match value {
		...
		Value::Object(map) => {
			let filtered: serde_json::Map<String, Value> =
				map.into_iter().filter_map(|(k, v)| strip_nulls(v).map(|v| (k, v))).collect();
...
fn strip_empty_defaults(value: Value) -> Option<Value> {
	match value {
		...
		Value::Array(arr) => {
			...
			if filtered.is_empty() { None } else { Some(Value::Array(filtered)) }
		}
		other => Some(other),
	}
}
...
fn find_changed_paths_recursive(
	...
	match (old, new) {
		(Value::Object(old_map), Value::Object(new_map)) => {
			...
			let all_keys: std::collections::HashSet<_> = old_map.keys().chain(new_map.keys()).collect();
			for key in all_keys {
				let path =
					if prefix.is_empty() { format!("/{}", key) } else { format!("{}/{}", prefix, key) };
...
fn reconcile_cst_object(
	...
	let prop_name =
		|prop: &CstObjectProp| -> Option<String> { prop.name().and_then(|n| n.decoded_value().ok()) };
...
fn json_value_to_cst_input(value: &Value) -> jsonc_parser::cst::CstInputValue {
	...
	match value {
		...
		Value::Array(arr) => CstInputValue::Array(arr.iter().map(json_value_to_cst_input).collect()),
		Value::Object(map) => CstInputValue::Object(
			map.iter().map(|(k, v)| (k.clone(), json_value_to_cst_input(v))).collect(),
		),
	}
}
...
pub enum ConfigLayer {
	...
	System,
	/// User-global settings (~/.config/dx/config.json)
	User,
...
impl ConfigResolver {
	...
	fn layer_write_path(&self, layer: ConfigLayer) -> Result<PathBuf, ConfigError> {
		match layer {
			...
			ConfigLayer::System => {
				Err(ConfigError::ValidationError("Cannot write to System layer".to_string()))
			}
		}
	}
	...
	pub fn save_to_layer(&self, config: &Config, layer: ConfigLayer) -> Result<(), ConfigError> {
		...
		let merged_value =
			serde_json::to_value(&merged).map_err(|e| ConfigError::SerializeError(e.to_string()))?;
	...
	pub fn save_to_layer_with_baseline(
		...
		let current_json =
			serde_json::to_value(current).map_err(|e| ConfigError::SerializeError(e.to_string()))?;
		let baseline_json =
			serde_json::to_value(baseline).map_err(|e| ConfigError::SerializeError(e.to_string()))?;
		let parent_json =
			serde_json::to_value(&parent).map_err(|e| ConfigError::SerializeError(e.to_string()))?;
...
impl Config {
	...
	///
	/// On macOS, prioritizes `~/.config/dx/config.json` if it exists.
	/// Then checks the standard system config directory.
	fn system_config_paths() -> Vec<PathBuf> {
		let mut paths = Vec::with_capacity(2);

		// macOS: Prioritize ~/.config/dx/config.json
		#[cfg(target_os = "macos")]
		if let Some(home) = dirs::home_dir() {
			let path = home.join(".config").join("dx").join(Config::FILENAME);
			if path.exists() {
				paths.push(path);
			}
		}
		...
		if let Some(config_dir) = dirs::config_dir() {
			let path = config_dir.join("dx").join(Config::FILENAME);
...
pub struct DirectoryContext {
	...
	/// e.g., ~/.local/share/dx on Linux, ~/Library/Application Support/fresh on macOS
...
impl DirectoryContext {
	...
	pub fn project_state_dir(&self, working_dir: &std::path::Path) -> std::path::PathBuf {
		...
		self.workspaces_dir().join(crate::workspace::encode_path_for_filename(&canonical))
	}
	...
	fn default_config_dir() -> Option<std::path::PathBuf> {
		#[cfg(target_os = "macos")]
		{
			dirs::home_dir().map(|p| p.join(".config").join("dx"))
		}

		#[cfg(not(target_os = "macos"))]
		{
			dirs::config_dir().map(|p| p.join("dx"))
		}
	}
}
...
mod tests {
	...
	fn resolver_loads_user_layer_with_comments() {
		...
		let config = resolver.resolve().unwrap();
		assert_eq!(config.editor.tab_size, 7, "commented user config should still apply tab_size");
		assert!(!config.editor.line_numbers, "commented user config should still apply line_numbers");
		drop(temp);
	}
	...
	fn clearing_default_lsp_args_persists_empty_and_survives_reload() {
		...
		resolver
			.save_changes_to_layer(&changes, &std::collections::HashSet::new(), ConfigLayer::User)
			.unwrap();
	...
	fn reconcile_preserves_comments_and_unchanged_inline_annotations() {
		...
		assert!(out.contains("// top-of-file note"), "file comment lost:\n{out}");
		assert!(
		...
		assert_eq!(reparsed.pointer("/editor/line_numbers"), Some(&serde_json::json!(false)));
	}
	...
	fn reconcile_appends_new_key_without_disturbing_comments() {
		...
		assert_eq!(reparsed.pointer("/editor/tab_size"), Some(&serde_json::json!(2)));
	}
...
"#;
	...
	fn realistic_commented_config_is_rejected_by_strict_json_but_accepted_by_loader() {
		...
		assert_eq!(v.pointer("/theme"), Some(&serde_json::json!("builtin://dracula")));
		assert_eq!(v.pointer("/languages/go/tab_size"), Some(&serde_json::json!(8)));
		assert_eq!(v.pointer("/keybindings/0/action"), Some(&serde_json::json!("next_window")));
		assert_eq!(
	...
	fn save_changes_does_not_clobber_unparseable_config() {
		...
		assert_eq!(after, original, "a failed save must leave the unparseable config file untouched");
		drop(temp);
	}
	...
	fn settings_save_preserves_full_realistic_config() {
		...
		let mut changes = std::collections::HashMap::new();
		changes.insert("/editor/auto_read_only".to_string(), serde_json::json!(true));
		resolver
			.save_changes_to_layer(&changes, &std::collections::HashSet::new(), ConfigLayer::User)
			.unwrap();
		...
		let saved = std::fs::read_to_string(&user_config_path).unwrap();
		// Comments survive.
		assert!(saved.contains("// stuff that's really thingy"), "editor comment lost:\n{saved}");
		assert!(saved.contains("// file explorer hooray:"), "file_explorer comment lost:\n{saved}");
		...
		assert_eq!(reparsed.pointer("/theme"), Some(&serde_json::json!("builtin://dracula")));
		// The change was applied.
		assert_eq!(reparsed.pointer("/editor/auto_read_only"), Some(&serde_json::json!(true)));
		drop(temp);
	}
	...
	fn resolver_project_overrides_user() {
		...
		let user_config_path = resolver.user_config_path();
		std::fs::create_dir_all(user_config_path.parent().unwrap()).unwrap();
		std::fs::write(&user_config_path, r#"{"editor": {"tab_size": 2, "line_numbers": false}}"#)
			.unwrap();
	...
	fn migration_adds_version() {
		...
		assert_eq!(migrated.get("version"), Some(&serde_json::json!(CURRENT_CONFIG_VERSION)));
	}
	...
	fn migration_v1_to_v2_is_idempotent() {
		...
		let left = migrated.pointer("/editor/status_bar/left").and_then(|v| v.as_array()).unwrap();
		let remote_count = left.iter().filter(|v| v.as_str() == Some("{remote}")).count();
		assert_eq!(
			remote_count, 1,
	...
	fn resolver_loads_legacy_camelcase_config() {
		...
		let user_config_path = resolver.user_config_path();
		std::fs::create_dir_all(user_config_path.parent().unwrap()).unwrap();
		std::fs::write(&user_config_path, r#"{"editor": {"tabSize": 3, "lineNumbers": false}}"#)
			.unwrap();
	...
	fn save_to_layer_writes_minimal_delta() {
		...
		resolver.save_to_layer(&config, ConfigLayer::Project).unwrap();
		...
		assert!(
			json.get("editor").and_then(|e| e.get("line_numbers")).is_none(),
		...
		assert!(
			json.get("editor").and_then(|e| e.get("scroll_offset")).is_none(),
	...
	fn issue_630_save_to_file_strips_settings_matching_defaults() {
		...
		assert_eq!(
			json.get("editor").and_then(|e| e.get("tab_size")).and_then(|v| v.as_u64()),
			Some(2),
	...
	fn toggle_lsp_preserves_command() {
		...
		assert!(!original_command.is_empty(), "Default python LSP should have a command");
	...
	fn issue_631_disabled_lsp_without_command_should_be_valid() {
		...
		assert_eq!(config.theme.0, "dracula", "Theme should be 'dracula' from config file");
	}
	...
	fn loading_lsp_without_command_uses_default() {
		...
		std::fs::write(&user_config_path, r#"{ "lsp": { "rust": { "enabled": false } } }"#).unwrap();
		...
		assert!(!config.lsp["rust"].as_slice()[0].enabled, "enabled should be false from file");
	}
	...
	fn settings_ui_toggle_lsp_preserves_command() {
		...
		assert!(config.lsp["rust"].as_slice()[0].enabled, "Default rust enabled should be true");
		...
		resolver.save_changes_to_layer(&changes, &deletions, ConfigLayer::User).unwrap();
		...
		assert!(!reloaded.lsp["rust"].as_slice()[0].enabled, "rust should be disabled");
		...
		assert!(final_config.lsp["rust"].as_slice()[0].enabled, "rust should be enabled");
	}
	...
	fn issue_806_manual_config_edits_lost_when_saving_from_ui() {
		...
		// Step 2: Load the config (simulating Dx startup)
		let config = resolver.resolve().unwrap();
		...
		assert_eq!(rust_analyzer.command, "rust-analyzer", "rust-analyzer command should be preserved");
		assert_eq!(
			rust_analyzer.args,
			Some(vec!["--log-file".to_string(), "/tmp/rust-analyzer-{pid}.log".to_string()]),
		...
		*config_json.pointer_mut("/editor/tab_size").expect("path should exist") = serde_json::json!(2);
		...
		resolver.save_to_layer(&modified_config, ConfigLayer::User).unwrap();
		...
		assert!(
			saved_json.get("lsp").and_then(|l| l.get("rust-analyzer")).is_some(),
		...
		let saved_args =
			saved_json.get("lsp").and_then(|l| l.get("rust-analyzer")).and_then(|r| r.get("args"));
		assert!(
			saved_args.is_some(),
		...
		assert_eq!(
			saved_json.get("editor").and_then(|e| e.get("tab_size")).and_then(|v| v.as_u64()),
			Some(2),
		...
		let reloaded = resolver.resolve().unwrap();
		assert_eq!(reloaded.editor.tab_size, 2, "tab_size change should be persisted");
		assert!(
	...
	fn issue_806_custom_lsp_entries_preserved_across_unrelated_changes() {
		...
		let config = resolver.resolve().unwrap();
		assert!(config.lsp.contains_key("my-custom-lsp"), "Custom LSP entry should be loaded");
		assert!(config.languages.contains_key("mycustomlang"), "Custom language should be loaded");

		// User changes only line_numbers in Settings UI
		let mut config_json = serde_json::to_value(&config).unwrap();
		*config_json.pointer_mut("/editor/line_numbers").expect("path should exist") =
		...
		assert!(
			saved_json.get("lsp").and_then(|l| l.get("my-custom-lsp")).is_some(),
		...
		assert!(
			saved_json.get("languages").and_then(|l| l.get("mycustomlang")).is_some(),
		...
		// Reload and verify
		let reloaded = resolver.resolve().unwrap();
		assert!(reloaded.lsp.contains_key("my-custom-lsp"), "Custom LSP should survive save/reload");
		assert!(
			reloaded.languages.contains_key("mycustomlang"),
			"Custom language should survive save/reload"
		);
		assert!(!reloaded.editor.line_numbers, "line_numbers change should be applied");
	}

	/// Issue #806 - Scenario 2: External file modification after Dx is running.
	///
	/// This is the most likely real-world scenario:
	/// 1. User starts Dx with default/existing config (loaded into memory)
	/// 2. User manually edits config.json WHILE Dx is running (external edit)
	/// 3. User opens Settings UI in Dx and changes a simple setting
	/// 4. User saves from Settings UI
	/// 5. BUG: The external edits are LOST because Dx's in-memory config
	///    doesn't have them
	///
	...
	fn issue_806_external_file_modification_lost_on_ui_save() {
		...
		// Step 1: User starts Dx with a simple config
		std::fs::write(&user_config_path, r#"{"theme": "monokai"}"#).unwrap();

		// Step 2: Dx loads the config (simulating startup)
		let config_at_startup = resolver.resolve().unwrap();
		assert_eq!(config_at_startup.theme.0, "monokai");
		assert!(!config_at_startup.lsp.contains_key("rust-analyzer"), "No custom LSP at startup");

		// Step 3: User externally edits config.json (e.g., with another editor)
		// to add custom LSP settings. Dx doesn't see this change yet.
		std::fs::write(
			&user_config_path,
			r#"{
                "theme": "monokai",
		...
		*config_json.pointer_mut("/editor/tab_size").expect("path should exist") = serde_json::json!(2);
		...
		assert!(
			...
			"BUG #806: External edits to config.json were lost! \
             The 'lsp' section added while Dx was running should be preserved. \
             Saved content: {}",
			saved_content
		);
	...
	fn save_to_layer_changing_to_default_value_should_persist() {
		...
		assert_eq!(baseline.theme.0, "dracula", "Theme should be 'dracula' from file");
		...
		resolver.save_to_layer_with_baseline(&config, &baseline, ConfigLayer::User).unwrap();

		// Step 5: Check what was saved to file
		let saved_content = std::fs::read_to_string(&user_config_path).unwrap();
		eprintln!("Saved config after changing to default theme:\n{}", saved_content);
	...
	fn universal_lsp_round_trip_via_config_resolver() {
		...
		assert_eq!(server.command, "quicklsp", "Command should come from defaults");
	}
	...
	fn universal_lsp_custom_server_merges_with_defaults() {
		...
		assert_eq!(config.universal_lsp["my-universal-server"].as_slice()[0].command, "my-server-bin");
