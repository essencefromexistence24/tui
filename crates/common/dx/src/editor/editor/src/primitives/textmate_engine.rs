...
use crate::primitives::highlight_types::{HighlightCategory, HighlightSpan, highlight_color};
...
impl TextMateEngine {
	...
	pub fn for_file(path: &Path, registry: &GrammarRegistry) -> Option<Self> {
		...
		let index = syntax_set.syntaxes().iter().position(|s| s.name == syntax.name)?;
	...
	pub fn highlight_viewport(
		...
		self.cache =
			Some(TextMateCache { range: desired_parse_start..parse_end, spans: spans.clone() });
		self.last_buffer_len = buffer.len();
...
fn scope_to_category(scope: &str) -> Option<HighlightCategory> {
	...
	if scope_lower.starts_with("markup.underline.link") || scope_lower.starts_with("markup.underline")
	{
...
mod tests {
	...
	fn test_scope_to_category() {
		assert_eq!(scope_to_category("comment.line"), Some(HighlightCategory::Comment));
		assert_eq!(scope_to_category("string.quoted"), Some(HighlightCategory::String));
		assert_eq!(scope_to_category("keyword.control"), Some(HighlightCategory::Keyword));
		assert_eq!(scope_to_category("keyword.operator"), Some(HighlightCategory::Operator));
		assert_eq!(scope_to_category("entity.name.function"), Some(HighlightCategory::Function));
		assert_eq!(scope_to_category("constant.numeric"), Some(HighlightCategory::Constant));
		assert_eq!(scope_to_category("variable.parameter"), Some(HighlightCategory::Variable));
	...
	fn test_diff_scopes_produce_categories() {
		...
		assert_eq!(scope_to_category("markup.inserted"), Some(HighlightCategory::String));
		assert_eq!(scope_to_category("markup.inserted.diff"), Some(HighlightCategory::String));
		assert_eq!(scope_to_category("markup.deleted"), Some(HighlightCategory::Keyword));
		assert_eq!(scope_to_category("markup.deleted.diff"), Some(HighlightCategory::Keyword));
		assert_eq!(scope_to_category("meta.diff.range"), Some(HighlightCategory::Function));
		assert_eq!(scope_to_category("meta.diff.header"), Some(HighlightCategory::Function));
	}
	...
	fn test_diff_parsing_produces_scopes() {
		...
		let syntax = ss.find_syntax_by_extension("diff").expect("Diff syntax should exist");
		...
		for line in &lines {
			...
			for (_offset, op) in &ops {
				...
				let scope_str =
					scopes.as_slice().iter().map(|s| s.build_string()).collect::<Vec<_>>().join(" ");
