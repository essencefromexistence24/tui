...
pub use dx_core::text_property::{TextProperty, TextPropertyEntry};
...
pub struct CollectedOverlay {
	...
	pub options: dx_core::api::OverlayOptions,
}
...
impl TextPropertyManager {
	...
	pub fn add(&mut self, property: TextProperty) {
		...
		let pos =
			self.properties.binary_search_by_key(&property.start, |p| p.start).unwrap_or_else(|e| e);
	...
	pub fn get_overlapping(&self, range: &Range<usize>) -> Vec<&TextProperty> {
		self.properties.iter().filter(|p| p.overlaps(range)).collect()
	}
	...
	pub fn remove_in_range(&mut self, range: &Range<usize>) {
		self.properties.retain(|p| !p.overlaps(range) && !range.contains(&p.start));
	}
	...
	pub fn from_entries(entries: Vec<TextPropertyEntry>) -> (String, Self, Vec<CollectedOverlay>) {
		...
		for entry in entries {
			...
			if !entry.properties.is_empty() {
				let property = TextProperty { start, end, properties: entry.properties };
				manager.add(property);
			}

			// Collect whole-entry style
			if let Some(style) = entry.style {
				collected_overlays.push(CollectedOverlay { range: start..end, options: style });
			}
			...
			for inline in entry.inline_overlays {
				...
				if abs_start < abs_end {
					collected_overlays
						.push(CollectedOverlay { range: abs_start..abs_end, options: inline.style });
					// Create a TextProperty for inline overlays with properties
					if !inline.properties.is_empty() {
						let property =
							TextProperty { start: abs_start, end: abs_end, properties: inline.properties };
						manager.add(property);
					}
				}
			}
...
mod tests {
	...
	fn test_text_property_with_properties() {
		let prop = TextProperty::new(0, 10)
			...
			.with_property("location", json!({"file": "test.rs", "line": 42, "column": 5}));

		assert_eq!(prop.get("severity"), Some(&json!("error")));
		assert_eq!(prop.get("location"), Some(&json!({"file": "test.rs", "line": 42, "column": 5})));
	...
	fn test_text_property_get_as() {
		...
		assert_eq!(loc, Some(Location { file: "test.rs".to_string(), line: 42, column: 5 }));
	}
