...
impl<'a> LineIterator<'a> {
	...
	pub(crate) fn new(
		...
		let line_start = if byte_pos == 0 {
			...
			let pos_to_load =
				if byte_pos >= buffer_len { buffer_len.saturating_sub(1) } else { byte_pos };
	...
	pub fn next_line(&mut self) -> Option<(usize, String)> {
		...
		let chunk = match self.buffer.get_text_range_mut(self.current_pos, bytes_to_scan) {
			Ok(data) => data,
			Err(e) => {
				tracing::error!("LineIterator: Failed to load chunk at offset {}: {}", self.current_pos, e);
				return None;
			}
		};
	...
	pub fn prev(&mut self) -> Option<(usize, String)> {
		...
		let chunk = match self.buffer.get_text_range_mut(scan_start, scan_len) {
			Ok(data) => data,
			Err(e) => {
				tracing::error!("LineIterator::prev(): Failed to load chunk at {}: {}", scan_start, e);
				return None;
			}
		};
		...
		let line_bytes = match self.buffer.get_text_range_mut(prev_line_start, prev_line_len) {
			Ok(data) => data,
			Err(e) => {
				tracing::error!("LineIterator::prev(): Failed to load line at {}: {}", prev_line_start, e);
				return None;
			}
		};
...
mod tests {
	...
	fn test_line_iterator_trailing_newline_emits_empty_line() {
		...
		let (pos, content) = iter.next_line().expect("Should emit empty line for trailing newline");
	...
	fn test_line_iterator_trailing_newline_starting_at_eof() {
		...
		let (pos, content) =
			iter.next_line().expect("Should emit empty line at EOF when starting there");
	...
	fn test_line_iterator_crlf() {
		...
		let (pos, line_content) = iter.next_line().expect("Should emit empty line after trailing CRLF");
	...
	fn test_line_iterator_crlf_from_middle() {
		...
		let iter = buffer.line_iterator(6, 80);
		assert_eq!(iter.current_position(), 5, "Iterator at byte 6 should find line start at byte 5");
		...
		assert_eq!(iter.current_position(), 10, "Iterator at byte 10 should be at line start already");
	}
	...
	fn test_line_iterator_large_single_line_chunked_correctly() {
		...
		assert_eq!(all_content.len(), content_len, "Total content length should match original");
		assert_eq!(all_content, content, "Reconstructed content should match original");
		...
		assert!(pos_1000 < pos_2000 && pos_2000 < pos_10000, "Markers should be in sequential order");
	}
}
