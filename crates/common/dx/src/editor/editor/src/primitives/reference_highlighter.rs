...
impl ReferenceHighlighter {
			...
			if let Some(locals_source) = get_locals_query(language) {
				match Query::new(&ts_language, locals_source) {
					Ok(query) => {
						...
						tracing::debug!("Locals query enabled for {:?} (scope-aware highlighting)", language);
					}
					Err(e) => {
						tracing::debug!(
	...
	fn highlight_with_locals(
		...
		let cursor_item = definitions
			...
			.or_else(|| {
				references
					.iter()
					.find(|(range, _)| cursor_position >= range.start && cursor_position <= range.end)
	...
	fn highlight_with_text_matching(
		...
		occurrences
			.into_iter()
			.map(|range| HighlightSpan { range, color: self.highlight_color, bg: None, category: None })
			.collect()
	}
...
mod tests {
	...
	fn test_cursor_at_end_of_buffer() {
		...
		let spans = highlighter.highlight_occurrences(&buffer, buffer.len(), 0, buffer.len(), 100_000);
...
"#;
		...
		let spans = highlighter.highlight_occurrences(&buffer, first_foo_pos, 0, buffer.len(), 100_000);
...
"#;
		...
		let spans = highlighter.highlight_occurrences(&buffer, outer_foo_pos, 0, buffer.len(), 100_000);
