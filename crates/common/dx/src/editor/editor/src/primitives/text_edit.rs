...
impl TextEdit {
	...
	pub fn with_text(text: &str) -> Self {
		...
		Self { lines, cursor_row: 0, cursor_col: 0, selection_anchor: None, multiline: true }
	}
	...
	pub fn selection_flat_range(&self) -> Option<(usize, usize)> {
		...
		Some((flat_offset(&self.lines, sr, sc), flat_offset(&self.lines, er, ec)))
	}
	...
	fn move_left_internal(&mut self) {
		if self.cursor_col > 0 {
			...
			self.cursor_col = crate::primitives::grapheme::prev_grapheme_boundary(line, self.cursor_col);
	...
	fn move_right_internal(&mut self) {
		...
		if self.cursor_col < line_len {
			...
			// `move_left_internal`).
			let line = &self.lines[self.cursor_row];
			self.cursor_col = crate::primitives::grapheme::next_grapheme_boundary(line, self.cursor_col);
