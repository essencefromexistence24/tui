...
impl GrammarLoader for LocalGrammarLoader {
	...
	fn languages_packages_dir(&self) -> Option<PathBuf> {
		self.config_dir.as_ref().map(|p| p.join("languages/packages"))
	}
...
impl GrammarRegistry {
	...
	/// 3. User-installed grammars from ~/.config/dx/grammars/
	/// 4. Language pack grammars from ~/.config/dx/languages/packages/
	...
	pub fn for_editor_with_additional(
		...
		Arc::new(Self::load_with_additional(&LocalGrammarLoader::new(config_dir), additional))
	}
	...
	pub fn load_with_additional(loader: &dyn GrammarLoader, additional: &[GrammarSpec]) -> Self {
		...
		let has_language_packs = loader.languages_packages_dir().is_some_and(|dir| loader.exists(&dir));
		let has_bundle_packs = loader.bundles_packages_dir().is_some_and(|dir| loader.exists(&dir));
		...
		let syntax_set = if !needs_builder {
			...
			tracing::info!("[grammar-build] Loaded {} syntaxes from packdump", ss.syntaxes().len());
			...
			if has_user_grammars {
				...
				tracing::info!("[grammar-build] Loading user grammars from {:?}...", grammars_dir);
				load_user_grammars(
					loader,
					&grammars_dir,
					&mut builder,
			...
			if has_language_packs {
				...
				tracing::info!("[grammar-build] Loading language pack grammars from {:?}...", packages_dir);
			...
			if has_bundle_packs {
				...
				tracing::info!("[grammar-build] Loading bundle grammars from {:?}...", bundles_dir);
			...
			if !additional.is_empty() {
				tracing::info!("[grammar-build] Adding {} plugin-registered grammars...", additional.len());
		...
		let manifest_aliases: Vec<(String, String)> = registry
			.grammar_sources()
			.values()
			.filter_map(|info| info.short_name.as_ref().map(|short| (short.clone(), info.name.clone())))
			.collect();
	...
	for path in entries {
		...
		if loader.exists(&manifest_path) {
			if let Ok(manifest) = parse_package_json(loader, &manifest_path) {
				process_manifest(loader, &path, manifest, builder, user_extensions, grammar_sources);
			}
			continue;
		}
...
fn parse_package_json(loader: &dyn GrammarLoader, path: &Path) -> Result<PackageManifest, String> {
	let content = loader.read_file(path).map_err(|e| format!("Failed to read file: {}", e))?;
...
fn process_manifest(
	...
	for grammar in &contributes.grammars {
		...
		tracing::info!("Loaded grammar {} from {:?}", grammar.scope_name, grammar_path);
		...
		grammar_sources.insert(
			...
			GrammarInfo {
				...
				source: GrammarSource::User { path: grammar_path.clone() },
				...
				for syntax in builder.syntaxes()[count_before..].iter() {
					grammar_sources.insert(
						...
						GrammarInfo {
							...
							source: GrammarSource::User { path: dir.to_path_buf() },
...
/// Dx-specific language pack manifest format
...
/// Load grammars from Dx language packages (installed via pkg manager).
///
/// These packages use a Dx-specific package.json format with:
/// ```json
/// {
///   "name": "hare",
///   "dx": {
...
fn load_language_pack_grammars(
	...
	let entries = match loader.read_dir(packages_dir) {
		Ok(entries) => entries,
		Err(e) => {
			tracing::debug!("Failed to read language packages directory {:?}: {}", packages_dir, e);
			return;
		}
	};
	...
	for package_path in entries {
		...
		// Try to parse as Dx language pack format
		...
		match syntect::parsing::SyntaxDefinition::load_from_str(
			...
			Err(e) => {
				tracing::warn!("Failed to parse grammar for language pack '{}': {}", manifest.name, e);
			}
		}
	}
}
...
fn load_bundle_grammars(
	...
	let entries = match loader.read_dir(bundles_dir) {
		Ok(entries) => entries,
		Err(e) => {
			tracing::debug!("Failed to read bundle packages directory {:?}: {}", bundles_dir, e);
			return;
		}
	};
	...
	for package_path in entries {
		...
		let manifest: crate::services::packages::PackageManifest = match serde_json::from_str(&content)
		{
			Ok(m) => m,
			Err(e) => {
				tracing::debug!("Failed to parse {:?}: {}", manifest_path, e);
...
mod tests {
	...
	impl MockGrammarLoader {
		fn new() -> Self {
			Self { grammars_dir: None, files: HashMap::new(), dirs: HashMap::new() }
		}
	...
	fn test_find_syntax_with_custom_languages_config() {
		let temp_dir = tempfile::tempdir().unwrap();
		let mut registry = Arc::try_unwrap(GrammarRegistry::for_editor(temp_dir.path().to_path_buf()))
			.ok()
		...
		let entry = registry.find_by_path(Path::new("CUSTOMBUILD"), None).unwrap();
		assert!(
		...
		let entry = registry.find_by_path(Path::new("script.myext"), None).unwrap();
		assert!(
			entry.display_name.to_lowercase().contains("bash")
				|| entry.display_name.to_lowercase().contains("shell"),
	...
	fn test_list_all_syntaxes() {
		...
		let mut syntaxes: Vec<_> =
			syntax_set.syntaxes().iter().map(|s| (s.name.as_str(), s.file_extensions.clone())).collect();
