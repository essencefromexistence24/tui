...
use dx_core::api::{BufferSavedDiff, JsCallbackId, PluginCommand};
...
fn buffer_line_byte_offset(
	...
	if want_end && current_line == line { Some(buffer_len) } else { None }
}
...
impl Editor {
	...
	pub fn shift_plugin_markers_for_edit(
		...
		let (eff_removed, eff_inserted) = if removed > 0 && inserted > 0 {
			if inserted >= removed { (0, inserted - removed) } else { (removed - inserted, 0) }
		} else {
			(removed, inserted)
		};
	...
	pub fn update_plugin_state_snapshot(&mut self) {
		let Some(snapshot_handle) = self.plugin_manager.read().unwrap().state_snapshot_handle() else {
			return;
		};
		let mut snapshot = snapshot_handle.write().unwrap();

		self.active_window_mut().populate_plugin_state_snapshot(&mut snapshot);
		...
		snapshot.workspace_trust_level = self.authority().workspace_trust.level().as_str().to_string();
		...
		snapshot.detected_env =
			crate::services::workspace_trust::detect_env(self.working_dir(), &self.config.env.detectors)
		...
		let dormant_infos =
			self.dormant_remote.iter().filter(|(id, _)| !self.windows.contains_key(id)).map(|(_, d)| {
		...
		let mut session_infos: Vec<dx_core::api::WindowInfo> = self
			.windows
			.values()
			.map(|s| {
				...
				let shared_worktree =
					slot.and_then(|m| m.get("shared_worktree")).and_then(|v| v.as_bool()).unwrap_or(false);
				dx_core::api::WindowInfo {
					id: s.id,
					label: s.label.clone(),
					...
					remote: s.authority_spec.remote_backend_info(self.session_keepalives.contains_key(&s.id)),
				}
			})
			.collect();
		...
		for (plugin_name, state_map) in &self.plugin_global_state {
			let entry = snapshot.plugin_global_states.entry(plugin_name.clone()).or_default();
	...
	pub fn handle_plugin_command(&mut self, command: PluginCommand) -> AnyhowResult<()> {
		match command {
			...
			PluginCommand::ClearVirtualLinesInRange { buffer_id, namespace, start, end, epoch } => {
				self.handle_clear_virtual_lines_in_range(buffer_id, namespace, start, end, epoch);
			}

			// ==================== Conceal Commands ====================
			PluginCommand::AddConceal { buffer_id, namespace, start, end, replacement, epoch } => {
				self.handle_add_conceal(buffer_id, namespace, start, end, replacement, epoch);
			}
			PluginCommand::ClearConcealNamespace { buffer_id, namespace } => {
				self.handle_clear_conceal_namespace(buffer_id, namespace);
			}
			PluginCommand::ClearConcealsInRange { buffer_id, start, end, epoch } => {
			...
			PluginCommand::ClearConcealsInRangeForNamespace {
				...
				self.handle_clear_conceals_in_range_for_namespace(buffer_id, namespace, start, end, epoch);
			}

			PluginCommand::AddFold { buffer_id, start, end, placeholder } => {
			...
			// ==================== Soft Break Commands ====================
			PluginCommand::AddSoftBreak { buffer_id, namespace, position, indent, epoch } => {
				self.handle_add_soft_break(buffer_id, namespace, position, indent, epoch);
			}
			PluginCommand::ClearSoftBreakNamespace { buffer_id, namespace } => {
				self.handle_clear_soft_break_namespace(buffer_id, namespace);
			}
			PluginCommand::ClearSoftBreaksInRange { buffer_id, start, end, epoch } => {
			...
			PluginCommand::SetLineWrap { buffer_id, split_id, enabled } => {
			...
			PluginCommand::SetViewState { buffer_id, key, value } => {
			...
			PluginCommand::SetWindowState { plugin_name, key, value } => {
			...
			PluginCommand::SetLineIndicators { buffer_id, lines, namespace, symbol, color, priority } => {
				self.handle_set_line_indicators(buffer_id, lines, namespace, symbol, color, priority);
			}
			PluginCommand::ClearLineIndicators { buffer_id, namespace } => {
				self.handle_clear_line_indicators(buffer_id, namespace);
			}
			PluginCommand::SetFileExplorerDecorations { namespace, decorations } => {
				self.active_window_mut().handle_set_file_explorer_decorations(namespace, decorations);
			}
			PluginCommand::ClearFileExplorerDecorations { namespace } => {
				self.active_window_mut().handle_clear_file_explorer_decorations(&namespace);
			}
			PluginCommand::SetFileExplorerSlots { namespace, slots } => {
				self.active_window_mut().handle_set_file_explorer_slots(namespace, slots);
			}
			PluginCommand::ClearFileExplorerSlots { namespace } => {
				self.active_window_mut().handle_clear_file_explorer_slots(&namespace);
			}
			...
			PluginCommand::AddPluginConfigField { plugin_name, field_name, field_schema } => {
			...
			PluginCommand::RegisterGrammar { language, grammar_path, extensions } => {
			...
			PluginCommand::StartPromptWithInitial {
				...
				self.handle_start_prompt_with_initial(label, prompt_type, initial_value, floating_overlay);
			}
			PluginCommand::StartPromptAsync { label, initial_value, callback_id } => {
			...
			PluginCommand::CreateWindowWithTerminal {
				...
				self
					.handle_create_window_with_terminal(root, label, cwd, command, title, resume, request_id);
			}
			...
			// ==================== File watching ====================
			PluginCommand::WatchPath { path, recursive, request_id } => {
			...
			PluginCommand::RegisterStatusBarElement { plugin_name, token_name, title } => {
				self.handle_register_status_bar_element(plugin_name, token_name, title);
			}
			PluginCommand::SetStatusBarValue { buffer_id, key, value } => {
			...
			PluginCommand::CloseOtherBuffersInSplit { buffer_id, split_id } => {
			...
			PluginCommand::CloseBuffersToRightInSplit { buffer_id, split_id } => {
				self.handle_close_buffers_to_right_in_split(buffer_id, split_id);
			}
			PluginCommand::CloseBuffersToLeftInSplit { buffer_id, split_id } => {
			...
			PluginCommand::StartAnimationVirtualBuffer { id, buffer_id, kind } => {
			...
			PluginCommand::SpawnProcess { command, args, cwd, stdout_to, callback_id } => {
				self.handle_spawn_process(command, args, cwd, stdout_to, callback_id);
			}

			PluginCommand::SpawnHostProcess { command, args, cwd, callback_id } => {
			...
			PluginCommand::AttachRemoteAgent { payload, request_id } => {
			...
			PluginCommand::SpawnProcessWait { process_id, callback_id } => {
			...
			PluginCommand::HttpFetch { url, target_path, callback_id } => {
				self.handle_http_fetch(url, target_path, callback_id);
			}

			PluginCommand::SpawnBackgroundProcess { process_id, command, args, cwd, callback_id } => {
			...
			PluginCommand::GetLineStartPosition { buffer_id, line, request_id } => {
				self.handle_get_line_start_position(buffer_id, line, request_id);
			}
			PluginCommand::GetLineEndPosition { buffer_id, line, request_id } => {
				self.handle_get_line_end_position(buffer_id, line, request_id);
			}
			PluginCommand::GetBufferLineCount { buffer_id, request_id } => {
			...
			PluginCommand::RefreshBufferFromDisk { buffer_id, request_id } => {
				self.handle_refresh_buffer_from_disk(buffer_id, request_id);
			}
			PluginCommand::SetBufferGroupPanelBuffer { group_id, panel_name, buffer_id, request_id } => {
				self.handle_set_buffer_group_panel_buffer(group_id, panel_name, buffer_id, request_id);
			}
			PluginCommand::ScrollToLineCenter { split_id, buffer_id, line } => {
			...
			// ==================== LSP Helper Commands ====================
			PluginCommand::ShowActionPopup { popup_id, title, message, actions, buffer_id } => {
				self.handle_show_action_popup(popup_id, title, message, actions, buffer_id);
			}

			PluginCommand::SetLspMenuContributions { plugin_id, language, items } => {
			...
			PluginCommand::CreateScrollSyncGroup { group_id, left_split, right_split } => {
			...
			// ==================== Buffer Groups ====================
			PluginCommand::CreateBufferGroup { name, mode, layout_json, request_id } => {
				self.handle_create_buffer_group(name, mode, layout_json, request_id);
			}
			PluginCommand::SetPanelContent { group_id, panel_name, entries } => {
			...
			PluginCommand::FocusPanel { group_id, panel_name } => {
			...
			PluginCommand::ReplaceInBuffer {
				...
				self.handle_replace_in_buffer(file_path, buffer_id, matches, replacement, callback_id);
			}

			PluginCommand::MountWidgetPanel { plugin, panel_id, buffer_id, spec } => {
				let key = crate::widgets::PanelKey::new(plugin, panel_id);
				self.handle_mount_widget_panel(key, buffer_id, spec);
			}

			PluginCommand::UpdateWidgetPanel { plugin, panel_id, spec } => {
				let key = crate::widgets::PanelKey::new(plugin, panel_id);
			...
			PluginCommand::WidgetCommand { plugin, panel_id, action } => {
				let key = crate::widgets::PanelKey::new(plugin, panel_id);
				self.handle_widget_command(&key, action);
			}

			PluginCommand::WidgetMutate { plugin, panel_id, mutation } => {
				let key = crate::widgets::PanelKey::new(plugin, panel_id);
			...
			PluginCommand::MountFloatingWidget {
				...
				let key = crate::widgets::PanelKey::new(plugin, panel_id);
				self.handle_mount_floating_widget(key, spec, width_pct, height_pct, as_dock, focus_marker);
			}

			PluginCommand::UpdateFloatingWidget { plugin, panel_id, spec } => {
				let key = crate::widgets::PanelKey::new(plugin, panel_id);
			...
			PluginCommand::FloatingPanelControl { plugin, panel_id, op, arg } => {
				let key = crate::widgets::PanelKey::new(plugin, panel_id);
	...
	fn handle_watch_path(&mut self, path: std::path::PathBuf, recursive: bool, request_id: u64) {
		let result = if let Some(ref bridge) = self.async_bridge {
			...
			Err("watchPath: no async bridge — file watching is unavailable in this build".to_string())
		};
		self.last_watch_response_for_test = Some((request_id, result.clone()));
		self.send_plugin_response(dx_core::api::PluginResponse::WatchPathRegistered {
			request_id,
			result,
		});
	}
	...
	fn handle_set_env(&mut self, snippet: String, dir: Option<String>) {
		...
		if self.authority().workspace_trust.level() == TrustLevel::Trusted {
			self.authority().env_provider.set(snippet, dir.map(std::path::PathBuf::from));
	...
	fn refresh_active_window_lsp_for_env(&mut self) {
		...
		let running: Vec<String> =
			self.windows.get(&active_id).map(|w| w.lsp.running_servers()).unwrap_or_default();
		if running.is_empty() {
			return;
		}
		let file_path = self
	...
	fn handle_open_file_in_background_routed(
		...
		window_id: Option<dx_core::WindowId>,
	) {
	...
	fn handle_set_prompt_title(&mut self, title: Vec<dx_core::api::StyledText>) {
		if let Some(prompt) = &mut self.active_window_mut().prompt {
			prompt.title = title;
		}
	}

	fn handle_set_prompt_footer(&mut self, footer: Vec<dx_core::api::StyledText>) {
		if let Some(prompt) = &mut self.active_window_mut().prompt {
			prompt.footer = footer;
		}
	}

	fn handle_set_prompt_toolbar(&mut self, spec: Option<dx_core::api::WidgetSpec>) {
		if let Some(prompt) = &mut self.active_window_mut().prompt {
	...
	fn handle_create_window(&mut self, root: std::path::PathBuf, label: String) {
		if !root.is_absolute() {
			tracing::warn!("CreateWindow rejected: root must be absolute, got {:?}", root);
		} else {
			let _ = self.create_window_at(root, label);
		}
	}

	fn handle_preview_window_in_rect(&mut self, id: Option<dx_core::WindowId>) {
	...
	fn handle_set_status_bar_value(&mut self, buffer_id: u64, key: String, value: String) {
		if let Err(e) = self.set_status_bar_value(dx_core::BufferId(buffer_id as usize), &key, value) {
	...
	fn handle_cancel_animation(&mut self, id: u64) {
		self.active_window_mut().animations.cancel(crate::view::animation::AnimationId::from_raw(id));
	}
	...
	fn handle_set_review_diff_hunks(&mut self, hunks: Vec<dx_core::api::ReviewHunk>) {
		self.active_window_mut().review_hunks = hunks;
		tracing::debug!("Set {} review hunks", self.active_window_mut().review_hunks.len());
	}

	fn handle_composite_next_hunk(&mut self, buffer_id: dx_core::BufferId) {
		...
		self.active_window_mut().composite_next_hunk(split_id, buffer_id);
	}

	fn handle_composite_prev_hunk(&mut self, buffer_id: dx_core::BufferId) {
		let split_id = self.active_window().effective_active_pair().0;
		self.active_window_mut().composite_prev_hunk(split_id, buffer_id);
	}
	...
	fn route_vbuf_to_existing_dock(
		...
		mode: String,
		read_only: bool,
		entries: Vec<dx_core::text_property::TextPropertyEntry>,
		panel_id: Option<&str>,
		show_line_numbers: bool,
		show_cursors: bool,
		editing_disabled: bool,
		...
		let buffer_id = self.active_window_mut().create_virtual_buffer(name.clone(), mode, read_only);
		...
		self.active_window_mut().set_pane_buffer(dock_leaf, buffer_id);
		...
		if let Some(req_id) = request_id {
			let result = dx_core::api::VirtualBufferResult {
				buffer_id: buffer_id.0 as u64,
				split_id: Some(dock_leaf.0.0 as u64),
			};
			self.plugin_manager.read().unwrap().resolve_callback(
				dx_core::api::JsCallbackId::from(req_id),
				serde_json::to_string(&result).unwrap_or_default(),
			);
		}
		tracing::info!("Routed virtual buffer '{}' into existing utility dock {:?}", name, dock_leaf);
	}
	...
	fn update_existing_vbuf_panel(
		...
		if let Some(&split_id) = splits.first() {
			...
			self.active_window_mut().set_pane_buffer(split_id, existing_buffer_id);
	...
	fn handle_execute_actions(&mut self, actions: Vec<dx_core::api::ActionSpec>) {
		use crate::input::keybindings::Action;
		...
		const PLUGIN_FORBIDDEN_ACTIONS: &[&str] =
			&["workspace_trust_trust", "workspace_trust_restrict", "workspace_trust_block"];
		...
		for action_spec in actions {
			...
			if let Some(action) = Action::from_str(&action_spec.action, &action_spec.args) {
				...
				tracing::debug!("Executed action '{}' {} time(s)", action_spec.action, action_spec.count);
			} else {
	...
	fn handle_define_macro(&mut self, register: String, steps: Vec<dx_core::api::ActionSpec>) {
		use crate::input::keybindings::Action;
		...
		for spec in &steps {
			match Action::from_str(&spec.action, &spec.args) {
				...
				None => {
					tracing::warn!("defineMacro['{}']: unknown action '{}' skipped", key, spec.action);
				}
			}
		}
	...
	fn handle_get_buffer_text(
		...
		let callback_id = dx_core::api::JsCallbackId::from(request_id);
		match result {
			Ok(text) => {
	...
	fn resolve_buffer_id(&self, buffer_id: BufferId) -> BufferId {
		if buffer_id.0 == 0 { self.active_buffer() } else { buffer_id }
	}
	...
	fn handle_get_buffer_line_count(&mut self, buffer_id: BufferId, request_id: u64) {
			...
			.get_mut(&actual_buffer_id)
		{
			...
			Some(if content.is_empty() { 1 } else { newlines + usize::from(!content.ends_with('\n')) })
		} else {
			None
		};
	...
	fn handle_open_file_streaming(&mut self, path: std::path::PathBuf, request_id: u64) {
		...
		if !self.authority().filesystem.exists(&path) {
			if let Some(parent) = path.parent() {
				if !parent.as_os_str().is_empty() {
					if let Err(e) = std::fs::create_dir_all(parent) {
						tracing::warn!("openFileStreaming: failed to create parent dir {:?}: {}", parent, e);
			...
			if let Err(e) = std::fs::write(&path, b"") {
				tracing::warn!("openFileStreaming: failed to create empty file at {:?}: {}", path, e);
				self.resolve_json_callback::<Option<u64>>(request_id, None);
				return;
			}
		}
		...
		let buffer_id = match self.open_file_no_focus(&path) {
			Ok(id) => id,
			Err(e) => {
				tracing::warn!("openFileStreaming: open_file_no_focus failed for {:?}: {}", path, e);
				self.resolve_json_callback::<Option<u64>>(request_id, None);
				return;
			}
		};
			...
			.get_mut(&active_split)
		{
			...
			vs.open_buffers.retain(|t| !matches!(t, TabTarget::Buffer(b) if *b == buffer_id));
		}
	...
	fn handle_scroll_to_line_center(&mut self, split_id: SplitId, buffer_id: BufferId, line: usize) {
	...
	fn handle_scroll_buffer_to_line(&mut self, buffer_id: BufferId, line: usize) {
		...
		self.active_window_mut().scroll_buffer_to_line_in_splits(buffer_id, &target_leaves, line);
	}
	...
	fn handle_spawn_host_process(
		...
		if let (Some(runtime), Some(bridge)) = (&self.tokio_runtime, &self.async_bridge) {
			...
			let effective_cwd =
				cwd.or_else(|| std::env::current_dir().map(|p| p.to_string_lossy().to_string()).ok());
			...
			if let crate::services::workspace_trust::SpawnDecision::Deny(reason) =
				self.authority().workspace_trust.decide(&command, effective_cwd.as_deref())
			{
			...
			runtime.spawn(async move {
				...
				// dx-plugin-runtime process.rs pattern.
				...
				#[allow(clippy::let_underscore_must_use)]
				let _ =
					sender.send(AsyncMessage::PluginProcessOutput { process_id, stdout, stderr, exit_code });
			});
		} else {
			self
				.plugin_manager
			...
			let handle = runtime.spawn(async move {
					...
					.spawn()
				{
					...
					Err(e) => {
						let _ = sender.send(crate::services::async_bridge::AsyncMessage::Plugin(
							dx_core::api::PluginAsyncMessage::ProcessExit {
								process_id,
				...
				if let Some(stdout) = stdout {
					...
					tokio::spawn(async move {
						...
						while let Ok(Some(line)) = lines.next_line().await {
							let _ = sender.send(crate::services::async_bridge::AsyncMessage::Plugin(
								dx_core::api::PluginAsyncMessage::ProcessStdout {
									process_id: pid,
									data: line + "\n",
								},
							));
				...
				if let Some(stderr) = stderr {
					...
					tokio::spawn(async move {
						...
						while let Ok(Some(line)) = lines.next_line().await {
							let _ = sender.send(crate::services::async_bridge::AsyncMessage::Plugin(
								dx_core::api::PluginAsyncMessage::ProcessStderr {
									process_id: pid,
									data: line + "\n",
								},
							));
	...
	fn handle_create_virtual_buffer_with_content(
		...
		let buffer_id = if hidden_from_tabs {
			self.active_window_mut().create_virtual_buffer_detached(name.clone(), mode.clone(), read_only)
		} else {
			self.active_window_mut().create_virtual_buffer(name.clone(), mode.clone(), read_only)
		};
		tracing::info!(
		...
		match self.set_virtual_buffer_content(buffer_id, entries) {
			Ok(()) => {
				...
				if let Some(line) = initial_cursor_line {
					...
					self.active_window_mut().set_buffer_cursor_in_splits(buffer_id, byte, &splits);
				}
				...
				if let Some(req_id) = request_id {
					...
					let result =
						dx_core::api::VirtualBufferResult { buffer_id: buffer_id.0 as u64, split_id: None };
					self.plugin_manager.read().unwrap().resolve_callback(
						dx_core::api::JsCallbackId::from(req_id),
						serde_json::to_string(&result).unwrap_or_default(),
					);
	...
	fn handle_create_virtual_buffer_in_split(
		...
		let split_result = if split_role == Some(crate::view::split::SplitRole::UtilityDock) {
			self.split_manager_mut().split_root_positioned(split_dir, buffer_id, ratio, before)
		} else {
			self.split_manager_mut().split_active_positioned(split_dir, buffer_id, ratio, before)
		};
		...
		let created_split_id = match split_result {
			Ok(new_split_id) => {
				...
				view_state.apply_config_defaults(crate::view::split::ViewConfigDefaults {
					...
					line_wrap: line_wrap
						.unwrap_or_else(|| self.active_window().resolve_line_wrap_for_buffer(buffer_id)),
					wrap_indent: self.config.editor.wrap_indent,
					wrap_column: self.active_window().resolve_wrap_column_for_buffer(buffer_id),
				...
				if let Some(target_role) = split_role {
					...
					self.split_manager_mut().set_leaf_role(new_split_id, Some(target_role));
					tracing::info!("Tagged new dock leaf {:?} with role {:?}", new_split_id, target_role);
				}

				tracing::info!("Created {:?} split with virtual buffer {:?}", split_dir, buffer_id);
				Some(new_split_id)
			}
			Err(e) => {
	...
	fn handle_create_virtual_buffer_in_existing_split(
		...
		tracing::info!("Displayed virtual buffer {:?} in split {:?}", buffer_id, split_id);
	...
	fn handle_show_action_popup(
		...
		title: String,
		message: String,
		actions: Vec<dx_core::api::ActionPopupAction>,
		...
		popup_obj.resolver =
			crate::view::popup::PopupResolver::PluginAction { popup_id: popup_id.clone() };
	...
	fn handle_set_lsp_menu_contributions(
		...
		if items.is_empty() {
			...
			self.active_window_mut().lsp_menu_contributions.insert(key, items);
		}
	...
	fn handle_create_window_with_terminal(
		...
		if !root.is_absolute() {
			let msg = format!("createWindowWithTerminal: root must be absolute, got {:?}", root);
			tracing::warn!("{}", msg);
			self.plugin_manager.read().unwrap().reject_callback(callback_id, msg);
			return;
		}
		...
		match self.create_window_with_terminal(
			...
			Ok((window_id, terminal_id, buffer_id)) => {
				let api_result = dx_core::api::SessionWithTerminalResult {
				...
				self
					...
					.read()
					.unwrap()
					.resolve_callback(callback_id, serde_json::to_string(&api_result).unwrap_or_default());
			}
			Err(e) => {
	...
	fn handle_create_terminal(
		...
		focus: Option<bool>,
		persistent: bool,
		target_session_id: Option<dx_core::WindowId>,
		command: Option<Vec<String>>,
		title: Option<String>,
		request_id: u64,
	) {
		...
		// between windows.
		let target_id =
			target_session_id.filter(|id| self.windows.contains_key(id)).unwrap_or(self.active_window);
		...
		// state.
		let prev_active =
			if is_active_target { Some(self.active_window().active_buffer()) } else { None };

		let result = {
			let target =
				self.windows.get_mut(&target_id).expect("target window present (existence checked above)");
				...
				if is_active_target {
					...
					if prev_active != Some(new_active) {
						...
						self.plugin_manager.read().unwrap().run_hook(
							"buffer_activated",
							crate::services::plugins::hooks::HookArgs::BufferActivated { buffer_id: new_active },
						);
					}
				}
				let api_result = dx_core::api::TerminalResult {
					buffer_id: buffer_id.0 as u64,
					terminal_id: terminal_id.0 as u64,
					split_id: created_split_id.map(|s| s.0.0 as u64),
				};
				self.plugin_manager.read().unwrap().resolve_callback(
					dx_core::api::JsCallbackId::from(request_id),
	...
	fn handle_override_theme_colors(
		...
		let pairs =
			overrides.into_iter().map(|(k, [r, g, b])| (k, ratatui::style::Color::Rgb(r, g, b)));
	...
	fn handle_await_next_key(&mut self, callback_id: dx_core::api::JsCallbackId) {
		...
		if let Some(payload) = self.active_window_mut().pending_key_capture_buffer.pop_front() {
			let json = serde_json::to_string(&payload).unwrap_or_else(|_| "null".to_string());
			self.plugin_manager.read().unwrap().resolve_callback(callback_id, json);
		} else {
			self.active_window_mut().pending_next_key_callbacks.push_back(callback_id);
		}
	}
	...
	fn handle_spawn_process(
		...
		if let (Some(runtime), Some(bridge)) = (&self.tokio_runtime, &self.async_bridge) {
			...
			runtime.spawn(async move {
				#[allow(clippy::let_underscore_must_use)]
				let outcome = spawner.spawn_cancellable(command, args, effective_cwd, stdout_to, kill_rx).await;
				match outcome {
					Ok(result) => {
						#[allow(clippy::let_underscore_must_use)]
						let _ = sender.send(AsyncMessage::PluginProcessOutput {
	...
	fn handle_kill_host_process(&mut self, process_id: u64) {
		...
		if let Some(tx) = self.host_process_handles.remove(&process_id) {
			...
			tracing::debug!("KillHostProcess: unknown process_id={} (already exited?)", process_id);
		}
	}
	...
	pub(crate) fn reconnect_dormant_session_if_needed(&mut self, window_id: dx_core::WindowId) {
	...
	pub(crate) fn force_reconnect_remote_session(&mut self, window_id: dx_core::WindowId) {
		self.start_remote_reconnect(window_id);
	}
	...
	fn start_remote_reconnect(&mut self, window_id: dx_core::WindowId) {
		let Some(spec) = self.windows.get(&window_id).map(|w| w.authority_spec.clone()) else {
			return;
		};
		match spec {
	...
	fn handle_attach_remote_agent(&mut self, payload: serde_json::Value, request_id: u64) {
		...
		let spec = match serde_json::from_value::<crate::services::authority::RemoteAgentSpec>(payload)
		{
			Ok(spec) => spec,
			Err(e) => {
	...
	pub(crate) fn start_remote_connect(
		...
		reconnect_window: Option<dx_core::WindowId>,
		request_id: u64,
	) {
		...
		let session_spec = crate::services::authority::SessionAuthoritySpec::RemoteAgent(spec.clone());
		...
		match spec.transport {
			...
			RemoteTransportSpec::Ssh { user, host, port, identity_file, remote_path, extra_args } => {
	...
	fn handle_delay(&mut self, callback_id: dx_core::api::JsCallbackId, duration_ms: u64) {
		if let (Some(runtime), Some(bridge)) = (&self.tokio_runtime, &self.async_bridge) {
			let sender = bridge.sender();
			let callback_id_u64 = callback_id.as_u64();
			runtime.spawn(async move {
				...
				#[allow(clippy::let_underscore_must_use)]
				let _ = sender.send(crate::services::async_bridge::AsyncMessage::Plugin(
					dx_core::api::PluginAsyncMessage::DelayComplete { callback_id: callback_id_u64 },
				));
			});
		} else {
	...
	fn handle_create_virtual_buffer(&mut self, name: String, mode: String, read_only: bool) {
		let buffer_id =
			self.active_window_mut().create_virtual_buffer(name.clone(), mode.clone(), read_only);
		tracing::info!("Created virtual buffer '{}' with mode '{}' (id={:?})", name, mode, buffer_id);
	...
	fn handle_mount_widget_panel(
		...
		if let Some(state) =
			self.windows.get_mut(&self.active_window).and_then(|w| w.buffers.get_mut(&buffer_id))
		{
		...
		if let Err(e) = self.set_virtual_buffer_content(buffer_id, entries.clone()) {
			...
			tracing::debug!("Mounted widget panel {} into buffer {:?}", panel_key, buffer_id);
		}
	...
	fn handle_update_widget_panel(
		...
		let prev = match self.widget_registry.instance_states(panel_key) {
			Some(s) => s.clone(),
			None => {
				tracing::debug!("UpdateWidgetPanel for unknown panel {} ignored (not mounted)", panel_key);
				return;
			}
		};
		let prev_focus =
			self.widget_registry.focus_key(panel_key).map(|s| s.to_string()).unwrap_or_default();
		let buffer_id_for_width =
			self.widget_registry.buffer_and_spec(panel_key).map(|(b, _)| b).unwrap_or(BufferId(0));
	...
	fn handle_widget_mutate(
		&mut self,
		panel_key: &crate::widgets::PanelKey,
		mutation: dx_core::api::WidgetMutation,
	) {
		use dx_core::api::WidgetMutation;

		// Look up the panel; bail if unknown.
		if self.widget_registry.get(panel_key).is_none() {
			tracing::debug!("WidgetMutate for unknown panel {} ignored (not mounted)", panel_key);
			return;
		}

		match mutation {
			WidgetMutation::SetValue { widget_key, value, cursor_byte } => {
			...
			WidgetMutation::SetChecked { widget_key, checked } => {
				// Toggle checked lives in the spec (not instance
				// state). Walk the spec, find the Toggle by key,
				// mutate.
				if let Some(panel) = self.widget_registry.get_mut(panel_key) {
					crate::widgets::set_toggle_checked_in_spec(&mut panel.spec, &widget_key, checked);
				}
			}
			...
			WidgetMutation::SetNumber { widget_key, value } => {
				...
				if let Some(panel) = self.widget_registry.get_mut(panel_key) {
					let (min, max) = match crate::widgets::find_widget_by_key(&panel.spec, &widget_key) {
						Some(dx_core::api::WidgetSpec::Number { min, max, .. }) => (*min, *max),
						_ => (None, None),
					};
			...
			WidgetMutation::SetDropdown { widget_key, index } => {
				...
				if let Some(panel) = self.widget_registry.get_mut(panel_key) {
					let len = match crate::widgets::find_widget_by_key(&panel.spec, &widget_key) {
						Some(dx_core::api::WidgetSpec::Dropdown { options, .. }) => options.len(),
						_ => 0,
					};
					let clamped = if len == 0 { 0 } else { index.clamp(0, len as i32 - 1) };
					let open = matches!(
					...
					panel.instance_states.insert(
						widget_key.clone(),
						crate::widgets::WidgetInstanceState::Dropdown { selected_index: clamped, open },
					);
				}
			}
			WidgetMutation::SetDualIncluded { widget_key, included } => {
				...
				if let Some(panel) = self.widget_registry.get_mut(panel_key) {
					let sanitized = match crate::widgets::find_widget_by_key(&panel.spec, &widget_key) {
						Some(dx_core::api::WidgetSpec::DualList { options, .. }) => {
			...
			WidgetMutation::SetItems { widget_key, items, item_keys } => {
				// List items live in the spec.
				if let Some(panel) = self.widget_registry.get_mut(panel_key) {
					crate::widgets::set_list_items_in_spec(&mut panel.spec, &widget_key, items, item_keys);
				}
			}
			...
			WidgetMutation::SetCheckedKeys { widget_key, checked, keys } => {
			...
			WidgetMutation::AppendTreeNodes { widget_key, new_nodes, new_item_keys } => {
				if let Some(panel) = self.widget_registry.get_mut(panel_key) {
			...
			WidgetMutation::SetRawEntries { widget_key, entries } => {
				if let Some(panel) = self.widget_registry.get_mut(panel_key) {
	...
	fn handle_mount_floating_widget(
		...
		let slot = if as_dock { super::PanelSlot::Dock } else { super::PanelSlot::Floating };
		...
		*self.panel_opt_mut(slot) = Some(FloatingWidgetState {
		...
		tracing::debug!("Mounted floating widget panel {} ({}%x{}%)", panel_key, width_pct, height_pct);
	...
	fn handle_update_floating_widget(
		...
		let Some(slot) = self.slot_of_panel(panel_key) else {
			tracing::debug!("UpdateFloatingWidget for unknown / mismatched panel {} ignored", panel_key);
			return;
		};
		let prev = self.widget_registry.instance_states(panel_key).cloned().unwrap_or_default();
		let prev_focus =
			self.widget_registry.focus_key(panel_key).map(|s| s.to_string()).unwrap_or_default();
		let panel_width = self.floating_panel_inner_width(slot);
		...
		if self
			.widget_registry
			.update(panel_key, spec, out.hits, out.instance_states, out.focus_key, out.tabbable)
			.is_err()
		{
			tracing::debug!(
	...
	fn handle_unmount_floating_widget(&mut self, panel_key: &crate::widgets::PanelKey) {
		let Some(slot) = self.slot_of_panel(panel_key) else {
			tracing::debug!("UnmountFloatingWidget for unknown / mismatched panel {} ignored", panel_key);
			return;
		};
	...
	fn handle_set_context(&mut self, name: String, active: bool) {
		if active {
			self.active_window_mut().active_custom_contexts.insert(name.clone());
			tracing::debug!("Set custom context: {}", name);
		} else {
			self.active_window_mut().active_custom_contexts.remove(&name);
	...
	fn handle_disable_lsp_for_language(&mut self, language: String) {
		...
		if let Err(e) = self.save_config() {
			...
			self.active_window_mut().status_message =
				Some(format!("LSP disabled for {} (config save failed)", language));
		} else {
			self.active_window_mut().status_message = Some(format!("LSP disabled for {}", language));
		}
	...
	fn handle_mark_buffer_read_only(&mut self, path: std::path::PathBuf) {
		...
		match buffer_id {
			...
			None => {
				tracing::warn!("markFileReadOnly: no open buffer for path {}", path.display());
			}
		}
	}
		...
		match uri.parse::<lsp_types::Uri>() {
			Ok(parsed_uri) => {
				...
				if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
					...
					if restarted {
						self.active_window_mut().status_message =
							Some(format!("LSP root updated for {} (restarting server)", language));
					} else {
						self.active_window_mut().status_message =
			...
			Err(e) => {
				...
				self.active_window_mut().status_message = Some(format!("Invalid LSP root URI: {}", e));
			}
		}
	}
	...
	fn handle_create_scroll_sync_group(
		...
		let success = self.active_window_mut().scroll_sync_manager.create_group_with_id(
			group_id,
			left_split,
			right_split,
		);
		if success {
			...
			tracing::warn!("Failed to create scroll sync group {} (ID already exists)", group_id);
		}
	}
	...
	fn handle_set_scroll_sync_anchors(
		...
		let sync_anchors: Vec<SyncAnchor> = anchors
			.into_iter()
			.map(|(left_line, right_line)| SyncAnchor { left_line, right_line })
			.collect();
		self.active_window_mut().scroll_sync_manager.set_anchors(group_id, sync_anchors);
		tracing::debug!("Set {} anchors for scroll sync group {}", anchor_count, group_id);
	}
	...
	fn handle_remove_scroll_sync_group(
		...
		if self.active_window_mut().scroll_sync_manager.remove_group(group_id) {
			...
			Ok(result) => {
				if let Some(req_id) = request_id {
					...
					self
						...
						.read()
						.unwrap()
						.resolve_callback(dx_core::api::JsCallbackId::from(req_id), json);
				}
			}
			Err(e) => {
	...
	fn handle_send_terminal_input(
		...
		if let Some(handle) = self.active_window().terminal_manager.get(terminal_id) {
			...
			tracing::trace!("Plugin sent {} bytes to terminal {:?}", data.len(), terminal_id);
		} else {
			tracing::warn!("Plugin tried to send input to non-existent terminal {:?}", terminal_id);
		}
	}
	...
	/// stop flow.
	fn handle_signal_window(&mut self, id: dx_core::WindowId, signal: &str) {
		...
		for (entry, result) in results {
			match result {
				...
				Err(e) => {
					tracing::warn!("SignalWindow {:?}: pid {} ({}): {}", id, entry.leader_pid, entry.label, e)
				}
			}
		}
	}
...
mod tests {
	...
	async fn kill_via_oneshot_terminates_long_running_child() {
		...
		let result =
			timeout(Duration::from_secs(5), async { tokio::join!(stdout_fut, stderr_fut, wait_fut) })
				.await;
...
impl Window {
	...
	pub(crate) fn populate_plugin_state_snapshot(
		&mut self,
		snapshot: &mut dx_core::api::EditorStateSnapshot,
	) {
		use dx_core::api::{BufferInfo, CursorInfo, ViewportInfo};
		...
		if snapshot.last_grammar_gen != current_gen {
			snapshot.available_grammars = self
				...
				.into_iter()
				.map(|g| dx_core::api::GrammarInfoSnapshot {
					name: g.name,
		...
		snapshot.macros = {
			...
			macros
				...
				.into_iter()
				.filter_map(|key| {
					macros.get(key).map(|actions| dx_core::api::MacroSnapshot {
		...
		let (mgr_ref, vs_ref) =
			self.buffers.splits().expect("active window must have a populated split layout");
		...
		for (buffer_id, state) in &self.buffers {
			let is_virtual = self.buffer_metadata.get(buffer_id).map(|m| m.is_virtual()).unwrap_or(false);
			...
			let view_mode = active_vs_opt
				...
				.unwrap_or("source");
			let compose_width =
				active_vs_opt.and_then(|vs| vs.buffer_state(*buffer_id)).and_then(|bs| bs.compose_width);
			...
			let splits: Vec<dx_core::SplitId> =
				mgr_ref.splits_for_buffer(*buffer_id).into_iter().map(|leaf_id| leaf_id.0).collect();
			...
			let diff = {
				...
				BufferSavedDiff { equal: diff.equal, byte_ranges: diff.byte_ranges.clone() }
			};
			...
			let is_hidden = self.buffer_metadata.get(buffer_id).is_some_and(|m| m.hidden_from_tabs);
			...
			if !state.text_properties.is_empty() {
				snapshot.buffer_text_properties.insert(*buffer_id, state.text_properties.all().to_vec());
			}
		}
		...
		self
			...
			.with_all_mut(|buffers_mut, mgr, vs_map| {
				...
				if let Some(active_vs) = vs_map.get(&active_split_id) {
					...
					if let Some(range) = primary_selection {
						if let Some(active_state) = buffers_mut.get_mut(&active_buf_id) {
							snapshot.selected_text = Some(active_state.get_text_range(range.start, range.end));
						}
					}
				...
				for (leaf_id, vs) in vs_map.iter() {
					...
					snapshot.splits.push(dx_core::api::SplitSnapshot {
		...
		// Clean up entries for buffers that are no longer open
		{
			...
			snapshot.plugin_view_states.retain(|bid, _| open_bids.contains(bid));
			// Plugin markers live only in the snapshot; drop closed buffers'.
			snapshot.plugin_markers.retain(|bid, _| open_bids.contains(bid));
		}
