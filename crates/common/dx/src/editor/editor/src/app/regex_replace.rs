...
pub fn build_search_regex(
	...
	let pattern = if use_regex {
		if whole_word { format!(r"\b{}\b", query) } else { query.to_string() }
	} else {
		let escaped = regex::escape(query);
		if whole_word { format!(r"\b{}\b", escaped) } else { escaped }
	};
...
pub fn build_regex(
	...
	let pattern = if whole_word { format!(r"\b{}\b", search) } else { search.to_string() };
