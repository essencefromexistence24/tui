...
impl crate::app::Editor {
	...
	pub(crate) fn window_resources(&self) -> WindowResources {
		WindowResources {
			...
			fs_manager: std::sync::Arc::new(crate::services::fs::FsManager::new(std::sync::Arc::new(
			...
			tokio_runtime: self.tokio_runtime.clone(), // Handle is cheap to clone
	...
	pub fn create_window_with_terminal(
	...
	) -> Result<(WindowId, dx_core::TerminalId, dx_core::BufferId), String> {
		let id = WindowId(self.next_window_id);
		self.next_window_id += 1;
		...
		if self.authority().filesystem.remote_connection_info().is_none() {
		...
		let spawn_result = {
			let target = self.windows.get_mut(&id).expect("just-inserted window must be present");
		...
		if let Some(target) = self.windows.get_mut(&id) {
			...
			if let Some(resume_argv) = resume.filter(|a| !a.is_empty()) {
				target.terminal_resume_commands.insert(terminal_id, resume_argv);
			}
		}
		...
		if let Some(pid) =
			self.windows.get(&id).and_then(|w| w.terminal_manager.get(terminal_id)).and_then(|h| h.pid())
		{
		...
		if previous_id != id {
			self.plugin_manager.read().unwrap().run_hook(
				...
				HookArgs::ActiveWindowChanged { previous_id: Some(previous_id.0), active_id: id.0 },
			);
		}
		#[cfg(feature = "plugins")]
	...
	pub fn set_active_window_animated(&mut self, id: WindowId, from_edge: &str) {
		let animate =
			self.active_window != id && self.windows.contains_key(&id) && self.config().editor.animations;
	...
	fn cycle_active_window(&mut self, delta: isize) {
		...
		let override_ids: Option<Vec<WindowId>> = self
			...
			.as_ref()
			.map(|order| {
				order.iter().copied().filter(|id| self.windows.contains_key(id)).collect::<Vec<_>>()
			})
	...
	pub(crate) fn build_fresh_layout_if_needed(&mut self, id: WindowId) -> Option<FreshLayoutSeed> {
		if !self.windows.get(&id).is_some_and(|s| s.buffers.splits().is_none() || s.buffers.is_empty())
		{
			return None;
		}
	...
	pub(crate) fn promote_dormant_remote(
		...
		let workspace = if let Some(name) = self.session_name.clone() {
			crate::workspace::Workspace::load_session(&name, &root).ok().flatten()
		} else {
	...
	pub(crate) fn ensure_dormant_shell(&mut self, id: WindowId) {
		...
		let mut window =
			Window::new(id, descriptor.label.clone(), root, authority, self.window_resources());
		window.terminal_width = self.terminal_width;
		window.terminal_height = self.terminal_height;
		window.plugin_state = descriptor.plugin_state.clone();
