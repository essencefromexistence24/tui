...
impl Editor {
	...
	pub(super) fn start_set_line_ending_prompt(&mut self) {
		...
		let current_index =
			options.iter().position(|(le, _, _)| *le == current_line_ending).unwrap_or(0);
		...
		let suggestions: Vec<crate::input::commands::Suggestion> = options
			...
			.map(|(le, name, desc)| {
				...
				crate::input::commands::Suggestion {
					...
					description: if is_current { Some("current".to_string()) } else { None },
	...
	pub(super) fn start_set_encoding_prompt(&mut self) {
		...
		let current_index =
			Encoding::all().iter().position(|enc| *enc == current_encoding).unwrap_or(0);
	...
	pub(super) fn start_reload_with_encoding_prompt(&mut self) {
		...
		let is_modified =
			self.buffers().get(&self.active_buffer()).map(|s| s.buffer.is_modified()).unwrap_or(false);
	...
	pub(super) fn start_set_language_prompt(&mut self) {
		...
		for (lang_id, lang_config) in &self.config.languages {
			if let Some(entry) = self.grammar_registry.find_by_name(&lang_config.grammar) {
				config_key_by_display.entry(entry.display_name.clone()).or_insert_with(|| lang_id.clone());
			}
		}
		...
		let mut entries: Vec<LangEntry> = self
			...
			.map(|entry| {
				...
				LangEntry { display_name: entry.display_name.clone(), config_key, source }
			})
			.collect();

		// Sort alphabetically for easier navigation
		entries.sort_unstable_by_key(|a| a.display_name.to_lowercase());
	...
	pub(super) fn start_select_theme_prompt(&mut self) {
		...
		self.active_window_mut().prompt = Some(crate::view::prompt::Prompt::with_suggestions(
			...
			PromptType::SelectTheme { original_theme: current_theme_key.to_string() },
			suggestions,
		));
		...
		if !key_or_name.is_empty() {
			if let Some(theme) = self.theme_registry.get_cloned(key_or_name) {
				*self.theme.write().unwrap() = theme;
				...
				let resolved =
					self.theme_registry.resolve_key(key_or_name).unwrap_or_else(|| key_or_name.to_string());
				let to_persist = self.theme_registry.portable_form(&resolved).unwrap_or(resolved);
				...
				self.set_status_message(
					t!("view.theme_changed", theme = self.theme.read().unwrap().name.clone()).to_string(),
				);
			} else {
	...
	pub(super) fn save_theme_to_config(&mut self) {
		...
		changes.insert("/theme".to_string(), serde_json::Value::String(self.config.theme.0.clone()));
	...
	pub(super) fn start_select_keybinding_map_prompt(&mut self) {
		...
		let current_index = all_maps.iter().position(|name| *name == current_map).unwrap_or(0);
	...
	pub(super) fn start_select_cursor_style_prompt(&mut self) {
		...
		let current_index =
			CursorStyle::OPTIONS.iter().position(|s| *s == current_style.as_str()).unwrap_or(0);
	...
	pub(super) fn apply_cursor_style(&mut self, style_name: &str) {
		...
		if let Some(style) = CursorStyle::parse(style_name) {
			...
			self.set_status_message(t!("view.cursor_style_changed", style = description).to_string());
		}
	}
	...
	pub(super) fn start_select_locale_prompt(&mut self) {
		...
		let current_index =
			available_locales.iter().position(|name| *name == current_locale).unwrap_or(0);
		...
		let suggestions: Vec<crate::input::commands::Suggestion> = available_locales
			...
			.map(|locale_name| {
					...
					crate::i18n::locale_display_name(locale_name)
				{
					...
					// Unknown locale
					if is_current { "(current)".to_string() } else { String::new() }
				};
				crate::input::commands::Suggestion {
					description_spans: None,
