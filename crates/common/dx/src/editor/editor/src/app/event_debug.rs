...
impl RecordedEvent {
	fn new(event: KeyEvent) -> Self {
		...
		Self { event, description, normalized }
	}
}
...
fn format_key_event(event: &KeyEvent) -> String {
	...
	if parts.len() > 1 { parts.join("+") } else { key_str }
}
...
impl EventDebug {
	...
	pub fn new() -> Self {
		Self { history: Vec::new(), active: true }
	}
...
mod tests {
	...
	fn test_max_history() {
		...
		for i in 0..15 {
			debug.record_event(KeyEvent::new(KeyCode::Char((b'a' + i) as char), KeyModifiers::NONE));
		}
