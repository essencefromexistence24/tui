...
impl Editor {
	...
	pub fn set_session_name(&mut self, name: Option<String>) {
		if let Some(ref session_name) = name {
			...
			let scope = crate::services::recovery::RecoveryScope::Session { name: session_name.clone() };
	...
	pub fn take_pending_clipboard(&mut self) -> Option<crate::services::clipboard::PendingClipboard> {
	...
	pub fn request_restart(&mut self, new_working_dir: PathBuf) {
		tracing::info!("Restart requested with new working directory: {}", new_working_dir.display());
	...
	pub fn focus_gained(&mut self) {
		self
			...
			.read()
			.unwrap()
			.run_hook("focus_gained", crate::services::plugins::hooks::HookArgs::FocusGained {});
	}
	...
	pub fn handle_input_event(&mut self, event: crossterm::event::Event) -> anyhow::Result<bool> {
		...
		match event {
			Ev::Key(key_event) if key_event.kind == KeyEventKind::Press => {
				...
				self.active_window_mut().log_keystroke(&key_code, &modifiers);
				...
				if self.take_paste_slow_path_armed() { Ok(false) } else { Ok(true) }
			}
	...
	pub(crate) fn dock_cols(&self) -> u16 {
		...
		self.compute_dock_split(size).0.map(|dock| dock.width).unwrap_or(0)
	}
	...
	fn notify_layout_changed(&mut self) {
		...
		let fe_cols = {
			...
			if win.file_explorer_visible {
				win.file_explorer_width.to_cols(self.terminal_width.saturating_sub(dock_cols))
			} else {
				0
			}
		};
		let signature = (self.terminal_width, self.terminal_height, dock_cols, fe_cols);
		...
		self.plugin_manager.read().unwrap().run_hook(
			"resize",
			dx_core::hooks::HookArgs::Resize { width: self.terminal_width, height: self.terminal_height },
		);
		...
		for panel_key in [
			...
			self.floating_widget_panel.as_ref().map(|f| f.panel_key.clone()),
		]
		.into_iter()
		.flatten()
		{
...
impl crate::app::window::Window {
	...
	pub fn apply_layout(&mut self, width: u16, height: u16, dock_cols: u16) {
		...
		let visible: Vec<(crate::model::event::LeafId, crate::model::event::BufferId, u16)> =
