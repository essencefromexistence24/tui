...
use crate::workspace::{
	...
	SerializedViewMode, UnnamedBufferRef, WORKSPACE_VERSION, Workspace, WorkspaceConfigOverrides,
	WorkspaceError, WorkspaceHistories,
};
...
impl Editor {
	...
	pub fn apply_active_window_explorer_default(
		...
		self.active_window_mut().apply_fresh_session_explorer_default(opened_files, workspace_restored);
	}
	...
	pub fn apply_hot_exit_recovery(&mut self) -> anyhow::Result<usize> {
		...
		for (buffer_id, file_path) in buffer_files {
			let recovery_id = self.recovery_service.lock().unwrap().get_buffer_id(Some(&file_path));
				...
				match loaded {
					Ok(crate::services::recovery::RecoveryResult::Recovered { content, .. }) => {
							...
							.get_mut(&buffer_id)
						{
							...
							let current_text = current.as_ref().map(|b| String::from_utf8_lossy(b).into_owned());
					...
					Ok(crate::services::recovery::RecoveryResult::RecoveredChunks { chunks, .. }) => {
						let mut mutated = false;
						if let Some(state) = self
							.windows
							.get_mut(&self.active_window)
							...
							.get_mut(&buffer_id)
						{
							for chunk in chunks.into_iter().rev() {
								...
								if chunk.original_len > 0 {
									state.buffer.delete(chunk.offset..chunk.offset + chunk.original_len);
								}
					...
					Ok(crate::services::recovery::RecoveryResult::OriginalFileModified {
						...
						let name = original_path.file_name().unwrap_or_default().to_string_lossy();
					...
					Err(e) => {
						tracing::debug!("Failed to load hot exit recovery for {:?}: {}", file_path, e);
					}
				}
			}
		}
	...
	pub fn save_workspace_for(&mut self, id: dx_core::WindowId) -> Result<(), WorkspaceError> {
	...
	pub fn restore_workspace_for(&mut self, id: dx_core::WindowId) -> Result<bool, WorkspaceError> {
		...
		if populated {
			...
			let win = self.windows.get_mut(&id).expect("window present for restore");
			...
			let old = self.windows.remove(&id).expect("window present for restore");
	...
	pub fn save_all_windows_workspaces(&mut self) -> Result<(), WorkspaceError> {
		let targets: Vec<dx_core::WindowId> = self
            .windows
            .iter()
	...
	pub(crate) fn materialize_window(&mut self, id: dx_core::WindowId) {
	...
	pub fn materialize_all_windows(&mut self) {
		let pending: Vec<dx_core::WindowId> = self.materialize_pending.iter().copied().collect();
		for id in pending {
...
impl crate::app::window::Window {
	...
	fn restore_bookmarks_from_workspace(
		...
		for (key, bookmark) in bookmarks {
			...
			if let Some(buffer) = self.buffers.get(&buffer_id) {
				...
				self.bookmarks.set(*key, Bookmark { buffer_id, position: pos });
			}
		}
	}
	...
	fn log_restore_summary(&mut self, session_name: Option<&str>) {
		...
		let restored_count = self.buffers.count_where(|id, _| {
			self.buffer_metadata.get(&id).is_some_and(|m| !m.hidden_from_tabs && !m.is_virtual())
		});
		if restored_count == 0 {
			return;
		}
		let msg = match session_name.map(|n| format!("session '{}'", n)) {
			...
			None => format!("Restored {} buffer(s) from previous session", restored_count),
		};
	...
	fn restore_terminal_from_workspace(
		...
		let terminals_root = self.resources.dir_context.terminal_dir_for(self.root.as_path());
		...
		let _ = crate::app::terminal::terminal_backing_fs().create_dir_all(
			log_path.parent().or_else(|| backing_path.parent()).unwrap_or(&terminals_root),
		);
		...
		self.terminal_log_files.insert(predicted_id, log_path.clone());
		self.terminal_backing_files.insert(predicted_id, backing_path.clone());
		...
		let terminal_id = match self.terminal_manager.spawn(
			...
			Ok(id) => id,
			Err(e) => {
				tracing::warn!("Failed to restore terminal {}: {}", terminal.terminal_index, e);
				return None;
			}
		};
		...
		if terminal_id != predicted_id {
			self.terminal_log_files.insert(terminal_id, log_path.clone());
			self.terminal_backing_files.insert(terminal_id, backing_path.clone());
		...
		if let Some(resume) = terminal.agent_resume.as_ref() {
			if !resume.argv.is_empty() {
				self.terminal_resume_commands.insert(terminal_id, resume.argv.clone());
			}
		}
	...
	fn restore_split_node(
		...
		match node {
			SerializedSplitNode::Leaf { file_path, split_id, label, unnamed_recovery_id, role } => {
				...
				let buffer_id = file_path
					...
					.or_else(|| unnamed_recovery_id.as_ref().and_then(|id| unnamed_buffers.get(id).copied()))
			...
			SerializedSplitNode::Terminal { terminal_index, split_id, label, role } => {
				let buffer_id =
					terminal_buffers.get(terminal_index).copied().unwrap_or(self.active_buffer());
			...
			SerializedSplitNode::Split { direction, first, second, ratio, split_id } => {
				...
				let second_buffer_id =
					get_first_leaf_buffer(second, path_to_buffer, terminal_buffers, unnamed_buffers)
						.unwrap_or(self.active_buffer());
					...
					.split_active(split_direction, second_buffer_id, *ratio)
				{
					Ok(new_leaf_id) => {
						...
						view_state.apply_config_defaults(crate::view::split::ViewConfigDefaults {
							...
							highlight_current_line: self.resources.config.editor.highlight_current_line,
		...
		let active_buffer_id = self
			...
			.with_all_mut(|__buffers_mut, _mgr, vs_map| {
				...
				for (rel_path, file_state) in &split_state.file_states {
					...
					let max_pos = __buffers_mut.get(&buffer_id).map(|b| b.buffer.len()).unwrap_or(0);
					...
					buf_state.cursors.primary_mut().sticky_column =
						(file_state.cursor.sticky_column != 0).then_some(file_state.cursor.sticky_column);
					...
					if let Some(state) = __buffers_mut.get_mut(&buffer_id) {
						...
						let line = state.buffer.offset_to_position(cursor_pos).map(|p| p.line).unwrap_or(0);
	...
	fn restore_prompt_histories(&mut self, histories: &WorkspaceHistories) {
		...
		for item in &histories.search {
			self.prompt_histories.entry("search".to_string()).or_default().push(item.clone());
		}
		for item in &histories.replace {
			self.prompt_histories.entry("replace".to_string()).or_default().push(item.clone());
		}
		for item in &histories.goto_line {
			self.prompt_histories.entry("goto_line".to_string()).or_default().push(item.clone());
		}
	}
	...
	fn open_workspace_files(
		...
		tracing::debug!("Workspace has {} files to restore: {:?}", file_paths.len(), file_paths);
	...
	fn restore_external_files(
		...
		tracing::debug!("Restoring {} external files: {:?}", external_files.len(), external_files);
		for abs_path in external_files {
			...
			match self.open_file_internal(abs_path) {
				Ok(buffer_id) => {
					...
					tracing::debug!("Restored external file {:?} as buffer {:?}", abs_path, buffer_id);
				}
	...
	pub(crate) fn save_all_global_file_states(&self) {
		for (leaf_id, view_state) in
			self.buffers.splits().map(|(_, vs)| vs).expect("window must have a populated split layout")
		{
	...
	pub(crate) fn sync_terminal_backing_files(&self) {
		...
		let terminals_to_sync: Vec<_> = self
			...
			.filter_map(|terminal_id| {
				self.terminal_backing_files.get(&terminal_id).map(|path| (terminal_id, path.clone()))
			})
			.collect();
				...
				if let Ok(mut state) = handle.state.lock() {
						...
						crate::app::terminal::terminal_backing_fs().open_file_for_append(&backing_path)
					{
						...
						if let Err(e) = state.flush_new_scrollback(&mut writer) {
							tracing::warn!("Failed to flush terminal {:?} scrollback: {}", terminal_id, e);
						}
					}
						...
						crate::app::terminal::terminal_backing_fs().open_file_for_append(&backing_path)
					{
						...
						if let Err(e) = state.append_visible_screen(&mut writer) {
							tracing::warn!("Failed to sync terminal {:?} to backing file: {}", terminal_id, e);
						}
					}
				}
			}
	...
	pub(crate) fn create_unnamed_recovery_buffer(
		...
		state.margins.configure_for_line_numbers(self.resources.config.editor.line_numbers);
		state
			.buffer
			.set_default_line_ending(self.resources.config.editor.default_line_ending.to_line_ending());
		state.buffer.insert(0, text);
		state.buffer.set_modified(true);
		state.buffer.set_recovery_pending(false);
	...
	pub(crate) fn seed_initial_layout(&mut self) {
		...
		self.buffer_metadata.insert(buf, crate::app::types::BufferMetadata::new());
		self.event_logs.insert(buf, crate::model::event::EventLog::new());
	}
	...
	pub(crate) fn sync_lsp_after_recovery_replay(&mut self, buffer_id: BufferId) {
		let Some(text) = self.buffers.get(&buffer_id).and_then(|state| state.buffer.to_string()) else {
			return;
		};
		let full_change =
			lsp_types::TextDocumentContentChangeEvent { range: None, range_length: None, text };
	...
	fn restore_unnamed_buffers(
		...
		tracing::debug!("Restoring {} unnamed buffers from recovery", unnamed_buffers.len());
		for unnamed_ref in unnamed_buffers {
			let entries = match self.resources.recovery_service.lock().unwrap().list_recoverable() {
				Ok(e) => e,
				Err(e) => {
			...
			let Some(entry) = entries.iter().find(|e| e.id == unnamed_ref.recovery_id) else {
				tracing::debug!("Recovery file not found for unnamed buffer {}", unnamed_ref.recovery_id);
				continue;
			};
			let loaded = self.resources.recovery_service.lock().unwrap().load_recovery(entry);
			match loaded {
				Ok(crate::services::recovery::RecoveryResult::Recovered { content, .. }) => {
					let text = String::from_utf8_lossy(&content).into_owned();
	...
	fn restore_hot_exit_changes(&mut self, path_to_buffer: &HashMap<PathBuf, BufferId>) {
		...
		let entries =
			self.resources.recovery_service.lock().unwrap().list_recoverable().unwrap_or_default();
		if entries.is_empty() {
			return;
		}
		...
		for buffer_id in buffer_ids {
			let file_path =
				self.buffers.get(&buffer_id).and_then(|s| s.buffer.file_path().map(|p| p.to_path_buf()));
			let Some(file_path) = file_path else { continue };

			let recovery_id =
				self.resources.recovery_service.lock().unwrap().get_buffer_id(Some(&file_path));
			...
			match loaded {
				Ok(crate::services::recovery::RecoveryResult::Recovered { content, .. }) => {
					...
					if let Some(state) = self.buffers.get_mut(&buffer_id) {
						...
						if current_text.as_deref() != Some(&text) {
							...
							state.buffer.set_recovery_pending(false);
							mutated = true;
							tracing::info!("Restored unsaved changes for {:?} from hot exit recovery", file_path);
						}
					}
				...
				Ok(crate::services::recovery::RecoveryResult::OriginalFileModified {
					...
					tracing::warn!("{} changed on disk; unsaved changes not restored", name);
					self
						.set_status_message(format!("{} changed on disk; unsaved changes not restored", name));
				}
				Ok(_) => {} // Corrupted, NotFound — skip
				Err(e) => {
					tracing::debug!("Failed to load hot exit recovery for {:?}: {}", file_path, e);
	...
	pub(crate) fn apply_workspace_layout(
		...
		tracing::debug!("Applying workspace layout with {} split states", workspace.split_states.len());
	...
	pub(crate) fn capture_workspace(&self) -> Workspace {
		...
		for terminal_id in self.terminal_buffers.values().map(|tb| tb.terminal_id) {
			if seen.insert(terminal_id) {
				...
				let (cols, rows) =
					handle.map(|h| h.size()).unwrap_or((self.terminal_width, self.terminal_height));
				let cwd = handle.and_then(|h| h.cwd());
				let shell = handle
					.map(|h| h.shell().to_string())
					.unwrap_or_else(crate::services::terminal::detect_shell);
				let log_path = self.terminal_log_files.get(&terminal_id).cloned().unwrap_or_else(|| {
					let root = self.resources.dir_context.terminal_dir_for(&self.root);
					root.join(format!("dx-terminal-{}.log", terminal_id.0))
				});
				let backing_path =
					self.terminal_backing_files.get(&terminal_id).cloned().unwrap_or_else(|| {
						let root = self.resources.dir_context.terminal_dir_for(&self.root);
						root.join(format!("dx-terminal-{}.txt", terminal_id.0))
					});
		...
		let (mgr, view_states) =
			self.buffers.splits().expect("window must have a populated split layout");
		...
		let terminal_id_map: HashMap<BufferId, TerminalId> =
			self.terminal_buffers.iter().map(|(b, tb)| (*b, tb.terminal_id)).collect();
		...
		let histories = WorkspaceHistories {
			search: self.prompt_histories.get("search").map(|h| h.items().to_vec()).unwrap_or_default(),
			replace: self.prompt_histories.get("replace").map(|h| h.items().to_vec()).unwrap_or_default(),
		...
		let read_only_files: Vec<PathBuf> = self
			...
			.map(|p| p.strip_prefix(&self.root).map(|rel| rel.to_path_buf()).unwrap_or(p))
			.collect();
		...
		let unnamed_buffers: Vec<UnnamedBufferRef> = if self.resources.config.editor.hot_exit {
			self
				...
				.filter_map(|(buffer_id, meta)| {
					...
					Some(UnnamedBufferRef { recovery_id, display_name: meta.display_name.clone() })
				})
				.collect()
		} else {
			Vec::new()
...
fn get_first_leaf_buffer(
	...
	match node {
		SerializedSplitNode::Leaf { file_path, unnamed_recovery_id, .. } => file_path
			.as_ref()
			.and_then(|p| path_to_buffer.get(p).copied())
			.or_else(|| unnamed_recovery_id.as_ref().and_then(|id| unnamed_buffers.get(id).copied())),
...
fn serialize_split_node_pruned(
	...
	match node {
		...
		SplitNode::Leaf { buffer_id, split_id, role } => {
			...
			let unnamed_recovery_id =
				if file_path.is_none() { meta.and_then(|m| m.recovery_id.clone()) } else { None };
		...
		SplitNode::Split { direction, first, second, ratio, split_id, .. } => {
			let raw_split_id: SplitId = (*split_id).into();
...
fn serialize_split_view_state(
	...
	let active_file_index = active_tab_index
		...
		.and_then(|tab| match tab {
			SerializedTabRef::File(path) => Some(open_files.iter().position(|p| p == path).unwrap_or(0)),
			_ => None,
		})
		.unwrap_or(0);
	...
	let active_compose_width =
		active_buffer.and_then(|id| view_state.keyed_states.get(&id)).and_then(|bs| bs.compose_width);
...
fn serialize_bookmarks(
	...
	bookmarks
		...
		.filter_map(|(key, bookmark)| {
			buffer_metadata.get(&bookmark.buffer_id).and_then(|meta| meta.file_path()).and_then(
				|abs_path| {
					abs_path.strip_prefix(working_dir).ok().map(|rel_path| {
						(
							key,
							SerializedBookmark { file_path: rel_path.to_path_buf(), position: bookmark.position },
						)
					})
				},
			)
