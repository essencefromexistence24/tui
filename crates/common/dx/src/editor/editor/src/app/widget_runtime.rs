...
fn find_scrollable_widget_key(spec: &dx_core::api::WidgetSpec) -> Option<String> {
	use dx_core::api::WidgetSpec;
	match spec {
...
fn collect_visible_tree_indices(
	...
	for (i, node) in nodes.iter().enumerate() {
		...
		let is_open = if node.has_children { !key.is_empty() && expanded.contains(&key) } else { true };
...
pub(super) fn translate_plugin_animation_kind(
	...
	match kind {
		PluginAnimationKind::SlideIn { from, duration_ms, delay_ms } => AnimationKind::SlideIn {
			from: match from {
...
impl Editor {
	...
	pub(super) fn set_widget_selected_index_state(
		...
		let new_state = match panel.instance_states.get(widget_key) {
			Some(WidgetInstanceState::Tree { scroll_offset, expanded_keys, .. }) => {
			...
			other => {
				let (prev_scroll, prev_index, prev_item_height, prev_user_scrolled) = match other {
					Some(WidgetInstanceState::List {
						...
						item_height,
						user_scrolled,
					}) => (*scroll_offset, *selected_index, *item_height, *user_scrolled),
					_ => (0, -1, 1, false),
				};
		...
		panel.instance_states.insert(widget_key.to_string(), new_state);
	}
	...
	pub(crate) fn deliver_widget_hit(
		...
		if hit.widget_kind == "dual_list" && hit.event_type == "dual_focus" {
			let to_included = hit.payload.get("column").and_then(|v| v.as_str()) == Some("included");
		...
		if !handled_specially {
			...
			self.fire_widget_event(panel_key, event_widget_key, hit.event_type.to_string(), payload);
		}
	}
	...
	pub fn deliver_widget_hit_by_index(&mut self, plugin: &str, panel_id: u64, hit_index: usize) {
		...
		let hit = self.widget_registry.get(&panel_key).and_then(|p| p.hits.get(hit_index).cloned());
		if let Some(hit) = hit {
	...
	pub fn deliver_widget_hit_semantic(
		...
		let hit = {
			...
			identity(true)
				.or_else(|| if widget_key.is_empty() { None } else { identity(false) })
	...
	fn synthesize_list_hit(
		...
		let dx_core::api::WidgetSpec::List { items, item_specs, item_keys, .. } = spec else {
			return None;
		};
		let total = if item_specs.is_empty() { items.len() } else { item_specs.len() };
	...
	pub(crate) fn fire_widget_event(
		...
		pm.run_hook_for_plugin(
			&panel_key.plugin,
			"widget_event",
			dx_core::hooks::HookArgs::WidgetEvent {
	...
	pub(super) fn rerender_widget_panel(&mut self, panel_key: &crate::widgets::PanelKey) {
		...
		let (buffer_id, _is_floating, panel_width, out_pieces) = {
			...
			let focus_marker =
				panel_slot.and_then(|slot| self.panel(slot)).map(|f| f.focus_marker).unwrap_or(false);
	...
	pub(super) fn handle_widget_command(
		&mut self,
		panel_key: &crate::widgets::PanelKey,
		action: dx_core::api::WidgetAction,
	) {
		use dx_core::api::WidgetAction;
		match action {
	...
	fn handle_widget_key(&mut self, panel_key: &crate::widgets::PanelKey, key: &str) {
		...
		match key {
			...
			"Up" | "Down" => {
				...
				match widget {
					Some(dx_core::api::WidgetSpec::List { .. }) => {
						self.handle_widget_select_move(panel_key, delta);
					}
					Some(dx_core::api::WidgetSpec::Tree { .. }) => {
						self.handle_widget_tree_select_move(panel_key, delta);
					}
					Some(dx_core::api::WidgetSpec::Text { rows, .. }) if *rows > 1 => {
					...
					Some(dx_core::api::WidgetSpec::Number { .. }) => {
					...
					Some(dx_core::api::WidgetSpec::Dropdown { .. }) => {
					...
					Some(dx_core::api::WidgetSpec::DualList { .. }) => {
					...
					_ => {
						...
						let scrollable =
							self.widget_registry.get(panel_key).and_then(|p| find_scrollable_widget_key(&p.spec));
						if let Some(target_key) = scrollable {
							let target_kind = self
								.widget_registry
								.get(panel_key)
								.and_then(|p| crate::widgets::find_widget_by_key(&p.spec, &target_key).cloned());
							match target_kind {
								Some(dx_core::api::WidgetSpec::List { .. }) => {
									self.handle_widget_select_move_for_key(panel_key, &target_key, delta);
								}
								Some(dx_core::api::WidgetSpec::Tree { .. }) => {
									self.handle_widget_tree_select_move_for_key(panel_key, &target_key, delta);
								}
								_ => {}
							}
						}
			...
			"PageUp" | "PageDown" => {
				...
				if let Some(dx_core::api::WidgetSpec::DualList { .. }) = widget {
				...
				let page = match widget {
					Some(dx_core::api::WidgetSpec::List { visible_rows, .. })
					| Some(dx_core::api::WidgetSpec::Tree { visible_rows, .. }) => {
			...
			"Left" | "Right" => match widget {
				Some(dx_core::api::WidgetSpec::Text { .. }) => {
					self.handle_widget_text_key(panel_key, key);
				}
				Some(dx_core::api::WidgetSpec::Tree { .. }) => {
				...
				Some(dx_core::api::WidgetSpec::DualList { .. }) => {
					...
					self.handle_widget_dual(panel_key, &focus_key, DualOp::SwitchColumn(key == "Right"));
				}
				_ => {}
			},
			"Backspace" | "Delete" | "Home" | "End" => {
				if let Some(dx_core::api::WidgetSpec::Text { .. }) = widget {
					self.handle_widget_text_key(panel_key, key);
				}
			}
			"Enter" => match widget {
				Some(dx_core::api::WidgetSpec::Button { .. })
				| Some(dx_core::api::WidgetSpec::Toggle { .. }) => {
					self.handle_widget_activate(panel_key);
				}
				Some(dx_core::api::WidgetSpec::DualList { .. }) => {
				...
				Some(dx_core::api::WidgetSpec::Text { rows, .. }) => {
					if *rows > 1 {
	...
	fn handle_widget_focus_advance(&mut self, panel_key: &crate::widgets::PanelKey, delta: i32) {
		...
		let cur_idx = panel.tabbable.iter().position(|k| k == &panel.focus_key).unwrap_or(0) as i32;
	...
	pub(crate) fn set_panel_focus_and_notify(
		...
		self.widget_registry.set_focus_key(panel_key, new_key.clone());
	...
	fn handle_widget_activate(&mut self, panel_key: &crate::widgets::PanelKey) {
		...
		let (event_type, payload) = match widget {
			...
			// in that race.
			Some(dx_core::api::WidgetSpec::Button { disabled: true, .. }) => return,
			Some(dx_core::api::WidgetSpec::Button { .. }) => ("activate", serde_json::json!({})),
			Some(dx_core::api::WidgetSpec::Toggle { checked, .. }) => {
	...
	fn move_focused_text_completion_index(
		...
		let spec_visible_rows = match crate::widgets::find_widget_by_key(&panel.spec, &focus_key) {
			Some(dx_core::api::WidgetSpec::Text { completions_visible_rows, .. }) => {
				*completions_visible_rows
			}
			_ => 0,
		};
		let visible = if spec_visible_rows == 0 { 5u32 } else { spec_visible_rows };
	...
	fn fire_list_activate(&mut self, panel_key: &crate::widgets::PanelKey, focus_key: &str) {
		...
		let (spec_sel, item_keys) = match widget {
			Some(dx_core::api::WidgetSpec::List { selected_index, item_keys, .. }) => {
				(*selected_index, item_keys.clone())
			}
			_ => return,
		};
		let sel = match panel.instance_states.get(focus_key) {
			Some(crate::widgets::WidgetInstanceState::List { selected_index, .. }) => *selected_index,
			_ => spec_sel,
		};
		if sel < 0 {
			return;
	...
	pub(super) fn handle_widget_number_adjust(
		...
		let (spec_value, min, max, step) =
			match crate::widgets::find_widget_by_key(&panel.spec, widget_key) {
				Some(dx_core::api::WidgetSpec::Number { value, min, max, step, .. }) => {
					(*value, *min, *max, *step)
				}
				_ => return,
			};
	...
	pub(super) fn handle_widget_dropdown_cycle(
		...
		let (options, spec_sel) = match crate::widgets::find_widget_by_key(&panel.spec, widget_key) {
			Some(dx_core::api::WidgetSpec::Dropdown { options, selected_index, .. }) => {
		...
		let (cur, open) = match panel.instance_states.get(widget_key) {
			Some(crate::widgets::WidgetInstanceState::Dropdown { selected_index, open }) => {
				(*selected_index, *open)
			}
			_ => (spec_sel, false),
		};
	...
	pub(super) fn handle_widget_dropdown_set(
		...
		if let Some(panel_mut) = self.widget_registry.get_mut(panel_key) {
			panel_mut.instance_states.insert(
				widget_key.to_string(),
				crate::widgets::WidgetInstanceState::Dropdown { selected_index: new_sel, open },
			);
		}
		self.rerender_widget_panel(panel_key);
		if changed {
	...
	pub(super) fn set_dropdown_open(
		...
		let spec_sel = match crate::widgets::find_widget_by_key(&panel.spec, widget_key) {
			Some(dx_core::api::WidgetSpec::Dropdown { selected_index, .. }) => *selected_index,
			_ => return,
		};
		let cur = match panel.instance_states.get(widget_key) {
		...
		if let Some(panel_mut) = self.widget_registry.get_mut(panel_key) {
			panel_mut.instance_states.insert(
				widget_key.to_string(),
				crate::widgets::WidgetInstanceState::Dropdown { selected_index: cur, open },
			);
		}
		self.rerender_widget_panel(panel_key);
	}
	...
	pub(super) fn handle_widget_dual(
		...
		let (options, excluded, spec_included) =
			match crate::widgets::find_widget_by_key(&panel.spec, widget_key) {
				Some(dx_core::api::WidgetSpec::DualList { options, excluded, included, .. }) => {
					(options.clone(), excluded.clone(), included.clone())
				}
				_ => return,
			};
		// Load current instance state (or seed from spec).
		let (mut included, mut active_included, mut avail_cur, mut incl_cur) = match panel
			.instance_states
			.get(widget_key)
		{
			...
			DualOp::MoveAcross => {
				if active_included {
					...
					if avail_cur < available.len() {
						...
						available = crate::widgets::dual_available_values(&options, &included, &excluded);
						avail_cur = clamp(avail_cur, available.len());
					}
				}
			}
	...
	fn handle_widget_dual_click(
		...
		let (mut included, _active, mut avail_cur, mut incl_cur) = match panel
			.instance_states
			.get(widget_key)
		{
			Some(crate::widgets::WidgetInstanceState::DualList {
		...
		let (options, excluded) = match crate::widgets::find_widget_by_key(&panel.spec, widget_key) {
			Some(dx_core::api::WidgetSpec::DualList { options, excluded, .. }) => {
	...
	pub(super) fn set_widget_list_selected_index(
		...
		if let Some(panel) = self.widget_registry.get_mut(panel_key) {
			let (prev_scroll, prev_item_height) = match panel.instance_states.get(widget_key) {
				Some(crate::widgets::WidgetInstanceState::List { scroll_offset, item_height, .. }) => {
	...
	fn handle_widget_select_move_for_key(
		...
		let (spec_sel, total, item_keys) = match widget {
			Some(dx_core::api::WidgetSpec::List {
				selected_index, items, item_specs, item_keys, ..
			}) => {
	...
	fn handle_widget_tree_select_move_for_key(
		...
		let (spec_sel, nodes, item_keys) = match widget {
			Some(dx_core::api::WidgetSpec::Tree { selected_index, nodes, item_keys, .. }) => {
		...
		let cur_pos = if cur_sel < 0 {
			if delta > 0 { -1 } else { visible_indices.len() as i32 }
		} else {
			visible_indices.iter().position(|&v| v as i32 == cur_sel).map(|p| p as i32).unwrap_or(-1)
		};
	...
	fn scroll_focused_text_completions(&mut self, panel_key: &crate::widgets::PanelKey, delta: i32) {
		let panel = match self.widget_registry.get(panel_key) {
			Some(p) => p,
			None => return,
		};
	...
	fn handle_widget_tree_wheel(
		...
		let (visible_rows, nodes, item_keys) = match widget {
			Some(dx_core::api::WidgetSpec::Tree { visible_rows, nodes, item_keys, .. }) => {
		...
		let cur_pos: Option<u32> = if cur_sel >= 0 {
			visible_indices.iter().position(|&v| v as i32 == cur_sel).map(|p| p as u32)
		} else {
			None
		};
	...
	fn handle_widget_list_wheel(
		...
		let (visible_rows, total) = match widget {
			Some(dx_core::api::WidgetSpec::List { visible_rows, items, item_specs, .. }) => {
				let total = if item_specs.is_empty() { items.len() } else { item_specs.len() };
	...
	fn fire_tree_toggle_if_checkable(
		...
		let (spec_sel, nodes, item_keys, checkable) = match widget {
			Some(dx_core::api::WidgetSpec::Tree {
				selected_index, nodes, item_keys, checkable, ..
		...
		let sel = match panel.instance_states.get(focus_key) {
			Some(crate::widgets::WidgetInstanceState::Tree { selected_index, .. }) => *selected_index,
			_ => spec_sel,
		};
		if sel < 0 {
			return false;
	...
	fn fire_tree_activate(&mut self, panel_key: &crate::widgets::PanelKey, focus_key: &str) {
		...
		let widget = crate::widgets::find_widget_by_key(&panel.spec, focus_key);
		let (spec_sel, item_keys) = match widget {
			Some(dx_core::api::WidgetSpec::Tree { selected_index, item_keys, .. }) => {
				(*selected_index, item_keys.clone())
			}
			_ => return,
		};
	...
	pub(super) fn panel_focused_widget_is_text(&self, panel_key: &crate::widgets::PanelKey) -> bool {
		let Some(panel) = self.widget_registry.get(panel_key) else {
			return false;
		};
		if panel.focus_key.is_empty() {
	...
	pub(super) fn focused_widget_selected_text(
		...
		match panel.instance_states.get(&panel.focus_key) {
			Some(crate::widgets::WidgetInstanceState::Text { editor, .. }) => editor.selected_text(),
			_ => None,
		}
	}
	...
	pub(super) fn handle_widget_select_all(&mut self, panel_key: &crate::widgets::PanelKey) -> bool {
	...
	fn ensure_focused_text_seeded(
		...
		let (value, cursor_byte, multiline) = match widget {
			Some(dx_core::api::WidgetSpec::Text { value, cursor_byte, rows, .. }) => {
		...
		let seed = if cursor_byte < 0 { value.len() } else { (cursor_byte as usize).min(value.len()) };
	...
	pub(super) fn floating_panel_inner_width(&self, slot: super::PanelSlot) -> u32 {
		...
		let pct = self.panel(slot).map(|f| f.width_pct.clamp(1, 100) as u32).unwrap_or(80);
	...
	pub(super) fn blur_floating_panel(&mut self, slot: super::PanelSlot) {
		...
		let widget_key =
			self.widget_registry.get(&panel_key).map(|p| p.focus_key.clone()).unwrap_or_default();
		self.fire_widget_event(&panel_key, widget_key, "blur".to_string(), serde_json::json!({}));
	}
	...
	pub(super) fn handle_close_split(&mut self, split_id: SplitId) {
			...
			.close_split(leaf_id)
		{
			Ok(()) => {
				...
				self.active_window_mut().forget_split_terminal_modes(leaf_id);
	...
	pub(super) fn flush_pending_grammars(&mut self) {
		...
		if self.needs_full_grammar_build {
			...
			for crate::primitives::grammar::GrammarSpec { language, extensions, .. } in &additional {
				let lang_config = self.config_mut().languages.entry(language.clone()).or_default();
		...
		self.pending_grammars.retain(|g| {
			...
			let all_mapped = !g.extensions.is_empty()
				&& g.extensions.iter().all(|ext| self.grammar_registry.find_by_extension(ext).is_some());
			if all_mapped {
				tracing::debug!(
		...
		if self.pending_grammars.is_empty() {
			tracing::info!("All pending grammars already loaded, resolving callbacks without rebuild");
