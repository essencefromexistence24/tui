...
impl Editor {
	...
	pub fn open_stdin_buffer(
		...
		// Clear modified flag - content is "dx" from stdin (vim behavior)
		...
		self.stdin_stream.start(temp_path.to_path_buf(), buffer_id, file_size, thread_handle);
	...
	pub fn poll_stdin_streaming(&mut self) -> bool {
		...
		if self.stdin_stream.record_growth(current_size) {
			if let Some(editor_state) = self
				...
				.get_mut(&buffer_id)
			{
				editor_state.buffer.extend_streaming(&temp_path, current_size);
			}
		...
		if let Some(outcome) = self.stdin_stream.take_finished_thread_outcome() {
			match outcome {
				...
				ThreadOutcome::Panic => {
					...
					self.active_window_mut().status_message = Some(t!("stdin.read_error_panic").to_string());
				}
			}
	...
	pub fn complete_stdin_streaming(&mut self) {
		...
		self.active_window_mut().status_message =
			Some(t!("stdin.read_complete", bytes = self.stdin_stream.last_known_size()).to_string());
	}
	...
	pub fn set_virtual_buffer_content(
		...
		self.active_window_mut().set_virtual_buffer_content(buffer_id, entries)
	}
}
...
impl crate::app::window::Window {
	...
	pub fn create_virtual_buffer(&mut self, name: String, mode: String, read_only: bool) -> BufferId {
			...
			.get_mut(&active_split)
		{
			...
			let mut view_state = SplitViewState::with_buffer(terminal_width, terminal_height, buffer_id);
	...
	pub fn set_virtual_buffer_content(
		...
		let state = self.buffers.get_mut(&buffer_id).ok_or_else(|| "Buffer not found".to_string())?;
		...
		// N entries (a big git-show diff can be ~500k overlays).
		{
			...
			for co in collected_overlays {
				...
				let mut overlay =
					Overlay::with_namespace(&mut state.marker_list, co.range, face, inline_ns.clone());
