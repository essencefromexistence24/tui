...
impl Editor {
	...
	pub fn expanded_menu_definitions(&self) -> Vec<dx_core::menu::Menu> {
	...
	pub fn all_menus_expanded(&self) -> Vec<dx_core::menu::Menu> {
		...
		let mut all: Vec<dx_core::menu::Menu> = match self.expanded_menus_cache.get() {
	...
	pub fn update_menu_context(&mut self) {
		...
		let can_copy = has_selection
			...
			|| self.file_explorer().as_ref().map(|fe| fe.get_selected().is_some()).unwrap_or(false);
...
impl crate::app::window::Window {
	...
	pub(crate) fn is_line_numbers_visible(&self) -> bool {
		...
		vs.get(&mgr.active_split()).map(|vs| vs.show_line_numbers).unwrap_or(true)
	}
	...
	pub(crate) fn is_line_wrap_enabled(&self) -> bool {
		let (mgr, vs) =
			self.buffers.splits().expect("active window must have a populated split layout");
		vs.get(&mgr.active_split()).map(|vs| vs.viewport.line_wrap_enabled).unwrap_or(false)
	}
	...
	pub(crate) fn is_file_explorer_showing_hidden(&self) -> bool {
		self.file_explorer.as_ref().map(|fe| fe.ignore_patterns().show_hidden()).unwrap_or(false)
	}

	/// Check if the file explorer is showing gitignored files.
	pub(crate) fn is_file_explorer_showing_gitignored(&self) -> bool {
		self.file_explorer.as_ref().map(|fe| fe.ignore_patterns().show_gitignored()).unwrap_or(false)
	}
	...
	pub(crate) fn has_same_buffer_splits(&self) -> bool {
		...
		if let Some(buf_id) = active_buf_id {
			vs.keys().any(|&s| s != active_split && mgr.buffer_for_split(s) == Some(buf_id))
		} else {
			false
		}
	}
