...
use crate::input::multi_cursor::{
	AddCursorResult, add_cursor_above, add_cursor_at_next_match, add_cursor_below,
...
impl Editor {
	...
	pub fn copy_selection(&mut self) {
		...
		let has_block_selection =
			self.active_cursors().iter().any(|(_, cursor)| cursor.has_block_selection());
		...
		let has_selection =
			self.active_cursors().iter().any(|(_, cursor)| cursor.selection_range().is_some());
		...
		if has_selection {
			...
			let positions: Vec<_> = self.active_cursors().iter().map(|(_, c)| c.position).collect();
			let state = self.active_state_mut();
			...
			if !text.is_empty() {
				self.clipboard.copy(text);
				self.active_window_mut().status_message = Some(t!("clipboard.copied_line").to_string());
			}
		}
	}
	...
	pub(crate) fn copy_block_selection_text(&mut self) -> String {
		...
		let block_virtual = self.active_state().buffer_settings.virtual_space.block_beyond_eol();
		...
		for (block_anchor, anchor_byte, cursor_byte, sticky) in block_infos {
			...
			let state = self.active_state_mut();
			let mut iter = state.buffer.line_iterator(start_byte, estimated_line_length);
			...
			loop {
				...
				if let Some((_offset, line_content)) = iter.next_line() {
					...
					let mut extracted: String =
						chars.iter().skip(min_col).take(max_col.saturating_sub(min_col)).collect();
	...
	pub fn copy_selection_with_theme(&mut self, theme_name: &str) {
		...
		if !has_selection {
			self.active_window_mut().status_message = Some(t!("clipboard.no_selection").to_string());
			return;
		}
		...
		let theme = match self.theme_registry.get_cloned(theme_name) {
			Some(t) => t,
			None => {
				self.active_window_mut().status_message = Some(format!("Theme '{}' not found", theme_name));
				return;
			}
		};
		...
		if self.clipboard.copy_html(&html, &text) {
			...
			self.clipboard.copy(text);
			self.active_window_mut().status_message = Some(t!("clipboard.copied_plain").to_string());
		}
	}
	...
	fn start_copy_with_formatting_prompt(&mut self) {
		...
		let current_index = available_themes
			...
			.or_else(|| {
				...
				available_themes
					.iter()
					.position(|info| crate::view::theme::normalize_theme_name(&info.name) == normalized)
			})
			.unwrap_or(0);
		...
		let suggestions: Vec<crate::input::commands::Suggestion> = available_themes
			...
			.map(|info| {
				...
				let description =
					if is_current { Some(format!("{} (current)", info.key)) } else { Some(info.key.clone()) };
	...
	pub fn cut_selection(&mut self) {
		...
		if has_selection {
			...
			if events.len() > 1 {
				// Use optimized bulk edit for multi-cursor cut
				if let Some(bulk_edit) = self.apply_events_as_bulk_edit(events, "Cut line".to_string()) {
					self.active_event_log_mut().append(bulk_edit);
				}
			} else if let Some(event) = events.into_iter().next() {
				self.log_and_apply_event(&event);
			...
			if !deletions.is_empty() {
				self.active_window_mut().status_message = Some(t!("clipboard.cut_line").to_string());
			}
		}
	}
	...
	pub fn paste(&mut self) {
		...
		if self.paste_pending.len() >= MAX_PENDING_PASTES {
			tracing::warn!("MAX_PENDING_PASTES ({}) reached, ignoring Ctrl+V", MAX_PENDING_PASTES);
			return;
		}
		...
		// paste falls back to Dx's own internal clipboard — restoring
		// the in-editor copy/paste round-trip that the pre-async
		// synchronous path provided (regression from #2155).
		let reader =
			self.system_clipboard_reader.unwrap_or(crate::services::clipboard::read_system_clipboard);
				...
				match inline_tx.send(text.clone()) {
					...
					Err(_) => {
						...
						if let Err(e) = bridge_sender
							.send(AsyncMessage::ClipboardPasteResult { request_id: thread_request_id, text })
						{
		...
		if !cursor_selections.is_empty() {
			...
			delete_events.sort_by(|a, b| {
				let pa = if let Event::Delete { range, .. } = a { range.start } else { 0 };
				let pb = if let Event::Delete { range, .. } = b { range.start } else { 0 };
				pb.cmp(&pa)
			});
		...
		let mut positions: Vec<usize> = self.active_cursors().iter().map(|(_, c)| c.position).collect();
		...
		let anchors: Vec<PasteAnchor> = {
			...
			positions
				...
				.map(|&pos| {
					...
					PasteAnchor { virtual_text_id: id }
				})
				.collect()
		};
	...
	pub(crate) fn resolve_pending_paste(&mut self, request_id: u64, text: Option<String>) {
		...
		let mut anchor_positions: Vec<(usize, usize)> = {
			let state = self.buffers().get(&pending.buffer_id).expect("checked above");
			pending
				.anchors
				.iter()
				.enumerate()
		...
		if let Some(raw_text) = text.filter(|s| !s.is_empty()) {
			...
			if let Err(e) =
				self.apply_events_to_buffer_as_bulk_edit(pending.buffer_id, events, "Paste".to_string())
			{
			...
			tracing::debug!("paste request {} resolved with no text — removing anchors", request_id);
		}
		...
		for anchor in pending.anchors {
			state.virtual_texts.remove(&mut state.marker_list, anchor.virtual_text_id);
		}
	}
	...
	pub fn cancel_pending_pastes_for_buffer(&mut self, buffer_id: BufferId) {
		self.paste_pending.retain(|_, pending| pending.buffer_id != buffer_id);
		if self.paste_pending.is_empty() {
			self.paste_render_suppress_until = None;
		}
	}
	...
	pub(crate) fn paste_bracketed_into_focused_panel(&mut self, text: &str) -> bool {
		...
		let slot = if self.floating_widget_panel.as_ref().is_some_and(|f| f.focused) {
	...
	pub fn paste_text(&mut self, paste_text: String) {
		...
		if self.active_window().focused_terminal_live() {
			self.active_window_mut().send_terminal_input(normalized.as_bytes());
			return;
		}
		...
		let cursor_data_with_text: Vec<_> = {
			...
			cursor_data
				...
				.map(|(cursor_id, selection, insert_position, virtual_cols)| {
					...
					(cursor_id, selection, insert_position, deleted_text, virtual_cols)
				})
				.collect()
		};
			...
			cursor_data_with_text.into_iter().enumerate()
		{
			if let (Some(range), Some(text)) = (selection, deleted_text) {
				events.push(Event::Delete { range, deleted_text: text, cursor_id });
			}
	...
	pub fn copy_buffer_path(&mut self, buffer_id: crate::model::event::BufferId, relative: bool) {
		...
		let Some(path) = path else {
			self.active_window_mut().status_message = Some(t!("clipboard.no_file_path").to_string());
			return;
		};

		let path_str = if relative {
			path.strip_prefix(self.working_dir()).unwrap_or(&path).to_string_lossy().into_owned()
		} else {
	...
	pub fn add_cursor_at_next_match(&mut self) {
		...
		match add_cursor_at_next_match(state, &cursors) {
			AddCursorResult::Success { cursor, total_cursors } => {
				...
				let event =
					Event::AddCursor { cursor_id: next_id, position: cursor.position, anchor: cursor.anchor };
			...
			AddCursorResult::WordSelected { word_start, word_end } => {
	...
	pub fn add_cursors_to_line_ends(&mut self) {
		...
		// surviving cursor.
		let mut existing: Vec<(CursorId, Cursor)> = cursors.iter().map(|(id, c)| (id, *c)).collect();
		...
		for &(cursor_id, cur) in existing.iter().skip(reuse) {
			events.push(Event::RemoveCursor { cursor_id, position: cur.position, anchor: cur.anchor });
		}
		...
		let next_free_id = cursors.iter().map(|(id, _)| id.0).max().map(|m| m + 1).unwrap_or(0);
		for (i, &pos) in positions.iter().enumerate().skip(reuse) {
			let new_id = CursorId(next_free_id + i - reuse);
			events.push(Event::AddCursor { cursor_id: new_id, position: pos, anchor: None });
		}

		let total = positions.len();
		let batch = Event::Batch { events, description: "Add cursors to line ends".to_string() };
	...
	pub fn yank_word_forward(&mut self) {
		let cursor_positions: Vec<_> = self.active_cursors().iter().map(|(_, c)| c.position).collect();
		let ranges: Vec<_> = {
			let state = self.active_state();
			cursor_positions
				.into_iter()
