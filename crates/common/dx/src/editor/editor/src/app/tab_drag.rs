...
impl Editor {
	...
	pub(super) fn compute_tab_drop_zone(
		...
		for (split_id, tab_layout) in &self.active_layout().tab_layouts {
			if matches!(tab_layout.hit_test(col, row), Some(TabHit::TabName(_) | TabHit::CloseButton(_)))
			{
	...
	fn reorder_tab_in_split(
			...
			.get_mut(&split_id)
		{
			...
			if let Some(current_idx) = view_state.open_buffers.iter().position(|t| *t == target) {
				...
				let adjusted_idx =
					if current_idx < target_idx { target_idx.saturating_sub(1) } else { target_idx };
	...
	fn move_tab_to_split(
		...
		if let Some((mgr, vs)) = self.windows.get_mut(&active_id).and_then(|w| w.buffers.splits_mut()) {
			if let Some(source_view_state) = vs.get_mut(&source_split_id) {
				source_view_state.open_buffers.retain(|t| *t != TabTarget::Buffer(buffer_id));
			...
			.get_mut(&target_split_id)
		{
			...
			if !target_view_state.has_buffer(buffer_id) {
				...
				target_view_state.open_buffers.insert(final_idx, TabTarget::Buffer(buffer_id));
			}
		}
		...
		self.active_window_mut().set_pane_buffer(target_split_id, buffer_id);
		self
			.windows
			.get_mut(&self.active_window)
			.and_then(|w| w.split_manager_mut())
		...
		if source_becomes_empty {
			...
			self.active_window_mut().forget_split_terminal_modes(source_split_id);
			if let Err(e) = self
				.windows
				.get_mut(&self.active_window)
				.and_then(|w| w.split_manager_mut())
	...
	fn create_split_from_tab(
		...
		let source_showed_buffer =
			self.split_manager().get_buffer_id(source_split_id.into()) == Some(buffer_id);
		let mut next_buffer_for_source: Option<BufferId> = None;
		let source_had_buffer = if let Some((mgr, vs)) =
			self.windows.get_mut(&active_id).and_then(|w| w.buffers.splits_mut())
		{
			...
			.split_active(direction, buffer_id, ratio)
		{
			...
			Err(e) => {
				...
				self.set_status_message(t!("error.split_failed", error = e.to_string()).to_string());
			}
		}
	}
}
