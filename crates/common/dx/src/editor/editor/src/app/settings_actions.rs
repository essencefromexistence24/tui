...
impl Editor {
	...
	pub fn open_settings(&mut self) {
		...
		match crate::view::settings::SettingsState::new_with_plugin_schemas(
			...
			Err(e) => {
				self.set_status_message(t!("settings.failed_to_open", error = e.to_string()).to_string());
			}
		}
	}
	...
	pub fn save_settings(&mut self) {
		...
		match resolver.save_changes_to_layer(&pending_changes, &pending_deletions, target_layer) {
			Ok(()) => {
				...
				self.set_status_message(t!("settings.saved_to_layer", layer = layer_name).to_string());
			...
			Err(e) => {
				...
				let err = e.to_string();
				self.set_status_message(t!("settings.failed_to_save", error = err.clone()).to_string());
		...
		for window in self.windows.values_mut() {
			for state in window.buffers.as_map_mut().values_mut() {
				...
				state.buffer_settings.virtual_space =
					state.buffer_settings.virtual_space_override.unwrap_or(resolved.virtual_space);
...
"#
			...
			self.authority().filesystem.write_file(&path, template.as_bytes())?;
		}
		...
		match self.open_file(&path) {
			Ok(_) => {
				...
				self.set_status_message(
					t!("settings.editing_config", layer = layer_name, path = path.display().to_string())
						.to_string(),
				);
			}
			Err(e) => {
	...
	pub fn settings_activate_current(&mut self) {
		...
		let focus_panel =
			self.settings_state.as_ref().map(|s| s.focus_panel()).unwrap_or(FocusPanel::Categories);

		if focus_panel == FocusPanel::Footer {
			let button_index = self.settings_state.as_ref().map(|s| s.footer_button_index).unwrap_or(2);
			match button_index {
				0 => {
	...
	pub fn settings_decrement_current(&mut self) {
		...
		if focus_panel == FocusPanel::Footer {
			if let Some(ref mut state) = self.settings_state {
				...
				state.footer_button_index =
					if state.footer_button_index == 0 { 3 } else { state.footer_button_index - 1 };
			}
			return;
		}
