...
pub struct CalibrationTarget {
	...
	pub name: &'static str,
	/// What Dx expects to receive (the normalized key)
	...
	vec![
		...
		CalibrationGroup {
			...
			targets: vec![
				...
				CalibrationTarget {
					name: "ALT+SHIFT+LEFT",
					expected: ExpectedKey::new(KeyCode::Left, KeyModifiers::ALT.union(KeyModifiers::SHIFT)),
				},
				CalibrationTarget {
					name: "ALT+SHIFT+RIGHT",
					expected: ExpectedKey::new(KeyCode::Right, KeyModifiers::ALT.union(KeyModifiers::SHIFT)),
				},
				CalibrationTarget {
					name: "CTRL+LEFT",
...
impl CalibrationWizard {
	...
	pub fn new() -> Self {
		...
		Self {
			step: CalibrationStep::Capture { group_idx: 0, key_idx: 0 },
			groups,
	...
	pub fn key_status(&self, flat_idx: usize) -> &KeyStatus {
		self.key_statuses.get(flat_idx).unwrap_or(&KeyStatus::Pending)
	}
	...
	pub fn handle_capture_key(&mut self, key: KeyEvent) -> WizardAction {
		...
		if key.code == expected.code && key.modifiers == expected.modifiers {
			...
			self.pending_translations.insert(raw_key.clone(), expected_key);
			self.captured_raw_keys.insert(flat_idx, raw_key);
			self.key_statuses[flat_idx] = KeyStatus::Captured;
			self.status_message = Some(
				t!("calibration.captured", key = format!("{:?}", key.code), target = target.name)
					.to_string(),
			);
		}
		...
		for (group_idx, group) in self.groups.iter().enumerate() {
			...
			for (key_idx, target) in group.targets.iter().enumerate() {
				...
				if translated.code == expected.code && translated.modifiers == expected.modifiers {
					...
					self.status_message = Some(t!("calibration.key_verified", key = target.name).to_string());
	...
	fn skip_current_group(&mut self) {
		if let CalibrationStep::Capture { group_idx, key_idx } = &self.step {
			...
			self.status_message = Some(t!("calibration.skipped_group", group = group.name).to_string());

			// Advance to next group
			if group_idx + 1 < self.groups.len() {
				self.step = CalibrationStep::Capture { group_idx: group_idx + 1, key_idx: 0 };
			} else {
	...
	fn advance_to_next(&mut self) {
		if let CalibrationStep::Capture { group_idx, key_idx } = &self.step {
			...
			if key_idx + 1 < group.targets.len() {
				...
				self.step = CalibrationStep::Capture { group_idx, key_idx: key_idx + 1 };
	...
	fn go_back(&mut self) -> bool {
		let (group_idx, key_idx) = match &self.step {
			...
			CalibrationStep::Verify => {
				...
				self.step = CalibrationStep::Capture { group_idx: last_group, key_idx: last_key };
		...
		self.step = CalibrationStep::Capture { group_idx: prev_group, key_idx: prev_key };
	...
	pub fn restart(&mut self) {
		...
		self.step = CalibrationStep::Capture { group_idx: 0, key_idx: 0 };
	...
	pub fn verification_progress(&self) -> (usize, usize) {
		let total: usize =
			self.key_statuses.iter().filter(|s| matches!(s, KeyStatus::Captured)).count();
...
mod tests {
	...
	fn test_wizard_creation() {
		...
		assert!(matches!(wizard.step, CalibrationStep::Capture { group_idx: 0, key_idx: 0 }));
	...
	fn test_skip_key() {
		...
		assert!(matches!(wizard.step, CalibrationStep::Capture { group_idx: 0, key_idx: 1 }));
	}
	...
	fn test_skip_group() {
		...
		assert!(matches!(wizard.step, CalibrationStep::Capture { group_idx: 1, key_idx: 0 }));
	}
	...
	fn test_abort_with_confirmation() {
		...
		assert!(matches!(wizard.pending_confirmation, PendingConfirmation::Abort));
