...
impl Editor {
	...
	pub(crate) fn handle_completion_response(
		...
		if !self.active_window_mut().pending_completion_requests.remove(&request_id) {
		...
		let prefix = if word_start < cursor_pos {
			self.active_state_mut().get_text_range(word_start, cursor_pos).to_lowercase()
		} else {
			String::new()
		};
		...
		let all_filtered: Vec<&lsp_types::CompletionItem> =
			all_items.iter().filter(|item| matches_prefix(item)).collect();
		...
		let mut all_popup_items = crate::app::popup_actions::lsp_items_to_popup_items(&all_filtered);
		...
		let lsp_labels: std::collections::HashSet<String> =
			all_popup_items.iter().map(|i| i.text.to_lowercase()).collect();
		all_popup_items.extend(
			buffer_word_items.into_iter().filter(|item| !lsp_labels.contains(&item.text.to_lowercase())),
		);
		...
		tracing::info!(
			...
			self.active_window_mut().completion_items.as_ref().map_or(0, |i| i.len())
		);
	...
	pub(crate) fn handle_goto_definition_response(
		...
		if let Some(scheme) =
			location.uri.scheme().map(|s| s.as_str().to_string()).filter(|s| s != "file")
		{
		...
		if let Some(position) = position {
			...
			let split_id = self
				...
				.active_split();
			self.active_window_mut().apply_event_to_buffer(buffer_id, split_id, &event);
			// Without this the cursor lands at the definition but the
			// viewport never scrolls when the target file is already
			// open (#1689).
			self.active_window_mut().ensure_active_cursor_visible_for_navigation(true);
		}
		...
		self.active_window_mut().status_message =
			Some(t!("lsp.jumped_to_definition", path = display_path, line = line + 1).to_string());
		...
		F: Fn(&LspHandle, &crate::app::types::LspUri, &str) -> R,
	{
		...
		// Ensure didOpen is sent to all handles
		if self.ensure_did_open_all(buffer_id, &uri, &language).is_none() {
			return Vec::new();
		}
		...
		lsp
			.handles_for_feature_mut(&language, feature)
			.into_iter()
	...
	fn ensure_did_open_all(
		...
		let lsp = self.lsp_mut()?;
		let handle_ids: Vec<u64> = lsp.get_handles(language).iter().map(|sh| sh.handle.id()).collect();

		let needs_open: Vec<u64> = {
			let metadata = self.active_window().buffer_metadata.get(&buffer_id)?;
			handle_ids.iter().filter(|id| !metadata.lsp_opened_with.contains(id)).copied().collect()
		};
		...
		if !needs_open.is_empty() {
			...
			for sh in lsp.get_handles_mut(language) {
				if needs_open.contains(&sh.handle.id()) {
					if let Err(e) =
						sh.handle.did_open(uri.as_uri().clone(), text.clone(), language.to_string())
					{
	...
	pub(crate) fn request_completion(&mut self) {
		...
		// regression test in
		// crates/dx-editor/tests/e2e/lsp_completion_duplicate_entries_1514.rs
		// and sinelaw/fresh#1514.
		if !self.active_window().pending_completion_requests.is_empty() {
			let ids: Vec<u64> = self.active_window_mut().pending_completion_requests.drain().collect();
			for request_id in ids {
				tracing::debug!("Canceling previous pending LSP completion request {}", request_id);
		...
		let results = self.with_all_lsp_for_buffer_feature(
			...
			|handle, uri, _language| {
				...
				let result =
					handle.completion(request_id, uri.as_uri().clone(), line as u32, character as u32);
				if result.is_ok() {
					tracing::info!(
		...
		if !sent_ids.is_empty() {
			self.active_window_mut().pending_completion_requests.extend(sent_ids);
		} else {
	...
	pub(crate) fn maybe_trigger_completion(&mut self, c: char) {
		...
		let is_lsp_trigger =
			self.lsp().as_ref().map(|lsp| lsp.is_completion_trigger_char(c, &language)).unwrap_or(false);
	...
	pub(crate) fn request_goto_definition(&mut self) -> AnyhowResult<()> {
		...
		let sent = self
			.with_lsp_for_buffer(buffer_id, LspFeature::Definition, |handle, uri, _language| {
				let result =
					handle.goto_definition(request_id, uri.as_uri().clone(), line as u32, character as u32);
				if result.is_ok() {
					tracing::info!("Requested go-to-definition at {}:{}:{}", uri.as_str(), line, character);
				}
				result.is_ok()
			})
			.unwrap_or(false);
	...
	pub fn request_hover(&mut self) -> AnyhowResult<()> {
		...
		let sent = self
			.with_lsp_for_buffer(buffer_id, LspFeature::Hover, |handle, uri, _language| {
				let result = handle.hover(request_id, uri.as_uri().clone(), line as u32, character as u32);
				if result.is_ok() {
					tracing::info!(
		...
		if sent {
			self.active_window_mut().next_lsp_request_id += 1;
			self.active_window_mut().hover.record_request(request_id, line as u32, character as u32);
		}
	...
	pub(crate) fn handle_hover_response(
		...
		let diagnostic_lines =
			hover_lsp_position.map(|pos| self.compose_hover_diagnostic_lines(pos)).unwrap_or_default();
		...
		tracing::debug!("LSP hover content (markdown={}):\n{}", is_markdown, contents);

		// Convert LSP range to byte offsets for highlighting
		if let Some(((start_line, start_char), (end_line, end_char))) = range {
			let state = self.active_state();
			let start_byte = state.buffer.lsp_position_to_byte(start_line as usize, start_char as usize);
			let end_byte = state.buffer.lsp_position_to_byte(end_line as usize, end_char as usize);
			self.active_window_mut().hover.set_symbol_range(Some((start_byte, end_byte)));
			tracing::debug!(
			...
			self.active_window_mut().hover.set_symbol_range(computed_range);
		}
		...
		use crate::view::markdown::{StyledLine, parse_markdown};
		...
		let hover_lines: Vec<StyledLine> = if contents.is_empty() {
			...
			parse_markdown(&contents, &self.theme.read().unwrap(), Some(&self.grammar_registry))
		} else {
			contents
				.lines()
				.map(|s| {
					let mut sl = StyledLine::new();
					sl.push(s.to_string(), Style::default().fg(self.theme.read().unwrap().popup_text_fg));
					sl
				})
				.collect()
		};
		...
		if has_diagnostic && !hover_lines.is_empty() {
			...
			sep.push("─".repeat(12), Style::default().fg(self.theme.read().unwrap().popup_border_fg));
			all_lines.push(sep);
		}
		...
		let content_width: usize = all_lines
			.iter()
			.map(|l| l.spans.iter().map(|s| UnicodeWidthStr::width(s.text.as_str())).sum::<usize>())
			.max()
			.unwrap_or(0);
	...
	fn compose_hover_diagnostic_lines(
		...
		let overlapping: Vec<&lsp_types::Diagnostic> =
			diagnostics.iter().filter(|d| lsp_range_contains(&d.range, hover_line, hover_char)).collect();
		...
		for (idx, diag) in overlapping.iter().enumerate() {
			...
			let (label, marker, severity_color) = match diag.severity {
				...
				Some(DiagnosticSeverity::WARNING) => {
					("Warning", "⚠", self.theme.read().unwrap().diagnostic_warning_fg)
				}
			...
			let header_style = Style::default().fg(severity_color).add_modifier(Modifier::BOLD);
	...
	pub fn apply_inlay_hints_to_state(
		...
		for hint in hints {
			...
			let byte_offset = state
				.buffer
				.lsp_position_to_byte(hint.position.line as usize, hint.position.character as usize);
			...
			let byte_here = if byte_offset < buf_len {
				state.buffer.slice_bytes(byte_offset..byte_offset + 1).first().copied()
			} else {
				None
			};
	...
	pub(crate) fn request_references(&mut self) -> AnyhowResult<()> {
		...
		let (line, character, symbol) = {
			...
			let symbol =
				String::from_utf8_lossy(&state.buffer.slice_bytes(word_start..word_end)).into_owned();
		...
		// Use helper to ensure didOpen is sent before the request
		let sent = self
			.with_lsp_for_buffer(buffer_id, LspFeature::References, |handle, uri, _language| {
				let result =
					handle.references(request_id, uri.as_uri().clone(), line as u32, character as u32);
				if result.is_ok() {
					tracing::info!(
	...
	pub(crate) fn request_implementation(&mut self) -> AnyhowResult<()> {
		...
		// Use helper to ensure didOpen is sent before the request
		let sent = self
			.with_lsp_for_buffer(buffer_id, LspFeature::Implementation, |handle, uri, _language| {
				let result =
					handle.implementation(request_id, uri.as_uri().clone(), line as u32, character as u32);
				if result.is_ok() {
					tracing::info!(
	...
	pub(crate) fn request_signature_help(&mut self) {
		...
		// Use helper to ensure didOpen is sent before the request
		let sent = self
			.with_lsp_for_buffer(buffer_id, LspFeature::SignatureHelp, |handle, uri, _language| {
				let result =
					handle.signature_help(request_id, uri.as_uri().clone(), line as u32, character as u32);
				if result.is_ok() {
					tracing::info!(
	...
	pub(crate) fn handle_signature_help_response(
		...
		let active_param =
			signature_help.active_parameter.or(signature.active_parameter).unwrap_or(0) as usize;
		...
		let mut popup =
			Popup::markdown(&content, &self.theme.read().unwrap(), Some(&self.grammar_registry));
	...
	pub(crate) fn request_code_actions(&mut self) -> AnyhowResult<()> {
		...
		// quiescent).
		if !self.active_window().pending_code_actions_requests.is_empty() {
			let ids: Vec<u64> = self.active_window_mut().pending_code_actions_requests.drain().collect();
			for request_id in ids {
				tracing::debug!("Canceling previous pending LSP code actions request {}", request_id);
				self.active_window_mut().send_lsp_cancel_request(request_id);
			}
		}
		self.active_window_mut().pending_code_actions_server_names.clear();
		...
		let diagnostics: Vec<lsp_types::Diagnostic> = {
			...
			window
				...
				.map(|diags| {
					diags
						.iter()
						.filter(|d| lsp_range_overlaps(&d.range, start_line, start_char, end_line, end_char))
						.cloned()
						.collect()
				})
				.unwrap_or_default()
		...
		if !sent_ids.is_empty() {
			...
			self.active_window_mut().pending_code_actions_requests.extend(sent_ids);
		}
	...
	pub(crate) fn handle_code_actions_response(
		...
		if !self.active_window_mut().pending_code_actions_requests.remove(&request_id) {
		...
		if actions.is_empty() {
			...
			if self.active_window().pending_code_actions_requests.is_empty()
				&& self.active_window_mut().pending_code_actions.as_ref().is_none_or(|a| a.is_empty())
			{
		...
		let tagged_actions: Vec<(String, lsp_types::CodeActionOrCommand)> =
			actions.into_iter().map(|a| (server_name.clone(), a)).collect();
		...
		let __buffer_id = self.active_buffer();
		let action_count = self.active_window().pending_code_actions.as_ref().map_or(0, |v| v.len());
		if let Some(state) = self
			.windows
			.get_mut(&self.active_window)
			.map(|w| &mut w.buffers)
	...
	pub(crate) fn execute_code_action(&mut self, index: usize) {
		...
		match action {
			lsp_types::CodeActionOrCommand::CodeAction(ca) => {
				...
				{
					tracing::info!("Code action '{}' needs resolve, sending codeAction/resolve", ca.title);
	...
	fn send_execute_command(&mut self, cmd: lsp_types::Command) {
		...
		let language = match self.buffers().get(&self.active_buffer()).map(|s| s.language.clone()) {
			Some(l) => l,
			None => return,
		};
		...
		if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
			for sh in lsp.get_handles_mut(&language) {
				if let Err(e) = sh.handle.execute_command(cmd.command.clone(), cmd.arguments.clone()) {
	...
	pub(crate) fn request_formatting(&mut self) {
		...
		if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
			if let Some(sh) = lsp.handle_for_feature_mut(&language, LspFeature::Format) {
				if let Err(e) =
					sh.handle.document_formatting(request_id, uri.as_uri().clone(), tab_size, insert_spaces)
				{
	...
	pub(crate) fn handle_references_response(
		...
		self
			.set_status_message(t!("lsp.found_references", count = count, symbol = &symbol).to_string());
		...
		tracing::info!("Fired lsp_references hook with {} locations for symbol '{}'", count, symbol);
	...
	pub(crate) fn apply_lsp_text_edits(
		...
		let cursor_id = {
			let split_id =
				self.split_manager_mut().splits_for_buffer(buffer_id).into_iter().next().unwrap_or_else(
					|| {
						self
							.windows
							.get(&self.active_window)
		...
		for edit in edits {
			...
			let old_text = if start_pos < end_pos && end_pos <= buffer_len {
				...
				format!("<invalid range: start={}, end={}, buffer_len={}>", start_pos, end_pos, buffer_len)
			};
			tracing::debug!(
			...
			if start_pos < end_pos {
				...
				let delete_event = Event::Delete { range: start_pos..end_pos, deleted_text, cursor_id };
			...
			if !edit.new_text.is_empty() {
				let insert_event =
					Event::Insert { position: start_pos, text: edit.new_text.clone(), cursor_id };
	...
	fn apply_text_document_edit(
		...
		if let Ok(path) = super::lsp_uri_to_host_path(&uri, self.authority().path_translation.as_ref())
		{
	...
	fn apply_resource_operation(&mut self, op: lsp_types::ResourceOp) -> AnyhowResult<()> {
		...
		match op {
			lsp_types::ResourceOp::Create(create) => {
				...
				let ignore_if_exists =
					create.options.as_ref().and_then(|o| o.ignore_if_exists).unwrap_or(false);
			...
			lsp_types::ResourceOp::Rename(rename) => {
				...
				let ignore_if_exists =
					rename.options.as_ref().and_then(|o| o.ignore_if_exists).unwrap_or(false);
				...
				if new_path.exists() {
					...
					if !overwrite {
						tracing::warn!("RenameFile: {:?} already exists and overwrite=false", new_path);
						return Ok(());
					}
				}
			...
			lsp_types::ResourceOp::Delete(delete) => {
				...
				let ignore_if_not_exists =
					delete.options.as_ref().and_then(|o| o.ignore_if_not_exists).unwrap_or(false);
					...
					let buffer_id = match self.open_file(&path) {
						...
						Err(e) => {
								...
								e.downcast_ref::<crate::model::buffer::LargeFileEncodingConfirmation>()
							{
								...
								self
									.set_status_message(t!("file.error_opening", error = e.to_string()).to_string());
							}
							return Ok(0);
						}
					};
		...
		match result {
			...
			Err(error) => {
				...
				if error.contains("content modified") || error.contains("-32801") {
					...
					self.active_window_mut().status_message = Some(t!("lsp.rename_cancelled").to_string());
				} else {
					self.active_window_mut().status_message =
	...
	pub(crate) fn apply_events_to_buffer_as_bulk_edit(
		...
		let batch_for_lsp = Event::Batch { events: events.clone(), description: description.clone() };
		...
		let old_cursors: Vec<(CursorId, usize, Option<usize>)> = self
			...
			.map(|bvs| bvs.cursors.iter().map(|(id, c)| (id, c.position, c.anchor)).collect())
			.unwrap_or_default();
		...
		let __win = self.windows.get_mut(&self.active_window).expect("active window must exist");
		let bulk_edit = __win
			.buffers
			.with_buffer_and_view_states(buffer_id, |state, vs_map| -> AnyhowResult<Event> {
				...
				let edit_refs: Vec<(usize, usize, &str)> =
					edits.iter().map(|(pos, del, text)| (*pos, *del, text.as_str())).collect();
		...
		self.active_window_mut().send_lsp_changes_for_buffer(buffer_id, lsp_changes);
	...
	fn show_rename_prompt(&mut self) -> AnyhowResult<()> {
		...
		let (word_start, word_end) = {
			...
			if word_start >= word_end {
				self.active_window_mut().status_message = Some(t!("lsp.no_symbol_at_cursor").to_string());
				return Ok(());
			}
	...
	pub(crate) fn perform_lsp_rename(
		...
		if sent {
			...
			.is_none()
		{
			self.active_window_mut().status_message = Some(t!("lsp.cannot_rename_unsaved").to_string());
		}
	}
	...
	pub(crate) fn request_inlay_hints_for_buffer(&mut self, buffer_id: BufferId) {
		...
		let (line_count, version) = if let Some(state) = self
			...
			.get(&buffer_id)
		{
			(state.buffer.line_count().unwrap_or(1000), state.buffer.version())
		} else {
			return;
		};
		...
		// Use helper to ensure didOpen is sent before the request
		let sent = self
			.with_lsp_for_buffer(buffer_id, LspFeature::InlayHints, |handle, uri, _language| {
				let result = handle.inlay_hints(request_id, uri.as_uri().clone(), 0, 0, last_line, 10000);
				if result.is_ok() {
					tracing::info!("Requested inlay hints for {} (request_id={})", uri.as_str(), request_id);
	...
	pub(crate) fn maybe_request_folding_ranges_debounced(&mut self, buffer_id: BufferId) {
		let Some(ready_at) = self.active_window().folding_ranges_debounce.get(&buffer_id).copied()
		else {
			return;
		};
		...
		self.active_window_mut().folding_ranges_debounce.remove(&buffer_id);
	...
	pub(crate) fn request_folding_ranges_for_buffer(&mut self, buffer_id: BufferId) {
		if self.active_window_mut().folding_ranges_in_flight.contains_key(&buffer_id) {
			return;
		}
		...
		match handle.folding_ranges(request_id, uri.as_uri().clone()) {
			Ok(()) => {
				__pending_folding
					.insert(request_id, super::FoldingRangeRequest { buffer_id, version: buffer_version });
	...
	pub(crate) fn maybe_request_semantic_tokens(&mut self, buffer_id: BufferId) {
		...
		if self.active_window_mut().semantic_tokens_in_flight.contains_key(&buffer_id) {
			return;
		}
		...
		// while the &mut lsp borrow is alive.
		let Some((buffer_version, existing_version, previous_result_id)) =
			self.windows.get(&__active_id).and_then(|w| w.buffers.get(&buffer_id)).map(|state| {
				(
					state.buffer.version(),
					state.semantic_tokens.as_ref().map(|s| s.version),
					state.semantic_tokens.as_ref().and_then(|s| s.result_id.clone()),
				)
			})
		else {
			return;
	...
	pub(crate) fn maybe_request_semantic_tokens_full_debounced(&mut self, buffer_id: BufferId) {
		if !self.config.editor.enable_semantic_tokens_full {
			self.active_window_mut().semantic_tokens_full_debounce.remove(&buffer_id);
			return;
		}

		let Some(ready_at) =
			self.active_window().semantic_tokens_full_debounce.get(&buffer_id).copied()
		else {
			return;
		};
		if Instant::now() < ready_at {
	...
	pub(crate) fn maybe_request_semantic_tokens_range(
			...
			__st_range_last.get(&buffer_id).copied()
		{
			if last_start == padded_start
				...
				&& now.duration_since(last_time) < Duration::from_millis(SEMANTIC_TOKENS_RANGE_DEBOUNCE_MS)
			{
				return;
			}
		}
		...
		let lsp_range = lsp_types::Range {
			start: lsp_types::Position { line: padded_start as u32, character: 0 },
			end: lsp_types::Position { line: padded_end as u32, character: end_char as u32 },
		};
		...
		match handle.semantic_tokens_range(request_id, uri.as_uri().clone(), lsp_range) {
			Ok(_) => {
				...
				__st_range_in_flight
					.insert(buffer_id, (request_id, padded_start, padded_end, buffer_version));
...
mod tests {
	...
	use super::{Editor, lsp_range_contains, lsp_range_overlaps};
	...
	fn test_inlay_hint_inserts_before_character() {
		...
		let lookup = state.virtual_texts.build_lookup(&state.marker_list, 0, state.buffer.len());
	...
	fn test_inlay_hint_uses_theme_key_for_foreground() {
		...
		let lookup = state.virtual_texts.build_lookup(&state.marker_list, 0, state.buffer.len());
		let vtexts = lookup.get(&1).expect("expected hint at byte offset 1");
		assert_eq!(vtexts[0].fg_theme_key.as_deref(), Some("editor.line_number_fg"));
	...
	fn test_inlay_hint_removed_when_its_range_is_deleted() {
		...
		let removed = state.virtual_texts.remove_in_range(&mut state.marker_list, 4, 10);
	...
	fn test_marker_delete_after_repeat_clear_recreate() {
		...
		assert_eq!(removed, 1, "exactly one marker inside [1005, 1206) should be removed");
