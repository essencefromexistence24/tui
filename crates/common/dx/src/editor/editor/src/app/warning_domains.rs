...
impl WarningDomain for GeneralWarningDomain {
	...
	fn label(&self) -> String {
		if self.count > 0 { format!("[⚠ {}]", self.count) } else { String::new() }
	}
	...
	fn popup_content(&self) -> WarningPopupContent {
		...
		WarningPopupContent { title: t!("warning.title").to_string(), message, actions }
	}
...
impl WarningDomain for LspWarningDomain {
	...
	fn popup_content(&self) -> WarningPopupContent {
		...
		WarningPopupContent { title, message, actions }
	}
}
