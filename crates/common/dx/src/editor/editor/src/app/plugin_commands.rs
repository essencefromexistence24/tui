...
const IGNORED_DIRS: &[&str] =
	&[".git", "node_modules", "target", "__pycache__", ".hg", ".svn", ".DS_Store"];
...
impl Editor {
	...
	pub(super) fn handle_clear_overlays_in_range(
		...
		if let Some(state) = self
			...
			.buffer_state_mut(buffer_id)
		{
			state.overlays.remove_in_range(&(start..end), &mut state.marker_list);
			// Note: Overlays are ephemeral, not added to event log for undo/redo
		}
	}
	...
	pub(super) fn handle_add_virtual_text(
			...
			.buffer_state_mut(buffer_id)
		{
			...
			let vtext_position =
				if before { VirtualTextPosition::BeforeChar } else { VirtualTextPosition::AfterChar };
	...
	pub(super) fn handle_add_virtual_text_styled(
		...
		position: usize,
		text: String,
		fg: Option<dx_core::api::OverlayColorSpec>,
		bg: Option<dx_core::api::OverlayColorSpec>,
		bold: bool,
		italic: bool,
		before: bool,
	) {
	...
	pub(super) fn handle_add_virtual_line(
		...
		position: usize,
		text: String,
		fg_color: Option<dx_core::api::OverlayColorSpec>,
		bg_color: Option<dx_core::api::OverlayColorSpec>,
		above: bool,
		namespace: String,
		priority: i32,
		gutter_glyph: Option<String>,
		gutter_color_spec: Option<dx_core::api::OverlayColorSpec>,
			...
			.buffer_state_mut(buffer_id)
		{
			...
			let placement =
				if above { VirtualTextPosition::LineAbove } else { VirtualTextPosition::LineBelow };
	...
	pub(super) fn handle_clear_virtual_lines_in_range(
			...
			.buffer_state_mut(buffer_id)
		{
			...
			let ns = VirtualTextNamespace::from_string(namespace);
			state.virtual_texts.clear_lines_in_range(&mut state.marker_list, &ns, start, end);
		}
	}
	...
	pub(super) fn handle_add_conceal(
			...
			.buffer_state_mut(buffer_id)
		{
			...
			state.conceals.add(&mut state.marker_list, namespace, start..end, replacement);
			#[cfg(feature = "plugins")]
			{
				self.plugin_render_requested = true;
			}
	...
	pub(super) fn handle_clear_conceal_namespace(
		...
		if let Some(state) = self
			...
			.buffer_state_mut(buffer_id)
		{
			state.conceals.clear_namespace(&namespace, &mut state.marker_list);
		}
	}
	...
	pub(super) fn handle_clear_conceals_in_range(
			...
			.buffer_state_mut(buffer_id)
		{
			...
			state.conceals.remove_in_range(&(start..end), &mut state.marker_list);
		}
	}
	...
	pub(super) fn handle_add_fold(
		...
		let changed = self.active_window_mut().add_fold(buffer_id, start, end, placeholder);
		#[cfg(feature = "plugins")]
		if changed {
			self.plugin_render_requested = true;
		}
	...
	pub(super) fn handle_set_folding_ranges(
		...
		self.active_window_mut().apply_folding_ranges_response(buffer_id, ranges);
		#[cfg(feature = "plugins")]
		{
			self.plugin_render_requested = true;
		}
	...
	pub(super) fn handle_add_soft_break(
			...
			.buffer_state_mut(buffer_id)
		{
			...
			state.soft_breaks.add(&mut state.marker_list, namespace, position, indent);
			#[cfg(feature = "plugins")]
			{
				self.plugin_render_requested = true;
			}
	...
	pub(super) fn handle_clear_soft_break_namespace(
		...
		if let Some(state) = self
			...
			.buffer_state_mut(buffer_id)
		{
			state.soft_breaks.clear_namespace(&namespace, &mut state.marker_list);
		}
	}
	...
	pub(super) fn handle_clear_soft_breaks_in_range(
			...
			.buffer_state_mut(buffer_id)
		{
			...
			state.soft_breaks.remove_in_range(start, end, &mut state.marker_list);
		}
	}
	...
	pub(super) fn handle_add_menu(&mut self, menu: crate::config::Menu, position: MenuPosition) {
		...
		self.menu_state.plugin_menus.insert(plugin_idx.min(self.menu_state.plugin_menus.len()), menu);
		tracing::info!(
	...
	pub(super) fn handle_remove_menu(&mut self, menu_label: String) {
		...
		self.menu_state.plugin_menus.retain(|m| m.label != menu_label);

		if self.menu_state.plugin_menus.len() < original_len {
			tracing::info!("Removed plugin menu '{}'", menu_label);
		} else {
			tracing::warn!("Plugin menu '{}' not found (note: cannot remove config menus)", menu_label);
		}
	}
	...
	pub(super) fn handle_set_buffer_cursor(&mut self, buffer_id: BufferId, position: usize) {
		...
		self.active_window_mut().set_buffer_cursor_in_splits(buffer_id, position, &splits);
	}
	...
	pub(super) fn handle_set_split_scroll(&mut self, split_id: SplitId, top_byte: usize) {
		...
		self.active_window_mut().set_split_scroll_to_byte(buffer_id, leaf_id, top_byte);
		tracing::debug!("SetSplitScroll: split {:?} scrolled to byte {}", split_id, top_byte);
	}
	...
	pub(super) fn handle_insert_text(&mut self, buffer_id: BufferId, position: usize, text: String) {
		...
		if let Some(state) = self
			...
			.buffer_state_mut(buffer_id)
		{
			let event = Event::Insert { position, text, cursor_id: CursorId(0) };
	...
	pub(super) fn handle_delete_range(&mut self, buffer_id: BufferId, range: std::ops::Range<usize>) {
			...
			.buffer_state_mut(buffer_id)
		{
			...
			let event = Event::Delete { range, deleted_text, cursor_id: CursorId(0) };
			// Apply to buffer with dummy cursors (real cursors adjusted below)
			state.apply(&mut Cursors::default(), &event);
			if let Some(log) = self.active_window_mut().event_logs.get_mut(&buffer_id) {
				log.append(event);
			...
			.splits_for_buffer(buffer_id)
		{
			if let Some(view_state) = self
				...
				.get_mut(&leaf_id)
			{
				view_state.cursors.adjust_for_edit(delete_start, delete_len, 0);
			}
		}
		// Keep search-match highlights consistent with the edit (issue #2414).
	...
	pub(super) fn handle_insert_at_cursor(&mut self, text: String) {
		...
		let text_len = text.len();
		let event = Event::Insert { position: cursor_pos, text, cursor_id: CursorId(0) };
		let split_id = self.split_manager().active_split();
		let active_buf = self.active_buffer();
		self.active_window_mut().apply_event_to_buffer(active_buf, split_id, &event);
	...
	pub(super) fn handle_delete_selection(&mut self) {
		...
		let deletions: Vec<_> =
			{ self.active_cursors().iter().filter_map(|(_, c)| c.selection_range()).collect() };
		...
		if !deletions.is_empty() {
			...
			let events: Vec<_> = deletions
				...
				.map(|range| {
					let deleted_text = state.get_text_range(range.start, range.end);
					Event::Delete { range: range.clone(), deleted_text, cursor_id: primary_id }
				})
				.collect();
	...
	pub(super) fn handle_show_buffer(&mut self, buffer_id: BufferId) {
		...
		if let Some(&group_id) = self.active_window_mut().buffer_to_group.get(&buffer_id) {
			...
			let panel_name = self.active_window_mut().buffer_groups.get(&group_id).and_then(|g| {
				g.panel_buffers.iter().find_map(|(name, &bid)| (bid == buffer_id).then(|| name.clone()))
			});
			if let Some(panel_name) = panel_name {
				self.focus_panel(group_id.0, panel_name);
				tracing::info!("Switched to group panel buffer {:?} via group {:?}", buffer_id, group_id);
				return;
			}
		}
	...
	pub(super) fn handle_close_other_buffers_in_split(
		...
		tracing::info!("Closed other buffers {:?} in split {:?}", buffer_id, split_id);
	}
		...
		if let Some(buffer_id) = self.split_manager().get_buffer_id(split_id.into()) {
			if let Some(view_state) = self.split_view_states_mut().get_mut(&split_id) {
				...
				if let Some(current_idx) =
					view_state.open_buffers.iter().position(|t| *t == TabTarget::Buffer(buffer_id))
				{
					if current_idx > 0 {
	...
	pub(super) fn handle_set_layout_hints(
		...
		let target_split = split_id.map(LeafId).unwrap_or(self.split_manager().active_split());
	...
	pub(super) fn handle_set_global_state(
		...
		match value {
			Some(v) => {
				self.plugin_global_state.entry(plugin_name).or_default().insert(key, v);
			}
			None => {
	...
	pub(super) fn handle_set_session_state(
		...
		match value {
			Some(v) => {
				session.plugin_state.entry(plugin_name).or_default().insert(key, v);
			}
			None => {
	...
	pub(super) fn handle_submit_view_transform(
		...
		if view_state.view_transform_stale {
			tracing::trace!("Rejecting stale SubmitViewTransform for split {:?}", target_split);
			return;
		}
	...
	pub(super) fn handle_set_line_indicators(
			...
			.buffer_state_mut(buffer_id)
		{
			...
			for line in lines {
				let byte_offset = state.buffer.line_start_offset(line).unwrap_or(0);
				state.margins.set_line_indicator(byte_offset, namespace.clone(), indicator.clone());
			}
		}
	}
	...
	pub(super) fn handle_start_prompt(
		...
		let mut prompt = Prompt::new(label, PromptType::Plugin { custom_type: prompt_type.clone() });
		...
		self.plugin_manager.read().unwrap().run_hook(
			"prompt_changed",
			HookArgs::PromptChanged { prompt_type: prompt_type.clone(), input: String::new() },
		);
	}
	...
	pub(super) fn handle_start_prompt_with_initial(
		...
		let mut prompt = Prompt::with_initial_text(
			label,
			PromptType::Plugin { custom_type: prompt_type.clone() },
			initial_value.clone(),
		);
		prompt.overlay = floating_overlay;
		self.active_window_mut().prompt = Some(prompt);
		...
		self.plugin_manager.read().unwrap().run_hook(
			"prompt_changed",
			HookArgs::PromptChanged { prompt_type: prompt_type.clone(), input: initial_value },
		);
	}
	...
	pub(super) fn handle_start_prompt_async(
		...
		self.active_window_mut().prompt =
			Some(Prompt::with_initial_text(label, PromptType::AsyncPrompt, initial_value.clone()));

		// Fire the prompt_changed hook
		use crate::services::plugins::hooks::HookArgs;
		self.plugin_manager.read().unwrap().run_hook(
			"prompt_changed",
			HookArgs::PromptChanged { prompt_type: "async_prompt".to_string(), input: initial_value },
		);
	}
	...
	pub(super) fn handle_set_prompt_suggestions(
		...
		let internal_suggestions: Vec<EditorSuggestion> = suggestions
			...
			.map(|s| {
				let source = s.source.map(|src| match src {
					dx_core::command::CommandSource::Builtin => CommandSource::Builtin,
					dx_core::command::CommandSource::Plugin(name) => CommandSource::Plugin(name),
				});
				EditorSuggestion {
					text: s.text,
		...
		if let Some(prompt) = &mut self.active_window_mut().prompt {
			...
			prompt.selected_suggestion = if prompt.suggestions.is_empty() {
				...
				let idx =
					selected_index.map(|i| (i as usize).min(prompt.suggestions.len() - 1)).unwrap_or(0);
				Some(idx)
			};
	...
	pub(super) fn handle_register_command(&self, command: dx_core::command::Command) {
		...
		self.command_registry.read().unwrap().register(internal_command);
	}
		...
		#[cfg(feature = "plugins")]
		{
			if let Some(snapshot_handle) = self.plugin_manager.read().unwrap().state_snapshot_handle() {
				if let Ok(mut snapshot) = snapshot_handle.write() {
					...
					snapshot.keybinding_labels.retain(|k, _| !k.ends_with(&format!("\0{}", name)));
					// Add current labels from plugin defaults in KeybindingResolver
					let keybindings_read = self.keybindings.read().unwrap();
					if let Some(mode_bindings) = keybindings_read.get_plugin_defaults().get(&mode_context) {
						for (key_code, modifiers) in mode_bindings.keys() {
							let label = crate::input::keybindings::format_keybinding(key_code, modifiers);
							if let Some((_key_str, cmd)) =
								bindings.iter().find(|(k, _)| parse_key_string(k) == Some((*key_code, *modifiers)))
							{
	...
	pub(super) fn handle_send_lsp_request(
		...
		let error = if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
			...
			if lsp.try_spawn(&language, None) != LspSpawnResult::Spawned {
				Some(format!("LSP server for '{}' is not running (auto_start disabled)", language))
		...
		if let Some(err_msg) = error {
			self
				...
				.read()
				.unwrap()
				.reject_callback(dx_core::api::JsCallbackId::from(request_id), err_msg);
		}
	}
	...
	pub(super) fn handle_register_grammar(
		...
		tracing::info!("Grammar registered for '{}' (call reload_grammars to apply)", language);
	}
	...
	pub(super) fn handle_register_language_config(
		&mut self,
		language: String,
		config: dx_core::api::LanguagePackConfig,
	) {
		...
		self.config_mut().languages.insert(language.clone(), lang_config);
	...
	pub(super) fn handle_register_lsp_server(
		&mut self,
		language: String,
		config: dx_core::api::LspServerPackConfig,
	) {
		...
		self
			.config_mut()
			.lsp
			.insert(language.clone(), crate::types::LspLanguageConfig::Multi(vec![lsp_config]));
	...
	pub(super) fn handle_reload_grammars(&mut self, callback_id: dx_core::api::JsCallbackId) {
		tracing::debug!(
	...
	pub(super) fn handle_grep_project(
		...
		if pattern.is_empty() {
			let json =
				serde_json::to_string(&Vec::<GrepMatch>::new()).unwrap_or_else(|_| "[]".to_string());
		...
		for (bid, state) in
			self.windows.get(&self.active_window).map(|w| &w.buffers).expect("active window present")
		{
		...
		if let Err(e) =
			self.authority().filesystem.walk_files(&cwd, IGNORED_DIRS, &cancel, &mut |path, _rel| {
	...
	pub(super) fn handle_begin_search(&mut self, args: BeginSearchArgs) {
		...
		let Some(handle) = entry else {
			tracing::warn!("BeginSearch: no handle registered for id {} — dropping request", handle_id);
			return;
		};
		...
		let finish_with =
			|handle: &Arc<dx_core::api::SearchHandleState>, truncated: bool, error: Option<String>| {
		...
		if source_buffer_id != 0 {
			...
			if let Some(content) = content {
				...
				for mat in regex.find_iter(bytes) {
					...
					let start = mat.start();
					let end = mat.end();
					running_line += bytes[counted_to..start].iter().filter(|&&b| b == b'\n').count();
					counted_to = start;
					let line_start =
		...
		let out_of_root_source = (source_buffer_id != 0)
			...
			.and_then(|bid| self.windows.get(&self.active_window).and_then(|w| w.buffers.get(&bid)))
		...
		let Some(runtime) = &self.tokio_runtime else {
			finish_with(&handle, false, Some("No tokio runtime available".to_string()));
			return;
		};
		...
		runtime.spawn(async move {
			...
			while let Some(file_path) = path_rx.recv().await {
				if handle_for_task.cancel.load(std::sync::atomic::Ordering::Relaxed) {
					break;
				}
				...
				// vec).
				let push_matches = move |task_handle: &Arc<dx_core::api::SearchHandleState>,
				...
				let join = tokio::task::spawn_blocking(move || {
					...
					if task_handle.cancel.load(std::sync::atomic::Ordering::Relaxed) {
						return;
					}
					...
					if let Some((bid, plan)) = dirty_plan {
						let matches = match plan.execute(&*fs, &pattern, &fs_opts, &regex, remaining, query_len)
						{
							Ok(m) => m,
							Err(e) => {
								tracing::debug!("BeginSearch: hybrid search failed {:?}: {}", file_path, e);
								return;
							}
						};
						...
						if !matches.is_empty() {
							...
							let file_matches: Vec<GrepMatch> = matches
								...
								.collect();
							match_count.fetch_add(file_matches.len(), std::sync::atomic::Ordering::Relaxed);
						...
						while !cursor.done {
							...
							let batch = match fs.search_file(&file_path, &pattern, &fs_opts, &mut cursor) {
								Ok(b) => b,
								Err(e) => {
							...
							match_count.fetch_add(batch.len(), std::sync::atomic::Ordering::Relaxed);
							let file_str = file_path.to_string_lossy().to_string();
	...
	pub(super) fn handle_replace_in_buffer(
		...
		if matches.is_empty() {
			let result = ReplaceResult { replacements: 0, buffer_id: 0 };
		...
		let buffer_id = if let Some(bid) = explicit_buffer {
			bid
		} else if let Some((&bid, _)) =
			self.buffers().iter().find(|(_, state)| state.buffer.file_path() == Some(&file_path))
		{
			bid
		} else {
			...
			match self.open_file_no_focus(&file_path) {
				...
				Err(e) => {
					self
						...
						.read()
						.unwrap()
						.reject_callback(callback_id, format!("Failed to open file {:?}: {}", file_path, e));
					return;
				}
			}
		};
		...
		let edits: Vec<(usize, usize, &str)> =
			sorted_matches.iter().map(|&(offset, len)| (offset, len, replacement.as_str())).collect();

		let replacements = edits.len();

		// Owned tuples for helpers that don't take references.
		let edits_owned: Vec<(usize, usize, String)> =
			sorted_matches.iter().map(|&(offset, len)| (offset, len, replacement.clone())).collect();
			...
			if let Some(path) = state.buffer.file_path().map(|p| p.to_path_buf()) {
				if let Err(e) = state.buffer.save_to_file(&path) {
					self
						...
						.read()
						.unwrap()
						.reject_callback(callback_id, format!("Failed to save file {:?}: {}", path, e));
					return;
				}
		...
		if let Some(event) = bulk_edit_event {
			...
			self.active_window_mut().invalidate_layouts_for_buffer(buffer_id);
			...
			let full_content_change = self
				...
				.map(|text| {
					vec![lsp_types::TextDocumentContentChangeEvent { range: None, range_length: None, text }]
				})
				.unwrap_or_default();
			if !full_content_change.is_empty() {
				self.active_window_mut().send_lsp_changes_for_buffer(buffer_id, full_content_change);
			}
		}

		let result = ReplaceResult { replacements, buffer_id: buffer_id.0 };
		let json = serde_json::to_string(&result).unwrap_or_else(|_| "null".to_string());
		self.plugin_manager.read().unwrap().resolve_callback(callback_id, json);
	}
	...
	pub(super) fn handle_start_animation_area(
		&mut self,
		id: u64,
		rect: dx_core::api::AnimationRect,
		kind: dx_core::api::PluginAnimationKind,
	) {
...
mod tests {
	...
	fn handle_set_layout_hints_targets_specified_buffer_not_active() {
		...
		editor.handle_set_layout_hints(
			other_buf,
			None,
			LayoutHints { compose_width: Some(80), column_guides: None },
		);

		let active_split = editor.split_manager().active_split();
		let view_state = editor.split_view_states().get(&active_split).expect("split view state");

		let other_state = view_state.buffer_state(other_buf).expect("other buffer keyed in split");
		assert_eq!(
		...
		let active_state = view_state.buffer_state(initial_buf).expect("active buffer keyed in split");
		assert_eq!(
