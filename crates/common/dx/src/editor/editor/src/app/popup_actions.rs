...
impl Editor {
		...
		match resolver {
			Some(PopupResolver::PluginAction { popup_id }) => {
				...
				self.plugin_manager.read().unwrap().run_hook(
					"action_popup_result",
					crate::services::plugins::hooks::HookArgs::ActionPopupResult { popup_id, action_id },
				);
	...
	fn insert_completion_text(&mut self, text: String) {
		...
		let cursor_data: Vec<(CursorId, usize, usize, String)> = {
			let positions: Vec<(CursorId, usize)> =
				self.active_cursors().iter().map(|(id, c)| (id, c.position)).collect();
			positions
				.into_iter()
				.map(|(id, pos)| {
					let word_start = {
	...
	pub fn handle_popup_cancel(&mut self) {
		...
		match resolver {
			Some(PopupResolver::PluginAction { popup_id }) => {
				tracing::info!("handle_popup_cancel: dismissing action popup id={}", popup_id);
				self.hide_popup();
				self.plugin_manager.read().unwrap().run_hook(
					"action_popup_result",
	...
	pub fn handle_popup_type_char(&mut self, c: char) {
		...
		if let Some(events) = self.active_window_mut().action_to_events(Action::InsertChar(c)) {
			if events.len() > 1 {
	...
	pub fn handle_popup_backspace(&mut self) {
		...
		if let Some(events) = self.active_window_mut().action_to_events(Action::DeleteBackward) {
			if events.len() > 1 {
				if let Some(bulk_edit) = self.apply_events_as_bulk_edit(events, "Backspace".to_string()) {
					self.active_event_log_mut().append(bulk_edit);
				}
			} else {
				for event in events {
	...
	fn refilter_completion_popup(&mut self) {
		...
		let lsp_items = self.active_window_mut().completion_items.clone().unwrap_or_default();
	...
	items
		...
		.map(|item| {
			...
			PopupListItemData {
				...
				icon,
				data: item.insert_text.clone().or_else(|| Some(item.label.clone())),
			}
		})
		.collect()
}
