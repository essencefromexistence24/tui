...
impl Editor {
				...
				vs.keyed_states
					...
					.map(|(&buf_id, buf_state)| {
						let folds = self
							.buffers()
							.get(&buf_id)
							.map(|state| buf_state.folds.collapsed_line_ranges(&state.buffer, &state.marker_list))
							.unwrap_or_default();
						(buf_id, buf_state.clone(), folds)
					})
					.collect::<Vec<(BufferId, crate::view::split::BufferViewState, Vec<CollapsedFoldLineRange>)>>(
					)
			});

		match self.split_manager_mut().split_active(direction, current_buffer_id, 0.5) {
			Ok(new_split_id) => {
				let mut view_state =
				...
				view_state.apply_config_defaults(crate::view::split::ViewConfigDefaults {
					...
					line_wrap: self.active_window().resolve_line_wrap_for_buffer(current_buffer_id),
					wrap_indent: self.config.editor.wrap_indent,
					wrap_column: self.active_window().resolve_wrap_column_for_buffer(current_buffer_id),
	...
	pub fn close_active_split(&mut self) {
			...
			.close_split(closing_split)
		{
			Ok(_) => {
				...
				self.active_window_mut().forget_split_terminal_modes(closing_split);
	...
	fn switch_split(&mut self, next: bool) {
		...
		self.active_window_mut().ensure_active_tab_visible(split_id, buffer, tabs_width);
	...
	pub fn get_split_areas(
		&self,
	) -> &[(LeafId, BufferId, ratatui::layout::Rect, ratatui::layout::Rect, usize, usize)] {
