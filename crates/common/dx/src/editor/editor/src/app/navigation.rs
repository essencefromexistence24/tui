...
impl Default for JumpOptions {
	fn default() -> Self {
		Self { clear_anchor: true, recenter_on_scroll: true }
	}
}
...
impl crate::app::window::Window {
	...
	pub fn ensure_cursor_visible_for_navigation(
		...
		self.buffers.with_buffer_and_split(active_buffer, active_split, |state, view_state| {
			...
			if needs_recenter {
				...
				view_state.viewport.center_on_position(&mut state.buffer, cursor_pos);
			...
			if !view_state.viewport.line_wrap_enabled {
				...
				if visible_width > 0 {
					...
					if cursor_visual_col < left + margin {
						view_state.viewport.left_column = cursor_visual_col.saturating_sub(margin);
