...
impl Editor {
				...
				match self.run_formatter(formatter, &path) {
					...
					ActionResult::CommandNotFound(cmd) => {
						self.active_window_mut().status_message =
							Some(format!("Formatter '{}' not found (install it for auto-formatting)", cmd));
					}
	...
	pub fn format_buffer(&mut self) -> Result<(), String> {
		let path = match self.active_state().buffer.file_path() {
			Some(p) => p.to_path_buf(),
			None => {
				return Err("Cannot format unsaved buffer (save first to detect language)".to_string());
			}
		};
		...
		let formatter = self.config.languages.get(&language).and_then(|lc| lc.formatter.clone());
		...
		match self.run_formatter(&formatter, &path) {
			ActionResult::Success(output) => {
				...
				self.set_status_message(
					t!("format.formatted_with", formatter = formatter.command.clone()).to_string(),
				);
				Ok(())
			}
	...
	fn run_formatter(&mut self, formatter: &FormatterConfig, file_path: &Path) -> ActionResult {
		...
		let mut child = match cmd.spawn() {
			Ok(c) => c,
			Err(e) => {
				return ActionResult::Error(format!("Failed to run '{}': {}", formatter.command, e));
			}
		};
		...
		if output.status.success() {
			...
			ActionResult::Error(format!("Formatter '{}' failed: {}", formatter.command, error_output))
		}
	}
	...
	fn run_on_save_action(
		...
		loop {
			match child.try_wait() {
				...
				Err(e) => {
					return ActionResult::Error(format!("Failed to wait for '{}': {}", action.command, e));
				}
			}
		}
	}
	...
	fn replace_buffer_with_output(&mut self, output: &str) -> Result<(), String> {
		...
		let delete_event =
			Event::Delete { range: 0..buffer_len, deleted_text: buffer_content, cursor_id };
		let insert_event = Event::Insert { position: 0, text: output.to_string(), cursor_id };
		...
		let batch = Event::Batch { events, description: "On-save format".to_string() };
