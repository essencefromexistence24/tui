...
impl Editor {
	...
	fn close_buffer_internal(&mut self, id: BufferId) -> anyhow::Result<()> {
		...
		if closing_active {
			...
			if return_to_group.is_some() && hidden {
				...
				{
					vs.open_buffers.retain(|t| *t != TabTarget::Buffer(replacement_buffer));
					vs.focus_history.retain(|t| *t != TabTarget::Buffer(replacement_buffer));
				}
			}
		}
		...
		for split_id in splits_to_update {
			self.active_window_mut().set_pane_buffer(split_id, replacement_buffer);
		}
		...
		self
			...
			.read()
			.unwrap()
			.run_hook("buffer_closed", dx_core::hooks::HookArgs::BufferClosed { buffer_id: id });
	...
	fn cleanup_closed_terminal(
		...
		// reuses it.
		self.active_window_mut().terminal_explicit_titles.remove(&id);
	...
	fn resolve_close_replacement(&mut self, id: BufferId, active_split: LeafId) -> CloseReplacement {
			...
			.and_then(|vs| {
				...
				vs.focus_history.iter().rev().find_map(|t| match t {
					...
					TabTarget::Group(leaf) => {
						...
						if self.active_window().grouped_subtrees.contains_key(leaf) { Some(*t) } else { None }
					}
				})
			});
		...
		let (buffer, created_empty) =
			match direct_replacement.or(already_keyed).or(fallback_buffer).or(any_remaining) {
				Some(bid) => (bid, false),
				None => {
					...
					if !self.config.editor.auto_create_empty_buffer_on_last_buffer_close {
	...
	fn purge_buffer_state(&mut self, id: BufferId) {
		...
		if let Some((request_id, _, _)) = self.active_window_mut().semantic_tokens_in_flight.remove(&id)
		{
			self.active_window_mut().pending_semantic_token_requests.remove(&request_id);
		}
		if let Some((request_id, _, _, _)) =
			self.active_window_mut().semantic_tokens_range_in_flight.remove(&id)
		{
			self.active_window_mut().pending_semantic_token_range_requests.remove(&request_id);
		}
		self.active_window_mut().semantic_tokens_range_last_request.remove(&id);
		self.active_window_mut().semantic_tokens_range_applied.remove(&id);
		self.active_window_mut().semantic_tokens_full_debounce.remove(&id);
	...
	pub fn switch_buffer(&mut self, id: BufferId) {
			...
			&& id != self.active_buffer()
		{
			...
			self.active_window_mut().position_history.commit_pending_movement();
		...
		if is_last_viewport {
			...
			if split_tabs.len() <= 1 && has_other_splits {
				...
				if let Err(e) = self.close_buffer(buffer_id) {
					tracing::debug!("close_tab_in_split: buffer cleanup after split close failed: {}", e);
				}
	...
	pub fn close_other_tabs_in_split(&mut self, keep_buffer_id: BufferId, split_id: LeafId) {
		...
		let tabs_to_close: Vec<_> =
			split_tabs.iter().filter(|&&id| id != keep_buffer_id).copied().collect();
	...
	fn reseat_tab_scroll_for_split(&mut self, split_id: LeafId) {
		...
		self.active_window_mut().ensure_active_tab_visible(split_id, active_buffer, tabs_width);
	}
	...
	fn close_tab_in_split_silent(&mut self, buffer_id: BufferId, split_id: LeafId) -> bool {
		...
		if is_last_viewport {
			...
			let current_idx = split_tabs.iter().position(|&id| id == buffer_id).unwrap_or(0);
			...
			if let Some(replacement) = replacement_buffer {
				self.active_window_mut().set_pane_buffer(split_id, replacement);
			}
			true
		}
	}
	...
	fn cycle_tab(&mut self, direction: i32) {
		...
		let targets: Vec<TabTarget> = view_state
			...
			.filter(|t| match t {
				TabTarget::Buffer(id) => {
					!self.active_window().buffer_metadata.get(id).map(|m| m.hidden_from_tabs).unwrap_or(false)
				}
		...
		self.active_window_mut().animate_tab_switch(active_split, direction.signum());
		...
		if let Some(entry) = self.active_window_mut().position_history.back() {
				...
				.contains_key(&target_buffer)
			{
				...
				let split_id = self
					...
					.expect("active window must have a populated split layout")
					.active_split();
				self.active_window_mut().apply_event_to_buffer(target_buffer, split_id, &event);
