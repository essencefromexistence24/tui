...
impl WindowBuffers {
	pub fn new() -> Self {
		Self { map: HashMap::new(), splits: None }
	}
	...
	{
		self.map.iter().find(|(id, state)| predicate(**id, state)).map(|(id, _)| *id)
	}
	...
	where
		F: FnMut(BufferId, &EditorState) -> bool,
	{
		self.map.iter().filter(|(id, state)| predicate(**id, state)).count()
	}

	pub fn paths(&self) -> Vec<PathBuf> {
		self.map.values().filter_map(|state| state.buffer.file_path().map(PathBuf::from)).collect()
	}

	pub fn languages(&self) -> HashSet<String> {
		self.map.values().map(|state| state.language.clone()).collect()
	}

	pub fn any_needs_semantic_redraw(&self) -> bool {
		self.map.values().any(|state| {
			state.reference_highlight_overlay.needs_redraw().is_some_and(|remaining| remaining.is_zero())
		})
	}
