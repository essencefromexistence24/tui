...
impl Editor {
	...
	pub(super) fn smart_home(&mut self) {
		...
		let cursors: Vec<(crate::model::event::CursorId, crate::model::cursor::Cursor)> =
			self.active_cursors().iter().map(|(id, c)| (id, *c)).collect();
		...
		for (cursor_id, cursor) in cursors {
			...
			let new_pos = if let Some(pos) = visual_target {
				...
				if cursor.position == first_non_ws { line_start } else { first_non_ws }
			};
		...
		let batch = if events.len() == 1 {
			...
			Event::Batch { events, description: "Smart home (multi-cursor)".to_string() }
		};
	...
	fn smart_home_visual_line(
		...
		use super::smart_home::{SmartHomeTarget, smart_home_target};

		let visual_start = self.active_layout().visual_line_start(split_id, cursor_pos, false)?;
		...
		let state = self
			...
			.get_mut(&buffer_id)?;
		let mut iter = state.buffer.line_iterator(visual_start, estimated_line_length);
		...
		let first_non_ws = if is_first_visual_row {
			let visual_end =
				self.active_layout().visual_line_end(split_id, cursor_pos, false).unwrap_or(visual_start);
		...
		match smart_home_target(cursor_pos, visual_start, is_first_visual_row, first_non_ws) {
			...
			SmartHomeTarget::PreviousVisualRowStart => {
				self.active_layout().visual_line_start(split_id, cursor_pos, true)
			}
		}
	}
	...
	pub(super) fn toggle_comment(&mut self) {
		...
		let comment_prefix: String = match comment_prefix {
			Some(prefix) => {
				...
				if prefix.ends_with(' ') { prefix } else { format!("{} ", prefix) }
			}
			...
			for &line_start in line_starts.iter().rev() {
				...
				if rest.starts_with(comment_prefix.trim()) {
					...
					let deleted_text = String::from_utf8_lossy(
						&state
							.buffer
							.slice_bytes(line_start + leading_ws..line_start + leading_ws + remove_len),
					)
					.to_string();
		...
		self.set_status_message(
			t!("lines.action", action = action_desc, count = line_starts.len()).to_string(),
		);
	}
