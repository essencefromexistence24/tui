...
impl crate::app::window::Window {
	...
	fn handle_page_motion(
		...
		let target_byte = {
			...
			self.buffers.with_buffer_and_split(buffer_id, split_id, |state, vs| {
				...
				vs.viewport.top_visual_row_source_byte(&mut state.buffer, &soft_breaks, &virtual_lines)
			})?
		};
	...
	fn handle_visual_line_movement(
		...
		let visual_action = match action {
			Action::MoveUp => VisualAction::UpDown { direction: -1, is_select: false },
			Action::MoveDown => VisualAction::UpDown { direction: 1, is_select: false },
			Action::SelectUp => VisualAction::UpDown { direction: -1, is_select: true },
			Action::SelectDown => VisualAction::UpDown { direction: 1, is_select: true },
				...
				if state.buffer_settings.virtual_space.cursor_beyond_eol() {
					let has_virtual_line_cursor = self
						...
						.is_some_and(|vs| {
							vs.cursors
								.iter()
								.any(|(_, c)| c.position == state.buffer.len() && c.virtual_lines_below > 0)
						});
			...
			cursors
				...
				.map(|(cursor_id, cursor)| {
					...
					let at_line_ending = if cursor.position < state.buffer.len() {
						let bytes = state.buffer.slice_bytes(cursor.position..cursor.position + 1);
					...
					let at_line_start = if cursor.position == 0 {
						...
						let prev = state.buffer.slice_bytes(cursor.position - 1..cursor.position);
						prev.first() == Some(&b'\n')
					};
					(
						cursor_id,
		...
		) in cursor_data
		{
			let (new_pos, new_sticky) = match &visual_action {
				VisualAction::UpDown { direction, is_select } => {
					...
					let from_pos = if deselect_on_move && !*is_select {
						if let Some(anchor) = anchor {
							if *direction < 0 { position.min(anchor) } else { position.max(anchor) }
						} else {
							position
						}
					} else {
					...
					let current_visual_col = self.layout_cache.byte_to_visual_column(split_id, from_pos)?;

					let goal_visual_col =
						if let Some(sticky) = sticky_column { sticky } else { current_visual_col };

					match self.layout_cache.move_visual_line(split_id, from_pos, goal_visual_col, *direction)
					{
				...
				VisualAction::LineEnd { .. } => {
					...
					match self.layout_cache.visual_line_end(split_id, position, allow_advance) {
				...
				VisualAction::LineStart { .. } => {
					...
					match self.layout_cache.visual_line_start(split_id, position, allow_advance) {
