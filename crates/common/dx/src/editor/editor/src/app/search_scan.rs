...
impl SearchScan {
	...
	pub(crate) fn match_count(&self) -> usize {
		self.active.as_ref().and_then(|a| a.chunked.as_ref()).map(|c| c.matches.len()).unwrap_or(0)
	}
	...
	pub(crate) fn take_finished(&mut self) -> Option<FinishedSearchScan> {
		...
		let match_ranges = chunked.matches.iter().map(|m| (m.byte_offset, m.length)).collect();
...
mod tests {
	...
	fn take_finished_drains_and_returns_match_ranges() {
		...
		let mut s = SearchScan::default();
		s.start(BufferId(7), Vec::new(), chunked, "abc".to_string(), None, true, false, false);
