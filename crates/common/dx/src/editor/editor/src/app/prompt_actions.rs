...
impl Editor {
		...
		match prompt_type {
			...
			PromptType::SwitchProject => {
				...
				if resolved_path.is_dir() {
					...
					self.set_status_message(
						t!("file.not_directory", path = resolved_path.display().to_string()).to_string(),
					);
				}
			}
			...
			PromptType::ReplaceSearch => {
				...
				self.start_prompt(
					...
					PromptType::Replace { search: input.clone() },
				);
			}
			...
			PromptType::QueryReplaceSearch => {
				...
				self.start_prompt(
					...
					PromptType::QueryReplace { search: input.clone() },
				);
			}
			...
			PromptType::GotoLine => {
					...
					.get(&buffer_id)
				{
					...
					match crate::input::quick_open::parse_goto_line_input(&input) {
						...
						None => {
							self.set_status_message(t!("error.invalid_line", input = input.trim()).to_string());
						}
					}
				} else {
			...
			PromptType::GotoByteOffset => {
				...
				let num_str =
					trimmed.strip_suffix('B').or_else(|| trimmed.strip_suffix('b')).unwrap_or(trimmed);
				match num_str.parse::<usize>() {
					Ok(offset) => {
						self.goto_byte_offset(offset);
						self.set_status_message(t!("goto.jumped_byte", offset = offset).to_string());
					}
					Err(_) => {
						self.set_status_message(t!("goto.invalid_byte_offset", input = &input).to_string());
					}
				}
			}
			PromptType::GotoLineScanConfirm => {
				...
				if answer == "y" || answer == "yes" {
					...
					self.start_prompt(t!("goto.byte_offset_prompt").to_string(), PromptType::GotoByteOffset);
				}
			}
			...
			PromptType::SetBackgroundBlend => match input.trim().parse::<f32>() {
				Ok(val) => {
					...
					self.set_status_message(
						t!("error.background_blend_set", value = format!("{:.2}", clamped)).to_string(),
					);
				}
				Err(_) => {
			...
			PromptType::RecordMacro => {
				self.handle_register_input(&input, |editor, c| editor.toggle_macro_recording(c), "Macro");
			}
			PromptType::PlayMacro => {
				self.handle_register_input(&input, |editor, c| editor.play_macro(c), "Macro");
			}
			PromptType::SaveMacroToInit => {
				self.handle_register_input(&input, |editor, c| editor.save_macro_to_init(c), "Macro");
			}
			PromptType::PromoteMacro => {
				self.handle_register_input(&input, |editor, c| editor.promote_macro_to_command(c), "Macro");
			}
			...
			PromptType::JumpToBookmark => {
				self.handle_register_input(&input, |editor, c| editor.jump_to_bookmark(c), "Bookmark");
			}
			PromptType::Plugin { custom_type } => {
				...
				self.plugin_manager.read().unwrap().run_hook(
					"prompt_confirmed",
					HookArgs::PromptConfirmed { prompt_type: custom_type.clone(), input, selected_index },
				);
			...
			PromptType::ConfirmSudoSave { info } => {
				...
				if input_lower == "y" || input_lower == "yes" {
					...
					match result {
						Ok(_) => {
							if let Err(e) =
								self.active_state_mut().buffer.finalize_external_save(info.dest_path.clone())
							{
			...
			PromptType::LspRename { original_text, start_pos, end_pos: _, overlay_handle } => {
				self.perform_lsp_rename(input, original_text, start_pos, overlay_handle);
			}
			PromptType::FileExplorerRename { original_path, original_name, is_new_file } => {
			...
			PromptType::ConfirmPasteConflict { src, dst, is_cut } => {
				match input.trim().to_lowercase().as_str() {
					...
					"r" | "rename" => {
						...
						let dst_dir = dst.parent().map(|p| p.to_path_buf()).unwrap_or_else(|| dst.clone());
			...
			PromptType::ConfirmMultiPasteConflict { safe, confirmed, mut pending, is_cut } => {
				...
				match input.trim() {
					...
					_ => {
						...
						self.prompt_next_paste_conflict(safe, confirmed, pending_with_current, is_cut);
					}
				}
			}
			PromptType::ConfirmLargeFileEncoding { path } => {
				let input_lower = input.trim().to_lowercase();
				let load_key = t!("file.large_encoding.key.load").to_string().to_lowercase();
				let encoding_key = t!("file.large_encoding.key.encoding").to_string().to_lowercase();
				let cancel_key = t!("file.large_encoding.key.cancel").to_string().to_lowercase();
			...
			PromptType::AsyncPrompt => {
				...
				if let Some(callback_id) = self.active_window_mut().pending_async_prompt_callback.take() {
	...
	pub(crate) fn save_file_as_with_checks(&mut self, full_path: std::path::PathBuf) {
		...
		if let Some(parent) = full_path.parent() {
			if !parent.as_os_str().is_empty() && !self.authority().filesystem.exists(parent) {
				let dir_name =
					parent.strip_prefix(self.working_dir()).unwrap_or(parent).display().to_string();
				self.start_prompt(
	...
	pub(crate) fn perform_save_file_as(&mut self, full_path: std::path::PathBuf) {
		...
		tracing::debug!("SaveFileAs BEFORE: event_log index={}, len={}", before_idx, before_len);
		...
		match self.active_state_mut().buffer.save_to_file(&full_path) {
			Ok(()) => {
				...
				self.active_window_mut().buffer_metadata.insert(active_buffer, metadata);
				...
				if let Some(buffer_to_close) = self.active_window_mut().pending_close_buffer.take() {
					if let Err(e) = self.force_close_buffer(buffer_to_close) {
						self
							.set_status_message(t!("file.saved_cannot_close", error = e.to_string()).to_string());
					} else {
						self.set_status_message(t!("buffer.saved_and_closed").to_string());
					}
				} else if !self.active_window_mut().pending_quit_unnamed_save.is_empty() {
					...
					self.active_window_mut().pending_quit_unnamed_save.retain(|id| *id != just_saved);
					self.set_status_message(
	...
	fn handle_set_page_width(&mut self, input: &str) {
		...
		if trimmed.is_empty() {
			...
			match trimmed.parse::<u16>() {
				...
				_ => {
					self.set_status_message(t!("error.invalid_page_width", input = input).to_string());
				}
			}
		}
	}
	...
	fn handle_open_file_with_encoding(&mut self, path: &std::path::Path, input: &str) {
		...
		match encoding {
			Some(enc) => {
				...
				let file_size =
					self.authority().filesystem.metadata(path).map(|m| m.size as usize).unwrap_or(0);
				...
				if file_size >= threshold && enc.requires_full_file_load() {
					...
					self.start_prompt(
						prompt_msg,
						PromptType::ConfirmLargeFileEncoding { path: path.to_path_buf() },
					);
					return;
				}
		...
		match encoding {
			Some(enc) => {
				...
				if let Err(e) = self.reload_with_encoding(enc) {
					...
					self.set_status_message(format!("Reloaded with {} encoding", enc.display_name()));
				}
			}
			None => {
				self.set_status_message(format!("Unknown encoding: {}", input));
	...
	fn handle_set_language(&mut self, input: &str) {
		...
		if let Some(detected) =
			DetectedLanguage::from_syntax_name(trimmed, &self.grammar_registry, &self.config.languages)
		{
			...
			self.plugin_manager.read().unwrap().run_hook(
				"language_changed",
				crate::services::plugins::hooks::HookArgs::LanguageChanged { buffer_id, language },
			);
		} else {
	...
	fn handle_confirm_close_buffer(&mut self, input: &str, buffer_id: BufferId) -> bool {
		...
		if first_char == save_first {
			// Save and close
			let has_path =
				self.buffers().get(&buffer_id).map(|s| s.buffer.file_path().is_some()).unwrap_or(false);
	...
	fn handle_confirm_quit(&mut self, input: &str) {
		...
		let quit_first = t!("prompt.key.quit").to_string().to_lowercase().chars().next();
	...
	fn handle_confirm_quit_modified(&mut self, input: &str) -> bool {
		...
		if first_char == save_first {
			...
			self.active_window_mut().pending_quit_unnamed_save = self.collect_unnamed_modified_buffers();
			if !self.start_next_quit_save_as() {
				self.should_quit = true;
			}
		} else if first_char == discard_first {
	...
	pub fn handle_stop_lsp_server(&mut self, input: &str) {
		...
		let has_server = self.lsp().as_ref().is_some_and(|lsp| lsp.has_handles(language));
		...
		let stopping_all = server_name.is_none()
			|| self.lsp().as_ref().map(|lsp| lsp.handle_count(language) <= 1).unwrap_or(true);
		...
		if stopping_all {
			...
			let buffer_ids: Vec<_> =
				self.buffers().iter().filter(|(_, s)| s.language == language).map(|(id, _)| *id).collect();
		...
		if let Some(lsp_configs) = self.config_mut().lsp.get_mut(language) {
			...
			if let Err(e) = self.save_config() {
				tracing::warn!("Failed to save config after disabling LSP auto-start: {}", e);
			} else {
