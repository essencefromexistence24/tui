...
impl Editor {
	...
	pub(super) fn start_search_prompt(
		...
		let selected_text = if let Some(range) = selection_range.clone() {
			...
			if !text.contains('\n') && !text.is_empty() { Some(text) } else { None }
		} else {
			None
		};
		...
		let default_text = selected_text
			.or_else(|| self.get_prompt_history("search").and_then(|h| h.last().map(|s| s.to_string())));
	...
	pub fn start_prompt_with_initial_text(
		...
		self.active_window_mut().prompt =
			Some(Prompt::with_initial_text(message, prompt_type, initial_text));
	}
	...
	pub(super) fn build_quick_open_context(&self) -> QuickOpenContext {
		...
		let has_lsp_config = {
			let language = self.buffers().get(&self.active_buffer()).map(|s| s.language.as_str());
			language.and_then(|lang| self.lsp().and_then(|lsp| lsp.get_config(lang))).is_some()
		};
		...
		QuickOpenContext {
			...
			active_buffer_path: self.active_state().buffer.file_path().map(|p| p.display().to_string()),
	...
	pub(super) fn update_quick_open_suggestions(&mut self, input: &str) {
		...
		if let Some(prompt) = &mut self.active_window_mut().prompt {
			...
			prompt.selected_suggestion = if prompt.suggestions.is_empty() { None } else { Some(0) };
		}
	...
	pub(super) fn restore_goto_line_preview_snapshot(&mut self) {
		...
		self.active_window_mut().apply_event_to_buffer(snap.buffer_id, snap.split_id, &event);
	...
	pub fn change_working_dir(&mut self, new_path: PathBuf) {
		...
		if self.find_window_by_root(&new_path).is_some_and(|w| w == old_id) {
			return;
		}
		...
		let new_id = if let Some(existing) = self.find_window_by_root(&new_path) {
			...
			if self.windows.get(&old_id).is_some_and(|w| w.authority_spec.is_remote()) {
				...
				let spec = self.windows.get(&old_id).map(|w| w.authority_spec.clone()).unwrap_or_default();
	...
	pub(super) fn handle_file_open_directory_loaded(
		...
		match result {
			Ok(entries) => {
				...
				let filter =
					self.active_window().prompt.as_ref().map(|p| p.input.clone()).unwrap_or_default();
				if !filter.is_empty() {
					if let Some(state) = &mut self.active_window_mut().file_open_state {
	...
	pub fn cancel_prompt(&mut self) {
		...
		if let Some(prompt) = prompt_clone {
			...
			match &prompt.prompt_type {
				...
				PromptType::LspRename { overlay_handle, .. } => {
					...
					let remove_overlay_event =
						crate::model::event::Event::RemoveOverlay { handle: overlay_handle.clone() };
				...
				PromptType::OpenFile | PromptType::SwitchProject | PromptType::SaveFileAs => {
					...
					if matches!(prompt.prompt_type, PromptType::SaveFileAs)
						&& !self.active_window_mut().pending_quit_unnamed_save.is_empty()
					{
		...
		// untouched.
		let was_overlay = self.active_window().prompt.as_ref().is_some_and(|p| p.overlay);
		if was_overlay {
	...
	pub fn confirm_prompt(&mut self) -> Option<(String, PromptType, Option<usize>)> {
		if let Some(prompt) = self.active_window_mut().prompt.take() {
					...
					if let Some(suggestion) = prompt.suggestions.get(selected_idx) {
						...
						if suggestion.disabled {
							self.set_status_message(
								t!("error.command_not_available", command = suggestion.text.clone()).to_string(),
							);
							return None;
						}
			...
			if matches!(prompt.prompt_type, PromptType::StopLspServer | PromptType::RestartLspServer) {
				let is_valid =
					prompt.suggestions.iter().any(|s| s.text == final_input || s.get_value() == final_input);
				if !is_valid {
					// Restore the prompt and don't confirm
					self.active_window_mut().prompt = Some(prompt);
					self
						.set_status_message(t!("error.no_lsp_match", input = final_input.clone()).to_string());
					return None;
				}
			}
	...
	pub fn active_prompt_has_search_options(&self) -> bool {
		self.active_window().prompt.as_ref().is_some_and(|p| p.prompt_type.has_search_options())
	}
	...
	pub(super) fn get_or_create_prompt_history(
		...
		self.active_window_mut().prompt_histories.entry(key.to_string()).or_default()
	}
	...
	pub(super) fn prompt_type_to_history_key(
		...
		match prompt_type {
			...
			PromptType::Replace { .. } | PromptType::QueryReplace { .. } => Some("replace".to_string()),
	...
	pub fn prompt_input(&self) -> Option<&str> {
		self.active_window().prompt.as_ref().map(|p| p.input.as_str())
	}
	...
	pub fn update_prompt_suggestions(&mut self) {
		...
		match prompt_type {
			...
			PromptType::GotoLine => {
				...
				if let Some(history) = self.active_window_mut().prompt_histories.get_mut("goto_line") {
					history.reset_navigation();
				}
			...
			PromptType::Plugin { custom_type } => {
				...
				self
					...
					.read()
					.unwrap()
					.run_hook("prompt_changed", HookArgs::PromptChanged { prompt_type: custom_type, input });
