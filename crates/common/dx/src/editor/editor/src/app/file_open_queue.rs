...
impl Editor {
	...
	pub fn queue_file_open(
		...
		self.active_window_mut().pending_file_opens.push(super::PendingFileOpen {
			path,
			line,
			column,
			end_line,
	...
	pub fn process_pending_file_opens(&mut self) -> bool {
		...
		for pending_file in pending {
			tracing::info!("[SYNTAX DEBUG] Processing pending file open: {:?}", pending_file.path);
			...
			match self.open_file(&pending_file.path) {
				Ok(_) => {
					...
					if let (Some(line), Some(end_line)) = (pending_file.line, pending_file.end_line) {
						self.select_range(line, pending_file.column, end_line, pending_file.end_column);
					...
					if let Some(wait_id) = pending_file.wait_id {
						...
						self.active_window_mut().wait_tracking.insert(buffer_id, (wait_id, has_popup));
					}
					processed_any = true;
				}
				Err(e) => {
	...
	pub fn remove_wait_tracking(&mut self, wait_id: u64) {
		self.active_window_mut().wait_tracking.retain(|_, (wid, _)| *wid != wait_id);
	}
}
