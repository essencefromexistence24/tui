...
impl Editor {
	...
	pub(crate) fn jump_to_line_column(&mut self, line: Option<usize>, column: Option<usize>) {
		...
		self
			.active_window_mut()
			.jump_active_cursor_to(clamped_position, super::navigation::JumpOptions::navigation());
	}
	...
	pub(crate) fn open_file_with_kind(
		...
		let active_had_path =
			self.buffers().get(&self.active_buffer()).and_then(|s| s.buffer.file_path()).is_some();

		let buffer_id = self.active_window_mut().open_file_no_focus_with_kind(path, kind)?;
		...
		let is_binary = self.buffers().get(&buffer_id).map(|s| s.buffer.is_binary()).unwrap_or(false);
	...
	fn reveal_active_split_if_hidden_by_maximize(&mut self) {
		...
		self.split_manager_mut().unmaximize_split().expect("a split is maximized (checked above)");
		self.relayout();
	}
	...
	pub fn open_file_with_encoding(
		...
		state.margins.configure_for_line_numbers(self.config.editor.line_numbers);
	...
	pub(crate) fn open_lsp_uri_target(
		...
		if translation.is_some() {
			...
			let container_path = uri
				.to_host_path(None)
				.ok_or_else(|| anyhow::anyhow!("URI is not a file path (container-side decode failed)"))?;
		...
		Err(anyhow::anyhow!("could not open {}: file not found", host_path.display()))
	}
	...
	fn fetch_and_open_container_file(
		...
		let result =
			runtime.block_on(spawner.spawn("cat".into(), vec![path_arg], None)).map_err(|e| {
				anyhow::anyhow!("could not open {} from container: {}", container_path.display(), e)
			})?;

		if result.exit_code != 0 {
			let first_stderr_line = result.stderr.lines().next().unwrap_or("(no error message)").trim();
			anyhow::bail!(
	...
	pub(crate) fn open_container_only_file(
		...
		let mut buffer =
			crate::model::buffer::Buffer::from_bytes(content, Arc::clone(&self.authority().filesystem));
		...
		if let Some(lang_config) = self.config.languages.get(&state.language) {
			...
			state.buffer_settings.use_tabs = lang_config.use_tabs.unwrap_or(self.config.editor.use_tabs);
			state.buffer_settings.tab_size = lang_config.tab_size.unwrap_or(self.config.editor.tab_size);
		} else {
...
impl crate::app::window::Window {
	...
	fn is_internal_data_artifact(&self, path: &Path) -> bool {
		let canonicalize =
			|p: &Path| self.authority().filesystem.canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
	...
	fn open_file_no_focus_inner(
		...
		let base_dir = if self.authority().filesystem.remote_connection_info().is_some() {
			self.authority().filesystem.home_dir().unwrap_or_else(|_| self.root.clone())
		} else {
			self.root.clone()
		};
		...
		let canonical_path = if file_exists {
			...
			if let Some(parent) = resolved_path.parent() {
				let canonical_parent = if parent.as_os_str().is_empty() {
					...
					self.authority().filesystem.canonicalize(parent).unwrap_or_else(|_| parent.to_path_buf())
				};
		...
		if let Some(lang_config) = self.resources.config.languages.get(&state.language) {
			...
			state.buffer_settings.use_tabs =
				lang_config.use_tabs.unwrap_or(self.resources.config.editor.use_tabs);
			// Use language-specific tab_size if set, otherwise fall back to global
			state.buffer_settings.tab_size =
				lang_config.tab_size.unwrap_or(self.resources.config.editor.tab_size);
		...
		state.reference_highlight_overlay.enabled = self.resources.config.editor.highlight_occurrences;
