...
//!
//! At startup Dx reads `~/.config/dx/init.ts` (if present) and feeds it
...
//! `~/.config/dx/logs/init.crashes` counts consecutive init.ts failures
...
/// Plugin name Dx uses when loading init.ts — stable so hot-reload works.
...
pub const STARTER_TEMPLATE: &str = r#"/// <reference path="./types/dx.d.ts" />
/// <reference path="./types/plugins.d.ts" />
const editor = getEditor();

// Dx init.ts — decisions that depend on the environment at startup.
//
// init.ts is NOT for:
...
//   - Depend on where/how Dx is starting (host, SSH, $TERM, project, ...)
//   - Would differ across machines or launches
//   - Can't live in a shared config.json without lying to teammates
//
// API reference: ~/.config/dx/types/dx.d.ts (same as plugins)
// Commands:  Ctrl+P -> "init: Reload", "init: Check"
// CLI:       dx --cmd init check | fresh --safe | fresh --no-init
...
// Example: env-driven profile (dx invoked as DX_PROFILE=writing dx).
// if (editor.getEnv("DX_PROFILE") === "writing") {
...
const INIT_TSCONFIG: &str = r#"{
  ...
  "files": ["init.ts", "types/dx.d.ts", "types/plugins.d.ts"]
}
"#;

/// Resolve the path to `dx.d.ts` inside the embedded-plugins cache.
...
fn embedded_fresh_dts_path() -> Option<PathBuf> {
	...
	let p = embedded.join("lib").join("dx.d.ts");
...
/// Refresh `~/.config/dx/types/dx.d.ts` from the embedded copy and
/// write `tsconfig.json` if it isn't already present.
///
/// `dx.d.ts` is **always overwritten** — it's an auto-generated API
/// mirror that must track the running binary. Keeping a stale copy in
/// `~/.config/dx/types/` would silently hide drift between the API
...
pub fn refresh_types_scaffolding(config_dir: &Path) {
	let Some(source) = embedded_fresh_dts_path() else {
		tracing::warn!(
			"init.ts: embedded dx.d.ts unavailable; \
	...
	let dest_dts = types_dir.join("dx.d.ts");
	if let Err(e) = std::fs::copy(&source, &dest_dts) {
		tracing::warn!(
			"init.ts: failed to copy dx.d.ts from {} to {}: {e}",
			source.display(),
			dest_dts.display()
		);
	}
...
/// Ensure `~/.config/dx/init.ts` exists. If absent, writes the starter
/// template. Also refreshes `types/dx.d.ts` + `tsconfig.json` so the
...
impl CrashState {
	fn load(config_dir: &Path) -> Self {
		...
		Self { count, last_increment_epoch: last }
	}
	...
	fn clear(config_dir: &Path) {
		...
		if let Err(e) = std::fs::remove_file(&path) {
			if e.kind() != std::io::ErrorKind::NotFound {
				tracing::debug!("init.ts crash-fuse: failed to clear {}: {e}", path.display());
			}
		}
	}
}
...
pub fn describe(outcome: &InitOutcome) -> String {
	match outcome {
		...
		InitOutcome::CrashFused { failures } => format!(
			"init.ts: skipped after {failures} consecutive failures — fix ~/.config/dx/init.ts or remove it"
		),
...
pub fn decide_load(config_dir: &Path, enabled: bool) -> LoadDecision {
	...
	match read_init_script(config_dir) {
		...
		Err(e) => LoadDecision::Skip(InitOutcome::Failed { message: format!("read failed: {e}") }),
		Ok(Some(source)) => {
...
/// Parse `~/.config/dx/init.ts` via oxc and report syntax errors.
///
...
pub fn check(config_dir: &Path) -> CheckReport {
	...
	CheckReport { ok: true, diagnostics: Vec::new(), path: init_ts_path(config_dir) }
}
...
pub fn check(config_dir: &Path) -> CheckReport {
	...
	let source = match std::fs::read_to_string(&path) {
		...
		Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
			return CheckReport { ok: true, diagnostics: Vec::new(), path };
		}
		Err(e) => {
			return CheckReport {
				ok: false,
	...
	CheckReport { ok: parser_ret.errors.is_empty(), diagnostics, path }
}
...
mod tests {
	...
	fn init_ts_path_is_under_config_dir() {
		...
		assert_eq!(p, PathBuf::from("/tmp/dx/init.ts"));
	}
	...
	fn starter_template_references_both_dts_files() {
		assert!(
			STARTER_TEMPLATE.contains(r#"/// <reference path="./types/dx.d.ts" />"#),
			"starter template must reference dx.d.ts"
		);
		assert!(
	...
	fn write_plugin_declarations_skips_empty_export_plugins() {
		...
		assert!(body.contains("// ── dashboard ─"), "dashboard section missing: {body}");
		assert!(body.contains("DashboardApi"), "dashboard API missing: {body}");
		assert!(
			!body.contains("// ── noop ─"),
