...
use crate::input::commands::{Command, CommandSource, Suggestion, get_all_commands};
...
impl CommandRegistry {
	...
	pub fn is_terminal_bypass_action(&self, action: &crate::input::keybindings::Action) -> bool {
		...
		plugins.iter().any(|cmd| {
			cmd.terminal_bypass && matches!(&cmd.action, Action::PluginAction(name) if name == target)
		})
	}
	...
	fn history_position(&self, command_name: &str) -> Option<usize> {
		self.command_history.iter().position(|name| name == command_name)
	}
	...
	pub fn register(&self, command: Command) {
		...
		tracing::debug!("CommandRegistry::register: plugin_commands now has {} items", commands.len());
	}
	...
	pub fn unregister_by_plugin(&self, plugin_name: &str) {
		...
		if removed > 0 {
			tracing::debug!("Unregistered {} commands from plugin '{}'", removed, plugin_name);
		}
	}
	...
	pub fn filter(
		...
		let is_visible = |cmd: &Command| -> bool {
			cmd.custom_contexts.is_empty()
				|| cmd.custom_contexts.iter().all(|ctx| {
					active_custom_contexts.contains(ctx) || active_buffer_mode.is_some_and(|mode| mode == ctx)
				})
		};
		...
		let make_suggestion =
			|cmd: &Command, score: i32, localized_name: String, localized_desc: String| {
				...
				if !has_lsp_config && matches!(cmd.action, Action::LspRestart | Action::LspToggleForBuffer)
				{
					available = false;
				}
				let keybinding =
					keybinding_resolver.get_keybinding_for_action(&cmd.action, current_context_ref.clone());
		...
		suggestions.sort_by(|(a, a_hist, a_score, a_kind), (b, b_hist, b_score, b_kind)| {
...
mod tests {
	...
	fn test_filter_commands() {
		...
		let results =
			registry.filter("save", KeyContext::Normal, &keybindings, false, &empty_contexts, None, true);
	...
	fn test_context_filtering() {
		...
		let empty_contexts = std::collections::HashSet::new();
		let results =
			registry.filter("", KeyContext::Normal, &keybindings, false, &empty_contexts, None, true);
		...
		let results =
			registry.filter("", KeyContext::Popup, &keybindings, false, &empty_contexts, None, true);
	...
	fn test_required_commands_exist() {
		...
		for (name, expected_action) in required_commands {
			...
			assert!(cmd.is_some(), "Command '{}' should exist in command palette", name);
			assert_eq!(
				cmd.unwrap().action,
				expected_action,
