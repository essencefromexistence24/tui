...
impl FuzzyMatch {
	...
	pub fn no_match() -> Self {
		Self { matched: false, score: 0, match_positions: Vec::new() }
	}
}
...
/// # Examples
/// ```
/// use dx::input::fuzzy::fuzzy_match;
///
...
mod tests {
	...
	fn test_fuzzy_ordering() {
		...
		let match1 = FuzzyMatch { matched: true, score: 100, match_positions: vec![] };
		let match2 = FuzzyMatch { matched: true, score: 50, match_positions: vec![] };
	...
	fn test_exact_match_scores_highest() {
		// "dx" should score higher against "dx" than against "dx-editor"
		let exact = fuzzy_match("dx", "dx");
		let longer = fuzzy_match("dx", "dx-editor");
	...
	fn test_exact_basename_match_scores_high() {
		// "dx" matching "dx-editor" should score higher than "dx" matching "freshness"
		let basename_match = fuzzy_match("dx", "dx-editor");
		let substring_match = fuzzy_match("dx", "freshness");
	...
	fn test_basename_prefix_beats_intra_segment_match() {
		...
		let prefix = fuzzy_match("ts", "crates/dx-editor/plugins/tsconfig.json");
		for distractor in &[
			"crates/dx-editor/plugins/pkg.ts",
			"crates/dx-editor/plugins/lib/finder.ts",
			"crates/dx-editor/plugins/lib/index.ts",
			"crates/dx-editor/plugins/diagnostics_panel.ts",
		] {
	...
	fn test_directory_segment_prefix_beats_intra_segment_match() {
		...
		let intra = fuzzy_match("ts", "crates/dx-editor/plugins/pkg.ts");
	...
	fn test_basename_prefix_outranks_directory_prefix() {
		...
		let basename = fuzzy_match("ts", "crates/dx-editor/plugins/tsconfig.json");
