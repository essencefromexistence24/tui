...
impl KeyEventKey {
	...
	pub fn to_key_event(&self) -> KeyEvent {
		KeyEvent::new(self.code.to_key_code(), KeyModifiers::from_bits_truncate(self.modifiers))
	}
}
...
impl KeyTranslator {
	...
	pub fn translate(&self, raw: KeyEvent) -> KeyEvent {
		...
		if let Some(normalized) = self.translations.get(&key) { normalized.to_key_event() } else { raw }
	}
	...
	pub fn load_from_file(path: &Path) -> Result<Self, std::io::Error> {
		...
		tracing::info!("Loaded {} key translations from {}", translator.len(), path.display());
	...
	pub fn save_to_file(&self, path: &Path) -> Result<(), std::io::Error> {
		...
		let entries: Vec<TranslationEntry> = self
			.translations
			.iter()
			.map(|(raw, expected)| TranslationEntry { raw: raw.clone(), expected: expected.clone() })
			.collect();
		...
		tracing::info!("Saved {} key translations to {}", self.len(), path.display());
...
mod tests {
	...
	fn test_key_event_key_serialization() {
		let key =
			KeyEventKey { code: SerializableKeyCode::Backspace, modifiers: KeyModifiers::NONE.bits() };
