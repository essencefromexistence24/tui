...
pub enum RoutedEvent {
	...
	ToSourceBuffer { buffer_id: BufferId, action: BufferAction },
...
impl CompositeInputRouter {
	...
	pub fn route_key_event(
		...
		match (event.modifiers, event.code) {
			...
			(KeyModifiers::NONE, KeyCode::Char('k')) => RoutedEvent::CompositeScroll(ScrollAction::Up(1)),
...
mod tests {
	...
	fn test_editable_routing() {
		...
		matches!(
			result,
			RoutedEvent::ToSourceBuffer { buffer_id: BufferId(2), action: BufferAction::Insert('x') }
		);
	}
}
