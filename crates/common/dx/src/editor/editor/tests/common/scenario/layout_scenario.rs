...
use dx::test_api::{Action, EditorTestApi, VirtualLineSpec};
...
pub struct LayoutScenario {
    ...
    /// can't import `config::Config` directly (the lint
...
/// of `view::virtual_text::VirtualTextPosition`.
...
/// Declarative subset of `config::EditorConfig` flags that
/// scenario-mode tests need to set without importing
/// `config::Config` directly. Each `Some(_)` overrides the
...
pub fn check_layout_scenario(s: LayoutScenario) -> Result<(), ScenarioFailure> {
    ...
    let effective_config: Option<config::Config> = match s.config.clone() {
        Some(cfg) => Some(cfg),
