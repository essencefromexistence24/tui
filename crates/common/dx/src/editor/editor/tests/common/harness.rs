...
fn init_test_environment() {
    ...
    input::keybindings::set_force_linux_keybindings(true);
    ...
    #[cfg(feature = "plugins")]
    dx_plugin_runtime::backend::set_panic_on_js_errors(true);
}
...
use dx::model::filesystem::{FileSystem, StdFileSystem};
use dx::primitives::highlight_engine::{HighlightEngine, HighlightStats};
use dx::services::fs::{BackendMetrics, SlowFileSystem, SlowFsConfig};
use dx::services::time_source::{SharedTimeSource, TestTimeSource};
use dx::{app::Editor, config::Config};
...
/// Copy the plugins/lib directory (contains TypeScript declarations like dx.d.ts)
...
impl EditorTestHarness {
    ...
    pub fn create(width: u16, height: u16, options: HarnessOptions) -> anyhow::Result<Self> {
        ...
        if !options.preserve_keybinding_map {
            config.active_keybinding_map = config::KeybindingMapName("default".to_string());
        }
        ...
        i18n::init_with_config(config.locale.as_option());
        ...
        let grammar_registry = if options.use_full_grammar_registry {
            Some(primitives::grammar::GrammarRegistry::for_editor(
        ...
        let mut editor = Editor::for_test(
            ...
            Some(working_dir),
            dir_context.clone(),
            view::color_support::ColorCapability::TrueColor,
            filesystem,
            Some(time_source),
            grammar_registry,
    ...
    pub fn recovery_dir(&self) -> Option<PathBuf> {
        ...
        let hash = workspace::encode_path_for_filename(self.editor.working_dir());
    ...
    pub fn apply_event(&mut self, event: model::event::Event) -> anyhow::Result<()> {
    ...
    pub fn api_mut(&mut self) -> &mut dyn test_api::EditorTestApi {
        &mut self.editor
    }
    ...
    pub fn tick_and_render(&mut self) -> anyhow::Result<()> {
        let _ = app::editor_tick(&mut self.editor, || Ok(()));
        self.render()?;
        Ok(())
    }
    ...
    pub fn wait_for_async_quiescence(&mut self, idle_ticks: usize) -> anyhow::Result<()> {
        ...
        while consecutive_idle < idle_ticks {
            let did_work = app::editor_tick(&mut self.editor, || Ok(()))?;
            self.render()?;
            if did_work {
                consecutive_idle = 0;
            } else {
