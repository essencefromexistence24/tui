...
use dx::model::filesystem::StdFileSystem;
...
impl Scenario {
    ...
    fn place_workspace(&self, root: &Path, label: &str) {
        ...
        let mut ws = workspace::Workspace::new(canonical.clone());
        ws.label = Some(label.to_string());
        let filename = format!(
            "{}.json",
            workspace::encode_path_for_filename(&canonical)
        );
        std::fs::write(
    ...
    fn place_remote_workspace(&self, root: &Path, label: &str) {
        ...
        let mut ws = workspace::Workspace::new(canonical.clone());
        ws.label = Some(label.to_string());
        ws.authority_spec = services::authority::SessionAuthoritySpec::RemoteAgent(
            services::authority::RemoteAgentSpec {
                transport: services::authority::RemoteTransportSpec::Ssh {
...
/// stable assertions.
fn window_roots(editor: &app::Editor) -> Vec<PathBuf> {
    let mut roots = Vec::new();
    // Window ids are small monotonic integers; scan a generous range.
    for id in 1..=64u64 {
        if let Some(w) = editor.session(dx_core::WindowId(id)) {
...
fn restore_previous_session_false_still_picks_window_but_skips_workspace() {
    ...
    let editor = app::Editor::for_test(
        config,
        80,
        24,
