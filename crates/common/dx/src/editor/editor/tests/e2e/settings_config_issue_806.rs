...
///
/// When the user manually edits config.json while Dx is running, then uses
...
///
/// Scenario:
/// 1. User starts Dx with initial config
/// 2. User manually edits config.json (while Dx is running) to add custom LSP settings
...
fn test_issue_806_external_config_edits_lost_on_settings_save() {
    ...
    // Step 2: User manually edits config.json WHILE Dx is running
...
/// 1. Dx starts with initial config (lsp.rust.auto_start = true) LOADED INTO MEMORY
/// 2. User externally edits config.json to change auto_start to false
/// 3. User opens Settings UI in the same Dx session
...
fn test_issue_806_external_edit_then_settings_change_and_save() {
    ...
    // Step 3: User externally edits config.json (while Dx is running)
