...
use dx::workspace::get_workspace_path;
...
fn test_session_file_location() {
    ...
    assert!(
        session_path.to_string_lossy().contains("dx"),
...
fn test_session_data_integrity() {
    ...
    let restored: workspace::Workspace = serde_json::from_str(&json).unwrap();
...
fn test_session_restores_split_labels() {
    use dx::model::event::SplitId;
    use dx::services::plugins::api::PluginCommand;
...
fn test_plugin_split_terminal_not_duplicated_in_active_split() {
    use dx::services::plugins::api::PluginCommand;
    use dx::workspace::SerializedTabRef;
    ...
    let transient_id = harness.editor().active_buffer_id();
    {
            ...
            .get_mut(&active_split)
        {
            vs.open_buffers.retain(
                |t| !matches!(t, view::split::TabTarget::Buffer(b) if *b == transient_id),
            );
        }
    }
