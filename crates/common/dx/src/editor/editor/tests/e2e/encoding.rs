...
/// - Dx load: Load Windows-1250 from disk → UTF-8 → save → verify on disk
#[test]
fn test_windows1250_encoding_conversions() {
    ...
    // Part 2: Dx load Windows-1250 from disk → UTF-8 → save
...
fn test_windows1251_encoding_conversions() {
    ...
    // Part 2: Dx load Windows-1251 from disk → UTF-8 → save
    drop(harness);
    let mut harness = EditorTestHarness::new(100, 24).unwrap();
    harness.open_file(&file_path).unwrap();
    harness.render().unwrap();
