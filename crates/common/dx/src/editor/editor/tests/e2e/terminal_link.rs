//! End-to-end tests for Ctrl+Click / Ctrl+hover "open path in Dx" from the
//! embedded terminal.
//!
...
use dx::model::event::BufferId;
use dx::services::terminal::TerminalId;
...
fn open_terminal_with_output(
    ...
    let (terminal_id, buffer_id, _leaf) = harness
        .editor_mut()
        .active_window_mut()
        .create_plugin_terminal(app::PluginTerminalSpec {
...
/// (resolved against Dx's working directory) and jumps to that line.
#[test]
...
/// directory when the file doesn't exist relative to Dx's workdir.
#[test]
fn ctrl_click_resolves_path_via_osc7_cwd() {
    // Dx's workdir (no `notes.txt` here).
