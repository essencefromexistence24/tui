...
///
/// When typing "dx", the file "dx" should appear before "dx-editor"
#[test]
fn test_filter_prioritizes_exact_matches() {
    ...
    // Note: "dx-editor" would normally sort before "dx" alphabetically
    fs::write(project_root.join("dx-editor"), "content").unwrap();
    fs::write(project_root.join("fresh.rs"), "content").unwrap();
    fs::write(project_root.join("dx"), "content").unwrap();
    ...
    // Wait for files to load
    harness
        .wait_until(|h| h.screen_to_string().contains("dx"))
        .expect("Files should be listed");

    // Type filter
    harness.type_text("dx").unwrap();
    harness.render().unwrap();
    ...
    for (i, line) in lines.iter().enumerate() {
        if line.contains("dx-editor") && fresh_editor_pos.is_none() {
            fresh_editor_pos = Some(i);
        }
        // Check for exact "dx" (not followed by other chars in the filename)
        // This is tricky because "dx" appears in other names
        // We need to look for the pattern where "dx" appears at the start of a filename column
    ...
    // Also search for the exact "dx" file (without extension)
    // It should appear somewhere - look for lines with just "dx" followed by whitespace
    for (i, line) in lines.iter().enumerate() {
        // Look for "dx" followed by spaces (not followed by "-", ".", or other chars in filename)
        if let Some(idx) = line.find("dx") {
            let after_fresh = &line[idx + 5..];
            // Check if this is the exact "dx" file (next char is space or end)
            if after_fresh.starts_with(' ')
                || after_fresh.starts_with('\n')
                || after_fresh.is_empty()
            {
                // Verify it's not dx-editor, fresh.rs, or freshness
                if !line.contains("dx-")
                    && !line.contains("fresh.")
    ...
    {
        // "dx" (exact match) should appear before "freshness" (substring match)
        assert!(
            fresh < freshness,
    ...
    // Also verify "dx-editor" (exact basename + suffix) scores higher than "freshness"
    if let (Some(fresh_editor), Some(freshness)) = (fresh_editor_pos, freshness_pos) {
        assert!(
            fresh_editor < freshness,
            "Exact basename 'dx-editor' (line {}) should appear before 'freshness' (line {})\nScreen:\n{}",
            fresh_editor,
            freshness,
            screen
        );
...
fn test_file_open_shortcuts_load_async_issue_903() {
    ...
    use dx::services::fs::SlowFsConfig;
