...
// Issue #1237 — `dx -a session file.txt` should add to restored session
// =========================================================================
...
fn test_flow_d_cli_files_added_to_restored_session() {
    ...
    // Session 2: restore session but also open a CLI file (simulates `dx -a mysession file.txt`)
    {
        let mut config = Config::default();
        config.editor.hot_exit = true;
...
// another folder.
//
// Standalone (non-session) Dx launches share a single user data dir, but
...
/// After exiting Dx in folder A and then opening / closing it in folder B,
...
fn test_unnamed_buffer_survives_use_in_another_folder() {
    ...
    // running Dx in two different working directories.
    let dir_context = DirectoryContext::for_testing(temp_dir.path());
