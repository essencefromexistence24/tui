...
/// the Dx Manual viewer shares the same `"special"` buffer mode and
...
fn test_fresh_manual_q_closes_buffer() {
    ...
    assert!(
        harness.screen_to_string().contains("*Dx Manual*"),
        "Dx Manual buffer should be visible before 'q'. Screen:\n{}",
        harness.screen_to_string()
    );
    ...
    assert!(
        !after.contains("*Dx Manual*"),
        "*Dx Manual* tab should be gone after pressing q.\nScreen:\n{}",
        after
    );
}
