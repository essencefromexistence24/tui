...
#[cfg(feature = "plugins")]
use dx_core::api::BufferSavedDiff;
#[cfg(feature = "plugins")]
...
fn uri_to_path(uri: &lsp_types::Uri) -> Result<PathBuf, String> {
    dx_core::file_uri::lsp_uri_to_path(uri).ok_or_else(|| "URI is not a file path".to_string())
}
...
pub struct Editor {
    ...
    /// with dx.d.ts and tsconfig.json
    ...
    pending_async_prompt_callback: Option<dx_core::api::JsCallbackId>,
    ...
    review_hunks: Vec<dx_core::api::ReviewHunk>,
...
impl Editor {
    ...
    fn with_options(
        ...
        // 4. User plugins directory (~/.config/dx/plugins)
        // 5. Package manager installed plugins (~/.config/dx/plugins/packages/*)
    ...
    fn send_plugin_response(&self, response: dx_core::api::PluginResponse) {
    ...
    pub fn resize(&mut self, width: u16, height: u16) {
        ...
        self.plugin_manager.read().unwrap().run_hook(
            "resize",
            dx_core::hooks::HookArgs::Resize { width, height },
        );
    }
                ...
                AsyncMessage::Plugin(plugin_msg) => {
                    ...
                    match plugin_msg {
                        ...
                        PluginAsyncMessage::ProcessExit {
                            ...
                            let result = dx_core::api::BackgroundProcessResult {
                                process_id,
                                exit_code,
                            };
                            self.plugin_manager.read().unwrap().resolve_callback(
    ...
    pub fn handle_plugin_command(&mut self, command: PluginCommand) -> AnyhowResult<()> {
        match command {
                ...
                if let (Some(runtime), Some(bridge)) = (&self.tokio_runtime, &self.async_bridge) {
                    ...
                    runtime.spawn(async move {
                        ...
                        let _ = sender.send(crate::services::async_bridge::AsyncMessage::Plugin(
                            dx_core::api::PluginAsyncMessage::DelayComplete {
            ...
            PluginCommand::CreateTerminal {
                ...
                let log_path =
                    terminal_root.join(format!("dx-terminal-{}.log", predicted_terminal_id.0));
                let backing_path =
                    terminal_root.join(format!("dx-terminal-{}.txt", predicted_terminal_id.0));
                ...
                match self.terminal_manager.spawn(
                    ...
                    Ok(terminal_id) => {
                        ...
                        if terminal_id != predicted_terminal_id {
                            ...
                            let backing_path =
                                terminal_root.join(format!("dx-terminal-{}.txt", terminal_id.0));
                            self.terminal_backing_files
                        ...
                        let result = dx_core::api::TerminalResult {
                            buffer_id: buffer_id.0 as u64,
