...
fn get_type_decl(type_name: &str) -> Option<String> {
	...
	match type_name {
		...
		"SearchHandle" => Some(
			"interface SearchHandle { searchId: number; take(): SearchTakeResult; cancel(): void; }"
				.to_string(),
		),
		...
		"CreateWindowWithTerminalOptions" => {
			Some(dx_core::api::CreateWindowWithTerminalOptions::decl(&cfg))
		}
		"SessionWithTerminalResult" => Some(dx_core::api::SessionWithTerminalResult::decl(&cfg)),

		// Composite buffer types (ts-rs renames these with Ts prefix)
		"TsCompositeLayoutConfig" | "CompositeLayoutConfig" => Some(CompositeLayoutConfig::decl(&cfg)),
		"TsCompositeSourceConfig" | "CompositeSourceConfig" => Some(CompositeSourceConfig::decl(&cfg)),
		...
		"TsLspMenuItem" | "LspMenuItem" => Some(dx_core::api::LspMenuItem::decl(&cfg)),
		...
		"OffsetUnit" => Some(dx_core::text_property::OffsetUnit::decl(&cfg)),
		"StyledSegment" => Some(dx_core::text_property::StyledSegment::decl(&cfg)),
		"StyledText" => Some(dx_core::api::StyledText::decl(&cfg)),

		// Widget library types — declarative plugin UI.
		// See docs/internal/plugin-widget-library-design.md.
		"WidgetSpec" => Some(dx_core::api::WidgetSpec::decl(&cfg)),
		"HintEntry" => Some(dx_core::api::HintEntry::decl(&cfg)),
		"ButtonKind" => Some(dx_core::api::ButtonKind::decl(&cfg)),
		"WidgetAction" => Some(dx_core::api::WidgetAction::decl(&cfg)),
		"WidgetMutation" => Some(dx_core::api::WidgetMutation::decl(&cfg)),
		"TreeNode" => Some(dx_core::api::TreeNode::decl(&cfg)),

		// Authority — payload schema for `editor.setAuthority(...)`.
		// Hand-written because the authoritative struct lives in
		// `dx-editor` and this crate must not depend on it
		// (principle 3: core is opaque to backend kinds). Keep this in
		// sync with `crates/dx-editor/src/services/authority/mod.rs`.
		...
		// authoritative `RemoteAgentSpec` struct lives in `dx-editor`.
		// Keep in sync with
		// `crates/dx-editor/src/services/authority/mod.rs`.
		...
		// `dx-editor::view::ui::status_bar::RemoteIndicatorOverride`
...
/// `crates/dx-editor/src/services/authority/mod.rs`'s `RemoteAgentSpec` /
/// `RemoteTransportSpec` — this crate must not depend on `dx-editor`, so the
...
/// sync with
/// `crates/dx-editor/src/view/ui/status_bar.rs::RemoteIndicatorOverride`
...
pub fn validate_typescript(source: &str) -> Result<(), String> {
	...
	if parser_ret.errors.is_empty() {
		...
		let errors: Vec<String> =
			parser_ret.errors.iter().map(|e: &oxc_diagnostics::OxcDiagnostic| e.to_string()).collect();
...
/// Generate and write the complete dx.d.ts file
///
...
"#;
	...
	// Determine output path - write to dx-editor/plugins/lib/dx.d.ts
	let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| ".".to_string());
	let output_path = std::path::Path::new(&manifest_dir)
        .parent() // crates/
        .and_then(|p| p.parent()) // workspace root
        .map(|p| p.join("crates/dx-editor/plugins/lib/dx.d.ts"))
        .unwrap_or_else(|| std::path::PathBuf::from("plugins/lib/dx.d.ts"));
...
mod tests {
	...
	/// Generate, validate, format, and write dx.d.ts
	/// Run with: cargo test -p dx-plugin-runtime write_fresh_dts_file -- --ignored --nocapture
	#[test]
	#[ignore]
	fn write_fresh_dts_file() {
		// write_fresh_dts validates syntax and formats before writing
		write_fresh_dts().expect("Failed to write dx.d.ts");
		println!("Successfully generated, validated, and formatted dx.d.ts");
	}

	/// Type check all plugins using TypeScript compiler
	/// Skips if tsc is not available in PATH
	/// Run with: cargo test -p dx-plugin-runtime type_check_plugins -- --ignored --nocapture
	#[test]
	#[ignore]
	fn type_check_plugins() {
		...
		match tsc_check {
			Ok(output) if output.status.success() => {
				println!("Found tsc: {}", String::from_utf8_lossy(&output.stdout).trim());
			}
			_ => {
		...
		let script_path = std::path::Path::new(&manifest_dir)
			.parent()
			.and_then(|p| p.parent())
			.map(|p| p.join("crates/dx-editor/plugins/check-types.sh"))
		...
		if stdout.contains("had type errors") || !output.status.success() {
			panic!(
				"TypeScript type check failed. Run 'crates/dx-editor/plugins/check-types.sh' to see details."
			);
		}
	...
	fn test_get_type_decl_aliases_resolve_same() {
		...
		for (rust_name, ts_name) in &alias_pairs {
			...
			assert!(rust_decl.is_some(), "get_type_decl should handle Rust name '{}'", rust_name);
			assert_eq!(
				rust_decl, ts_decl,
	...
	fn test_terminal_types_exist() {
		...
		assert!(decl.contains("bufferId"), "TerminalResult should have bufferId field");
		assert!(decl.contains("terminalId"), "TerminalResult should have terminalId field");
		assert!(decl.contains("splitId"), "TerminalResult should have splitId field");

		let terminal_opts = get_type_decl("CreateTerminalOptions");
		assert!(terminal_opts.is_some(), "CreateTerminalOptions should be defined");
	}
	...
	fn test_cursor_info_type_exists() {
		...
		assert!(decl.contains("position"), "CursorInfo should have position field");
		assert!(decl.contains("selection"), "CursorInfo should have selection field");
	}
	...
	fn test_generated_dts_no_undefined_type_references() {
		...
		for line in interface_section.lines() {
			...
			for word in trimmed.split(|c: char| !c.is_alphanumeric() && c != '_') {
				...
				if word.chars().next().is_some_and(|c| c.is_uppercase()) && !defined_types.contains(word) {
	...
	fn test_editor_api_cursor_methods_have_typed_returns() {
		...
		assert!(
			...
			api.lines().find(|l| l.contains("getPrimaryCursor")).unwrap_or("not found")
		);
		...
		assert!(
			...
			api.lines().find(|l| l.contains("getAllCursors")).unwrap_or("not found")
		);
		...
		assert!(
			...
			api.lines().find(|l| l.contains("getAllCursorPositions")).unwrap_or("not found")
		);
	}
	...
	fn test_editor_api_terminal_methods_use_defined_types() {
		...
		assert!(api.contains("TerminalResult"), "createTerminal should reference TerminalResult");
	}
	...
	fn test_all_editor_api_methods_present() {
		...
		assert!(missing.is_empty(), "Missing methods in EditorAPI interface: {:?}", missing);
	}
}
