...
//!
//! In `dx-editor/src/app/plugin_commands.rs`, add the handler:
//! ```rust,ignore
...
//! ```
//!
//! And dispatch it in `dx-editor/src/app/mod.rs`.
//!
//! ## 6. Regenerate TypeScript Definitions
//!
//! Run: `cargo test -p dx-plugin-runtime write_fresh_dts_file -- --ignored`
//!
//! This validates TypeScript syntax and writes `plugins/lib/dx.d.ts`.
...
use dx_core::command::Command;
use dx_core::overlay::OverlayNamespace;
use dx_core::text_property::TextPropertyEntry;
use dx_core::{BufferId, SplitId};
use dx_parser_js::{
	bundle_module, has_es_imports, has_es_module_syntax, strip_imports_and_exports,
	transpile_typescript,
};
use dx_plugin_api_macros::{plugin_api, plugin_api_impl};
...
fn js_to_json(ctx: &rquickjs::Ctx<'_>, val: Value<'_>) -> serde_json::Value {
	...
	match val.type_of() {
		...
		Type::Int => {
			val.as_int().map(|n| serde_json::Value::Number(n.into())).unwrap_or(serde_json::Value::Null)
		}
		Type::Float => val
			.as_float()
			.map(|f| {
		...
		Type::Array => {
			if let Some(arr) = val.as_array() {
				let items: Vec<serde_json::Value> =
					arr.iter().filter_map(|item| item.ok()).map(|item| js_to_json(ctx, item)).collect();
...
fn get_text_properties_at_cursor_typed(
	...
	buffer_id: u32,
) -> dx_core::api::TextPropertiesAtCursor {
	use dx_core::api::TextPropertiesAtCursor;
...
fn js_value_to_string(ctx: &rquickjs::Ctx<'_>, val: &Value<'_>) -> String {
	...
	match val.type_of() {
		...
		Type::String => val.as_string().and_then(|s| s.to_string().ok()).unwrap_or_default(),
	...
	if err.is_exception() {
		...
		if !exc.is_undefined() && !exc.is_null() {
			...
			if let Some(exc_obj) = exc.as_object() {
				let message: String =
					exc_obj.get::<_, String>("message").unwrap_or_else(|_| "Unknown error".to_string());
...
fn parse_text_property_entry(
	...
	let style: Option<dx_core::api::OverlayOptions> =
	...
	let inline_overlays: Vec<dx_core::text_property::InlineOverlay> = obj
	...
	let pad_to_chars: Option<u32> = obj.get::<_, f64>("padToChars").ok().map(|v| v.max(0.0) as u32);
	let truncate_to_chars: Option<u32> =
		obj.get::<_, f64>("truncateToChars").ok().map(|v| v.max(0.0) as u32);

	let segments: Vec<dx_core::text_property::StyledSegment> = obj
...
pub struct PluginTrackedState {
	...
	pub terminal_ids: Vec<dx_core::TerminalId>,
...
) -> rquickjs::Result<dx_core::api::AnimationRect> {
	Ok(dx_core::api::AnimationRect {
...
fn parse_animation_kind(
	obj: &rquickjs::Object<'_>,
) -> rquickjs::Result<dx_core::api::PluginAnimationKind> {
	use dx_core::api::{PluginAnimationEdge, PluginAnimationKind};
	...
	match kind.as_str() {
		"slideIn" | "" => {
			...
			Ok(PluginAnimationKind::SlideIn { from, duration_ms, delay_ms })
		}
...
pub struct JsEditorApi {
	...
	#[qjs(skip_trace)]
	services: Arc<dyn dx_core::services::PluginServiceBridge>,
	#[qjs(skip_trace)]
...
fn parse_options<'js>(
	...
	match value {
		...
		_ => Err(throw_js(ctx, &format!("{}(\"{}\"): options must be an object", method, field))),
	}
}
...
fn require_integer<'js>(
	...
	match opts.get(key) {
		Some(v) => v.as_i64().ok_or_else(|| {
			throw_js(ctx, &format!("{}(\"{}\"): `{}` must be an integer", method, field, key))
		}),
		None => Err(throw_js(ctx, &format!("{}(\"{}\"): `{}` is required", method, field, key))),
	}
}
...
fn require_number<'js>(
	...
	match opts.get(key) {
		Some(v) => v.as_f64().ok_or_else(|| {
			throw_js(ctx, &format!("{}(\"{}\"): `{}` must be a number", method, field, key))
		}),
		None => Err(throw_js(ctx, &format!("{}(\"{}\"): `{}` is required", method, field, key))),
	}
}
...
fn check_range<'js>(
	...
	if let Some(min) = minimum {
		if default < min {
			return Err(throw_js(
				ctx,
				&format!("{}(\"{}\"): default ({}) is below minimum ({})", method, field, default, min),
			));
		}
	}
	if let Some(max) = maximum {
		if default > max {
			return Err(throw_js(
				ctx,
				&format!("{}(\"{}\"): default ({}) is above maximum ({})", method, field, default, max),
			));
		}
	}
	if let (Some(min), Some(max)) = (minimum, maximum) {
		if min > max {
			return Err(throw_js(
				ctx,
				&format!("{}(\"{}\"): minimum ({}) is greater than maximum ({})", method, field, min, max),
			));
		}
	}
	Ok(())
...
impl JsEditorApi {
	...
	fn send_field_registration(&self, field_name: &str, field_schema: serde_json::Value) {
		let _ = self.command_sender.send(PluginCommand::AddPluginConfigField {
	...
	fn hook_epoch_for(&self, buffer_id: u32) -> Option<u64> {
		self.current_hook_epoch.get().filter(|(b, _)| *b == buffer_id).map(|(_, epoch)| epoch)
	}
	...
	fn current_field_value(&self, field_name: &str) -> Option<serde_json::Value> {
		self.state_snapshot.read().ok().and_then(|s| {
			s.config.pointer(&format!("/plugins/{}/settings/{}", self.plugin_name, field_name)).cloned()
		})
	}
}
...
impl JsEditorApi {
	...
	pub fn export_plugin_api<'js>(
		...
		if name.is_empty() {
			let msg = rquickjs::String::from_str(ctx.clone(), "exportPluginApi: name must be non-empty")?;
		...
		let obj = match api.as_object() {
			...
			None => {
				let msg =
					rquickjs::String::from_str(ctx.clone(), "exportPluginApi: api must be an object")?;
				return Err(ctx.throw(msg.into_value()));
			}
		};
		let persistent = rquickjs::Persistent::save(&ctx, obj);
		self.plugin_api_exports.borrow_mut().insert(name, (self.plugin_name.clone(), persistent));
		Ok(true)
	}
	...
	pub fn get_plugin_api<'js>(
		...
		let persistent = self.plugin_api_exports.borrow().get(&name).map(|(_exporter, p)| p.clone());
		match persistent {
			Some(p) => {
	...
	pub fn has_active_search(&self) -> bool {
		self.state_snapshot.read().map(|s| s.has_active_search).unwrap_or(false)
	}
	...
	pub fn get_macro<'js>(
		...
		let steps = if let Ok(s) = self.state_snapshot.read() {
			s.macros.iter().find(|m| m.register == register).map(|m| m.steps.clone())
		} else {
			None
		};
		match steps {
	...
	pub fn define_macro(&self, register: String, steps: Vec<ActionSpec>) -> bool {
		self.command_sender.send(PluginCommand::DefineMacro { register, steps }).is_ok()
	}
	...
	pub fn play_macro(&self, register: String) -> bool {
		self.command_sender.send(PluginCommand::PlayMacroByRegister { register }).is_ok()
	}
	...
	pub fn register_command<'js>(
		...
		description: String,
		handler_name: String,
		#[plugin_api(ts_type = "string | null")] context: rquickjs::function::Opt<rquickjs::Value<'js>>,
		#[plugin_api(ts_type = "{ terminalBypass?: boolean } | null")] options: rquickjs::function::Opt<
			rquickjs::Value<'js>,
		>,
	) -> rquickjs::Result<bool> {
		...
		let tracking_key =
			if name.starts_with('%') { format!("{}:{}", plugin_name, name) } else { name.clone() };
		{
			let names = self.registered_command_names.borrow();
			if let Some(existing_plugin) = names.get(&tracking_key) {
				if existing_plugin != &plugin_name {
					let msg =
						format!("Command '{}' already registered by plugin '{}'", name, existing_plugin);
					tracing::warn!("registerCommand collision: {}", msg);
					return Err(ctx.throw(rquickjs::String::from_str(ctx.clone(), &msg)?.into_value()));
				}
		...
		// Record ownership
		self.registered_command_names.borrow_mut().insert(tracking_key, plugin_name.clone());

		// Store action handler mapping with its plugin name
		self.registered_actions.borrow_mut().insert(
			handler_name.clone(),
			PluginHandler { plugin_name: self.plugin_name.clone(), handler_name: handler_name.clone() },
		);
		...
		let terminal_bypass: bool = options
			...
			.and_then(|v| {
				if v.is_null() || v.is_undefined() {
					...
					v.into_object().and_then(|obj| obj.get::<&str, bool>("terminalBypass").ok())
				}
			})
			.unwrap_or(false);
		...
		Ok(self.command_sender.send(PluginCommand::RegisterCommand { command }).is_ok())
	}
	...
	pub fn unregister_command(&self, name: String) -> bool {
		...
		let tracking_key =
			if name.starts_with('%') { format!("{}:{}", self.plugin_name, name) } else { name.clone() };
		self.registered_command_names.borrow_mut().remove(&tracking_key);
		self.command_sender.send(PluginCommand::UnregisterCommand { name }).is_ok()
	}
	...
	pub fn set_context(&self, name: String, active: bool) -> bool {
		...
		self.command_sender.send(PluginCommand::SetContext { name, active }).is_ok()
	}

	/// Execute a built-in action
	pub fn execute_action(&self, action_name: String) -> bool {
		self.command_sender.send(PluginCommand::ExecuteAction { action_name }).is_ok()
	}
	...
	pub fn cancel_prompt(&self) -> bool {
		self.command_sender.send(PluginCommand::CancelPrompt).is_ok()
	}
	...
	pub fn register_status_bar_element(&self, token_name: String, title: String) -> bool {
		...
		self
			.command_sender
			.send(PluginCommand::RegisterStatusBarElement { plugin_name, token_name, title })
			.is_ok()
	}
	...
	pub fn set_status_bar_value(&self, buffer_id: u64, token_name: String, value: String) -> bool {
		...
		self.command_sender.send(PluginCommand::SetStatusBarValue { buffer_id, key, value }).is_ok()
	}
	...
	pub fn get_primary_cursor<'js>(&self, ctx: rquickjs::Ctx<'js>) -> rquickjs::Result<Value<'js>> {
		let cursor =
			if let Ok(s) = self.state_snapshot.read() { s.primary_cursor.clone() } else { None };
	...
	pub fn get_all_cursors<'js>(&self, ctx: rquickjs::Ctx<'js>) -> rquickjs::Result<Value<'js>> {
		let cursors =
			if let Ok(s) = self.state_snapshot.read() { s.all_cursors.clone() } else { Vec::new() };
	...
	pub fn get_viewport<'js>(&self, ctx: rquickjs::Ctx<'js>) -> rquickjs::Result<Value<'js>> {
		let viewport = if let Ok(s) = self.state_snapshot.read() { s.viewport.clone() } else { None };
	...
	pub fn get_screen_size<'js>(&self, ctx: rquickjs::Ctx<'js>) -> rquickjs::Result<Value<'js>> {
		let size = if let Ok(s) = self.state_snapshot.read() {
			dx_core::api::ScreenSize { width: s.terminal_width, height: s.terminal_height }
		} else {
			dx_core::api::ScreenSize { width: 0, height: 0 }
		};
	...
	pub fn list_splits<'js>(&self, ctx: rquickjs::Ctx<'js>) -> rquickjs::Result<Value<'js>> {
		let splits = if let Ok(s) = self.state_snapshot.read() { s.splits.clone() } else { Vec::new() };
	...
	#[plugin_api(async_promise, js_name = "getLineStartPosition", ts_return = "number | null")]
	...
	pub fn get_line_start_position_start(&self, _ctx: rquickjs::Ctx<'_>, line: u32) -> u64 {
		...
		let _ = self.command_sender.send(PluginCommand::GetLineStartPosition {
	...
	pub fn get_buffer_line_count_start(&self, _ctx: rquickjs::Ctx<'_>) -> u64 {
		...
		let _ = self
			.command_sender
			.send(PluginCommand::GetBufferLineCount { buffer_id: BufferId(0), request_id: id });
		id
	}
	...
	pub fn get_composite_cursor_info_start(&self, _ctx: rquickjs::Ctx<'_>) -> u64 {
		let id = self.alloc_request_id();
		let _ = self.command_sender.send(PluginCommand::GetCompositeCursorInfo { request_id: id });
		id
	}
	...
	pub fn get_buffer_saved_diff<'js>(
		...
		let diff = if let Ok(s) = self.state_snapshot.read() {
			s.buffer_saved_diffs.get(&BufferId(buffer_id as usize)).cloned()
		} else {
			None
		};
	...
	pub fn insert_at_cursor(&self, text: String) -> bool {
		self.command_sender.send(PluginCommand::InsertAtCursor { text }).is_ok()
	}
	...
	pub fn open_file_in_background(
		...
		self
			...
			.send(PluginCommand::OpenFileInBackground {
				path: PathBuf::from(path),
				window_id: window_id.0.map(dx_core::WindowId),
			})
			.is_ok()
	}
	...
	#[plugin_api(async_promise, js_name = "openFileStreaming", ts_return = "number | null")]
	#[qjs(rename = "_openFileStreamingStart")]
	pub fn open_file_streaming_start(&self, _ctx: rquickjs::Ctx<'_>, path: String) -> u64 {
		let id = self.alloc_request_id();
		let _ = self
			.command_sender
			.send(PluginCommand::OpenFileStreaming { path: PathBuf::from(path), request_id: id });
		id
	}
	...
	#[plugin_api(async_promise, js_name = "refreshBufferFromDisk", ts_return = "number | null")]
	#[qjs(rename = "_refreshBufferFromDiskStart")]
	pub fn refresh_buffer_from_disk_start(&self, _ctx: rquickjs::Ctx<'_>, buffer_id: u32) -> u64 {
		let id = self.alloc_request_id();
		let _ = self.command_sender.send(PluginCommand::RefreshBufferFromDisk {
			buffer_id: BufferId(buffer_id as usize),
			request_id: id,
		});
		id
	...
	pub fn show_buffer(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::ShowBuffer { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn close_buffer(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::CloseBuffer { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn close_all_buffers_in_split(&self, split_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::CloseAllBuffersInSplit { split_id: SplitId(split_id as usize) })
			.is_ok()
	}
	...
	pub fn move_tab_to_right(&self) -> bool {
		self.command_sender.send(PluginCommand::MoveTabRight).is_ok()
	}
	...
	pub fn animate_area<'js>(
		...
		let _ = self.command_sender.send(PluginCommand::StartAnimationArea { id, rect, kind });
		Ok(id)
	}
	...
	pub fn animate_virtual_buffer<'js>(
		...
		let kind = parse_animation_kind(&kind)?;
		let id = self.alloc_animation_id();
		let _ = self.command_sender.send(PluginCommand::StartAnimationVirtualBuffer {
			id,
			buffer_id: BufferId(buffer_id as usize),
			kind,
		});
	...
	pub fn cancel_animation(&self, id: u64) -> bool {
		self.command_sender.send(PluginCommand::CancelAnimation { id }).is_ok()
	}
	...
	pub fn on<'js>(&self, _ctx: rquickjs::Ctx<'js>, event_name: String, handler_name: String) {
		...
		self
			...
			.entry(event_name)
			.or_default()
			.push(PluginHandler { plugin_name: self.plugin_name.clone(), handler_name });
	}

	/// Unsubscribe from an event
	pub fn off(&self, event_name: String, handler_name: String) {
		if let Some(list) =
			self.event_handlers.write().expect("event_handlers poisoned").get_mut(&event_name)
		{
	...
	pub fn get_authority_label(&self) -> String {
		self.state_snapshot.read().map(|s| s.authority_label.clone()).unwrap_or_default()
	}
	...
	pub fn workspace_trust_level(&self) -> String {
		self.state_snapshot.read().map(|s| s.workspace_trust_level.clone()).unwrap_or_default()
	}
	...
	pub fn env_active(&self) -> bool {
		self.state_snapshot.read().map(|s| s.env_active).unwrap_or(false)
	}
	...
	pub fn detected_env(&self) -> String {
		self.state_snapshot.read().map(|s| s.detected_env.clone()).unwrap_or_default()
	}
	...
	pub fn path_join(&self, parts: rquickjs::function::Rest<String>) -> String {
		...
		if leading_slashes > 0 { format!("{}{}", prefix, joined) } else { joined }
	}
	...
	pub fn file_uri_to_path(&self, uri: String) -> String {
		dx_core::file_uri::file_uri_to_path(&uri)
	...
	pub fn path_to_file_uri(&self, path: String) -> String {
		dx_core::file_uri::path_to_file_uri(std::path::Path::new(&path)).unwrap_or_default()
	}
		...
		let entries: Vec<DirEntry> = match std::fs::read_dir(&path) {
			Ok(entries) => entries
				...
				.map(|entry| {
					...
					DirEntry { name: entry.file_name().to_string_lossy().to_string(), is_file, is_dir }
				})
				.collect(),
			Err(e) => {
	...
	/// For safety, the path must be under the OS temp directory or the Dx
	...
	pub fn remove_path(&self, path: String) -> bool {
		...
		let config_dir =
			self.services.config_dir().canonicalize().unwrap_or_else(|_| self.services.config_dir());
	...
	pub fn parse_jsonc<'js>(
		...
		let value: serde_json::Value =
			jsonc_parser::parse_to_serde_value(&text, &Default::default())
				.map_err(|e| rquickjs::Error::new_from_js_message("parseJsonc", "", &e.to_string()))?;
	...
	pub fn define_config_boolean<'js>(
		...
		ctx: rquickjs::Ctx<'js>,
		name: String,
		#[plugin_api(ts_type = "{ default: boolean; description?: string }")] options: rquickjs::Object<
			'js,
		>,
	) -> rquickjs::Result<bool> {
		...
		let default = match opts.get("default") {
			...
			_ => {
				return Err(throw_js(
					&ctx,
					&format!("defineConfigBoolean(\"{}\"): `default` (boolean) is required", name),
				));
			}
		};
		...
		Ok(self.current_field_value(&name).and_then(|v| v.as_bool()).unwrap_or(default))
	}
	...
	pub fn define_config_integer<'js>(
		...
		self.send_field_registration(&name, serde_json::Value::Object(field));
		Ok(self.current_field_value(&name).and_then(|v| v.as_i64()).unwrap_or(default))
	}
	...
	pub fn define_config_number<'js>(
		...
		self.send_field_registration(&name, serde_json::Value::Object(field));
		Ok(self.current_field_value(&name).and_then(|v| v.as_f64()).unwrap_or(default))
	}
	...
	pub fn define_config_string<'js>(
		...
		ctx: rquickjs::Ctx<'js>,
		name: String,
		#[plugin_api(ts_type = "{ default: string; description?: string }")] options: rquickjs::Object<
			'js,
		>,
		...
		let default = match opts.get("default") {
			...
			_ => {
				return Err(throw_js(
					&ctx,
					&format!("defineConfigString(\"{}\"): `default` (string) is required", name),
				));
			}
		};
		let description = string_opt(&opts, "description");
	...
	pub fn define_config_enum<'js>(
		...
		let values: Vec<String> = match opts.get("values") {
				...
				for v in arr {
					match v {
						...
						_ => {
							return Err(throw_js(
								&ctx,
								&format!("defineConfigEnum(\"{}\"): `values` must be an array of strings", name),
							));
						}
					}
				}
			...
			_ => {
				return Err(throw_js(
					&ctx,
					&format!("defineConfigEnum(\"{}\"): `values` (non-empty string[]) is required", name),
				));
			}
		};
		let default = match opts.get("default") {
			...
			_ => {
				return Err(throw_js(
					&ctx,
					&format!("defineConfigEnum(\"{}\"): `default` (string) is required", name),
				));
			}
		};
		if !values.contains(&default) {
			return Err(throw_js(
				&ctx,
				&format!("defineConfigEnum(\"{}\"): `default` must be one of {:?}", name, values),
			));
		}
		let description = string_opt(&opts, "description");
		let mut field = serde_json::Map::new();
		...
		self.send_field_registration(&name, serde_json::Value::Object(field));
		let current = self.current_field_value(&name).and_then(|v| v.as_str().map(|s| s.to_string()));
	...
	pub fn define_config_string_array<'js>(
		...
		let default: Vec<String> = match opts.get("default") {
			...
			_ => {
				return Err(throw_js(
					&ctx,
					&format!("defineConfigStringArray(\"{}\"): `default` (string[]) is required", name),
				));
			}
		};
		let description = string_opt(&opts, "description");
	...
	pub fn reload_themes(&self) {
		let _ = self.command_sender.send(PluginCommand::ReloadThemes { apply_theme: None });
	}

	/// Reload theme registry and apply a theme atomically
	pub fn reload_and_apply_theme(&self, theme_name: String) {
		let _ = self.command_sender.send(PluginCommand::ReloadThemes { apply_theme: Some(theme_name) });
	}
	...
	pub fn register_grammar<'js>(
		...
		Ok(
			self
				.command_sender
				.send(PluginCommand::RegisterGrammar { language, grammar_path, extensions })
				.is_ok(),
		)
	}
	...
	pub fn register_language_config<'js>(
		...
		Ok(self.command_sender.send(PluginCommand::RegisterLanguageConfig { language, config }).is_ok())
	}
	...
	pub fn register_lsp_server<'js>(
		...
		self.registered_lsp_servers.borrow_mut().insert(language.clone(), self.plugin_name.clone());

		Ok(self.command_sender.send(PluginCommand::RegisterLspServer { language, config }).is_ok())
	}
	...
	pub fn reload_grammars_start(&self, _ctx: rquickjs::Ctx<'_>) -> u64 {
		...
		let _ = self
			.command_sender
			.send(PluginCommand::ReloadGrammars { callback_id: dx_core::api::JsCallbackId::new(id) });
		id
	}
	...
	pub fn get_terminal_dir(&self) -> String {
		...
		self.services.terminal_dir(&working_dir).to_string_lossy().to_string()
	}
	...
	pub fn get_working_data_dir(&self) -> String {
		let working_dir = self
			...
			.map(|s| s.working_dir.clone())
			.unwrap_or_else(|_| std::path::PathBuf::from("."));
		self.services.working_data_dir(&working_dir).to_string_lossy().to_string()
	}

	/// Get themes directory path
	pub fn get_themes_dir(&self) -> String {
		self.services.config_dir().join("themes").to_string_lossy().to_string()
	}

	/// Apply a theme by name
	pub fn apply_theme(&self, theme_name: String) -> bool {
		self.command_sender.send(PluginCommand::ApplyTheme { theme_name }).is_ok()
	}
	...
	pub fn override_theme_colors<'js>(
		...
		let to_u8 = |n: &serde_json::Value| -> Option<u8> {
			n.as_i64().or_else(|| n.as_f64().map(|f| f as i64)).map(|v| v.clamp(0, 255) as u8)
		};
		...
		Ok(self.command_sender.send(PluginCommand::OverrideThemeColors { overrides: clamped }).is_ok())
	}
	...
	pub fn kill_process(&self, process_id: u64) -> bool {
		self.command_sender.send(PluginCommand::KillBackgroundProcess { process_id }).is_ok()
	}
	...
	pub fn close_composite_buffer(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::CloseCompositeBuffer { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn composite_next_hunk(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::CompositeNextHunk { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn composite_prev_hunk(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::CompositePrevHunk { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn add_overlay<'js>(
		...
		use dx_core::api::OverlayColorSpec;
	...
	pub fn clear_all_overlays(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::ClearAllOverlays { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn remove_overlay(&self, buffer_id: u32, handle: String) -> bool {
		use dx_core::overlay::OverlayHandle;
		self
			.command_sender
	...
	///
	/// Backed by the editor's own width logic (`dx_core::display_width`), so
	...
	pub fn char_width(&self, code_point: u32) -> u32 {
		char::from_u32(code_point).map(dx_core::display_width::char_width).unwrap_or(0) as u32
	}
	...
	pub fn string_width(&self, text: String) -> u32 {
		dx_core::display_width::str_width(&text) as u32
	}
	...
	pub fn clear_folds(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::ClearFolds { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn set_folding_ranges<'js>(
		...
		for obj in ranges_arr {
			...
			let kind = obj.get::<_, String>("kind").ok().and_then(|s| match s.as_str() {
		...
		Ok(
			self
				.command_sender
				.send(PluginCommand::SetFoldingRanges { buffer_id: BufferId(buffer_id as usize), ranges })
				.is_ok(),
		)
	}
	...
	pub fn submit_view_transform<'js>(
		...
		use dx_core::api::{LayoutHints, ViewTokenWire, ViewTransformPayload};
		...
		let parsed_layout_hints = if let Some(hints_obj) = layout_hints.into_inner() {
			...
			Some(LayoutHints { compose_width, column_guides })
		} else {
			None
		};
	...
	pub fn set_file_explorer_decorations<'js>(
		...
		use dx_core::file_explorer::FileExplorerDecoration;
		...
		let decorations: Vec<FileExplorerDecoration> = decorations
			...
			.map(|obj| {
				...
				let color = if color_val.is_string() {
					...
					dx_core::api::OverlayColorSpec::ThemeKey(key)
					...
					dx_core::api::OverlayColorSpec::Rgb(arr[0], arr[1], arr[2])
				} else {
					return Err(rquickjs::Error::FromJs {
						from: "value",
						to: "color",
				...
				Ok(FileExplorerDecoration { path: std::path::PathBuf::from(path), symbol, color, priority })
			})
			.collect::<rquickjs::Result<Vec<_>>>()?;
	...
	pub fn clear_file_explorer_decorations(&self, namespace: String) -> bool {
		...
		self
			.command_sender
			.send(PluginCommand::ClearFileExplorerDecorations { namespace: scoped_namespace })
			.is_ok()
	}
	...
	pub fn set_file_explorer_slots<'js>(
		...
		use dx_core::file_explorer::FileExplorerSlotEntry;
		let scoped_namespace = format!("{}::{}", self.plugin_name, namespace);
		...
		Ok(
			self
				.command_sender
				.send(PluginCommand::SetFileExplorerSlots { namespace: scoped_namespace, slots })
				.is_ok(),
		)
	}
	...
	pub fn clear_file_explorer_slots(&self, namespace: String) -> bool {
		...
		self
			.command_sender
			.send(PluginCommand::ClearFileExplorerSlots { namespace: scoped_namespace })
			.is_ok()
	}
	...
	pub fn add_virtual_text_styled<'js>(
		...
		let _ = self.command_sender.send(PluginCommand::AddVirtualTextStyled {
			buffer_id: BufferId(buffer_id as usize),
			virtual_text_id,
			position: position as usize,
			text,
	...
	pub fn clear_virtual_texts(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::ClearVirtualTexts { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn add_virtual_line<'js>(
		...
		let gutter_glyph = options.get::<_, String>("gutterGlyph").ok().filter(|s| !s.is_empty());
		...
		let text_overlays: Vec<dx_core::api::VirtualLineTextOverlay> = options
			.get::<_, rquickjs::Value<'js>>("textOverlays")
			.ok()
			.filter(|v| !v.is_undefined() && !v.is_null())
			.and_then(|v| rquickjs_serde::from_value(v).ok())
			.map(|v: Vec<dx_core::api::VirtualLineTextOverlay>| {
	...
	pub fn prompt_start(&self, _ctx: rquickjs::Ctx<'_>, label: String, initial_value: String) -> u64 {
		let id = self.alloc_request_id();
	...
	pub fn begin_key_capture(&self) -> bool {
		self.command_sender.send(PluginCommand::SetKeyCaptureActive { active: true }).is_ok()
	}
	...
	pub fn end_key_capture(&self) -> bool {
		self.command_sender.send(PluginCommand::SetKeyCaptureActive { active: false }).is_ok()
	}
	...
	pub fn get_next_key_start(&self, _ctx: rquickjs::Ctx<'_>) -> u64 {
		let id = self.alloc_request_id();
		let _ =
			self.command_sender.send(PluginCommand::AwaitNextKey { callback_id: JsCallbackId::new(id) });
		id
	}
	...
	pub fn set_prompt_suggestions(
		&self,
		suggestions: Vec<dx_core::command::Suggestion>,
	...
	pub fn set_prompt_input_sync(&self, sync: bool) -> bool {
		self.command_sender.send(PluginCommand::SetPromptInputSync { sync }).is_ok()
	}
	...
	pub fn set_prompt_title(
		&self,
		#[plugin_api(ts_type = "StyledText[]")] title: Vec<dx_core::api::StyledText>,
	) -> bool {
		self.command_sender.send(PluginCommand::SetPromptTitle { title }).is_ok()
	}
	...
	pub fn set_prompt_footer(
		&self,
		#[plugin_api(ts_type = "StyledText[]")] footer: Vec<dx_core::api::StyledText>,
	) -> bool {
		self.command_sender.send(PluginCommand::SetPromptFooter { footer }).is_ok()
	}
	...
	pub fn set_prompt_status(&self, status: String) -> bool {
		self.command_sender.send(PluginCommand::SetPromptStatus { status }).is_ok()
	}
	...
	pub fn set_prompt_toolbar<'js>(
		...
		let spec = if spec_obj.is_null() || spec_obj.is_undefined() {
			...
			match serde_json::from_value::<dx_core::api::WidgetSpec>(json) {
				Ok(s) => Some(s),
				Err(e) => {
		...
		Ok(self.command_sender.send(PluginCommand::SetPromptToolbar { spec }).is_ok())
	}
	...
	pub fn toggle_overlay_toolbar_widget(&self, key: String) -> bool {
		self.command_sender.send(PluginCommand::ToggleOverlayToolbarWidget { key }).is_ok()
	}
	...
	pub fn set_prompt_selected_index(&self, index: u32) -> bool {
		self.command_sender.send(PluginCommand::SetPromptSelectedIndex { index }).is_ok()
	}
	...
	pub fn define_mode(
		...
		let bindings: Vec<(String, String)> = bindings_arr
			.into_iter()
			.filter_map(|arr| if arr.len() >= 2 { Some((arr[0].clone(), arr[1].clone())) } else { None })
			.collect();
		...
		// and execute them in the correct plugin context
		{
			...
			for (_, cmd_name) in &bindings {
				registered.insert(
					cmd_name.clone(),
					PluginHandler { plugin_name: self.plugin_name.clone(), handler_name: cmd_name.clone() },
				);
			}
		}
	...
	pub fn set_editor_mode(&self, mode: Option<String>) -> bool {
		self.command_sender.send(PluginCommand::SetEditorMode { mode }).is_ok()
	}

	/// Get the current editor mode
	pub fn get_editor_mode(&self) -> Option<String> {
		self.state_snapshot.read().ok().and_then(|s| s.editor_mode.clone())
	}
	...
	pub fn close_split(&self, split_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::CloseSplit { split_id: SplitId(split_id as usize) })
			.is_ok()
	}
	...
	pub fn focus_split(&self, split_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::FocusSplit { split_id: SplitId(split_id as usize) })
			.is_ok()
	}
	...
	pub fn create_window(&self, root: String, label: String) -> bool {
		self
			.command_sender
			.send(PluginCommand::CreateWindow { root: std::path::PathBuf::from(root), label })
			.is_ok()
	}
	...
	pub fn set_active_window(&self, id: u64) -> bool {
		self.command_sender.send(PluginCommand::SetActiveWindow { id: dx_core::WindowId(id) }).is_ok()
	}
	...
	pub fn set_active_window_animated(&self, id: u64, from_edge: String) -> bool {
		self
			.command_sender
			.send(PluginCommand::SetActiveWindowAnimated { id: dx_core::WindowId(id), from_edge })
			.is_ok()
	}
	...
	pub fn set_window_cycle_order(&self, ids: Vec<i64>) -> bool {
		self
			...
			.send(PluginCommand::SetWindowCycleOrder {
				ids: ids.into_iter().filter(|n| *n > 0).map(|n| dx_core::WindowId(n as u64)).collect(),
			})
			.is_ok()
	}
	...
	pub fn close_window(&self, id: u64) -> bool {
		self.command_sender.send(PluginCommand::CloseWindow { id: dx_core::WindowId(id) }).is_ok()
	}
	...
	pub fn prewarm_window(&self, id: u64) -> bool {
		self.command_sender.send(PluginCommand::PrewarmWindow { id: dx_core::WindowId(id) }).is_ok()
	}
	...
	pub fn unwatch_path(&self, handle: u64) -> bool {
		self.command_sender.send(PluginCommand::UnwatchPath { handle }).is_ok()
	}
	...
	pub fn preview_window_in_rect(&self, id: u64) -> bool {
		let sid = if id == 0 { None } else { Some(dx_core::WindowId(id)) };
		self.command_sender.send(PluginCommand::PreviewWindowInRect { id: sid }).is_ok()
	}
	...
	pub fn clear_window_preview(&self) -> bool {
		self.command_sender.send(PluginCommand::PreviewWindowInRect { id: None }).is_ok()
	}
	...
	pub fn list_windows<'js>(&self, ctx: rquickjs::Ctx<'js>) -> rquickjs::Result<Value<'js>> {
		let sessions: Vec<dx_core::api::WindowInfo> =
			self.state_snapshot.read().map(|s| s.windows.clone()).unwrap_or_default();
		rquickjs_serde::to_value(ctx, &sessions)
			.map_err(|e| rquickjs::Error::new_from_js_message("serialize", "WindowInfo", &e.to_string()))
	}

	/// The currently active session id. Always present in
	/// `listWindows()`.
	pub fn active_window(&self) -> u64 {
		self.state_snapshot.read().map(|s| s.active_window_id.0).unwrap_or(1)
	}
	...
	pub fn set_split_ratio(&self, split_id: u32, ratio: f32) -> bool {
		self
			.command_sender
			.send(PluginCommand::SetSplitRatio { split_id: SplitId(split_id as usize), ratio })
			.is_ok()
	}
	...
	pub fn set_split_label(&self, split_id: u32, label: String) -> bool {
		self
			.command_sender
			.send(PluginCommand::SetSplitLabel { split_id: SplitId(split_id as usize), label })
			.is_ok()
	}
	...
	pub fn clear_split_label(&self, split_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::ClearSplitLabel { split_id: SplitId(split_id as usize) })
			.is_ok()
	}

	/// Find a split by label (async)
	#[plugin_api(async_promise, js_name = "getSplitByLabel", ts_return = "number | null")]
	#[qjs(rename = "_getSplitByLabelStart")]
	pub fn get_split_by_label_start(&self, _ctx: rquickjs::Ctx<'_>, label: String) -> u64 {
		let id = self.alloc_request_id();
		let _ = self.command_sender.send(PluginCommand::GetSplitByLabel { label, request_id: id });
		id
	}
	...
	pub fn distribute_splits_evenly(&self) -> bool {
		...
		self.command_sender.send(PluginCommand::DistributeSplitsEvenly { split_ids: vec![] }).is_ok()
	}
	...
	pub fn set_buffer_show_cursors(&self, buffer_id: u32, show: bool) -> bool {
		self
			.command_sender
			.send(PluginCommand::SetBufferShowCursors { buffer_id: BufferId(buffer_id as usize), show })
			.is_ok()
	}
	...
	pub fn set_line_numbers(&self, buffer_id: u32, enabled: bool) -> bool {
		self
			.command_sender
			.send(PluginCommand::SetLineNumbers { buffer_id: BufferId(buffer_id as usize), enabled })
			.is_ok()
	}
	...
	pub fn set_indentation_guide(&self, buffer_id: u32, enabled: bool) -> bool {
		self
			.command_sender
			.send(PluginCommand::SetIndentationGuide { buffer_id: BufferId(buffer_id as usize), enabled })
			.is_ok()
	}
	...
	pub fn set_view_mode(&self, buffer_id: u32, mode: String) -> bool {
		self
			.command_sender
			.send(PluginCommand::SetViewMode { buffer_id: BufferId(buffer_id as usize), mode })
			.is_ok()
	}
	...
	pub fn set_view_state<'js>(
		...
		let json_value =
			if value.is_undefined() || value.is_null() { None } else { Some(js_to_json(&ctx, value)) };
		...
		if let Ok(mut snapshot) = self.state_snapshot.write() {
			if let Some(ref json_val) = json_value {
				snapshot.plugin_view_states.entry(bid).or_default().insert(key.clone(), json_val.clone());
			} else {
		...
		self
			.command_sender
			.send(PluginCommand::SetViewState { buffer_id: bid, key, value: json_value })
			.is_ok()
	}
	...
	pub fn create_marker<'js>(
		...
		let marker = PluginMarker { start: start as usize, end: end as usize, payload: payload_json };
		if let Ok(mut snapshot) = self.state_snapshot.write() {
			snapshot.plugin_markers.entry(bid).or_default().insert(key, marker);
			true
		} else {
			false
		}
	...
	pub fn update_marker<'js>(
		...
		let payload_json = js_to_json(&ctx, payload);
		if let Ok(mut snapshot) = self.state_snapshot.write() {
			if let Some(m) = snapshot.plugin_markers.get_mut(&bid).and_then(|m| m.get_mut(&key)) {
	...
	pub fn get_marker<'js>(
		...
		let bid = BufferId(buffer_id as usize);
		if let Ok(snapshot) = self.state_snapshot.read() {
			if let Some(m) = snapshot.plugin_markers.get(&bid).and_then(|map| map.get(&key)) {
				let obj = rquickjs::Object::new(ctx.clone())?;
				obj.set("id", key)?;
				obj.set("start", m.start as u32)?;
				obj.set("end", m.end as u32)?;
	...
	pub fn set_window_state<'js>(
		...
		if let Ok(mut snapshot) = self.state_snapshot.write() {
			match &json_value {
				...
				None => {
					if let Some(map) = snapshot.active_session_plugin_states.get_mut(&self.plugin_name) {
						map.remove(&key);
						if map.is_empty() {
							snapshot.active_session_plugin_states.remove(&self.plugin_name);
						}
					}
				}
			}
	...
	pub fn set_scroll_sync_anchors<'js>(
		...
		let anchors: Vec<(usize, usize)> = anchors
			...
			.filter_map(
				|pair| {
					if pair.len() >= 2 { Some((pair[0] as usize, pair[1] as usize)) } else { None }
				},
			)
			.collect();
		self.command_sender.send(PluginCommand::SetScrollSyncAnchors { group_id, anchors }).is_ok()
	}

	/// Remove a scroll sync group
	pub fn remove_scroll_sync_group(&self, group_id: u32) -> bool {
		self.command_sender.send(PluginCommand::RemoveScrollSyncGroup { group_id }).is_ok()
	}
	...
	pub fn execute_actions(&self, actions: Vec<ActionSpec>) -> bool {
		self.command_sender.send(PluginCommand::ExecuteActions { actions }).is_ok()
	}

	/// Show an action popup
	///
	/// Takes a typed ActionPopupOptions struct - serde validates field names at runtime
	pub fn show_action_popup(&self, opts: dx_core::api::ActionPopupOptions) -> bool {
		self
			.command_sender
	...
	pub fn set_lsp_menu_contributions(
		...
		plugin_id: String,
		language: String,
		items: Vec<dx_core::api::LspMenuItem>,
	) -> bool {
		self
			.command_sender
			.send(PluginCommand::SetLspMenuContributions { plugin_id, language, items })
			.is_ok()
	}

	/// Disable LSP for a specific language
	pub fn disable_lsp_for_language(&self, language: String) -> bool {
		self.command_sender.send(PluginCommand::DisableLspForLanguage { language }).is_ok()
	}

	/// Restart LSP server for a specific language
	pub fn restart_lsp_for_language(&self, language: String) -> bool {
		self.command_sender.send(PluginCommand::RestartLspForLanguage { language }).is_ok()
	}
	...
	pub fn register_lsp_uri_scheme(&self, scheme: String) -> bool {
		self.command_sender.send(PluginCommand::RegisterLspUriScheme { scheme }).is_ok()
	}
	...
	pub fn mark_file_read_only(&self, path: String) -> bool {
		self
			.command_sender
			.send(PluginCommand::MarkBufferReadOnly { path: std::path::PathBuf::from(path) })
			.is_ok()
	}
	...
	pub fn set_lsp_root_uri(&self, language: String, uri: String) -> bool {
		self.command_sender.send(PluginCommand::SetLspRootUri { language, uri }).is_ok()
	}
	...
	pub fn get_all_diagnostics<'js>(&self, ctx: rquickjs::Ctx<'js>) -> rquickjs::Result<Value<'js>> {
		use dx_core::api::{JsDiagnostic, JsPosition, JsRange};
				...
				for diag in diags {
					result.push(JsDiagnostic {
						...
						range: JsRange {
							...
							end: JsPosition { line: diag.range.end.line, character: diag.range.end.character },
						},
	...
	#[plugin_api(async_promise, js_name = "createVirtualBuffer", ts_return = "VirtualBufferResult")]
	#[qjs(rename = "_createVirtualBufferStart")]
	pub fn create_virtual_buffer_start(
		&self,
		_ctx: rquickjs::Ctx<'_>,
		opts: dx_core::api::CreateVirtualBufferOptions,
	) -> rquickjs::Result<u64> {
		let id = self.alloc_request_id();
	...
	pub fn create_virtual_buffer_in_split_start(
		&self,
		_ctx: rquickjs::Ctx<'_>,
		opts: dx_core::api::CreateVirtualBufferInSplitOptions,
	) -> rquickjs::Result<u64> {
		let id = self.alloc_request_id();
	...
	pub fn create_virtual_buffer_in_existing_split_start(
		&self,
		_ctx: rquickjs::Ctx<'_>,
		opts: dx_core::api::CreateVirtualBufferInExistingSplitOptions,
	) -> rquickjs::Result<u64> {
		let id = self.alloc_request_id();
	...
	pub fn set_panel_content<'js>(
		...
		let entries: Vec<TextPropertyEntry> =
			entries_arr.iter().filter_map(|obj| parse_text_property_entry(&ctx, obj)).collect();
		Ok(
			self
				.command_sender
				.send(PluginCommand::SetPanelContent { group_id: group_id as usize, panel_name, entries })
				.is_ok(),
		)
	}
	...
	pub fn close_buffer_group(&self, group_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::CloseBufferGroup { group_id: group_id as usize })
			.is_ok()
	}
	...
	pub fn focus_buffer_group_panel(&self, group_id: u32, panel_name: String) -> bool {
		self
			.command_sender
			.send(PluginCommand::FocusPanel { group_id: group_id as usize, panel_name })
			.is_ok()
	}
	...
	#[plugin_api(async_promise, js_name = "setBufferGroupPanelBuffer", ts_return = "boolean")]
	...
	pub fn set_buffer_group_panel_buffer_start(
		...
		let id = self.alloc_request_id();
		let _ = self.command_sender.send(PluginCommand::SetBufferGroupPanelBuffer {
	...
	///
	/// `spec` is a `WidgetSpec` JSON tree (see dx.d.ts for the
	...
	pub fn mount_widget_panel<'js>(
		...
		let json = js_to_json(&ctx, spec_obj);
		let spec: dx_core::api::WidgetSpec = match serde_json::from_value(json) {
			Ok(s) => s,
			Err(e) => {
	...
	///
	/// `action` is a `WidgetAction` JSON object — see dx.d.ts for
	...
	pub fn widget_command<'js>(
		...
		let action: dx_core::api::WidgetAction = match serde_json::from_value(json) {
			Ok(a) => a,
			Err(e) => {
	...
	/// See `WidgetMutation` in dx.d.ts for the shapes.
	...
	pub fn widget_mutate<'js>(
		...
		let mutation: dx_core::api::WidgetMutation = match serde_json::from_value(json) {
			Ok(m) => m,
			Err(e) => {
	...
	pub fn spawn_process_start(
		...
		let effective_cwd = cwd.0.filter(|s| !s.is_empty()).or_else(|| {
			self.state_snapshot.read().ok().map(|s| s.working_dir.to_string_lossy().to_string())
		});
		let stdout_to_path = stdout_to.0.filter(|s| !s.is_empty()).map(std::path::PathBuf::from);
		tracing::info!(
	...
	/// as `spawnProcess`.
	#[plugin_api(async_thenable, js_name = "spawnHostProcess", ts_return = "SpawnResult")]
	...
	pub fn kill_host_process(&self, process_id: u64) -> bool {
		self.command_sender.send(PluginCommand::KillHostProcess { process_id }).is_ok()
	}
	...
	/// the `AuthorityPayload` type in `dx-editor`; plugins should
	...
	pub fn set_authority(
		...
		let _ = self.command_sender.send(PluginCommand::SetAuthority { payload: json });
		true
	}
	...
	///
	/// The payload schema (`RemoteAgentSpec`) lives in `dx-editor`;
	...
	pub fn attach_remote_agent(
		...
		let id = self.alloc_request_id();
		let _ =
			self.command_sender.send(PluginCommand::AttachRemoteAgent { payload: json, request_id: id });
		id
	}
	...
	pub fn set_env(&self, snippet: String, dir: Option<String>) {
		let _ = self.command_sender.send(PluginCommand::SetEnv { snippet, dir });
	}
	...
	pub fn set_remote_indicator_state(
		...
		let _ = self.command_sender.send(PluginCommand::SetRemoteIndicatorState { state: json });
		true
	}
	...
	pub fn clear_remote_indicator_state(&self) {
		let _ = self.command_sender.send(PluginCommand::ClearRemoteIndicatorState);
	}
	...
	pub fn http_fetch_start(&self, _ctx: rquickjs::Ctx<'_>, url: String, target_path: String) -> u64 {
		let id = self.alloc_request_id();
		tracing::info!(
	...
	pub fn spawn_process_wait_start(&self, _ctx: rquickjs::Ctx<'_>, process_id: u64) -> u64 {
		...
		let _ = self
			.command_sender
			.send(PluginCommand::SpawnProcessWait { process_id, callback_id: JsCallbackId::new(id) });
		id
	}
	...
	pub fn delay_start(&self, _ctx: rquickjs::Ctx<'_>, duration_ms: u64) -> u64 {
		...
		let _ = self
			.command_sender
			.send(PluginCommand::Delay { callback_id: JsCallbackId::new(id), duration_ms });
		id
	}
	...
	pub fn search_handle_take<'js>(
		...
		let entry = self.search_handles.lock().ok().and_then(|m| m.get(&handle_id).cloned());
	...
	pub fn replace_in_file_start(
		...
		let match_pairs: Vec<(usize, usize)> =
			matches.iter().map(|m| (m[0] as usize, m[1] as usize)).collect();
	...
	pub fn create_terminal_start(
		&self,
		_ctx: rquickjs::Ctx<'_>,
		opts: rquickjs::function::Opt<dx_core::api::CreateTerminalOptions>,
	) -> rquickjs::Result<u64> {
		let id = self.alloc_request_id();

		let opts = opts.0.unwrap_or(dx_core::api::CreateTerminalOptions {
			cwd: None,
			direction: None,
			ratio: None,
			focus: None,
	...
	pub fn create_window_with_terminal_start(
		&self,
		_ctx: rquickjs::Ctx<'_>,
		opts: dx_core::api::CreateWindowWithTerminalOptions,
	) -> rquickjs::Result<u64> {
		let id = self.alloc_request_id();
		if let Ok(mut owners) = self.async_resource_owners.lock() {
			owners.insert(id, self.plugin_name.clone());
		}
		let _ = self.command_sender.send(PluginCommand::CreateWindowWithTerminal {
	...
	pub fn send_terminal_input(&self, terminal_id: u64, data: String) -> bool {
		self
			...
			.send(PluginCommand::SendTerminalInput {
				terminal_id: dx_core::TerminalId(terminal_id as usize),
				data,
			})
			.is_ok()
	}
	...
	pub fn close_terminal(&self, terminal_id: u64) -> bool {
		self
			.command_sender
			.send(PluginCommand::CloseTerminal { terminal_id: dx_core::TerminalId(terminal_id as usize) })
			.is_ok()
	}
	...
	pub fn signal_window(&self, id: f64, signal: String) -> bool {
		self
			.command_sender
			.send(PluginCommand::SignalWindow { id: dx_core::WindowId(id as u64), signal })
			.is_ok()
	}
	...
	pub fn refresh_lines(&self, buffer_id: u32) -> bool {
		self
			.command_sender
			.send(PluginCommand::RefreshLines { buffer_id: BufferId(buffer_id as usize) })
			.is_ok()
	}
	...
	pub fn unload_plugin_start(&self, _ctx: rquickjs::Ctx<'_>, name: String) -> u64 {
		...
		let _ = self
			.command_sender
			.send(PluginCommand::UnloadPlugin { name, callback_id: JsCallbackId::new(id) });
		id
	}
	...
	pub fn reload_plugin_start(&self, _ctx: rquickjs::Ctx<'_>, name: String) -> u64 {
		...
		let _ = self
			.command_sender
			.send(PluginCommand::ReloadPlugin { name, callback_id: JsCallbackId::new(id) });
		id
	}
	...
	pub fn list_plugins_start(&self, _ctx: rquickjs::Ctx<'_>) -> u64 {
		let id = self.alloc_request_id();
		let _ =
			self.command_sender.send(PluginCommand::ListPlugins { callback_id: JsCallbackId::new(id) });
		id
	}
}
...
fn parse_view_token(
	obj: &rquickjs::Object<'_>,
	idx: usize,
) -> rquickjs::Result<dx_core::api::ViewTokenWire> {
	use dx_core::api::{ViewTokenWire, ViewTokenWireKind};
	...
	let source_offset: Option<usize> =
		obj.get("sourceOffset").ok().or_else(|| obj.get("source_offset").ok());
	...
	Ok(ViewTokenWire { source_offset, kind, style })
}
...
fn parse_view_token_style(
	obj: &rquickjs::Object<'_>,
	idx: usize,
) -> rquickjs::Result<Option<dx_core::api::ViewTokenStyle>> {
	use dx_core::api::{TokenColor, ViewTokenStyle};
...
pub struct QuickJsBackend {
	...
	pub services: Arc<dyn dx_core::services::PluginServiceBridge>,
...
impl QuickJsBackend {
	...
	pub fn new() -> Result<Self> {
		...
		let services = Arc::new(dx_core::services::NoopServiceBridge);
	...
	pub fn with_state_responses_and_resources(
		...
		let main_context =
			Context::full(&runtime).map_err(|e| anyhow!("Failed to create QuickJS context: {}", e))?;
	...
	pub async fn load_module_with_source(&mut self, path: &str, _plugin_source: &str) -> Result<()> {
		...
		let filename = path_buf.file_name().and_then(|s| s.to_str()).unwrap_or("plugin.ts");
		...
		if has_es_imports(&source) {
			...
			match bundle_module(&path_buf) {
				...
				Err(e) => {
					tracing::warn!("Plugin {} uses ES imports but bundling failed: {}. Skipping.", path, e);
			...
			let js_code =
				if filename.ends_with(".ts") { transpile_typescript(&source, filename)? } else { source };
			self.execute_js(&js_code, path)?;
		}
	...
	pub(crate) fn execute_js(&mut self, code: &str, source_name: &str) -> Result<()> {
		...
		let plugin_name =
			Path::new(source_name).file_stem().and_then(|s| s.to_str()).unwrap_or("unknown");
		...
		let context = {
			...
			if let Some(ctx) = contexts.get(plugin_name) {
				...
				let ctx = Context::full(&self.runtime).map_err(|e| {
					anyhow!("Failed to create QuickJS context for plugin {}: {}", plugin_name, e)
				})?;
	...
	pub fn cleanup_plugin(&self, plugin_name: &str) {
		...
		{
			let mut handlers_map = self.event_handlers.write().expect("event_handlers poisoned");
		...
		self.registered_actions.borrow_mut().retain(|_, h| h.plugin_name != plugin_name);

		// 4. Remove callback contexts for this plugin
		self.callback_contexts.borrow_mut().retain(|_, pname| pname != plugin_name);
		...
		if let Some(tracked) = self.plugin_tracked_state.borrow_mut().remove(plugin_name) {
			...
			for (buf_id, ns) in &tracked.overlay_namespaces {
				if seen_overlay_ns.insert((buf_id.0, ns.clone())) {
					...
					let _ = self.command_sender.send(PluginCommand::ClearConcealNamespace {
						buffer_id: *buf_id,
						namespace: OverlayNamespace::from_string(ns.clone()),
					});
					let _ = self.command_sender.send(PluginCommand::ClearSoftBreakNamespace {
						buffer_id: *buf_id,
						namespace: OverlayNamespace::from_string(ns.clone()),
					});
				}
			...
			for (buf_id, ns) in &tracked.line_indicator_namespaces {
				if seen_li_ns.insert((buf_id.0, ns.clone())) {
					let _ = self
						.command_sender
						.send(PluginCommand::ClearLineIndicators { buffer_id: *buf_id, namespace: ns.clone() });
				}
			}
			...
			let mut seen_fe_ns: std::collections::HashSet<String> = std::collections::HashSet::new();
			for ns in &tracked.file_explorer_namespaces {
				if seen_fe_ns.insert(ns.clone()) {
					let _ = self
						.command_sender
						.send(PluginCommand::ClearFileExplorerDecorations { namespace: ns.clone() });
					let _ = self
						.command_sender
						.send(PluginCommand::ClearFileExplorerSlots { namespace: ns.clone() });
				}
			}
			...
			for ctx_name in &tracked.contexts_set {
				if seen_ctx.insert(ctx_name.clone()) {
					let _ = self
						.command_sender
						.send(PluginCommand::SetContext { name: ctx_name.clone(), active: false });
				}
			}
			...
			for process_id in &tracked.background_process_ids {
				let _ = self
					.command_sender
					.send(PluginCommand::KillBackgroundProcess { process_id: *process_id });
			}

			// Remove scroll sync groups created by this plugin
			for group_id in &tracked.scroll_sync_group_ids {
				let _ =
					self.command_sender.send(PluginCommand::RemoveScrollSyncGroup { group_id: *group_id });
			}

			// Close virtual buffers created by this plugin
			for buffer_id in &tracked.virtual_buffer_ids {
				let _ = self.command_sender.send(PluginCommand::CloseBuffer { buffer_id: *buffer_id });
			}

			// Close composite buffers created by this plugin
			for buffer_id in &tracked.composite_buffer_ids {
				let _ =
					self.command_sender.send(PluginCommand::CloseCompositeBuffer { buffer_id: *buffer_id });
			}

			// Close terminals created by this plugin
			for terminal_id in &tracked.terminal_ids {
				let _ =
					self.command_sender.send(PluginCommand::CloseTerminal { terminal_id: *terminal_id });
			}
			...
			for handle in &tracked.watch_handles {
				let _ = self.command_sender.send(PluginCommand::UnwatchPath { handle: *handle });
			}
		}
		...
		self.plugin_api_exports.borrow_mut().retain(|_, (exporter, _)| exporter != plugin_name);

		// Clear collision tracking maps so another plugin can re-register these names
		self.registered_command_names.borrow_mut().retain(|_, pname| pname != plugin_name);
		self.registered_grammar_languages.borrow_mut().retain(|_, pname| pname != plugin_name);
		self.registered_language_configs.borrow_mut().retain(|_, pname| pname != plugin_name);
		self.registered_lsp_servers.borrow_mut().retain(|_, pname| pname != plugin_name);

		tracing::debug!("cleanup_plugin: cleaned up runtime state for plugin '{}'", plugin_name);
	}
	...
	pub async fn emit_to(
		...
		self.services.set_js_execution_state(format!("hook '{}'", event_name));
		...
		let handlers =
			self.event_handlers.read().expect("event_handlers poisoned").get(event_name).cloned();
	...
	pub fn start_action(&mut self, action_name: &str) -> Result<()> {
		...
		let plugin_contexts = self.plugin_contexts.borrow();
		let context = plugin_contexts.get(&plugin_name).unwrap_or(&self.main_context);
	...
	pub fn send_hook_completed(&self, hook_name: String) {
		let _ = self.command_sender.send(PluginCommand::HookCompleted { hook_name });
	}
	...
	pub fn resolve_callback(&mut self, callback_id: dx_core::api::JsCallbackId, result_json: &str) {
		...
		context.with(|ctx| {
			...
			let json_value: serde_json::Value = match serde_json::from_str(result_json) {
				Ok(v) => v,
				Err(e) => {
					tracing::error!("resolve_callback: failed to parse JSON for callback_id={}: {}", id, e);
					return;
				}
			};
	...
	pub fn reject_callback(&mut self, callback_id: dx_core::api::JsCallbackId, error: &str) {
		let id = callback_id.as_u64();
...
mod tests {
	use super::*;
	use dx_core::api::{BufferInfo, CursorInfo};
	use std::sync::mpsc;
	...
	impl TestServiceBridge {
		fn new() -> Self {
			Self { en_strings: std::sync::Mutex::new(HashMap::new()) }
		}
	}

	impl dx_core::services::PluginServiceBridge for TestServiceBridge {
		fn as_any(&self) -> &dyn std::any::Any {
			self
		}
		fn translate(&self, _plugin_name: &str, key: &str, _args: &HashMap<String, String>) -> String {
			self.en_strings.lock().unwrap().get(key).cloned().unwrap_or_else(|| key.to_string())
		}
		...
		fn register_command(&self, _command: dx_core::command::Command) {}
	...
	fn test_event_handler_registration() {
		...
		// Register a handler
		backend.event_handlers.write().unwrap().entry("test_event".to_string()).or_default().push(
			PluginHandler { plugin_name: "test".to_string(), handler_name: "testHandler".to_string() },
		);
	...
	async fn test_execute_action_sync_function() {
		...
		backend.registered_actions.borrow_mut().insert(
			...
			PluginHandler { plugin_name: "test".to_string(), handler_name: "my_sync_action".to_string() },
		);
	...
	async fn test_emit_to_targets_single_plugin() {
		...
		backend.emit_to("widget_event", &event_data, Some("beta")).await.unwrap();
		match rx.try_recv().unwrap() {
			PluginCommand::SetStatus { message } => assert_eq!(message, "beta got 7"),
			cmd => panic!("Expected SetStatus, got {:?}", cmd),
		}
		assert!(rx.try_recv().is_err(), "targeted emit must not reach the other plugin");

		backend.emit_to("widget_event", &event_data, None).await.unwrap();
	...
	async fn test_emit_event_preserves_integers_beyond_i32() {
		...
		match cmd {
			PluginCommand::SetStatus { message } => {
				assert_eq!(message, "big: 4503599627370001 neg: -4503599627370001 small: 42");
			}
			_ => panic!("Expected SetStatus from event handler, got {:?}", cmd),
		}
	}
	...
	fn test_api_get_cursor_position_from_state() {
		...
		// Set up cursor position in state
		{
			...
			state.primary_cursor = Some(CursorInfo { position: 42, selection: None, line: Some(0) });
		}
	...
	fn test_api_get_cursor_line_small_and_large_file() {
		...
		{
			let mut state = state_snapshot.write().unwrap();
			state.primary_cursor = Some(CursorInfo { position: 120, selection: None, line: Some(7) });
			state.all_cursors = vec![
				CursorInfo { position: 120, selection: None, line: Some(7) },
				CursorInfo { position: 200, selection: None, line: Some(12) },
			];
		}
		...
		backend.plugin_contexts.borrow().get("probe_small").unwrap().clone().with(|ctx| {
			let global = ctx.globals();
		...
		let state_snapshot2 = Arc::new(RwLock::new(EditorStateSnapshot::new()));
		{
			...
			state.primary_cursor = Some(CursorInfo { position: 5_000_000, selection: None, line: None });
			state.all_cursors = vec![CursorInfo { position: 5_000_000, selection: None, line: None }];
		}

		let services2 = Arc::new(dx_core::services::NoopServiceBridge);
		...
		backend2.plugin_contexts.borrow().get("probe_large").unwrap().clone().with(|ctx| {
			let global = ctx.globals();
			// The honest API reports "unknown" as null in large-file mode.
			let primary_null: bool = global.get("_primaryLineIsNull").unwrap();
			assert!(primary_null, "primary.line should be null in large-file mode");
			let all_null: bool = global.get("_allLineIsNull").unwrap();
			assert!(all_null, "getAllCursors()[0].line should be null in large-file mode");
	...
	fn test_file_uri_to_path_and_back() {
		...
		backend.plugin_contexts.borrow().get("test").unwrap().clone().with(|ctx| {
			...
			#[cfg(not(windows))]
			{
				assert_eq!(global.get::<_, String>("_path1").unwrap(), "/home/user/file.txt");
				assert_eq!(global.get::<_, String>("_path2").unwrap(), "/home/user/my file.txt");
				assert_eq!(global.get::<_, String>("_path3").unwrap(), "");
				assert_eq!(global.get::<_, String>("_uri1").unwrap(), "file:///home/user/file.txt");
				assert_eq!(global.get::<_, String>("_roundtrip").unwrap(), "/home/user/file.txt");
			}

			#[cfg(windows)]
			{
				// Issue #1071: encoded colon must be decoded to proper Windows path
				assert_eq!(global.get::<_, String>("_path1").unwrap(), "C:\\Users\\admin\\Repos\\file.cs");
				assert_eq!(global.get::<_, String>("_path2").unwrap(), "C:\\Users\\admin\\Repos\\file.cs");
				assert_eq!(global.get::<_, String>("_path3").unwrap(), "");
				assert_eq!(
	...
	fn test_typescript_transpilation() {
		use dx_parser_js::transpile_typescript;
	...
	fn test_plugin_translation_with_registered_strings() {
		...
		if let Some(bridge) = backend.services.as_any().downcast_ref::<TestServiceBridge>() {
	...
	fn test_api_add_overlay() {
		...
		let cmd = rx.try_recv().unwrap();
		match cmd {
			PluginCommand::AddOverlay { buffer_id, namespace, range, options } => {
				use dx_core::api::OverlayColorSpec;
				assert_eq!(buffer_id.0, 1);
				assert!(namespace.is_some());
				assert_eq!(namespace.unwrap().as_str(), "highlight");
				assert_eq!(range, 10..20);
				assert!(matches!(options.fg, Some(OverlayColorSpec::Rgb(255, 128, 0))));
				assert!(matches!(options.bg, Some(OverlayColorSpec::Rgb(50, 50, 50))));
	...
	fn test_api_override_theme_colors_drops_malformed_entries() {
		...
		match rx.try_recv().unwrap() {
			PluginCommand::OverrideThemeColors { overrides } => {
				...
				assert_eq!(overrides.get("floats_are_fine").copied(), Some([10, 20, 30]));
			}
			other => panic!("Expected OverrideThemeColors, got {other:?}"),
		}
	}
	...
	fn test_api_get_theme_data_present() {
		...
		let (tx, _rx) = mpsc::channel();
		let state_snapshot = Arc::new(RwLock::new(EditorStateSnapshot::new()));
		let services = Arc::new(ThemeCacheTestBridge { inner: TestServiceBridge::new() });
		let mut backend = QuickJsBackend::with_state(state_snapshot, tx, services).unwrap();
	...
	impl dx_core::services::PluginServiceBridge for ThemeCacheTestBridge {
		fn as_any(&self) -> &dyn std::any::Any {
			self
		}
		...
		fn register_command(&self, command: dx_core::command::Command) {
	...
	fn test_api_session_lifecycle_dispatches_commands() {
		...
		match create {
			dx_core::api::PluginCommand::CreateWindow { root, label } => {
		...
		match activate {
			dx_core::api::PluginCommand::SetActiveWindow { id } => {
				assert_eq!(id, dx_core::WindowId(7));
			}
		...
		match close {
			dx_core::api::PluginCommand::CloseWindow { id } => {
				assert_eq!(id, dx_core::WindowId(3));
			}
	...
	fn test_api_list_sessions_reads_snapshot() {
...

		{
			let mut state = state_snapshot.write().unwrap();
			state.windows = vec![
				dx_core::api::WindowInfo {
					id: dx_core::WindowId(1),
					label: "main".into(),
			...
			state.active_window_id = dx_core::WindowId(2);
		}
	...
	fn test_typescript_preamble_generated() {
		...
		println!("Generated {} bytes of TypeScript preamble", JSEDITORAPI_TS_PREAMBLE.len());
	}
	...
	fn test_typescript_editor_api_generated() {
		...
		println!("Generated {} bytes of EditorAPI interface", JSEDITORAPI_TS_EDITOR_API.len());
	}
	...
	fn test_api_set_global_state() {
		...
		let cmd = rx.try_recv().unwrap();
		match cmd {
			PluginCommand::SetGlobalState { plugin_name, key, value } => {
	...
	fn test_api_get_global_state_roundtrip() {
		...
		backend.plugin_contexts.borrow().get("test_plugin").unwrap().clone().with(|ctx| {
			let global = ctx.globals();
			let result: bool = global.get("_result").unwrap();
			assert!(result, "getGlobalState should return the value set by setGlobalState");
		});
	}
	...
	fn test_api_set_session_state_roundtrip() {
		...
		backend.plugin_contexts.borrow().get("test_plugin").unwrap().clone().with(|ctx| {
			let global = ctx.globals();
			let count: i64 = global.get::<_, rquickjs::Object>("_result").unwrap().get("count").unwrap();
			assert_eq!(count, 7, "getWindowState should return the value set by setWindowState");
			let missing = global.get::<_, rquickjs::Value>("_missing").unwrap();
			assert!(missing.is_undefined(), "getWindowState for an unset key must be undefined");
		});
	}
	...
	fn test_api_get_global_state_missing_key() {
		...
		backend.plugin_contexts.borrow().get("test_plugin").unwrap().clone().with(|ctx| {
			...
			assert!(is_undefined, "getGlobalState for missing key should return undefined");
		});
	}
	...
	fn test_api_global_state_isolation_between_plugins() {
		...
		let mut backend_a =
			QuickJsBackend::with_state(state_snapshot.clone(), tx.clone(), services.clone()).unwrap();
		backend_a
			.execute_js(
				r#"
            const editor = getEditor();
		...
		backend_a.plugin_contexts.borrow().get("plugin_a").unwrap().clone().with(|ctx| {
			let global = ctx.globals();
			let a_value: String = global.get("_aValue").unwrap();
			assert_eq!(a_value, "from_plugin_a", "Plugin A should see its own value, not plugin B's");
		});
		...
		backend_b.plugin_contexts.borrow().get("plugin_b").unwrap().clone().with(|ctx| {
			let global = ctx.globals();
			let b_value: String = global.get("_bValue").unwrap();
			assert_eq!(b_value, "from_plugin_b", "Plugin B should see its own value, not plugin A's");
		});
	}
	...
	fn test_register_command_collision_different_plugins() {
		...
		assert!(result.is_err(), "Second plugin registering the same command name should fail");
		let err_msg = result.unwrap_err().to_string();
		assert!(err_msg.contains("already registered"), "Error should mention collision: {}", err_msg);
	}
	...
	fn test_register_command_non_i18n_still_collides() {
		...
		assert!(result.is_err(), "Non-%-prefixed names should still collide across plugins");
	}
}
