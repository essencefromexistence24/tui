...
use dx_core::api::PluginAsyncMessage as AsyncMessage;
...
mod tests {
	...
	async fn test_spawn_simple_command() {
		...
		spawn_plugin_process(1, "echo".to_string(), vec!["hello".to_string()], None, sender).await;

		let msg = receiver.recv().unwrap();
		match msg {
			AsyncMessage::ProcessOutput { process_id, stdout, stderr, exit_code } => {
	...
	async fn test_spawn_with_args() {
		...
		let msg = receiver.recv().unwrap();
		match msg {
			AsyncMessage::ProcessOutput { process_id, stdout, exit_code, .. } => {
	...
	async fn test_spawn_nonexistent_command() {
		...
		spawn_plugin_process(3, "this_command_does_not_exist_12345".to_string(), vec![], None, sender)
			.await;
	...
	async fn test_spawn_failing_command() {
		...
		let msg = receiver.recv().unwrap();
		match msg {
			AsyncMessage::ProcessOutput { process_id, exit_code, .. } => {
