...
use dx_core::api::{EditorStateSnapshot, JsCallbackId, PluginCommand, SearchHandleRegistry};
use dx_core::hooks::HookArgs;
...
pub use dx_core::config::PluginConfig;
...
pub enum PluginRequest {
	...
	LoadPlugin { path: PathBuf, response: oneshot::Sender<Result<()>> },

	/// Resolve an async callback with a result (for async operations like SpawnProcess, Delay)
	ResolveCallback { callback_id: dx_core::api::JsCallbackId, result_json: String },

	/// Reject an async callback with an error
	RejectCallback { callback_id: dx_core::api::JsCallbackId, error: String },

	/// Load all plugins from a directory
	LoadPluginsFromDir { dir: PathBuf, response: oneshot::Sender<Vec<String>> },
	...
	UnloadPlugin { name: String, response: oneshot::Sender<Result<()>> },

	/// Reload a plugin by name
	ReloadPlugin { name: String, response: oneshot::Sender<Result<()>> },

	/// Execute a plugin action
	ExecuteAction { action_name: String, response: oneshot::Sender<Result<()>> },
	...
	RunHook { hook_name: String, args: HookArgs, target: Option<String> },

	/// Check if any handlers are registered for a hook
	HasHookHandlers { hook_name: String, response: oneshot::Sender<bool> },

	/// List all loaded plugins
	ListPlugins { response: oneshot::Sender<Vec<TsPluginInfo>> },
	...
	TrackAsyncResource { plugin_name: String, resource: TrackedAsyncResource },
...
pub enum TrackedAsyncResource {
	VirtualBuffer(dx_core::BufferId),
	CompositeBuffer(dx_core::BufferId),
	Terminal(dx_core::TerminalId),
	WatchHandle(u64),
}
...
impl PluginThreadHandle {
	...
	pub fn spawn(services: Arc<dyn dx_core::services::PluginServiceBridge>) -> Result<Self> {
	...
	pub fn check_thread_health(&mut self) {
		if let Some(handle) = &self.thread_handle {
			if handle.is_finished() {
				tracing::error!("check_thread_health: plugin thread is finished, checking for panic");
	...
	pub fn deliver_response(&self, response: dx_core::api::PluginResponse) {
		...
		use dx_core::api::PluginResponse;

		match response {
			PluginResponse::VirtualBufferCreated { request_id, buffer_id, split_id } => {
				// Track the created buffer for cleanup on plugin unload
				self.track_async_resource(request_id, TrackedAsyncResource::VirtualBuffer(buffer_id));
			...
			PluginResponse::BufferText { request_id, text } => match text {
				Ok(content) => {
					...
					let result = serde_json::to_string(&content).unwrap_or_else(|_| "\"\"".to_string());
			...
			PluginResponse::CompositeBufferCreated { request_id, buffer_id } => {
				// Track the created buffer for cleanup on plugin unload
				self.track_async_resource(request_id, TrackedAsyncResource::CompositeBuffer(buffer_id));
				// Return just the buffer_id number, not an object
				self.resolve_callback(JsCallbackId(request_id), buffer_id.0.to_string());
			}
			PluginResponse::LineStartPosition { request_id, position } => {
				self.resolve_json_callback(request_id, position, "null");
			}
			PluginResponse::LineEndPosition { request_id, position } => {
				self.resolve_json_callback(request_id, position, "null");
			}
			PluginResponse::BufferLineCount { request_id, count } => {
				self.resolve_json_callback(request_id, count, "null");
			}
			PluginResponse::TerminalCreated { request_id, buffer_id, terminal_id, split_id } => {
			...
			PluginResponse::SplitByLabel { request_id, split_id } => {
			...
			PluginResponse::WatchPathRegistered { request_id, result } => match result {
				Ok(handle) => {
					self.track_async_resource(request_id, TrackedAsyncResource::WatchHandle(handle));
	...
	fn track_async_resource(&self, request_id: u64, resource: TrackedAsyncResource) {
		let plugin_name =
			self.async_resource_owners.lock().ok().and_then(|mut owners| owners.remove(&request_id));
		if let Some(plugin_name) = plugin_name {
			if let Some(sender) = self.request_sender.as_ref() {
				fire_and_forget(sender.send(PluginRequest::TrackAsyncResource { plugin_name, resource }));
			}
		}
	}
	...
	pub fn load_plugin(&self, path: &Path) -> Result<()> {
		...
		self
			...
			.send(PluginRequest::LoadPlugin { path: path.to_path_buf(), response: tx })
	...
	pub fn load_plugins_from_dir(&self, dir: &Path) -> Vec<String> {
		...
		if sender
			.send(PluginRequest::LoadPluginsFromDir { dir: dir.to_path_buf(), response: tx })
			.is_err()
		{
			return vec!["Plugin thread not responding".to_string()];
		}

		rx.recv().unwrap_or_else(|_| vec!["Plugin thread closed".to_string()])
	}
	...
	pub fn load_plugins_from_dir_with_config(
		...
		if sender
			...
			.is_err()
		{
			return (vec!["Plugin thread not responding".to_string()], HashMap::new());
		}

		rx.recv().unwrap_or_else(|_| (vec!["Plugin thread closed".to_string()], HashMap::new()))
	}
	...
	pub fn unload_plugin(&self, name: &str) -> Result<()> {
		...
		self
			...
			.as_ref()
			.ok_or_else(|| anyhow!("Plugin thread shut down"))?
			.send(PluginRequest::UnloadPlugin { name: name.to_string(), response: tx })
			.map_err(|_| anyhow!("Plugin thread not responding"))?;
	...
	pub fn reload_plugin(&self, name: &str) -> Result<()> {
		...
		self
			...
			.as_ref()
			.ok_or_else(|| anyhow!("Plugin thread shut down"))?
			.send(PluginRequest::ReloadPlugin { name: name.to_string(), response: tx })
			.map_err(|_| anyhow!("Plugin thread not responding"))?;
	...
	pub fn execute_action_async(&self, action_name: &str) -> Result<oneshot::Receiver<Result<()>>> {
		...
		self
			...
			.as_ref()
			.ok_or_else(|| anyhow!("Plugin thread shut down"))?
			.send(PluginRequest::ExecuteAction { action_name: action_name.to_string(), response: tx })
			.map_err(|_| anyhow!("Plugin thread not responding"))?;
	...
	pub fn has_hook_handlers(&self, hook_name: &str) -> bool {
		...
		if sender
			.send(PluginRequest::HasHookHandlers { hook_name: hook_name.to_string(), response: tx })
			.is_err()
		{
			return false;
		}
	...
	pub fn list_plugins(&self) -> Vec<TsPluginInfo> {
		...
		if sender.send(PluginRequest::ListPlugins { response: tx }).is_err() {
			return vec![];
		}
	...
	pub fn process_commands_until_hook_completed(
		...
		loop {
			...
			match self.command_receiver.recv_timeout(remaining) {
				Ok(PluginCommand::HookCompleted { hook_name: ref name }) if name == hook_name => {
	...
	pub fn resolve_callback(&self, callback_id: dx_core::api::JsCallbackId, result_json: String) {
		if let Some(sender) = self.request_sender.as_ref() {
			fire_and_forget(sender.send(PluginRequest::ResolveCallback { callback_id, result_json }));
		}
	}

	/// Reject an async callback in the plugin runtime
	/// Called by the app when async operations fail
	pub fn reject_callback(&self, callback_id: dx_core::api::JsCallbackId, error: String) {
		if let Some(sender) = self.request_sender.as_ref() {
...
fn respond_to_pending(
	pending_responses: &PendingResponses,
	response: dx_core::api::PluginResponse,
) -> bool {
...
mod plugin_thread_tests {
	...
	fn respond_to_pending_sends_lsp_response() {
		...
		respond_to_pending(
			&pending,
			PluginResponse::LspRequest { request_id: 123, result: Ok(json!({ "key": "value" })) },
		);
	...
	fn respond_to_pending_handles_virtual_buffer_created() {
		...
		respond_to_pending(
			...
			PluginResponse::VirtualBufferCreated {
				request_id: 456,
				buffer_id: dx_core::BufferId(7),
				split_id: Some(dx_core::SplitId(1)),
			},
		);
	...
	loop {
		...
		if crate::backend::has_fatal_js_error() {
			if let Some(error_msg) = crate::backend::take_fatal_js_error() {
				tracing::error!("Fatal JS error detected, terminating plugin thread: {}", error_msg);
...
async fn run_hook_internal_rc(
	...
	let json_data = dx_core::hooks::hook_args_to_json(args)?;
	tracing::trace!(
		hook = hook_name,
	...
	runtime.borrow_mut().emit_to(hook_name, &json_data, target).await?;
	tracing::trace!(hook = hook_name, emit_ms = emit_start.elapsed().as_millis(), "emit completed");
...
async fn handle_request(
	...
	match request {
		...
		PluginRequest::LoadPluginsFromDirWithConfig { dir, plugin_configs, response } => {
		...
		PluginRequest::LoadPluginFromSource { source, name, is_typescript, response } => {
		...
		PluginRequest::ExecuteAction { action_name, response } => {
			...
			fire_and_forget(
				response.send(Err(anyhow::anyhow!("Internal error: ExecuteAction in wrong handler"))),
			);
		}

		PluginRequest::RunHook { hook_name, args, target } => {
			...
			if let Err(e) =
				run_hook_internal_rc(Rc::clone(&runtime), &hook_name, &args, target.as_deref()).await
			{
		...
		PluginRequest::HasHookHandlers { hook_name, response } => {
		...
		PluginRequest::ResolveCallback { callback_id, result_json } => {
			...
			tracing::trace!("ResolveCallback: done resolving callback_id={}", callback_id);
		}
		...
		PluginRequest::TrackAsyncResource { plugin_name, resource } => {
...
fn prepare_plugin(path: &Path) -> Result<PreparedPlugin> {
	...
	let filename = path.file_name().and_then(|s| s.to_str()).unwrap_or("plugin.ts");

	// Extract dependencies before transpilation
	let dependencies = dx_parser_js::extract_plugin_dependencies(&source);
	...
	let declarations = if filename.ends_with(".ts") {
		match dx_parser_js::emit_isolated_declarations(&source, filename) {
			Ok(dts) => Some(dts),
			Err(e) => {
				tracing::warn!("Plugin {} isolated-declarations emit failed: {}", path.display(), e);
				None
			}
		}
	} else {
	...
	let js_code = if dx_parser_js::has_es_imports(&source) {
		match dx_parser_js::bundle_module(path) {
			Ok(bundled) => bundled,
			Err(e) => {
				tracing::warn!(
	...
	} else if dx_parser_js::has_es_module_syntax(&source) {
		let stripped = dx_parser_js::strip_imports_and_exports(&source);
		if filename.ends_with(".ts") {
			dx_parser_js::transpile_typescript(&stripped, filename)?
		} else {
			stripped
		}
	} else if filename.ends_with(".ts") {
		dx_parser_js::transpile_typescript(&source, filename)?
	} else {
		source
	};
	...
	let i18n = if i18n_path.exists() {
		std::fs::read_to_string(&i18n_path).ok().and_then(|content| serde_json::from_str(&content).ok())
	} else {
		None
	};
...
fn execute_prepared_plugin(
	...
	if let Some(ref i18n) = prepared.i18n {
		runtime.borrow_mut().services.register_plugin_strings(&prepared.name, i18n.clone());
		tracing::debug!("Loaded i18n strings for plugin '{}'", prepared.name);
	}

	let path_str = prepared.path.to_str().ok_or_else(|| anyhow!("Invalid path encoding"))?;

	let exec_start = std::time::Instant::now();
	runtime.borrow_mut().execute_js(&prepared.js_code, path_str)?;
...
async fn load_plugin_internal(
	...
	tracing::debug!("load_plugin_internal: starting module load for plugin '{}'", plugin_name);
	...
	if i18n_path.exists() {
		if let Ok(content) = std::fs::read_to_string(&i18n_path) {
			...
			>(&content)
			{
				runtime.borrow_mut().services.register_plugin_strings(&plugin_name, strings);
	...
	runtime.borrow_mut().load_module_with_source(path_str, &plugin_name).await?;
...
async fn load_plugins_from_dir_internal(
	...
	tracing::debug!("load_plugins_from_dir_internal: scanning directory {:?}", dir);
		...
		Ok(entries) => {
			for entry in entries.flatten() {
				...
				if ext == Some("ts") || ext == Some("js") {
					tracing::debug!("load_plugins_from_dir_internal: attempting to load {:?}", path);
...
async fn load_plugins_from_dir_with_config_internal(
	...
	tracing::debug!("load_plugins_from_dir_with_config_internal: scanning directory {:?}", dir);
	let mut errors = Vec::new();
	...
	for (name, prepared) in &prepared_map {
		if !prepared.dependencies.is_empty() {
			tracing::debug!("Plugin '{}' declares dependencies: {:?}", name, prepared.dependencies);
	...
	let load_order = match dx_parser_js::topological_sort_plugins(&plugin_names, &dependency_map) {
		Ok(order) => order,
		Err(e) => {
...
fn load_plugin_from_source_internal(
	...
	runtime.borrow_mut().execute_source(source, name, is_typescript)?;
	...
	tracing::info!("Buffer plugin '{}' loaded successfully, total plugins: {}", name, plugins.len());
...
fn unload_plugin_internal(
	...
	if plugins.remove(name).is_some() {
		...
		runtime.borrow_mut().services.unregister_plugin_strings(name);

		// Remove all commands registered by this plugin
		runtime.borrow().services.unregister_commands_by_plugin(name);
...
async fn reload_plugin_internal(
	...
	let path = plugins.get(name).ok_or_else(|| anyhow!("Plugin '{}' not found", name))?.path.clone();
...
mod tests {
	use super::*;
	use dx_core::hooks::hook_args_to_json;
	...
	fn test_hook_args_to_json_prompt_changed() {
		let args =
			HookArgs::PromptChanged { prompt_type: "search".to_string(), input: "test".to_string() };
		let json = hook_args_to_json(&args).unwrap();
