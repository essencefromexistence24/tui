...
use windows_sys::Win32::System::Console::{
	ENABLE_VIRTUAL_TERMINAL_INPUT, ENABLE_WINDOW_INPUT, FOCUS_EVENT, GetConsoleMode, GetStdHandle,
...
pub fn enable_vt_input() -> io::Result<u32> {
	unsafe {
		...
		if handle.is_null() || ptr::eq(handle, INVALID_HANDLE_VALUE) {
			return Err(io::Error::other("Failed to get stdin handle"));
		}
		...
		tracing::debug!("Enabled VT input mode (old=0x{:x}, new=0x{:x})", old_mode, new_mode,);
...
impl VtInputReader {
	...
	pub fn spawn() -> Self {
		...
		Self { rx, event_handle, _thread: thread }
	}
	...
	fn reader_loop(tx: std::sync::mpsc::Sender<VtInputEvent>, event_handle: *mut c_void) {
		...
		let mut records: [MaybeUninit<INPUT_RECORD>; 16384] = [const { MaybeUninit::uninit() }; 16384];
		...
		loop {
			...
			if last_heartbeat.elapsed() >= Duration::from_secs(30) {
				...
				unsafe {
					...
					if GetConsoleMode(handle, &mut current_mode) != 0 && current_mode != desired_mode {
						tracing::warn!(
			...
			let mut count: u32 = 0;
			let ok = unsafe {
				ReadConsoleInputW(handle, records[0].as_mut_ptr(), records.len() as u32, &mut count)
			};
			...
			for rec in records.iter().take(count as usize) {
				let rec = unsafe { rec.assume_init_ref() };
				match rec.EventType as u32 {
					...
					WINDOW_BUFFER_SIZE_EVENT => {
						if !vt_bytes.is_empty() {
							structured_events
								.push(VtInputEvent::VtBytes(strip_corrupt_mouse(std::mem::take(&mut vt_bytes))));
						}
			...
			if !vt_bytes.is_empty() && tx.send(VtInputEvent::VtBytes(vt_bytes)).is_err() {
				return;
			}
	...
	while i < bytes.len() {
			...
			&& (i == 0 || bytes[i - 1] != 0x1b)
		{
			...
			if let Some(seq_len) = match_sgr_mouse(&bytes[i..]) {
				tracing::debug!("Stripped corrupt mouse sequence ({} bytes) at offset {}", seq_len, i,);
