...
//! 3. Automatically writes to `plugins/lib/dx.d.ts` during compilation
//!
//! ## Usage
//!
//! ```rust,ignore
...
//! ## File Output
//!
//! The macro automatically writes `plugins/lib/dx.d.ts` when:
	...
	match ty {
		Type::Path(type_path) => {
			...
			match type_name.as_str() {
				...
				"u8" | "u16" | "u32" | "u64" | "i8" | "i16" | "i32" | "i64" | "usize" | "isize" | "f32"
...
fn parse_method(method: &ImplItemFn) -> Option<ApiMethod> {
	...
	Some(ApiMethod { js_name, kind, params, return_type, doc, ts_raw })
}
...
fn generate_ts_method(method: &ApiMethod) -> String {
	...
	if let Some(raw) = &method.ts_raw {
		...
		// Method signature
		let params: String =
			method.params.iter().map(ParamInfo::to_typescript).collect::<Vec<_>>().join(", ");
...
fn generate_ts_preamble() -> &'static str {
	r#"/**
 * Dx Editor TypeScript Plugin API
 *
 * This file provides type definitions for the Dx editor's TypeScript plugin system.
 ...
 * Generated by dx-plugin-api-macros + ts-rs from JsEditorApi impl
 */
...
/// - `{IMPL_NAME}_JS_METHODS: &[&str]` - List of all JS method names
///
/// The main crate should combine these with ts-rs generated types to create dx.d.ts.
///
/// # Example
///
/// ```rust,ignore
...
pub fn plugin_api_impl(_attr: TokenStream, item: TokenStream) -> TokenStream {
	...
	let methods: Vec<ApiMethod> = input
		.items
		.iter()
		.filter_map(|item| if let ImplItem::Fn(method) = item { parse_method(method) } else { None })
		.collect();
		...
		if let ImplItem::Fn(method) = item {
			for arg in &mut method.sig.inputs {
				if let FnArg::Typed(pat_type) = arg {
					pat_type.attrs.retain(|attr| !attr.path().is_ident("plugin_api"));
				}
			}
		}
	}
	...
	let expanded = quote! {
			...
			///
			/// Combine with ts-rs types and EDITOR_API to create dx.d.ts
			pub const #preamble_const: &str = #preamble;

			/// TypeScript EditorAPI interface (methods only)
			///
			/// Combine with preamble and ts-rs types to create dx.d.ts
			...
			///
			/// These types need to be defined (via ts-rs or manually) in dx.d.ts.
...
mod tests {
	...
	fn test_api_kind_wrap_return_type() {
		...
		assert_eq!(ApiKind::AsyncPromise.wrap_return_type("number"), "Promise<number>");
		assert_eq!(
	...
	fn test_extract_type_references() {
		...
		assert_eq!(extract_type_references("ProcessHandle<SpawnResult>"), vec!["SpawnResult"]);
		assert_eq!(extract_type_references("Promise<BufferInfo>"), vec!["BufferInfo"]);

		// Union with null
		assert_eq!(extract_type_references("CursorInfo | null"), vec!["CursorInfo"]);
	...
	fn test_extract_type_references_renamed_types() {
		...
		assert_eq!(extract_type_references("TsCompositeHunk[]"), vec!["TsCompositeHunk"]);
		assert_eq!(
			extract_type_references("TsCreateCompositeBufferOptions"),
			vec!["TsCreateCompositeBufferOptions"]
		);
		assert_eq!(extract_type_references("PromptSuggestion[]"), vec!["PromptSuggestion"]);
	}

	#[test]
	fn test_extract_type_references_terminal_types() {
		assert_eq!(extract_type_references("Promise<TerminalResult>"), vec!["TerminalResult"]);
		assert_eq!(extract_type_references("CreateTerminalOptions"), vec!["CreateTerminalOptions"]);
	}
	...
	fn test_generate_ts_method_with_renamed_param_type() {
		...
		let ts = generate_ts_method(&method);
		assert!(
			ts.contains("updateCompositeAlignment(bufferId: number, hunks: TsCompositeHunk[]): boolean;")
		);
	}
	...
	fn test_generate_ts_method_terminal() {
		...
		let ts = generate_ts_method(&method);
		assert!(ts.contains("createTerminal(opts?: CreateTerminalOptions): Promise<TerminalResult>;"));
	}
	...
	fn test_all_known_types_are_passthrough_or_renamed() {
		...
		for type_name in &passthrough_types {
			...
			let ts = rust_to_typescript(&ty, &[]);
			assert_eq!(&ts, type_name, "Type {} should pass through unchanged", type_name);
		}
		...
		for (rust_name, ts_name) in &renamed {
			...
			let ts = rust_to_typescript(&ty, &[]);
			assert_eq!(&ts, ts_name, "Type {} should be renamed to {}", rust_name, ts_name);
		}
	}
}
