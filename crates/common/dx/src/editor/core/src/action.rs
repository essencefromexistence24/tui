...
mod tests {
	...
	fn from_when_clause_handles_aliases_whitespace_and_rejects_unknown() {
		...
		assert_eq!(KeyContext::from_when_clause("file_explorer"), Some(KeyContext::FileExplorer));
	...
	fn allows_text_input_iff_normal_or_prompt() {
		for ctx in [
			...
			assert_eq!(ctx.allows_text_input(), expected, "{:?} text-input expectation violated", ctx);
		}
	}
}
