...
mod tests {
    ...
    fn imported_catalog_contains_all_checked_out_n8n_directories() {
        ...
        // Baseline: 442 nodes-base + 122 langchain classes from the real
        // `dist/known/nodes.json` inventories (see `static_nodes.rs`).
        assert!(
            n8n_count >= 500,
            "expected at least the imported n8n baseline, got {n8n_count}"
        );
    }

    #[test]
    fn imported_catalog_contains_flow_like_baseline() {
        let flow_count = catalog()
            .iter()
            .filter(|node| node.source == NodeSource::FlowLike)
            .count();
        // Baseline: 1845 `impl NodeLogic for X` nodes across 11 catalog
        // families, extracted from Rheosoph/flow-like @ 1545749d
        // (see `static_nodes.rs`).
        assert!(
            flow_count >= 1500,
            "expected at least the imported flow-like baseline, got {flow_count}"
        );
    }
