...
`impl NodeLogic for ...` definition (1,845 in Rheosoph/flow-like @ 1545749d).
If those source trees are not installed, the checked-in fallback inventory in
`src/static_nodes.rs` is used — generated from those same sources, so the
fallback carries the full 564 n8n + 1,845 flow-like baseline (2,409 nodes plus
the native slice). Regenerate it with
`crates/common/connect/scripts/regen_static_catalog.py`. The native slice includes `dx.set`, `dx.if`, `dx.merge`, `dx.noop`, and
