...
pub fn compress(body: &str, _intensity: &str) -> HeadroomResult<HeadroomOutput> {
    {
        ...
        Ok(HeadroomOutput {
            text,
            arrays_compacted: compacted,
            format: if compacted > 0 {
                "toon".into()
            } else {
                "passthrough".into()
            },
...
fn compact_value(value: &serde_json::Value) -> Option<String> {
    match value {
        serde_json::Value::Array(arr) => {
            ...
            if all_objects(arr) {
                compact_object_array(arr)
            } else if all_strings(arr) {
                compact_string_array(arr)
            } else {
                None
            }
        }
...
fn compact_object_array(arr: &[serde_json::Value]) -> Option<String> {
    ...
    for key in &keys {
        ...
        let sep = if values.iter().any(|v| v.contains(',')) {
            "|"
        } else {
            ","
        };
...
fn compact_inline_arrays(text: &str, count: &mut u32) -> HeadroomResult<String> {
    ...
    result = re
        .replace_all(&result, |caps: &regex_lite::Captures| {
            *count += 1;
            format!(
                "--- omni-tabular (inline {} items) ---",
                caps[0].matches(',').count() + 1
            )
        })
        .to_string();
    Ok(result)
