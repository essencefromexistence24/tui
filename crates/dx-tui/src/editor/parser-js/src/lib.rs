...
///
/// Dx runs this over every TypeScript plugin at load time so the
...
/// signatures into the aggregate `plugins.d.ts`. Many Dx plugins
...
pub fn emit_isolated_declarations(source: &str, filename: &str) -> Result<String> {
	let allocator = Allocator::default();
	let source_type = SourceType::from_path(filename).unwrap_or_default().with_module(true);
...
pub fn topological_sort_plugins(
	...
	if result.len() != plugin_names.len() {
		...
		let cycle_plugins: Vec<String> =
			plugin_names.iter().filter(|n| !in_result.contains(n.as_str())).cloned().collect();
		return Err(anyhow!(
...
mod tests {
	...
	fn emit_isolated_declarations_script_hides_internals() {
		...
		assert!(!dts.contains("Internal"), "non-exported interface leaked into .d.ts: {dts}");
		assert!(!dts.contains("internalConst"), "non-exported const leaked into .d.ts: {dts}");
		assert!(!dts.contains("internalFn"), "non-exported function leaked into .d.ts: {dts}");
	}
	...
	fn emit_isolated_declarations_keeps_exports_and_registry_augmentation() {
		...
		assert!(dts.contains("DxPluginRegistry"), "registry augmentation missing: {dts}");
	...
	fn test_extract_module_bindings() {
		...
		assert!(reexports.iter().any(|r| r.source_path == "./types.ts" && r.exported_name.is_none()));
		// export *
	}
...
"#;
	...
	fn test_topological_sort_diamond() {
		...
		let names = vec!["d".to_string(), "c".to_string(), "b".to_string(), "a".to_string()];
		let mut deps = std::collections::HashMap::new();
		deps.insert("b".to_string(), vec!["a".to_string()]);
	...
	fn test_topological_sort_missing_dependency() {
		...
		assert!(result.is_err());
		let err = result.unwrap_err().to_string();
		assert!(err.contains("not installed"), "Error should mention missing dep: {}", err);
	}
	...
	fn test_topological_sort_independent_plugins_alphabetical() {
		...
		let names =
			vec!["zebra".to_string(), "alpha".to_string(), "beta".to_string(), "gamma".to_string()];
		let mut deps = std::collections::HashMap::new();
