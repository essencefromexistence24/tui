...
impl Backend for CaptureBackend {
	...
	fn window_size(&mut self) -> Result<WindowSize, Self::Error> {
		Ok(WindowSize { columns_rows: self.size, pixels: ratatui::layout::Size::new(0, 0) })
	}
...
impl EditorAdapter {
	pub fn new() -> Self {
		Self { editor: None, needs_init: false, last_cursor: None }
	}
	...
	pub fn render(&mut self, area: Rect, buf: &mut Buffer) {
		...
		if let Ok(completed) = terminal.draw(|frame| editor.render(frame)) {
			...
			self.last_cursor =
				Some((Position::new(abs_x, abs_y), crossterm::cursor::SetCursorStyle::SteadyBlock));
		}
	}
