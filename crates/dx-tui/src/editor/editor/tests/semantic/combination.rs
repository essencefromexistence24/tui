...
/// Permutation length per soak case (default 100; set DX_SOAK_LEN
/// to change).
fn soak_len() -> usize {
    std::env::var("DX_SOAK_LEN")
        .ok()
...
/// DX_SOAK_CASES to run more).
fn soak_cases() -> u32 {
    std::env::var("DX_SOAK_CASES")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(3)
}
...
proptest! {
    ...
    /// 100 scenarios; `DX_SOAK_LEN=...  DX_SOAK_CASES=...`.
    #[test]
