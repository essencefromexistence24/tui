...
fn test_fs() -> std::sync::Arc<dyn model::filesystem::FileSystem + Send + Sync> {
...
fn test_buffer_cursor_adjustment_on_insert() {
    let mut state = EditorState::new(
        80,
        24,
        config::LARGE_FILE_THRESHOLD_BYTES as usize,
        test_fs(),
    );
...
fn test_overlay_events() {
    use dx::model::event::{OverlayFace, UnderlineStyle};
...
fn test_lsp_diagnostic_to_overlay() {
    ...
    let theme = view::theme::Theme::load_builtin(theme::THEME_DARK).unwrap();
    ...
    match face {
        view::overlay::OverlayFace::Background { color } => {
...
mod event_inverse_tests {
    use dx::model::event::{CursorId, Event, OverlayFace, UnderlineStyle};
    use dx::view::overlay::{OverlayHandle, OverlayNamespace};
