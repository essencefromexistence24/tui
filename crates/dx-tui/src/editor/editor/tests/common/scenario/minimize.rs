...
//!
//! Gated behind `DX_MUTATION=1`. For each `BufferScenario`, greedily
...
//! `$DX_MUTATION_REPORT` (default `target/scenario-minimization.jsonl`).
//!
...
fn enabled() -> bool {
    std::env::var_os("DX_MUTATION").is_some()
}
