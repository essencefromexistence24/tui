...
//! persistence keys off `$XDG_DATA_HOME/dx`, and we build the harness's
...
fn restored_remote_terminal_reconnects_over_ssh_after_restart() -> anyhow::Result<()> {
    ...
    // Isolate ALL editor persistence into the temp tree: `$XDG_DATA_HOME/dx`
    ...
    let dir_context = DirectoryContext {
        data_dir: xdg_data.join("dx"), // == get_data_dir()
