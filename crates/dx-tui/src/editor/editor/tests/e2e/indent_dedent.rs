...
fn test_multicursor_indent_with_selections() {
    use dx::model::event::{CursorId, Event};
    ...
    harness
        .editor_mut()
        .apply_event_to_active_buffer(&Event::AddCursor {
    ...
    for indent_count in 1..=3 {
        ...
        let c1 = cursors.get(CursorId(1)).unwrap();
        assert_eq!(
            c1.position,
            4 + indent_offset,
        ...
        let c2 = cursors.get(CursorId(2)).unwrap();
        assert_eq!(
            c2.position,
            13 + indent_offset * 2,
        ...
        let c3 = cursors.get(CursorId(3)).unwrap();
        assert_eq!(
            c3.position,
            22 + indent_offset * 3,
    ...
    {
        let cursors = harness.editor().active_cursors();
        let cursors_vec: Vec<_> = cursors.iter().collect();
        assert_eq!(
            cursors_vec.len(),
            4,
...
/// See: https://github.com/anomalyco/dx/issues/1304
#[test]
