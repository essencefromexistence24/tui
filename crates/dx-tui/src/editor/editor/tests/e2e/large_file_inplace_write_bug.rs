...
use dx::model::buffer::TextBuffer;
use dx::model::filesystem::{
...
impl FileSystem for NotOwnerFileSystem {
    ...
    fn search_file(
        ...
        path: &Path,
        pattern: &str,
        opts: &model::filesystem::FileSearchOptions,
        cursor: &mut model::filesystem::FileSearchCursor,
    ) -> io::Result<Vec<model::filesystem::SearchMatch>> {
        model::filesystem::default_search_file(&*self.inner, path, pattern, opts, cursor)
    }
...
fn test_inplace_write_crash_recovery() {
    ...
    let recovery_dir = services::recovery::RecoveryStorage::get_recovery_dir().unwrap();
    let hash = services::recovery::path_hash(&file_path);
    ...
    let recovery: services::recovery::InplaceWriteRecovery =
...
fn test_successful_inplace_write_cleans_up_recovery() {
    use dx::services::recovery::RecoveryStorage;
    use std::fs;
