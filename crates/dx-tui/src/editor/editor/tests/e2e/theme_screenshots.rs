...
// the DX_THEME env var (defaults to "dark").
//
// Usage:
//   DX_THEME=dracula cargo nextest run --package dx-editor --test e2e_tests theme_screenshot_gallery -- --ignored --nocapture
//   # Then:
...
use dx::config::{Config, ThemeName};
use dx::config_io::DirectoryContext;
use dx::model::event::{Event, OverlayColorSpec, OverlayFace, OverlayOptions};
use dx::view::overlay::OverlayNamespace;
use std::fs;
use std::path::{Path, PathBuf};
use std::process::Command;

fn theme_name() -> String {
    std::env::var("DX_THEME").unwrap_or_else(|_| "dark".to_string())
}
...
///
/// Set DX_THEME env var to choose the theme (defaults to "dark").
...
fn theme_screenshot_gallery() {
    ...
    config.theme = config::ThemeName(theme.clone());
...
//
// Usage:
//   DX_THEME_BASE_REF=origin/master \
//     cargo nextest run --package dx-editor --test e2e_tests \
//     -E 'test(theme_diff_gallery)' --run-ignored ignored-only --no-capture
// ===========================================================================

/// Workspace root (two levels up from `crates/dx-editor`).
...
fn theme_relpath(file: &str) -> String {
    format!("crates/dx-editor/themes/{}", file)
}
...
/// `DX_THEME_BASE_REF` → merge-base with origin/master → origin/master →
/// master. Returns `None` if nothing resolves (e.g. a shallow clone).
fn resolve_base_ref(root: &Path) -> Option<String> {
    if let Ok(r) = std::env::var("DX_THEME_BASE_REF") {
        let r = r.trim();
        if !r.is_empty() {
...
fn scene_lsp_status(s: &mut BlogShowcase, theme_token: &str, theme_json: &str) {
    ...
    config.lsp.insert(
        "rust".to_string(),
        types::LspLanguageConfig::Multi(vec![services::lsp::LspServerConfig {
            ...
            enabled: true,
            auto_start: true,
            process_limits: services::process_limits::ProcessLimits::default(),
...
fn theme_diff_gallery() {
    ...
    match &base_ref {
        ...
        None => println!(
            "theme-diff: no base ref resolved (set DX_THEME_BASE_REF); rendering 'after' only"
        ),
    }

    let render_all = std::env::var("DX_THEME_DIFF_ALL").is_ok();
    let only: Option<Vec<String>> = std::env::var("DX_THEME_DIFF_THEMES")
        .ok()
