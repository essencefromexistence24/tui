...
use dx::services::lsp::LspServerConfig;
use dx::services::process_limits::ProcessLimits;
use dx::types::LspLanguageConfig;
use std::fs;
...
fn test_diagnostics_panel_plugin_loads() {
    ...
    harness
        .wait_until(|h| {
            ...
            let overlays = h.editor().active_state().overlays.all();
            let diagnostic_ns = services::lsp::diagnostics::lsp_diagnostic_namespace();
            overlays
                .iter()
...
fn test_diagnostics_api_with_fake_lsp() -> anyhow::Result<()> {
    ...
    let test_plugin = r#"/// <reference path="./lib/dx.d.ts" />
const editor = getEditor();
