...
"#,
    ...
    // Create the packages directory (simulating ~/.config/dx/plugins/packages)
    let packages_dir = plugins_dir.join("packages");
    fs::create_dir_all(&packages_dir).unwrap();
...
fn test_pkg_install_plugin_empty_registry() {
    ...
    // djb2("https://github.com/anomalyco/dx-plugins-registry") = 193934da
...
"#,
    ...
    // Also copy lib to bundle plugins dir so the plugin can find dx.d.ts
    let bundle_plugins_lib_dir = plugins_dir.join("lib");
    fs::create_dir_all(&bundle_plugins_lib_dir).unwrap();
    fs::copy(
        project_plugins_dir.join("lib").join("dx.d.ts"),
        bundle_plugins_lib_dir.join("dx.d.ts"),
    )
    .unwrap();
...
fn test_uninstall_plugin_removes_commands() {
    ...
    fs::write(
        test_plugin_dir.join("main.ts"),
        r#"
/// <reference path="../../../plugins/lib/dx.d.ts" />
const editor = getEditor();
