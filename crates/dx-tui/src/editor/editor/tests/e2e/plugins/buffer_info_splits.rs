...
fn buffer_info_splits_reports_both_splits_after_split_horizontal() {
    ...
    harness
        .editor_mut()
        .dispatch_action_for_tests(input::keybindings::Action::SplitHorizontal);
    harness.tick_and_render().unwrap();
...
fn buffer_info_splits_drives_refocus_pattern() {
    ...
    harness
        .editor_mut()
        .handle_plugin_command(services::plugins::api::PluginCommand::FocusSplit {
            split_id: dx_core::SplitId(hello_split),
        })
        .unwrap();
    harness.tick_and_render().unwrap();
