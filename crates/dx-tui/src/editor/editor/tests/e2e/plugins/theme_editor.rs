...
fn test_save_builtin_theme_produces_valid_file() {
    ...
    let theme_file: Result<view::theme::ThemeFile, _> = serde_json::from_str(&content);
    assert!(
        theme_file.is_ok(),
...
/// ~/.config/dx/themes is not created by the editor.
#[test]
fn test_issue_1180_save_theme_creates_themes_directory() {
    ...
    // This simulates a fresh install where ~/.config/dx/themes doesn't exist.
    let context_temp = tempfile::TempDir::new().unwrap();
    let dir_context = DirectoryContext::for_testing(context_temp.path());
    ...
    harness.type_text("my-dx-theme").unwrap();
    harness.render().unwrap();
    ...
    let saved_path = dir_context.themes_dir().join("my-dx-theme.json");
    assert!(
        saved_path.exists(),
        "Saved theme file should exist at {:?}.\nThemes dir exists: {}\nFiles in themes dir: {:?}",
        saved_path,
    ...
    let theme = theme_file.unwrap();
    assert_eq!(theme.name, "my-dx-theme");
}
