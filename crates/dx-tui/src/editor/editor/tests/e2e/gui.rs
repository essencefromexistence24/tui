...
fn test_parse_file_location_plain_path() {
    let (path, line, col) = gui::parse_file_location("src/main.rs");
...
fn test_parse_file_location_with_line() {
    let (path, line, col) = gui::parse_file_location("src/main.rs:42");
    assert_eq!(path, PathBuf::from("src/main.rs"));
...
fn test_parse_file_location_with_line_and_col() {
    let (path, line, col) = gui::parse_file_location("src/main.rs:42:10");
    assert_eq!(path, PathBuf::from("src/main.rs"));
    assert_eq!(line, Some(42));
...
fn test_parse_file_location_non_numeric_suffix() {
    ...
    let (path, line, col) = gui::parse_file_location("foo:bar");
...
fn test_parse_file_location_windows_drive() {
    ...
    let (path, line, col) = gui::parse_file_location(r"C:\file.rs:10:5");
...
fn test_pixel_to_cell_typical_font() {
    ...
    let (col, row) = gui::pixel_to_cell((150.0, 60.0), cell_size);
    assert_eq!(col, 10);
    assert_eq!(row, 2);
}
...
fn test_grid_dimensions_for_default_window() {
    let cell_size = (14.4, 28.8);
    let (cols, rows) = gui::cell_dimensions_to_grid(1280.0, 800.0, cell_size);
