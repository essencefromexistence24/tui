...
use dx::config::{Config, TerminalShellConfig};
use dx::services::terminal::TerminalState;
...
/// backing file directory (~/.local/share/dx/terminals/).
#[test]
#[cfg(not(windows))] // Uses Unix shell
fn test_open_file_from_terminal_uses_correct_directory() {
    ...
    // backing file directory (which would be something like ~/.local/share/dx/terminals/)
    let screen = harness.screen_to_string();
...
fn test_windows_terminal_shows_prompt_and_executes_command() {
    ...
    let marker = "DX_TERMINAL_TEST_12345";
...
fn write_to_terminal_by_buffer(
    ...
    terminal_buffer: model::event::BufferId,
    bytes: &[u8],
) {
    let terminal_id = harness
        .editor()
...
fn setup_text_buffer_beside_terminal(
    ...
    write_to_terminal_by_buffer(harness, terminal_buffer, b"echo DX_READY\n");
    harness
        .wait_until(|h| h.screen_to_string().contains("DX_READY"))
        .unwrap();
