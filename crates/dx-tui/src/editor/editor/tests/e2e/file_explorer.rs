...
fn test_file_explorer_focus_after_delete() {
    ...
    assert!(
        matches!(
            key_context_before,
            input::keybindings::KeyContext::FileExplorer
        ),
...
fn test_file_explorer_new_file_opens_rename_prompt_and_buffer() {
    ...
    assert!(
        matches!(key_context, input::keybindings::KeyContext::Normal),
...
fn test_file_explorer_search_highlight_positions() {
    ...
    if let Some(selected_entry) = explorer.get_selected_entry() {
        ...
        let fuzzy_result = input::fuzzy::fuzzy_match(query, &selected_entry.name);
        println!(
...
fn test_file_explorer_escape_clears_search() {
    ...
    assert!(
        matches!(
            harness.editor().get_key_context(),
            input::keybindings::KeyContext::Normal
        ),
...
///
/// Reproduces https://github.com/anomalyco/dx/issues/1079:
...
/// Regression test for https://github.com/anomalyco/dx/issues/1388:
...
/// Regression test for https://github.com/anomalyco/dx/issues/1431
#[test]
...
fn test_file_explorer_minimum_render_width_is_5_cols() {
    use dx::config::{Config, ExplorerWidth};
    let mut config = Config::default();
...
fn test_file_explorer_side_left() {
    use dx::config::{Config, ExplorerWidth, FileExplorerSide};
...
fn test_file_explorer_side_workspace_serialization() {
    use dx::config::{Config, FileExplorerSide};
...
fn test_file_explorer_custom_tree_indicators() {
    ...
    config.file_explorer.width = config::ExplorerWidth::Columns(30);
