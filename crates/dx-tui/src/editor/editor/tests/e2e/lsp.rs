...
fn test_lsp_diagnostics_display() -> anyhow::Result<()> {
    use dx::model::event::{Event, OverlayFace};
...
fn test_lsp_completion_filtering() -> anyhow::Result<()> {
    ...
    if let Some(popup) = state.popups.top() {
        if let view::popup::PopupContent::List { items, .. } = &popup.content {
...
fn test_semantic_tokens_range_preserves_overlays_on_edit() -> anyhow::Result<()> {
    ...
    let ns = services::lsp::semantic_tokens::lsp_semantic_tokens_namespace();
    harness.wait_until(|h| {
        let state = h.editor().active_state();
        state
            .overlays
...
fn test_lsp_typing_performance_with_many_diagnostics() -> anyhow::Result<()> {
    ...
    services::lsp::diagnostics::apply_diagnostics_to_state(
        ...
        &view::theme::Theme::load_builtin(theme::THEME_DARK).unwrap(),
    );
...
fn test_handle_rename_response_with_document_changes() -> anyhow::Result<()> {
    ...
    let uri = dx_core::file_uri::path_to_lsp_uri(&test_file).unwrap();
...
fn test_pull_diagnostics_message_handling() -> anyhow::Result<()> {
    use dx::services::async_bridge::AsyncMessage;
    ...
    let uri = dx_core::file_uri::path_to_lsp_uri(&test_file).expect("Should create URI");
...
fn test_inlay_hints_render_on_screen() -> anyhow::Result<()> {
    use dx::view::virtual_text::VirtualTextPosition;
...
"#,
    ...
    config.lsp.insert(
        "c".to_string(),
        types::LspLanguageConfig::Multi(vec![types::LspServerConfig {
            command: "python3".to_string(),
            args: Some(vec![script_path.to_string_lossy().to_string()]),
            enabled: true,
            auto_start: true,
            process_limits: types::ProcessLimits::default(),
            initialization_options: None,
            env: Default::default(),
            language_id_overrides: Default::default(),
            root_markers: Default::default(),
...
fn test_restart_lsp_prompt_shows_suggestions() -> anyhow::Result<()> {
    ...
    config.lsp.insert(
        "rust".to_string(),
        types::LspLanguageConfig::Multi(vec![
            services::lsp::LspServerConfig {
...
fn lsp_spawn_failure_writes_stub_log_for_view_log_popup() -> anyhow::Result<()> {
    ...
    let log_path = services::log_dirs::lsp_log_path("pascal");
