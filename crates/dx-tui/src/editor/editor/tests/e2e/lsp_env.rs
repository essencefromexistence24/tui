...
///
/// Uses a fake LSP server that reads `DX_TEST_ENV_VAR` from its
...
fn test_lsp_env_vars_passed_to_server() -> anyhow::Result<()> {
    ...
    // DX_TEST_ENV_VAR. If it was passed correctly, we see "hello_from_config".
    assert!(
