...
//
// Usage:
//   cargo test --package dx-editor --test e2e_tests blog_showcase_ -- --ignored --nocapture
...
fn blog_showcase_productivity_flash_jump() {
    use dx::input::keybindings::Action::PluginAction;
...
"#;
    ...
    // up before capture.  (Pre-existing Dx quirk: the status's
...
// =========================================================================
// Blog Post 4: What's New in Dx (0.2.3–0.2.9)
// =========================================================================
...
Install with `cargo install dx-editor` or download a binary from the [releases page](https://github.com/anomalyco/dx/releases).

Run `fresh` to start editing. Press **Ctrl+P** to open the command palette.

## Architecture

Dx is built on a **piece table** data structure for efficient text manipulation. The rendering pipeline uses [ratatui](https://ratatui.rs/) for terminal UI.
...
Settings are stored in `~/.config/dx/config.json`. You can edit them via the **Settings UI** or directly in the JSON file.

Keybindings live in the same config file under the `keybindings` key. Use the **Keybinding Editor** for a visual interface.

## Remote Editing

Dx supports editing files over **SSH** with `fresh user@host:path`. The connection uses your existing SSH keys or prompts for a password.

Large file operations like *line scanning* run server-side — only the index is transferred, not the file content.

## License

Released under the **Apache 2.0** license. See [LICENSE](https://github.com/anomalyco/dx/blob/master/LICENSE) for details.
"#;
...
// =========================================================================
// Blog Post 5: What's New in Dx (0.2.11–0.2.18)
// =========================================================================
...
"#,
    ...
    let mut s = BlogShowcase::new(
        "dx-0.2.18/project-search-replace",
...
"#,
    ...
    // Inject fake diagnostics via the proper API
    {
        ...
        let state = h.editor_mut().active_state_mut();
        let theme = view::theme::Theme::load_builtin("dark").unwrap();
        services::lsp::diagnostics::apply_diagnostics_to_state(state, &diagnostics, &theme);
    }

    let mut s = BlogShowcase::new(
        "dx-0.2.18/inline-diagnostics",
        "Inline Diagnostics",
...
fn blog_showcase_fresh_0_2_18_whitespace_indicators() {
    ...
    let mut s = BlogShowcase::new(
        "dx-0.2.18/whitespace-indicators",
...
pkgs.mkShell {
  ...
  shellHook = ''
    echo "Dx development environment"
...
fn blog_showcase_fresh_0_2_18_hot_exit() {
    ...
    let dir_context = config_io::DirectoryContext::for_testing(temp_dir.path());
...
fn build_git_log_demo_repo() -> GitTestRepo {
    ...
    repo.create_file("README.md", "# Dx demo\n\nA tiny sample project.\n");
    repo.git_add_all();
...
fn blog_showcase_fresh_0_2_26_dashboard() {
    ...
    let dir_context = config_io::DirectoryContext::for_testing(temp.path());
...
fn blog_showcase_fresh_0_2_26_init_ts() {
    ...
    let mut s = BlogShowcase::new(
        ...
        "Dx auto-loads ~/.config/dx/init.ts. Run init: Edit from the palette to open it with a starter template; init: Reload re-runs without restarting.",
    );
...
// and Dx attaches the full remote-agent stack (filesystem + terminal +
...
/// a local user-space sshd via `/etc/hosts`), and watch Dx attach the
...
fn blog_showcase_fresh_0_4_0_ssh_session() {
    ...
    let mut s = BlogShowcase::new(
        "dx-0.4.0/ssh-session",
        "New SSH Session",
        "Start a remote SSH session from the Orchestrator's New Workspace dialog: \
         pick the SSH backend and point it at a host. Dx attaches its \
...
/// crate with a `build.rs`) and Dx raises a full-screen security prompt that
...
fn blog_showcase_fresh_0_4_0_workspace_trust() {
    ...
    let store = services::workspace_trust::TrustStore::for_project_dir(&store_path);
    h.editor()
        .authority()
        .workspace_trust
    ...
    let mut s = BlogShowcase::new(
        "dx-0.4.0/workspace-trust",
        "Workspace Trust",
        "Open a folder that can run code — a project manifest, a build script, a \
         direnv/mise env — and Dx raises a full-screen prompt that names what \
