...
impl DirEntry {
	...
	pub fn new(path: PathBuf, name: String, entry_type: EntryType) -> Self {
		Self { path, name, entry_type, metadata: None, symlink_target_is_dir: false }
	}
...
impl Default for FileSearchCursor {
	fn default() -> Self {
		Self { offset: 0, running_line: 1, done: false, end_offset: None }
	}
}
...
impl FileSearchCursor {
	...
	pub fn for_range(offset: usize, end_offset: usize, running_line: usize) -> Self {
		Self { offset, running_line, done: false, end_offset: Some(end_offset) }
	}
}
...
pub trait FileSystem: Send + Sync {
	...
	fn unique_temp_path(&self, dest_path: &Path) -> PathBuf {
		...
		let file_name = dest_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("dx-save"));
	...
	/// - `uid`: Owner user ID
	/// - `gid`: Owner group ID
	fn sudo_write(&self, path: &Path, data: &[u8], mode: u32, uid: u32, gid: u32) -> io::Result<()>;
...
pub fn build_search_regex(
	...
	let re_pattern = if opts.fixed_string { regex::escape(pattern) } else { pattern.to_string() };
	let re_pattern = if opts.whole_word { format!(r"\b{}\b", re_pattern) } else { re_pattern };
	let re_pattern = if opts.case_sensitive { re_pattern } else { format!("(?i){}", re_pattern) };
	...
	regex::bytes::Regex::new(&re_pattern).map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, e))
}
...
fn has_binary_extension(path: &Path) -> bool {
	...
	BINARY_FILE_EXTENSIONS.iter().any(|candidate| candidate.eq_ignore_ascii_case(ext))
}
...
pub fn default_search_file(
	...
	for m in regex.find_iter(&chunk) {
		...
		line_at += chunk[counted_to..m.start()].iter().filter(|&&b| b == b'\n').count();
		counted_to = m.start();

		// Find line boundaries for context
		let line_start =
			chunk[..m.start()].iter().rposition(|&b| b == b'\n').map(|p| p + 1).unwrap_or(0);
...
impl StdFileSystem {
	...
	fn kernel_writable(path: &Path) -> Option<bool> {
		...
		let rc =
			unsafe { libc::faccessat(libc::AT_FDCWD, c_path.as_ptr(), libc::W_OK, libc::AT_EACCESS) };
		Some(rc == 0)
	}
...
impl FileSystem for StdFileSystem {
	...
	fn open_file_for_write(&self, path: &Path) -> io::Result<Box<dyn FileWriter>> {
		let file = std::fs::OpenOptions::new().write(true).truncate(true).open(path)?;
		Ok(Box::new(StdFileWriter(file)))
	}
	...
	fn read_dir(&self, path: &Path) -> io::Result<Vec<DirEntry>> {
		...
		for entry in std::fs::read_dir(path)? {
			...
			if file_type.is_symlink() {
				dir_entry.symlink_target_is_dir =
	...
	fn sudo_write(&self, path: &Path, data: &[u8], mode: u32, uid: u32, gid: u32) -> io::Result<()> {
		...
		let status = Command::new("sudo")
			.args(["chown", &format!("{}:{}", uid, gid), &path.to_string_lossy()])
			.hide_window()
			.status()?;
		if !status.success() {
...
impl NoopFileSystem {
	fn unsupported<T>() -> io::Result<T> {
		Err(io::Error::new(io::ErrorKind::Unsupported, "Filesystem not available"))
	}
}
...
mod tests {
	...
	fn test_read_dir() {
		...
		fs.write_file(&temp_dir.path().join("file1.txt"), b"content1").unwrap();
		fs.write_file(&temp_dir.path().join("file2.txt"), b"content2").unwrap();
	...
	fn test_dir_entry_types() {
		...
		let dir = DirEntry::new(PathBuf::from("/dir"), "dir".to_string(), EntryType::Directory);
		assert!(dir.is_dir());
		assert!(!dir.is_file());
	...
	fn test_metadata_builder() {
		let meta = FileMetadata::default().with_hidden(true).with_readonly(true);
	...
	fn test_write_patched_insert_only() {
		...
		let ops = vec![WriteOp::Insert { data: b"All new content" }];
	...
	fn test_search_file_no_matches() {
		...
		let opts = make_search_opts(true);
		let mut cursor = FileSearchCursor::new();
		let matches = fs.search_file(&path, "NOTFOUND", &opts, &mut cursor).unwrap();
	...
	fn test_search_file_regex() {
		...
		let mut cursor = FileSearchCursor::new();
		let matches = fs.search_file(&path, r"[a-z]+\d+", &opts, &mut cursor).unwrap();
	...
	fn test_search_file_binary_extension_skipped_despite_text_content() {
		...
		assert!(cursor.done);
		assert!(matches.is_empty(), ".png extension should short-circuit before content scan");
	}
	...
	fn test_search_file_binary_extension_case_insensitive() {
		...
		for name in ["IMG.JPG", "archive.tar.gz", "weights.SafeTensors"] {
			...
			let opts = make_search_opts(true);
			let mut cursor = FileSearchCursor::new();
			let matches = fs.search_file(&path, "definitely", &opts, &mut cursor).unwrap();

			assert!(cursor.done, "{} should be marked done", name);
			assert!(matches.is_empty(), "{} matched but extension should have skipped it", name);
		}
	}
	...
	fn test_search_file_midstream_nul_aborts_scan() {
		...
		let opts = make_search_opts(true);
		let mut cursor = FileSearchCursor::new();
		let matches = fs.search_file(&path, "PATTERN_AFTER_NUL", &opts, &mut cursor).unwrap();

		assert!(cursor.done);
		assert!(matches.is_empty(), "mid-stream NUL should abort scan and discard pseudo-matches");
	}
	...
	fn test_multiline_regex_dotall_implicit() {
		...
		let opts_lit = FileSearchOptions { fixed_string: true, ..opts };
	...
	fn test_search_file_oversized_skipped() {
		...
		let opts = make_search_opts(true);
		let mut cursor = FileSearchCursor::new();
		let matches = fs.search_file(&path, "UNIQUE_TAIL_MARKER", &opts, &mut cursor).unwrap();

		assert!(cursor.done, "oversized file should be marked done in one call");
	...
	fn test_search_file_cursor_multi_chunk() {
		...
		while !cursor.done {
			let batch = fs.search_file(&path, "line 5000", &opts, &mut cursor).unwrap();
	...
	fn test_search_file_cursor_no_duplicates() {
		...
		while !cursor.done {
			let batch = fs.search_file(&path, "MARKER_", &opts, &mut cursor).unwrap();
			all_matches.extend(batch);
			batches += 1;
		}
	...
	fn test_walk_files_std_basic() {
		...
		fs.walk_files(tmp.path(), &["node_modules", "target"], &cancel, &mut |_path, rel| {
	...
	fn test_walk_files_std_skip_dirs() {
		...
		fs.walk_files(tmp.path(), &["node_modules", "target", "deep"], &cancel, &mut |_path, rel| {
			found.push(rel.to_string());
			true
		})
		.unwrap();
	...
	fn test_walk_files_std_on_file_returns_false() {
		...
		fs.walk_files(tmp.path(), &["node_modules", "target"], &cancel, &mut |_path, _rel| {
			count += 1;
	...
	fn test_walk_files_noop_returns_error() {
		...
		let result = fs.walk_files(Path::new("/noop/path"), &[], &cancel, &mut |_path, _rel| true);
	...
	fn test_is_writable_matches_kernel_for_owner_writable() {
		...
		fs.set_permissions(&path, &FilePermissions::from_mode(0o600)).unwrap();

		let c_path = std::ffi::CString::new(path.as_os_str().as_bytes()).unwrap();
		let kernel_writable =
			unsafe { libc::faccessat(libc::AT_FDCWD, c_path.as_ptr(), libc::W_OK, libc::AT_EACCESS) }
				== 0;
		assert!(kernel_writable, "owner-writable file must be writable per kernel");
	...
	///
	/// Run manually with:
	///   sudo cargo test -p dx-editor --features runtime \
	...
	fn test_is_writable_respects_posix_acl() {
		...
		let acl_status = Command::new("setfacl")
			...
			.status()
			.unwrap();
		assert!(acl_status.success(), "setfacl failed (does the filesystem support ACLs?)",);
		...
		assert!(exited_normally, "child terminated abnormally; status={status}");
		assert_eq!(
			exit_code, 0,
