...
impl StringBuffer {
	...
	pub fn new(id: usize, data: Vec<u8>) -> Self {
		...
		StringBuffer {
			id,
			data: BufferData::Loaded { data, line_starts: Some(line_starts) },
	...
	pub fn new_loaded(id: usize, data: Vec<u8>, compute_lines: bool) -> Self {
		let line_starts = if compute_lines { Some(Self::compute_line_starts(&data)) } else { None };
		StringBuffer { id, data: BufferData::Loaded { data, line_starts }, stored_file_offset: None }
	}
	...
	pub fn new_unloaded(id: usize, file_path: PathBuf, file_offset: usize, bytes: usize) -> Self {
		StringBuffer {
			id,
			data: BufferData::Unloaded { file_path, file_offset, bytes },
			stored_file_offset: None,
		}
	}
	...
	pub fn nth_line_start_in_piece(
		...
		if ls < piece_end { Some(ls) } else { None }
	}
	...
	pub fn load(&mut self, fs: &dyn crate::model::filesystem::FileSystem) -> io::Result<()> {
		match &self.data {
			...
			BufferData::Unloaded { file_path, file_offset, bytes } => {
				...
				self.data = BufferData::Loaded { data: buffer, line_starts: Some(line_starts) };
	...
	pub fn create_chunk_buffer(
		...
		match &self.data {
			BufferData::Unloaded { file_path, file_offset, bytes } => {
				...
				let mut buf =
					StringBuffer::new_unloaded(new_id, file_path.clone(), absolute_file_offset, chunk_bytes);
...
impl LeafData {
	pub fn new(
		...
		LeafData { location, offset, bytes, line_feed_cnt }
	}
}
...
impl PieceTreeNode {
	...
	fn find_by_offset(&self, offset: usize) -> Option<OffsetFindResult> {
		match self {
			Self::Internal { left_bytes, left, right, .. } => {
			...
			Self::Leaf { location, offset: piece_offset, bytes, line_feed_cnt } => {
				if offset < *bytes {
	...
	fn total_bytes(&self) -> usize {
		match self {
			Self::Internal { left_bytes, right, .. } => left_bytes + right.total_bytes(),
	...
	fn update_leaf_lf_by_index(
		...
		match self.as_ref() {
			Self::Leaf { location, offset, bytes, .. } => {
				if leaf_index == 0 {
					(
						Arc::new(Self::Leaf {
							location: *location,
			...
			Self::Internal { left_bytes, lf_left, left, right } => {
	...
	pub(crate) fn collect_leaves(&self, leaves: &mut Vec<LeafData>) {
		match self {
			...
			Self::Leaf { location, offset, bytes, line_feed_cnt } => {
		...
		match self {
			Self::Internal { left_bytes, lf_left, left, right } => {
				...
				if target_line <= lines_after_left {
					let result = left.find_piece_for_line(current_offset, lines_before, target_line);
					// Fall back to right if left doesn't contain it
					result.or_else(|| {
						right.find_piece_for_line(current_offset + left_bytes, lines_after_left, target_line)
					})
				} else {
					right.find_piece_for_line(current_offset + left_bytes, lines_after_left, target_line)
				}
			...
			Self::Leaf { location, offset, bytes, line_feed_cnt } => {
				...
				if target_line >= lines_before && target_line <= lines_in_piece {
					Some((current_offset, location.buffer_id(), *offset, *bytes, lines_before))
				} else {
					None
				}
			}
...
impl PieceTree {
	...
	pub fn new(
		...
		PieceTree {
			root: Arc::new(PieceTreeNode::Leaf { location, offset, bytes, line_feed_cnt }),
			total_bytes: bytes,
		}
	}
	...
	pub fn from_leaves(leaves: &[LeafData]) -> Self {
		...
		PieceTree { root: Self::build_balanced(leaves), total_bytes }
	}
	...
	fn build_balanced(leaves: &[LeafData]) -> Arc<PieceTreeNode> {
		...
		Arc::new(PieceTreeNode::Internal { left_bytes, lf_left, left, right })
	}
	...
	fn path_copy_insert(
		...
		match node.as_ref() {
			PieceTreeNode::Internal { left_bytes, lf_left, left, right, .. } => {
				let left_end = current_offset + left_bytes;
			...
			PieceTreeNode::Leaf { location, offset, bytes, line_feed_cnt } => {
				...
				if offset_in_piece > 0 && insert_offset < piece_end {
					...
					let lf_after =
						Self::compute_line_feeds_static(buffers, *location, offset + before_bytes, after_bytes);

					let leaf_before = LeafData::new(*location, *offset, before_bytes, lf_before);
					let leaf_after = LeafData::new(*location, offset + before_bytes, after_bytes, lf_after);
	...
	fn path_copy_delete(
		...
		match node.as_ref() {
			...
			PieceTreeNode::Leaf { location, offset, bytes, line_feed_cnt: _ } => {
				...
				if before_bytes > 0 && after_bytes > 0 {
					...
					let leaf_before = LeafData::new(*location, *offset, before_bytes, lf_before);
					let leaf_after =
						LeafData::new(*location, offset + bytes - after_bytes, after_bytes, lf_after);
					Self::build_balanced(&[leaf_before, leaf_after])
				} else if before_bytes > 0 {
					// Keep only the part before the delete
					let lf_cnt = Self::compute_line_feeds_static(buffers, *location, *offset, before_bytes);
					Arc::new(PieceTreeNode::Leaf {
						location: *location,
						offset: *offset,
						bytes: before_bytes,
					...
					let lf_cnt =
						Self::compute_line_feeds_static(buffers, *location, offset + skip, after_bytes);
					Arc::new(PieceTreeNode::Leaf {
						location: *location,
						offset: offset + skip,
						bytes: after_bytes,
	...
	pub fn cursor_at_offset(&self, offset: usize) -> Cursor {
		Cursor { byte_offset: offset.min(self.total_bytes), line: 0, col: 0 }
	}
	...
	fn collect_leaves_with_split(
		...
		match node.as_ref() {
			PieceTreeNode::Internal { left_bytes, left, right, .. } => {
			...
			PieceTreeNode::Leaf { location, offset, bytes, line_feed_cnt } => {
				...
				if split_offset > current_offset && split_offset < piece_end {
					...
					if offset_in_piece > 0 {
						let lf_cnt =
							Self::compute_line_feeds_static(buffers, *location, *offset, offset_in_piece);
					...
					if remaining > 0 {
						...
						leaves.push(LeafData::new(*location, offset + offset_in_piece, remaining, lf_cnt));
					}
		...
		if let Some(result) = self.root.find_by_offset(offset) {
			...
			if let Some(buffer) = buffers.get(buffer_id) {
				...
				if let Some(line_starts) = buffer.get_line_starts() {
					...
					let piece_start_line =
						line_starts.binary_search(&piece_info.offset).unwrap_or_else(|i| i.saturating_sub(1));
				...
				if let Some(data) = buffer.get_data() {
					...
					let line_in_piece = data[piece_start..scan_end].iter().filter(|&&b| b == b'\n').count();
					let doc_line = lines_before + line_in_piece;
					...
					let column = if line_in_piece == 0 {
						...
						let last_nl =
							data[piece_start..scan_end].iter().rposition(|&b| b == b'\n').unwrap_or(0);
				...
				if let Some(lf_count) = piece_info.line_feeds {
					...
					let column =
						if avg_bytes_per_line > 0 { offset_in_piece % avg_bytes_per_line } else { 0 };
					return Some((doc_line, column));
				}
			}
		}
	...
	pub fn position_to_offset(&self, line: usize, column: usize, buffers: &[StringBuffer]) -> usize {
		...
		self.find_offset_for_line(line, column, buffers).unwrap_or(self.total_bytes)
	}
	...
	fn collect_leaves_with_multi_split(
		...
		match node.as_ref() {
			PieceTreeNode::Internal { left_bytes, left, right, .. } => {
				...
				self.collect_leaves_with_multi_split(left, current_offset, split_points, leaves, buffers);
				self.collect_leaves_with_multi_split(
					right,
					current_offset + left_bytes,
					split_points,
				...
				if split_offsets.is_empty() {
					...
					for split_offset in split_offsets {
						if split_offset > prev_offset {
							...
							leaves.push(LeafData::new(*location, offset + prev_offset, chunk_bytes, lf_cnt));
						}
					...
					if prev_offset < *bytes {
						...
						let lf_cnt =
							Self::compute_line_feeds_static(buffers, *location, offset + prev_offset, remaining);
...
impl PieceRangeIter {
	fn new(root: &Arc<PieceTreeNode>, start: usize, end: usize) -> Self {
		...
		PieceRangeIter { pieces, current_index: 0 }
	}
...
mod tests {
	...
	fn test_offset_to_position_column_after_modification() {
		...
		assert_eq!(pos, Some((1, 4)), "After modification: position 25 should be line 1, column 4");
	...
	fn test_bulk_edit_single_delete() {
		...
		let delta =
			tree.apply_bulk_edits(&edits, &buffers, |_text| (BufferLocation::Added(1), 0, 0, Some(0)));
	...
	fn test_bulk_edit_multiple_deletes_descending() {
		...
		let delta =
			tree.apply_bulk_edits(&edits, &buffers, |_| (BufferLocation::Added(1), 0, 0, Some(0)));
	...
	fn test_bulk_edit_consistency_check() {
		...
		assert_eq!(sum, tree.total_bytes(), "Piece sum {} != total_bytes {}", sum, tree.total_bytes());
	}
	...
	fn test_bulk_edit_vs_sequential_equivalence() {
		...
		tree2.insert(10, BufferLocation::Added(next_id2), 0, 3, Some(0), &buffers2);
		next_id2 += 1;
	...
	fn bulk_edit_strategy() -> impl Strategy<Value = Vec<BulkEditOp>> {
		prop::collection::vec(
			(0usize..100, 0usize..20, "[a-zA-Z0-9]{0,10}").prop_map(
				|(position, delete_len, insert_text)| BulkEditOp { position, delete_len, insert_text },
			),
			1..20,
		)
	}
