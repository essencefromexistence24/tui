...
impl Default for CompositeLayout {
	fn default() -> Self {
		CompositeLayout::SideBySide { ratios: vec![0.5, 0.5], show_separator: true }
	}
}
...
impl SourcePane {
	...
	pub fn new(buffer_id: BufferId, label: impl Into<String>, editable: bool) -> Self {
		Self { buffer_id, label: label.into(), editable, style: PaneStyle::default(), range: None }
	}
...
impl PaneStyle {
	...
	pub fn old_diff() -> Self {
		Self { remove_bg: Some((80, 30, 30)), gutter_style: GutterStyle::Both, ..Default::default() }
	}
...
impl LineAlignment {
	...
	pub fn from_hunks(hunks: &[DiffHunk], old_line_count: usize, new_line_count: usize) -> Self {
		...
		for hunk in hunks {
			...
			if let Some(ops) = hunk.ops.as_deref() {
					...
					match op {
						...
						'-' => {
							rows.push(AlignedRow {
								pane_lines: vec![Some(SourceLineRef { line: o, byte_range: 0..0 }), None],
						...
						'+' => {
							rows.push(AlignedRow {
								pane_lines: vec![None, Some(SourceLineRef { line: n, byte_range: 0..0 })],
				...
				for i in 0..max_lines {
					let old_idx = if i < old_hunk_lines { Some(hunk.old_start + i) } else { None };
					let new_idx = if i < new_hunk_lines { Some(hunk.new_start + i) } else { None };
					...
					rows.push(AlignedRow {
						pane_lines: vec![
							old_idx.map(|l| SourceLineRef { line: l, byte_range: 0..0 }),
							new_idx.map(|l| SourceLineRef { line: l, byte_range: 0..0 }),
						],
						row_type,
					});
				}
		...
		while old_line < old_line_count {
			rows.push(AlignedRow {
				pane_lines: vec![Some(SourceLineRef { line: old_line, byte_range: 0..0 }), None],
				row_type: RowType::Deletion,
			});
			old_line += 1;
		}
		while new_line < new_line_count {
			rows.push(AlignedRow {
				pane_lines: vec![None, Some(SourceLineRef { line: new_line, byte_range: 0..0 })],
				row_type: RowType::Addition,
			});
			new_line += 1;
		}
...
impl AlignedRow {
	...
	pub fn context(old_line: usize, new_line: usize) -> Self {
		Self {
			pane_lines: vec![
				Some(SourceLineRef { line: old_line, byte_range: 0..0 }),
				Some(SourceLineRef { line: new_line, byte_range: 0..0 }),
			],
			row_type: RowType::Context,
		}
	}
	...
	pub fn hunk_header() -> Self {
		Self { pane_lines: vec![None, None], row_type: RowType::HunkHeader }
	}
	...
	pub fn has_content(&self, pane_index: usize) -> bool {
		self.pane_lines.get(pane_index).map(|opt| opt.is_some()).unwrap_or(false)
	}
}
...
impl DiffHunk {
	...
	pub fn new(old_start: usize, old_count: usize, new_start: usize, new_count: usize) -> Self {
		Self { old_start, old_count, new_start, new_count, header: None, ops: None }
	}
...
mod tests {
	...
	fn test_composite_buffer_focus() {
		let sources =
			vec![SourcePane::new(BufferId(1), "OLD", false), SourcePane::new(BufferId(2), "NEW", true)];
