...
impl ControlEvent {
	...
	pub fn simple(name: impl Into<String>) -> Self {
		Self { name: name.into(), data: Value::Null }
	}
...
impl EventBroadcaster {
	pub fn new(max_history: usize) -> Self {
		Self { events: Arc::new(Mutex::new(VecDeque::with_capacity(max_history))), max_history }
	}
...
mod tests {
	...
	fn test_event_matching() {
		let event =
			ControlEvent::new("lsp:status_changed", json!({"language": "rust", "status": "running"}));
