...
impl Cursors {
	...
	pub fn new() -> Self {
		...
		Self { cursors, next_id: 1, primary_id }
	}

	/// Get the primary cursor
	pub fn primary(&self) -> &Cursor {
		self.cursors.get(&self.primary_id).expect("Primary cursor should always exist")
	}

	/// Get the primary cursor mutably
	pub fn primary_mut(&mut self) -> &mut Cursor {
		self.cursors.get_mut(&self.primary_id).expect("Primary cursor should always exist")
	}
	...
	pub fn remove(&mut self, id: CursorId) -> Option<Cursor> {
		...
		if id == self.primary_id {
			self.primary_id =
				*self.cursors.keys().next().expect("Should have at least one cursor remaining");
		}
	...
	pub fn remove_secondary(&mut self) {
		...
		let first_id =
			*self.cursors.keys().min_by_key(|id| id.0).expect("Should have at least one cursor");
		let first_cursor = *self.cursors.get(&first_id).expect("First cursor should exist");
	...
	pub fn selections(&self) -> Vec<Range<usize>> {
		self.cursors.values().filter_map(|c| c.selection_range()).collect()
	}
}
