...
impl CoordMap {
	...
	pub fn record_delete(&mut self, version: u64, pos: usize, len: usize) {
		self.push(EditDelta { version, pos, removed: len, inserted: 0 });
	}
	...
	pub fn record_replace(&mut self, version: u64, pos: usize, removed: usize, inserted: usize) {
		self.push(EditDelta { version, pos, removed, inserted });
	}
...
mod tests {
	...
	fn insert_before_shifts_right() {
		...
		assert_eq!(m.map(5, 0), Some(8), "right-gravity: coord at insert point moves");
	...
	fn eviction_raises_floor_and_returns_none() {
		...
		assert_eq!(m.len(), 2);
		assert_eq!(m.map(0, 0), None, "from before the evicted edit: unmappable");
	...
	fn replacement_shifts_by_net_delta() {
		...
		assert_eq!(m.map(13, 0), Some(10), "inside the span: clamps to net-delete start");
		...
		assert_eq!(g.map(11, 0), Some(15), "inside the span: net insert shifts by +4");
