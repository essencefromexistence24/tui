...
//!
//! Dx cursors are byte offsets, so a cursor can never *store* a position
...
mod tests {
	...
	fn zero_without_sticky_or_mid_line() {
		...
		assert_eq!(cursor_virtual_columns(VirtualSpaceMode::On, &buf, &cursor_at(2, None)), 0);
		// Mid-line position → not virtual even with a large sticky.
		assert_eq!(cursor_virtual_columns(VirtualSpaceMode::On, &buf, &cursor_at(1, Some(10))), 0);
		// Sticky within the line width → not virtual.
		assert_eq!(cursor_virtual_columns(VirtualSpaceMode::On, &buf, &cursor_at(2, Some(1))), 0);
		// Sticky exactly at the line width → not virtual.
		assert_eq!(cursor_virtual_columns(VirtualSpaceMode::On, &buf, &cursor_at(2, Some(2))), 0);
	}
