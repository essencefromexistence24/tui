...
pub enum LineNumber {
	...
	Relative { line: usize, from_cached_line: usize },
}
...
impl TextBuffer {
	...
	fn load_large_file_internal(
		...
		Ok(TextBuffer {
			...
			buffers: vec![buffer],
			next_buffer_id: 1,
			persistence: Persistence::new(fs, Some(path.to_path_buf()), saved_root, Some(file_size)),
	...
	pub fn save(&mut self) -> anyhow::Result<()> {
		if let Some(path) = self.persistence.file_path_owned() {
			...
			anyhow::bail!(io::Error::new(io::ErrorKind::NotFound, "No file path associated with buffer",))
		}
	}
	...
	pub fn refresh_saved_root_if_unmodified(&mut self) {
		self.persistence.refresh_saved_root_if_unmodified(&self.piece_tree);
	}
	...
	pub fn diff_since_saved(&self) -> PieceTreeDiff {
		...
		self.persistence.diff_since_saved(&self.piece_tree, &self.buffers)
	}
	...
	pub fn replace_content(&mut self, new_content: &str) {
		...
		if bytes > 0 {
			self.piece_tree =
				PieceTree::new(BufferLocation::Added(buffer_id), 0, bytes, Some(line_feed_cnt));
		} else {
	...
	pub fn get_text_range_mut(&mut self, offset: usize, bytes: usize) -> Result<Vec<u8>> {
		...
		while current_offset < end_offset {
			...
			for piece_view in self.piece_tree.iter_pieces_in_range(current_offset, end_offset) {
				...
				let needs_loading = self.buffers.get(buffer_id).map(|b| !b.is_loaded()).unwrap_or(false);
				...
				if read_end > read_start {
					...
					let data = buffer.get_data().context("Buffer data unavailable after load")?;
			...
			if !made_progress && !restarted_iteration {
				...
				tracing::error!("Piece tree stats: {} total bytes", self.piece_tree.stats().total_bytes);
				anyhow::bail!(
	...
	pub fn prepare_viewport(&mut self, start_offset: usize, line_count: usize) -> Result<()> {
		...
		tracing::trace!(bytes_to_load, total_bytes = self.total_bytes(), "prepare_viewport loading");
	...
	fn chunk_split_and_load(
		...
		let buffer_bytes = self.buffers.get(buffer_id).and_then(|b| b.unloaded_bytes()).unwrap_or(0);
		let needs_chunk_split = piece_view.bytes > LOAD_CHUNK_SIZE || buffer_bytes > piece_view.bytes;
		...
		if !needs_chunk_split {
			...
			let _span =
				tracing::info_span!("load_small_buffer", piece_bytes = piece_view.bytes, buffer_id,)
					.entered();
			self
				.buffers
				.get_mut(buffer_id)
		...
		let _span =
			tracing::info_span!("chunk_split_and_load", piece_bytes = piece_view.bytes, buffer_id,)
				.entered();
		...
		let (chunk_start_in_buffer, chunk_bytes) = if piece_view.bytes <= LOAD_CHUNK_SIZE {
			...
			let start = (piece_view.buffer_offset + offset_in_piece) / CHUNK_ALIGNMENT * CHUNK_ALIGNMENT;
			let bytes =
				LOAD_CHUNK_SIZE.min((piece_view.buffer_offset + piece_view.bytes).saturating_sub(start));
			(start, bytes)
		};
	...
	pub(crate) fn get_all_text_string(&self) -> Option<String> {
		self.get_all_text().map(|bytes| String::from_utf8_lossy(&bytes).into_owned())
	}
	...
	pub(crate) fn slice_bytes(&self, range: Range<usize>) -> Vec<u8> {
		self.get_text_range(range.start, range.end.saturating_sub(range.start)).unwrap_or_default()
	}
	...
	pub fn search_scan_next_chunk(
		...
		let chunk_bytes =
			self.get_text_range_mut(doc_offset, chunk_info.byte_len).map_err(std::io::Error::other)?;
		...
		let buf_doc_offset = if overlap_len > 0 { state.overlap_doc_offset } else { doc_offset };
		...
		let newlines_in_overlap = search_buf[..overlap_len].iter().filter(|&&b| b == b'\n').count();
		...
		for m in state.regex.find_iter(&search_buf) {
			...
			line_at += search_buf[counted_to..m.start()].iter().filter(|&&b| b == b'\n').count();
			counted_to = m.start();

			// Find line boundaries in search_buf for context
			let line_start =
				search_buf[..m.start()].iter().rposition(|&b| b == b'\n').map(|p| p + 1).unwrap_or(0);
	...
	pub fn search_hybrid_plan(&mut self) -> Option<HybridSearchPlan> {
		...
		for leaf in &leaves {
			...
			let is_unloaded_stored = matches!(
				(&leaf.location, buf),
				(BufferLocation::Stored(_), Some(StringBuffer { data: BufferData::Unloaded { .. }, .. }),)
			);

			if is_unloaded_stored {
				let file_offset = match buf.unwrap().data {
					BufferData::Unloaded { file_offset: fo, .. } => fo + leaf.offset,
					_ => unreachable!(),
				};

				// Merge with previous unloaded region if contiguous
				if let Some(SearchRegion::Unloaded { file_offset: prev_fo, bytes: prev_bytes, .. }) =
					regions.last_mut()
				{
				...
				regions.push(SearchRegion::Unloaded { file_offset, bytes: leaf.bytes, doc_offset });
			} else {
				...
				if let Some(SearchRegion::Loaded { data: prev_data, .. }) = regions.last_mut() {
	...
	pub fn scan_leaf(&self, leaf: &crate::model::piece_tree::LeafData) -> std::io::Result<usize> {
		...
		let count = match &buffer.data {
			crate::model::piece_tree::BufferData::Loaded { data, .. } => {
				...
				data[leaf.offset..end].iter().filter(|&&b| b == b'\n').count()
			}
			crate::model::piece_tree::BufferData::Unloaded { file_path, file_offset, .. } => {
				let read_offset = *file_offset as u64 + leaf.offset as u64;
				self.persistence.fs().count_line_feeds_in_range(file_path, read_offset, leaf.bytes)?
			}
		};
		Ok(count)
	}
	...
	pub fn rebuild_with_pristine_saved_root(&mut self, scan_updates: &[(usize, usize)]) {
		...
		for &(offset, location, buf_offset, bytes, lf_cnt) in &insertions {
			tree.insert(offset + insert_delta, location, buf_offset, bytes, lf_cnt, &self.buffers);
			insert_delta += bytes;
		}
	...
	pub fn resolve_line_byte_offset(&mut self, target_line: usize) -> Option<usize> {
		...
		let piece_data: Vec<u8> = match &buffer.data {
			...
			crate::model::piece_tree::BufferData::Unloaded { file_path, file_offset, .. } => {
				...
				self.persistence.fs().read_range(file_path, read_offset, piece_bytes).ok()?
			}
		};
	...
	pub fn get_line_number(&self, byte_offset: usize) -> usize {
		self.offset_to_position(byte_offset).map(|pos| pos.line).unwrap_or_else(|| {
					...
					if read_end > read_start {
						...
						if let Some(data) = buffer.get_data() {
							if buffer_end <= data.len() {
								...
								self.current_piece_data = Some(data[buffer_start..buffer_end].to_vec());
								self.current_piece_offset = 0;
								continue;
							}
						}
...
impl<'a> Iterator for OverlappingChunks<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let valid_start = if is_first { 0 } else { self.overlap.min(self.buffer.len()) };
...
impl TextBufferLineIterator {
	pub(crate) fn new(buffer: &mut TextBuffer, byte_pos: usize, max_lines: usize) -> Result<Self> {
		...
		if byte_pos >= buffer_len {
			return Ok(Self { lines: Vec::new(), current_index: 0, has_more: false });
		}
		...
		for _ in 0..max_lines {
			...
			if !found_newline && current_offset + line_len < buffer_len {
				...
				let content =
					if has_newline { line_string[..line_string.len() - 1].to_string() } else { line_string };

				lines.push(LineData { byte_offset: line_start, content, has_newline, line_number });
		...
		Ok(Self { lines, current_index: 0, has_more })
	}
}
