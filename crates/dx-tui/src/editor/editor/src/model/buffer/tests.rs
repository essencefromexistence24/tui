...
mod large_file_support {
	...
	fn test_buffer_load_method() {
		...
		File::create(&file_path).unwrap().write_all(test_data).unwrap();
	...
	fn test_load_force_text_skips_binary_detection() {
		...
		.unwrap();
		assert!(!buffer.is_binary(), "force_text small-file load must not flag the buffer binary");
		...
		assert!(!buffer.is_binary(), "force_text large-file load must not flag the buffer binary");
	}
	...
	fn test_issue_657_search_on_large_file_unloaded_buffer() {
		...
		assert!(!buffer.buffers[0].is_loaded(), "Buffer should be unloaded initially");
		...
		assert!(buffer.to_string().is_some(), "After get_text_range_mut(), to_string() should work");
	}
	...
	fn test_large_file_threshold_boundary() {
		...
		File::create(&file_path2).unwrap().write_all(&test_data2).unwrap();
	...
	fn test_large_file_incremental_save() {
		...
		assert!(
			saved_content[second_chunk_start..second_chunk_start + 100].iter().all(|&b| b == b'B'),
	...
	fn test_large_file_save_with_multiple_edits() {
		...
		assert_eq!(saved.len(), expected_len, "Length should be original + edits");
	}
}
...
fn test_offset_to_position_after_insert() {
	...
	let pos = buffer.offset_to_position(2).expect("small buffer should have line metadata");
	assert_eq!(pos.line, 1, "Byte 2 (start of inserted line) should be on line 1");

	let pos = buffer.offset_to_position(4).expect("small buffer should have line metadata");
	assert_eq!(pos.line, 2, "Byte 4 (start of 'b') should be on line 2 after insert");
}
...
fn test_line_iterator_with_offset_to_position() {
	...
	for byte_pos in 0..=buffer.len() {
		...
		let expected_line =
			buffer.offset_to_position(byte_pos).expect("small buffer should have line metadata").line;
		let expected_line_start =
			buffer.position_to_offset(Position { line: expected_line, column: 0 });
...
fn test_piece_tree_line_count_after_insert() {
	...
	assert_eq!(actual_line_count, Some(expected_line_count), "Line count mismatch after insert");
}
...
#[test]
fn test_detect_crlf() {
	assert_eq!(super::format::detect_line_ending(b"hello\r\nworld\r\n"), LineEnding::CRLF);
}
...
mod large_file_encoding_tests {
	...
	fn test_large_file_encoding_confirmation_display() {
		...
		assert!(display.contains("requires full load"), "Display: {}", display);
	}
...
mod rebuild_pristine_saved_root_tests {
	...
	fn test_no_edits_arc_ptr_eq() {
		...
		assert!(Arc::ptr_eq(buf.persistence.saved_root(), &buf.piece_tree.root()));
	...
	fn test_diff_byte_ranges_are_document_absolute_after_eof_insert() {
		...
		assert!(!diff.byte_ranges.is_empty(), "byte_ranges should not be empty");
	...
	fn large_file_buffer_unloaded(path: &std::path::Path, file_size: usize) -> TextBuffer {
		let fs: Arc<dyn crate::model::filesystem::FileSystem + Send + Sync> =
			Arc::new(crate::model::filesystem::StdFileSystem);
		let buffer =
			crate::model::piece_tree::StringBuffer::new_unloaded(0, path.to_path_buf(), 0, file_size);
	...
	fn test_unloaded_buffer_no_edits_line_count() {
		...
		assert_eq!(buf.line_count(), Some(expected_lf + 1), "after rebuild, line_count must be exact");
		assert!(buf.file_kind.has_line_feed_scan());
	}
	...
	fn test_viewport_load_after_rebuild_does_not_load_entire_file() {
		...
		impl crate::model::filesystem::FileSystem for TrackingFs {
			...
			fn metadata(&self, path: &Path) -> std::io::Result<crate::model::filesystem::FileMetadata> {
			...
			fn read_dir(&self, path: &Path) -> std::io::Result<Vec<crate::model::filesystem::DirEntry>> {
		...
		let fs: Arc<dyn crate::model::filesystem::FileSystem + Send + Sync> = Arc::new(TrackingFs {
			inner: crate::model::filesystem::StdFileSystem,
	...
	fn test_viewport_load_after_rebuild_preserves_line_counts() {
		...
		assert!(line_count_before.is_some(), "line_count must be Some after rebuild");
...
mod chunked_search {
	...
	fn case_insensitive_regex() {
		...
		let state = buf.search_scan_all(make_regex("(?i)hello"), 100, 5).unwrap();
		assert_eq!(state.matches.len(), 3);
		assert_eq!(state.matches[0].line, 1);
	...
	fn multi_chunk_line_numbers_correct() {
		...
		let state = buffer.search_scan_all(make_regex("line_050"), 100, 8).unwrap();
		assert_eq!(state.matches.len(), 1);
		let m = &state.matches[0];
		assert_eq!(m.line, 50);
		assert_eq!(m.column, 1);
	...
	fn multi_chunk_no_duplicate_matches() {
		...
		for (i, m) in state.matches.iter().enumerate() {
			assert_eq!(m.line, i + 1, "Match {} should be on line {}, got {}", i, i + 1, m.line);
		}
	}
	...
	fn overlap_mid_line_line_numbers() {
		...
		let state = buffer.search_scan_all(make_regex("TARGET_HERE"), 100, 11).unwrap();
		assert_eq!(state.matches.len(), 1);
		let m = &state.matches[0];
...
mod hybrid_search {
	...
	fn hybrid_matches_scan_all_for_loaded_buffer() {
		...
		let hybrid = buf.search_hybrid("foo", &opts, regex.clone(), 100, 3).unwrap();
	...
	fn hybrid_finds_matches_in_unloaded_regions() {
		...
		let has_unloaded = leaves.iter().any(|l| {
			matches!(l.location, BufferLocation::Stored(_))
				&& buf.buffers.get(l.location.buffer_id()).map(|b| !b.is_loaded()).unwrap_or(false)
		});
	...
	fn hybrid_boundary_match() {
		...
		let opts = make_opts();
		let matches = buf.search_hybrid("AAAAABBBBB", &opts, regex, 100, 10).unwrap();
	...
	fn hybrid_max_matches_respected() {
		...
		let opts = crate::model::filesystem::FileSearchOptions { max_matches: 3, ..make_opts() };
