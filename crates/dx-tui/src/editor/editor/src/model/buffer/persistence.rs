...
impl Persistence {
	pub fn new(
		...
		Self { fs, file_path, modified: false, recovery_pending: false, saved_root, saved_file_size }
	}
	...
	pub fn apply_chunk_load_to_saved_root(
		...
		for leaf in &leaves {
			...
			if chunk_end < leaf_end {
				new_leaves.push(LeafData::new(leaf.location, chunk_end, leaf_end - chunk_end, None));
			}
		}
	...
	pub fn diff_since_saved(
		...
		if !self.modified {
			return PieceTreeDiff { equal: true, byte_ranges: Vec::new(), nodes_visited: 0 };
		}
		...
		let total_changed_bytes: usize =
			structure_diff.byte_ranges.iter().map(|r| r.end.saturating_sub(r.start)).sum();
		...
		if total_changed_bytes <= MAX_VERIFY_BYTES && !structure_diff.byte_ranges.is_empty() {
			if self.verify_content_differs_in_ranges(&structure_diff.byte_ranges, piece_tree, buffers) {
				return structure_diff;
			} else {
	...
	fn verify_content_differs_in_ranges(
		...
		for range in byte_ranges {
			...
			let saved_slice = extract_range_from_tree(&self.saved_root, range.start, range.end, buffers);
...
fn tree_total_bytes(root: &Arc<PieceTreeNode>) -> usize {
	match root.as_ref() {
		PieceTreeNode::Internal { left_bytes, right, .. } => left_bytes + tree_total_bytes(right),
