...
pub enum MarkerType {
	...
	LineAnchor { estimated_line: usize, confidence: AnchorConfidence },
}
...
impl IntervalTree {
	...
	fn insert_with_id(
		...
		debug_assert!(!self.marker_map.contains_key(&id), "insert_with_id: id {} already in use", id);
		let marker = Marker { id, interval: Interval { start, end }, marker_type, right_gravity };
	...
	pub fn insert_line_anchor(
		...
		self.insert_with_type(start, end, MarkerType::LineAnchor { estimated_line, confidence })
	}
	...
	pub fn delete(&mut self, id: MarkerId) -> bool {
		...
		let remove_rc = if two_children {
			...
			mem::swap(&mut node_rc.borrow_mut().marker, &mut succ.borrow_mut().marker);
		...
		match parent {
			...
			Some(ref p) => {
				let is_left = p.borrow().left.as_ref().is_some_and(|l| Rc::ptr_eq(l, &remove_rc));
				if is_left {
		...
		if delta > 0 {
			...
			if has_mover && !stayers.is_empty() {
				...
				for (id, end, _rg, mtype) in stayers {
					...
					// in adjust_recursive.
					let new_end = if end > pos { (end as i64 + delta) as u64 } else { end };
	...
	fn collect_starts_at(
		...
		match pos.cmp(&start) {
			...
			Ordering::Equal => {
				...
				out.push((nb.marker.id, end, nb.marker.right_gravity, nb.marker.marker_type.clone()));
	...
	pub fn update_line_anchor(
		...
		if let Some(node_rc) = self.marker_map.get(&id) {
			let mut node = node_rc.borrow_mut();
			node.marker.marker_type = MarkerType::LineAnchor { estimated_line, confidence };
			true
		} else {
			false
		}
...
impl IntervalTree {
	...
	fn adjust_recursive(node_opt: &mut NodePtr, pos: u64, delta: i64) {
		...
		let shift_end = if left_gravity && delta > 0 { end > pos } else { end >= pos };
		if shift_end {
			node.marker.interval.end = (end as i64 + delta).max(node.marker.interval.start as i64) as u64;
		}
...
mod tests {
	...
	fn get_pos(tree: &IntervalTree, id: MarkerId) -> (u64, u64) {
		tree.get_position(id).unwrap_or_else(|| panic!("Marker ID {} not found.", id))
	}
	...
	fn test_gravity_reversal_preserves_bst_for_later_edits() {
		...
		assert_eq!(get_pos(&tree, right), (15, 15), "right-gravity marker moves");
	...
	fn test_gravity_reversal_after_clamp_with_unordered_ids() {
		...
		assert_eq!(tree.debug_dump().len(), 2, "tree leaked a duplicate node: {:?}", tree.debug_dump());
	}
	...
	fn test_problematic_lazy_delta_scenario() {
		...
		assert_eq!(get_pos(&tree, id_l), (100, 200), "L(100) should expand to (100, 200).");

		// P(200) should be shifted (200 >= 150) -> 250
		assert_eq!(get_pos(&tree, id_p), (250, 300), "P(200) did not shift correctly. Should be 250.");

		// R(300) should be shifted (300 >= 150) -> 350
		assert_eq!(get_pos(&tree, id_r), (350, 400), "R(300) did not shift correctly. Should be 350.");
	}
	...
	fn test_interval_spanning_edit() {
		...
		assert_eq!(get_pos(&tree, id_s), (50, 210), "Spanning marker end did not move correctly.");
	}
