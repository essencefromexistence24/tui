...
fn render_args(args: &HashMap<String, Value>) -> Option<String> {
	...
	let parts: Vec<String> = keys
		...
		.map(|k| {
			let v = &args[*k];
			format!("{}: {}", k, serde_json::to_string(v).unwrap_or_else(|_| "null".to_string()))
		})
		.collect();
...
fn typed_text_summary(actions: &[Action]) -> Option<String> {
	...
	Some(runs.iter().map(|r| format!("{:?}", r)).collect::<Vec<_>>().join(" "))
}

/// Turn a register key into a valid JS identifier fragment for a handler name.
fn sanitize_ident(c: char) -> String {
	if c.is_ascii_alphanumeric() { c.to_string() } else { format!("u{:04x}", c as u32) }
}
...
pub fn generate_define_block(register: char, actions: &[Action]) -> String {
	...
	out.push_str(&format!("// fresh:macro {} — generated by Dx, editable\n", register));
...
pub fn generate_promote_block(register: char, actions: &[Action]) -> String {
	...
	let mut out = String::new();
	out.push_str(&format!("// fresh:macro {} — promoted to a command; edit freely\n", register));
	if let Some(t) = typed_text_summary(actions) {
		out.push_str(&format!("// types: {}\n", t));
	}
	out.push_str(&format!("registerHandler({}, async function () {{\n", js_string(&handler)));
