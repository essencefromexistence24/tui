...
impl Editor {
			...
			&self.active_layout().split_areas
		{
			...
			let (gutter_width, collapsed_header_bytes) = {
				...
				let headers = self
					...
					.get(split_id)
					.map(|vs| vs.folds.collapsed_header_bytes(&state.buffer, &state.marker_list))
					.unwrap_or_default();
	...
	pub(super) fn handle_editor_click(
		...
		let modifiers_str =
			if modifiers.contains(KeyModifiers::SHIFT) { "shift".to_string() } else { String::new() };
			...
			match target {
				Some(byte_pos) => {
					...
					if let Some(s) = state {
						...
						(Some(line.min(u32::MAX as usize) as u32), Some(col_b.min(u32::MAX as usize) as u32))
					} else {
						(None, None)
					}
				}
		...
		if self.plugin_manager.read().unwrap().has_hook_handlers("mouse_click") {
				...
				.get(&buffer_id)
		{
			...
			let Some(click_target) = super::click_geometry::screen_to_buffer_position_with_overshoot(
				col,
				row,
				content_rect,
				gutter_width,
			...
			let onclick_action = state
				...
				.iter()
				.find_map(|prop| prop.get("onClick").and_then(|v| v.as_str()).map(|s| s.to_string()));

			(toggle_fold_byte, onclick_action, click_target, cursor_snapshot)
		} else {
			return Ok(());
		};
		...
		if virtual_lines_below > 0 {
			...
			target_position =
				self.buffers().get(&buffer_id).map(|state| state.buffer.len()).unwrap_or(target_position);
		}

		if toggle_fold_byte.is_some() {
			self.active_window_mut().toggle_fold_at_byte(buffer_id, target_position);
			return Ok(());
		}
		...
		if let Some(action_name) = onclick_action {
			...
			tracing::debug!("onClick triggered at position {}: action={}", target_position, action_name);
		...
		let new_sticky_column = self.buffers().get(&buffer_id).and_then(|state| {
			...
			{
				if let Some(width) =
					crate::model::virtual_space::line_width_at_content_end(&state.buffer, target_position)
				{
