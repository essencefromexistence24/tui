...
fn find_word_boundary_left(line: &str, from_column: usize) -> usize {
	...
	while pos > 0
		&& graphemes.get(pos.saturating_sub(1)).is_some_and(|g| g.chars().all(|c| c.is_whitespace()))
	{
		pos -= 1;
	}
...
fn find_word_end_right(line: &str, from_column: usize, line_length: usize) -> usize {
	...
	if pos < graphemes.len()
		&& !graphemes[pos].chars().any(|c| c.is_alphanumeric() || c == '_')
		&& !graphemes[pos].chars().all(|c| c.is_whitespace())
	{
		...
		while pos < graphemes.len() && graphemes[pos].chars().any(|c| c.is_alphanumeric() || c == '_') {
			pos += 1;
		}
	}
...
impl crate::app::window::Window {
	...
	pub fn active_composite_cursor_info(&self) -> Option<(usize, usize, Vec<Option<usize>>)> {
		...
		let lines: Vec<Option<usize>> = (0..pane_count)
			.map(|i| row.and_then(|r| r.get_pane_line(i)).map(|line_ref| line_ref.line))
			.collect();
	...
	pub fn close_composite_buffer(&mut self, buffer_id: BufferId) {
		...
		self.composite_view_states.retain(|(_, bid), _| *bid != buffer_id);
	}
	...
	fn sync_editor_cursor_from_composite(&mut self, split_id: LeafId, buffer_id: BufferId) {
		...
		if let Some(state) = self.buffers.get_mut(&buffer_id) {
			state.primary_cursor_line_number = crate::model::buffer::LineNumber::Absolute(display_line);
		}
...
pub(crate) struct CreateCompositeBufferArgs {
	pub name: String,
	pub mode: String,
	pub layout_config: dx_core::api::CompositeLayoutConfig,
	pub source_configs: Vec<dx_core::api::CompositeSourceConfig>,
	pub hunks: Option<Vec<dx_core::api::CompositeHunk>>,
...
impl Editor {
	...
	pub fn get_composite_view_state(
		...
		if !self.active_window().composite_buffers.contains_key(&buffer_id) {
			return None;
		}

		let pane_count = self.active_window().composite_buffers.get(&buffer_id)?.pane_count();
	...
	pub fn create_composite_buffer(
		...
		let composite = CompositeBuffer::new(buffer_id, name.clone(), mode.clone(), layout, sources);
		self.active_window_mut().composite_buffers.insert(buffer_id, composite);
		...
		self.active_window_mut().event_logs.insert(buffer_id, crate::model::event::EventLog::new());
	...
	fn get_cursor_line_info(&self, split_id: LeafId, buffer_id: BufferId) -> CursorLineInfo {
		...
		let view_state = self.active_window().composite_view_states.get(&(split_id, buffer_id));
		...
		if let (Some(composite), Some(view_state)) = (composite, view_state) {
			...
			let pane_width =
				view_state.pane_widths.get(view_state.focused_pane).copied().unwrap_or(40) as usize;

			CursorLineInfo { content, length, pane_width }
		} else {
			CursorLineInfo { content: String::new(), length: 0, pane_width: 40 }
		}
	}
	...
	fn apply_cursor_movement(
			...
			match movement {
				...
				CursorMovement::Left => {
					if view_state.cursor_column > 0 {
						...
						if let Some(composite) = composite {
							wrapped_to_new_line = wrap_cursor_to_prev_content_row(view_state, composite);
						}
					}
				}
				CursorMovement::Right => {
					if view_state.cursor_column < line_info.length {
						...
						if let Some(composite) = composite {
							wrap_cursor_to_next_content_row(view_state, composite, max_row, viewport_height);
						}
					}
				}
				...
				CursorMovement::WordEnd => {
					let new_col =
						find_word_end_right(&line_info.content, view_state.cursor_column, line_info.length);
					if new_col > view_state.cursor_column {
						view_state.cursor_column = new_col;
						view_state.sticky_column = new_col;
						scroll_panes_right_to_cursor(view_state, line_info.pane_width);
		...
		if is_vertical || wrapped_to_new_line {
			...
			if let Some(view_state) =
				self.active_window_mut().composite_view_states.get_mut(&(split_id, buffer_id))
			{
					...
					&& matches!(movement, CursorMovement::Left | CursorMovement::WordLeft)
				{
					...
					if visible_width > 0 && view_state.cursor_column >= visible_width {
						let new_left = view_state.cursor_column.saturating_sub(visible_width.saturating_sub(1));
						for viewport in &mut view_state.pane_viewports {
	...
	fn handle_cursor_movement_action(
		...
		self.active_window_mut().sync_editor_cursor_from_composite(split_id, buffer_id);
	...
	pub fn handle_composite_action(
		...
		let _view_state = self.active_window().composite_view_states.get(&(split_id, buffer_id))?;

		match action {
			// Tab switches between panes
			Action::InsertTab => {
				self.active_window_mut().composite_focus_next(split_id, buffer_id);
				Some(true)
			}
			...
			Action::MoveRight => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::Right, false)
			}
			Action::MoveLineStart | Action::SmartHome => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::LineStart, false)
			}
			Action::MoveLineEnd => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::LineEnd, false)
			}
			Action::MoveWordLeft => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::WordLeft, false)
			}
			Action::MoveWordRight => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::WordRight, false)
			}
			Action::MoveWordEnd | Action::ViMoveWordEnd => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::WordEnd, false)
			}
			...
			Action::SelectLineStart => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::LineStart, true)
			}
			Action::SelectLineEnd => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::LineEnd, true)
			}
			Action::SelectWordLeft => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::WordLeft, true)
			}
			Action::SelectWordRight => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::WordRight, true)
			}
			Action::SelectWordEnd | Action::ViSelectWordEnd => {
				self.handle_cursor_movement_action(split_id, buffer_id, CursorMovement::WordEnd, true)
			}
			...
			Action::ScrollDown | Action::ScrollUp => {
				...
				self.active_window_mut().composite_scroll(split_id, buffer_id, delta);
				Some(true)
			}
	...
	fn handle_composite_copy(&mut self, split_id: LeafId, buffer_id: BufferId) {
		let text = {
			...
			let view_state = match self.active_window().composite_view_states.get(&(split_id, buffer_id))
			{
				Some(vs) => vs,
				None => return,
			};
	...
	pub(crate) fn handle_create_composite_buffer(&mut self, args: CreateCompositeBufferArgs) {
		...
		let layout = match layout_config.layout_type.as_str() {
			"stacked" => CompositeLayout::Stacked { spacing: layout_config.spacing.unwrap_or(1) },
		...
		if let Some(hunk_configs) = hunks {
			...
			let old_line_count = self
				.buffers()
				.get(&self.active_window().composite_buffers.get(&buffer_id).unwrap().sources[0].buffer_id)
				.and_then(|s| s.buffer.line_count())
				.unwrap_or(0);
			let new_line_count = self
				.buffers()
				.get(&self.active_window().composite_buffers.get(&buffer_id).unwrap().sources[1].buffer_id)
				.and_then(|s| s.buffer.line_count())
				.unwrap_or(0);

			let alignment = LineAlignment::from_hunks(&diff_hunks, old_line_count, new_line_count);
			self.active_window_mut().set_composite_alignment(buffer_id, alignment);
		}

		// Store initial focus hunk for the first render to apply
		if initial_focus_hunk.is_some() {
			if let Some(composite) = self.active_window_mut().composite_buffers.get_mut(&buffer_id) {
				composite.initial_focus_hunk = initial_focus_hunk;
			}
		}

		tracing::info!("Created composite buffer '{}' with mode '{}' (id={:?})", name, mode, buffer_id);
		...
		if let Some(req_id) = _request_id {
			...
			self
				...
				.read()
				.unwrap()
				.resolve_callback(dx_core::api::JsCallbackId::from(req_id), result);
			tracing::info!("CreateCompositeBuffer: resolve_callback sent for request_id={}", req_id);
		}
	}
	...
	pub(crate) fn handle_update_composite_alignment(
		&mut self,
		buffer_id: BufferId,
		hunk_configs: Vec<dx_core::api::CompositeHunk>,
	) {
	...
	pub(crate) fn handle_composite_click(
		...
		let pane_idx = if let Some(view_state) =
			self.active_window().composite_view_states.get(&(split_id, buffer_id))
		{
		...
		let (pane_start_x, left_column) = if let Some(view_state) =
			self.active_window().composite_view_states.get(&(split_id, buffer_id))
		{
			let mut x = content_rect.x;
			for (i, &width) in view_state.pane_widths.iter().enumerate() {
			...
			let left_col = view_state.pane_viewports.get(pane_idx).map(|vp| vp.left_column).unwrap_or(0);
			(x, left_col)
		} else {
			(content_rect.x, 0)
		};
		let gutter_width = 4; // Line number width
		let visual_col = col.saturating_sub(pane_start_x).saturating_sub(gutter_width) as usize;
...
fn wrap_cursor_to_prev_content_row(
	...
	let has_content = |row: usize| {
		composite.alignment.get_row(row).is_some_and(|r| r.get_pane_line(focused_pane).is_some())
	};
...
fn scroll_panes_right_to_cursor(view_state: &mut CompositeViewState, pane_width: usize) {
	...
	let current_left =
		view_state.pane_viewports.get(view_state.focused_pane).map(|v| v.left_column).unwrap_or(0);
