...
impl Editor {
	...
	pub(super) fn complete_terminal_mode_side_effects(&mut self) {
		if self.active_window().focused_terminal_live() && self.active_window().is_editing_disabled() {
...
impl Window {
	...
	pub(super) fn focus_split(&mut self, split_id: LeafId, buffer_id: BufferId) -> FocusSplitOutcome {
		...
		let is_interactive_widget_panel =
			self.buffers.get(&buffer_id).map(|s| s.interactive_widget_panel).unwrap_or(false);
