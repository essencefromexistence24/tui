...
//! to disambiguate from Dx's pre-existing workspace-recovery and
...
impl TerminalBuffer {
	...
	pub fn new_live(terminal_id: crate::services::terminal::TerminalId) -> Self {
		Self { terminal_id, scrollback_splits: std::collections::HashSet::new() }
	}
...
pub struct Window {
	...
	pub file_explorer_slot_overrides:
		HashMap<String, Vec<dx_core::file_explorer::FileExplorerSlotEntry>>,
	...
	pub pending_async_prompt_callback: Option<dx_core::api::JsCallbackId>,
	...
	pub pending_dir_poll_rx: Option<
		std::sync::mpsc::Receiver<(
			Vec<(crate::view::file_tree::NodeId, PathBuf, Option<std::time::SystemTime>)>,
	...
	pub pending_next_key_callbacks: std::collections::VecDeque<dx_core::api::JsCallbackId>,
	...
	pub pending_key_capture_buffer: std::collections::VecDeque<dx_core::api::KeyEventPayload>,
	...
	pub review_hunks: Vec<dx_core::api::ReviewHunk>,
...
impl Window {
	...
	pub fn apply_folding_ranges_response(
		...
		state.folding_ranges.set_from_lsp(&state.buffer, &mut state.marker_list, lsp_ranges);
	}
	...
	pub fn has_pending_lsp_requests(&self) -> bool {
		!self.pending_completion_requests.is_empty() || self.pending_goto_definition_request.is_some()
	}
	...
	pub(crate) fn cancel_pending_lsp_requests(&mut self) {
		...
		if let Some(request_id) = self.pending_goto_definition_request.take() {
			tracing::debug!("Canceling pending LSP goto-definition request {}", request_id);
			self.send_lsp_cancel_request(request_id);
		}
	}
	...
	pub fn toggle_tab_bar(&mut self) {
		...
		let key = if self.tab_bar_visible { "toggle.tab_bar_shown" } else { "toggle.tab_bar_hidden" };
	...
	pub fn toggle_status_bar(&mut self) {
		...
		let key =
			if self.status_bar_visible { "toggle.status_bar_shown" } else { "toggle.status_bar_hidden" };
		self.set_status_message(rust_i18n::t!(key).to_string());
	}
	...
	pub fn toggle_debug_highlights(&mut self) {
		...
		if let Some(state) = self.buffers.get_mut(&buffer_id) {
			...
			let key =
				if state.debug_highlight_mode { "toggle.debug_mode_on" } else { "toggle.debug_mode_off" };
			self.set_status_message(rust_i18n::t!(key).to_string());
		}
	}
	...
	pub(super) fn update_modified_from_event_log(&mut self) {
		let buffer_id = self.active_buffer();
		let is_at_saved =
			self.event_logs.get(&buffer_id).map(|log| log.is_at_saved_position()).unwrap_or(false);
		if let Some(state) = self.buffers.get_mut(&buffer_id) {
	...
	pub fn dismiss_lsp_language(&mut self, language: &str) {
		self.user_dismissed_lsp_languages.insert(language.to_string());
	}
	...
	pub(crate) fn server_supports_code_action_resolve(&self) -> bool {
		let Some(language) = self.buffers.get(&self.active_buffer()).map(|s| s.language.clone()) else {
			return false;
		};
		{
			let lsp = &self.lsp;
	...
	pub(crate) fn send_prepare_rename(&mut self) {
		...
		let (line, character) = self.active_state().buffer.position_to_lsp_position(cursor_pos);
...

		{
			...
			if let Some(sh) = lsp.handle_for_feature_mut(&language, LspFeature::Rename) {
				if let Err(e) =
					sh.handle.prepare_rename(request_id, uri.as_uri().clone(), line as u32, character as u32)
				{
			...
			for sh in lsp.get_handles_mut(&language) {
				if sh.capabilities.completion_resolve {
					if let Err(e) = sh.handle.completion_resolve(request_id, item.clone()) {
						tracing::warn!("Failed to send completionItem/resolve to '{}': {}", sh.name, e);
					}
					return;
				}
			}
			...
			.with_buffer_and_view_states(buffer_id, |state, vs_map| {
				for vs in vs_map.values_mut() {
					if vs.keyed_states.contains_key(&buffer_id) {
						...
						buf_state.folds.add(&mut state.marker_list, start, end, placeholder.clone());
					}
				}
			})
			.is_some()
	...
	pub fn set_buffer_cursor_in_splits(
		...
		self.buffers.with_buffer_and_view_states(buffer_id, |state, vs_map| {
			...
			if moved_any {
				let line = state.buffer.offset_to_position(position).map(|p| p.line).unwrap_or(0);
	...
	pub fn set_split_scroll_to_byte(
		...
		self.buffers.with_buffer_and_split(buffer_id, leaf_id, |state, view_state| {
			...
			let target_line = state.buffer.offset_to_position(clamped_byte).map(|p| p.line).unwrap_or(0);
			view_state.viewport.scroll_to(&mut state.buffer, target_line);
	...
	pub fn restore_buffer_state_in_split(
		...
		self.buffers.with_buffer_and_split(buffer_id, split_id, |buffer_state, vs| {
	...
	pub fn scroll_split_by_lines(
		...
		leaf_id: LeafId,
		delta: i32,
		view_transform_tokens: Option<Vec<dx_core::api::ViewTokenWire>>,
		tab_size: usize,
	) {
		self.buffers.with_buffer_and_split(buffer_id, leaf_id, |state, view_state| {
			...
			if let Some(tokens) = view_transform_tokens {
				...
				view_state.viewport.scroll_up(buffer, &soft_breaks, &virtual_lines, lines_to_scroll);
			} else {
				let lines_to_scroll = delta as usize;
				view_state.viewport.scroll_down(buffer, &soft_breaks, &virtual_lines, lines_to_scroll);
			}
			view_state.viewport.set_skip_ensure_visible();
			...
			if let Some(folds) = view_state.keyed_states.get(&buffer_id).map(|bs| &bs.folds) {
				if !folds.is_empty() {
					...
					{
						let target_line =
							if delta >= 0 { range.end_line.saturating_add(1) } else { range.header_line };
						let target_byte = buffer.line_start_offset(target_line).unwrap_or_else(|| buffer.len());
	...
	pub fn scroll_overlay_preview_by_lines(&mut self, delta: i32) -> bool {
		...
		let (soft_breaks, virtual_lines) = match self.buffers.get(&buffer_id) {
			Some(s) => (s.collect_soft_break_positions(), s.collect_virtual_line_positions()),
			None => return false,
		};
		let Some(state) = self.buffers.get_mut(&buffer_id) else {
			return false;
		...
		if delta < 0 {
			viewport.scroll_up(buffer, &soft_breaks, &virtual_lines, delta.unsigned_abs() as usize);
		} else {
	...
	pub fn clear_lsp_overlays_for_buffer(
		...
		self.buffers.with_buffer_and_view_states(buffer_id, |state, vs_map| {
			state.overlays.clear_namespace(diagnostic_namespace, &mut state.marker_list);
	...
	pub fn new(
		...
		Self {
			...
			file_explorer_decoration_cache: crate::view::file_tree::FileExplorerDecorationCache::default(
			),
			...
			search_state: None,
			search_namespace: crate::view::overlay::OverlayNamespace::from_string("search".to_string()),
	...
	pub fn effective_active_pair(&self) -> (LeafId, BufferId) {
		...
		let outer_buf = mgr.active_buffer_id().expect("Editor always has at least one buffer");
	...
	pub fn active_state(&self) -> &crate::state::EditorState {
		...
		self.buffers.get(&buf).expect("active buffer must be present in window")
	}

	/// Mutable handle to this window's active buffer state.
	pub fn active_state_mut(&mut self) -> &mut crate::state::EditorState {
		let buf = self.active_buffer();
		self.buffers.get_mut(&buf).expect("active buffer must be present in window")
	}
	...
	pub fn active_event_log(&self) -> &crate::model::event::EventLog {
		let buf = self.active_buffer();
		self.event_logs.get(&buf).expect("active buffer must have an event log")
	}

	/// Mutable event log for the active buffer.
	pub fn active_event_log_mut(&mut self) -> &mut crate::model::event::EventLog {
		let buf = self.active_buffer();
		self.event_logs.get_mut(&buf).expect("active buffer must have an event log")
	}
	...
	pub fn split_terminal_scrollback(&self, split: LeafId, buffer_id: BufferId) -> bool {
		self.terminal_buffer(buffer_id).is_some_and(|tb| tb.is_scrollback_in(split))
	}
	...
	pub fn initialized_lsp_server_count(&self, language: &str) -> usize {
		self.lsp.get_handles(language).iter().filter(|sh| sh.capabilities.initialized).count()
	}
	...
	pub fn completion_capable_lsp_server_count(&self, language: &str) -> usize {
		self.lsp.get_handles(language).iter().filter(|sh| sh.capabilities.completion).count()
	}
	...
	pub fn is_lsp_server_ready(&self, language: &str) -> bool {
		...
		self.lsp_server_statuses.iter().any(|((lang, server_name), status)| {
			...
			self.lsp.server_scope(server_name).map(|scope| scope.accepts(language)).unwrap_or(false)
		})
	}
	...
	pub fn redirect_active_split_away_from_dock_if_needed(&mut self) {
		...
		let target = mgr
			.last_focused_where(is_editor_leaf)
			.or_else(|| mgr.root().leaf_split_ids().into_iter().find(|leaf| is_editor_leaf(*leaf)));
	...
	pub(crate) fn take_pending_semantic_token_range_request(
		...
		if let Some(request) = self.pending_semantic_token_range_requests.remove(&request_id) {
			self.semantic_tokens_range_in_flight.remove(&request.buffer_id);
			Some(request)
		} else {
			None
		}
	...
	pub fn move_cursor_to_visible_area(&mut self, split_id: LeafId, buffer_id: BufferId) {
		...
		if let Some(state) = self.buffers.get_mut(&buffer_id) {
			...
			if let Some(view_state) = self.split_view_states_mut().and_then(|vs| vs.get_mut(&split_id)) {
	...
	pub fn split_at_position(&self, col: u16, row: u16) -> Option<(LeafId, BufferId)> {
		for &(split_id, buffer_id, content_rect, scrollbar_rect, _, _) in &self.layout_cache.split_areas
		{
	...
	pub fn check_diagnostic_pull_timer(&mut self) -> bool {
		...
		if let Err(e) =
			sh.handle.document_diagnostic(request_id, uri.as_uri().clone(), previous_result_id)
		{
			tracing::debug!("Failed to pull diagnostics after edit for {}: {}", uri.as_str(), e);
		} else {
			tracing::debug!(
	...
	pub fn open_local_file(&mut self, path: &std::path::Path) -> anyhow::Result<BufferId> {
		...
		let resolved_path = if path.is_relative() { self.root.join(path) } else { path.to_path_buf() };
		...
		self.event_logs.insert(buffer_id, crate::model::event::EventLog::new());
	...
	pub fn search_match_at_primary_cursor(&self) -> Option<std::ops::Range<usize>> {
		...
		if pos < start + len { Some(start..start + len) } else { None }
	}
	...
	pub fn update_search_highlights(
		...
		for mat in regex.find_iter(&visible_text) {
			...
			let overlay = crate::view::overlay::Overlay::with_namespace_fixed_end(
				...
				crate::view::overlay::OverlayFace::Style { style: search_style },
				ns.clone(),
			)
	...
	pub fn reevaluate_search_overlays_around(
		...
		let win_end = state.buffer.line_start_offset(end_line + 1).unwrap_or(buf_len).min(buf_len);
	...
	pub fn schedule_folding_ranges_refresh(&mut self, buffer_id: BufferId) {
		...
		let next_time =
			std::time::Instant::now() + std::time::Duration::from_millis(FOLDING_RANGES_DEBOUNCE_MS);
	...
	pub fn schedule_semantic_tokens_full_refresh(&mut self, buffer_id: BufferId) {
		...
		self.semantic_tokens_full_debounce.insert(buffer_id, next_time);
	}
	...
	pub(crate) fn send_lsp_changes_for_buffer(
		...
		let metadata = match self.buffer_metadata.get(&buffer_id) {
			Some(m) => m,
			None => {
				tracing::debug!("send_lsp_changes_for_buffer: no metadata for buffer {:?}", buffer_id);
				return;
			}
		};
		...
		let language = match self.buffers.get(&buffer_id).map(|s| s.language.clone()) {
			Some(l) => l,
			None => {
				tracing::debug!("send_lsp_changes_for_buffer: no buffer state for {:?}", buffer_id);
				return;
			}
		};
		...
		if !handles_needing_open.is_empty() {
			let text = match self.buffers.get(&buffer_id).and_then(|s| s.buffer.to_string()) {
				Some(t) => t,
				None => {
					tracing::debug!("send_lsp_changes_for_buffer: buffer text not available for didOpen");
					return;
				}
			};
			...
			let lsp = &mut self.lsp;
			for sh in lsp.get_handles_mut(&language) {
				if handles_needing_open.iter().any(|(_, id)| *id == sh.handle.id()) {
					if let Err(e) = sh.handle.did_open(uri.as_uri().clone(), text.clone(), language.clone()) {
						tracing::warn!("Failed to send didOpen to '{}' before didChange: {}", sh.name, e);
					} else {
						tracing::debug!(
		...
		if any_sent {
			...
			if let Some(state) = self.buffers.get(&buffer_id) {
				if let Some(path) = state.buffer.file_path() {
					crate::services::lsp::diagnostics::invalidate_cache_for_file(&path.to_string_lossy());
				}
			}

			self.scheduled_diagnostic_pull =
				Some((buffer_id, std::time::Instant::now() + std::time::Duration::from_millis(1000)));

			if self.resources.config.editor.enable_inlay_hints {
				self.scheduled_inlay_hints_request = Some((
					buffer_id,
					std::time::Instant::now() + std::time::Duration::from_millis(INLAY_HINTS_DEBOUNCE_MS),
				));
			}
		}
	}
	...
	pub fn adjust_other_split_cursors_for_event(&mut self, event: &Event) {
		...
		let buffer_len = self.buffers.get(&current_buffer_id).map(|s| s.buffer.len()).unwrap_or(0);
		let Some((mgr, vs_map)) = self.buffers.splits_mut() else {
			return;
		};
	...
	pub(crate) fn handle_scroll_event(&mut self, line_offset: isize) {
		...
		if let Some(group) = self.scroll_sync_manager.find_group_for_split(active_split.into()) {
		...
		for split_id in splits_to_scroll {
			...
			let view_transform_tokens =
				vs_map.get(&split_id).and_then(|vs| vs.view_transform.as_ref()).map(|vt| vt.tokens.clone());
	...
	pub(crate) fn handle_set_viewport_event(&mut self, top_line: usize) {
		...
		if self.scroll_sync_manager.is_split_synced(active_split.into()) {
			if let Some(group) = self.scroll_sync_manager.find_group_for_split_mut(active_split.into()) {
			...
			let (left, right) = match self.scroll_sync_manager.find_group_for_split(active_split.into()) {
	...
	pub fn set_pane_buffer(&mut self, leaf: LeafId, buffer_id: BufferId) {
		let (mgr, vs_map) =
			self.buffers.splits_mut().expect("active window must have a populated split layout");
