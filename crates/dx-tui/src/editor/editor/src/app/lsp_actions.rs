...
impl Editor {
		...
		for (buffer_id, buf_path) in buffers_for_language {
			...
			if let Some(__win) = self.windows.get_mut(&__active_id) {
				...
				for (name, handle_id) in handles_needing_open {
					let sh = lsp.get_handles_mut(&lang_id).into_iter().find(|s| s.handle.id() == handle_id);
					...
					let __pending = &mut __win.pending_inlay_hints_requests;
					{
							...
							lsp.handle_for_feature_mut(&lang_id, crate::types::LspFeature::InlayHints)
						{
							...
							*__next_id += 1;
							...
							if let Err(e) = sh.handle.inlay_hints(request_id, uri.clone(), 0, 0, last_line, 10000)
							{
								tracing::debug!("Failed to request inlay hints for {}: {}", uri.as_str(), e);
							} else {
								__pending.insert(
									request_id,
									super::InlayHintsRequest { buffer_id, version: buffer_version },
								);
							}
						}
					}
	...
	pub fn handle_lsp_stop(&mut self) {
		let running_languages: Vec<String> =
			self.lsp().as_ref().map(|lsp| lsp.running_servers()).unwrap_or_default();
		...
		for lang in &running_languages {
			let server_names: Vec<String> =
				self.lsp().as_ref().map(|lsp| lsp.server_names_for_language(lang)).unwrap_or_default();
	...
	pub fn handle_lsp_toggle_for_buffer(&mut self) {
		...
		let lsp_configured = self.lsp().as_ref().and_then(|lsp| lsp.get_config(&language)).is_some();
		...
		if let Some(target) = action_key.strip_prefix("autostart:") {
			...
			if let Some((language, server_name)) = target.split_once('/') {
				if let Some(lsp_configs) = self.config_mut().lsp.get_mut(language) {
					...
					if let Err(e) = self.save_config() {
						tracing::warn!("Failed to save config after enabling LSP auto-start: {}", e);
					} else {
			...
			let running_server_names: Vec<String> = self
				...
				.filter_map(|((l, name), status)| {
					if l == &lang
						&& !matches!(status, crate::services::async_bridge::LspServerStatus::Shutdown,)
					{
						Some(name.clone())
					} else {
						None
...
impl crate::app::window::Window {
	...
	pub fn toggle_fold_at_byte(&mut self, buffer_id: crate::model::event::BufferId, byte_pos: usize) {
		...
		self.buffers.with_buffer_and_split(buffer_id, split_id, |state, view_state| {
			...
			if buf_state.folds.remove_by_header_byte(&state.buffer, &mut state.marker_list, header_byte) {
				return;
			}

			// Also unfold if the byte position is inside an existing fold.
			if buf_state.folds.remove_if_contains_byte(&mut state.marker_list, byte_pos) {
				return;
			}
			...
			if !state.folding_ranges.is_empty() {
				...
				let resolved = state.folding_ranges.resolved(&state.buffer, &state.marker_list);
				...
				let placeholder =
					range.collapsed_text.as_ref().filter(|text| !text.trim().is_empty()).cloned();
				...
				let load_end = byte_pos.saturating_add(max_scan_bytes).min(state.buffer.len());
				drop(state.buffer.get_text_range_mut(load_start, load_end - load_start));
...
fn create_fold(
	...
	buf_state.cursors.map(|cursor| {
		...
		let anchor_in_hidden =
			cursor.anchor.is_some_and(|anchor| anchor >= start_byte && anchor < end_byte);
	...
	buf_state.folds.add(&mut state.marker_list, start_byte, end_byte, placeholder);
...
impl Editor {
	...
	pub(crate) fn send_did_close_to_server(&mut self, language: &str, server_name: &str) {
		let uris: Vec<_> = self
			...
			.filter_map(|(id, _)| {
				self.active_window().buffer_metadata.get(id).and_then(|m| m.file_uri()).cloned()
			})
			.collect();
	...
	pub(crate) fn stop_lsp_server_and_cleanup(
		...
		let stopping_names: Vec<String> = if let Some(name) = server_name {
			...
			self.lsp().map(|lsp| lsp.server_names_for_language(language)).unwrap_or_default()
		};

		let __active_id = self.active_window;

		let stopped = if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
		...
		for name in &stopping_names {
			self.active_window_mut().lsp_server_statuses.remove(&(language.to_string(), name.clone()));
		...
		let any_handle_left = self.lsp().as_ref().is_some_and(|lsp| lsp.has_handles(language));
		if !any_handle_left {
			self.active_window_mut().lsp_progress.retain(|_, info| info.language != language);
		}
	...
	pub(crate) fn disable_lsp_for_buffer(&mut self, buffer_id: crate::model::event::BufferId) {
		...
		if let Some(uri) =
			self.active_window().buffer_metadata.get(&buffer_id).and_then(|m| m.file_uri()).cloned()
		{
			let language = self.buffers().get(&buffer_id).map(|s| s.language.clone()).unwrap_or_default();
			let __active_id = self.active_window;
			if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
				// Broadcast didClose to all handles for this language
				if !lsp.has_handles(&language) {
					tracing::warn!("disable_lsp_for_buffer: no handle for language '{}'", language);
				} else {
					for sh in lsp.get_handles_mut(&language) {
						tracing::info!(
							"Sending didClose for {} to '{}' (language: {})",
		...
		if let Some(uri_str) = uri {
			...
			self.active_window_mut().stored_push_diagnostics.remove(&uri_str);
			self.active_window_mut().stored_pull_diagnostics.remove(&uri_str);
			self.active_window_mut().diagnostic_result_ids.remove(&uri_str);
		...
		self.active_window_mut().folding_ranges_in_flight.remove(&buffer_id);
		...
		self.active_window_mut().clear_lsp_overlays_for_buffer(buffer_id, &diagnostic_ns);
	}
	...
	fn send_lsp_did_open_for_buffer(
		...
		let (uri, text) = {
			...
			let text = self.buffers().get(&buffer_id).and_then(|state| state.buffer.to_string());
			(uri, text)
		};
		...
		use crate::services::lsp::manager::LspSpawnResult;
		let file_path =
			self.active_window().buffer_metadata.get(&buffer_id).and_then(|m| m.file_path()).cloned();
		...
		let inlay_buffer_info: Option<(u32, u32, u64)> =
			self.windows.get(&self.active_window).and_then(|w| w.buffers.get(&buffer_id)).map(|state| {
				let line_count = state.buffer.line_count().unwrap_or(1000);
				(line_count.saturating_sub(1) as u32, 10000u32, state.buffer.version())
			});
		let __active_id = self.active_window;
		let enable_inlay_hints = self.config.editor.enable_inlay_hints;
		...
		if let Err(e) = handle.document_diagnostic(request_id, uri.as_uri().clone(), previous_result_id)
		{
		...
		if enable_inlay_hints {
			let (last_line, last_char, buffer_version) = inlay_buffer_info.unwrap_or((999, 10000, 0));
				...
				handle.inlay_hints(request_id, uri.as_uri().clone(), 0, 0, last_line, last_char)
			{
				...
				__win
					.pending_inlay_hints_requests
					.insert(request_id, super::InlayHintsRequest { buffer_id, version: buffer_version });
			}
		}
	...
	pub(crate) fn setup_plugin_dev_lsp(&mut self, buffer_id: BufferId, content: &str) {
		...
		// Use the exact cached extraction location for dx.d.ts
		#[cfg(feature = "embed-plugins")]
		let fresh_dts_path = {
			let Some(embedded_dir) = crate::services::plugins::embedded::get_embedded_plugins_dir()
			else {
				tracing::warn!("Cannot set up plugin dev LSP: embedded plugins directory not available");
				return;
			};
			let path = embedded_dir.join("lib").join("dx.d.ts");
			if !path.exists() {
				tracing::warn!("Cannot set up plugin dev LSP: dx.d.ts not found at {:?}", path);
				return;
			}
			path
		};
		...
		match PluginDevWorkspace::create(buffer_id_num, content, &fresh_dts_path) {
			Ok(workspace) => {
				...
				if let Some(uri) = plugin_file_uri {
					if let Some(metadata) = self.active_window_mut().buffer_metadata.get_mut(&buffer_id) {
						metadata.kind =
							super::types::BufferKind::File { path: plugin_file.clone(), uri: Some(uri) };
						metadata.lsp_enabled = true;
						metadata.lsp_disabled_reason = None;
				...
				self.active_window_mut().plugin_dev_workspaces.insert(buffer_id, workspace);

				// Actually spawn the LSP server and send didOpen for this buffer
				self.send_lsp_did_open_for_buffer(buffer_id, "typescript");

				// Add the plugin workspace folder so tsserver discovers tsconfig.json + dx.d.ts
				if let Some(lsp) = self.lsp() {
					if let Some(handle) = lsp.get_handle("typescript") {
						if let Some(uri) = super::types::file_path_to_lsp_uri_with_translation(
							...
							let name =
								workspace_dir.file_name().unwrap_or_default().to_string_lossy().into_owned();
							if let Err(e) = handle.add_workspace_folder(uri, name) {
								tracing::warn!("Failed to add plugin workspace folder: {}", e);
							} else {
								tracing::info!("Added plugin workspace folder: {:?}", workspace_dir);
							}
						}
					}
				}
