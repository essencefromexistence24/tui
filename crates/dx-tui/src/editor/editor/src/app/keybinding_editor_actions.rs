...
use crate::view::keybinding_editor::{KeybindingEditorAction, handle_keybinding_editor_input};
				...
				if editor.edit_dialog.is_some() {
					...
					if let Some(r) = layout.dialog_action_field {
						if point_in_rect(r, col, row) {
							if let Some(ref mut dialog) = editor.edit_dialog {
								dialog.focus_area = 1;
								dialog.mode = crate::app::keybinding_editor::EditMode::EditingAction;
							}
						}
					}
					if let Some(r) = layout.dialog_context_field {
						if point_in_rect(r, col, row) {
							if let Some(ref mut dialog) = editor.edit_dialog {
								dialog.focus_area = 2;
								dialog.mode = crate::app::keybinding_editor::EditMode::EditingContext;
							}
						}
					}
					self.keybinding_editor = Some(editor);
