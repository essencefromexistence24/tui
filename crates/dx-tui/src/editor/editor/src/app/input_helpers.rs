...
impl Editor {
	...
	pub(super) fn start_switch_to_tab_prompt(&mut self) {
		...
		let current_index = open_buffers.iter().position(|&id| id == self.active_buffer()).unwrap_or(0);
		...
		let suggestions: Vec<crate::input::commands::Suggestion> = open_buffers
			...
			.map(|&buffer_id| {
				...
				let is_modified = self.buffers().get(&buffer_id).is_some_and(|b| b.buffer.is_modified());
		...
		if let Some(events) = self.active_window_mut().action_to_events(action) {
			if events.len() > 1 {
				...
				if has_buffer_mods {
					...
					let batch = Event::Batch { events: events.clone(), description: action_description };
	...
	pub(super) fn track_cursor_movement(&mut self, event: &Event) {
		...
		if let Event::MoveCursor { new_position, new_anchor, .. } = event {
			let buffer_id = self.active_buffer();
	...
	pub(super) fn try_route_composite_key(
		...
		match CompositeInputRouter::route_key_event(composite, view_state, key_event) {
			...
			RoutedEvent::SwitchPane(dir) => {
				match dir {
					Direction::Next => self.active_window_mut().composite_focus_next(split_id, buffer_id),
					Direction::Prev => self.active_window_mut().composite_focus_prev(split_id, buffer_id),
				}
				Some(Ok(()))
			}
