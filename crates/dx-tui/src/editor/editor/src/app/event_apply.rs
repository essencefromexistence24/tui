	...
	pub fn log_and_apply_event(&mut self, event: &Event) {
		...
		if let Event::Delete { range, .. } = event {
			...
			if !displaced.is_empty() {
				self.active_event_log_mut().set_displaced_markers_on_last(displaced);
			}
		} else {
			self.active_event_log_mut().append(event.clone());
		}
	...
	pub fn apply_event_to_active_buffer(&mut self, event: &Event) {
		...
		match event {
			...
			Event::SetViewport { top_line } => {
				self.active_window_mut().handle_set_viewport_event(*top_line);
				return;
			}
			Event::Recenter => {
		...
		if event.modifies_buffer() {
			self.active_window_mut().promote_active_buffer_from_preview();
		}
		...
		// (issue #1620).
		{
			...
			self.active_window_mut().apply_event_to_keyed_buffer(active_buf, split_id, event);
		}
		...
		match event {
			...
			Event::Batch { events, .. } => {
				let has_edits =
					events.iter().any(|e| matches!(e, Event::Insert { .. } | Event::Delete { .. }));
				if has_edits {
					let buf = self.active_buffer();
					let win = self.active_window_mut();
					win.invalidate_layouts_for_buffer(buf);
		...
		self.active_window_mut().adjust_other_split_cursors_for_event(event);
		...
		if lsp_changes.is_empty() && event.modifies_buffer() {
			if let Some(full_text) = self.active_state().buffer.to_string() {
				let full_change =
					vec![TextDocumentContentChangeEvent { range: None, range_length: None, text: full_text }];
				let buf = self.active_buffer();
				self.active_window_mut().send_lsp_changes_for_buffer(buf, full_change);
			}
		} else {
			let buf = self.active_buffer();
			self.active_window_mut().send_lsp_changes_for_buffer(buf, lsp_changes);
		}
	}
	...
	pub fn apply_events_as_bulk_edit(
		...
		for (cursor_id, ref mut pos, ref mut anchor) in &mut new_cursors {
			...
			for event in &events {
				...
				} = event
				{
					if event_cursor == cursor_id {
						...
						let shift = if insert_at_cursor_pos { calc_shift(original_pos) } else { 0 };
			...
			if !found_move_cursor {
				...
				for event in &events {
					match event {
						Event::Insert { position, text, cursor_id: event_cursor }
							if event_cursor == cursor_id =>
						{
						...
						Event::Delete { range, cursor_id: event_cursor, .. } if event_cursor == cursor_id => {
		...
		// https://github.com/anomalyco/dx/issues/1958.
		self
			.windows
			.get_mut(&self.active_window)
			.map(|w| &mut w.buffers)
		...
		let full_content_change = self
			...
			.map(|text| vec![TextDocumentContentChangeEvent { range: None, range_length: None, text }])
			.unwrap_or_default();
	...
	fn trigger_plugin_hooks_for_event(&mut self, event: &Event, line_info: EventLineInfo) {
		...
		let hook_args = match event {
			Event::Insert { position, text, .. } => {
				...
				if let Some(seen) = self.active_window_mut().seen_byte_ranges.get_mut(&buffer_id) {
		...
		let edit_changed_line_count =
			matches!(event, Event::Insert { .. } | Event::Delete { .. }) && line_info.line_delta != 0;
