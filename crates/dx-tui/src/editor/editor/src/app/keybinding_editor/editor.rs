...
use crate::input::keybindings::{
	Action, KeyContext, KeybindingResolver, format_keybinding, normalize_key,
};
...
impl KeybindingEditor {
	...
	pub fn new(
		...
		let bindings = Self::resolve_all_bindings(config, resolver, mode_registry, command_registry);
		...
		let mut mode_contexts: Vec<String> = resolver
			...
			.keys()
			.filter_map(|ctx| {
				if let KeyContext::Mode(name) = ctx { Some(format!("mode:{}", name)) } else { None }
			})
			.collect();
		mode_contexts.sort();
	...
	fn keybinding_to_resolved(
		...
		if !kb.keys.is_empty() {
			...
			let original_config = if source == BindingSource::Custom { Some(kb.clone()) } else { None };
	...
	fn expand_variant_actions(actions: &mut Vec<String>, menu_names: &[String], config: &Config) {
		...
		let mut keymaps: Vec<String> =
			["default", "emacs", "vscode", "macos"].map(String::from).to_vec();
	...
	pub fn update_autocomplete(&mut self) {
		if let Some(ref mut dialog) = self.edit_dialog {
			...
			dialog.autocomplete_selected = if dialog.autocomplete_visible { Some(0) } else { None };
				...
				match self.search_mode {
					SearchMode::Text => {
						if !self.search_query.is_empty() {
							...
							let matches = binding.action.to_lowercase().contains(&query)
								...
								|| binding.command_name.as_ref().is_some_and(|n| n.to_lowercase().contains(&query));
							if !matches {
								continue;
							}
						}
					...
					SearchMode::RecordKey => {
						if let Some(search_key) = self.search_key_code {
							if !binding.is_chord {
								let key_matches =
									binding.key_code == search_key && binding.modifiers == self.search_modifiers;
								if !key_matches {
									continue;
								}
							} else {
	...
	fn build_display_rows(&mut self) {
		...
		for (plugin_name, indices) in sections {
			...
			let collapsed =
				if has_active_filter { false } else { self.collapsed_sections.contains(&plugin_name) };
	...
	pub fn selected_is_section_header(&self) -> bool {
		matches!(self.display_rows.get(self.selected), Some(DisplayRow::SectionHeader { .. }))
	}
	...
	pub fn cycle_context_filter(&mut self) {
		...
		let current_idx = contexts.iter().position(|c| c == &self.context_filter).unwrap_or(0);
	...
	pub fn open_edit_dialog(&mut self) {
		if let Some(idx) = self.selected_binding_index() {
			...
			self.edit_dialog =
				Some(EditBindingState::new_edit_with_modes(idx, &binding, &self.mode_contexts));
		}
	}
	...
	fn override_binding_with_noop(&mut self, idx: usize) -> DeleteResult {
		...
		let noop_kb = Keybinding {
			key: if binding.is_chord { String::new() } else { key_code_to_config_name(binding.key_code) },
			...
			args: HashMap::new(),
			when: if binding.context.is_empty() { None } else { Some(binding.context.clone()) },
		};
	...
	pub fn apply_edit_dialog(&mut self) -> Option<String> {
		...
		if !self.is_valid_action(&dialog.action_text) {
			let err_msg =
				t!("keybinding_editor.error_unknown_action", action = &dialog.action_text).to_string();
			let mut dialog = dialog;
			dialog.action_error = Some(
				t!("keybinding_editor.error_unknown_action_short", action = &dialog.action_text)
					.to_string(),
			);
			self.edit_dialog = Some(dialog);
			return Some(err_msg);
		...
		// Update display
		let key_display = format_keybinding(&key_code, &modifiers);
		let action_display = KeybindingResolver::format_action_from_str_with_args(&bare_action, &args);
...
mod tests {
	...
	fn make_editor(extra_menus: &[&str]) -> KeybindingEditor {
		...
		let mut menu_names: Vec<String> =
			["File", "Edit", "View"].iter().map(|s| s.to_string()).collect();
	...
	fn dropdown_lists_menu_open_variants_not_bare_entry() {
		...
		assert!(
			editor.available_actions.contains(&"menu_open:File".to_string()),
			"expected dropdown to list `menu_open:File`, got {:?}",
			editor.available_actions
		);
		assert!(
			editor.available_actions.contains(&"menu_open:Edit".to_string()),
	...
	fn dropdown_includes_plugin_menus_passed_in() {
		...
		assert!(
			editor.available_actions.contains(&"menu_open:MyPluginMenu".to_string()),
	...
	fn dropdown_lists_builtin_keybinding_maps() {
		...
		assert!(
			!editor.available_actions.iter().any(|a| a == "switch_keybinding_map"),
