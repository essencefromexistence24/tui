...
/// Phase-timing helper used when `DX_TEST_TIMING=1` is set so test
...
impl InitTimer {
	fn start(label: &'static str) -> Self {
		let enabled = std::env::var("DX_TEST_TIMING").is_ok_and(|v| !v.is_empty() && v != "0");
		let now = std::time::Instant::now();
		if enabled {
			eprintln!("[timing] {label}  start");
		}
		Self { label, start: now, last: now, enabled }
	}
...
fn set_dot_path(root: &mut serde_json::Value, path: &str, value: serde_json::Value) {
	...
	for seg in &segments[..segments.len() - 1] {
		...
		cur = cur.as_object_mut().unwrap().entry((*seg).to_string()).or_insert(serde_json::Value::Null);
	}
...
fn load_startup_plugins(
	...
	// Always check user config plugins directory (~/.config/dx/plugins)
	...
	// Check for package manager installed plugins (~/.config/dx/plugins/packages/*)
	...
	if defer_plugin_load {
		...
		#[cfg(feature = "plugins")]
		{
			...
			let mut dir_receivers: Vec<(
				std::path::PathBuf,
				dx_plugin_runtime::thread::oneshot::Receiver<
					dx_plugin_runtime::thread::PluginsDirLoadResult,
				>,
			)> = Vec::with_capacity(plugin_dirs.len());
			for plugin_dir in &plugin_dirs {
				tracing::info!("Submitting async TypeScript plugin load for: {:?}", plugin_dir);
					...
					.spawn(move || {
						for (dir, rx) in dir_receivers {
							...
							match rx.recv() {
								...
								Err(e) => {
									tracing::warn!("plugin-load-forwarder: dir {:?} recv failed: {}", dir, e);
								}
							}
						}
						if let Some(rx) = declarations_rx {
							match rx.recv() {
								...
								Err(e) => {
									tracing::warn!("plugin-load-forwarder: list_plugins recv failed: {}", e);
								}
							}
						}
					})
...
pub(super) struct EditorParts {
	...
	// Async / IO
	pub(super) tokio_runtime: Option<tokio::runtime::Handle>,
	...
	// just installs them.
	pub(super) windows: HashMap<dx_core::WindowId, crate::app::window::Window>,
	...
	pub(super) dormant_remote:
		HashMap<dx_core::WindowId, crate::app::orchestrator_persistence::PersistedWindow>,
	pub(super) active_window: dx_core::WindowId,
...
fn load_prompt_histories(
	...
	for history_name in ["search", "replace", "goto_line"] {
		...
		let history =
			crate::input::input_history::InputHistory::load_from_file(&path).unwrap_or_else(|e| {
...
fn build_persisted_window_shells(
	...
	active_window_id: dx_core::WindowId,
	...
	HashMap<dx_core::WindowId, crate::app::window::Window>,
	HashMap<dx_core::WindowId, crate::app::orchestrator_persistence::PersistedWindow>,
) {
	...
	for ps in &env.windows {
		...
		let id = if ps.id == active_window_id.0 {
			let fresh = dx_core::WindowId(next_fresh_id);
			next_fresh_id += 1;
			fresh
		} else {
			dx_core::WindowId(ps.id)
		};
		...
		if matches!(ps.authority_spec, crate::services::authority::SessionAuthoritySpec::RemoteAgent(_))
		{
...
impl Editor {
	...
	pub fn with_working_dir_opts(
		...
		Self::with_options(
			...
			grammar_registry,
			defer_plugin_load,
			None, // tokio_handle: let standalone path create its own runtime
		)
	}
	...
	pub fn for_test(
		...
		let mut editor = Self::with_options(
			...
			grammar_registry,
			false,
			None, // tokio_handle: let test path create its own runtime
		)?;
	...
	/// When `tokio_handle` is `Some`, it reuses that runtime handle instead
	/// of creating a separate tokio runtime (avoids two thread pools).
	#[allow(clippy::too_many_arguments)]
	fn with_options(
		mut config: Config,
		width: u16,
		...
		let scan_result = crate::services::packages::scan_installed_packages(&dir_context.config_dir);
		...
		let theme_inner = theme_registry.get_cloned(&config.theme).unwrap_or_else(|| {
			tracing::warn!("Theme '{}' not found, falling back to default theme", config.theme.0);
			theme_registry
				.get_cloned(&crate::config::ThemeName(crate::view::theme::THEME_HIGH_CONTRAST.to_string()))
		...
		state.margins.configure_for_line_numbers(config.editor.line_numbers);
		...
		let (active_window_id, _active_window_root) = picked_active
			.map(|w| (dx_core::WindowId(w.id), w.root.clone()))
			.unwrap_or((dx_core::WindowId(1), working_dir.clone()));

		t.phase("buffer_state");
		// Use the provided tokio handle (from the embedding TUI runtime)
		// or try to capture the current context handle, or fall back to
		// creating a small dedicated runtime.
		let tokio_runtime =
			tokio_handle.or_else(|| tokio::runtime::Handle::try_current().ok()).or_else(|| {
				// Fallback: create a small dedicated runtime for standalone use.
		...
		if tokio_runtime.is_none() {
			tracing::warn!("No Tokio runtime available - async features disabled");
		}
		...
		let update_checker = if check_for_updates {
			...
			Some(crate::services::release_checker::start_periodic_update_check(
		...
		let recovery_service = {
			let recovery_config =
				RecoveryConfig { enabled: recovery_enabled, ..RecoveryConfig::default() };
			...
			// Issue #1550: without per-CWD scoping, opening Dx in a
			// second folder would clobber the first folder's unsaved
			// unnamed buffers on shutdown.
			let scope =
				crate::services::recovery::RecoveryScope::Standalone { working_dir: working_dir.clone() };
		...
		let (active_label, active_root, active_plugin_state, active_authority_spec) = picked_active
			.map(|w| (w.label.clone(), w.root.clone(), w.plugin_state.clone(), w.authority_spec.clone()))
			.unwrap_or_else(|| {
				(
					String::new(),
					working_dir.clone(),
		...
		active_win.prompt_histories.extend(load_prompt_histories(&dir_context));
		...
		let background_shell_resources = crate::app::window_resources::WindowResources {
			fs_manager: Arc::new(FsManager::new(Arc::new(crate::model::filesystem::StdFileSystem))),
			..base_resources
		};
		...
		let max_existing = windows.keys().chain(dormant_remote.keys()).map(|k| k.0).max().unwrap_or(0);
		let next_window_id =
			persisted_env.as_ref().map(|env| env.next_id.max(max_existing + 1)).unwrap_or(2);

		let key_translator =
			crate::input::key_translator::KeyTranslator::load_from_config_dir(&dir_context.config_dir)
				.unwrap_or_default();
		...
		let needs_seed: Vec<dx_core::WindowId> = editor
			.windows
			.iter()
		...
		editor.materialize_pending =
			editor.windows.keys().copied().filter(|id| *id != editor.active_window).collect();
	...
	pub(super) fn start_background_grammar_build(
		...
		std::thread::Builder::new()
			...
			.spawn(move || {
				...
				drop(sender.send(crate::services::async_bridge::AsyncMessage::GrammarRegistryBuilt {
					registry,
					callback_ids,
				}));
			})
	...
	/// Auto-load `~/.config/dx/init.ts` if present, through the existing
	...
	pub fn load_init_script(&mut self, enabled: bool) {
		use crate::init_script::{
			CheckSeverity, InitOutcome, LoadDecision, check, decide_load, describe, record_success,
		...
		if enabled {
			...
			if !report.ok {
				for d in &report.diagnostics {
					...
					tracing::warn!("init.ts pre-load {level} at {}:{}: {}", d.line, d.column, d.message);
				}
			}
		}
		...
		let outcome = match decide_load(&config_dir, enabled) {
			...
			LoadDecision::Load { source } => {
				if !self.plugin_manager.read().unwrap().is_active() {
					InitOutcome::Failed {
						message: "plugin runtime inactive (--no-plugins); init.ts cannot run".into(),
					}
				} else {
					match self.plugin_manager.read().unwrap().load_plugin_from_source(
						...
						Err(e) => InitOutcome::Failed { message: format!("{e}") },
					}
				}
			}
		};
	...
	pub fn load_init_script_async(&mut self, enabled: bool) {
		use crate::init_script::{
			CheckSeverity, InitOutcome, LoadDecision, check, decide_load, refresh_types_scaffolding,
		};
		...
		let outcome_now: Option<PluginInitScriptOutcome> = match decide_load(&config_dir, enabled) {
			LoadDecision::Skip(outcome) => Some(match outcome {
				...
				InitOutcome::CrashFused { failures } => PluginInitScriptOutcome::CrashFused { failures },
		...
		if let Some(outcome) = outcome_now {
			...
			if let Some(bridge) = &self.async_bridge {
				drop(
					bridge
						.sender()
						.send(crate::services::async_bridge::AsyncMessage::PluginInitScriptLoaded(outcome)),
				);
			} else {
	...
	fn spawn_init_script_forwarder(&self, source: String) {
		...
		let Some(rx) = self.plugin_manager.read().unwrap().load_plugin_from_source_request(
			&source,
			crate::init_script::INIT_PLUGIN_NAME,
			true,
		) else {
		...
		std::thread::Builder::new()
			...
			.spawn(move || {
				...
				drop(
					sender.send(crate::services::async_bridge::AsyncMessage::PluginInitScriptLoaded(outcome)),
				);
			})
			.ok();
	}
	...
	pub fn handle_add_plugin_config_field(
		...
		tracing::trace!("Registering plugin config field: {}.{}", plugin_name, field_name);
		...
		let updated_schema = {
			...
			if let Some(props) =
				schema.as_object_mut().and_then(|o| o.get_mut("properties")).and_then(|p| p.as_object_mut())
			{
		...
		if let Err(msg) = crate::plugin_schemas::validate_plugin_schema(&updated_schema) {
			...
			self.set_status_message(format!("defineConfig({}.{}): {}", plugin_name, field_name, msg));
			return;
		}
	...
	pub(crate) fn handle_plugin_init_script_loaded(
		...
		use crate::init_script::{InitOutcome, describe, record_success};
	...
	pub fn fire_plugins_loaded_hook(&self) {
		...
		if self.plugin_manager.read().unwrap().is_active() {
			self
				...
				.read()
				.unwrap()
				.run_hook("plugins_loaded", crate::services::plugins::hooks::HookArgs::PluginsLoaded {});
		}
	}
...
fn populate_builtin_keybinding_labels(
	...
	let known_suffixes: Vec<String> =
		contexts.iter().map(|c| format!("\0{}", c.to_when_clause())).collect();
	snapshot.keybinding_labels.retain(|k, _| !known_suffixes.iter().any(|s| k.ends_with(s)));
	...
	let action_names = Action::all_action_names().into_iter().chain(plugin_action_names);
