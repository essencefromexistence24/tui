...
pub(crate) fn compose_lsp_status(
	...
	if lsp_progress.values().any(|info| info.language == current_language) {
		const SPINNER: &[char] = &['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
		...
		let idx = std::time::SystemTime::now()
			...
			.unwrap_or(0)
			% SPINNER.len();
		return (centered(&format!("LSP {}", SPINNER[idx])), LspIndicatorState::On);
	}
	...
	let configured_count = lsp_config
		.get(current_language)
		.map(|cfg| cfg.as_slice().iter().filter(|c| !c.command.is_empty()).count())
		.unwrap_or(0);
	if configured_count > 0 {
		...
		let any_enabled =
			lsp_config.get(current_language).is_some_and(|cfg| cfg.as_slice().iter().any(|c| c.enabled));
...
mod tests {
	...
	fn configured_for(lang: &str, command: &str) -> HashMap<String, LspLanguageConfig> {
		...
		server.enabled = true;
		m.insert(lang.to_string(), LspLanguageConfig::Single(Box::new(server)));
		m
	}
	...
	fn off_dismissed_when_all_servers_disabled_in_config() {
		...
		config.insert("rust".to_string(), LspLanguageConfig::Single(Box::new(server)));
		let (text, state) = compose_lsp_status(
			"rust",
			None,
			&HashMap::new(),
	...
	fn error_wins_over_running() {
		...
		statuses.insert(("rust".to_string(), "clippy".to_string()), LspServerStatus::Error);
		let (text, state) = compose_lsp_status(
			"rust",
			None,
			&HashMap::new(),
