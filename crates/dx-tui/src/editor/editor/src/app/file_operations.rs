...
impl Editor {
	...
	pub fn save(&mut self) -> anyhow::Result<()> {
		...
		if !self.authority().filesystem.is_remote_connected() {
			anyhow::bail!(
				...
				self.authority().filesystem.remote_connection_info().unwrap_or("unknown host")
			);
		}
	...
	pub(crate) fn finalize_save_buffer(
		...
		if let Some(ref p) = path {
			self.plugin_manager.read().unwrap().run_hook(
				"after_file_save",
				crate::services::plugins::hooks::HookArgs::AfterFileSave { buffer_id, path: p.clone() },
			);
		}
		...
		if !silent {
			match self.run_on_save_actions() {
				Ok(true) => {
					...
					if self.active_window_mut().status_message.as_deref() == Some(&t!("status.file_saved")) {
	...
	pub fn auto_save_persistent_buffers(&mut self) -> anyhow::Result<usize> {
		...
		if self.time_source.elapsed_since(self.active_window().last_persistent_auto_save) < interval {
			return Ok(0);
		}
				...
				match state.buffer.save() {
					...
					Err(e) => {
						...
						if e.downcast_ref::<SudoSaveRequired>().is_some() {
							tracing::debug!("Auto-save skipped for {:?} (sudo required)", path.display());
						} else {
	...
	pub(crate) fn collect_unnamed_modified_buffers(&self) -> Vec<BufferId> {
			...
			self.windows.get(&self.active_window).map(|w| &w.buffers).expect("active window present")
		{
			...
			let is_unnamed = state.buffer.file_path().map(|p| p.as_os_str().is_empty()).unwrap_or(true);
			if is_unnamed {
				out.push(*id);
			}
		}
	...
	pub(crate) fn start_next_quit_save_as(&mut self) -> bool {
		while let Some(buffer_id) = self.active_window_mut().pending_quit_unnamed_save.first().copied()
		{
			...
			let still_dirty_unnamed = self
				...
				.map(|s| {
					s.buffer.is_modified()
						&& s.buffer.file_path().map(|p| p.as_os_str().is_empty()).unwrap_or(true)
				})
				.unwrap_or(false);
			...
			self.start_prompt(t!("file.save_as_prompt").to_string(), PromptType::SaveFileAs);
			return true;
		}
		false
	}
		...
		for (id, path) in to_save {
				...
				.get_mut(&id)
			{
				match state.buffer.save() {
					...
					Err(e) => {
						if e.downcast_ref::<SudoSaveRequired>().is_some() {
							tracing::debug!("Auto-save on exit skipped for {} (sudo required)", path.display());
						} else {
	...
	pub fn revert_file(&mut self) -> anyhow::Result<bool> {
		let path = match self.active_state().buffer.file_path() {
			...
			None => {
				self.active_window_mut().status_message = Some(t!("status.no_file_to_revert").to_string());
				return Ok(false);
			}
		};
	...
	pub fn toggle_auto_revert(&mut self) {
		...
		if self.active_window().auto_revert_enabled {
			self.active_window_mut().status_message = Some(t!("status.auto_revert_enabled").to_string());
		} else {
			self.active_window_mut().status_message = Some(t!("status.auto_revert_disabled").to_string());
		}
	}
	...
	fn process_dir_poll_results(
		&mut self,
		results: Vec<(crate::view::file_tree::NodeId, PathBuf, Option<std::time::SystemTime>)>,
		...
		for (node_id, path, mtime_opt) in results {
			...
			if let Some(&stored_mtime) = self.active_window_mut().dir_mod_times.get(&path) {
				if current_mtime != stored_mtime {
					self.active_window_mut().dir_mod_times.insert(path.clone(), current_mtime);
					dirs_to_refresh.push((node_id, path.clone()));
					tracing::debug!("Directory changed: {:?}", path);
				}
			} else {
				self.active_window_mut().dir_mod_times.insert(path, current_mtime);
			}
		}
	...
	pub(crate) fn notify_lsp_file_opened(
		...
		self.active_window_mut().notify_lsp_file_opened(path, buffer_id, metadata);
	}
	...
	pub(crate) fn notify_lsp_file_changed(&mut self, path: &Path) {
		...
		let Some((buffer_id, content, language)) = self
			...
			.and_then(|(id, state)| state.buffer.to_string().map(|t| (*id, t, state.language.clone())))
		else {
			return;
		};
		...
		// Send didOpen to any handles that haven't received it yet
		{
			...
			if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
				for sh in lsp.get_handles_mut(&language) {
					...
					if let Err(e) = sh.handle.did_open(lsp_uri.clone(), content.clone(), language.clone()) {
						tracing::warn!("Failed to send didOpen to LSP '{}' before didChange: {}", sh.name, e);
					} else {
						tracing::debug!(
		...
		if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
			...
			for sh in lsp.get_handles_mut(&language) {
				if let Err(e) = sh.handle.did_change(lsp_uri.clone(), vec![content_change.clone()]) {
	...
	pub fn handle_file_changed(&mut self, changed_path: &str) {
		...
		for buffer_id in buffer_ids {
			...
			if self.active_window().terminal_buffers.contains_key(&buffer_id) {
				continue;
			}
			...
			let current_mtime =
				match self.authority().filesystem.metadata(&path).ok().and_then(|m| m.modified) {
					Some(mtime) => mtime,
					None => continue, // Can't read file, skip
				};

			let dominated_by_stored =
				self.file_mod_times().get(&path).map(|stored| current_mtime <= *stored).unwrap_or(false);
			...
			if state.buffer.is_modified() {
				self.active_window_mut().status_message =
					Some(format!("File {} changed on disk (buffer has unsaved changes)", path.display()));
				continue;
			}
			...
			if self.active_window().auto_revert_enabled {
				...
				let still_needs_revert =
					self.file_mod_times().get(&path).map(|stored| current_mtime > *stored).unwrap_or(true);
	...
	pub fn check_save_conflict(&self) -> Option<std::time::SystemTime> {
		...
		let current_mtime = self.authority().filesystem.metadata(path).ok().and_then(|m| m.modified)?;
...
impl crate::app::window::Window {
	...
	pub(crate) fn notify_lsp_file_opened(
		...
		// Check file size
		let file_size = self.authority().filesystem.metadata(path).ok().map(|m| m.size).unwrap_or(0);
		if file_size > self.resources.config.editor.large_file_threshold_bytes {
			let reason = format!("File too large ({} bytes)", file_size);
			tracing::debug!("Skipping LSP for large file: {} ({})", path.display(), reason);
			...
			LspSpawnResult::Spawned => {
				...
				if enable_inlay_hints {
						...
						lsp.handle_for_feature_mut(&language, crate::types::LspFeature::InlayHints)
					{
						...
						if let Err(e) =
							sh.handle.inlay_hints(request_id, uri.as_uri().clone(), 0, 0, last_line, last_char)
						{
