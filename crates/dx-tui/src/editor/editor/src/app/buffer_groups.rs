...
#[cfg(feature = "plugins")]
use dx_core::api::BufferGroupResult;
#[cfg(feature = "plugins")]
...
impl super::Editor {
	...
	pub(super) fn create_buffer_group(
		...
		self.active_window_mut().grouped_subtrees.insert(group_leaf_id, grouped_node);
		...
		for (panel_name, leaf_id) in &panel_splits {
			let buffer_id =
				*panel_buffers.get(panel_name).ok_or(format!("Panel '{}' has no buffer", panel_name))?;
		...
		for buffer_id in panel_buffers.values() {
			self.active_window_mut().buffer_to_group.insert(*buffer_id, group_id);
		}

		self.active_window_mut().buffer_groups.insert(group_id, group);

		// Build result
		let panels: HashMap<String, u64> =
			panel_buffers.iter().map(|(name, bid)| (name.clone(), bid.0 as u64)).collect();

		Ok(BufferGroupResult { group_id: group_id.0 as u64, panels })
	}
	...
	fn build_split_tree(
		...
		match node {
			GroupLayoutNode::Scrollable { id, buffer_id: Some(bid), .. }
			| GroupLayoutNode::Fixed { id, buffer_id: Some(bid), .. } => {
				let split_id = self
					.windows
					.get_mut(&self.active_window)
					.and_then(|w| w.split_manager_mut())
			...
			GroupLayoutNode::Scrollable { buffer_id: None, .. }
			| GroupLayoutNode::Fixed { buffer_id: None, .. } => {
				Err("Layout leaf has no buffer_id".to_string())
			}
			GroupLayoutNode::Split { direction, ratio, first, second } => {
				...
				let split_id = self
					...
					.expect("active window must have a populated split layout")
					.allocate_split_id();
				let mut split = SplitNode::split(*direction, first_node, second_node, *ratio, split_id);
				// Apply fixed sizes from children
				let fixed_first_size = fixed_height_of(first);
				let fixed_second_size = fixed_height_of(second);
				if let SplitNode::Split { fixed_first, fixed_second, .. } = &mut split {
	...
	fn build_group_layout(
		...
		match desc {
			...
			LayoutDesc::Fixed { id, height, scrollable } => {
			...
			LayoutDesc::Split { direction, ratio, first, second } => {
	...
	pub(super) fn close_buffer_group(&mut self, group_id: usize) {
		...
		if let Some(group) = self.active_window_mut().buffer_groups.remove(&bg_id) {
			...
			if let Some(group_leaf_id) = group.representative_split {
				...
				self.active_window_mut().grouped_subtrees.remove(&group_leaf_id);
				...
				for vs in self
					...
					.values_mut()
				{
					vs.open_buffers.retain(|t| *t != TabTarget::Group(group_leaf_id));
	...
	pub(super) fn focus_panel(&mut self, group_id: usize, panel_name: String) {
		...
		if let Some(host_split) = host_split {
			...
			if let Some(crate::view::split::SplitNode::Grouped { active_inner_leaf, .. }) =
				self.active_window_mut().grouped_subtrees.get_mut(&group_leaf_id)
			{
	...
	pub(super) fn set_buffer_group_panel_buffer(
		...
		let buffer_exists = self
			...
			.unwrap_or(false);
		if !buffer_exists {
			tracing::warn!("setBufferGroupPanelBuffer: buffer {:?} not found", new_buffer_id);
			return false;
		}

		// Look up the panel's inner leaf id, and the prior buffer id
		// we're replacing.
		let (panel_leaf, prior_buffer_id) = match self.active_window_mut().buffer_groups.get_mut(&bg_id)
		{
			Some(group) => {
				...
				let prior = group.panel_buffers.insert(panel_name.clone(), new_buffer_id);
				(leaf, prior)
			}
			None => {
		...
		self.active_window_mut().buffer_to_group.insert(new_buffer_id, bg_id);
		...
		let wrap_column = self.active_window().resolve_wrap_column_for_buffer(new_buffer_id);
		...
		if let Some(meta) = self.active_window_mut().buffer_metadata.get_mut(&new_buffer_id) {
			meta.hidden_from_tabs = true;
		}
	...
	pub(crate) fn activate_group_tab(&mut self, split_id: LeafId, group_leaf: LeafId) {
		...
		let Some(crate::view::split::SplitNode::Grouped { active_inner_leaf, .. }) =
		...
		if self
			...
			!= split_id
		{
			self.active_window_mut().promote_preview_if_not_in_split(split_id);
			if self.active_window_mut().key_context == crate::input::keybindings::KeyContext::FileExplorer
			{
				self.active_window_mut().key_context = crate::input::keybindings::KeyContext::Normal;
			}
			self
	...
	pub(crate) fn set_grouped_split_ratio(
		...
		self.active_window_mut().set_grouped_split_ratio(container, new_ratio)
	}
