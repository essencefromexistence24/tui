...
impl crate::app::window::Window {
	...
	pub fn ensure_active_tab_visible(
		...
		self.buffers.with_all_mut(|buffer_map, _mgr, vs_map| {
			...
			let max_visible_width =
				crate::view::ui::tabs::tabs_render_width(total_tabs_width, available_width as usize);
	...
	pub(super) fn sync_scroll_groups(&mut self) {
		let (mgr, vs_map) = self.buffers.splits().expect("window must have a populated split layout");
		...
		if let Some(active_buf_id) = active_buffer_id {
			...
			let active_viewport_height =
				vs_map.get(&active_split).map(|vs| vs.viewport.visible_line_count()).unwrap_or(0);
			...
			if let Some(top_byte) = active_top_byte {
				...
				if !other_splits.is_empty() {
					...
					let (_, vs_map_mut) =
						self.buffers.splits_mut().expect("window must have a populated split layout");
	...
	pub(super) fn pre_sync_ensure_visible(&mut self, active_split: LeafId) {
		...
		if let Some((left_split, right_split)) = group_info {
			...
			let other_split: SplitId = if active_sid == left_split { right_split } else { left_split };
