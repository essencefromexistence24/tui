...
//! Dx, jumping to any `:line:col` the text encoded. Ctrl+hover underlines
//! such a path to signal it's clickable.
//!
//! Relative paths are resolved against, in order: the terminal's working
//! directory as reported via OSC 7 (tracks `cd`), then Dx's working
...
impl Editor {
	...
	pub(crate) fn try_open_terminal_link(
		...
		Some(self.handle_open_file_at_location(resolved, link.line, link.column).map(|()| true))
	}
	...
	fn compute_terminal_link_hover(
		...
		Some(crate::app::window::TerminalLinkHover { buffer_id, row: term_row, cols: link.range })
	}
	...
	/// joined against the terminal's OSC 7 cwd, then against Dx's working
	...
	fn resolve_terminal_path(&self, raw: &str, term_cwd: Option<&Path>) -> Option<PathBuf> {
		...
		candidates.into_iter().find(|p| fs.is_file(p).unwrap_or(false))
	}
}
