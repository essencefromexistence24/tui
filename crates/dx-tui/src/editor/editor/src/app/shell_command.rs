...
impl Editor {
	...
	pub fn execute_shell_command(&mut self, command: &str) -> Result<String, String> {
		...
		let output =
			child.wait_with_output().map_err(|e| format!("Failed to wait for command: {}", e))?;
	...
	fn replace_with_shell_output(
		...
		if has_selection {
			...
			if let Some((start, end, deleted_text)) = selection_info {
				...
				let delete_event = Event::Delete { range: start..end, deleted_text, cursor_id };
				let insert_event = Event::Insert { position: start, text: output.to_string(), cursor_id };
			...
			// Apply as a batch for atomic undo
			let batch = Event::Batch { events, description: "Shell command replace buffer".to_string() };
			self.active_event_log_mut().append(batch.clone());
			self.apply_event_to_active_buffer(&batch);
		}
	...
	pub(crate) fn run_shell_command_blocking(&mut self, command: &str) -> anyhow::Result<()> {
		...
		let status = child.wait().map_err(|e| anyhow::anyhow!("Failed to wait for command: {}", e))?;
