...
impl Window {
	...
	pub(crate) fn apply_remote_terminal_env(&self, mut wrapper: TerminalWrapper) -> TerminalWrapper {
		use crate::services::remote::{SSH_EXEC_LOGIN_SHELL, ssh_remote_env_launcher};
	...
	pub fn spawn_terminal_session(
		...
		let name_stem = if persistent {
			format!("dx-terminal-{}", predicted_terminal_id.0)
		} else {
			let nanos = std::time::SystemTime::now()
				.duration_since(std::time::UNIX_EPOCH)
				.map(|d| d.as_nanos())
				.unwrap_or(0);
			format!("dx-terminal-eph-{}-{}", predicted_terminal_id.0, nanos)
		};
		let log_path = terminal_root.join(format!("{}.log", name_stem));
		let backing_path = terminal_root.join(format!("{}.txt", name_stem));
		self.terminal_backing_files.insert(predicted_terminal_id, backing_path.clone());
		...
		match self.terminal_manager.spawn(
			...
			Ok(terminal_id) => {
				...
				if terminal_id != predicted_terminal_id {
					...
					let backing_path = terminal_root.join(format!("dx-terminal-{}.txt", terminal_id.0));
					self.terminal_backing_files.insert(terminal_id, backing_path);
				}
				if !persistent {
			...
			Err(e) => {
				self.set_status_message(t!("terminal.failed_to_open", error = e.to_string()).to_string());
	...
	pub fn create_terminal_buffer_attached(
		...
		let mut state =
			EditorState::new_with_path(large_file_threshold, terminal_backing_fs(), backing_file.clone());
		...
		self.terminal_buffers.insert(buffer_id, TerminalBuffer::new_live(terminal_id));
	...
	pub fn create_plugin_terminal(
		...
		let PluginTerminalSpec { cwd, direction, ratio, focus, persistent, command, title } = spec;
		...
		let (buffer_id, created_split_id) = if let Some(split_dir) = direction {
			...
			match active_split {
				Some(parent) => {
					...
					let highlight_current_line = self.resources.config.editor.highlight_current_line;
					...
					match split_result {
						...
						Err(e) => {
							...
							if let Some(view_state) =
								self.split_view_states_mut().and_then(|m| m.get_mut(&parent))
							{
								view_state.add_buffer(buffer_id);
								view_state.viewport.line_wrap_enabled = false;
							}
	...
	pub fn sync_terminal_titles(&mut self) {
		...
		let interval_due =
			self.terminal_fg_poll_at.is_none_or(|last| now.duration_since(last) >= FG_POLL_INTERVAL);
		if interval_due {
		...
		for (buffer_id, tb) in self.terminal_buffers.iter() {
			...
			let osc = self.terminal_manager.get(tb.terminal_id).and_then(|handle| {
	...
	pub fn last_focused_terminal(&self) -> Option<TerminalId> {
		...
		self.terminal_buffers.values().map(|tb| tb.terminal_id).max_by_key(|t| t.0)
	}
	...
	pub fn respawn_terminals_through_authority(&mut self) -> usize {
		...
		let bindings: Vec<(BufferId, TerminalId)> =
		...
		for (buffer_id, old_id) in bindings {
			...
			let (cols, rows) = handle.map(|h| h.size()).unwrap_or_else(|| self.get_terminal_dimensions());
			...
			let resume_argv = self
				...
				.cloned();
			let launch_argv =
				self.terminal_commands.get(&old_id).filter(|argv| !argv.is_empty()).cloned();
			...
			if let Some(pid) = self.terminal_manager.get(new_id).and_then(|h| h.pid()) {
				self.process_groups.register(pid, format!("terminal #{}", new_id.0));
			}
...
impl Editor {
	...
	pub(crate) fn spawn_terminal_session(&mut self) -> Option<TerminalId> {
		...
		self.active_window_mut().spawn_terminal_session(None, true, None)
	}
	...
	pub fn open_terminal(&mut self) {
		let Some((terminal_id, buffer_id)) = self.active_window_mut().open_terminal_in_window() else {
			return;
		};
	...
	pub(crate) fn create_terminal_buffer_detached(&mut self, terminal_id: TerminalId) -> BufferId {
		self.active_window_mut().create_terminal_buffer_detached(terminal_id)
	}
	...
	pub fn close_terminal(&mut self) {
		...
		if let Some(terminal_id) = self.active_window().get_terminal_id(buffer_id) {
			...
			self.active_window_mut().ephemeral_terminals.remove(&terminal_id);

			// Clean up backing/rendering file
			let backing_file = self.active_window_mut().terminal_backing_files.remove(&terminal_id);
			...
			if let Some(log_file) = self.active_window_mut().terminal_log_files.remove(&terminal_id) {
	...
	fn focus_terminal_buffer(&mut self, terminal_id: TerminalId) {
		...
		let target_split = self.active_window().buffers.splits().and_then(|(mgr, vs)| {
			mgr.splits_for_buffer(buffer_id).into_iter().next().or_else(|| {
				vs.iter()
	...
	fn selection_or_cursor_line_text(&mut self) -> String {
		if self.active_cursors().iter().any(|(_, cursor)| cursor.has_block_selection()) {
			return self.copy_block_selection_text();
		}

		let ranges: Vec<_> =
			self.active_cursors().iter().filter_map(|(_, cursor)| cursor.selection_range()).collect();
		if !ranges.is_empty() {
		...
		let positions: Vec<_> =
			self.active_cursors().iter().map(|(_, cursor)| cursor.position).collect();
		let state = self.active_state_mut();
		let mut text = String::new();
	...
	pub fn enter_terminal_mode(&mut self) {
		if self.active_window().is_terminal_buffer(self.active_buffer()) {
			...
			self.active_window_mut().set_split_terminal_scrollback(__leaf, __active, false);
			self.active_window_mut().key_context = crate::input::keybindings::KeyContext::Terminal;
			...
			if let Some(terminal_id) = self.active_window().get_terminal_id(self.active_buffer()) {
				...
				if let Some(backing_path) = self.active_window().terminal_backing_files.get(&terminal_id) {
					if let Some(handle) = self.active_window().terminal_manager.get(terminal_id) {
						if let Ok(state) = handle.state.lock() {
							...
							if let Err(e) = terminal_backing_fs().set_file_length(backing_path, truncate_pos) {
	...
	pub fn enter_terminal_scrollback(&mut self) {
		if self.active_window().is_terminal_buffer(self.active_buffer()) {
			...
			self.active_window_mut().set_split_terminal_scrollback(leaf, buf, true);
...
impl Window {
	...
	pub fn send_terminal_key(
		...
		let app_cursor = self.get_active_terminal_state().map(|s| s.is_app_cursor()).unwrap_or(false);
		if let Some(bytes) =
	...
	pub fn send_terminal_mouse(
		...
		let use_sgr = self.get_active_terminal_state().map(|s| s.uses_sgr_mouse()).unwrap_or(true);
		...
		let wants_mouse =
			self.get_active_terminal_state().map(|s| s.wants_mouse_events()).unwrap_or(false);
		let uses_alt_scroll = !wants_mouse
			&& self.get_active_terminal_state().map(|s| s.uses_alternate_scroll()).unwrap_or(false);
	...
	pub(crate) fn editor_content_area(&self) -> ratatui::layout::Rect {
		...
		let file_explorer_width =
			if self.file_explorer_visible { self.file_explorer_width.to_cols(chrome_width) } else { 0 };
		let editor_x = match self.file_explorer_side {
			crate::config::FileExplorerSide::Left => self.dock_cols.saturating_add(file_explorer_width),
	...
	pub fn sync_terminal_to_buffer(&mut self, buffer_id: BufferId) {
		...
		if let Ok(new_state) = EditorState::from_file_with_languages_force_text(
			...
			let anchor_byte =
				history_end_byte.map(|h| (h as usize).min(total_bytes)).unwrap_or(total_bytes);
...
impl Editor {
	...
	pub fn terminal_manager(&self) -> &crate::services::terminal::TerminalManager {
		&self.windows.get(&self.active_window).expect("active window must exist").terminal_manager
	}
	...
	pub fn terminal_backing_files(
		...
		&self.windows.get(&self.active_window).expect("active window must exist").terminal_backing_files
	}
...
pub mod render {
	...
	pub fn render_terminal_content(
		...
		for (row_idx, row) in content.iter().enumerate() {
			...
			for (col_idx, cell) in row.iter().enumerate() {
				...
				if cursor_visible && row_idx as u16 == cursor_pos.1 && col_idx as u16 == cursor_pos.0 {
					style = style.add_modifier(Modifier::REVERSED);
				}
	...
	mod tests {
		...
		fn link_highlight_underlines_only_its_span() {
			...
			let mut buf = Buffer::empty(area);
			let row: Vec<TerminalCell> =
				"abcdef".chars().map(|c| TerminalCell { c, ..Default::default() }).collect();
			let content = vec![row];
...
mod title_tests {
	...
	fn uses_single_source_when_only_one_present() {
		assert_eq!(combine_terminal_title(Some("bash"), None).as_deref(), Some("bash"));
		assert_eq!(
