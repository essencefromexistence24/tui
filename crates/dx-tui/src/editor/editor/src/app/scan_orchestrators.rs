...
impl Editor {
	...
	fn process_line_scan_batch(&mut self, buffer_id: BufferId) -> std::io::Result<()> {
		...
		let has_state =
			self.windows.get(&active_id).expect("active window present").buffers.contains_key(&buffer_id);
		if !has_state {
			return Ok(());
		}
		...
		'outer: while results.len() + io_work.len() < concurrency {
			...
			for chunk in batch {
				...
				let leaf = self.windows.get(&active_id).expect("active window present").line_scan.leaves()
					[chunk.leaf_index];
				let leaf = &leaf;
		...
		for (leaf_idx, count) in results {
			self.active_window_mut().line_scan.append_update(leaf_idx, count);
		}
	...
	fn finish_line_scan_ok(&mut self) {
		...
		{
			let _span =
				tracing::info_span!("rebuild_with_pristine_saved_root", updates = finished.updates.len())
					.entered();
			state.buffer.rebuild_with_pristine_saved_root(&finished.updates);
		}
	...
	pub fn process_search_scan(&mut self) -> bool {
		...
		if self.active_window_mut().search_scan.is_done() {
			...
			self.set_status_message(format!("Searching... {}% ({} matches so far)", pct, match_count));
		}
		true
	}
	...
	fn process_search_scan_batch(
		...
		for _ in 0..concurrency {
			...
			self.active_window_mut().search_scan.restore_chunked(chunked);
