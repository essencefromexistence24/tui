...
impl Editor {
	...
	pub fn show_lsp_status_popup(&mut self) {
		...
		if self.active_state().popups.top().is_some_and(is_lsp_status_popup) {
			self.hide_popup();
			return;
		}
		...
		let missing_servers: Vec<String> = self
			...
			.unwrap_or_default();
		let user_dismissed = self.active_window().is_lsp_language_user_dismissed(&language);
	...
	pub fn refresh_lsp_status_popup_if_open(&mut self) {
		...
		let was_focused = self.active_state().popups.top().map(|p| p.focused).unwrap_or(true);
		self.hide_popup();
	...
	fn collect_lsp_status_servers(&self, language: &str) -> LspStatusServers {
		...
		let configured_servers: Vec<String> = self
			...
			.get(language)
			.map(|cfg| {
				cfg.as_slice().iter().filter(|c| !c.command.is_empty()).map(|c| c.display_name()).collect()
			})
			.unwrap_or_default();
		...
		let any_enabled =
			self.config.lsp.get(language).is_some_and(|cfg| cfg.as_slice().iter().any(|c| c.enabled));
		...
		LspStatusServers { names, running, missing, auto_start, user_dismissed, any_enabled }
	}
	...
	fn push_lsp_server_rows(
		...
		let missing_label =
			if authority_is_container { "not installed in container" } else { "binary not in PATH" };

		for name in &servers.names {
			let status = servers.running.get(name).copied();
			let is_active = status.map(|s| !matches!(s, LspServerStatus::Shutdown)).unwrap_or(false);
			...
			items.push(crate::view::popup::PopupListItem::new(format!("{icon} {name} ({label})")));
			...
			if let Some(info) =
				self.active_window().lsp_progress.values().find(|info| info.language == language)
			{
				let mut line = format!("    ⏳ {}", truncate_to_cells(&info.title, LSP_PROGRESS_FIELD_MAX));
				if let Some(ref msg) = info.message {
					line.push_str(&format!(" · {}", truncate_to_cells(msg, LSP_PROGRESS_FIELD_MAX)));
				}
			...
			if is_active {
				...
				if is_manual {
					items.push(
						crate::view::popup::PopupListItem::new(format!("    Start {name} (always)"))
							.with_data(format!("autostart:{language}/{name}")),
					);
				}

				let start_label =
					if is_manual { format!("    Start {name} once") } else { format!("    Start {name}") };
				// All dormant servers for a language share the same `start:`
				// key; only emit the row once.
				let start_key = format!("start:{language}");
				if !items.iter().any(|i| i.data.as_deref() == Some(start_key.as_str())) {
					items.push(crate::view::popup::PopupListItem::new(start_label).with_data(start_key));
				}
			}
		}
	}
	...
	fn push_lsp_footer_rows(
		...
		if !contributed.is_empty() {
			items.push(crate::view::popup::PopupListItem::new("  ─ Plugin actions ─".to_string()));
	...
	fn present_lsp_status_popup(
		...
		let max_static_item_width = items
			...
			.unwrap_or(20);
		let popup_width =
			(max_static_item_width.max(LSP_PROGRESS_LINE_MAX) as u16 + 4).clamp(30, LSP_POPUP_WIDTH_MAX);

		// Pre-select the first actionable item (skip header items with no data
		// and disabled items like a non-existent View Log).
		let first_actionable = items.iter().position(|i| i.data.is_some() && !i.disabled).unwrap_or(0);
		...
		let position = self
			...
			.map(|(status_row, col_start, _)| crate::view::popup::PopupPosition::AboveStatusBarAt {
				x: col_start,
				status_row,
			})
			.unwrap_or(crate::view::popup::PopupPosition::BottomRight);

		let focus_hint = if !focused { self.popup_focus_key_hint() } else { None };
		let popup = Popup {
			kind: PopupKind::List,
			title: Some(format!("LSP Servers ({language})")),
			description: None,
			transient: false,
			content: PopupContent::List { items, selected: first_actionable },
			position,
			width: popup_width,
			max_height: 15,
			bordered: true,
	...
	pub fn show_remote_indicator_popup(&mut self) {
		...
		let is_disconnected = connection.as_deref().is_some_and(|c| c.contains("(Disconnected)"));
		let is_container = connection.as_deref().is_some_and(|c| c.starts_with("Container:"));
		...
		if let Some(over) = self.remote_indicator_override.clone() {
			match over {
				RemoteIndicatorOverride::Connecting { label } => {
					let suffix =
						label.filter(|s| !s.is_empty()).map(|s| format!(" — {}", s)).unwrap_or_default();
				...
				RemoteIndicatorOverride::FailedAttach { error } => {
					let suffix =
						error.filter(|s| !s.is_empty()).map(|s| format!(" — {}", s)).unwrap_or_default();
		...
		let core_failed_attach =
			!override_handled && self.active_window().remote_reconnect_error.as_deref().is_some();
		if core_failed_attach {
			let err = self.active_window().remote_reconnect_error.clone().unwrap_or_default();
			...
			items
				.push(PopupListItem::new("    Retry".to_string()).with_data("retry_reconnect".to_string()));
		}
		...
		if !override_handled && !core_failed_attach {
			match (connection.as_deref(), is_disconnected) {
				...
				(Some(label), false) => {
					...
					if is_container {
						items.push(
							PopupListItem::new("    Reopen Locally".to_string()).with_data("detach".to_string()),
						);
						items.push(
				...
				(Some(_), true) => {
					...
					if is_remote_agent {
						items.push(
							PopupListItem::new("    Reconnect".to_string()).with_data("reconnect".to_string()),
						);
					}
				}
				// Local authority.
	...
	pub fn show_read_only_popup(&mut self) {
		...
		let position = self
			...
			.map(|(status_row, col_start, _)| PopupPosition::AboveStatusBarAt {
				x: col_start,
				status_row,
			})
	...
	pub fn handle_read_only_menu_action(&mut self, action_key: &str) {
		match action_key {
			"toggle_read_only" => {
				if let Err(e) = self.handle_action(crate::input::keybindings::Action::ToggleReadOnly) {
					tracing::warn!("read-only menu: toggling read-only failed: {}", e);
				}
			}
			"cancel" => {}
			other => {
				tracing::warn!("handle_read_only_menu_action: unknown action key '{}'", other);
			}
		}
	}
	...
	pub fn handle_remote_indicator_action(&mut self, action_key: &str) {
		...
		if let Some(plugin_action) = action_key.strip_prefix("plugin:") {
			...
			if let Err(e) = self
				.handle_action(crate::input::keybindings::Action::PluginAction(plugin_action.to_string()))
			{
				tracing::warn!("remote indicator popup: dispatching '{}' failed: {}", plugin_action, e);
			}
			return;
		}
		tracing::warn!("handle_remote_indicator_action: unknown action key '{}'", action_key);
	}
	...
	pub fn maybe_prompt_workspace_trust(&mut self) {
		...
		let markers = crate::services::workspace_trust::executable_content_markers(self.working_dir());
	...
	pub fn show_workspace_trust_popup(&mut self, cancellable: bool) {
		...
		if self.global_popups.top().is_some_and(|p| matches!(p.resolver, PopupResolver::WorkspaceTrust))
		{
			return;
		}
		...
		let popup_width = (items
			.iter()
			.map(|i| {
				let detail_w = i.detail.as_deref().map(unicode_width::UnicodeWidthStr::width).unwrap_or(0);
	...
	pub(crate) fn handle_workspace_trust_key(
		...
		// cancels it instead.
		if !cancellable {
			let resolved = self.keybindings.read().ok().map(|kb| kb.resolve(event, KeyContext::Normal));
	...
	pub fn show_file_message_popup(&mut self, message: &str) {
		...
		let mut popup = Popup::markdown(&md, &self.theme.read().unwrap(), Some(&self.grammar_registry));
	...
	pub fn show_settings_save_error_popup(
		...
		for (i, para) in [detail.as_str(), unchanged.as_str(), open_hint.as_str()].iter().enumerate() {
			if i > 0 {
				lines.push(String::new());
			}
		...
		let popup = {
			...
			let mut p = Popup::text(lines, &theme).with_title(title).with_focused(true);
			p.transient = false;
