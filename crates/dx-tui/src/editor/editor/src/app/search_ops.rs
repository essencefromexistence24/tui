...
impl Editor {
	...
	pub(super) fn update_search_highlights(&mut self, query: &str) {
		...
		self.active_window_mut().update_search_highlights(query, search_fg, search_bg);
	}
	...
	pub(super) fn finalize_search(
		...
		let current_match_index = matches.iter().position(|&pos| pos >= cursor_pos).unwrap_or(0);
		...
		if is_large {
			...
			for (&pos, &len) in match_ranges.iter().map(|(p, _)| p).zip(match_lengths.iter()) {
		...
		let msg = if self.active_window_mut().search_state.as_ref().unwrap().search_range.is_some() {
			format!(
	...
	fn start_search_scan(&mut self, query: &str, regex: regex::Regex) {
			...
			.get_mut(&buffer_id)
		{
			...
			let scan =
				state.buffer.search_scan_init(bytes_regex, super::SearchState::MAX_MATCHES, query.len());
	...
	fn find_match_in_direction(&mut self, direction: SearchDirection) {
		...
		if let Some(ref mut search_state) = self.active_window_mut().search_state {
			...
			let match_positions: &[usize] =
				if use_overlays { &overlay_positions } else { &search_state.matches };
			...
			self.set_status_message(
				t!("search.match_of", current = target_index + 1, total = matches_len).to_string(),
			);
			...
			let find_key = self.get_keybinding_for_action("find").unwrap_or_else(|| "Ctrl+F".to_string());
		...
		match search_text {
			Some(text) if !text.is_empty() => {
				...
				if let Some(ref search_state) = self.active_window().search_state {
					...
					let landed_at_start = selection_start.map(|start| cursor_after == start).unwrap_or(false);
	...
	pub(super) fn perform_replace(&mut self, search: &str, replacement: &str) {
		...
		let matches: Vec<(usize, usize, String)> = if let Some(ref regex) = compiled_regex {
			...
			while current_pos < buffer_len {
				if let Some(offset) =
					state.buffer.find_next_in_range(search, current_pos, Some(current_pos..buffer_len))
				{
		...
		for (match_pos, match_len, expanded_replacement) in &matches {
			...
			let deleted_text = self.active_state_mut().get_text_range(*match_pos, match_pos + match_len);
		...
		// Set status message
		self.set_status_message(
			t!("search.replaced", count = count, search = search, replace = replacement).to_string(),
		);
	}
	...
	pub(super) fn start_interactive_replace(&mut self, search: &str, replacement: &str) {
		...
		let (first_match_pos, first_match_len) = if let Some(ref regex) = compiled_regex {
			...
			let found = state
				...
				.or_else(|| {
					if start_pos > 0 {
						state.buffer.find_next_regex_in_range(regex, 0, Some(0..start_pos))
					} else {
						None
					}
				});
			...
			'a' | 'A' | '!' => {
				...
				if total_count > 0 {
					...
					for &(match_pos, match_len) in &all_matches {
						let deleted_text =
							self.active_state_mut().get_text_range(match_pos, match_pos + match_len);
						...
						events.push(Event::Insert { position: match_pos, text: replacement_text, cursor_id });
					}
		...
		if let Some(ref regex) = ir_state.regex {
			...
			if ir_state.has_wrapped {
				...
				if let Some(match_pos) =
					state.buffer.find_next_regex_in_range(&regex, start_pos, search_range)
				{
				...
				let state = self.active_state();
				if let Some(match_pos) = state.buffer.find_next_regex_in_range(&regex, 0, wrap_range) {
					let match_len = self.get_regex_match_len(&regex, match_pos).unwrap_or(0);
					return Some((match_pos, match_len, true));
				}
			...
			if ir_state.has_wrapped {
				let search_range = Some(start_pos..ir_state.start_pos);
				if let Some(match_pos) =
					state.buffer.find_next_in_range(&ir_state.search, start_pos, search_range)
				{
	...
	pub(super) fn replace_current_match(
		...
		let events = vec![
			...
			Event::Delete { range: range.clone(), deleted_text, cursor_id },
			Event::Insert { position: match_pos, text: replacement_text, cursor_id },
		];
