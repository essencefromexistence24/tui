...
impl Editor {
	...
	pub(super) fn ensure_help_panel_mode_registered(&mut self) {
		let mode_ctx = KeyContext::Mode(HELP_PANEL_MODE.to_string());
		{
			...
			kb.load_plugin_default(mode_ctx, KeyCode::Char('q'), KeyModifiers::NONE, Action::CloseTab);
		}
		self.mode_registry.register(
			BufferMode::new(HELP_PANEL_MODE).with_read_only(true).with_inherit_normal_bindings(true),
		);
	}
}
...
impl Window {
	...
	pub fn open_keyboard_shortcuts(&mut self) {
		...
		let bindings = self.resources.keybindings.read().unwrap().get_all_bindings();
