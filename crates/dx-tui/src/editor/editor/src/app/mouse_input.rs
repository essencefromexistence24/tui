...
use crate::view::popup_mouse::{PopupHitTester, popup_areas_to_layout_info};
...
impl Editor {
	...
	pub fn handle_mouse(&mut self, mouse_event: crossterm::event::MouseEvent) -> AnyhowResult<bool> {
		use crossterm::event::{MouseButton, MouseEventKind};
		...
		tracing::trace!("handle_mouse: kind={:?}, col={}, row={}", mouse_event.kind, col, row);
		...
		if !chrome_drag_active {
			if let Some(result) =
				self.active_window_mut().try_forward_mouse_to_terminal(col, row, mouse_event)
			{
				return result;
			}
		}
		...
		// Dx (jumping to any :line:col it encodes). Handled before normal
		...
		match mouse_event.kind {
			MouseEventKind::Down(MouseButton::Left) => {
				if is_double_click || is_triple_click {
					if let Some((buffer_id, byte_pos)) = self.fold_toggle_line_at_screen_position(col, row) {
						self.active_window_mut().toggle_fold_at_byte(buffer_id, byte_pos);
						needs_render = true;
			...
			MouseEventKind::Up(MouseButton::Left) => {
				...
				let was_dragging_separator =
					self.active_window_mut().mouse_state.dragging_separator.is_some();
				...
				if let Some(drag_state) = self.active_window_mut().mouse_state.dragging_tab.take() {
					if drag_state.is_dragging() {
						if let Some(drop_zone) = drag_state.drop_zone {
							self.execute_tab_drop(drag_state.buffer_id, drag_state.source_split_id, drop_zone);
						}
					}
				}
			...
			MouseEventKind::Moved => {
				// Dispatch MouseMove hook to plugins (fire-and-forget, no blocking check)
				{
					...
					self
						...
						.read()
						.unwrap()
						.run_hook("mouse_move", HookArgs::MouseMove { column: col, row, content_x, content_y });
				}
				...
				let term_link_changed = self.update_terminal_link_hover(col, row, mouse_event.modifiers);
				...
				if let Some((popup_rect, Some(button_row_offset))) = self.theme_info_popup_rect() {
					...
					let new_highlighted =
						row == button_row && col >= popup_rect.x && col < popup_rect.x + popup_rect.width;
				...
				let now_over = self
					...
					.as_ref()
					.map(|d| {
						d.scrollbar_hover_zones
							.iter()
							.any(|z| col >= z.x && col < z.x + z.width && row >= z.y && row < z.y + z.height)
					})
					.unwrap_or(false);
			...
			MouseEventKind::ScrollLeft => {
				...
				self.active_window_mut().handle_horizontal_scroll(col, row, -3)?;
				needs_render = true;
			}
			...
			MouseEventKind::Down(MouseButton::Right) => {
				...
				if self.overlay_prompt_active() {
					needs_render = true;
				} else if mouse_event.modifiers.contains(crossterm::event::KeyModifiers::CONTROL) {
	...
	fn handle_vertical_scroll(
		...
		if modifiers.contains(crossterm::event::KeyModifiers::SHIFT) {
			self.active_window_mut().handle_horizontal_scroll(col, row, delta)?;
		...
		} else if self.handle_project_explorer_scroll(col, row, delta) {
			// project explorer consumed the scroll
			...
			.unwrap_or(false)
		{
			...
			self.active_window_mut().handle_mouse_scroll(col, row, delta)?;
		}
		Ok(())
	}

	fn handle_project_explorer_scroll(&mut self, col: u16, row: u16, delta: i32) -> bool {
		let Some(area) = self.active_layout().file_explorer_area else {
			return false;
		};
		...
		let max_offset = explorer.get_display_nodes().len().saturating_sub(viewport);
		let offset = if delta < 0 {
			explorer.get_scroll_offset().saturating_sub((-delta) as usize)
		} else {
			explorer.get_scroll_offset().saturating_add(delta as usize).min(max_offset)
		};
		explorer.set_scroll_offset(offset);
		true
	}
	...
	fn handle_overlay_prompt_scroll(&mut self, col: u16, row: u16, delta: i32) -> bool {
		...
		let results_visible =
			self.active_chrome().prompt_results_area.map(|r| r.height as usize).unwrap_or(0);
		if let Some(preview) = preview_area {
			if in_rect(col, row, preview) {
				self.active_window_mut().scroll_overlay_preview_by_lines(delta);
				return true;
			}
		}
	...
	pub(super) fn update_hover_target(&mut self, col: u16, row: u16) -> bool {
		...
		if let Some(active_menu_idx) = self.menu_state.active_menu {
			let all_menus: Vec<crate::config::Menu> =
				self.menus.menus.iter().chain(self.menu_state.plugin_menus.iter()).cloned().collect();
			...
			if let Some(HoverTarget::SubmenuItem(depth, item_idx)) = new_target {
				...
				if let Some(items) = self.menu_state.get_current_items(&all_menus, active_menu_idx) {
					...
					if let Some(crate::config::MenuItem::Submenu { items: sub_items, .. }) =
						items.get(item_idx)
					{
		...
		if old_target != new_target
			&& matches!(old_target, Some(HoverTarget::FileExplorerStatusIndicator(_)))
		{
	...
	fn update_lsp_hover_state(&mut self, col: u16, row: u16) {
		...
		if self.active_window_mut().theme_info_popup.is_some()
			...
			|| self.active_window_mut().file_explorer_context_menu.is_some()
			|| self.is_lsp_status_popup_open()
		{
			if self.active_window_mut().mouse_state.lsp_hover_state.is_some() {
		...
		let cached_mappings = self.active_layout().view_line_mappings.get(&split_id).cloned();
		let gutter_width =
			self.buffers().get(&buffer_id).map(|s| s.margins.left_total_width() as u16).unwrap_or(0);
		let fallback = self.buffers().get(&buffer_id).map(|s| s.buffer.len()).unwrap_or(0);
	...
	fn compute_hover_target(&self, col: u16, row: u16) -> Option<HoverTarget> {
		self
			.hover_target_in_floating_overlays(col, row)
	...
	fn hover_target_in_chrome(&self, col: u16, row: u16) -> Option<HoverTarget> {
				...
				if let Some(explorer) = self.file_explorer().as_ref() {
					...
					if item_index < display_nodes.len() {
						...
						if let Some(node) = explorer.tree().get_node(node_id) {
							...
							let neutral_fg = if node.entry.metadata.as_ref().map(|m| m.is_hidden).unwrap_or(false)
							{
							...
							let slot_context = crate::view::file_tree::ExplorerSlotContext {
								...
								is_hidden: node.entry.metadata.as_ref().map(|m| m.is_hidden).unwrap_or(false),
							...
							if let Some((slot_start, slot_end)) =
								...
								if col >= slot_start && col < slot_end {
									return Some(HoverTarget::FileExplorerStatusIndicator(node.entry.path.clone()));
								}
							}
						}
					}
		...
		for (split_id, direction, sep_x, sep_y, sep_length) in &self.active_layout().separator_areas {
			let is_on_separator = match direction {
				SplitDirection::Horizontal => row == *sep_y && col >= *sep_x && col < sep_x + sep_length,
	...
	fn handle_editor_double_click(
		...
		self.active_window_mut().apply_event_to_buffer(buffer_id, leaf_id, &event);
	...
	pub(super) fn overlay_prompt_active(&self) -> bool {
		self.active_window().prompt.as_ref().is_some_and(|p| p.overlay)
	}
	...
	fn handle_click_context_menus(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		if self.active_window_mut().file_explorer_context_menu.is_some() {
	...
	fn suggestion_at(&self, col: u16, row: u16) -> Option<usize> {
		...
		let relative_row = (row - inner_rect.y) as usize;
		let item_idx = start_idx + relative_row;
		if item_idx < total_count { Some(item_idx) } else { None }
	}
	...
	fn handle_click_prompt_scrollbar(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		...
		let prompt = self.windows.get_mut(&active_window_id).and_then(|w| w.prompt.as_mut())?;
		...
		self.active_window_mut().mouse_state.dragging_prompt_scrollbar = true;
		Some(Ok(()))
	}
	...
	fn handle_click_popup_scrollbar(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		...
		self.active_window_mut().mouse_state.dragging_popup_scrollbar = Some(popup_idx);
		self.active_window_mut().mouse_state.drag_start_row = Some(row);
		let current_scroll =
			self.active_state().popups.get(popup_idx).map(|p| p.scroll_offset).unwrap_or(0);
	...
	fn handle_click_global_popups(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		for (popup_idx, popup_rect, inner_rect, scroll_offset, num_items) in
			self.active_chrome().global_popup_areas.clone().into_iter().rev()
		{
	...
	fn handle_click_buffer_popups(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		...
		let close_hit = self.active_chrome().popup_areas.iter().rev().find_map(
			|(_idx, popup_rect, _inner, _scroll, _n, _sb, _tl)| {
				...
				let cb_x = popup_rect.x + popup_rect.width - 4;
				if row == popup_rect.y && col >= cb_x && col < cb_x + 3 { Some(()) } else { None }
			},
		);
			...
			popup_areas.iter().rev()
		{
			...
			let link_url = {
				...
				state.popups.top().and_then(|p| p.link_at_position(relative_col, relative_row))
			};
	...
	fn handle_click_menu_bar(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		if self.active_window_mut().menu_bar_visible {
			...
			let hit = self.active_chrome().menu_layout.as_ref().and_then(|ml| ml.menu_at(col, row));
	...
	fn handle_click_file_explorer_area(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
			...
			&& row < explorer_area.y + explorer_area.height.saturating_sub(1)
		{
			if let Some(explorer) = self.file_explorer().as_ref() {
				let display_nodes = explorer.get_display_nodes();
				let scroll_offset = explorer.get_scroll_offset();
				let viewport_height = explorer_area.height.saturating_sub(2) as usize;
				let track_height = viewport_height;
				let relative_row = row.saturating_sub(explorer_area.y + 1) as usize;
				...
				if state.is_thumb_row(track_height, relative_row) {
					// Click on thumb → start drag
					self.active_window_mut().mouse_state.dragging_file_explorer_scrollbar = true;
					self.active_window_mut().mouse_state.drag_start_file_explorer_scroll =
						Some(scroll_offset);
					self.active_window_mut().mouse_state.drag_start_row = Some(relative_row as u16);
				} else {
					// Click on track → jump
					let new_offset = state.click_to_offset(track_height, relative_row);
					if let Some(explorer) = self.file_explorer_mut() {
						explorer.set_scroll_offset(new_offset);
					}
					self.active_window_mut().mouse_state.dragging_file_explorer_scrollbar = true;
					self.active_window_mut().mouse_state.drag_start_file_explorer_scroll = Some(new_offset);
					self.active_window_mut().mouse_state.drag_start_row = Some(relative_row as u16);
				}
			}
			return Some(Ok(()));
		...
		if col == border_x && row >= explorer_area.y && row < explorer_area.y + explorer_area.height {
			...
			self.active_window_mut().mouse_state.drag_start_explorer_width =
	...
	fn handle_click_scrollbar(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		...
		if is_on_thumb {
			...
			if self.active_window().is_composite_buffer(buffer_id) {
				if let Some(vs) = self.active_window().composite_view_states.get(&(split_id, buffer_id)) {
					self.active_window_mut().mouse_state.drag_start_composite_scroll_row =
						Some(vs.scroll_row);
				}
			} else {
				let snap = self
	...
	fn handle_click_horizontal_scrollbar(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		let (split_id, buffer_id, hscrollbar_rect, max_content_width, is_on_thumb) =
			self.active_layout().horizontal_scrollbar_areas.iter().find_map(
				|(split_id, buffer_id, hscrollbar_rect, max_content_width, thumb_start, thumb_end)| {
						...
						&& row < hscrollbar_rect.y + hscrollbar_rect.height
					{
						...
						Some((*split_id, *buffer_id, *hscrollbar_rect, *max_content_width, on_thumb))
					} else {
						None
					}
				},
		...
		self.focus_split(split_id, buffer_id);
		self.active_window_mut().mouse_state.dragging_horizontal_scrollbar = Some(split_id);
		if is_on_thumb {
			...
			if let Some(vs) = self
				...
				.get(&split_id)
			{
				self.active_window_mut().mouse_state.drag_start_left_column = Some(vs.viewport.left_column);
			}
		} else {
			...
			let ratio =
				if track_width > 1.0 { (relative_col / (track_width - 1.0)).clamp(0.0, 1.0) } else { 0.0 };
			if let Some(vs) = self
				.windows
	...
	fn handle_click_search_options(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		...
		match layout.checkbox_at(col, row)? {
			...
			SearchOptionsHover::WholeWord => Some(self.handle_action(Action::ToggleSearchWholeWord)),
			SearchOptionsHover::Regex => Some(self.handle_action(Action::ToggleSearchRegex)),
			SearchOptionsHover::ConfirmEach => Some(self.handle_action(Action::ToggleSearchConfirmEach)),
	...
	fn handle_click_split_separator(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		...
		for (split_id, direction, sep_x, sep_y, sep_length) in &separator_areas {
			...
			if is_on_separator {
				self.active_window_mut().mouse_state.dragging_separator = Some((*split_id, *direction));
				self.active_window_mut().mouse_state.drag_start_position = Some((col, row));
				let ratio = self
					.split_manager_mut()
	...
	fn handle_click_split_controls(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		...
		if let Some(split_id) = close_split_id {
			...
			{
				self.set_status_message(t!("error.cannot_close_split", error = e.to_string()).to_string());
			} else {
				// Drop the closed split from every terminal's scrollback set.
				self.active_window_mut().forget_split_terminal_modes(split_id);
				let new_active = self
					.windows
					.get(&self.active_window)
					.and_then(|w| w.buffers.splits())
	...
	fn handle_click_tab_bar(&mut self, col: u16, row: u16) -> Option<AnyhowResult<()>> {
		...
		let tab_hit = self.active_layout().tab_layouts.iter().find_map(|(split_id, tab_layout)| {
		...
		match hit {
			...
			TabHit::TabName(target) => {
				let direction = self
					...
					.unwrap_or(0);
				self.active_window_mut().animate_tab_switch(split_id, direction);
				match target {
					crate::view::split::TabTarget::Buffer(buffer_id) => {
						self.focus_split(split_id, buffer_id);
						self.active_window_mut().promote_buffer_from_preview(buffer_id);
						self.active_window_mut().mouse_state.dragging_tab =
							Some(super::types::TabDragState::new(buffer_id, split_id, (col, row)));
					}
					crate::view::split::TabTarget::Group(group_leaf) => {
	...
	pub(super) fn handle_mouse_drag(&mut self, col: u16, row: u16) -> AnyhowResult<()> {
		...
		if self.overlay_prompt_active() && !self.active_window().mouse_state.dragging_prompt_scrollbar {
			return Ok(());
		}
		...
		if let Some(dragging_split_id) =
			self.active_window_mut().mouse_state.dragging_horizontal_scrollbar
		{
			...
			for (split_id, _buffer_id, hscrollbar_rect, max_content_width, thumb_start, thumb_end) in
				&hscrollbar_areas
			{
				if *split_id == dragging_split_id {
					let track_width = hscrollbar_rect.width as f64;
					...
					if let (Some(drag_start_hcol), Some(drag_start_left_column)) = (
							...
							.get_mut(&dragging_split_id)
						{
							...
							if max_scroll > 0 {
								...
								let scroll_offset = (col_offset as f64 * scroll_per_pixel).round() as i64;
								let new_left = (drag_start_left_column as i64 + scroll_offset).max(0) as usize;
		...
		if let Some(popup_idx) = self.active_window_mut().mouse_state.selecting_in_popup {
			...
			if let Some((_, _, inner_rect, scroll_offset, _, _, _)) =
				self.active_chrome().popup_areas.iter().find(|(idx, _, _, _, _, _, _)| *idx == popup_idx)
			{
		...
		if self.active_window_mut().mouse_state.dragging_prompt_scrollbar {
			use crate::view::ui::scrollbar::ScrollbarState;
			...
			if let (Some(sb_rect), Some(prompt)) =
				(sb_rect, self.windows.get_mut(&active_window_id).and_then(|w| w.prompt.as_mut()))
			{
				let visible = suggestions_area_visible.unwrap_or(prompt.suggestions.len().min(10));
				let total = prompt.suggestions.len();
				let track_height = sb_rect.height as usize;
		...
		if let Some(popup_idx) = self.active_window_mut().mouse_state.dragging_popup_scrollbar {
			// Find the popup's scrollbar rect from cached layout
			if let Some((_, _, inner_rect, _, _, Some(sb_rect), total_lines)) =
				self.active_chrome().popup_areas.iter().find(|(idx, _, _, _, _, _, _)| *idx == popup_idx)
			{
				let track_height = sb_rect.height as usize;
				let visible_lines = inner_rect.height as usize;
		...
		// If dragging file explorer scrollbar, update scroll position
		if self.active_window().mouse_state.dragging_file_explorer_scrollbar {
			let explorer_area = self.active_layout().file_explorer_area.unwrap_or_default();
			let viewport_height = explorer_area.height.saturating_sub(2) as usize;
			let relative_row = row.saturating_sub(explorer_area.y + 1) as usize;
			let (drag_start_row, drag_start_offset) = {
				let ms = &self.active_window().mouse_state;
				let dsr = ms.drag_start_row.unwrap_or(relative_row as u16) as usize;
				let dso = ms.drag_start_file_explorer_scroll.unwrap_or(0);
				(dsr, dso)
			};

			if let Some(explorer) = self.file_explorer_mut() {
				let display_nodes_len = explorer.get_display_nodes().len();
				let scroll_offset = explorer.get_scroll_offset();
				...
				let new_offset =
					state.drag_to_offset(viewport_height, drag_start_row, drag_start_offset, relative_row);
				explorer.set_scroll_offset(new_offset);
			}
			return Ok(());
		}
	...
	fn handle_text_selection_drag(&mut self, col: u16, row: u16) -> AnyhowResult<()> {
		...
		let Some(anchor_position) = self.active_window_mut().mouse_state.drag_selection_anchor else {
			return Ok(());
		};
		...
		let Some((target_position, new_position, anchor_position, new_sticky_column)) =
			self.active_window().buffers.get(&buffer_id).and_then(|state| {
				...
				let (new_position, anchor_pos) = if drag_by_words {
					if target_position >= anchor_position {
						(find_word_end(&state.buffer, target_position), anchor_position)
					} else {
		...
		let (primary_cursor_id, old_position, old_anchor, old_sticky_column) = self
			...
			.map(|vs| {
				...
				(vs.cursors.primary_id(), cursor.position, cursor.anchor, cursor.sticky_column)
			})
	...
	pub(super) fn handle_file_explorer_border_drag(&mut self, col: u16) -> AnyhowResult<()> {
		let Some((start_col, _start_row)) = self.active_window_mut().mouse_state.drag_start_position
		else {
			return Ok(());
		};
		let Some(start_width) = self.active_window_mut().mouse_state.drag_start_explorer_width else {
			return Ok(());
		};
		...
		if let Some(explorer_area) = self.active_layout().file_explorer_area {
			...
			// skip title row
			{
				...
				self.active_window_mut().file_explorer_context_menu = Some(
					super::types::FileExplorerContextMenu::new(col, row + 1, is_multi, is_root_selected),
				);
				return Ok(());
			}
		}
		...
		match code {
			...
			KeyCode::Enter => {
				let selected = self
					...
					.new_tab_menu
					.as_ref()
					.map(|menu| (super::types::NewTabMenuItem::all()[menu.highlighted], menu.split_id));
				self.active_window_mut().new_tab_menu = None;
	...
	pub(super) fn handle_file_explorer_context_menu_key(
		...
		match code {
			...
			KeyCode::Enter => {
				let item = {
					let menu = self.active_window_mut().file_explorer_context_menu.as_ref()?;
	...
	fn show_file_explorer_status_tooltip(&mut self, path: std::path::PathBuf, col: u16, row: u16) {
		...
		let has_custom_trailing_override =
			self.active_window().file_explorer_slot_override_cache.has_trailing_override_for_path(&path);
			...
			if is_directory {
				if let Some(modified_files) = self.get_modified_files_in_directory(&path) {
					...
					for (i, file) in modified_files.iter().take(MAX_FILES).enumerate() {
						...
						let display_name =
							file.strip_prefix(&path).unwrap_or(file).to_string_lossy().to_string();
						lines.push(format!("  {}", display_name));
						if i == MAX_FILES - 1 && modified_files.len() > MAX_FILES {
							lines.push(format!("  ... and {} more", modified_files.len() - MAX_FILES));
							break;
						}
					}
				}
	...
	fn get_modified_files_in_directory(
		...
		let modified_files =
			self.active_window().file_explorer_decoration_cache.direct_paths_under(dir_path);
	...
	fn handle_floating_widget_panel_wheel(
		...
		let is_dock =
			matches!(self.panel(slot).map(|f| f.placement), Some(super::PanelPlacement::LeftDock { .. }));
		scrolled || is_dock
	}
	...
	fn try_widget_scrollbar_press(&mut self, slot: super::PanelSlot, col: u16, row: u16) -> bool {
		...
		for t in &tracks {
			...
			let pressed =
				self.panel_mut(slot).and_then(|fwp| fwp.scrollbar_mouse.press(state, t.rect, col, row));
	...
	fn try_widget_scrollbar_drag(&mut self, slot: super::PanelSlot, row: u16) -> bool {
		...
		let track = self
			.panel(slot)
			.and_then(|fwp| fwp.scrollbar_tracks.iter().find(|t| t.list_key == key).cloned());
		let Some(t) = track else {
			return false;
		};
		let state = ScrollbarState::new(t.total, t.visible, t.scroll);
		let new_offset =
			self.panel_mut(slot).and_then(|fwp| fwp.scrollbar_mouse.drag(state, t.rect, row));
	...
	pub(super) fn release_widget_scrollbar(&mut self) {
		for fwp in [self.dock.as_mut(), self.floating_widget_panel.as_mut()].into_iter().flatten() {
	...
	fn apply_widget_scroll(
		...
		let moved_sel =
			self.widget_registry.set_list_scroll(panel_key, list_key, new_offset as u32, visible as u32);
	...
	fn handle_floating_widget_context_click(
		...
		let entries = self.panel(slot).map(|f| f.entries.clone()).unwrap_or_default();
		...
		let (mut payload, key, kind) =
			match self.widget_registry.hit_test(slot.buffer_id(), brow, bcol as u32) {
		...
		if !self.plugin_manager.read().unwrap().has_hook_handlers("widget_event") {
			return false;
		}
	...
	fn dismiss_floating_panel_with_cancel(&mut self, slot: super::PanelSlot) {
		...
		let widget_key =
			self.widget_registry.get(&panel_key).map(|p| p.focus_key.clone()).unwrap_or_default();
		self.fire_widget_event(&panel_key, widget_key, "cancel".to_string(), serde_json::json!({}));
	...
	fn handle_floating_widget_click(&mut self, slot: super::PanelSlot, col: u16, row: u16) {
		...
		let (mut hit_payload, hit_event, hit_key, hit_kind) =
			match self.widget_registry.hit_test(slot.buffer_id(), brow, bcol as u32) {
				Some((_, hit)) => {
					(hit.payload.clone(), hit.event_type.to_string(), hit.widget_key.clone(), hit.widget_kind)
				}
				None => {
					tracing::debug!(
							target: "dx::dock",
	...
	fn clear_active_window_drag_state(&mut self) {
		...
		ms.dragging_file_explorer_scrollbar = false;
		ms.drag_start_file_explorer_scroll = None;
