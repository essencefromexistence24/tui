...
use super::Editor;
use super::terminal_input::{TerminalModeInputHandler, should_enter_terminal_mode};
...
impl Editor {
	...
	pub fn dispatch_terminal_input(&mut self, event: &KeyEvent) -> Option<InputResult> {
		...
		if self.active_window().focused_terminal_live() {
			...
			let bypass_action = {
				...
				if self.command_registry.read().unwrap().is_terminal_bypass_action(&action) {
					Some(action)
				} else {
					None
				}
			...
			&& should_enter_terminal_mode(event)
		{
			...
			self.active_window_mut().send_terminal_key(event.code, event.modifiers);
			return Some(InputResult::Consumed);
		}
	...
	fn dispatch_modal_keyboard(&mut self, event: &KeyEvent) -> Option<InputResult> {
		...
		Some(match kind {
			LayerKind::Settings => {
				let result = {
					let settings =
						self.settings_state.as_mut().expect("Settings layer implies settings_state present");
	...
	pub fn dispatch_modal_input(&mut self, event: &KeyEvent) -> Option<InputResult> {
		...
		if self.active_window().prompt.is_some() {
			...
			if event.modifiers.contains(crossterm::event::KeyModifiers::ALT) {
				if let crossterm::event::KeyCode::Char(_) = event.code {
					let prompt_action = self
						...
						.read()
						.unwrap()
						.resolve_in_context_only(event, crate::input::keybindings::KeyContext::Prompt);
			...
			if self.is_file_open_active() {
				...
				let __win = self.windows.get_mut(&active_window_id).expect("active window present");
	...
	pub fn process_deferred_actions(&mut self, ctx: InputContext) {
		...
		for action in ctx.deferred_actions {
			if let Err(e) = self.execute_deferred_action(action) {
				self.set_status_message(t!("error.deferred_action", error = e.to_string()).to_string());
			}
		}
	}
		...
		match action {
			...
			DeferredAction::PreviewThemeFromPrompt => {
				if let Some(prompt) = &self.active_window_mut().prompt {
					if matches!(prompt.prompt_type, crate::view::prompt::PromptType::SelectTheme { .. }) {
			...
			DeferredAction::PromptSelectionChanged { selected_index } => {
				...
				let plugin_custom_type =
					self.active_window().prompt.as_ref().and_then(|p| match &p.prompt_type {
						crate::view::prompt::PromptType::Plugin { custom_type } => Some(custom_type.clone()),
						_ => None,
					});
			...
			DeferredAction::SendTerminalMouse { col, row, kind, modifiers } => {
				self.active_window_mut().send_terminal_mouse(col, row, kind, modifiers);
			}
			...
			DeferredAction::EnterScrollbackMode => {
				...
				self.enter_terminal_scrollback();
				self.set_status_message(
					"Scrollback mode - use PageUp/Down to scroll (Ctrl+Space to resume)".to_string(),
				);
	...
	fn prompt_history_prev(&mut self) {
		...
		let prompt_info =
			self.active_window().prompt.as_ref().map(|p| (p.prompt_type.clone(), p.input.clone()));
	...
	fn prompt_history_next(&mut self) {
		let prompt_type = self.active_window().prompt.as_ref().map(|p| p.prompt_type.clone());
	...
	fn overlay_toolbar_keys(&self) -> Vec<String> {
		self.active_chrome().prompt_toolbar_hits.iter().map(|(k, _)| k.clone()).collect()
	}
	...
	fn cycle_overlay_focus(&mut self, forward: bool) -> bool {
		...
		let has_toolbar =
			self.active_window().prompt.as_ref().is_some_and(|p| p.toolbar_widget.is_some());
		if !has_toolbar {
			return false;
		}
		...
		let cur = self.active_window().prompt.as_ref().and_then(|p| p.toolbar_focus.clone());
		// Ring includes the input as the `None` slot.
		let next: Option<String> = match cur {
			None => Some(if forward { keys[0].clone() } else { keys[keys.len() - 1].clone() }),
	...
	fn activate_focused_overlay_toggle(&mut self) {
		let key = self.active_window().prompt.as_ref().and_then(|p| p.toolbar_focus.clone());
		if let Some(key) = key {
	...
	pub(crate) fn toggle_overlay_toolbar_widget(&mut self, key: &str) {
		...
		let event: Option<(&'static str, serde_json::Value)> = {
			...
			match crate::widgets::find_widget_by_key(spec, key) {
				...
				Some(dx_core::api::WidgetSpec::Button { .. }) => Some(("activate", serde_json::json!({}))),
				_ => None,
			}
		};
	...
	fn handle_overlay_toolbar_key(&mut self, event: &KeyEvent) -> Option<InputResult> {
		...
		let focused = self.active_window().prompt.as_ref().is_some_and(|p| p.toolbar_focus.is_some());
		...
		match event.code {
			...
			KeyCode::Up | KeyCode::Down | KeyCode::PageUp | KeyCode::PageDown | KeyCode::Char(_)
				if focused =>
			{
				if let Some(p) = self.active_window_mut().prompt.as_mut() {
