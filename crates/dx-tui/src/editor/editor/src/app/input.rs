...
fn key_event_to_payload(ev: &crossterm::event::KeyEvent) -> dx_core::api::KeyEventPayload {
...
impl Editor {
	...
	fn try_resolve_next_key_callback(&mut self, key_event: &crossterm::event::KeyEvent) -> bool {
		...
		if let Some(callback_id) = self.active_window_mut().pending_next_key_callbacks.pop_front() {
		...
		if self.active_window_mut().key_capture_active {
			self.active_window_mut().pending_key_capture_buffer.push_back(payload);
			return true;
		}
		false
	}
...
impl Editor {
	...
	pub(crate) fn resolve_unfocused_popup_action(
		...
		let popup_visible = self.global_popups.is_visible() || self.active_state().popups.is_visible();
	...
	pub(crate) fn resolve_completion_popup_action(
		...
		match self.keybindings.read().unwrap().resolve_in_context_only(event, KeyContext::Completion) {
	...
	pub(crate) fn overlay_layers(&self) -> Vec<crate::app::overlay::Layer> {
		...
		if !trust_on_top && (self.global_popups.is_visible() || self.active_state().popups.is_visible())
		{
			layers.push(Layer {
		...
		let base_context = if self.active_window().is_composite_buffer(self.active_buffer()) {
	...
	pub fn handle_key(
		...
		tracing::trace!("Editor.handle_key: code={:?}, modifiers={:?}", code, modifiers);
		...
		if self.active_window().is_event_debug_active() {
			self.active_window_mut().handle_event_debug_input(&key_event);
			return Ok(());
		}
		...
		if self.floating_widget_panel.as_ref().is_some_and(|f| f.focused)
		...
		if self.dock.as_ref().is_some_and(|f| f.focused) {
			...
			let resolved = self.keybindings.read().ok().map(|kb| kb.resolve(&key_event, ctx));
		...
		if popup_visible_on_screen {
			...
			let (is_transient_popup, has_selection) = {
				let popup = self.global_popups.top().or_else(|| self.active_state().popups.top());
				(popup.is_some_and(|p| p.transient), popup.is_some_and(|p| p.has_selection()))
			};
			...
			let resolved_action =
				self.keybindings.read().ok().map(|kb| kb.resolve(&key_event, context.clone()));
			let is_focus_popup_key =
				matches!(resolved_action, Some(crate::input::keybindings::Action::PopupFocus));
		...
		if should_check_mode_bindings {
						...
						if let Some(panel_id) = self.focused_text_widget_panel_for_buffer(buffer_id) {
							...
							let handled = match code {
								...
								KeyCode::Right if ctrl => {
									self.with_focused_text_editor(&panel_id, |e| e.move_word_right_selecting())
								}
								KeyCode::Left => {
								...
								KeyCode::Up => self.with_focused_text_editor(&panel_id, |e| e.move_up_selecting()),
								KeyCode::Down => {
			...
			if let Some(ref mode_name) = self.active_window().editor_mode {
				...
				tracing::debug!("Mode '{}' is not read-only, allowing key through", mode_name);
			}
		}
		...
		// Action system via CompositeBuffer context bindings.
		{
			...
			if self.active_window().is_composite_buffer(active_buf) {
				if let Some(handled) = self.try_route_composite_key(active_split, active_buf, &key_event) {
					return handled;
				}
			}
		}
		...
		let (chord_result, action) = {
			let keybindings = self.keybindings.read().unwrap();
			let chord_result =
				keybindings.resolve_chord(&self.active_window().chord_state, &key_event, context.clone());
	...
	pub(crate) fn handle_action(&mut self, action: Action) -> AnyhowResult<()> {
		...
		match action {
			...
			Action::WorkspaceTrustTrust => {
				self.set_workspace_trust_level(crate::services::workspace_trust::TrustLevel::Trusted);
			}
			Action::WorkspaceTrustRestrict => {
				self.set_workspace_trust_level(crate::services::workspace_trust::TrustLevel::Restricted);
			}
			Action::WorkspaceTrustBlock => {
				self.set_workspace_trust_level(crate::services::workspace_trust::TrustLevel::Blocked);
			}
			...
			Action::SaveAs => {
				...
				let current_path = self
					...
					.map(|p| {
						...
						p.strip_prefix(self.working_dir()).unwrap_or(p).to_string_lossy().to_string()
					})
					.unwrap_or_default();
				self.start_prompt_with_initial_text(
					t!("file.save_as_prompt").to_string(),
			...
			Action::GotoLine => {
				...
				if has_line_index {
					self.start_prompt(t!("file.goto_line_prompt").to_string(), PromptType::GotoLine);
				} else {
					self.start_prompt(
			...
			Action::Revert => {
				...
				if self.active_state().buffer.is_modified() {
					...
					self.start_prompt(
						t!("prompt.revert_confirm", revert_key = revert_key, cancel_key = cancel_key)
							.to_string(),
					...
					if let Err(e) = self.revert_file() {
						self
							.set_status_message(t!("error.failed_to_revert", error = e.to_string()).to_string());
					}
				}
			}
			...
			Action::FormatBuffer => {
				if let Err(e) = self.format_buffer() {
					self.set_status_message(t!("error.format_failed", error = e.to_string()).to_string());
				}
			}
			Action::TrimTrailingWhitespace => match self.trim_trailing_whitespace() {
				...
				Err(e) => {
					self.set_status_message(t!("error.trim_whitespace_failed", error = e).to_string());
				}
			},
			Action::EnsureFinalNewline => match self.ensure_final_newline() {
				...
				Err(e) => {
					self.set_status_message(t!("error.ensure_newline_failed", error = e).to_string());
				}
			},
			Action::Copy => {
			...
			Action::CycleLiveGrepProvider => {
				...
				if !in_live_grep {
					self
						.set_status_message("Cycle Live Grep provider only works inside Live Grep".to_string());
					return Ok(());
				}
			...
			Action::ToggleLineWrap => {
				...
				for leaf_id in leaf_ids {
					let buffer_id =
						self.split_manager_mut().get_buffer_id(leaf_id.into()).unwrap_or(BufferId(0));
					let effective_wrap = self.active_window().resolve_line_wrap_for_buffer(buffer_id);
			...
			Action::ToggleCurrentLineHighlight => {
				...
				for leaf_id in leaf_ids {
					if let Some(view_state) = self
						...
						.get_mut(&leaf_id)
					{
						view_state.highlight_current_line = self.config.editor.highlight_current_line;
					}
				}
			...
			Action::ToggleOccurrenceHighlight => {
				...
				for window in self.windows.values_mut() {
					for (_, state) in &mut window.buffers {
						...
						if !new_value {
							state.reference_highlight_overlay.clear(&mut state.overlays, &mut state.marker_list);
						}
					}
				}
				...
				self.set_status_message(t!("view.occurrence_highlight_state", state = state).to_string());
			}
			Action::ToggleReadOnly => {
				...
				let is_now_read_only = self
					...
					.map(|m| !m.read_only)
					.unwrap_or(false);
				self.active_window_mut().mark_buffer_read_only(buffer_id, is_now_read_only);
			...
			Action::SetBackground => {
				let default_path = self
					...
					.as_ref()
					.and_then(|p| {
						p.strip_prefix(self.working_dir()).ok().map(|rel| rel.to_string_lossy().to_string())
					})
			...
			Action::ToggleDockFocus => {
				...
				match self.dock.as_ref().map(|d| d.focused) {
					...
					None => {
						return self
							.handle_action(Action::PluginAction("orchestrator_dock_toggle".to_string()));
					}
				}
			}
			...
			Action::FileExplorerSearchClear => self.active_window_mut().file_explorer_search_clear(),
			...
			Action::FileExplorerExtendSelectionDown => {
				self.active_window_mut().file_explorer_extend_selection_down()
			}
			Action::FileExplorerToggleSelect => self.active_window_mut().file_explorer_toggle_select(),
			Action::FileExplorerSelectAll => self.active_window_mut().file_explorer_select_all(),
			Action::RemoveSecondaryCursors => {
				// Convert action to events and apply them
				if let Some(events) =
					self.active_window_mut().action_to_events(Action::RemoveSecondaryCursors)
				{
					...
					self.active_window_mut().ensure_cursor_visible_for_split(active_buffer, active_split);
				}
			}
			...
			Action::SmartHome => {
				...
				let buffer_id = self.active_buffer();
				if self.active_window().is_composite_buffer(buffer_id) {
					if let Some(_handled) = self.handle_composite_action(buffer_id, &Action::SmartHome) {
						return Ok(());
					}
				}
				self.smart_home();
			...
			Action::ToggleSearchCaseSensitive => {
				...
				let state = if self.active_window().search_case_sensitive { "enabled" } else { "disabled" };
				self.set_status_message(t!("search.case_sensitive_state", state = state).to_string());
				self.refresh_active_search();
			}
			Action::ToggleSearchWholeWord => {
				self.active_window_mut().search_whole_word = !self.active_window().search_whole_word;
				let state = if self.active_window().search_whole_word { "enabled" } else { "disabled" };
			...
			Action::ToggleSearchRegex => {
				...
				let state = if self.active_window().search_use_regex { "enabled" } else { "disabled" };
				self.set_status_message(t!("search.regex_state", state = state).to_string());
				self.refresh_active_search();
			}
			Action::ToggleSearchConfirmEach => {
				self.active_window_mut().search_confirm_each = !self.active_window().search_confirm_each;
				let state = if self.active_window().search_confirm_each { "enabled" } else { "disabled" };
			...
			Action::PromptSaveMacroToInit => {
				self.start_prompt("Save macro to init.ts (0-9): ".to_string(), PromptType::SaveMacroToInit);
			}
			Action::PromptPromoteMacro => {
				self.start_prompt("Promote macro to command (0-9): ".to_string(), PromptType::PromoteMacro);
			}
			...
			Action::DeleteBackward => {
				...
				if let Some(events) = self.active_window_mut().action_to_events(Action::DeleteBackward) {
					if events.len() > 1 {
			...
			Action::PluginAction(action_name) => {
				...
				#[cfg(feature = "plugins")]
				{
					let result = self.plugin_manager.read().unwrap().execute_action_async(&action_name);
					if let Some(result) = result {
						match result {
							...
							Err(e) => {
								self.set_status_message(t!("view.plugin_error", error = e.to_string()).to_string());
			...
			Action::LoadPluginFromBuffer => {
				#[cfg(feature = "plugins")]
				{
					...
					let content = String::from_utf8_lossy(&buffer.slice_bytes(0..total)).to_string();
					...
					let load_result =
						self.plugin_manager.read().unwrap().load_plugin_from_source(&content, &name, is_ts);
					match load_result {
						Ok(()) => {
			...
			Action::InitReload => {
				...
				// Re-fire plugins_loaded so handlers expecting a "dx"
			...
			Action::InitEdit => {
				...
				match crate::init_script::ensure_starter(&config_dir) {
					Ok(path) => {
						...
						let declarations = self.plugin_manager.read().unwrap().plugin_declarations();
			...
			Action::InitCheck => {
				// Run the same parse check as `dx --cmd init check` but
				...
				if report.ok && report.diagnostics.is_empty() {
					...
					self.set_status_message(format!("init.ts: {} warning(s)", report.diagnostics.len()));
				}
			}
			Action::OpenTerminal => {
				self.open_terminal();
			...
			Action::FocusTerminal => {
				...
				if self.active_window().is_terminal_buffer(self.active_buffer()) {
			...
			Action::ToggleKeyboardCapture => {
				...
				if self.active_window().focused_terminal_live() {
					self.active_window_mut().keyboard_capture = !self.active_window_mut().keyboard_capture;
			...
			Action::TerminalPaste => {
				...
				if self.active_window().focused_terminal_live() {
					if let Some(text) = self.clipboard.paste() {
						self.active_window_mut().send_terminal_input(text.as_bytes());
					}
				}
			}
			...
			Action::CloseSettings => {
				...
				let has_changes = self.settings_state.as_ref().is_some_and(|s| s.has_changes());
				if has_changes {
	...
	fn dispatch_floating_widget_key(
		...
		if matches!(self.panel(slot).map(|f| f.placement), Some(super::PanelPlacement::LeftDock { .. }))
		{
			let on_filter =
				self.widget_registry.focus_key(&panel_key).map(|k| k == "filter").unwrap_or(false);
				...
				KeyCode::Enter => {
						...
						.unwrap_or(true)
					{
						...
						self.handle_widget_command(
							&panel_key,
							dx_core::api::WidgetAction::Key { key: "Enter".to_string() },
						);
					}
					return true;
				}
		...
		let key_name: Option<&str> = match code {
			...
			KeyCode::Tab => {
				Some(if modifiers.contains(KeyModifiers::SHIFT) { "Shift+Tab" } else { "Tab" })
			}
		...
		if let Some(name) = key_name {
			...
			self.handle_widget_command(
				&panel_key,
				dx_core::api::WidgetAction::Key { key: name.to_string() },
			);
			return true;
		}
		if let KeyCode::Char(c) = code {
			...
			if ch == ' ' {
				self.handle_widget_command(
					&panel_key,
					dx_core::api::WidgetAction::Key { key: "Space".to_string() },
				);
				return true;
			}
			self.handle_widget_command(
				&panel_key,
				dx_core::api::WidgetAction::TextInputChar { text: ch.to_string() },
			);
			return true;
		}
