...
impl Editor {
	...
	pub(crate) fn resolve_remote_attach(&self, request_id: u64) {
		self
			...
			.read()
			.unwrap()
			.resolve_callback(dx_core::api::JsCallbackId::from(request_id), "null".to_string());
	}
	...
	pub(crate) fn reject_remote_attach(&self, request_id: u64, error: String) {
		...
		self
			...
			.read()
			.unwrap()
			.reject_callback(dx_core::api::JsCallbackId::from(request_id), error);
	}
	...
	pub fn process_async_messages(&mut self) -> bool {
		...
		let needs_render = messages.iter().any(|m| {
			!matches!(m, AsyncMessage::Plugin(dx_core::api::PluginAsyncMessage::DelayComplete { .. }))
		});
		tracing::trace!(async_message_count = messages.len(), "received async messages");

		for message in messages {
			match message {
				AsyncMessage::LspDiagnostics { uri, diagnostics, server_name } => {
				...
				AsyncMessage::LspImplementation { request_id, locations } => {
				...
				AsyncMessage::LspCodeActionResolved { request_id: _, action } => {
					self.handle_lsp_code_action_resolved(action);
				}
				AsyncMessage::LspCompletionResolved { request_id: _, item } => {
					if let Ok(resolved) = item {
						self.handle_completion_resolved(resolved);
					}
				}
				AsyncMessage::LspFormatting { request_id: _, uri, edits } => {
					if !edits.is_empty() {
				...
				AsyncMessage::LspPrepareRename { request_id: _, result } => {
				...
				AsyncMessage::LspInlayHints { request_id, uri, hints } => {
					self.handle_lsp_inlay_hints(request_id, uri, hints);
				}
				AsyncMessage::LspFoldingRanges { request_id, uri, ranges } => {
					self.handle_lsp_folding_ranges(request_id, uri, ranges);
				}
				AsyncMessage::LspSemanticTokens { request_id, uri, response } => {
				...
				AsyncMessage::LspDynamicCapabilities { language, server_name, register, registrations } => {
					self.handle_lsp_dynamic_capabilities(language, server_name, register, registrations);
				}
				...
				AsyncMessage::LspProgress { language, token, value } => {
					self.handle_lsp_progress(language, token, value);
				}
				AsyncMessage::LspWindowMessage { language, message_type, message } => {
					self.handle_lsp_window_message(language, message_type, message);
				}
				AsyncMessage::LspLogMessage { language, message_type, message } => {
					self.handle_lsp_log_message(language, message_type, message);
				}
				AsyncMessage::LspStatusUpdate { language, server_name, status, message: _ } => {
				...
				AsyncMessage::TerminalExited { terminal, exit_code } => {
					self.handle_terminal_exited(terminal, exit_code);
				}

				AsyncMessage::LspServerRequest { language, server_command, method, params } => {
					self.handle_lsp_server_request(language, server_command, method, params);
				}
				AsyncMessage::PluginLspResponse { language: _, request_id, result } => {
				...
				AsyncMessage::RemoteAttachFailed { error, request_id, reconnect_window } => {
					self.handle_remote_attach_failed(error, request_id, reconnect_window);
				}
				AsyncMessage::PluginProcessOutput { process_id, stdout, stderr, exit_code } => {
					...
					self.handle_plugin_process_output(
						dx_core::api::JsCallbackId::from(process_id),
						stdout,
						stderr,
						exit_code,
					);
				}
				AsyncMessage::GrammarRegistryBuilt { registry, callback_ids } => {
					self.handle_grammar_registry_built(registry, callback_ids);
				}
				AsyncMessage::QuickOpenFilesLoaded { cwd, files, complete } => {
					self.handle_quick_open_files_loaded(cwd, files, complete);
				}
				AsyncMessage::PluginsDirLoaded { dir, errors, discovered_plugins } => {
	...
	fn handle_lsp_initialized(
		...
		tracing::info!("LSP server '{}' initialized for language: {}", server_name, language);
	...
	fn handle_lsp_error(
		...
		self.active_window_mut().status_message = Some(format!("LSP error ({}): {}", language, error));
		...
		if let Some(log_path) = stderr_log_path {
			...
			if has_content {
				...
				match self.open_file_no_focus(&log_path) {
					Ok(buffer_id) => {
						self.active_window_mut().mark_buffer_read_only(buffer_id, true);
						self.active_window_mut().status_message =
							Some(format!("LSP error ({}): {} - See stderr log", language, error));
					}
					Err(e) => {
	...
	fn handle_plugin_async_message(&mut self, plugin_msg: dx_core::api::PluginAsyncMessage) {
		use dx_core::api::{JsCallbackId, PluginAsyncMessage};
		match plugin_msg {
			PluginAsyncMessage::ProcessOutput { process_id, stdout, stderr, exit_code } => {
				self.handle_plugin_process_output(
			...
			PluginAsyncMessage::ProcessExit { process_id, callback_id, exit_code } => {
				self.background_process_handles.remove(&process_id);
				let result = dx_core::api::BackgroundProcessResult { process_id, exit_code };
				self.plugin_manager.read().unwrap().resolve_callback(
					JsCallbackId::from(callback_id),
					serde_json::to_string(&result).unwrap(),
				);
			}
			PluginAsyncMessage::LspResponse { language: _, request_id, result } => {
				self.handle_plugin_lsp_response(request_id, result);
			}
	...
	fn handle_terminal_output(&mut self, terminal: dx_core::WindowTerminalId) {
		...
		if self.config.terminal.jump_to_end_on_output && !self.active_window().focused_terminal_live() {
			// Check if active buffer is this terminal
			if let Some(active_terminal_id) = self.active_window().get_terminal_id(self.active_buffer()) {
	...
	fn handle_path_changed(
		...
		self.path_changes_for_test.push((handle, path.clone(), kind.as_str()));
		self.plugin_manager.read().unwrap().run_hook(
			"path_changed",
	...
	pub(crate) fn reattach_window(&mut self, window_id: dx_core::WindowId) {
	...
	fn ensure_remote_reconnect_forwarders(&mut self) {
		let (Some(runtime), Some(bridge)) = (self.tokio_runtime.as_ref(), self.async_bridge.as_ref())
		else {
			return;
		};
		...
		for (id, notify) in to_spawn {
			...
			runtime.spawn(async move {
				loop {
					...
					if sender.send(AsyncMessage::RemoteReconnected { connection_id: id }).is_err() {
	...
	pub(crate) fn poll_remote_connection_changes(&mut self) -> bool {
		let current: std::collections::HashMap<dx_core::WindowId, bool> = self
			.windows
			.iter()
			.filter_map(|(id, w)| {
				...
				fs.remote_connection_info().is_some().then(|| (*id, fs.is_remote_connected()))
			})
			.collect();
	...
	fn handle_terminal_exited(
		&mut self,
		terminal: dx_core::WindowTerminalId,
		...
		if let Some((&buffer_id, _)) =
			self.active_window().terminal_buffers.iter().find(|(_, tb)| tb.terminal_id == terminal_id)
		{
			...
			if !preserve_for_reconnect {
				...
				for leaf in dead_splits {
					self.active_window_mut().set_split_terminal_scrollback(leaf, buffer_id, true);
				}
			}
			...
			if self.active_buffer() == buffer_id
				&& self.active_window().key_context == crate::input::keybindings::KeyContext::Terminal
			{
			...
			if let Some(backing_path) =
				self.active_window().terminal_backing_files.get(&terminal_id).cloned()
			{
				if let Ok(mut file) =
	...
	fn handle_remote_attach_ready(
		...
		match mode {
			...
			crate::services::async_bridge::RemoteAttachMode::Window { label, command } => {
				...
				match self.create_remote_session_window(authority, keepalive, root, label, command, spec) {
			...
			crate::services::async_bridge::RemoteAttachMode::Reconnect { window_id } => {
				...
				if self.dormant_remote.contains_key(&window_id) {
					...
					tracing::info!("Reconnected dormant session {window_id} ({})", authority.display_label);
	...
	fn handle_quick_open_files_loaded(
		...
		if let Some((provider, _)) = self.quick_open_registry.get_provider_for_input("") {
			if let Some(fp) =
				provider.as_any().downcast_ref::<crate::input::quick_open::providers::FileProvider>()
			{
				if complete {
...
mod tests {
	...
	fn dive_reconnect_failure_records_error_on_its_window() {
		...
		let err = editor.windows.get(&win).unwrap().remote_reconnect_error.clone();
		assert_eq!(
			err.as_deref(),
	...
	fn non_reconnect_attach_failure_sets_no_window_error() {
		...
		assert!(
			editor.windows.get(&win).unwrap().remote_reconnect_error.is_none(),
