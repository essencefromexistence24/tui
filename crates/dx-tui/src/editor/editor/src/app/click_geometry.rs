...
pub(crate) fn adjust_content_rect_for_compose(
	...
	if let Some(cw) = compose_width {
		...
		if clamped < content_rect.width {
			...
			return Rect::new(content_rect.x + left_pad, content_rect.y, clamped, content_rect.height);
		}
	}
	content_rect
}
...
pub(crate) fn screen_to_buffer_position_with_overshoot(
	...
	Some(ClickTarget { position, col_overshoot, row_overshoot, text_col })
}
...
mod tests {
	...
	fn screen_to_buffer_position_returns_fallback_when_no_mappings() {
		...
		let pos = screen_to_buffer_position(
			/* col */ 5, /* row */ 5, /* content_rect */ r, /* gutter_width */ 3,
			/* cached_mappings */ &None, /* fallback_position */ 42,
			/* allow_gutter_click */ true, /* compose_width */ None,
		);
