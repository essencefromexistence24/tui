...
pub fn file_path_to_lsp_uri(path: &Path) -> Option<lsp_types::Uri> {
	dx_core::file_uri::path_to_lsp_uri(path)
}
...
impl LspUri {
	...
	pub fn from_host_path(
		...
		let mapped =
			translation.and_then(|t| t.host_to_remote(path)).unwrap_or_else(|| path.to_path_buf());
		dx_core::file_uri::path_to_lsp_uri(&mapped).map(Self)
	}
	...
	pub fn to_host_path(
		...
		let raw = dx_core::file_uri::lsp_uri_to_path(&self.0)?;
		Some(translation.and_then(|t| t.remote_to_host(&raw)).unwrap_or(raw))
	}
...
mod lsp_uri_tests {
	...
	fn to_host_path_under_remote_root_translates_back() {
		...
		let host = LspUri::from_wire(wire).to_host_path(Some(&translation())).expect("file:// URI");
...
mod uri_encoding_tests {
	...
	fn test_brackets_in_path() {
		...
		assert!(uri.is_some(), "URI should be computed for path with brackets");
		let uri = uri.unwrap();
		assert!(
	...
	fn test_normal_path() {
		...
		assert!(s.ends_with("project/main.go"), "Should end with the path: {}", s);
	}
