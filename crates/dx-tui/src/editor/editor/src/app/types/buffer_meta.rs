...
impl BufferMetadata {
	...
	pub fn new() -> Self {
		Self {
			kind: BufferKind::File { path: PathBuf::new(), uri: None },
	...
	pub fn with_file(
		...
		Self {
			kind: BufferKind::File { path: canonical_path, uri: file_uri },
			display_name,
			lsp_enabled: true,
			lsp_disabled_reason: None,
	...
	pub fn with_container_file(container_path: PathBuf, uri: LspUri) -> Self {
		...
		Self {
			kind: BufferKind::File { path: container_path, uri: Some(uri) },
			display_name,
			lsp_enabled: true,
			lsp_disabled_reason: None,
			read_only: true,
...
mod is_library_path_tests {
	...
	fn cargo_git_checkouts_are_library_files() {
		assert!(check("/home/user/.cargo/git/checkouts/some-dep-abcdef/abcdef/src/lib.rs"));
	}
