...
pub(crate) struct StatusBarChrome {
	...
	/// `StatusBarClickable`.
	pub clickable: Vec<(crate::view::ui::status_bar::StatusBarClickable, u16, u16, u16)>,
...
impl WindowLayoutCache {
	...
	pub fn byte_to_visual_column(&self, split_id: LeafId, byte_pos: usize) -> Option<usize> {
		...
		let last_real_col = row
			...
			.enumerate()
			.rev()
			.find(|(_, &char_idx)| row.char_source_bytes.get(char_idx).is_some_and(|b| b.is_some()))
	...
	pub fn move_visual_line(
		...
		let navigable = |idx: usize| -> bool {
			mappings.get(idx).map(|m| m.char_source_bytes.iter().any(|b| b.is_some())).unwrap_or(false)
		};
		loop {
