...
use crate::config::{Menu, MenuExt, MenuItem, generate_dynamic_items};
...
impl Editor {
		...
		F: FnOnce(&mut Menu) -> R,
	{
		...
		if let Some(idx) = self.menu_state.plugin_menus.iter().position(|m| m.label == label) {
				...
				if d < self.menu_state.submenu_path.len() {
					...
					match current_items.get(submenu_idx) {
						...
						Some(MenuItem::DynamicSubmenu { source, .. }) => {
							current_items = generate_dynamic_items(source, &self.menu_state.themes_dir);
						}
						_ => return Ok(Some(Ok(()))),
					}
				} else {
