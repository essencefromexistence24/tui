...
impl Window {
	...
	pub(crate) fn detect_terminal_scrollback_link_at(
		...
		let (split_id, content_rect) =
			self.layout_cache.split_areas.iter().find_map(|(sid, bid, rect, _, _, _)| {
				(*bid == active
					&& col >= rect.x
		...
		let char_col = line.char_indices().take_while(|(b, _)| *b < pos.column).count();
		...
		let cwd = self
			...
			.and_then(|h| h.state.lock().ok().and_then(|s| s.cwd().map(|p| p.to_path_buf())));
	...
	fn get_terminal_content_area_at_position(&self, col: u16, row: u16) -> Option<(BufferId, Rect)> {
