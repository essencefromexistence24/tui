...
impl Editor {
	...
	pub(super) fn start_macro_recording(&mut self, key: char) {
		...
		self.set_status_message(
			t!("macro.recording_with_hint", key = key, stop_hint = stop_hint).to_string(),
		);
	}
	...
	fn build_macro_stop_hint(&self, _key: char) -> String {
		...
		let palette_key =
			self.get_keybinding_for_action("command_palette").unwrap_or_else(|| "Ctrl+P".to_string());
	...
	pub(super) fn stop_macro_recording(&mut self) {
		...
		self.set_status_message(
			t!("macro.saved", key = key, count = action_count, play_hint = play_hint).to_string(),
		);
	}
	...
	pub(super) fn record_macro_action(&mut self, action: &Action) {
		if let Action::PromptConfirm = action {
			if let Some(prompt) = &self.active_window_mut().prompt {
				...
				self.active_window_mut().macros.record_transformed(Action::PromptConfirmWithText(text));
				return;
			}
		}
	...
	pub(super) fn show_macro_in_buffer(&mut self, key: char) {
		...
		let (json, actions_len) = match self.active_window_mut().macros.get(key) {
			Some(actions) => {
				...
				let specs: Vec<dx_core::api::ActionSpec> =
					actions.iter().map(|a| a.to_action_spec()).collect();
				let json = match serde_json::to_string_pretty(&specs) {
					Ok(json) => json,
					Err(e) => {
						self
							.set_status_message(t!("macro.serialize_failed", error = e.to_string()).to_string());
						return;
					}
				};
				(json, actions.len())
		...
		self.active_window_mut().event_logs.insert(buffer_id, EventLog::new());
		...
		let metadata = BufferMetadata {
			kind: BufferKind::Virtual { mode: "macro-view".to_string() },
		...
		self.set_status_message(t!("macro.shown_buffer", key = key, count = actions_len).to_string());
	}
	...
	pub(super) fn list_macros_in_buffer(&mut self) {
		...
		// Set metadata
		let metadata = BufferMetadata {
			kind: BufferKind::Virtual { mode: "macro-list".to_string() },
	...
	fn write_macro_to_init(&mut self, key: char, promote: bool) {
		...
		let path = match crate::init_script::ensure_starter(&config_dir) {
			Ok(p) => p,
			Err(e) => {
				self.set_status_message(t!("macro.init_write_failed", error = e.to_string()).to_string());
				return;
			}
		};
