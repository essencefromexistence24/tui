...
use self::types::{
	DEFAULT_BACKGROUND_FILE, LspMenuItem, LspMessageEntry, LspProgressInfo, SearchState,
	TabContextMenu,
};
...
use crate::view::ui::{
	ExplorerDecorations, FileExplorerRenderer, SplitRenderer, StatusBarRenderer, SuggestionsRenderer,
};
...
fn lsp_uri_to_host_path(
	...
	uri.to_host_path(translation).ok_or_else(|| "URI is not a file path".to_string())
}
...
pub struct Editor {
	...
	pending_grammar_callbacks: Vec<dx_core::api::JsCallbackId>,
	...
	/// Tokio runtime handle for async I/O tasks (shared from parent runtime).
	...
	remote_connected_cache: HashMap<dx_core::WindowId, bool>,
	...
	pub(crate) windows: HashMap<dx_core::WindowId, crate::app::window::Window>,
	...
	pub(crate) session_keepalives: HashMap<dx_core::WindowId, Box<dyn std::any::Any + Send>>,
	...
	pub(crate) active_window: dx_core::WindowId,
	...
	/// `materialize_window`.
	pub(crate) materialize_pending: std::collections::HashSet<dx_core::WindowId>,
	...
	pub(crate) window_cycle_order: Option<Vec<dx_core::WindowId>>,
	...
	pub(crate) plugin_schemas: std::sync::Arc<std::sync::RwLock<HashMap<String, serde_json::Value>>>,
	...
	pending_plugin_actions:
		Vec<(String, crate::services::plugins::thread::oneshot::Receiver<anyhow::Result<()>>)>,
	...
	pub(crate) preview_window_id: Option<dx_core::WindowId>,
	...
	pub(crate) pending_vb_animations: Vec<(u64, BufferId, dx_core::api::PluginAnimationKind)>,
...
pub(crate) struct FloatingWidgetState {
	...
	/// time.
	pub entries: Vec<dx_core::text_property::TextPropertyEntry>,
...
impl Editor {
	/// Get the editor's tokio runtime handle (for entering its context before blocking).
	...
	fn load_ansi_background(&mut self, input: &str) -> AnyhowResult<()> {
		...
		self.set_status_message(
			t!("view.background_set", path = canonical.display().to_string()).to_string(),
		);
	...
	pub fn buffer_count_for_tests(&self) -> usize {
		self.windows.get(&self.active_window).map(|w| &w.buffers).expect("active window present").len()
	}
	...
	pub fn leaf_gutter_width(&self, leaf: dx_core::LeafId, buffer_id: BufferId) -> u16 {
	...
	pub fn active_event_log(&self) -> &EventLog {
		self.active_window().event_logs.get(&self.active_buffer()).unwrap()
	}

	/// Get the event log for the active buffer (mutable)
	pub fn active_event_log_mut(&mut self) -> &mut EventLog {
		let buffer_id = self.active_buffer();
		self.active_window_mut().event_logs.get_mut(&buffer_id).unwrap()
	}
	...
	pub fn set_status_bar_value(
		...
		for window in self.windows.values_mut() {
			if window.buffers.contains_key(&buffer_id) {
				window.status_bar_values.entry(buffer_id).or_default().insert(key.to_string(), value);
				return Ok(());
			}
		}
	...
	fn remove_plugin_status_bar_elements(&mut self, plugin_name: &str) {
		...
		self.status_bar_token_registry.lock().unwrap().retain(|k, _| !k.starts_with(&prefix));
		for window in self.windows.values_mut() {
...
mod tests {
	...
	fn parse_key_string_shift_tab_normalizes_to_backtab() {
		...
		// matches.
		assert_eq!(parse_key_string("S-Tab"), Some((KeyCode::BackTab, KeyModifiers::NONE)),);
		assert_eq!(parse_key_string("BackTab"), Some((KeyCode::BackTab, KeyModifiers::NONE)),);
		// Plain Tab is unaffected.
		assert_eq!(parse_key_string("Tab"), Some((KeyCode::Tab, KeyModifiers::NONE)),);
	}
	...
	fn unmounting_centered_modal_does_not_request_full_redraw() {
		...
		assert!(editor.floating_widget_panel.is_none(), "centered modal should be unmounted");
		assert!(
			!editor.take_full_redraw_request(),
	...
	fn test_action_to_events_insert_char() {
		...
		let events = editor.active_window_mut().action_to_events(Action::InsertChar('a'));
	...
	fn test_action_to_events_move_right() {
		...
		let events = editor.active_window_mut().action_to_events(Action::MoveRight);
		assert!(events.is_some());
		...
		match &events[0] {
			Event::MoveCursor { new_position, new_anchor, .. } => {
	...
	fn test_action_to_events_insert_newline() {
		...
		let events = editor.active_window_mut().action_to_events(Action::InsertNewline);
		assert!(events.is_some());
	...
	fn test_action_to_events_unimplemented() {
		...
		assert!(editor.active_window_mut().action_to_events(Action::Save).is_none());
		assert!(editor.active_window_mut().action_to_events(Action::Quit).is_none());
		assert!(editor.active_window_mut().action_to_events(Action::Undo).is_none());
	}
	...
	fn test_action_to_events_delete_backward() {
		...
		let events = editor.active_window_mut().action_to_events(Action::DeleteBackward);
		assert!(events.is_some());
	...
	fn test_action_to_events_delete_forward() {
		...
		let events = editor.active_window_mut().action_to_events(Action::DeleteForward);
		assert!(events.is_some());
	...
	fn test_action_to_events_select_right() {
		...
		let events = editor.active_window_mut().action_to_events(Action::SelectRight);
		assert!(events.is_some());
	...
	fn test_action_to_events_select_all() {
		...
		let events = editor.active_window_mut().action_to_events(Action::SelectAll);
		assert!(events.is_some());
	...
	fn test_action_to_events_document_nav() {
		...
		let events = editor.active_window_mut().action_to_events(Action::MoveDocumentStart);
		assert!(events.is_some());
		let events = events.unwrap();
		match &events[0] {
			Event::MoveCursor { new_position, .. } => {
		...
		let events = editor.active_window_mut().action_to_events(Action::MoveDocumentEnd);
		assert!(events.is_some());
		let events = events.unwrap();
		match &events[0] {
			Event::MoveCursor { new_position, .. } => {
	...
	fn test_action_to_events_remove_secondary_cursors() {
		...
		let events = editor.active_window_mut().action_to_events(Action::RemoveSecondaryCursors);
		assert!(events.is_some());
	...
	fn test_action_to_events_scroll() {
		...
		// Test ScrollUp
		let events = editor.active_window_mut().action_to_events(Action::ScrollUp);
		assert!(events.is_some());
		let events = events.unwrap();
		assert_eq!(events.len(), 1);
		match &events[0] {
		...
		// Test ScrollDown
		let events = editor.active_window_mut().action_to_events(Action::ScrollDown);
		assert!(events.is_some());
		let events = events.unwrap();
		assert_eq!(events.len(), 1);
		match &events[0] {
	...
	fn repro_2414_plugin_edit_same_line_next_match() {
		...
		let m = editor.active_window().search_state.as_ref().unwrap().matches.clone();
	...
	fn test_search_scan_completes_when_capped() {
		...
		let fake_chunks = vec![
			crate::model::buffer::LineScanChunk { leaf_index: 0, byte_len: 100, already_known: true },
			crate::model::buffer::LineScanChunk { leaf_index: 1, byte_len: 100, already_known: true },
		];
		...
		assert!(result, "process_search_scan should return true (needs render)");
		...
		assert!(search_state.capped, "search_state should be marked as capped");
	}
	...
	fn test_bookmarks() {
		...
		assert_eq!(editor.active_window().bookmarks.get('1').map(|b| b.position), Some(7));
	...
	fn test_lsp_rename_didchange_positions_bug() {
		...
		editor.active_state_mut().buffer = Buffer::from_str(initial, 1024 * 1024, test_filesystem());
		...
		let batch = Event::Batch {
			events: vec![
				...
				Event::Insert { position: 23, text: "value".to_string(), cursor_id },
				...
				Event::Insert { position: 7, text: "value".to_string(), cursor_id },
			],
		...
		assert_eq!(first_del_range.start.line, 1, "First delete should be on line 1 (BEFORE)");
		assert_eq!(
		...
		eprintln!(
			"  Delete at line {}, char {}-{}",
			first_del_range.start.line, first_del_range.start.character, first_del_range.end.character
		);
	...
	fn test_lsp_rename_twice_consecutive() {
		...
		let events1 = vec![
			...
			Event::Delete { range: 23..26, deleted_text: "val".to_string(), cursor_id },
			Event::Insert { position: 23, text: "value".to_string(), cursor_id },
			// First occurrence (at position 7, line 0, char 7)
			Event::Delete { range: 7..10, deleted_text: "val".to_string(), cursor_id },
			Event::Insert { position: 7, text: "value".to_string(), cursor_id },
		];

		// Create batch for LSP change verification
		let batch1 = Event::Batch { events: events1.clone(), description: "LSP Rename 1".to_string() };
		...
		assert_eq!(after_first, "fn foo(value: i32) {\n    value + 1\n}\n", "After first rename");
		...
		let events2 = vec![
			...
			Event::Delete { range: 25..30, deleted_text: "value".to_string(), cursor_id },
			Event::Insert { position: 25, text: "x".to_string(), cursor_id },
			// First occurrence (at position 7, line 0, char 7)
			Event::Delete { range: 7..12, deleted_text: "value".to_string(), cursor_id },
			Event::Insert { position: 7, text: "x".to_string(), cursor_id },
		];

		// Create batch for LSP change verification
		let batch2 = Event::Batch { events: events2.clone(), description: "LSP Rename 2".to_string() };
		...
		assert_eq!(after_second, "fn foo(x: i32) {\n    x + 1\n}\n", "After second rename");
	}
	...
	fn test_ensure_active_tab_visible_static_offset() {
...

		{
			...
			view_state.open_buffers =
				vec![TabTarget::Buffer(buf1), TabTarget::Buffer(buf2), TabTarget::Buffer(buf3)];
		...
		editor.active_window_mut().ensure_active_tab_visible(split_id, buf1, 25);
		assert_eq!(editor.split_view_states().get(&split_id).unwrap().tab_scroll_offset, 0);

		// Now make the last tab active and ensure offset moves forward but stays bounded.
		editor.active_window_mut().ensure_active_tab_visible(split_id, buf3, 25);
	...
	fn close_others_re_anchors_tab_scroll_to_surviving_tab() {
		...
		for i in 0..6 {
			...
			editor
				...
				.unwrap()
				.buffer
				.rename_file_path(std::path::PathBuf::from(format!("long_filename_for_tab_{:02}.txt", i)));
			buffers.push(id);
		}
		...
		let keep = buffers[5];
		editor.active_window_mut().split_manager_mut().unwrap().set_split_buffer(split_id, keep);
		{
			let view_state = editor.split_view_states_mut().get_mut(&split_id).unwrap();
			view_state.open_buffers = buffers.iter().map(|b| TabTarget::Buffer(*b)).collect::<Vec<_>>();
		...
		assert_eq!(remaining_targets, vec![keep], "Close Others should leave only the kept buffer");
		let name_len = editor
			.buffers()
			.get(&keep)
			.unwrap()
