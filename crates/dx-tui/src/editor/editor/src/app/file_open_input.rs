...
impl Editor {
	...
	pub fn handle_file_open_action(&mut self, action: &Action) -> bool {
		...
		match action {
			...
			Action::PromptAcceptSuggestion => {
				...
				let selected_info = self.active_window_mut().file_open_state.as_ref().and_then(|s| {
					s.selected_index
						.and_then(|idx| s.entries.get(idx))
						.map(|e| (e.fs_entry.name.clone(), e.fs_entry.is_dir(), e.fs_entry.path.clone()))
				});
			...
			Action::PromptBackspace => {
				let filter_empty = self
					...
					.unwrap_or(true);
				let prompt_empty =
					self.active_window().prompt.as_ref().map(|p| p.input.is_empty()).unwrap_or(true);
	...
	fn file_open_open_file_at_location(
		...
		let detect_encoding =
			self.active_window_mut().file_open_state.as_ref().map(|s| s.detect_encoding).unwrap_or(true);
		...
		if let Err(e) = self.open_file(&path) {
				...
				e.downcast_ref::<crate::model::buffer::LargeFileEncodingConfirmation>()
			{
				...
				self.start_prompt(
					prompt_msg,
					PromptType::ConfirmLargeFileEncoding { path: confirmation.path.clone() },
				);
			} else {
			...
			self.set_status_message(t!("file.opened", path = path.display().to_string()).to_string());
		}
	}
		...
		let suggestions: Vec<crate::input::commands::Suggestion> = Encoding::all()
			...
			.map(|enc| {
				...
				crate::input::commands::Suggestion {
					...
					description: if is_default { Some("default".to_string()) } else { None },
	...
	fn file_open_create_new_file(&mut self, path: std::path::PathBuf) {
		...
		if let Err(e) = self.open_file(&path) {
			...
			self
				.set_status_message(t!("file.created_new", path = path.display().to_string()).to_string());
		}
	}
	...
	pub fn update_file_open_filter(&mut self) {
		...
		let filter = self.active_window().prompt.as_ref().map(|p| p.input.clone()).unwrap_or_default();
		...
		if filter.contains('/') {
			...
			let full_path =
				if tilde_expanded.is_absolute() { tilde_expanded } else { current_dir.join(&filter) };
	...
	pub fn file_open_toggle_hidden(&mut self) {
		if let Some(state) = &mut self.active_window_mut().file_open_state {
			...
			// Show status message
			let msg = if new_state { "Showing hidden files" } else { "Hiding hidden files" };
	...
	pub fn handle_file_open_scroll(&mut self, delta: i32) -> bool {
		...
		let visible_rows =
			self.active_window_mut().file_browser_layout.as_ref().map(|l| l.visible_rows).unwrap_or(10);
	...
	pub fn handle_file_open_click(&mut self, x: u16, y: u16) -> bool {
		...
		if layout.is_in_list(x, y) {
			let scroll_offset =
				self.active_window_mut().file_open_state.as_ref().map(|s| s.scroll_offset).unwrap_or(0);
	...
	pub fn compute_file_browser_hover(&self, x: u16, y: u16) -> Option<super::types::HoverTarget> {
		...
		if layout.is_in_list(x, y) {
			let scroll_offset =
				self.active_window().file_open_state.as_ref().map(|s| s.scroll_offset).unwrap_or(0);

			if let Some(idx) = layout.click_to_index(y, scroll_offset) {
				let total_entries =
					self.active_window().file_open_state.as_ref().map(|s| s.entries.len()).unwrap_or(0);
