...
impl Editor {
	...
	pub fn render(&mut self, frame: &mut Frame) {
		...
		let mut prompt_is_overlay = self.active_window().prompt.as_ref().is_some_and(|p| p.overlay);
		let mut has_suggestions =
			self.active_window().prompt.as_ref().is_some_and(|p| !p.suggestions.is_empty())
				&& !prompt_is_overlay;
		...
		let mut main_chunks = Layout::default()
			...
			.constraints(vec![
				Constraint::Length(if self.active_window_mut().menu_bar_visible { 1 } else { 0 }), // Menu bar
				...
				Constraint::Length(
					...
					if (self.active_window_mut().prompt_line_visible || self.active_window().prompt.is_some())
						&& !prompt_is_overlay
					{
						1
					} else {
		...
		if self.plugin_manager.read().unwrap().is_active() {
			...
			for (split_id, buffer_id, split_area) in visible_buffers {
				...
				let __win = self.windows.get_mut(&__active_id).expect("active window must exist");
				...
				let added = __win
					...
					.with_buffer_and_view_states(buffer_id, |state, vs_map| {
						...
						if pm_guard.has_subscribers("view_transform_request") {
							...
							let viewport_end =
								base_tokens.last().and_then(|t| t.source_offset).unwrap_or(viewport_start);
			...
			if dispatched_any {
				...
				prompt_is_overlay = self.active_window().prompt.as_ref().is_some_and(|p| p.overlay);
				has_suggestions =
					self.active_window().prompt.as_ref().is_some_and(|p| !p.suggestions.is_empty())
						&& !prompt_is_overlay;
				...
				main_chunks = Layout::default()
					.direction(Direction::Vertical)
					.constraints(vec![
						Constraint::Length(if self.active_window_mut().menu_bar_visible { 1 } else { 0 }),
						Constraint::Min(0),
						Constraint::Length(
							if !self.active_window_mut().status_bar_visible || has_suggestions || has_file_browser
							{
		...
		let lsp_waiting = !self.active_window().pending_completion_requests.is_empty()
			|| self.active_window().pending_goto_definition_request.is_some();
		...
		let __win = self.windows.get_mut(&active_window_id).expect("active window must exist");
		...
		let __scrollback_view_splits: std::collections::HashSet<crate::model::event::LeafId> = __win
			.buffers
			.splits()
			.map(|(_, vs_map)| {
				vs_map
					.iter()
					.filter(|(leaf, svs)| __win.split_terminal_scrollback(**leaf, svs.active_buffer))
					.map(|(leaf, _)| *leaf)
					.collect()
			})
			.unwrap_or_default();
		...
		if self.plugin_manager.read().unwrap().is_active() {
			...
			{
				let current =
					(view_state.viewport.top_byte, view_state.viewport.width, view_state.viewport.height);
				...
				let (changed, previous) = match self.active_window().previous_viewports.get(split_id) {
		...
		let __vp_win = self.windows.get_mut(&self.active_window).expect("active window present");
		__vp_win.previous_viewports.clear();
		let (_, __vp_vs_map) =
			__vp_win.buffers.splits().expect("active window must have a populated split layout");
		...
		self.active_window().render_terminal_splits(frame, &split_areas, true);
		...
		let mut separator_areas = self.split_manager_mut().get_separators_with_ids(editor_content_area);
		...
		if self.active_window().prompt.as_ref().is_some_and(|p| p.overlay) {
		...
		let top_is_trust_modal = self
			.global_popups
			.top()
			.is_some_and(|p| matches!(p.resolver, crate::view::popup::PopupResolver::WorkspaceTrust));
		...
		crate::view::color_support::convert_buffer_colors(frame.buffer_mut(), self.color_capability);

		// Frame-buffer animations run last so they mutate the final paint.
		self.active_window_mut().animations.apply_all(frame.buffer_mut());
			...
			if !prompt.overlay {
				...
				if matches!(
					...
					if let Some(file_open_state) = &self.active_window().file_open_state {
						StatusBarRenderer::render_file_open_prompt(frame, area, prompt, file_open_state, theme);
					} else {
	...
	fn render_buffer_popups(
		...
		let popup_info: Vec<_> = {
			...
			if state.popups.is_visible() {
				...
				let primary_cursor = primary_cursor.unwrap_or_else(|| crate::model::cursor::Cursor::new(0));

				// Compute gutter width so we know where text content starts
				let gutter_width =
					viewport.as_ref().map(|vp| vp.gutter_width(&state.buffer) as u16).unwrap_or(0);
				...
				let word_start_screen_pos = {
					...
					let word_start = find_completion_word_start(&state.buffer, primary_cursor.position);
				...
				let (base_x, base_y) =
					content_rect.map(|r| (r.x + gutter_width, r.y)).unwrap_or((gutter_width, 1));

				let cursor_screen_pos = (cursor_screen_pos.0 + base_x, cursor_screen_pos.1 + base_y);
				let word_start_screen_pos =
					(word_start_screen_pos.0 + base_x, word_start_screen_pos.1 + base_y);
	...
	fn render_software_cursor_and_capture(&mut self, frame: &mut Frame, size: ratatui::layout::Rect) {
	...
	fn render_panels_and_modals(
		...
		if self.floating_widget_panel.is_some() {
			...
			let fullscreen = self.floating_widget_panel.as_ref().map(|f| f.fullscreen).unwrap_or(false);
			...
			let modal_area = if fullscreen || is_anchored { size } else { chrome_area };
		...
		let trust_layout = if top_is_trust_modal {
			...
			Some(crate::view::workspace_trust_dialog::render_workspace_trust_dialog(
				frame,
	...
	fn split_file_explorer_area(
		...
		let file_explorer_should_show = self.file_explorer_visible()
			&& (self.file_explorer().is_some() || self.active_window().file_explorer_sync_in_progress);
		...
		let explorer_cols = self.active_window().file_explorer_width.to_cols(main_content_area.width);
	...
	fn render_dormant_shell_page(&mut self, frame: &mut Frame, area: ratatui::layout::Rect) {
		...
		let connecting = self.remote_attach_inflight.contains(&(u64::MAX - active_id.0));
		...
		let lines: [(&str, Style); 5] = [
			(detail.as_str(), Style::default().fg(fg).bg(bg).add_modifier(Modifier::BOLD)),
			("", Style::default().bg(bg)),
		...
		let draw_centered = |buf: &mut ratatui::buffer::Buffer, y: u16, text: &str, style: Style| {
		...
		draw_centered(buf, top + lines.len() as u16, retry_hint, Style::default().fg(dim).bg(bg));
	}
	...
	fn render_file_explorer(&mut self, frame: &mut Frame, area: ratatui::layout::Rect) {
		...
		let __win = self.windows.get_mut(&active_id).expect("active window must exist");
	...
	fn render_status_bar_row(
		...
		let current_language =
			self.buffers().get(&self.active_buffer()).map(|s| s.language.clone()).unwrap_or_default();
		...
		if self.active_window().status_bar_visible && !has_suggestions && !has_file_browser {
			...
			let (warning_level, general_warning_count) = if self.config.warnings.show_status_indicator {
				...
				(lsp_level, self.active_window().warning_domains.general.count)
			} else {
				(WarningLevel::None, 0)
			};
			...
			let remote_connecting =
				self.remote_attach_inflight.contains(&(u64::MAX - self.active_window_id().0))
			...
			let is_read_only =
				self.active_window().buffer_metadata.get(&active_buf).map(|m| m.read_only).unwrap_or(false);
			...
			let status_bar_layout = __win
				...
				.with_buffer_and_view_states(active_buf, |state, vs_map| {
					let cursors = vs_map.get(&active_split).map(|v| &v.cursors).unwrap_or(&default_cursors);
					...
					let mut sb_rec = crate::app::types::CellThemeRecorder::new(&mut status_bar_runs);
	...
	fn render_modal_overlays(&mut self, frame: &mut Frame, chrome_area: ratatui::layout::Rect) {
		...
		let settings_visible =
			draw_settings && self.settings_state.as_ref().map(|s| s.visible).unwrap_or(false);
		if settings_visible {
	...
	fn render_menu_bar(&mut self, frame: &mut Frame, menu_bar_area: ratatui::layout::Rect) {
		if self.active_window().menu_bar_visible {
			...
			let new_menu_layout = crate::view::ui::MenuRenderer::render(
				...
				hover_target.as_ref(),
				menu_bar_mnemonics,
				Some(&mut crate::app::types::CellThemeRecorder::new(&mut menu_runs)),
				draw_chrome,
			);
			drop(keybindings);
	...
	fn pre_sync_and_scroll_sync(&mut self) {
			...
			.active_split();
		{
			...
			self.active_window_mut().pre_sync_ensure_visible(active_split);
		}
		{
	...
	fn prepare_visible_buffers_for_render(&mut self) {
		...
		let prep_targets: Vec<(BufferId, usize, u16)> = {
			let win = self.windows.get(&active_id).expect("active window must exist");
			let (mgr, vs_map) =
				win.buffers.splits().expect("active window must have a populated split layout");
			vs_map
				.iter()
		...
		let win_buffers =
			&mut self.windows.get_mut(&active_id).expect("active window must exist").buffers;
	...
	fn cursor_obscured_by_overlay(&self, x: u16, y: u16) -> bool {
		...
		if self.active_chrome().popup_areas.iter().any(|entry| inside(entry.1)) {
			return true;
		}
		if self.active_chrome().global_popup_areas.iter().any(|entry| inside(entry.1)) {
			return true;
		}
	...
	fn render_session_preview_into_rect(
		...
		__win_for_preview.buffers.with_all_mut(|preview_buffers, mgr, view_states| {
	...
	fn prepare_overlay_preview(&mut self) {
		...
		let abs_path =
			if path_buf.is_absolute() { path_buf } else { self.working_dir().join(&path_buf) };
		// Canonicalize for buffer-dedup parity with open_file_no_focus.
		let abs_path = self.authority().filesystem.canonicalize(&abs_path).unwrap_or(abs_path);

		// If the standalone state already targets this path, just
		// re-seed the cursor and skip the file-load roundtrip.
		let already_target = self.active_window().overlay_preview_state.as_ref().is_some_and(|st| {
			self
				.windows
				.get(&self.active_window)
				.map(|w| &w.buffers)
		...
		let buffer_id = if already_target {
			self.active_window_mut().overlay_preview_state.as_ref().unwrap().buffer_id
		} else {
			...
			let was_open =
				self.buffers().iter().any(|(_, s)| s.buffer.file_path() == Some(abs_path.as_path()));
			...
			if !was_open {
				...
				let (__mgr, __vs_map) =
					__win.buffers.splits_mut().expect("active window must have a populated split layout");
				if let Some(source_state) = __vs_map.get_mut(&source_split) {
					if source_state.active_buffer == buffer_id {
						let fallback =
							source_state.open_buffers.iter().find_map(|t| t.as_buffer()).or_else(|| {
								__buffer_keys
									.iter()
									.copied()
		...
		let prev_preview_buffer =
			self.active_window().overlay_preview_state.as_ref().map(|s| s.buffer_id);
		...
		let byte_offset = self
			...
			.get(&buffer_id)
			.map(|s| {
				s.buffer.position_to_offset(crate::model::piece_tree::Position { line, column: col })
			})
			.unwrap_or(0);
		...
		let query = self.active_window().prompt.as_ref().map(|p| p.input.clone()).unwrap_or_default();
		...
		let preview_ns =
			crate::view::overlay::OverlayNamespace::from_string("overlay-preview-search".to_string());
		...
		if let Some(win) = self.windows.get_mut(&active_id) {
			...
			if let (Some(state), Some(pstate)) = (preview_buffer, preview_state) {
				...
				if pstate.centered_byte != Some(byte_offset) {
					...
					pstate.view_state.viewport.center_on_position(&mut state.buffer, byte_offset);
				...
				state.overlays.clear_namespace(&preview_ns, &mut state.marker_list);
			...
			if let Some(prev) = prev_preview_buffer {
				if prev != buffer_id {
					if let Some(prev_state) = win.buffers.get_mut(&prev) {
						prev_state.overlays.clear_namespace(&preview_ns, &mut prev_state.marker_list);
					}
				}
			}
		}
	...
	fn render_overlay_prompt(&mut self, frame: &mut Frame, area: ratatui::layout::Rect) {
		...
		let toolbar_rows: usize = self
			...
			.unwrap_or(0);
		let footer_visible =
			self.active_window().prompt.as_ref().map(|p| !p.footer.is_empty()).unwrap_or(false);
		...
		let default_title: Vec<dx_core::api::StyledText> = {
			...
			use dx_core::api::{OverlayColorSpec, OverlayOptions, StyledText};
			let keybindings = self.keybindings.read().unwrap();
			...
			if hints.is_empty() {
				...
				for (i, (k, verb)) in hints.into_iter().enumerate() {
					if i > 0 {
						segs.push(StyledText { text: " · ".into(), style: sep_style.clone() });
					}
					segs.push(StyledText { text: k, style: hint_style.clone() });
					segs.push(StyledText { text: format!(" {verb}"), style: None });
				}
				segs
			}
		};
		let title_segs: &[dx_core::api::StyledText] =
			if prompt.title.is_empty() { &default_title } else { &prompt.title };
		let normal_title_style = Style::default().fg(theme.prompt_fg).add_modifier(Modifier::BOLD);
		...
		let (results_area, preview_area) = if show_preview {
			...
			(
				Rect { x: body.x, y: body.y, width: results_w, height: body.height },
				Some(Rect {
					x: body.x + results_w,
					y: body.y,
		...
		let input_row = Rect { x: inner.x, y: inner.y, width: inner.width, height: 1 };
		...
		let dim = Style::default().fg(theme.popup_border_fg).bg(theme.editor_bg);
		...
		if let Some(out) = &toolbar_widget_out {
			...
			for hit in &out.hits {
				...
				self.active_chrome_mut().prompt_toolbar_hits.push((hit.widget_key.clone(), rect));
			}
		} else if draw && !prompt.title.is_empty() && inner.height >= 2 {
			let toolbar = Rect { x: inner.x, y: inner.y + 1, width: inner.width, height: 1 };
			frame.render_widget(
				Paragraph::new(Line::from(title_spans)).style(Style::default().bg(theme.suggestion_bg)),
				toolbar,
			);
		}
		...
		if draw && inner.height >= 2 + toolbar_h {
			let sep = Rect { x: inner.x, y: inner.y + 1 + toolbar_h, width: inner.width, height: 1 };
			let sep_style = Style::default().fg(theme.popup_border_fg).bg(theme.suggestion_bg);
		...
		if results_area.height >= 1 {
			...
			if needs_scrollbar {
				use crate::view::ui::scrollbar::{ScrollbarColors, ScrollbarState, render_scrollbar};
				...
				let state =
					ScrollbarState::new(prompt.suggestions.len(), inner_rows.max(1), prompt.scroll_offset);
				if draw {
					render_scrollbar(frame, scrollbar_rect, &state, &ScrollbarColors::from_theme(&theme));
				}
		...
		if draw && footer_h == 1 && inner.height >= 1 {
			...
			let footer_spans: Vec<Span> = prompt
				...
				.collect();
			frame.render_widget(
				Paragraph::new(Line::from(footer_spans)).style(Style::default().bg(theme.suggestion_bg)),
				footer_row,
			);
		}
		...
		if let Some(preview_rect) = preview_area {
					...
					.is_some_and(|sid| sid != self.active_window && self.windows.contains_key(&sid))
			{
				...
				let __win = self.windows.get_mut(&self.active_window).expect("active window present");
				...
				preview_state.view_state.viewport.resize(inner.width, inner.height);
		...
		match &self.active_window().mouse_state.hover_target {
					...
					if sid == split_id && dir == direction {
						...
						match dir {
							SplitDirection::Horizontal => {
								...
								frame.render_widget(paragraph, ratatui::layout::Rect::new(*x, *y, *length, 1));
							}
							SplitDirection::Vertical => {
								for offset in 0..*length {
									let paragraph = Paragraph::new(Span::styled("│", hover_style));
									frame.render_widget(paragraph, ratatui::layout::Rect::new(*x, y + offset, 1, 1));
								}
							}
						}
					}
			...
			Some(HoverTarget::ScrollbarThumb(split_id)) => {
					...
					&self.active_layout().split_areas
				{
					if sid == split_id {
						let hover_style =
							Style::default().bg(self.theme.read().unwrap().scrollbar_thumb_hover_fg);
					...
					&self.active_layout().split_areas
				{
					if sid == split_id {
						let track_hover_style =
							Style::default().bg(self.theme.read().unwrap().scrollbar_track_hover_fg);
						let paragraph = Paragraph::new(Span::styled(" ", track_hover_style));
						frame.render_widget(
							paragraph,
							ratatui::layout::Rect::new(scrollbar_rect.x, scrollbar_rect.y + hovered_row, 1, 1),
						);
					}
				}
			}
			Some(HoverTarget::FileExplorerBorder) => {
				...
				if let Some(explorer_area) = self.active_layout().file_explorer_area {
					...
					for row_offset in 0..explorer_area.height {
						...
						frame.render_widget(
							paragraph,
							ratatui::layout::Rect::new(border_x, explorer_area.y + row_offset, 1, 1),
						);
					}
				}
			}
	...
	fn render_tab_drop_zone(&self, frame: &mut Frame, drag_state: &super::types::TabDragState) {
		...
		let highlight_area = match drop_zone {
			...
			TabDropZone::SplitLeft(_) => {
				...
				ratatui::layout::Rect::new(content_rect.x, content_rect.y, width, content_rect.height)
			}
			...
			TabDropZone::SplitTop(_) => {
				...
				ratatui::layout::Rect::new(content_rect.x, content_rect.y, content_rect.width, height)
			}
	...
	pub fn recompute_layout(&mut self, width: u16, height: u16) {
		...
		let constraints = vec![
			Constraint::Length(if self.active_window_mut().menu_bar_visible { 1 } else { 0 }),
			Constraint::Min(0),
			Constraint::Length(if self.active_window_mut().status_bar_visible { 1 } else { 0 }), // status bar
			Constraint::Length(0), // search options (doesn't matter for layout)
			Constraint::Length(if self.active_window_mut().prompt_line_visible { 1 } else { 0 }), // prompt line
		];
		let main_chunks =
			Layout::default().direction(Direction::Vertical).constraints(constraints).split(size);
		let main_content_area = main_chunks[1];
		...
		let active_window_id = self.active_window;
		let __win_l = self.windows.get_mut(&active_window_id).expect("active window must exist");
	...
	fn update_terminal_title(&mut self, display_name: &str) {
		...
		let new_title = crate::services::terminal_title::build_window_title(display_name, project_name);
	...
	pub fn save_histories(&self) {
		...
		if let Err(e) = self.authority().filesystem.create_dir_all(&self.dir_context.data_dir) {
	...
	pub(super) fn compute_dock_split(
		...
		let dock = ratatui::layout::Rect { x: size.x, y: size.y, width, height: size.height };
	...
	pub(super) fn render_floating_widget_panel(
		...
		let overlay_rect = match placement {
			...
			super::PanelPlacement::Anchored { x, y } => {
				...
				use crate::primitives::display_width::str_width;
				let content_w = entries.iter().map(|e| str_width(&e.text) as u16).max().unwrap_or(0);
		...
		let dock_border_fg =
			if is_dock && panel_focused { theme.cursor } else { theme.popup_border_fg };
		let block = Block::default()
			.borders(if is_dock { Borders::RIGHT } else { Borders::ALL })
		...
		for (i, entry) in entries.iter().take(max_rows).enumerate() {
			let recorder = is_dock
				.then(|| (&mut self.active_chrome_mut().cell_theme_map, dock_sw, "Orchestrator Dock"));
		...
		for emb in &embeds {
			...
			let rect = ratatui::layout::Rect { x: ex, y: ey, width: w, height: h };
			self.preview_window_id = Some(dx_core::WindowId(emb.window_id as u64));
		...
		let mut scrollbar_hover_zones: Vec<ratatui::layout::Rect> = Vec::new();
		{
			...
			for region in &scroll_regions {
				...
				if dock_overlay_scrollbar {
					sb_x = sb_x.saturating_add(1).min(inner.x + inner.width.saturating_sub(1));
				}
				...
				let sb_rect = ratatui::layout::Rect { x: sb_x, y: sb_y, width: 1, height: sb_h };
				...
				let zone = ratatui::layout::Rect { x: inner.x, y: sb_y, width: inner.width, height: sb_h };
		...
		for o in &overlays {
			...
			let row_rect = ratatui::layout::Rect { x: inner.x, y: row_y, width: inner.width, height: 1 };
			frame.render_widget(Clear, row_rect);
			frame.render_widget(Block::default().style(panel_bg_style), row_rect);
			let recorder = is_dock
				.then(|| (&mut self.active_chrome_mut().cell_theme_map, overlay_sw, "Orchestrator Dock"));
			paint_text_property_entry(frame, &o.entry, inner.x, row_y, inner.width, &theme, recorder);
		}

		if let Some(fc) = focus_cursor {
			let cx = inner.x.saturating_add(byte_to_screen_col(
				entries.get(fc.buffer_row as usize).map(|e| e.text.as_str()).unwrap_or(""),
	...
	fn resolve_overlay_style(
		opts: &dx_core::api::OverlayOptions,
		theme: &crate::view::theme::Theme,
	) -> ratatui::style::Style {
...
fn paint_dock_seamless_active_tab(
	...
	inner: ratatui::layout::Rect,
	entries: &[dx_core::text_property::TextPropertyEntry],
	max_rows: usize,
...
pub(crate) fn paint_text_property_entry(
	frame: &mut ratatui::Frame,
	entry: &dx_core::text_property::TextPropertyEntry,
	x: u16,
	y: u16,
	width: u16,
	theme: &crate::view::theme::Theme,
	...
	mut recorder: Option<(&mut Vec<crate::app::types::CellThemeInfo>, u16, &'static str)>,
) {
	use dx_core::api::OverlayColorSpec;
	use ratatui::style::Style;
	use ratatui::text::{Line, Span};
	...
	let (mut base_fg_key, mut base_bg_key) =
		(Some(Cow::Borrowed("ui.suggestion_fg")), Some(Cow::Borrowed("ui.suggestion_bg")));
	...
	let base_style = if let Some(opts) = normalized.style.as_ref() {
		...
		if resolved.bg.is_none() { resolved.bg(base_bg) } else { resolved }
	} else {
	...
	let boundaries: std::collections::BTreeSet<usize> = std::iter::once(0)
		...
		.chain(normalized.inline_overlays.iter().flat_map(|o| [snap(o.start), snap(o.end)]))
		.collect();
	...
	for win in bounds.windows(2) {
		...
		if let Some((map, sw, region)) = recorder.as_mut() {
			record_entry_span_cells(map, *sw, region, y, col_cursor, span_w, x, width, &fg_key, &bg_key);
		}
	...
	let rect = ratatui::layout::Rect { x, y, width, height: 1 };
