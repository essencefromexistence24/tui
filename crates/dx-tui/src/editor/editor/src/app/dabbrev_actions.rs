...
impl Editor {
	...
	fn dabbrev_cycle(&mut self) {
		...
		if cursor_pos == prev_end && prev_end <= self.active_state().buffer.len() {
			...
			let delete_event = Event::Delete { range: word_start..prev_end, deleted_text, cursor_id };
		...
		if state.index >= state.candidates.len() {
			...
			let insert_event =
				Event::Insert { position: word_start, text: state.original_prefix.clone(), cursor_id };
			...
			let insert_event = Event::Insert { position: word_start, text: next, cursor_id };
			self.log_and_apply_event(&insert_event);
	...
	fn dabbrev_expand_first(&mut self) {
		...
		let prefix = self.active_state_mut().get_text_range(word_start, cursor_pos);
		if prefix.is_empty() {
			return;
		}
		...
		let delete_event = Event::Delete { range: word_start..cursor_pos, deleted_text, cursor_id };
		self.log_and_apply_event(&delete_event);

		let first = candidates[0].clone();
		let insert_event = Event::Insert { position: word_start, text: first, cursor_id };
		self.log_and_apply_event(&insert_event);

		self.active_window_mut().dabbrev_state =
			Some(DabbrevCycleState { original_prefix: prefix, word_start, candidates, index: 0 });
	}
	...
	pub(crate) fn collect_other_buffer_slices(
			...
			self.windows.get(&self.active_window).map(|w| &w.buffers).expect("active window present")
		{
			...
			slices.push(OtherBufferSlice { buffer_id: buf_id.0 as u64, bytes, label });
		}
	...
	pub(crate) fn get_buffer_completion_popup_items(
		...
		let candidates = self.active_window_mut().completion_service.request(&ctx, &buffer_window);
