...
impl crate::app::window::Window {
	...
	pub(crate) fn resolve_page_view_for_buffer(&self, buffer_id: BufferId) -> Option<Option<usize>> {
	...
	pub(crate) fn preferred_split_for_file(&self) -> LeafId {
		let (mgr, _) = self.buffers.splits().expect("active window must have a populated split layout");
...
impl Editor {
	...
	pub fn open_file_preview(&mut self, path: &Path) -> anyhow::Result<BufferId> {
		...
		let previously_file_backed: HashSet<BufferId> = self
			...
			.iter()
			.filter_map(|(id, state)| {
				state.buffer.file_path().and_then(
					|p| {
						if p.as_os_str().is_empty() { None } else { Some(*id) }
					},
				)
			})
			.collect();
	...
	pub(crate) fn relocate_buffers_for_rename(
		...
		for &id in &affected {
			...
			let file_uri =
				super::types::LspUri::from_host_path(&new_path, self.authority().path_translation.as_ref());
			let display_name =
			...
			if let Some(metadata) = self.active_window_mut().buffer_metadata.get_mut(&id) {
				metadata.kind = super::BufferKind::File { path: new_path.clone(), uri: file_uri };
	...
	pub(crate) fn active_buffer_has_line_index(&self) -> bool {
		self.buffers().get(&self.active_buffer()).is_none_or(|s| s.buffer.line_count().is_some())
	}
	...
	pub fn goto_line_col(&mut self, line: usize, column: Option<usize>) {
			...
			.get(&buffer_id)
		{
			...
			let new_anchor = if old_cursor.deselect_on_move { None } else { old_cursor.anchor };
	...
	pub fn select_range(
			...
			.get(&buffer_id)
		{
			...
			let start_pos =
				state.buffer.line_col_to_position(start_line_0.min(max_line), start_col_0).min(buffer_len);
			let end_pos =
				state.buffer.line_col_to_position(end_line_0.min(max_line), end_col_0).min(buffer_len);
	...
	pub fn new_buffer(&mut self) -> BufferId {
		...
		state.buffer.set_default_line_ending(self.config.editor.default_line_ending.to_line_ending());
	...
	pub fn get_mouse_hover_state(&self) -> Option<(usize, u16, u16)> {
		self.active_window().mouse_state.lsp_hover_state.map(|(pos, _, x, y)| (pos, x, y))
	}

	/// Check if a transient popup (hover/signature help) is currently visible
	pub fn has_transient_popup(&self) -> bool {
		self.active_state().popups.top().is_some_and(|p| p.transient)
	}
