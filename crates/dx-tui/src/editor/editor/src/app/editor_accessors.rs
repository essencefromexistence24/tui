...
impl Editor {
	...
	pub(super) fn send_plugin_response(&self, response: dx_core::api::PluginResponse) {
	...
	pub fn keybinding_event_for_action(
		...
		self.keybindings.read().unwrap().get_keybinding_event_for_action(action, context)
	}
	...
	pub fn active_buffer_mode(&self) -> Option<&str> {
		...
		if let Some(mode) = win.buffer_metadata.get(&buffer_id).and_then(|meta| meta.virtual_mode()) {
			return Some(mode);
		}
	...
	pub fn effective_mode(&self) -> Option<&str> {
		...
		self.active_buffer_mode().or(self.active_window().editor_mode.as_deref())
	}
	...
	pub fn next_periodic_redraw_deadline(&self) -> Option<std::time::Instant> {
		...
		[lsp_progress_deadline, anim_deadline, paste_deadline].into_iter().flatten().min()
	}
	...
	pub fn terminal_title_poll_deadline(&self) -> Option<std::time::Instant> {
		...
		for window in self.windows.values() {
			let has_auto =
				window.terminal_buffers.keys().any(|b| !window.terminal_explicit_titles.contains(b));
			if !has_auto {
				continue;
			}
	...
	pub fn terminal_titles_need_poll(&self) -> bool {
		self.terminal_title_poll_deadline().is_some_and(|d| d <= std::time::Instant::now())
	}
	...
	pub fn is_update_available(&self) -> bool {
		self.update_checker.as_ref().map(|c| c.is_update_available()).unwrap_or(false)
	}

	/// Get the latest version string if an update is available
	pub fn latest_version(&self) -> Option<&str> {
		self.update_checker.as_ref().and_then(|c| c.latest_version())
	}

	/// Get the cached release check result (for shutdown notification)
	pub fn get_update_result(&self) -> Option<&crate::services::release_checker::ReleaseCheckResult> {
		self.update_checker.as_ref().and_then(|c| c.get_cached_result())
	}
	...
	pub fn set_boot_authority(&mut self, authority: crate::services::authority::Authority) {
		...
		let bg_roots: Vec<(dx_core::WindowId, std::path::PathBuf)> = self
			.windows
			.iter()
		...
		let mut installs: Vec<(dx_core::WindowId, crate::services::authority::Authority)> = bg_roots
			.into_iter()
			.map(|(id, root)| {
				(id, crate::services::authority::Authority::local_scoped(self.session_scope_for(&root)))
			})
			.collect();
		...
		for (id, a) in installs {
			if let Some(w) = self.windows.get_mut(&id) {
				w.lsp.set_long_running_spawner(a.long_running_spawner.clone());
	...
	pub fn active_window_id(&self) -> dx_core::WindowId {
		self.active_window
	}
	...
	pub(crate) fn adopt_active_window_authority(&mut self, previous_label: &str) {
		...
		let (label_changed, fs, sp) = {
			let a = &self.active_window().authority;
			(a.display_label != previous_label, a.filesystem.clone(), a.process_spawner.clone())
		};
		self.quick_open_registry.set_file_backends(fs, sp);
		if label_changed {
			#[cfg(feature = "plugins")]
	...
	pub fn active_window(&self) -> &crate::app::window::Window {
		self.windows.get(&self.active_window).expect("active_window id must be a member of sessions")
	}
	...
	pub fn active_session_id(&self) -> dx_core::WindowId {
		self.active_window
	}
	...
	pub(crate) fn alloc_buffer_id(&mut self) -> dx_core::BufferId {
	...
	pub fn session(&self, id: dx_core::WindowId) -> Option<&crate::app::window::Window> {
		self.windows.get(&id)
	}
	...
	#[doc(hidden)]
	pub fn dormant_remote_sessions_for_test(&self) -> Vec<(dx_core::WindowId, String)> {
		self.dormant_remote.iter().map(|(id, d)| (*id, d.label.clone())).collect()
	}
	...
	#[doc(hidden)]
	pub fn panel_ids_for_test(&self) -> &std::collections::HashMap<String, dx_core::BufferId> {
		self.panel_ids()
	}
	...
	#[doc(hidden)]
	pub fn insert_panel_id_for_test(&mut self, key: String, buffer_id: dx_core::BufferId) {
	...
	pub fn active_window_mut(&mut self) -> &mut crate::app::window::Window {
		...
		self.windows.get_mut(&id).expect("active_window id must be a member of sessions")
	}
	...
	pub(crate) fn slot_of_panel(
		...
		if self.floating_widget_panel.as_ref().is_some_and(|f| &f.panel_key == panel_key) {
			Some(crate::app::PanelSlot::Floating)
		} else if self.dock.as_ref().is_some_and(|f| &f.panel_key == panel_key) {
	...
	pub(crate) fn buffer_display_name(&self, id: dx_core::BufferId) -> Option<String> {
		self.active_window().buffer_metadata.get(&id).map(|m| m.display_name.clone())
	}

	/// Whether a buffer has unsaved changes (for the tab's modified dot).
	pub(crate) fn buffer_is_modified(&self, id: dx_core::BufferId) -> bool {
		self.buffers().get(&id).map(|s| s.buffer.is_modified()).unwrap_or(false)
	}
	...
	pub(crate) fn split_view_states_mut(
		&mut self,
	) -> &mut std::collections::HashMap<crate::model::event::LeafId, crate::view::split::SplitViewState>
	{
		&mut self
			.active_window_mut()
			.buffers
	...
	pub fn buffer_ids_under_path(&self, root: &std::path::Path) -> Vec<BufferId> {
		self
			...
			.filter_map(|(id, state)| {
				...
				if p == root || p.starts_with(root) { Some(*id) } else { None }
			})
			.collect()
	}
	...
	pub fn check_warning_log(&mut self) -> bool {
		...
		self.active_window_mut().warning_domains.general.add_warnings(new_warning_count);
		self.active_window_mut().warning_domains.general.set_log_path(path);
	...
	pub fn open_warning_log(&mut self) {
		if let Some(path) = self.active_window_mut().warning_domains.general.log_path.clone() {
			// Use open_local_file since log files are always local
			match self.active_window_mut().open_local_file(&path) {
				Ok(buffer_id) => {
					self.active_window_mut().mark_buffer_read_only(buffer_id, true);
	...
	pub fn check_mouse_hover_timer(&mut self) -> bool {
		...
		self.active_window_mut().hover.set_screen_position((screen_x, screen_y));
