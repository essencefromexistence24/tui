...
fn get_parent_dir_path(node: &TreeNode) -> PathBuf {
	if node.is_dir() {
		...
		node.entry.path.parent().map(|p| p.to_path_buf()).unwrap_or_else(|| node.entry.path.clone())
	}
}
...
fn get_parent_node_id(
	...
	if node_is_dir {
		...
		tree.get_node(selected_id).and_then(|n| n.parent).unwrap_or(selected_id)
	}
}
...
impl Editor {
	...
	fn file_explorer_preview_selected(&mut self) {
		...
		let path =
			match self.file_explorer().as_ref().and_then(|explorer| explorer.get_selected_entry()) {
				Some(entry) if !entry.is_dir() => entry.path.clone(),
				_ => return,
			};

		if let Err(e) = self.open_file_preview(&path) {
			tracing::debug!("file_explorer_preview_selected: skipping preview for {:?}: {}", path, e);
		}
	}
	...
	pub fn file_explorer_toggle_expand(&mut self) {
		...
		if let (Some(runtime), Some(explorer)) = (
			...
			self.windows.get_mut(&active_id).and_then(|w| w.file_explorer.as_mut()),
		) {
			let result = runtime.block_on(explorer.toggle_with_chain(selected_id));

			let final_name = explorer.tree().get_node(selected_id).map(|n| n.entry.name.clone());
			let final_expanded =
				explorer.tree().get_node(selected_id).map(|n| n.is_expanded()).unwrap_or(false);
				...
				Ok(()) => {
					if final_expanded {
						...
						if let Some((dir_path, is_symlink)) = node_info {
							crate::app::file_operations::load_gitignore_via_fs(fs.as_ref(), explorer, &dir_path);
	...
	pub fn file_explorer_refresh(&mut self) {
		let (selected_id, node_name) = if let Some(explorer) = self.file_explorer() {
			if let Some(selected_id) = explorer.get_selected() {
				let node_name = explorer.tree().get_node(selected_id).map(|n| n.entry.name.clone());
		...
		if let (Some(runtime), Some(explorer)) = (
			...
			match result {
				...
				Err(e) => {
					self
						.set_status_message(t!("explorer.error_refreshing", error = e.to_string()).to_string());
				}
			}
		}
	}
	...
	pub fn file_explorer_new_file(&mut self) {
		...
		let fs = std::sync::Arc::clone(&self.authority().filesystem);
		if let Some(explorer) = self.windows.get_mut(&active_id).and_then(|w| w.file_explorer.as_mut())
		{
			if let Some(selected_id) = explorer.get_selected() {
				let node = explorer.tree().get_node(selected_id);
				if let Some(node) = node {
					...
					if let Some(runtime) = &self.tokio_runtime {
						...
						match result {
							Ok(_) => {
								let parent_id = get_parent_node_id(explorer.tree(), selected_id, node.is_dir());
								let tree = explorer.tree_mut();
	...
	pub fn file_explorer_delete(&mut self) {
		...
		let paths: Vec<(PathBuf, bool)> = selected_ids
			...
			.filter_map(|&id| explorer.tree().get_node(id).map(|n| (n.entry.path.clone(), n.is_dir())))
			.collect();
		...
		if paths.len() == 1 {
			...
			self.start_prompt(
				t!("explorer.delete_multi_confirm", count = count, names = &names).to_string(),
	...
	/// For remote files: moves to ~/.local/share/dx/trash/ on remote
			...
			Ok(_) => {
				...
				for id in to_close {
					if let Err(e) = self.force_close_buffer(id) {
						tracing::warn!("Failed to close buffer {:?} after delete of {:?}: {}", id, path, e);
					}
				}
					...
					self.windows.get_mut(&active_id).and_then(|w| w.file_explorer.as_mut())
				{
					if let Some(runtime) = &self.tokio_runtime {
						...
						if let Some(node) = explorer.tree().get_node_by_path(&path) {
							...
							if let Err(e) = runtime.block_on(explorer.tree_mut().reload_expanded_node(parent_id))
							{
							...
							if count > 0 {
								let new_index =
									if let Some(idx) = deleted_index { idx.min(count.saturating_sub(1)) } else { 0 };
	...
	/// Move a file/directory to the remote trash directory (~/.local/share/dx/trash/)
	fn move_to_remote_trash(&self, path: &std::path::Path) -> std::io::Result<()> {
		// Get remote home directory
		let home = self.authority().filesystem.home_dir()?;
		let trash_dir = home.join(".local/share/dx/trash");
	...
	pub fn perform_file_explorer_rename(
		...
		let new_path =
			original_path.parent().map(|p| p.join(&new_name)).unwrap_or_else(|| original_path.clone());
	...
	pub fn file_explorer_toggle_hidden(&mut self) {
		...
		let msg =
			if show_hidden { t!("explorer.showing_hidden") } else { t!("explorer.hiding_hidden") };
		...
		self.persist_config_change("/file_explorer/show_hidden", serde_json::Value::Bool(show_hidden));
	}
	...
	pub(super) fn execute_resolved_multi_paste(
		...
		for (src, dst) in safe.into_iter().chain(to_overwrite) {
			match self.paste_one_fs_op(&src, &dst, is_cut) {
				...
				PasteOpOutcome::SourceRemovalFailed { dst: landed_dst, err } => {
		...
		if !partial_moves.is_empty() {
			...
			let name = first_dst.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_default();
			let msg = if partial_moves.len() == 1 {
				t!("explorer.move_source_removal_failed", name = &name, error = first_err.to_string())
					.to_string()
			} else {
				t!("explorer.move_source_removal_failed_n", count = partial_moves.len()).to_string()
			};
			self.set_status_message(msg);
					...
					match copy_result {
						Ok(()) => {
							...
							match remove_result {
								Ok(()) => PasteOpOutcome::Ok,
								Err(remove_err) => {
									PasteOpOutcome::SourceRemovalFailed { dst: dst.to_path_buf(), err: remove_err }
								}
							}
						}
						Err(copy_err) => {
	...
	fn refresh_tree_after_paste(&mut self, src: &Path, dst: &Path, is_cut: bool) {
		...
		// `self.tokio_runtime`.
		let Some(explorer) = self.windows.get_mut(&active_id).and_then(|w| w.file_explorer.as_mut())
		else {
			return;
		};
		if let Some(runtime) = &self.tokio_runtime {
	...
	pub fn perform_file_explorer_paste(&mut self, src: PathBuf, dst: PathBuf, is_cut: bool) {
		...
		match self.paste_one_fs_op(&src, &dst, is_cut) {
			...
			PasteOpOutcome::SourceRemovalFailed { dst: landed_dst, err } => {
				...
				self.set_status_message(
					t!("explorer.move_source_removal_failed", name = &name, error = err.to_string())
						.to_string(),
				);
	...
	pub fn file_explorer_duplicate(&mut self) {
		...
		for src in &sources {
			...
			let dst =
				unique_paste_name(&*self.authority().filesystem, parent, &file_name.to_string_lossy());
		...
		let msg = if let Some(e) = &first_error {
			...
			let name =
				succeeded[0].1.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_default();
	...
	pub fn file_explorer_copy_path(&mut self, relative: bool) {
		...
		let rendered: Vec<String> = paths
			...
			.map(|p| {
				if relative {
					p.strip_prefix(&working_dir).unwrap_or(p).to_string_lossy().into_owned()
				} else {
...
impl crate::app::window::Window {
	...
	pub(crate) fn init_file_explorer(&mut self) {
		...
		let root_exists = self.authority().filesystem.is_dir(&self.root).unwrap_or(false);
		...
		runtime.spawn(async move {
			match FileTree::new(root_path, fs_manager).await {
				Ok(mut tree) => {
					...
					let _ = sender.send(AsyncMessage::FileExplorerInitialized { window: window_id, view });
				}
				Err(e) => {
	...
	pub(crate) fn install_initialized_file_explorer(
		...
		let show_hidden = self.pending_file_explorer_show_hidden.take().unwrap_or(defaults.show_hidden);
		view.ignore_patterns_mut().set_show_hidden(show_hidden);
		let show_gitignored =
			self.pending_file_explorer_show_gitignored.take().unwrap_or(defaults.show_gitignored);
	...
	pub fn file_explorer_search_clear(&mut self) {
		if matches!(self.file_explorer_clipboard, Some(FileExplorerClipboard { is_cut: true, .. })) {
	...
	pub fn handle_set_file_explorer_decorations(
		...
		let normalized: Vec<crate::view::file_tree::FileExplorerDecoration> = decorations
			...
			.filter_map(|mut decoration| {
				let path =
					if decoration.path.is_absolute() { decoration.path } else { root.join(&decoration.path) };
	...
	pub fn handle_set_file_explorer_slots(
		&mut self,
		namespace: String,
		slots: Vec<dx_core::file_explorer::FileExplorerSlotEntry>,
	) {
		let root = self.root.clone();
		let normalized: Vec<dx_core::file_explorer::FileExplorerSlotEntry> = slots
			.into_iter()
			.filter_map(|mut slot| {
				let path = if slot.path.is_absolute() { slot.path } else { root.join(&slot.path) };
				let path = crate::app::normalize_path(&path);
				if crate::app::explorer_path_under_root(&path, &root) {
		...
		self.file_explorer_slot_overrides.insert(namespace, normalized);
	...
	pub fn rebuild_file_explorer_decoration_cache(&mut self) {
		...
		let symlink_mappings =
			self.file_explorer.as_ref().map(|fe| fe.collect_symlink_mappings()).unwrap_or_default();
	...
	pub fn sync_file_explorer_to_active_file(&mut self) {
		...
		let runtime_handle = self.resources.tokio_runtime.clone();
		...
		if let (Some(runtime), Some(sender)) = (runtime_handle, sender) {
			...
			runtime.spawn(async move {
				...
				// Receiver may have been dropped during shutdown.
				#[allow(clippy::let_underscore_must_use)]
				let _ = sender.send(crate::services::async_bridge::AsyncMessage::FileExplorerExpandedToPath {
					window: window_id,
					view,
				});
			});
...
pub(super) fn format_path_preview_for_prompt(paths: &[PathBuf], max_shown: usize) -> String {
	let names: Vec<String> = paths
		.iter()
		.map(|p| {
			let raw = p.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_default();
