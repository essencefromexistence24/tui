	...
	pub(super) fn collect_lsp_changes(&self, event: &Event) -> Vec<TextDocumentContentChangeEvent> {
		match event {
			Event::Insert { position, text, .. } => {
				tracing::trace!("collect_lsp_changes: processing Insert at position {}", position);
			...
			Event::Delete { range, .. } => {
				...
				let (start_line, start_char) =
					self.active_state().buffer.position_to_lsp_position(range.start);
			...
			Event::Batch { events, .. } => {
				...
				tracing::trace!("collect_lsp_changes: processing Batch with {} events", events.len());
	...
	pub(super) fn calculate_event_line_info(&self, event: &Event) -> super::types::EventLineInfo {
		match event {
			Event::Insert { position, text, .. } => {
				...
				super::types::EventLineInfo { start_line, end_line, line_delta: lines_added as i32 }
			}
			Event::Delete { range, deleted_text, .. } => {
				...
				super::types::EventLineInfo { start_line, end_line, line_delta: -(lines_removed as i32) }
			}
			Event::Batch { events, .. } => {
...
impl crate::app::window::Window {
	...
	pub(super) fn notify_lsp_save_buffer(&mut self, buffer_id: BufferId) {
		...
		let metadata = match self.buffer_metadata.get(&buffer_id) {
			Some(m) => m,
			None => {
				tracing::debug!("notify_lsp_save_buffer: no metadata for buffer {:?}", buffer_id);
				return;
			}
		};

		if !metadata.lsp_enabled {
			tracing::debug!("notify_lsp_save_buffer: LSP disabled for buffer {:?}", buffer_id);
			return;
		}
		...
		// Only send didSave if LSP is already running (respect auto_start setting)
		{
			...
			if lsp.try_spawn(&language, file_path.as_deref()) != LspSpawnResult::Spawned {
				tracing::debug!("notify_lsp_save: LSP not running for {} (auto_start disabled)", language);
				return;
			}
			// Broadcast didSave to all handles for this language
			let mut any_sent = false;
			for sh in lsp.get_handles_mut(&language) {
				if let Err(e) = sh.handle.did_save(uri.as_uri().clone(), Some(full_text.clone())) {
