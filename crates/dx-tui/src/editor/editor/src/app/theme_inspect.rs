...
impl Editor {
	...
	pub(super) fn show_theme_info_popup(&mut self, col: u16, row: u16) -> AnyhowResult<()> {
		if let Some(info) = self.resolve_theme_key_at(col, row) {
			...
			self.active_window_mut().theme_info_popup =
				Some(ThemeInfoPopup { position: (popup_x, popup_y), info, button_highlighted: false });
		}
		Ok(())
	}
	...
	pub(super) fn fire_theme_inspect_hook(&mut self, key: String) {
		...
		self
			...
			.read()
			.unwrap()
			.run_hook("theme_inspect_key", HookArgs::ThemeInspectKey { theme_name, key });
	}
	...
	fn resolve_theme_key_at(&self, col: u16, row: u16) -> Option<ThemeKeyInfo> {
		...
		let fg_color = cell.fg_key.as_ref().and_then(|k| theme.resolve_theme_key(k));
		let bg_color = cell.bg_key.as_ref().and_then(|k| theme.resolve_theme_key(k));
	...
	pub(super) fn render_theme_info_popup(&self, frame: &mut Frame) {
		...
		let key_style =
			Style::default().fg(theme.popup_text_fg).add_modifier(ratatui::style::Modifier::BOLD);
		...
		lines.push(Line::from(Span::styled(" \u{25b6} Open in Theme Editor ", button_style)));
...
fn compute_popup_rect(
	...
	let x = if position.0 + width > screen_w { screen_w.saturating_sub(width) } else { position.0 };
	let y =
		if position.1 + height > screen_h { position.1.saturating_sub(height + 1) } else { position.1 };
