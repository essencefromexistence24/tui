...
impl crate::app::window::Window {
	...
	pub(super) fn handle_mouse_scroll(&mut self, col: u16, row: u16, delta: i32) -> AnyhowResult<()> {
		...
		self.resources.plugin_manager.read().unwrap().run_hook(
			"mouse_scroll",
			dx_core::hooks::HookArgs::MouseScroll { buffer_id, delta, col, row },
		);
		...
		if self.is_composite_buffer(buffer_id) {
			let max_row = self
				...
				.unwrap_or(0);
			if let Some(view_state) = self.composite_view_states.get_mut(&(target_split, buffer_id)) {
		...
		self.scroll_split_by_lines(buffer_id, target_split, delta, view_transform_tokens, tab_size);
	...
	pub(super) fn handle_horizontal_scroll(
		...
		if self.is_composite_buffer(buffer_id) {
			...
			if let Some(vs) = self.composite_view_states.get_mut(&(target_split, buffer_id)) {
			...
			if buffer_len <= large_file_threshold {
				...
				if line_wrap_enabled {
					...
					if max_scroll_line == 0 || scrollbar_height <= 1 {
						...
						let thumb_size = thumb_size_raw.max(1).min(max_thumb_size).min(scrollbar_height);
						...
						if max_thumb_start == 0 {
							...
							let target_scroll_ratio = ((target_thumb_row - scrollbar_rect.y as f64)
								/ max_thumb_start as f64)
								.clamp(0.0, 1.0);

							// Map scroll ratio to target line
							let target_line = (target_scroll_ratio * max_scroll_line as f64).round() as usize;
							let target_line = target_line.min(max_scroll_line);

							// Find byte position of target line
							let target_byte =
								state.buffer.line_start_offset(target_line).unwrap_or(drag_start_top_byte);
	...
	pub(super) fn handle_scrollbar_jump(
		...
		// Handle composite buffers - use row-based scrolling
		if self.is_composite_buffer(buffer_id) {
			return self.handle_composite_scrollbar_jump(ratio, split_id, buffer_id, scrollbar_rect);
		}
	...
	fn handle_composite_scrollbar_jump(
		...
		let total_rows = self.composite_buffers.get(&buffer_id).map(|c| c.row_count()).unwrap_or(0);
	...
	fn handle_composite_scrollbar_drag_relative(
		...
		let thumb_row_at_start = scrollbar_rect.y as f64 + start_scroll_ratio * max_thumb_start as f64;
