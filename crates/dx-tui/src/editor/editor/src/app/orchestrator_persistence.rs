...
//!
//! State lives under the platform data dir (`$XDG_DATA_HOME/dx/`),
...
fn is_local_authority_spec(spec: &crate::services::authority::SessionAuthoritySpec) -> bool {
	matches!(spec, crate::services::authority::SessionAuthoritySpec::Local)
}
...
pub(crate) fn read_persisted_windows_env(
	...
	Some(PersistedWindows { version: CURRENT_VERSION, active: 0, next_id, windows })
}
...
fn workspace_file_for(data_dir: &Path, root: &Path) -> PathBuf {
	let filename =
		format!("{}.json", crate::workspace::encode_path_for_filename(&canonical_key(root)));
...
fn discover_sessions(
	...
	tracing::debug!(count = entries.len(), "discover_sessions: read_dir returned");
	let mut found: Vec<(
		PathBuf,
		String,
		SessionState,
...
fn migrate_windows_json_into_workspaces(
	...
	for w in &env.windows {
		...
		if let Some(obj) = val.as_object_mut() {
			obj.entry("label").or_insert_with(|| serde_json::Value::String(w.label.clone()));
...
pub(crate) fn pick_active_window_for_cwd<'a>(
	...
	let env = env?;
	if let Some(w) = env.windows.iter().find(|w| w.id == env.active && window_matches_cwd(w, cwd)) {
		return Some(w);
	}
	env.windows.iter().filter(|w| window_matches_cwd(w, cwd)).max_by_key(|w| w.id)
}
...
fn migrate_legacy_windows(
	...
	for legacy_p in legacy_to_rename {
		...
		if let Err(e) = filesystem.rename(&legacy_p, &backup) {
			tracing::warn!("orchestrator migration: failed to rename {legacy_p:?} → {backup:?}: {e}");
		}
	}
	tracing::info!(
...
fn migrate_legacy_plugin_state(
	...
	tracing::info!("orchestrator persistence: migrated plugin state for {} plugins", merged.len());
}
	...
	pub fn save_orchestrator_state(&self) {
		...
		for (plugin, map) in &self.plugin_global_state {
			if !plugin_name_is_safe(plugin) {
				tracing::warn!("orchestrator persistence: skipping plugin with unsafe name: {plugin:?}");
				continue;
			}
			if map.is_empty() {
				continue;
			...
			match serde_json::to_vec_pretty(map) {
				Ok(bytes) => {
					...
					if let Err(e) = self.authority().filesystem.rename(&tmp, &path) {
						tracing::warn!("orchestrator persistence: failed to rename {tmp:?} → {path:?}: {e}");
					}
				}
				Err(e) => {
					tracing::warn!("orchestrator persistence: failed to serialise plugin {plugin}: {e}");
				}
			}
		}
	}
...
fn read_orch_session_meta(
	...
	let project_path =
		slot.and_then(|m| m.get("project_path")).and_then(|v| v.as_str()).map(PathBuf::from);
	let shared_worktree =
...
mod tests {
	...
	fn paths_live_under_data_dir_not_working_dir() {
		...
		let data_dir = Path::new("/tmp/dx-data");
		...
		assert!(wp.starts_with(data_dir), "windows_path must live under data_dir, got {wp:?}");
		assert!(sd.starts_with(data_dir), "state_dir must live under data_dir, got {sd:?}");
		assert!(psp.starts_with(data_dir), "plugin_state_path must live under data_dir, got {psp:?}");
		...
		for p in [&wp, &sd, &psp] {
			...
			for component in p.components() {
				if let std::path::Component::Normal(c) = component {
					assert_ne!(c, ".fresh", "orchestrator path must not contain a `.fresh` component: {p:?}");
				}
			}
		}
	}
	...
	fn pick_active_never_crosses_projects() {
		...
		let picked =
			pick_active_window_for_cwd(Some(&env), Path::new("/repoB")).expect("a /repoB session exists");
		assert_eq!(picked.id, 3, "must pick the /repoB session, not env.active=2");
	}
	...
	fn pick_active_reopens_last_used_for_cwd() {
		...
		let env = env_with(
			2,
			vec![make_window(2, "/repoA", Some("/repoA")), make_window(5, "/repoA", Some("/repoA"))],
		);
		let picked =
			pick_active_window_for_cwd(Some(&env), Path::new("/repoA")).expect("matching window");
		assert_eq!(picked.id, 2, "env.active is the last-used session for the cwd");
	}
	...
	fn pick_active_returns_none_when_no_window_matches_cwd() {
		...
		let env = env_with(
			1,
			vec![make_window(1, "/repoA", Some("/repoA")), make_window(2, "/repoB", Some("/repoB"))],
		);
	...
	fn pick_active_falls_back_to_root_when_project_path_missing() {
		...
		let env = env_with(2, vec![make_window(1, "/repoA", None), make_window(2, "/repoB", None)]);
		let picked =
			pick_active_window_for_cwd(Some(&env), Path::new("/repoA")).expect("matching window");
	...
	fn discover_reads_authority_spec_so_remote_sessions_arent_lost() {
		...
		let remote =
			sessions.iter().find(|s| s.label == "remote-session").expect("remote session discovered");
		assert_eq!(
			remote.authority_spec, spec,
			"the remote backend spec round-trips through discovery"
		);
		let local =
			sessions.iter().find(|s| s.label == "local-session").expect("local session discovered");
		assert_eq!(
	...
	fn migrate_folds_windows_json_into_workspace_files_and_retires_it() {
		...
		assert!(global_p.with_extension("json.retired.bak").exists(), "a .retired.bak is kept");
		let val: serde_json::Value = serde_json::from_slice(&std::fs::read(&ws_path).unwrap()).unwrap();
		assert_eq!(
