...
impl Editor {
	...
	pub(crate) fn sync_lsp_after_recovery_replay(&mut self, buffer_id: BufferId) {
		self.active_window_mut().sync_lsp_after_recovery_replay(buffer_id);
	}
	...
	pub fn end_recovery_session(&mut self) -> AnyhowResult<()> {
		...
		if hot_exit {
			...
			Ok(self.recovery_service.lock().unwrap().end_session_preserving(&preserve_ids)?)
		} else {
		...
		self
			...
			.filter_map(|(buffer_id, meta)| {
				...
				meta.recovery_id.clone().or_else(|| {
					...
					Some(self.recovery_service.lock().unwrap().get_buffer_id(file_path.as_deref()))
				})
			})
			.collect()
	}
	...
	pub fn has_recovery_files(&self) -> AnyhowResult<bool> {
		Ok(self.recovery_service.lock().unwrap().should_offer_recovery()?)
	}
	...
	pub fn recover_all_buffers(&mut self) -> AnyhowResult<usize> {
		...
		for entry in entries {
			let accepted = self.recovery_service.lock().unwrap().accept_recovery(&entry);
			match accepted {
				...
				Ok(RecoveryResult::RecoveredChunks { original_path, chunks }) => {
	...
	pub fn discard_all_recovery(&mut self) -> AnyhowResult<usize> {
		Ok(self.recovery_service.lock().unwrap().discard_all_recovery()?)
	}
			...
			match loaded {
				Ok(RecoveryResult::Recovered { original_path, content }) => {
					...
					if let Some(path) = original_path {
						match self.open_file(&path) {
							...
							Err(e) => {
									...
									e.downcast_ref::<crate::model::buffer::LargeFileEncodingConfirmation>()
								{
									...
									tracing::warn!("Hot-exit restore failed to open {}: {}", path.display(), e);
								}
							}
						}
					} else {
						...
						self.sync_lsp_after_recovery_replay(buffer_id);
						restored += 1;
						tracing::info!("Hot-exit restore: reopened unnamed buffer (recovery_id={})", entry.id);
					}
				}
				Ok(RecoveryResult::RecoveredChunks { original_path, chunks }) => {
					match self.open_file(&original_path) {
						...
						Err(e) => {
							tracing::warn!("Hot-exit restore failed to open {}: {}", original_path.display(), e);
						}
					}
				}
				Ok(RecoveryResult::OriginalFileModified { id, original_path }) => {
	...
	pub fn auto_recovery_save_dirty_buffers(&mut self) -> AnyhowResult<usize> {
		...
		let interval =
			std::time::Duration::from_secs(self.config.editor.auto_recovery_save_interval_secs as u64);
		if self.time_source.elapsed_since(self.active_window().last_auto_recovery_save) < interval {
			return Ok(0);
		}
	...
	fn save_pending_recovery_buffers(&mut self) -> AnyhowResult<usize> {
		...
		let buffers_needing_recovery: Vec<_> = self
			...
			.filter_map(|(buffer_id, state)| {
				...
				if state.buffer.is_recovery_pending() { Some(*buffer_id) } else { None }
			})
			.collect();
		...
		let buffer_info: Vec<_> = buffers_needing_recovery
			...
			.filter_map(|buffer_id| {
				...
				let recovery_id = if let Some(ref stored_id) = meta.recovery_id {
					...
					self.recovery_service.lock().unwrap().get_buffer_id(path.as_deref())
				};
	...
	pub fn delete_buffer_recovery(&mut self, buffer_id: BufferId) -> AnyhowResult<()> {
		...
		self.recovery_service.lock().unwrap().delete_buffer_recovery(&recovery_id)?;
	...
	fn save_buffer_to_recovery(
		...
		if state.buffer.is_large_file() {
			...
			let chunks = vec![crate::services::recovery::types::RecoveryChunk::new(0, 0, content)];
			self.recovery_service.lock().unwrap().save_buffer(
				recovery_id,
				chunks,
				path,
