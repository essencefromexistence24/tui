...
impl Editor {
	...
	pub fn add_overlay(
		...
		state.overlays.all().last().map(|o| o.handle.clone()).unwrap_or_default()
	}
	...
	pub(super) fn dismiss_transient_popups(&mut self) {
		...
		let is_transient_popup = self.active_state().popups.top().is_some_and(|p| p.transient);
	...
	pub(super) fn scroll_popup(&mut self, delta: i32) {
		...
		if let Some(popup) = self.active_state_mut().popups.top_mut() {
			popup.scroll_by(delta);
			tracing::debug!("Scrolled popup by {}, new offset: {}", delta, popup.scroll_offset);
		}
	}
	...
	pub fn show_lsp_confirmation_popup(&mut self, language: &str) {
		...
		self.show_popup_with_resolver(
			popup,
			crate::view::popup::PopupResolver::LspConfirm { language: language.to_string() },
		);
	}
	...
	pub fn handle_lsp_confirmation_response(&mut self, language: &str, action: &str) -> bool {
		...
		match action {
			"allow_once" => {
				...
				if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
					...
					if lsp.force_spawn(&language, file_path.as_deref()).is_some() {
						...
						self.set_status_message(t!("lsp.server_started", language = language).to_string());
					} else {
						self.set_status_message(t!("lsp.failed_to_start", language = language).to_string());
					}
				}
			...
			"allow_always" => {
				...
				if let Some(lsp) = self.windows.get_mut(&__active_id).map(|w| &mut w.lsp) {
					...
					if lsp.force_spawn(&language, file_path.as_deref()).is_some() {
						...
						self.set_status_message(t!("lsp.server_started_auto", language = language).to_string());
					} else {
						self.set_status_message(t!("lsp.failed_to_start", language = language).to_string());
					}
				}
			...
			_ => {
				...
				self.set_status_message(t!("lsp.startup_cancelled", language = language).to_string());
			}
		}
		...
		let __next_id = &mut __win.next_lsp_request_id;
		{
			...
			if lsp.force_spawn(language, file_path.as_deref()).is_some() {
				...
				for sh in lsp.get_handles_mut(language) {
					if let Err(e) =
						sh.handle.did_open(uri.as_uri().clone(), text.clone(), file_language.clone())
					{
				...
				if any_opened {
					...
					if let Some(handle) = lsp.get_handle_mut(language) {
						...
						{
							tracing::debug!("Failed to request pull diagnostics (server may not support): {}", e);
						}
						...
						if enable_inlay_hints {
							...
							{
								tracing::debug!("Failed to request inlay hints (server may not support): {}", e);
							} else {
								self.active_window_mut().pending_inlay_hints_requests.insert(
									request_id,
									super::InlayHintsRequest { buffer_id: active_buffer, version: buffer_version },
								);
							}
						}
					}
	...
	pub fn has_pending_lsp_confirmation(&self) -> bool {
		...
		self.global_popups.top().is_some_and(matches_lsp_confirm)
			|| self.active_state().popups.top().is_some_and(matches_lsp_confirm)
	}
