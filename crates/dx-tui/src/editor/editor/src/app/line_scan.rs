...
impl LineScan {
	...
	pub(crate) fn progress_percent(&self) -> usize {
		match &self.active {
			Some(a) => (a.scanned_bytes * 100).checked_div(a.total_bytes).unwrap_or(100),
			None => 100,
		}
	}
...
mod tests {
	...
	fn chunk(leaf_index: usize, byte_len: usize, already_known: bool) -> LineScanChunk {
		LineScanChunk { leaf_index, byte_len, already_known }
	}
	...
	fn start_then_take_walks_chunks_in_order() {
		let mut s =
			empty_scan_of(vec![chunk(0, 100, false), chunk(1, 200, false), chunk(2, 50, true)], 350);
