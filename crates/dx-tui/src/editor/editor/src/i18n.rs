//! Internationalization (i18n) support for Dx Editor
//!
	...
	let template: &String = match lang_map.and_then(|m| m.get(key)) {
		...
		None => {
			tracing::debug!(
				...
				plugin_map
					.get(&locale)
					.or_else(|| plugin_map.get("en"))
					.map(|m| m.keys().take(5).collect::<Vec<_>>())
			);
			return key.to_string();
		}
	};
...
///
/// This function also attempts to match region-specific locales supported by Dx,
...
fn detect_locale() -> Option<String> {
	...
	if lang.is_empty() || lang == "C" || lang == "POSIX" { None } else { Some(lang.to_lowercase()) }
}
...
mod tests {
	...
	fn test_all_locales_have_required_keys() {
		...
		let locales = available_locales();
		assert!(locales.len() >= 2, "Should have at least 2 locales (en and at least one other)");
	...
	fn t_returns_key_for_unknown_lookup() {
		assert_eq!(t("cli.does_not_exist.anywhere"), "cli.does_not_exist.anywhere");
	}
}
