...
impl DisplacedMarker {
	...
	pub fn decode(tagged_id: u64, position: usize) -> Self {
		if (tagged_id >> 63) == 1 {
			Self::Margin { id: tagged_id & !(1u64 << 63), position }
		} else {
			Self::Main { id: tagged_id, position }
		}
	}
}
...
impl EditorState {
	...
	pub fn from_file_with_languages(
		...
		let buffer = Buffer::load_from_file(path, large_file_threshold, fs)?;
		let first_line = buffer.first_line_lossy();
		let detected = DetectedLanguage::from_path(path, first_line.as_deref(), registry, languages);
		let mut state = Self::new_from_buffer(buffer);
		state.apply_language(detected);
		Ok(state)
	}
	...
	pub fn map_plugin_range(
		...
		Some((self.map_plugin_coord(start, epoch)?, self.map_plugin_coord(end, epoch)?))
	}
	...
	fn apply_insert(
		...
		self.coord_map.record_insert(self.buffer.version(), position, text.len());
		...
		if cursor_id == cursors.primary_id() {
			self.primary_cursor_line_number = match self.primary_cursor_line_number {
				...
				LineNumber::Relative { line, from_cached_line } => {
					LineNumber::Relative { line: line + newlines_inserted, from_cached_line }
				}
			};
		}
	}
	...
	fn apply_delete(
		...
		let primary_newlines_removed = if cursor_id == cursors.primary_id() {
			...
			let bytes_before_cursor =
				cursor_pos.saturating_sub(range.start).min(len).min(deleted_text.len());
		...
		self.virtual_texts.remove_in_range(&mut self.marker_list, range.start, range.end);
		...
		// Record the edit for forward coordinate mapping, keyed by the
		// post-edit version — same shift the marker tree just applied.
		self.coord_map.record_delete(self.buffer.version(), range.start, len);
	...
	pub fn apply(&mut self, cursors: &mut Cursors, event: &Event) {
		match event {
			...
			Event::Delete { range, cursor_id, deleted_text } => {
				self.apply_delete(cursors, range, *cursor_id, deleted_text)
			}

			Event::MoveCursor { cursor_id, new_position, new_anchor, new_sticky_column, .. } => {
				self.apply_move_cursor(cursors, *cursor_id, *new_position, *new_anchor, *new_sticky_column)
			}
			...
			Event::Scroll { .. } | Event::SetViewport { .. } | Event::Recenter => {
				...
				tracing::warn!(
					"View event {:?} reached EditorState.apply() - should be handled by SplitViewState",
					event
				);
			}
			...
			Event::AddOverlay { namespace, range, face, priority, message, extend_to_line_end, url } => {
			...
			Event::RemoveOverlay { handle } => {
				...
				self.overlays.remove_by_handle(handle, &mut self.marker_list);
			}
			...
			Event::ClearNamespace { namespace } => {
				...
				self.overlays.clear_namespace(namespace, &mut self.marker_list);
			}
			...
			Event::AddMarginAnnotation { line, position, content, annotation_id } => {
			...
			Event::BulkEdit { new_snapshot, new_cursors, edits, displaced_markers, .. } => {
	...
	fn apply_move_cursor(
		...
		if let Some(cursor) = cursors.get_mut(cursor_id) {
			...
			if let Some(idx) = self.pending_virtual_lines.iter().position(|(id, _)| *id == cursor_id) {
	...
	fn apply_add_margin_annotation(
		...
		let annotation = match annotation_id {
			Some(id) => MarginAnnotation::with_id(line, margin_position, margin_content, id.clone()),
	...
	fn apply_bulk_edit(
		...
		for ns in &namespaces {
			self
				.overlays
				.clear_namespace(&OverlayNamespace::from_string(ns.to_string()), &mut self.marker_list);
		}
	...
	pub(crate) fn replay_bulk_marker_adjustments(&mut self, edits: &[(usize, usize, usize)]) {
		...
		for &(pos, del_len, ins_len) in edits {
			...
			self.coord_map.record_replace(version, pos, del_len, ins_len);
	...
	pub fn capture_displaced_markers(&self, range: &Range<usize>) -> Vec<(u64, usize)> {
		...
		for (marker_id, start, _end) in self.marker_list.query_range(range.start, range.end) {
			if start > range.start && start < range.end {
				displaced.push(DisplacedMarker::Main { id: marker_id.0, position: start }.encode());
			}
		}
		for (marker_id, start, _end) in self.margins.query_indicator_range(range.start, range.end) {
			if start > range.start && start < range.end {
				displaced.push(DisplacedMarker::Margin { id: marker_id.0, position: start }.encode());
			}
		}
		displaced
	}
...
fn convert_event_face_to_overlay_face(event_face: &EventOverlayFace) -> OverlayFace {
	match event_face {
		...
		EventOverlayFace::Background { color } => {
			OverlayFace::Background { color: Color::Rgb(color.0, color.1, color.2) }
		}
		EventOverlayFace::Foreground { color } => {
			OverlayFace::Foreground { color: Color::Rgb(color.0, color.1, color.2) }
		}
...
impl EditorState {
	...
	pub fn collect_soft_break_positions(&self) -> Vec<(usize, u16)> {
		...
		self.soft_breaks.query_viewport(0, self.buffer.len() + 1, &self.marker_list)
	}
	...
	pub fn get_line_at_offset(&mut self, offset: usize) -> Option<(usize, String)> {
		...
		let viewport = self
			.get_viewport_content(crate::model::document_model::DocumentPosition::byte(line_start), 1)
			.ok()?;

		viewport.lines.first().map(|line| (line.byte_offset, line.content.clone()))
	}
	...
	pub fn get_text_to_end_of_line(&mut self, cursor_pos: usize) -> Result<String> {
		...
		let viewport = self
			.get_viewport_content(crate::model::document_model::DocumentPosition::byte(cursor_pos), 1)?;
	...
	pub fn semantic_tokens_result_id(&self) -> Option<&str> {
		self.semantic_tokens.as_ref().and_then(|store| store.result_id.as_deref())
	}
}
...
impl DocumentModel for EditorState {
	...
	fn offset_to_position(&self, offset: usize) -> DocumentPosition {
		if self.has_line_index() {
			if let Some(pos) = self.buffer.offset_to_position(offset) {
				DocumentPosition::LineColumn { line: pos.line, column: pos.column }
			} else {
	...
	fn get_range(&mut self, start: DocumentPosition, end: DocumentPosition) -> Result<String> {
		...
		let bytes = self.buffer.get_text_range_mut(start_offset, end_offset - start_offset)?;
	...
	fn get_line_content(&mut self, line_number: usize) -> Option<String> {
		...
		if let Some((_start, content)) = iter.next_line() {
			...
			let line_content =
				if has_newline { content[..content.len() - 1].to_string() } else { content };
			Some(line_content)
		} else {
			None
		}
...
mod tests {
	...
	fn test_apply_insert() {
		...
		state.apply(&mut cursors, &Event::Insert { position: 0, text: "hello".to_string(), cursor_id });
	...
	fn test_apply_delete() {
		...
		state.apply(
			&mut cursors,
			&Event::Insert { position: 0, text: "hello world".to_string(), cursor_id },
		);

		state.apply(
			&mut cursors,
			&Event::Delete { range: 5..11, deleted_text: " world".to_string(), cursor_id },
		);
	...
	fn test_edits_reset_goal_column() {
		...
		state.apply(&mut cursors, &Event::Insert { position: 5, text: "!".to_string(), cursor_id });
		assert_eq!(cursors.primary().sticky_column, None, "insert resets goal");

		cursors.primary_mut().sticky_column = Some(42);
		state.apply(
			&mut cursors,
			&Event::Delete { range: 5..6, deleted_text: "!".to_string(), cursor_id },
		);
	...
	fn coord_map_remaps_through_real_edits() {
		...
		state.apply(
			&mut cursors,
			&Event::Insert { position: 0, text: "line0\nTARGET\n".to_string(), cursor_id },
		);
		let v0 = state.buffer.version();
		let target_at_v0 = 6usize;

		// No edits since v0: identity, and a None epoch is always verbatim.
		assert_eq!(state.map_plugin_coord(target_at_v0, Some(v0)), Some(target_at_v0));
		assert_eq!(state.map_plugin_coord(target_at_v0, None), Some(target_at_v0));

		// Insert two lines above the target — it must shift right by 4 bytes.
		let above = "X\nY\n";
		state.apply(&mut cursors, &Event::Insert { position: 0, text: above.to_string(), cursor_id });
		...
		state.apply(
			&mut cursors,
			&Event::Delete { range: 0..above.len(), deleted_text: above.to_string(), cursor_id },
		);
		...
		assert_eq!(state.map_plugin_coord(target_at_v0 + above.len(), Some(v1)), Some(target_at_v0),);
	}
	...
	fn test_apply_add_cursor() {
		...
		state.apply(&mut cursors, &Event::AddCursor { cursor_id, position: 5, anchor: None });
	...
	fn test_apply_many() {
		...
		let events = vec![
			Event::Insert { position: 0, text: "hello ".to_string(), cursor_id },
			Event::Insert { position: 6, text: "world".to_string(), cursor_id },
		];
	...
	fn test_cursor_adjustment_after_insert() {
		...
		state
			.apply(&mut cursors, &Event::AddCursor { cursor_id: CursorId(1), position: 5, anchor: None });

		// Insert at position 0 - should push second cursor forward
		state.apply(&mut cursors, &Event::Insert { position: 0, text: "abc".to_string(), cursor_id });
	...
	mod document_model_tests {
		...
		fn test_get_viewport_content() {
			...
			let content = state.get_viewport_content(DocumentPosition::ByteOffset(0), 3).unwrap();
		...
		fn test_get_range() {
			...
			let text =
				state.get_range(DocumentPosition::ByteOffset(0), DocumentPosition::ByteOffset(5)).unwrap();
			assert_eq!(text, "hello");

			let text2 =
				state.get_range(DocumentPosition::ByteOffset(6), DocumentPosition::ByteOffset(11)).unwrap();
		...
		fn test_insert_delete() {
			...
			// Insert text
			let bytes_inserted = state.insert(DocumentPosition::ByteOffset(6), "beautiful ").unwrap();
			assert_eq!(bytes_inserted, 10);
			assert_eq!(state.buffer.to_string().unwrap(), "hello beautiful world");

			// Delete text
			state.delete(DocumentPosition::ByteOffset(6), DocumentPosition::ByteOffset(16)).unwrap();
			assert_eq!(state.buffer.to_string().unwrap(), "hello world");
		}
		...
		fn test_replace() {
			...
			state
				.replace(DocumentPosition::ByteOffset(0), DocumentPosition::ByteOffset(5), "hi")
				.unwrap();
	...
	mod virtual_text_integration_tests {
		...
		fn test_virtual_text_position_tracking_on_insert() {
			...
			state.apply(
				&mut cursors,
				&Event::Insert { position: 6, text: "beautiful ".to_string(), cursor_id },
			);
		...
		fn test_virtual_text_position_tracking_on_delete() {
			...
			state.apply(
				&mut cursors,
				&Event::Delete { range: 6..16, deleted_text: "beautiful ".to_string(), cursor_id },
			);
