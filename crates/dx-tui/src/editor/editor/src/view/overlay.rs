...
impl OverlayFace {
	...
	pub fn from_options(options: &dx_core::api::OverlayOptions) -> Self {
...
impl OverlayManager {
	...
	pub fn new() -> Self {
		Self { overlays: Vec::new(), marker_to_idx: HashMap::new() }
	}
	...
	pub fn replace_range_in_namespace(
		...
		if range.start < range.end {
			...
			let mut candidates: Vec<usize> =
				hits.iter().filter_map(|(mid, _, _)| self.marker_to_idx.get(mid).copied()).collect();
	...
	pub fn in_range(&self, range: &Range<usize>, marker_list: &MarkerList) -> Vec<&Overlay> {
		self.overlays.iter().filter(|o| o.overlaps(range, marker_list)).collect()
	}
	...
	pub fn query_viewport(
		...
		let marker_positions: HashMap<_, _> =
			visible_markers.into_iter().map(|(id, start, _end)| (id, start)).collect();
		...
		self
			...
			.filter_map(|overlay| {
				...
				let start_pos = start_in_vp.or_else(|| marker_list.get_position(overlay.start_marker))?;
				...
				if included { Some((overlay, range)) } else { None }
			})
			.collect()
	}
...
impl Overlay {
	...
	pub fn error(marker_list: &mut MarkerList, range: Range<usize>, message: Option<String>) -> Self {
		let mut overlay = Self::with_priority(
			marker_list,
			range,
			OverlayFace::Underline { color: Color::Red, style: UnderlineStyle::Wavy },
	...
	pub fn warning(
		...
		let mut overlay = Self::with_priority(
			marker_list,
			range,
			OverlayFace::Underline { color: Color::Yellow, style: UnderlineStyle::Wavy },
	...
	pub fn info(marker_list: &mut MarkerList, range: Range<usize>, message: Option<String>) -> Self {
		let mut overlay = Self::with_priority(
			marker_list,
			range,
			OverlayFace::Underline { color: Color::Blue, style: UnderlineStyle::Wavy },
	...
	pub fn hint(marker_list: &mut MarkerList, range: Range<usize>, message: Option<String>) -> Self {
		let mut overlay = Self::with_priority(
			marker_list,
			range,
			OverlayFace::Underline { color: Color::Gray, style: UnderlineStyle::Dotted },
...
mod tests {
	...
	fn test_overlay_creation_with_markers() {
		...
		let overlay =
			Overlay::new(&mut marker_list, 5..10, OverlayFace::Background { color: Color::Red });
	...
	fn test_overlay_adjusts_with_insert() {
		...
		let overlay =
			Overlay::new(&mut marker_list, 10..20, OverlayFace::Background { color: Color::Red });
	...
	fn test_overlay_adjusts_with_delete() {
		...
		let overlay =
			Overlay::new(&mut marker_list, 20..30, OverlayFace::Background { color: Color::Red });
	...
	fn test_overlay_namespace_clear() {
		...
		let overlay3 =
			Overlay::new(&mut marker_list, 25..30, OverlayFace::Background { color: Color::Green });
	...
	///
	/// Run with:
	///   cargo nextest run -p dx-editor --no-capture \
	...
	fn perf_full_buffer_rebuild_pass() {
		...
		let overlay_byte =
			|line: usize, k: usize| -> usize { line * LINE_BYTES + k * (LINE_BYTES / OVERLAYS_PER_LINE) };
		let make_overlay = |ml: &mut MarkerList, line: usize, k: usize| {
			let s = overlay_byte(line, k);
			Overlay::new(ml, s..(s + 2), OverlayFace::Background { color: Color::Red })
		};
	...
	mod proptests {
		...
		fn arb_overlay_spec() -> impl Strategy<Value = (usize, usize, i32)> {
			(0..(BUFFER_SIZE - MAX_OVERLAY_LEN), 1..=MAX_OVERLAY_LEN, -5i32..=5i32)
		}
