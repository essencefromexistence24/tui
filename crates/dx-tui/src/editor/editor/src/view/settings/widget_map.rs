...
use dx_core::api::{DualListOption, OverlayColorSpec, OverlayOptions, WidgetSpec};
use dx_core::text_property::{InlineOverlay, OffsetUnit, StyledSegment, TextPropertyEntry};
	...
	match control {
		...
		SettingControl::Number(s) => {
			...
			let (edit_text, edit_cursor, edit_sel) = match &s.editor {
				Some(ed) => {
					let sel =
						ed.selection_flat_range().map(|(a, b)| (a as i32, b as i32)).unwrap_or((-1, -1));
		...
		SettingControl::Text(s) => {
			...
			let sel = if s.editing {
				s.editor.selection_flat_range().map(|(a, b)| (a as i32, b as i32)).unwrap_or((-1, -1))
			} else {
				(-1, -1)
			};
			WidgetSpec::Text {
				...
				// lands.
				cursor_byte: if s.editing { s.cursor_byte() as i32 } else { -1 },
				focused: s.editing,
				label: s.label.clone(),
				placeholder: if s.placeholder.is_empty() { None } else { Some(s.placeholder.clone()) },
				rows: 1,
				field_width: 0,
				max_visible_chars: 0,
				full_width: true,
		...
		SettingControl::DualList(s) => WidgetSpec::DualList {
			options: s
				.all_options
				.iter()
				.map(|(value, label)| DualListOption { value: value.clone(), label: label.clone() })
				.collect(),
		...
		SettingControl::ObjectArray(s) => {
			...
			let rows: Vec<TextPropertyEntry> = s
				...
				.map(|(idx, b)| {
					...
					let action = b.get(field).and_then(|v| v.as_str()).unwrap_or("(no action)");
		...
		SettingControl::Json(s) => {
			...
			let lines = s.lines();
			let inner_width = lines.iter().map(|l| l.chars().count()).max().unwrap_or(0).max(20) + 2;
...
fn text_list_add_row(s: &crate::view::controls::TextListState, focused: bool) -> WidgetSpec {
	...
	if show_input {
		...
		if let Some(b) = caret_byte {
			let ch_len = entry.text[b..].chars().next().map(|c| c.len_utf8()).unwrap_or(1);
			entry.inline_overlays.push(InlineOverlay {
				start: b,
				end: b + ch_len,
		...
		WidgetSpec::Raw { entries: vec![entry], key: None }
	} else if add_focused {
...
fn text_list_item_row(item: &str, row_focused: bool, editing: bool, cursor: usize) -> WidgetSpec {
	...
	if row_focused && editing {
		...
		let b = cell_start + item.char_indices().nth(cursor).map(|(b, _)| b).unwrap_or(item.len());
		let ch_len = entry.text[b..].chars().next().map(|c| c.len_utf8()).unwrap_or(1);
		entry.inline_overlays.push(InlineOverlay {
			start: b,
			end: b + ch_len,
...
fn json_line_row(
	...
	let col_to_byte = |col: usize| -> usize {
		padded.char_indices().nth(col).map(|(b, _)| b).unwrap_or(padded.len())
	};
	if let Some(((sr, sc), (er, ec))) = selection {
		...
		if line_idx >= sr && line_idx <= er {
			...
			let to = if line_idx == er { ec } else { line.chars().count() };
			if to > from {
				entry.inline_overlays.push(InlineOverlay {
...
fn raw_row(text: String) -> WidgetSpec {
	WidgetSpec::Raw { entries: vec![TextPropertyEntry::text(text)], key: None }
}
...
fn seg(text: &str, fg_key: Option<&str>) -> StyledSegment {
	StyledSegment {
		text: text.to_string(),
		style: fg_key
			.map(|k| OverlayOptions { fg: Some(OverlayColorSpec::theme_key(k)), ..Default::default() }),
		overlays: Vec::new(),
	}
}
...
fn segments_row(segments: Vec<StyledSegment>) -> TextPropertyEntry {
	TextPropertyEntry { segments, ..TextPropertyEntry::text("") }
}
...
fn list_selection(focus: FocusState, focused: Option<usize>) -> i32 {
	if focus == FocusState::Focused { focused.map(|i| i as i32).unwrap_or(-1) } else { -1 }
}
...
pub fn settings_items_to_widget(items: &[SettingItem]) -> WidgetSpec {
		...
		if item.is_section_start {
			if let Some(section) = item.section.as_deref() {
				if !children.is_empty() {
					children.push(WidgetSpec::Divider { ch: "─".to_string(), style: None, key: None });
				}
	...
	WidgetSpec::Col { children, key: Some("settings-page".to_string()) }
}
...
mod tests {
	...
	fn toggle_maps_to_toggle_widget() {
		...
		match setting_control_to_widget("/editor/word_wrap", &SettingControl::Toggle(s)) {
			WidgetSpec::Toggle { checked, label, key, .. } => {
				assert!(checked);
	...
	fn integer_number_maps_directly() {
		...
		match setting_control_to_widget("/editor/tab_size", &SettingControl::Number(s)) {
			WidgetSpec::Number { value, min, max, integer, percent, .. } => {
				assert_eq!(value, 4.0);
	...
	fn percentage_number_unscales_by_100() {
		...
		match setting_control_to_widget("/ui/opacity", &SettingControl::Number(s)) {
			WidgetSpec::Number { value, percent, max, .. } => {
	...
	fn dropdown_maps_options_and_selection() {
		...
		match setting_control_to_widget("/ui/theme", &SettingControl::Dropdown(s)) {
			WidgetSpec::Dropdown { options, selected_index, .. } => {
	...
	fn text_maps_value_and_placeholder() {
		let s = TextInputState::new("Formatter").with_value("rustfmt").with_placeholder("command");
		match setting_control_to_widget("/fmt/cmd", &SettingControl::Text(s)) {
			WidgetSpec::Text { value, placeholder, rows, .. } => {
	...
	fn dual_list_maps_options_included_excluded() {
		...
		match setting_control_to_widget("/statusbar/elements", &SettingControl::DualList(s)) {
			WidgetSpec::DualList { options, included, excluded, .. } => {
	...
	fn page_builds_col_with_section_headers_and_dividers() {
		let items = vec![
			...
			item(
				"/ui/theme",
				SettingControl::Dropdown(DropdownState::new(vec!["Light".into(), "Dark".into()], "Theme")),
				Some("UI"),
			),
		];
		...
		let items = vec![
			...
			item(
				...
				SettingControl::Dropdown(
					DropdownState::new(vec!["Light".into(), "Dark".into()], "Theme").with_selected(1),
				),
				None,
			),
		];
	...
	fn complex_control_maps_to_placeholder() {
		let c = SettingControl::Complex { type_name: "opaque".into() };
	...
	fn rendered_fg_keys(out: &crate::widgets::RenderOutput) -> Vec<String> {
		...
		for e in &out.entries {
			let styles = e.inline_overlays.iter().map(|o| &o.style).chain(e.style.as_ref());
			for s in styles {
	...
	fn text_list_focused_item_renders_block_caret() {
		...
		let spec = setting_control_to_widget("/languages/cpp/extensions", &SettingControl::TextList(s));
		let out = crate::widgets::render_spec(&spec, &HashMap::new(), "", u32::MAX);
		let row = out.entries.iter().find(|e| e.text.contains("[cc")).expect("focused item row");
		let cell_start = "  [".len();
		let caret = row
			.inline_overlays
			.iter()
		...
		let other = out.entries.iter().find(|e| e.text.contains("[cpp")).expect("unfocused item row");
		assert!(
	...
	fn text_list_focused_but_not_editing_shows_no_caret() {
		...
		assert!(row.text.contains("Enter:edit"), "nav-mode row shows the edit hint: {:?}", row.text);
	}
	...
	fn composite_rows_use_surface_readable_theme_keys() {
		...
		for control in
			[SettingControl::Map(map), SettingControl::ObjectArray(arr), SettingControl::TextList(list)]
		{
	...
	fn object_array_add_new_row_highlights_when_focused() {
		...
		let out = crate::widgets::render_spec(&spec, &HashMap::new(), "", u32::MAX);
		let add_unfocused =
			out.entries.iter().find(|e| e.text.contains("[+] Add new")).expect("add-new row");
		assert!(
			add_unfocused.style.as_ref().and_then(|s| s.bg.as_ref()).is_none(),
		...
		assert!(
			add_focused.style.as_ref().is_some_and(|s| s.extend_to_line_end),
