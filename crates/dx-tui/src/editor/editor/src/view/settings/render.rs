...
fn render_horizontal_layout(
	...
	let chunks =
		Layout::horizontal([Constraint::Length(24), Constraint::Length(1), Constraint::Min(40)])
			.split(content_area);
...
fn render_vertical_layout(
	...
	let categories_area =
		Rect::new(content_area.x, content_area.y, content_area.width, category_height);

	// Separator line
	let sep_y = content_area.y + category_height;

	// Settings area
	let settings_area = Rect::new(content_area.x, sep_y + 1, content_area.width, settings_height);
...
fn render_categories_horizontal(
	...
	for (i, page) in state.pages.iter().enumerate() {
		...
		let style = if is_selected && is_focused {
			...
			Style::default().fg(theme.menu_highlight_fg).add_modifier(Modifier::BOLD)
		} else {
			Style::default().fg(theme.popup_text_fg)
		};

		let indicator_style =
			if has_modified { Style::default().fg(theme.menu_highlight_fg) } else { style };

		// Add separator between categories
		if i > 0 {
			spans.push(Span::styled(" │ ", Style::default().fg(theme.split_separator_fg)));
			total_width += 3;
		}
		...
		layout.categories.push((i, Rect::new(cat_x, area.y, cat_width, 1)));
	}
...
fn render_categories(
	...
	state.categories_scroll.update_content_height(&rows, area.width);
	...
	let row_data: Vec<RowData> = rows
		.iter()
		.map(|row| match *row {
			TreeRow::Category { idx, expandable, expanded } => {
				let page = &state.pages[idx];
				RowData {
					chevron: if expandable { if expanded { "▼" } else { "▶" } } else { " " },
	...
	let panel_layout = state.categories_scroll.render(
		...
		|frame, info, row| {
			...
			let fg = if data.is_selected {
				if focus_panel == FocusPanel::Categories { theme.menu_highlight_fg } else { theme.menu_fg }
			} else {
				theme.popup_text_fg
			};
			...
			let selected_marker =
				if data.is_selected && focus_panel == FocusPanel::Categories { ">" } else { " " };
			...
			if data.has_changes {
				spans.push(Span::styled("● ", Style::default().fg(theme.menu_highlight_fg).bg(bg)));
			} else {
				spans.push(Span::styled("  ", style));
			}
			if let Some(icon) = data.icon {
				spans
					.push(Span::styled(icon.to_string(), Style::default().fg(theme.popup_border_fg).bg(bg)));
			} else {
				spans.push(Span::styled(" ", style));
			}
			let label = if data.is_plugin_category {
				let prefix_width: usize = spans.iter().map(|span| str_width(span.content.as_ref())).sum();
...
fn render_settings_panel(
	...
	if area.height > 0 && area.width > 0 {
		...
		let title_style = Style::default().fg(theme.editor_fg).add_modifier(Modifier::BOLD);
		frame
			.render_widget(Paragraph::new(title).style(title_style), Rect::new(area.x, y, area.width, 1));
		y += 1;
	}
	...
	if page_nullable && state.current_category_has_values() {
		...
		let btn_style = if is_hovered {
			Style::default().fg(theme.menu_hover_fg).bg(theme.menu_hover_bg)
		} else {
	...
	state.scroll_panel.update_content_height(&page.items, area.width);
	...
	let max_label_width = page
		...
		.max();
	let pending_dirty_by_item: Vec<bool> =
		page.items.iter().map(|item| state.path_has_pending_change(&item.path)).collect();
	...
	let panel_layout = state.scroll_panel.render(
		...
		|frame, info, item| {
			render_setting_item_pure(
				...
				theme,
				max_label_width,
				pending_dirty_by_item.get(info.index).copied().unwrap_or(false),
			)
		},
		theme,
	);
...
impl BandViewport {
	...
	fn band_rect(&self, logical_y: u16, rows: u16) -> Option<Rect> {
		...
		Some(Rect::new(self.area.x, physical_y, self.area.width, visible_h))
	}
}
...
fn inset_by_chrome(r: Rect, style: &ItemBoxStyle) -> Rect {
	inset_horizontal(r, style.card_border_cols + style.focus_indicator_cols, style.card_border_cols)
}
...
fn render_section_header(
	...
	let header_style = Style::default().fg(theme.editor_fg).add_modifier(Modifier::BOLD);
	frame.render_widget(
...
fn render_inherit_affordance(
	...
	if item.is_null {
		...
		let badge_x = control_rect.x.saturating_add(control_rect.width).saturating_sub(badge_len);
		if badge_x > control_rect.x {
			frame.render_widget(
				Paragraph::new(badge_text)
					.style(Style::default().fg(theme.line_number_fg).add_modifier(Modifier::ITALIC)),
		...
		let btn_x = control_rect.x.saturating_add(control_rect.width).saturating_sub(btn_len);
...
fn render_description_band(
	...
	if plan.description_rows > 0 {
		let Some(desc_rect) =
			vp.band_rect(plan.description_y(), plan.description_rows).map(|r| inset_by_chrome(r, style))
		else {
			return;
		};
		let desc_skip = vp.skip_top.saturating_sub(plan.description_y());
		let max_text_width =
			desc_rect.width.saturating_sub(style.description_right_padding_cols) as usize;
		...
		let Some(layer_rect) = vp.band_rect(plan.description_y(), 1).map(|r| inset_by_chrome(r, style))
		else {
			return;
		};
		frame.render_widget(
...
fn render_setting_item_pure(
	...
	if let Some(card_rect) =
		vp.band_rect(card_logical_top, card_logical_bottom.saturating_sub(card_logical_top))
	{
	...
	if let Some(content_rect) =
		vp.band_rect(content_logical_top, content_logical_bottom.saturating_sub(content_logical_top))
	{
		let inner_area = inset_horizontal(content_rect, style.card_border_cols, style.card_border_cols);
		...
		if is_selected && label_row_visible {
			frame.render_widget(
				Paragraph::new(">")
					.style(Style::default().fg(theme.settings_selected_fg).add_modifier(Modifier::BOLD)),
		...
		if let Some(control_rect) =
			vp.band_rect(content_logical_top, plan.control_rows).map(|r| inset_by_chrome(r, &style))
		{
	...
	SettingItemLayoutInfo { control: control_layout, inherit_button: inherit_button_area }
}
...
fn render_control(
	...
	match control {
		...
		SettingControl::Toggle(_) => {
			...
			let out = render_scalar_via_widget(frame, area, control, name, theme, label_width, prev);
		...
		SettingControl::Dropdown(state) => {
			...
			if state.open {
				for h in out.hits.iter().filter(|h| h.event_type == "dropdown_select") {
			...
			let visible = state.options.len().min(crate::widgets::DROPDOWN_VISIBLE_OPTIONS);
			let scroll_offset = state.scroll_offset.min(state.options.len().saturating_sub(visible));
			ControlLayoutInfo::Dropdown { button_area, option_areas, scroll_offset }
		}
		...
		SettingControl::Text(state) => {
			...
			if read_only {
				...
				let value_area = Rect::new(area.x + label_w, area.y, area.width.saturating_sub(label_w), 1);

				frame.render_widget(Paragraph::new(label.clone()).style(label_style), label_area);
				frame.render_widget(Paragraph::new(value.as_str()).style(value_style), value_area);
		...
		SettingControl::Map(state) => {
			...
			let entry_rows = state
				...
				.collect();
			let add_row_area =
				if state.no_add { None } else { row_rect(first_entry_row + state.entries.len() as u16) };
			ControlLayoutInfo::Map { entry_rows, add_row_area }
		}
			...
			let entry_rows = state
				...
				.filter_map(|(i, _)| {
					(1 + i as u16).checked_sub(skip_rows).and_then(|dst| {
						(dst < area.height).then(|| (i, Rect::new(area.x, area.y + dst, area.width, 1)))
					})
				})
				.collect();
		...
		SettingControl::Json(state) => {
			let focus_key =
				if matches!(state.focus, crate::view::controls::FocusState::Focused) { name } else { "" };
			render_control_via_widget(
				frame,
				area,
				control,
...
fn render_button(
	...
	if is_focused {
		...
		let style = Style::default().fg(theme.menu_hover_fg).bg(theme.menu_hover_bg);
		frame.render_widget(Paragraph::new(text).style(style), area);
	} else {
		let fg = if dimmed { theme.line_number_fg } else { theme.popup_text_fg };
...
fn render_footer(
	...
	let current_is_nullable_set =
		state.current_item().map(|item| item.nullable && !item.is_null).unwrap_or(false);
	...
	let cancel_width =
		str_width(if cancel_focused { &cancel_text_focused } else { &cancel_text }) as u16;
	let save_width = str_width(if save_focused { &save_text_focused } else { &save_text }) as u16;
	let reset_width = str_width(if reset_focused { &reset_text_focused } else { &reset_text }) as u16;
	let layer_width = str_width(if layer_focused { &layer_text_focused } else { &layer_text }) as u16;
	let edit_width = str_width(if edit_focused { &edit_text_focused } else { &edit_text }) as u16;
	let gap: u16 = 2;
	...
	let cancel_x = footer_area.x.saturating_add(footer_area.width.saturating_sub(cancel_width));
	let save_x = cancel_x.saturating_sub(save_width + gap);
	let reset_x = if show_reset { save_x.saturating_sub(reset_width + gap) } else { 0 };
	let layer_x = if show_layer { reset_x.saturating_sub(layer_width + gap) } else { 0 };
	...
	let help_start_x = if show_edit { edit_x + edit_width + 2 } else { footer_area.x };
	...
	frame.render_widget(Paragraph::new(help_line), Rect::new(help_start_x, footer_y, help_width, 1));
}
...
fn build_keyhint_line<'a>(text: &str, theme: &Theme) -> Line<'a> {
	let key_style = Style::default().fg(theme.popup_text_fg).bg(theme.split_separator_fg);
	let desc_style = Style::default().fg(theme.line_number_fg);
...
fn render_footer_vertical(
	...
	let footer_y = modal_area.y.saturating_add(modal_area.height.saturating_sub(footer_height));
	let footer_width = modal_area.width.saturating_sub(2);
...
fn render_search_header(frame: &mut Frame, area: Rect, state: &SettingsState, theme: &Theme) {
	...
	let indicator_style = Style::default().fg(theme.menu_active_fg).add_modifier(Modifier::BOLD);
	...
	let (sel_start, sel_end) = state
		...
		.unwrap_or((-1, -1));
	let field_spec = dx_core::api::WidgetSpec::Text {
	...
	if suffix_x < area.x + area.width && !(count_text.is_empty() && scroll_indicator.is_empty()) {
		...
		frame.render_widget(Paragraph::new(suffix), Rect::new(suffix_x, area.y, suffix_w, 1));
	}
}
...
fn render_search_results(
	...
	let content_area =
		Rect::new(area.x, area.y, area.width.saturating_sub(scrollbar_width), area.height);

	let mut y = content_area.y;

	for (idx, result) in state.search_results.iter().enumerate().skip(state.search_scroll_offset) {
		...
		render_search_result_item(frame, item_area, result, is_selected, is_hovered, theme, layout);
		y += 3;
	}
...
fn render_search_result_item(
	...
	let (display_name, display_desc) = match &result.deep_match {
		...
		Some(DeepMatch::MapValue { matched_text, key, .. }) => {
			(matched_text.clone(), Some(format!("{} > {}", result.item.name, key)))
		}
		Some(DeepMatch::TextListItem { text, .. }) => (text.clone(), Some(result.item.name.clone())),
	...
	let indicator_style = if is_selected {
		Style::default().fg(theme.settings_selected_fg).add_modifier(Modifier::BOLD)
	} else {
		name_style
	};
	let mut name_line = build_highlighted_text(
		...
		&result.name_matches,
		name_style,
		Style::default().fg(theme.diagnostic_warning_fg).add_modifier(Modifier::BOLD),
	);
	name_line.spans.insert(0, Span::styled(indicator, indicator_style));
	frame.render_widget(Paragraph::new(name_line), Rect::new(area.x, area.y, area.width, 1));

	// Second line: Breadcrumb
	let breadcrumb_style = Style::default().fg(theme.line_number_fg).add_modifier(Modifier::ITALIC);
	...
	frame
		.render_widget(Paragraph::new(breadcrumb_line), Rect::new(area.x, area.y + 1, area.width, 1));
...
fn render_change_list(
	...
	for (i, change) in changes.iter().take((dialog_height as usize).saturating_sub(7)).enumerate() {
...
fn render_destructive_buttons(
	...
	for (idx, label) in options.iter().enumerate() {
		...
		let style = if is_selected && is_destructive {
			...
			Style::default().fg(theme.diagnostic_error_fg).add_modifier(Modifier::BOLD)
		} else {
			Style::default().fg(theme.popup_text_fg)
		};
		let text = if is_selected { format!(">[ {} ]", label) } else { format!(" [ {} ]", label) };
		let w = label.len() as u16 + 5;
		frame.render_widget(Paragraph::new(text).style(style), Rect::new(x, button_y, w, 1));
		x += w + 2;
	}
}
...
fn render_reset_dialog(frame: &mut Frame, parent_area: Rect, state: &SettingsState, theme: &Theme) {
	...
	// Prompt text
	frame.render_widget(
		Paragraph::new("Discard all pending changes?").style(Style::default().fg(theme.popup_text_fg)),
		Rect::new(inner.x, inner.y, inner.width, 1),
	);
	render_change_list(frame, inner, inner.y + 2, &changes, dialog_height, theme);
...
fn render_entry_delete_confirm(
	...
	let body = if !state.entry_delete_target_name.is_empty() {
		format!("This will permanently remove \"{}\".", state.entry_delete_target_name)
	} else if state.entry_delete_target_is_array_item {
...
fn render_entry_items(
	...
	for (idx, item) in dialog.items.iter().enumerate() {
		...
		if item.is_section_start {
			if let Some(ref section_name) = item.section {
				...
				if header_end > scroll_offset && screen_y < inner.y + inner.height {
					...
					if skip_h == 0 {
						let section_style =
							Style::default().fg(theme.line_number_fg).add_modifier(Modifier::BOLD);
						frame.render_widget(
		...
		let skip_rows =
			if item_start < scroll_offset { (scroll_offset - item_start) as u16 } else { 0 };
		...
		if is_focused {
			let indicator_y = if item.control.is_composite() {
				...
				if visible_sub < render_height as u16 { screen_y + visible_sub } else { screen_y }
			} else {
				screen_y
			};
		...
		if !item.read_only && skip_rows == 0 && control_area.width > 0 {
			...
			let inherits = dialog.inheritable_fields.contains(item.path.trim_start_matches('/'));
			if item.nullable && item.is_null {
				...
				let positions = super::entry_dialog::layout_field_action_buttons(&buttons, right_edge);
				let focused = if dialog.selected_item == idx { dialog.field_button_focus } else { None };
				for (bi, ((_, label), (_, x, w))) in buttons.iter().zip(positions.iter()).enumerate() {
					if *x <= control_area.x {
						continue;
					}
	...
	if needs_scroll {
		...
		let scrollbar_state = ScrollbarState::new(total_content_height, viewport_height, scroll_offset);
...
fn render_entry_buttons(
	...
	let delete_idx = if has_delete { Some(buttons.len() - 1) } else { None };
...
fn render_entry_footer(
	...
	if !dialog.focus_on_buttons && helper_y > inner.y {
		...
		if let Some(text) = text {
			...
			let helper_style = Style::default().fg(theme.line_number_fg).add_modifier(Modifier::ITALIC);
			frame.render_widget(
				Paragraph::new(truncated).style(helper_style),
				Rect::new(dialog_area.x + 2, helper_y, dialog_area.width.saturating_sub(4), 1),
			);
		}
	}
	...
	let (text, style) = if has_invalid_json && !is_json_control {
		("⚠ Invalid JSON - fix before leaving field", Style::default().fg(theme.diagnostic_warning_fg))
	} else if has_invalid_json {
		("⚠ Invalid JSON", Style::default().fg(theme.diagnostic_warning_fg))
	} else if is_json_control {
		("↑↓←→:Move  Enter:Newline  Tab/Esc:Exit", Style::default().fg(theme.line_number_fg))
	} else if dialog.editing_text {
		("Enter/Tab:Commit field  Esc:Cancel", Style::default().fg(theme.line_number_fg))
	} else {
...
fn render_entry_dialog_inner(
	...
	let border_color =
		if dialog.is_dirty() { theme.diagnostic_warning_fg } else { theme.popup_border_fg };
	let block = Block::default()
		.title(title)
		.borders(Borders::ALL)
		.border_type(BorderType::Rounded)
...
fn render_help_overlay(frame: &mut Frame, parent_area: Rect, theme: &Theme) {
	...
	let help_items = [
		...
		(
			"Actions",
			vec![("Ctrl+S", "Save settings"), ("Esc", "Close settings"), ("?", "Toggle this help")],
		),
	];
	...
	for (section_name, bindings) in &help_items {
		...
		// Section header
		let header_style = Style::default().fg(theme.menu_active_fg).add_modifier(Modifier::BOLD);
		frame.render_widget(
			Paragraph::new(*section_name).style(header_style),
