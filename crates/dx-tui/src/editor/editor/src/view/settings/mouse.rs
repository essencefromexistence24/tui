...
impl Editor {
	...
	pub(crate) fn handle_settings_mouse(
		...
		let showing_confirm =
			self.settings_state.as_ref().map(|s| s.showing_confirm_dialog).unwrap_or(false);
		if showing_confirm {
					...
					if let Some(page) = state.pages.get_mut(state.selected_category) {
						if let Some(item) = page.items.get_mut(idx) {
							if let SettingControl::Map(map_state) = &mut item.control {
								...
								map_state.focused_entry =
									if row_idx < map_state.entries.len() { Some(row_idx) } else { None };
							}
						}
					}
					is_add_new
	...
	pub(crate) fn entry_dialog_activate_button(&mut self, kind: &str) {
		...
		let has_delete = state.entry_dialog().map(|d| !d.is_new && !d.no_delete).unwrap_or(false);
		match kind {
	...
	fn settings_scroll_up(&mut self, delta: usize) -> bool {
		self.settings_state.as_mut().map(|state| state.scroll_up(delta)).unwrap_or(false)
	}

	fn settings_scroll_down(&mut self, delta: usize) -> bool {
		self.settings_state.as_mut().map(|state| state.scroll_down(delta)).unwrap_or(false)
	}

	fn settings_scrollbar_click(&mut self, row: u16) {
		if let Some(ref scrollbar_area) =
			self.active_chrome().settings_layout.as_ref().and_then(|l| l.scrollbar_area)
		{
	...
	fn settings_scrollbar_drag(&mut self, col: u16, row: u16) -> bool {
		...
		{
			let in_scrollbar_x =
				col >= scrollbar_area.x.saturating_sub(1) && col <= scrollbar_area.x + scrollbar_area.width;
	...
	fn search_scrollbar_click(&mut self, row: u16) {
		if let Some(ref scrollbar_area) =
			self.active_chrome().settings_layout.as_ref().and_then(|l| l.search_scrollbar_area)
		{
			if scrollbar_area.height > 0 {
				let relative_y = row.saturating_sub(scrollbar_area.y);
				let ratio = relative_y as f32 / scrollbar_area.height as f32;
	...
	fn entry_dialog_update_hover(&mut self, col: u16, row: u16) -> bool {
		...
		if row == layout.button_y {
			...
			for (idx, label) in buttons.iter().enumerate() {
				if idx > 0 {
					let gap = if Some(idx) == delete_idx { DELETE_GAP } else { BUTTON_GAP };
					x += gap;
				}
	...
	fn get_confirm_dialog_button_at(&self, col: u16, row: u16) -> Option<usize> {
		let modal_area = self.active_chrome().settings_layout.as_ref().map(|l| l.modal_area)?;

		let changes_count =
			self.settings_state.as_ref().map(|s| s.get_change_descriptions().len()).unwrap_or(0);

		// Calculate dialog dimensions (same as in render_confirm_dialog)
		let dialog_width = 50u16.min(modal_area.width.saturating_sub(4));
		let dialog_height = (7 + changes_count as u16).min(20).min(modal_area.height.saturating_sub(4));
