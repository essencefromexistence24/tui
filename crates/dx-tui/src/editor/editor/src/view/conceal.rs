...
impl ConcealManager {
	...
	pub fn new() -> Self {
		Self { ranges: Vec::new(), marker_to_idx: HashMap::new(), version: 0 }
	}
	...
	pub fn add(
		...
		self.ranges.push(ConcealRange { namespace, start_marker, end_marker, replacement });
...
mod tests {
	...
	fn test_conceal_remove_in_range_keeps_only_disjoint() {
		...
		let kept: Vec<_> =
			manager.query_viewport(0, 1000, &marker_list).into_iter().map(|(r, _)| r).collect();
	...
	fn perf_full_buffer_rebuild_pass() {
		...
		let conceal_byte =
			|line: usize, k: usize| -> usize { line * LINE_BYTES + k * (LINE_BYTES / CONCEALS_PER_LINE) };
	...
	mod proptests {
		...
		enum Op {
			...
			Add { start: usize, len: usize, ns_idx: u8 },
