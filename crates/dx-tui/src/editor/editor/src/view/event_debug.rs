...
pub fn render_event_debug(frame: &mut Frame, area: Rect, debug: &EventDebug, theme: &Theme) {
	...
	if debug.history.is_empty() {
		...
		for (i, recorded) in debug.history.iter().enumerate() {
			...
			if let Some(normalized) = &recorded.normalized {
				...
				spans.push(Span::styled(normalized, Style::default().fg(theme.diagnostic_hint_fg)));
	...
	if let Some(details) = debug.last_event_details() {
		...
		control_lines
			.push(Line::from(vec![Span::styled(details, Style::default().fg(theme.line_number_fg))]));
	}
