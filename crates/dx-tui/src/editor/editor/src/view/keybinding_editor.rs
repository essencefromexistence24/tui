...
use crate::input::keybindings::{KeybindingResolver, format_keybinding, normalize_key};
use crate::view::dimming::apply_dimming;
use crate::view::theme::Theme;
use crate::view::ui::scrollbar::{ScrollbarColors, render_scrollbar};
...
fn keybinding_modal_area(area: Rect) -> Rect {
	...
	Rect { x, y, width: modal_width, height: modal_height }
}
...
pub fn render_keybinding_editor(
	...
	// Border
	let title = format!(" {} \u{2500} [{}] ", t!("keybinding_editor.title"), editor.active_keymap);
...
fn render_header(frame: &mut Frame, area: Rect, editor: &KeybindingEditor, theme: &Theme) {
	...
	let mut path_spans = vec![
		...
		Span::styled(&editor.config_file_path, Style::default().fg(theme.diagnostic_info_fg)),
	];
	if !editor.keymap_names.is_empty() {
		...
		path_spans
			.push(Span::styled(editor.keymap_names.join(", "), Style::default().fg(theme.popup_text_fg)));
	}
			...
			SearchMode::Text => {
				let mut spans = vec![
					Span::styled(
						...
						Style::default().fg(theme.help_key_fg).add_modifier(Modifier::BOLD),
					),
					Span::styled(&editor.search_query, Style::default().fg(theme.popup_text_fg)),
				];
	...
	let count_str = if filtered == total {
		...
		t!("keybinding_editor.bindings_filtered", filtered = filtered, total = total).to_string()
	};
	...
	let filter_spans = vec![
		...
		Span::styled(format!("  {}", count_str), Style::default().fg(theme.popup_text_fg)),
		Span::styled(
...
fn render_table(frame: &mut Frame, area: Rect, editor: &mut KeybindingEditor, theme: &Theme) {
	...
	let fixed_cols = key_col_width + action_name_col_width + context_col_width + source_col_width + 5; // +5 for spacers
	...
	let header = Line::from(vec![
		...
		Span::styled(" ", Style::default()),
		Span::styled(
			pad_right(&t!("keybinding_editor.header_action"), action_name_col_width as usize),
			Style::default().fg(theme.help_key_fg).add_modifier(Modifier::BOLD),
		),
		Span::styled(" ", Style::default()),
		Span::styled(
			pad_right(&t!("keybinding_editor.header_description"), description_col_width as usize),
			Style::default().fg(theme.help_key_fg).add_modifier(Modifier::BOLD),
		),
		Span::styled(" ", Style::default()),
		Span::styled(
			pad_right(&t!("keybinding_editor.header_context"), context_col_width as usize),
			Style::default().fg(theme.help_key_fg).add_modifier(Modifier::BOLD),
		),
		Span::styled(" ", Style::default()),
		Span::styled(
			pad_right(&t!("keybinding_editor.header_source"), source_col_width as usize),
			Style::default().fg(theme.help_key_fg).add_modifier(Modifier::BOLD),
		),
	]);
	...
	editor.scroll.set_content_height(editor.display_rows.len() as u16);

	let visible_rows = table_area.height as usize;
	let scroll_offset = editor.scroll.offset as usize;

	for (display_idx, display_row) in
		editor.display_rows.iter().skip(scroll_offset).take(visible_rows).enumerate()
	{
		...
		let row_area = Rect { y: row_y, height: 1, ..table_area };

		match display_row {
			DisplayRow::SectionHeader { plugin_name, collapsed, binding_count } => {
				...
				let header_style = Style::default().fg(row_fg).bg(row_bg).add_modifier(Modifier::BOLD);
				...
				frame.render_widget(Paragraph::new("").style(Style::default().bg(row_bg)), row_area);
			...
			DisplayRow::Binding(binding_idx) => {
				...
				let key_style = Style::default()
					.fg(if is_selected { theme.popup_text_fg } else { theme.help_key_fg })
					.bg(row_bg);
				let action_name_style = Style::default()
					.fg(if is_selected { theme.popup_text_fg } else { theme.diagnostic_info_fg })
					.bg(row_bg);
				let action_style = Style::default().fg(row_fg).bg(row_bg);
				let context_style =
					Style::default().fg(if is_selected { row_fg } else { theme.popup_text_fg }).bg(row_bg);
				let source_style = Style::default()
					.fg(
						if binding.source == BindingSource::Custom || binding.source == BindingSource::Plugin {
							if is_selected { theme.popup_text_fg } else { theme.diagnostic_info_fg }
						} else {
				...
				let row = Line::from(vec![
					Span::styled(indicator, Style::default().fg(theme.help_key_fg).bg(row_bg)),
					Span::styled(pad_right(&binding.key_display, key_col_width as usize), key_style),
					...
					Span::styled(pad_right(&binding.context, context_col_width as usize), context_style),
...
fn help_line<'a>(key: &'a str, desc: &'a str, theme: &Theme, is_header: bool) -> Line<'a> {
	if is_header {
		Line::from(vec![Span::styled(
			key,
			Style::default().fg(theme.popup_text_fg).add_modifier(Modifier::BOLD),
		)])
	} else {
		Line::from(vec![
			Span::styled(
...
fn render_edit_dialog(
	...
	let field_bg = if key_focused { theme.popup_selection_bg } else { theme.popup_bg };
	let key_label_style = if key_focused {
		Style::default().fg(theme.help_key_fg).bg(field_bg).add_modifier(Modifier::BOLD)
	} else {
		Style::default().fg(theme.popup_text_fg).bg(field_bg)
	};
	let key_value_style = Style::default().fg(theme.popup_text_fg).bg(field_bg);
	if key_focused {
		frame.render_widget(Paragraph::new("").style(Style::default().bg(field_bg)), chunks[2]);
	}
	let mut key_spans = vec![
		Span::styled(format!("   {:9}", t!("keybinding_editor.label_key")), key_label_style),
	...
	if key_focused && dialog.capturing_special {
		key_spans.push(Span::styled(
			...
			Style::default().fg(theme.diagnostic_warning_fg).bg(field_bg).add_modifier(Modifier::BOLD),
		));
	...
	let field_bg = if action_focused { theme.popup_selection_bg } else { theme.popup_bg };
	...
	if action_focused {
		frame.render_widget(Paragraph::new("").style(Style::default().bg(field_bg)), chunks[3]);
	}
	let mut action_spans = vec![
		Span::styled(format!("   {:9}", t!("keybinding_editor.label_action")), action_label_style),
		Span::styled(action_display, action_value_style),
	];
	if action_focused && dialog.mode == EditMode::EditingAction {
		action_spans.push(Span::styled("_", Style::default().fg(theme.cursor).bg(field_bg)));
	}
			...
			frame.render_widget(
				Paragraph::new(Line::from(vec![
					...
					Span::styled(
						...
						Style::default().fg(theme.popup_text_fg).add_modifier(Modifier::ITALIC),
					),
				])),
				chunks[4],
			);
	...
	let field_bg = if ctx_focused { theme.popup_selection_bg } else { theme.popup_bg };
	...
	if ctx_focused {
		frame.render_widget(Paragraph::new("").style(Style::default().bg(field_bg)), chunks[5]);
	}
	frame.render_widget(
		Paragraph::new(Line::from(vec![
			Span::styled(format!("   {:9}", t!("keybinding_editor.label_context")), ctx_label_style),
			Span::styled(
	...
	let save_style = if btn_focused && dialog.selected_button == 0 {
		Style::default().fg(theme.popup_bg).bg(theme.help_key_fg).add_modifier(Modifier::BOLD)
	} else {
		Style::default().fg(theme.popup_text_fg)
	};
	...
	editor.layout.dialog_buttons = Some((
		Rect { x: save_x, y: chunks[8].y, width: save_text.len() as u16, height: 1 },
		Rect { x: cancel_x, y: chunks[8].y, width: cancel_text.len() as u16, height: 1 },
	));
...
fn render_autocomplete_popup(
	...
	let suggestion_count = dialog.autocomplete_suggestions.len().min(MAX_AUTOCOMPLETE_VISIBLE);
	...
	let scroll_offset =
		if selected >= MAX_AUTOCOMPLETE_VISIBLE { selected - MAX_AUTOCOMPLETE_VISIBLE + 1 } else { 0 };
...
fn render_confirm_dialog(
	...
	for (i, opt) in options.iter().enumerate() {
		...
		btn_rects.push(Rect { x: x_offset, y: chunks[2].y, width: text_len, height: 1 });
			...
			match (event.code, event.modifiers) {
				...
				(KeyCode::Left, _) if dialog.context_option_index > 0 => {
					...
					if let Some(key_code) = dialog.key_code {
						dialog.conflicts = editor.find_conflicts(key_code, dialog.modifiers, &dialog.context);
					}
				}
				(KeyCode::Right, _) if dialog.context_option_index + 1 < dialog.context_options.len() => {
					dialog.context_option_index += 1;
					dialog.context = dialog.context_options[dialog.context_option_index].clone();
					if let Some(key_code) = dialog.key_code {
						dialog.conflicts = editor.find_conflicts(key_code, dialog.modifiers, &dialog.context);
...
mod tests {
	...
	fn modal_centres_within_offset_area_left_of_dock() {
		...
		assert!(modal.x + modal.width <= chrome.x + chrome.width, "modal overflows the right edge");
		assert!(modal.y + modal.height <= chrome.y + chrome.height, "modal overflows the bottom edge");
	...
	fn make_editor() -> KeybindingEditor {
		...
		let menu_names: Vec<String> = ["File", "Edit", "View"].iter().map(|s| s.to_string()).collect();
		KeybindingEditor::new(
			&config,
			&resolver,
			&mode_registry,
			&cmd_registry,
			String::from("/tmp/dx-config.toml"),
			&menu_names,
		)
	}
	...
	fn add_dialog_records_shift_when_terminal_omits_shift_modifier() {
		// Regression for https://github.com/anomalyco/dx/issues/1899
