...
impl ColorCapability {
	...
	/// Can be overridden with DX_COLOR_MODE env var: "truecolor", "256", or "16"
	pub fn detect() -> Self {
		// Check for manual override first
		if let Ok(mode) = std::env::var("DX_COLOR_MODE") {
...
fn rgb_to_256(r: u8, g: u8, b: u8) -> u8 {
	...
	let r_idx = if r < 48 { 0 } else { ((r as u16 - 35) * 5 / 220).min(5) as u8 };
	let g_idx = if g < 48 { 0 } else { ((g as u16 - 35) * 5 / 220).min(5) as u8 };
	let b_idx = if b < 48 { 0 } else { ((b as u16 - 35) * 5 / 220).min(5) as u8 };
...
mod tests {
	...
	fn test_rgb_to_256_light_gray_no_overflow() {
		...
		assert!(idx_245 >= 231, "RGB(245,245,245) should be light, got index {}", idx_245);
		assert!(idx_248 >= 231, "RGB(248,248,248) should be light, got index {}", idx_248);
	...
	fn test_rgb_to_16_basic_colors() {
		...
		// Pure green
		assert!(matches!(rgb_to_16(0, 255, 0), Color::LightGreen | Color::Green));
		// Pure blue
		assert!(matches!(rgb_to_16(0, 0, 255), Color::LightBlue | Color::Blue));
		// Black
