...
fn render_confirmation_dialog(
	...
	let content = vec![
		...
		Line::from(""),
		Line::from(vec![
			Span::styled(format!("[{}]", confirm_key), Style::default().fg(theme.diagnostic_error_fg)),
	...
	let para =
		Paragraph::new(content).style(Style::default().fg(theme.editor_fg)).wrap(Wrap { trim: true });
...
fn render_capture_phase(
	...
	let flat_base = groups[..group_idx].iter().map(|g| g.targets.len()).sum::<usize>();
...
fn render_verify_phase(
	...
	let instructions = vec![
		...
		Line::from(vec![
			...
			Span::styled(translation_count.to_string(), Style::default().fg(theme.diagnostic_info_fg)),
		]),
	];
...
fn render_all_keys_ok(
	...
	let content = vec![
		...
		Line::from(vec![Span::styled(
			...
			Style::default().fg(theme.diagnostic_info_fg).add_modifier(Modifier::BOLD),
		)]),
		Line::from(""),
	...
	frame.render_widget(Paragraph::new("").style(Style::default().fg(theme.editor_fg)), chunks[1]);
