...
fn hanging_indent_width(leading_spaces: usize, max_width: usize) -> usize {
	if leading_spaces + 10 > max_width { 0 } else { leading_spaces }
}
...
pub fn wrap_styled_lines(lines: &[StyledLine], max_width: usize) -> Vec<StyledLine> {
	...
	for line in lines {
		let total_width: usize =
			line.spans.iter().map(|s| unicode_width::UnicodeWidthStr::width(s.text.as_str())).sum();
...
fn split_at_width(text: &str, available: usize) -> (&str, &str) {
	...
	let byte_idx = text.char_indices().nth(take_chars).map(|(i, _)| i).unwrap_or(text.len());
...
impl<'a> Iterator for WordSplitter<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		if word.is_empty() { None } else { Some((word, width)) }
	}
}
...
fn highlight_code_to_styled_lines(
	...
	for span in spans {
		...
		if span.range.start > pos {
			...
			add_text_to_lines(&mut result, &text, Style::default().fg(default_fg).bg(code_bg), None);
		}

		// Add highlighted text
		let text = String::from_utf8_lossy(&bytes[span.range.start..span.range.end]);
		add_text_to_lines(&mut result, &text, Style::default().fg(span.color).bg(code_bg), None);
		...
		match event {
			Event::Start(tag) => {
				match tag {
					...
					Tag::List(_) | Tag::Item if !lines.last().map(|l| l.spans.is_empty()).unwrap_or(true) => {
						lines.push(StyledLine::new());
					}
					Tag::Paragraph => {
				...
				if in_code_block {
					...
					let spans = if let Some(reg) = registry {
						if !code_block_lang.is_empty() {
							...
							let non_ws_bytes = text.bytes().filter(|b| !b.is_ascii_whitespace()).count();
							let good_coverage = non_ws_bytes == 0 || highlighted_bytes * 5 >= non_ws_bytes;
							if good_coverage { s } else { Vec::new() }
						} else {
							Vec::new()
						}
					} else {
					...
					if !spans.is_empty() {
						let highlighted_lines = highlight_code_to_styled_lines(&text, &spans, theme);
						...
						let code_style = Style::default().fg(theme.popup_text_fg).bg(theme.inline_code_bg);
			...
			Event::Code(code) => {
				...
				let style = Style::default().fg(theme.popup_text_fg).bg(theme.inline_code_bg);
				if let Some(line) = lines.last_mut() {
...
mod tests {
	...
	fn test_bold_text() {
		...
		assert!(
			bold_span.unwrap().style.add_modifier.contains(Modifier::BOLD),
	...
	fn test_italic_text() {
		...
		assert!(
			italic_span.unwrap().style.add_modifier.contains(Modifier::ITALIC),
	...
	fn test_strikethrough_text() {
		...
		assert!(
			strike_span.unwrap().style.add_modifier.contains(Modifier::CROSSED_OUT),
	...
	fn test_inline_code() {
		...
		assert!(code_span.unwrap().style.bg.is_some(), "Inline code should have background color");
	}
	...
	fn test_body_text_uses_popup_text_fg() {
		...
		let span = lines[0].spans.iter().find(|s| s.text.contains("string")).expect("body text span");
	...
	fn test_inline_code_uses_popup_text_fg() {
		...
		let lines = parse_markdown("Use `println!` to print", &theme, None);
		let code_span =
			lines[0].spans.iter().find(|s| s.text.contains("println")).expect("inline code span");
	...
	fn test_unknown_language_code_block_uses_popup_text_fg() {
		...
		let code_line =
			lines.iter().find(|l| l.plain_text().contains("plain text")).expect("code block line");
		let span = code_line.spans.iter().find(|s| s.text.contains("plain")).expect("code span");
		assert_eq!(span.style.fg, Some(theme.popup_text_fg));
	}
	...
	fn test_code_block() {
		...
		let has_bg = code_line.unwrap().spans.iter().any(|s| s.style.bg.is_some());
	...
	fn test_code_block_unknown_language_fallback() {
		...
		let all_text: String = lines.iter().map(|l| l.plain_text()).collect::<Vec<_>>().join("");
		assert!(all_text.contains("some code here"), "Should contain the code");
	...
	fn test_heading() {
		...
		assert!(has_modifier(heading_line, Modifier::BOLD), "Heading should be bold");
	...
	fn test_link() {
		...
		assert!(style.add_modifier.contains(Modifier::UNDERLINED), "Link should be underlined");
	...
	fn test_link_url_stored() {
		...
		assert_eq!(click_span.unwrap().link_url, None, "Non-link span should not have URL");
	}
	...
	fn test_link_at_column() {
		...
		assert_eq!(line.link_at_column(0), None, "Column 0 should not be a link");
		assert_eq!(line.link_at_column(5), None, "Column 5 should not be a link");

		// Column 6-9: "here" - link
		assert_eq!(line.link_at_column(6), Some("https://example.com"), "Column 6 should be the link");
		assert_eq!(line.link_at_column(9), Some("https://example.com"), "Column 9 should be the link");

		// Column 10+: " for more" - no link
		assert_eq!(line.link_at_column(10), None, "Column 10 should not be a link");
	}
	...
	fn test_paragraph_separation() {
		...
		assert!(lines[1].spans.is_empty(), "Second line should be empty (paragraph break)");
	...
	fn test_soft_break_becomes_newline() {
		...
		assert!(lines.len() >= 2, "Soft break should create separate lines, got {} lines", lines.len());
		let all_text: String = lines.iter().map(|l| l.plain_text()).collect();
		assert!(all_text.contains("one") && all_text.contains("two"), "Should contain both lines");
	}
	...
	fn test_nested_formatting() {
		...
		assert!(style.add_modifier.contains(Modifier::BOLD), "Should be bold");
		assert!(style.add_modifier.contains(Modifier::ITALIC), "Should be italic");
	}
	...
	fn test_lsp_hover_docstring() {
		...
		let all_text: String = lines.iter().map(|l| l.plain_text()).collect();
		assert!(all_text.contains("PurePath subclass"), "Should contain docstring");
	}
	...
	fn test_only_whitespace() {
		...
		for line in &lines {
			...
			assert!(text.trim().is_empty(), "Whitespace-only input should not produce content");
		}
	}
	...
	fn test_wrap_text_line_at_word_boundaries() {
		...
		for (i, line) in wrapped.iter().enumerate() {
			...
			if !line.is_empty() {
				assert!(!line.starts_with(' '), "Line {} should not start with space: {:?}", i, line);
			}
		...
		let wrapped_words: Vec<&str> =
			wrapped.iter().flat_map(|line| line.split_whitespace()).collect();
		assert_eq!(
		...
		assert_eq!(wrapped[2], "offers methods", "Third line should contain remaining words");
	}
	...
	fn test_wrap_text_line_long_word() {
		...
		assert!(wrapped.len() > 1, "Long word should be split into multiple lines");
	...
	fn test_wrap_styled_lines_long_hover_content() {
		...
		let original_text: String =
			lines.iter().flat_map(|l| l.spans.iter().map(|s| s.text.as_str())).collect();
	...
	fn test_wrap_text_lines_multiple() {
		...
		assert!(wrapped.iter().any(|l| l.is_empty()), "Should preserve empty lines");
	...
	fn test_signature_help_doc_indent_preserved() {
		...
		eprintln!("[TEST] orig_indent={}, cont_indent={}", orig_indent, cont_indent);
		assert_eq!(cont_indent, orig_indent, "Continuation line should have same indent as original");
	}
}
