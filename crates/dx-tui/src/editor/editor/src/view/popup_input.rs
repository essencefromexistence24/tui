...
mod tests {
	...
	fn create_popup_with_items(count: usize) -> PopupManager {
		...
		let items: Vec<PopupListItem> =
			(0..count).map(|i| PopupListItem::new(format!("Item {}", i))).collect();
	...
	fn test_popup_enter_confirms() {
		...
		assert!(ctx.deferred_actions.iter().any(|a| matches!(a, DeferredAction::ConfirmPopup)));
	}
	...
	fn test_popup_escape_cancels() {
		...
		assert!(ctx.deferred_actions.iter().any(|a| matches!(a, DeferredAction::ClosePopup)));
	}
