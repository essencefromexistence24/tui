...
impl Viewport {
	...
	pub fn new(width: u16, height: u16) -> Self {
		Self {
			...
			wrap_row_cache: crate::view::line_wrap_cache::LineWrapCache::with_byte_budget(512 * 1024),
		}
	}
	...
	fn containing_hidden_range(
		...
		hidden_ranges.iter().find(|&&(start, end)| pos >= start && pos < end).copied()
	}
	...
	fn count_visual_rows_for_line(
		...
		if !line_breaks.is_empty() {
			...
			let effective_width =
				wrap_config.first_line_width.saturating_add(wrap_config.gutter_width).max(2);
...
impl Viewport {
	...
	pub fn top_visual_row_source_byte(
		...
		let touches_soft_break = soft_breaks.iter().any(|(p, _)| *p >= line_start && *p < line_end);
		let touches_virtual = virtual_lines.iter().any(|p| *p >= line_start && *p <= line_end);
		...
		let wrap_config =
			WrapConfig::new(self.effective_width() as usize, gutter_width, true, self.wrap_indent);
		let effective_width =
			wrap_config.first_line_width.saturating_add(wrap_config.gutter_width).max(2);
		...
		let idx = self.top_view_line_offset.min(seg_bytes.len().saturating_sub(1));
	...
	pub fn center_on_position(&mut self, buffer: &mut Buffer, position: usize) {
		...
		let match_row_in_line = if position > line_start {
			...
			Self::count_visual_rows_for_line(line_start, position, &prefix, &wrap_config, &[], &[], None)
				.saturating_sub(1)
		} else {
			0
		};
	...
	fn find_view_line_for_byte(&self, view_lines: &[ViewLine], target_byte: usize) -> usize {
		...
		if let Some(last) = view_lines.last() {
			...
			{
				let best_max =
					view_lines[best_match].char_source_bytes.iter().filter_map(|b| *b).max().unwrap_or(0);
	...
	fn snap_to_logical_line_start(&mut self, view_lines: &[ViewLine], target_idx: usize) {
		...
		if target_is_wrap_continuation {
			...
			if let Some(new_top_byte) = self.get_source_byte_for_view_line(view_lines, line_start_idx) {
				self.top_byte = new_top_byte;
			}
	...
	fn scroll_cursor_within_margin(
		...
		let effective_offset =
			if apply_margin { self.scroll_offset.min(viewport_height / 2) } else { 0 };
	...
	fn scroll_cursor_column_into_view(
		...
		let line_visual_width = line.visual_width().saturating_sub(usize::from(line.ends_with_newline));
	...
	fn ensure_column_visible_simple(
		...
		let visible_width =
			(self.width as usize).saturating_sub(gutter_width).saturating_sub(scrollbar_width);
	...
	fn set_top_byte_with_limit(
		...
		tracing::trace!("DEBUG set_top_byte_with_limit: proposed_top_byte={}", proposed_top_byte);
	...
	fn clamp_top_byte_unwrapped(
		...
		while iter.next_line().is_some() {
			...
			if lines_visible >= viewport_height {
				...
				tracing::trace!("DEBUG: Full viewport available, setting top_byte={}", proposed_top_byte);
				self.top_byte = proposed_top_byte;
				return;
			}
		}
		...
		tracing::trace!("DEBUG: Backtracking from byte {}", backtrack_iter.current_position());
		for i in 0..lines_short {
			let pos_before = backtrack_iter.current_position();
			if backtrack_iter.prev().is_none() {
				tracing::trace!("DEBUG: Hit beginning of buffer at backtrack iteration {}", i);
				break; // Hit the beginning of the buffer
			}
			let pos_after = backtrack_iter.current_position();
			tracing::trace!("DEBUG: Backtrack iteration {}: {} -> {}", i, pos_before, pos_after);
		}

		let final_top_byte = backtrack_iter.current_position();
		tracing::trace!("DEBUG: After backtracking, setting top_byte={}", final_top_byte);
	...
	fn check_wrapped_visibility(
		...
		loop {
			...
			if let Some((_, line_content)) = iter.next_line() {
				let layout = Self::compute_line_layout(line_content.trim_end_matches('\n'), &wrap_config);
		...
		while visual_rows_counted < target_visual_rows {
			...
			if let Some((_, line_content)) = iter.next_line() {
				...
				if cursor_near_top && new_total >= target_visual_rows {
					let rows_from_this_line = target_visual_rows.saturating_sub(visual_rows_counted);
	...
	pub fn cursor_screen_position(&self, buffer: &mut Buffer, cursor: &Cursor) -> (u16, u16) {
		...
		let wrap_config = if self.line_wrap_enabled {
			let gutter_width = self.gutter_width(buffer);
			Some(WrapConfig::new(self.effective_width() as usize, gutter_width, true, self.wrap_indent))
		} else {
			None
		};
		...
		while let Some((line_byte, content)) = iter.next_line() {
			...
			if let Some(ref config) = wrap_config {
				...
				let line_text = content.trim_end_matches(['\n', '\r']);
				screen_row +=
					Self::count_visual_rows_for_line(line_byte, line_end, line_text, config, &[], &[], None);
			} else {
				screen_row += 1;
			}
		}
		...
		let (screen_col, additional_rows) = if let Some(ref config) = wrap_config {
			...
			// the cursor.
			let effective_width = config.first_line_width.saturating_add(config.gutter_width).max(2);
...
mod tests {
	...
	fn test_ensure_line_visible() {
		let mut buffer = Buffer::from_str_test(
			"line1\nline2\nline3\nline4\nline5\nline6\nline7\nline8\nline9\nline10\nline11\nline12\nline13\nline14\nline15\nline16\nline17\nline18\nline19\nline20\nline21\nline22\nline23\nline24\nline25\nline26\nline27\nline28\nline29\nline30\nline31\nline32\nline33\nline34\nline35\nline36\nline37\nline38\nline39\nline40\nline41\nline42\nline43\nline44\nline45\nline46\nline47\nline48\nline49\nline50\nline51",
		);
		let mut vp = Viewport::new(80, 24);
		vp.scroll_offset = 3;
	...
	fn test_cursor_screen_position_with_wrapped_lines_above() {
		...
		assert!(y >= 6, "expected cursor visual row >= 6 (3 wrapped lines above × >=2 rows), got {y}");
	}
	...
	fn test_ensure_column_visible_resets_to_zero() {
		...
		assert!(vp.left_column > 0, "Should have scrolled right, but left_column = {}", vp.left_column);
