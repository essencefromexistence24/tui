...
fn opening_for_closing(ch: char) -> Option<char> {
	BRACKET_PAIRS.iter().find_map(|(open, close)| if *close == ch { Some(*open) } else { None })
}
...
impl BracketHighlightOverlay {
	...
	pub fn update(
		...
		let matching_pos =
			self.find_matching_bracket(buffer, cursor_position, opening, closing, forward, skip_ranges);
		...
		if let Some(match_pos) = matching_pos {
			...
			let match_overlay =
				Overlay::with_namespace(marker_list, match_pos..match_pos + 1, match_face, ns.clone())
					.with_priority_value(10);
	...
	fn calculate_nesting_depth(
		...
		if is_opening { stack.len() } else { stack.len().saturating_sub(1) }
	}
		...
		for (idx, byte) in bytes.iter().enumerate() {
			...
			if is_opening_bracket(c) {
				...
				if pos >= viewport_start {
					...
					let overlay = Overlay::with_namespace(marker_list, pos..pos + 1, face, ns.clone())
...
mod tests {
	...
	fn test_find_matching_bracket_skips_comment_bracket() {
		...
		assert_eq!(overlay.find_matching_bracket(&buffer, 0, '(', ')', true, &[]), Some(2));

		// Treat byte 2 (the first `)`) as inside a comment: it should be
		// skipped, so the match becomes the `)` at 4.
		assert_eq!(overlay.find_matching_bracket(&buffer, 0, '(', ')', true, &[2..3]), Some(4));
	}
	...
	fn test_nesting_depth_skips_comment_brackets() {
		...
		assert_eq!(overlay.calculate_nesting_depth(&buffer, 2, true, &[1..2]), 1);
	}
}
