...
mod tests {
	...
	fn test_character_keys_defer_to_handler() {
		...
		assert!(matches!(ctx.deferred_actions[0], DeferredAction::InteractiveReplaceKey('y')));
	}
	...
	fn test_escape_cancels() {
		...
		assert!(matches!(result, InputResult::Consumed));
		assert_eq!(ctx.deferred_actions.len(), 1);
		assert!(matches!(ctx.deferred_actions[0], DeferredAction::CancelInteractiveReplace));
	}
