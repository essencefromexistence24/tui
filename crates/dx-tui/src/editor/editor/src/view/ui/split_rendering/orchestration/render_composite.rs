...
use crate::view::ui::view_pipeline::{ViewLine, should_show_line_number};
...
pub(crate) fn render_composite_buffer(
	...
	let effective_editor_bg = if use_terminal_bg { Color::Reset } else { theme.editor_bg };
	...
	render_pane_headers(buf, area, composite, &layout, view_state.focused_pane, theme);
...
fn compute_pane_layout(composite: &CompositeBuffer, area_width: u16) -> PaneLayout {
	...
	PaneLayout { widths, separator_width, show_separator }
}
...
fn build_pane_render_data(
	...
	for (pane_idx, source) in composite.sources.iter().enumerate() {
		...
		let (Some(first_line), Some(last_line)) =
			(visible_lines.iter().copied().min(), visible_lines.iter().copied().max())
		else {
			pane_render_data.push(None);
			continue;
		};
		...
		let end_byte =
			source_state.buffer.line_start_offset(last_line + 1).unwrap_or(source_state.buffer.len());
...
fn render_past_end_row(
	...
	for &width in &layout.widths {
		...
		let eof =
			Paragraph::new(text).style(Style::default().fg(theme.line_number_fg).bg(theme.after_eof_bg));
	...
	for (pane_idx, &width) in layout.widths.iter().enumerate() {
		...
		if layout.show_separator && pane_idx < layout.pane_count() - 1 {
			...
			let sep = Paragraph::new("│")
				.style(Style::default().fg(theme.split_separator_fg).bg(theme.editor_bg));
...
fn render_row_pane(
	...
	let left_column = view_state.get_pane_viewport(pane_idx).map(|v| v.left_column).unwrap_or(0);
	...
	let Some(source_line_ref) = aligned_row.get_pane_line(pane_idx) else {
		...
		let pane_has_selection = is_focused_pane
			&& selection_cols.map(|(start, end)| start == 0 && end == usize::MAX).unwrap_or(false);
	...
	let view_line_opt = pane_data.and_then(|data| {
		data.line_to_view_line.get(&source_line_ref.line).and_then(|&idx| data.lines.get(idx))
	});
	let highlight_spans = pane_data.map(|data| data.highlight_spans.as_slice()).unwrap_or(&[]);
	...
	let pane_selection_cols = if is_focused_pane { selection_cols } else { None };
...
fn render_view_line_content(
	...
	for (char_idx, ch) in chars.iter().enumerate() {
		...
		let in_selection =
			selection_cols.map(|(start, end)| col >= start && col < end).unwrap_or(false);
