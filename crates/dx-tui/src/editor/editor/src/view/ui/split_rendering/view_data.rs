...
use dx_core::api::ViewTransformPayload;
...
fn md_syntax_namespace() -> dx_core::overlay::OverlayNamespace {
	dx_core::overlay::OverlayNamespace::from_string("md-syntax".to_string())
}
...
pub(super) fn build_view_data(
	...
	if is_compose && !state.soft_breaks.is_empty() {
		let viewport_end =
			tokens.iter().filter_map(|t| t.source_offset).next_back().unwrap_or(viewport.top_byte) + 1;
		let soft_breaks =
			state.soft_breaks.query_viewport(viewport.top_byte, viewport_end, &state.marker_list);
	...
	let effective_width = if line_wrap_enabled {
		let base =
			if let Some(col) = viewport.wrap_column { col.min(content_width) } else { content_width };
		...
		let viewport_end = tokens
			...
			.filter_map(|t| {
				...
				let len = match &t.kind {
					dx_core::api::ViewTokenWireKind::Text(s) => s.len(),
					_ => 1,
				};
				Some(start + len)
			})
	...
	let at_buffer_end = if has_view_transform {
		...
		let max_source_offset = tokens.iter().filter_map(|t| t.source_offset).max().unwrap_or(0);
	...
	{
		use crate::view::line_wrap_cache::{CacheViewMode, LineWrapKey, pipeline_inputs_version};
		...
		while i < source_lines.len() {
			...
			if !has_injected {
				...
				state.line_wrap_cache.put(make_key(line_start_byte, cache_view_mode), arc.clone());
				...
				if !matches!(cache_view_mode, CacheViewMode::Source) {
					state.line_wrap_cache.put(make_key(line_start_byte, CacheViewMode::Source), arc);
				}
			}
			i = j;
		}
	...
	let lines = inject_virtual_lines(source_lines, state, theme, virtual_line_wrap_width, is_compose);
	let placeholder_style = fold_placeholder_style(theme);
	let lines = apply_folding(lines, &state.buffer, &state.marker_list, folds, &placeholder_style);
