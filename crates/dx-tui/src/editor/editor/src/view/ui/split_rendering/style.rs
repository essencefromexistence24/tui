...
use crate::view::theme::{Theme, TokenColorExt, color_to_rgb};
...
pub(super) fn fold_placeholder_style(theme: &Theme) -> ViewTokenStyle {
	...
	ViewTokenStyle { fg, bg: None, bold: false, italic: true, underline: false }
}
...
pub(super) fn dim_color_for_tilde(color: Color) -> Color {
	match color {
		...
		Color::Indexed(idx) => {
			...
			if idx < 16 { Color::Rgb(50, 50, 50) } else { Color::Rgb(40, 40, 40) }
		}
...
pub(super) fn create_wrapped_virtual_lines(
	...
	text_overlays: &[dx_core::api::VirtualLineTextOverlay],
) -> Vec<ViewLine> {
	...
	if chunk_ranges.is_empty() {
		...
		return vec![build_virtual_view_line("", &token_style, gutter_glyph, 0, text_overlays)];
	}
...
mod tests {
	...
	fn create_wrapped_virtual_lines_prefers_word_boundary() {
		...
		assert!(first.ends_with(' '), "expected first row to end at a word boundary; got {first:?}",);
	}
}
