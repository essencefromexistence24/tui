...
fn back_up_to_prior_space(
	...
	if !wrapped[..space_idx].iter().any(|t| !matches!(t.kind, ViewTokenWireKind::Space)) {
		return None;
	}
	...
	Some(BackUpPlan { tail_start, tail_width })
}
...
pub(crate) fn apply_wrapping_transform(
	...
	/// double-count it.
	#[inline]
	fn effective_width(available_width: usize, _line_indent: usize, _on_continuation: bool) -> usize {
		available_width
	}
		...
		match &token.kind {
			...
			ViewTokenWireKind::Text(text) => {
					...
					&& (text_visual_width <= fresh_line_capacity || current_line_width >= row_floor)
				{
					...
					emit_break_with_indent(&mut wrapped, &mut current_line_width, &cached_indent_string);
				}
				...
				if current_line_width + text_visual_width > eff_width && !ansi::contains_ansi_codes(text) {
					...
					while grapheme_idx < graphemes.len() {
						...
						for &(_byte_offset, grapheme) in &graphemes[grapheme_idx..] {
							...
							if chunk_visual_width + g_width > remaining_width && chunk_grapheme_count > 0 {
								break;
							}
						...
						if chunk_grapheme_count > 1 {
							...
							let row_floor = eff_width.saturating_sub(MAX_LOOKBACK).max(eff_width / 2);
							let chunk_floor_from_cursor = row_floor.saturating_sub(current_line_width);
							...
							let mut best_target_byte =
								word_bounds[wb_lo..wb_hi].iter().rev().copied().find(|&b| b >= floor_byte);
								...
								&& end_byte <= slice_end_hard
							{
								...
								// both sources.
								if end_byte <= slice_end_hard && end_byte >= floor_byte && end_byte > b {
									best_target_byte = Some(end_byte);
								}
							}
							...
							if let Some(target_byte) = best_target_byte {
								...
								if new_count > 0 && new_count < chunk_grapheme_count {
									...
									for &(_b, g) in &graphemes[grapheme_idx..grapheme_idx + new_count] {
										let w = if g == "\t" {
											visual_layout::tab_expansion_width(col)
										} else {
						...
						let chunk_end_byte = if grapheme_idx + chunk_grapheme_count < graphemes.len() {
							graphemes[grapheme_idx + chunk_grapheme_count].0
						} else {
							text.len()
						};
			...
			ViewTokenWireKind::Space => {
				...
				if current_line_width + 1 > eff_width {
					...
					if let Some(BackUpPlan { tail_start, tail_width }) = back_up {
	...
	for token in tokens {
		...
		match &token.kind {
			...
			ViewTokenWireKind::Space | ViewTokenWireKind::Newline | ViewTokenWireKind::Break => {
				if let Some(cidx) = is_concealed(conceal_ranges, &sorted, &mut conceal_cursor, offset) {
...
pub(super) fn inject_virtual_lines(
	...
	let viewport_start =
		source_lines.first().and_then(|l| l.char_source_bytes.iter().find_map(|m| *m)).unwrap_or(0);
	...
	let mut virtual_lines =
		state.virtual_texts.query_lines_in_range(&state.marker_list, viewport_start, viewport_end);
	...
	for source_line in source_lines {
		...
		let line_end_byte = source_line.char_source_bytes.iter().rev().find_map(|m| *m).map(|b| b + 1);
			...
			for (anchor_pos, vtext) in &virtual_lines {
					...
					&& vtext.position == VirtualTextPosition::LineAbove
				{
					let glyph = vtext
						.gutter_glyph
						.as_ref()
						.map(|g| (g.clone(), vtext.gutter_color.unwrap_or(theme.line_number_fg)));
...
pub fn splice_inline_virtual_text(
	...
	let inline = state.virtual_texts.query_inline_in_range(&state.marker_list, start, end);
	if inline.is_empty() {
		return tokens;
	}
	...
	for (pos, vtext) in inline {
		...
		match vtext.position {
			VirtualTextPosition::BeforeChar => {
				before.entry(pos).or_default().push((vtext.text.clone(), style));
			}
			VirtualTextPosition::AfterChar => {
				after
					.entry(pos)
					.or_default()
					.push(InlineHintCell { text: format!(" {}", vtext.text), style });
			}
	...
	for token in tokens {
		...
		match (&token.kind, src) {
			...
			(kind, Some(anchor)) => {
				...
				if let Some(hints) = before.get(&anchor) {
					for (text, style) in hints {
						let padded = if anchor_is_newline { format!(" {text} ") } else { format!("{text} ") };
