...
use dx_core::api::{ViewTokenStyle, ViewTokenWire, ViewTokenWireKind};
...
impl<'a> ViewLineIterator<'a> {
	...
	fn skip_folded_tokens(&mut self) {
		while self.token_idx < self.tokens.len() {
			...
			let in_skip =
				self.fold_skip.get(self.fold_cursor).is_some_and(|r| r.start <= offset && offset < r.end);
			if !in_skip {
				return;
			}
			self.token_idx += 1;
		...
		while byte_idx < t_bytes.len() {
			...
			let valid = match std::str::from_utf8(remaining) {
				...
				Err(e) => {
					...
					if valid_up_to == 0 {
						...
						if self.binary_mode {
							acc.push_escape(&format_unprintable_byte(b), source, token_style.clone());
						} else {
			...
			for (g_byte_offset, grapheme) in valid.grapheme_indices(true) {
					...
					if has_unprintable {
						...
						for ch in grapheme.chars() {
							...
							if ch_byte < 0x80 && is_unprintable_byte(ch_byte as u8) {
								acc.push_escape(&format_unprintable_byte(ch_byte as u8), src, token_style.clone());
							} else {
				...
				if grapheme == "\t" {
					...
					acc.push_tab(source, token_style.clone(), self.tab_expansion_width(acc.col));
					continue;
				}
...
impl<'a> Iterator for ViewLineIterator<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let mut ansi_parser = if self.ansi_aware { Some(AnsiParser::new()) } else { None };
		...
		while self.token_idx < self.tokens.len() {
			...
			match &token.kind {
				ViewTokenWireKind::Text(t) => {
					self.push_text_token(t, token.source_offset, token_style, &mut acc, &mut ansi_parser);
					self.token_idx += 1;
				}
				...
				ViewTokenWireKind::BinaryByte(b) => {
					...
					acc.push_escape(&format_unprintable_byte(*b), token.source_offset, token_style);
					self.token_idx += 1;
				}
			}
		}
...
pub fn should_show_line_number(line: &ViewLine) -> bool {
	...
	if line.char_source_bytes.is_empty() {
		...
		return matches!(line.line_start, LineStart::Beginning | LineStart::AfterSourceNewline);
	}

	let first_char_is_source = line.char_source_bytes.first().map(|m| m.is_some()).unwrap_or(false);
...
impl Layout {
	...
	pub fn new(lines: Vec<ViewLine>, source_range: Range<usize>) -> Self {
		...
		Self { lines, source_range, total_view_lines, total_injected_lines, byte_to_line }
	}
	...
	pub fn get_source_byte_for_line(&self, line_idx: usize) -> Option<usize> {
		...
		self.lines[line_idx].char_source_bytes.iter().find_map(|m| *m)
	}
...
mod tests {
	...
	fn make_text_token(text: &str, source_offset: Option<usize>) -> ViewTokenWire {
		ViewTokenWire { kind: ViewTokenWireKind::Text(text.to_string()), source_offset, style: None }
	}

	fn make_newline_token(source_offset: Option<usize>) -> ViewTokenWire {
		ViewTokenWire { kind: ViewTokenWireKind::Newline, source_offset, style: None }
	}

	fn make_break_token() -> ViewTokenWire {
		ViewTokenWire { kind: ViewTokenWireKind::Break, source_offset: None, style: None }
	}
	...
	fn test_injected_header_then_source() {
		...
		assert!(
			...
			lines[1].line_start,
			lines[1].char_source_bytes.first().map(|m| m.is_some()).unwrap_or(false)
		);
	}
	...
	fn test_double_width_visual_mappings() {
		...
		assert_eq!(lines[0].source_byte_at_visual_col(0), Some(0), "Column 0 should map to byte 0");
		assert_eq!(lines[0].source_byte_at_visual_col(1), Some(0), "Column 1 should map to byte 0");

		// Both columns of 好 should map to byte 3
		assert_eq!(lines[0].source_byte_at_visual_col(2), Some(3), "Column 2 should map to byte 3");
		assert_eq!(lines[0].source_byte_at_visual_col(3), Some(3), "Column 3 should map to byte 3");
	...
	fn test_mixed_width_visual_mappings() {
		...
		assert_eq!(lines[0].source_byte_at_visual_col(0), Some(0), "Column 0 (a) should map to byte 0");
		assert_eq!(
		...
		assert_eq!(lines[0].source_byte_at_visual_col(3), Some(4), "Column 3 (b) should map to byte 4");
		assert_eq!(
			lines[0].source_byte_at_visual_col(4),
			Some(5),
	...
	fn test_crlf_char_source_bytes_single_line() {
		...
		assert_eq!(lines[0].char_source_bytes.len(), 4, "Expected 4 chars: a, b, c, newline");
		assert_eq!(lines[0].char_source_bytes[0], Some(0), "char 'a' should map to byte 0");
		assert_eq!(lines[0].char_source_bytes[1], Some(1), "char 'b' should map to byte 1");
		assert_eq!(lines[0].char_source_bytes[2], Some(2), "char 'c' should map to byte 2");
		assert_eq!(
	...
	fn test_adjacent_tabs_visual_cols_monotonic() {
		...
		let tokens = vec![make_text_token("\t\t", Some(0)), make_newline_token(Some(2))];
	...
	fn test_crlf_visual_to_source_mapping() {
		...
		assert_eq!(lines[0].source_byte_at_visual_col(2), Some(2), "Line 1 col 2 (newline)");
		...
		assert_eq!(lines[1].source_byte_at_visual_col(2), Some(6), "Line 2 col 2 (newline)");
	}
}
