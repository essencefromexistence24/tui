...
use super::super::layout::{
	ComposeLayout, calculate_compose_layout, calculate_view_anchor, calculate_viewport_end,
};
...
use super::render_line::{LastLineEnd, LineRenderInput, LineRenderOutput, render_view_lines};
...
pub(crate) fn resolve_cursor_fallback(
	...
	if buffer_ends_with_newline {
		if let Some(end) = last_line_end {
			...
			let y = if end.terminated_with_newline { end.pos.1.saturating_add(1) } else { end.pos.1 };
...
pub(crate) fn compute_buffer_layout(
	...
	state.margins.update_width_for_buffer(estimated_lines, show_line_numbers);
	...
	if !selection.cursor_positions.is_empty()
		&& !selection.cursor_positions.contains(&selection.primary_cursor_position)
	{
		tracing::warn!(
	...
	let _ = state.buffer.populate_line_cache(viewport.top_byte, adjusted_visible_count);
...
pub(crate) fn draw_buffer_in_split(
	...
	let editor_block =
		Block::default().borders(Borders::NONE).style(Style::default().bg(effective_editor_bg));
	...
	let cursor = cursor.map(|(cx, cy)| {
		...
		if selection.primary_virtual_lines > 0 {
			let x = gutter_width as u16
				+ selection.primary_virtual_line_col.saturating_sub(layout_output.left_column) as u16;
	...
	if let Some(guides) = compose_column_guides {
		let guide_style = Style::default().fg(theme.line_number_fg).add_modifier(Modifier::DIM);
...
pub(crate) fn render_buffer_in_split(
	...
	let crate::view::ui::RenderStyle { theme, ansi_background, cfg } = style;
