...
pub(crate) fn selection_context(state: &EditorState, cursors: &Cursors) -> SelectionContext {
	...
	let mut ranges: Vec<Range<usize>> = cursors
		...
		.filter_map(|(_, cursor)| {
			...
			if cursor.selection_mode == SelectionMode::Block { None } else { cursor.selection_range() }
		})
		.collect();
	...
	for (_, cursor) in cursors.iter() {
		let vcols = crate::model::virtual_space::cursor_virtual_columns(vs_mode, &state.buffer, cursor);
		if vcols > 0 {
			let entry = virtual_cols_at.entry(cursor.position).or_insert(0);
			*entry = (*entry).max(vcols);
		}
	}
	let primary_virtual_cols =
		crate::model::virtual_space::cursor_virtual_columns(vs_mode, &state.buffer, cursors.primary());
	let primary_virtual_lines =
		crate::model::virtual_space::cursor_virtual_lines(vs_mode, &state.buffer, cursors.primary());
	let primary_virtual_line_col =
		if primary_virtual_lines > 0 { cursors.primary().sticky_column.unwrap_or(0) } else { 0 };
...
pub(crate) fn decoration_context(
	...
	use dx_languages::HighlightCategory;
	let mut bracket_skip_ranges: Vec<std::ops::Range<usize>> = highlight_spans
		.iter()
		.filter(|span| {
			matches!(span.category, Some(HighlightCategory::Comment) | Some(HighlightCategory::String))
		})
	...
	let md_emphasis_ns = dx_core::overlay::OverlayNamespace::from_string("md-emphasis".to_string());
	let mut semantic_token_spans = Vec::new();
	let mut viewport_overlays = Vec::new();
	for (overlay, range) in
		state.overlays.query_viewport(viewport_start, viewport_end, &state.marker_list)
	{
	...
	let diagnostic_lines: HashSet<usize> = viewport_overlays
		...
		.filter_map(|(overlay, range)| {
			if overlay.namespace.as_ref() == Some(&diagnostic_ns) {
				return Some(indent_folding::find_line_start_byte(&state.buffer, range.start));
			}
			None
		})
		.collect();
	...
	let diagnostic_inline_texts: HashMap<usize, (String, Style)> = if diagnostics_inline_text {
		...
		for (overlay, range) in &viewport_overlays {
			...
			if let Some(ref message) = overlay.message {
				...
				let dominated =
					by_line.get(&line_start).is_some_and(|(_, _, existing_pri)| *existing_pri >= priority);
				if !dominated {
		...
		by_line.into_iter().map(|(k, (msg, style, _))| (k, (msg, style))).collect()
	} else {
		HashMap::new()
	};
	...
	let mut line_indicators =
		state.margins.get_indicators_for_viewport(viewport_start, viewport_end, |byte_offset| {
