...
use super::layout::{
	SplitLayout, render_separator, resolve_view_preferences, split_buffers_for_tabs, split_layout,
...
pub(crate) fn render_content(
	...
	Vec<(crate::model::event::ContainerId, SplitDirection, u16, u16, u16)>, // hit areas for separators inside active Grouped subtrees
) {
	...
	for (main_split_id, split_id, buffer_id, split_area, kind) in visible_buffers {
		...
		let layout = if is_inner_group_leaf {
			...
			SplitLayout {
				...
				content_rect: Rect::new(
					split_area.x,
					split_area.y,
					split_area.width.saturating_sub(if panel_show_vscroll { 1 } else { 0 }),
					split_area.height,
				),
		...
		let tab_hover_for_split = hovered_tab.and_then(|(hover_buf, hover_split, is_close)| {
			if hover_split == split_id { Some((hover_buf, is_close)) } else { None }
		});
		...
		let is_synthetic_placeholder =
			buffer_metadata.get(&buffer_id).is_some_and(|m| m.synthetic_placeholder);
		...
		if buffers.get(&buffer_id).is_some_and(|s| s.is_composite_buffer) {
		...
		if let Some(state) = state_opt {
			...
			let viewport_clone = view_state_opt.map(|vs| vs.viewport.clone()).unwrap_or_else(|| {
				crate::view::viewport::Viewport::new(layout.content_rect.width, layout.content_rect.height)
			});
			...
			let has_selection = hide_current_line_on_selection
				&& split_cursors.iter().any(|(_, c)| c.selection_range().is_some());
			...
			let is_virtual_buffer = buffer_metadata.get(&buffer_id).is_some_and(|m| m.is_virtual());
			let effective_rulers: &[usize] = if is_virtual_buffer { &[] } else { &view_prefs.rulers };
...
fn render_split_tab_bar(
	...
	if show_close_btn {
		...
		let close_fg = if is_hovered { theme.tab_close_hover_fg } else { theme.line_number_fg };
		let close_button =
	...
	if show_maximize_btn {
		...
		let max_fg = if is_hovered { theme.tab_close_hover_fg } else { theme.line_number_fg };
...
fn render_composite_split(
	...
	let initial_focus_hunk =
		composite_buffers.get_mut(&buffer_id).and_then(|c| c.initial_focus_hunk.take());
	...
	if let Some(svs) = split_view_states {
		if let Some(split_vs) = svs.get_mut(&split_id) {
			...
			{
				split_vs.viewport.resize(layout.content_rect.width, layout.content_rect.height);
			}
		}
	}
	...
	let view_state = composite_view_states
		...
		.or_insert_with(|| crate::view::composite_view::CompositeViewState::new(buffer_id, pane_count));
...
fn render_grouped_separators(
	...
	for (main_split_id, _main_buffer_id, split_area) in base_visible {
		let active_group =
			split_view_states.and_then(|svs| svs.get(main_split_id)).and_then(|vs| vs.active_group_tab);
		...
		if let crate::view::split::SplitNode::Grouped { layout, .. } = grouped {
			for (id, direction, x, y, length) in layout.get_separators_with_ids(main_layout.content_rect)
			{
				render_separator(buf, direction, x, y, length, theme);
	...
	for (_, _, _, scrollbar_rect, thumb_start, thumb_end) in split_areas {
		for row in 0..scrollbar_rect.height {
			...
			sb_runs.push(crate::app::types::ThemeRun {
				...
				fg_key: Some(if is_thumb { "ui.scrollbar_thumb_fg" } else { "ui.scrollbar_track_fg" }),
				bg_key: Some("editor.bg"),
				region: if is_thumb { "Scrollbar Thumb" } else { "Scrollbar Track" },
			});
		}
	}
...
pub(crate) fn compute_content_layout(
	...
	for (split_id, buffer_id, split_area) in visible_buffers {
		...
		// Suppress chrome (tab bar) for splits in buffer groups
		let split_tab_bar_visible =
			tab_bar_visible && !split_view_states.get(&split_id).is_some_and(|vs| vs.suppress_chrome);
		...
		// Get viewport from SplitViewState (authoritative source)
		let viewport_clone =
			split_view_states.get(&split_id).map(|vs| vs.viewport.clone()).unwrap_or_else(|| {
				crate::view::viewport::Viewport::new(layout.content_rect.width, layout.content_rect.height)
			});
		let mut viewport = viewport_clone;
		...
		// Get cursors from the split's view state
		let split_cursors =
			split_view_states.get(&split_id).map(|vs| vs.cursors.clone()).unwrap_or_default();
		// Resolve hidden fold byte ranges so ensure_visible can skip
		// folded lines when counting distance to the cursor.
		let hidden_ranges: Vec<(usize, usize)> = split_view_states
			.get(&split_id)
		...
		let effective_highlight_current_line = view_prefs.highlight_current_line && state.show_cursors;

		let mut empty_folds = FoldManager::new();
		let folds =
			split_view_states.get_mut(&split_id).map(|vs| &mut vs.folds).unwrap_or(&mut empty_folds);
...
fn render_placeholder_hint(
	...
	const HINT: &str = "Ctrl+P  command palette   ·   Ctrl+O  open file   ·   Ctrl+E  file explorer";
