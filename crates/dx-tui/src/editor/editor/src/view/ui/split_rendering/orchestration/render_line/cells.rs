...
use super::super::super::char_style::{CharStyleContext, CharStyleOutput, compute_char_style};
use super::super::super::spans::{
	DebugSpanTracker, SpanAccumulator, push_debug_tag, push_span_with_map, span_bg_info_at,
	span_color_at, span_info_at,
};
...
use super::{CursorTracker, SpanCursors, cursor_indicator_style};
...
pub(super) fn render_line_cells<'a, 'c>(
	...
	let mut pass = CellPass {
		...
		debug_tracker: input.state.debug_highlight_mode.then(DebugSpanTracker::default),
...
impl CellPass<'_, '_, '_> {
	...
	fn process_char(&mut self, ch: char) -> ControlFlow<()> {
		...
		let byte_pos =
			self.input.view_line.char_source_bytes.get(self.display_char_idx).copied().flatten();
		...
		let Some(ansi_style) = self.parse_ansi(ch) else {
			...
			if byte_pos == Some(self.input.selection.primary_cursor_position) {
				self.cursor.place(self.cell_screen_x(), self.input.current_row);
			}
	...
	fn render_visible_cell(&mut self, ch: char, byte_pos: Option<usize>, ansi_style: Style) {
		...
		let (display_char, is_whitespace_indicator) = if is_indentation_guide {
			let guide_char = self.input.indentation_guide_glyph.trim().chars().next().unwrap_or('▏');
	...
	fn is_indentation_guide_cell(&self, ch: char, byte_pos: Option<usize>) -> bool {
		...
		if matches!(self.input.view_line.line_start, LineStart::AfterInjectedNewline) {
			return false;
		}
		...
		match self.input.indentation_guide {
			...
			IndentationGuideMode::All => self.input.indentation_guide_columns.contains(&self.col_offset),
	...
	fn cursor_hits_cell(&self, byte_pos: Option<usize>) -> bool {
		...
		if !self.input.selection.cursor_positions.contains(&bp) || bp >= self.input.state.buffer.len() {
			return false;
		}
		...
		let prev_byte_pos =
			self.input.view_line.char_source_bytes.get(prev_char_idx).copied().flatten();
	...
	fn resolve_cell_style(
		...
		let token_style =
			input.view_line.char_styles.get(self.display_char_idx).and_then(|s| s.as_ref());
		...
		let highlight_bg_theme_key = highlight_bg.and(highlight_theme_key).or(highlight_theme_key);
		let semantic_token_color = match byte_pos {
			Some(bp) => {
				span_color_at(&input.decorations.semantic_token_spans, &mut self.span_cursors.semantic, bp)
			}
			None => None,
		};
		...
		// filtering.
		let cell_overlays: &[&Overlay] =
			if byte_pos.is_some() { self.overlay_sweep.at_cursor() } else { &[] };

		let CharStyleOutput { style, is_secondary_cursor, fg_theme_key, bg_theme_key, region } =
	...
	fn emit_cell(&mut self, display_char: &str, style: Style, byte_pos: Option<usize>, ch: char) {
		...
		for c in display_char.chars() {
			self.span_acc.push(c, style, byte_pos, self.line_spans, self.line_view_map);
		}
	...
	fn place_cell_cursor(
		...
		if is_cursor && ch == '\n' {
			let should_add_indicator = if self.input.is_active { is_secondary_cursor } else { true };
