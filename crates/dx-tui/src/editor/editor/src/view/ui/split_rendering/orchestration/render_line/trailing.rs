...
fn render_implicit_line_into(
	...
	let implicit_cursor_bg = if implicit_is_cursor_line && ctx.highlight_current_line && ctx.is_active
	{
	...
	if let Some(bg) = implicit_cursor_bg {
		let gutter_w =
			if ctx.state.margins.left_config.enabled { ctx.state.margins.left_total_width() } else { 0 };
		let content_width = ctx.render_area.width.saturating_sub(gutter_w as u16) as usize;
		if content_width > 0 {
			implicit_line_spans.push(Span::styled(" ".repeat(content_width), Style::default().bg(bg)));
		}
	}
...
fn push_left_margin(
	...
	if ctx.decorations.diagnostic_lines.contains(&implicit_line_byte) {
	...
	let rendered_text = if ctx.byte_offset_mode && ctx.show_line_numbers {
		format!("{:>width$}", implicit_gutter_num, width = ctx.state.margins.left_config.width)
	} else {
		let estimated_lines = ctx
			.state
			.buffer
			.line_count()
			.unwrap_or((ctx.state.buffer.len() / ctx.state.buffer.estimated_line_length()).max(1));
	...
	if ctx.state.margins.left_config.show_separator {
		...
		spans.push(Span::styled(ctx.state.margins.left_config.separator.to_string(), sep_style));
	}
}

fn ensure_trailing_mapping(ctx: &PostRowContext<'_>, acc: &mut PostRowAccumulator<'_>) {
	let last_mapped_byte = acc.view_line_mappings.last().map(|m| m.line_end_byte).unwrap_or(0);
	let near_buffer_end = last_mapped_byte + 2 >= ctx.state.buffer.len();
	let already_mapped = acc
		.view_line_mappings
		.last()
		.is_some_and(|m| m.char_source_bytes.is_empty() && m.line_end_byte == ctx.state.buffer.len());
