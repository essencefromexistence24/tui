...
use super::scrollbar::{ScrollbarColors, ScrollbarState, render_scrollbar};
use super::status_bar::truncate_path;
use crate::app::file_open::{
	FileOpenSection, FileOpenState, SortMode, format_modified, format_size,
};
...
impl FileBrowserRenderer {
	...
	pub fn render(
		...
		let title_line = if truncated_path.truncated {
			Line::from(vec![
				Span::raw(" "),
				Span::styled(truncated_path.prefix.clone(), Style::default().fg(theme.popup_border_fg)),
				Span::styled(
					format!("{}[...]", truncated_path.sep),
					Style::default().fg(theme.menu_highlight_fg),
				),
				Span::styled(truncated_path.suffix.clone(), Style::default().fg(theme.popup_border_fg)),
				Span::raw(" "),
			])
		} else {
			Line::from(title)
		...
		// Header area
		let header_area =
			Rect::new(inner_area.x, inner_area.y + nav_height, content_width, header_height);
		...
		let is_scrollbar_hovered =
			matches!(hover_target, Some(crate::app::HoverTarget::FileBrowserScrollbar));
	...
	fn render_navigation(
		...
		let hidden_shortcut_text =
			if hidden_shortcut.is_empty() { String::new() } else { format!(" ({})", hidden_shortcut) };

		let is_hidden_hovered =
		...
		let hidden_style = if is_hidden_hovered {
			...
			Style::default().fg(theme.menu_highlight_fg).bg(theme.popup_bg)
		} else {
		...
		let hidden_shortcut_style = if is_hidden_hovered {
			...
			Style::default().fg(theme.help_separator_fg).bg(theme.popup_bg)
		};
		...
		checkbox_spans
			.push(Span::styled(" │ ", Style::default().fg(theme.help_separator_fg).bg(theme.popup_bg)));

		// Detect Encoding checkbox with underlined E
		let encoding_icon = if state.detect_encoding { "☑" } else { "☐" };
		let is_encoding_hovered =
			matches!(hover_target, Some(HoverTarget::FileBrowserDetectEncodingCheckbox));
		...
		let encoding_underline_style = if is_encoding_hovered {
			...
			Style::default().fg(theme.help_key_fg).bg(theme.popup_bg).add_modifier(Modifier::UNDERLINED)
		};
		...
		checkbox_spans.push(Span::styled(format!("{} Detect ", encoding_icon), encoding_style));
		...
		if !encoding_shortcut.is_empty() {
			checkbox_spans
				.push(Span::styled(format!(" ({})", encoding_shortcut), encoding_shortcut_style));
		}
		...
		if remaining > 0 {
			checkbox_spans.push(Span::styled(" ".repeat(remaining), Style::default().bg(theme.popup_bg)));
		}
		...
		if nav_remaining > 0 {
			nav_spans.push(Span::styled(" ".repeat(nav_remaining), Style::default().bg(theme.popup_bg)));
		}
	...
	fn render_header(
		...
		let hover_header_style =
			Style::default().fg(theme.menu_hover_fg).bg(theme.menu_hover_bg).add_modifier(Modifier::BOLD);
		...
		let name_header = format!(
			" {}{}",
			t!("file_browser.name"),
			if state.sort_mode == SortMode::Name { sort_arrow } else { " " }
		);
		let is_name_hovered =
			matches!(hover_target, Some(HoverTarget::FileBrowserHeader(SortMode::Name)));
		...
		let size_header = format!(
			...
			format!(
				"{}{}",
				t!("file_browser.size"),
				if state.sort_mode == SortMode::Size { sort_arrow } else { " " }
			),
			width = size_col_width
		);
		let is_size_hovered =
			matches!(hover_target, Some(HoverTarget::FileBrowserHeader(SortMode::Size)));
		...
		let modified_header = format!(
			...
			format!(
				...
				if state.sort_mode == SortMode::Modified { sort_arrow } else { " " }
			),
			width = date_col_width
		);
		let is_modified_hovered =
			matches!(hover_target, Some(HoverTarget::FileBrowserHeader(SortMode::Modified)));
	...
	fn render_file_list(
		...
		if let Some(error) = &state.error {
			let error_line = Line::from(Span::styled(
				...
				Style::default().fg(theme.diagnostic_error_fg).bg(theme.popup_bg),
			));
		...
		for (view_idx, entry) in visible_entries.iter().enumerate() {
			...
			let base_style = if is_selected {
				...
				Style::default().fg(theme.help_separator_fg).bg(theme.popup_bg).add_modifier(Modifier::DIM)
			} else {
		...
		while lines.len() < visible_rows {
			lines.push(Line::from(Span::styled(" ".repeat(width), Style::default().bg(theme.popup_bg))));
		}
