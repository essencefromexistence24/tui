...
impl FileExplorerRenderer {
	...
	pub fn render(
		...
		for row in area.y..area.y + area.height {
			rec.run(area.x, row, area.width, Some("editor.fg"), Some("editor.bg"), "File Explorer");
		}
		...
		// Available width for content (subtract borders, cursor indicator, and scrollbar)
		...
		let items: Vec<ListItem> = visible_items
			...
			.map(|(viewport_idx, &(node_id, indent))| {
				...
				let fuzzy_match = if search_active { view.get_match_for_node(node_id) } else { None };
				Self::render_node(
					view,
					deco,
					node_id,
		...
		let title = if search_active {
			...
			let hostname = host.split('@').next_back().unwrap_or(host).split(':').next().unwrap_or(host);
		...
		let remote_disconnected =
			remote_connection.map(|c| c.contains("(Disconnected)")).unwrap_or(false);
		let (title_style, border_style) = if remote_disconnected {
			...
			(
				Style::default().fg(theme.editor_bg).bg(theme.editor_fg).add_modifier(Modifier::BOLD),
				Style::default().fg(theme.cursor),
			)
		} else {
			(Style::default().fg(theme.line_number_fg), Style::default().fg(theme.split_separator_fg))
		};
		...
		if let Some(selected) = selected_index {
			if selected >= scroll_offset && selected < scroll_offset + viewport_height {
				...
				let bg_key = if is_focused { "editor.selection_bg" } else { "editor.current_line_bg" };
				rec.run(inner_x, row, inner_w, Some("editor.fg"), Some(bg_key), "File Explorer");
			}
		}
		...
		let close_fg =
			if close_button_hovered { theme.tab_close_hover_fg } else { theme.line_number_fg };
		let close_button = ratatui::widgets::Paragraph::new("×").style(Style::default().fg(close_fg));
		...
		// Paint the project-panel scrollbar last so row selection and plugin
		// decorations cannot overwrite its track or thumb.
		if display_nodes.len() > viewport_height {
			...
			let scrollbar_state =
				ScrollbarState::new(display_nodes.len(), viewport_height, scroll_offset);
	...
	fn build_node_line(
		...
		let leading_slot_width =
			slot_resolution.leading.as_ref().map(|slot| slot.width() + 1).unwrap_or(0);
		...
		let trailing_slot_width =
			slot_resolution.trailing.as_ref().map(|slot| slot.width()).unwrap_or(0);
		...
		if node.is_dir() {
			...
			spans.push(Span::styled(indicator, Style::default().fg(theme.diagnostic_warning_fg)));
		...
		if let Some(fm) = fuzzy_match {
			...
			spans.push(Span::styled(node.entry.name.clone(), Style::default().fg(base_fg)));
		}
		...
		if node.is_error() {
			spans.push(Span::styled(error_text, Style::default().fg(theme.diagnostic_error_fg)));
		}
	...
	pub(crate) fn trailing_slot_screen_bounds(ctx: TrailingSlotBoundsCtx) -> Option<(u16, u16)> {
		...
		let error_width = if node.is_error() { str_width(" [Error]") } else { 0 };
		let total_right_width = trailing_slot_width + error_width;
		let min_gap = 1;
		let padding = if left_side_width + min_gap + total_right_width < content_width {
			content_width - left_side_width - total_right_width
	...
	fn render_name_with_highlights(
		...
		let highlight_style = Style::default().fg(theme.search_match_fg).bg(theme.search_match_bg);
...
mod tests {
	...
	async fn create_renderer_view() -> (TempDir, FileTreeView) {
		...
		let mut tree =
			crate::view::file_tree::FileTree::new(root.to_path_buf(), manager).await.unwrap();
	...
	async fn renderer_line_shows_plugin_decoration_badge() {
		...
		let decorations = FileExplorerDecorationCache::rebuild(
			vec![crate::view::file_tree::FileExplorerDecoration {
				path: schema_path,
				symbol: "M".to_string(),
				color: dx_core::api::OverlayColorSpec::ThemeKey("ui.file_status_modified_fg".into()),
				priority: 50,
			}],
			view.tree().root_path(),
			&HashMap::new(),
	...
	async fn directories_render_bubbled_plugin_status() {
		...
		let decorations = FileExplorerDecorationCache::rebuild(
			vec![crate::view::file_tree::FileExplorerDecoration {
				path: schema_path,
				symbol: "R".to_string(),
				color: dx_core::api::OverlayColorSpec::ThemeKey("ui.file_status_renamed_fg".into()),
				priority: 40,
			}],
			view.tree().root_path(),
			&HashMap::new(),
		...
		let line =
			build_line(&view, src_id, 1, &decorations, &FileExplorerSlotOverrideCache::default(), &theme);
	...
	async fn default_slot_providers_allow_explicit_slot_and_name_color_overrides() {
		...
		let slot_overrides = FileExplorerSlotOverrideCache::rebuild(
			vec![dx_core::file_explorer::FileExplorerSlotEntry {
				path: schema_path.clone(),
				leading: Some(dx_core::file_explorer::FileExplorerLeadingSlot {
					text: "PL".to_string(),
					color: dx_core::api::OverlayColorSpec::ThemeKey("syntax.string".into()),
					min_width: 2,
				}),
				trailing: Some(dx_core::file_explorer::FileExplorerTrailingSlot {
					text: "X".to_string(),
					color: dx_core::api::OverlayColorSpec::ThemeKey("syntax.type".into()),
					tooltip: Some(dx_core::file_explorer::FileExplorerTooltip {
						title: "Plugin".to_string(),
						lines: vec!["Overridden".to_string()],
					}),
				}),
				name_color: Some(dx_core::api::OverlayColorSpec::ThemeKey(
		...
		assert!(line.spans.iter().any(|span| {
			span.content.as_ref() == "schema.ts" && span.style.fg == Some(theme.file_status_added_fg)
		}));
	}
	...
	async fn default_slot_providers_fall_back_when_only_name_color_is_overridden() {
		...
		let slot_overrides = FileExplorerSlotOverrideCache::rebuild(
			vec![dx_core::file_explorer::FileExplorerSlotEntry {
				...
				leading: None,
				trailing: None,
				name_color: Some(dx_core::api::OverlayColorSpec::ThemeKey("syntax.string".into())),
				priority: 50,
				suppress_leading: false,
				suppress_trailing: false,
				suppress_name_color: false,
	...
	async fn trailing_slot_bounds_track_rendered_right_edge_geometry() {
		...
		let slot_resolution =
			crate::view::file_tree::default_slot_providers().resolver().resolve(&slot_context);
