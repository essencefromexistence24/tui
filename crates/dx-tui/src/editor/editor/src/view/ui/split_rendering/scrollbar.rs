...
pub(super) fn scrollbar_line_counts(
	...
	let total_lines =
		if buffer_len > 0 { state.buffer.get_line_number(buffer_len.saturating_sub(1)) + 1 } else { 1 };
...
pub(super) fn scrollbar_visual_row_counts(
	...
	use crate::view::line_wrap_cache::{CacheViewMode, pipeline_inputs_version};
	use crate::view::visual_row_index::{VisualRowIndexKey, ensure_built};
	...
	let wrap_config =
		WrapConfig::new(viewport.width as usize, gutter_width, true, viewport.wrap_indent);
	let effective_width = wrap_config.first_line_width.saturating_add(gutter_width).max(2);
...
mod tests {
	...
	fn state_with_wrapping_lines(n: usize) -> EditorState {
		...
		let mut state =
			EditorState::new(80, 24, crate::config::LARGE_FILE_THRESHOLD_BYTES as usize, fs);
	...
	fn small_wrapped_buffer_uses_exact_visual_rows() {
		...
		let buffer_len = state.buffer.len();
		let (total, _) =
			scrollbar_line_counts(&mut state, &vp, crate::config::LARGE_FILE_THRESHOLD_BYTES, buffer_len);
		assert!(
			total > 100,
