...
pub(super) fn split_layout(
	...
	let hscrollbar_height = if show_horizontal_scrollbar { 1u16 } else { 0u16 };
	...
	let content_rect = Rect::new(
		...
		split_area.height.saturating_sub(tabs_height).saturating_sub(hscrollbar_height),
	);
	...
	SplitLayout { tabs_rect, content_rect, scrollbar_rect, horizontal_scrollbar_rect }
}
...
pub(super) fn sync_viewport_to_content(
	...
	let size_changed = viewport.width != content_rect.width || viewport.height != content_rect.height;
	...
	for (idx, line) in view_lines.iter().enumerate() {
		if let Some(first_source) = line.char_source_bytes.iter().find_map(|m| *m) {
			if first_source >= top_byte {
				...
				return ViewAnchor { start_line_idx: start_idx };
			}
		}
	}
...
pub(super) fn calculate_compose_layout(
	...
	if !should_compose {
		return ComposeLayout { render_area: area, left_pad: 0, right_pad: 0 };
	}
...
pub(super) fn render_separator(
	...
	let style = Style::default().fg(theme.split_separator_fg).bg(theme.editor_bg);
	match direction {
