...
pub(super) fn compute_char_style(ctx: &CharStyleContext) -> CharStyleOutput {
	...
	for overlay in ctx.active_overlays {
		match &overlay.face {
			...
			OverlayFace::ThemedStyle { fallback_style, fg_theme, bg_theme, fg_on_collision_only } => {
	...
	CharStyleOutput { style, is_secondary_cursor, fg_theme_key, bg_theme_key, region }
}
...
mod tests {
	...
	use crate::view::theme::{THEME_TERMINAL, Theme};
	...
	fn diff_add(marker_list: &mut MarkerList) -> Overlay {
		themed_overlay(marker_list, "editor.diff_add_collision_fg", "editor.diff_add_bg", true)
	}
