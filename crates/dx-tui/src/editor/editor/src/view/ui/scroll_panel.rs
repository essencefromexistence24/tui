...
impl ScrollState {
	...
	pub fn new(viewport: u16) -> Self {
		Self { offset: 0, viewport, content_height: 0 }
	}
	...
	pub fn to_scrollbar_state(&self) -> ScrollbarState {
		ScrollbarState::new(self.content_height as usize, self.viewport as usize, self.offset as usize)
	}
}
...
impl ScrollablePanel {
	...
	pub fn with_viewport(viewport: u16) -> Self {
		Self { scroll: ScrollState::new(viewport) }
	}
	...
	pub fn content_width(&self, area_width: u16) -> u16 {
		if self.scroll.needs_scrollbar() { area_width.saturating_sub(1) } else { area_width }
	}
		...
		F: Fn(&mut Frame, RenderInfo, &I) -> L,
	{
		...
		for (idx, item) in items.iter().enumerate() {
			...
			if visible_h > 0 {
				...
				let info = RenderInfo { area: item_area, skip_top, index: idx };
				let layout = render_item(frame, info, item);
				layouts.push(ItemLayoutInfo { index: idx, content_y, area: item_area, layout });
			}
		...
		ScrollablePanelLayout { content_area, scrollbar_area, item_layouts: layouts }
	}
...
mod tests {
	...
	fn test_panel_update_content_height() {
		...
		let items = vec![TestItem { height: 3 }, TestItem { height: 5 }, TestItem { height: 2 }];
	...
	fn test_panel_ensure_focused_visible_with_subfocus() {
		...
		let items = vec![TestItemWithRegions {
			height: 10,
			regions: vec![
				FocusRegion { id: 0, y_offset: 0, height: 1 },
				FocusRegion { id: 1, y_offset: 3, height: 1 },
				FocusRegion { id: 2, y_offset: 7, height: 1 },
			],
		}];
		panel.update_content_height(&items, TEST_WIDTH);
