...
impl From<Rect> for RectView {
	fn from(r: Rect) -> Self {
		RectView { x: r.x, y: r.y, w: r.width, h: r.height }
	}
}
...
fn item_view(editor: &Editor, item: &dx_core::menu::MenuItem) -> MenuItemView {
	use dx_core::menu::MenuItem::*;
	match item {
		Separator { .. } => MenuItemView::Sep,
		Action { label, action, args, when: _, checkbox } => MenuItemView::Action {
			label: label.clone(),
			action: action.clone(),
			args: args.clone(),
		...
		DynamicSubmenu { label, .. } => {
			MenuItemView::Submenu { label: label.clone(), items: Vec::new() }
		}
		Label { info } => MenuItemView::Label { label: info.clone() },
	}
}
		...
		let dropdown = chrome.menu_layout.as_ref().and_then(|ml| {
			...
			Some(DropdownView {
				...
				items: ml
					.item_areas
					.iter()
					.map(|(index, r)| ItemArea { index: *index, rect: RectView::from(*r) })
					.collect(),
				submenus: ml
					.submenu_areas
					.iter()
				...
				submenu_boxes: ml
					.submenu_boxes
					.iter()
					.map(|(depth, r)| SubmenuBoxArea { depth: *depth, rect: RectView::from(*r) })
					.collect(),
			})
		});
...
pub struct PaletteView {
	...
	#[serde(skip_serializing_if = "Option::is_none")]
	pub toolbar: Option<dx_core::api::WidgetSpec>,
	#[serde(skip_serializing_if = "Option::is_none")]
...
impl Editor {
	...
	pub fn status_view(&self) -> Option<StatusView> {
		...
		Some(StatusView { rect: RectView { x: sx, y: sy, w: sw, h: 1 }, segments })
	}
		...
		Some(PaletteView {
			...
			preview_rect: chrome.prompt_preview_area.and_then(|r| {
				(r.width > 1 && r.height > 0).then(|| {
					RectView::from(Rect::new(r.x.saturating_add(1), r.y, r.width.saturating_sub(1), r.height))
				})
			}),
			suggestions: p
				.suggestions
...
pub enum PopupContentView {
	List { items: Vec<PopupItemView>, selected: usize },
...
fn project_popup(
	...
	let content = match &p.content {
		...
		PopupContent::Text(lines) | PopupContent::Custom(lines) => {
			PopupContentView::Lines { lines: lines.clone() }
		}
...
impl Editor {
	...
	pub fn file_explorer_view(&self) -> Option<FileExplorerView> {
		...
		let rows = view
			...
			.collect();
		let title = tree.get_node(tree.root_id()).map(|n| n.entry.name.clone()).unwrap_or_default();
...
pub struct WidgetSurfaceView {
	...
	pub spec: dx_core::api::WidgetSpec,
...
impl Editor {
	...
	pub fn widgets_view(&self) -> Vec<WidgetSurfaceView> {
		let mut out = Vec::new();
		for (kind, slot) in
			[("dock", self.dock.as_ref()), ("floatingModal", self.floating_widget_panel.as_ref())]
		{
			...
			for (key, st) in &panel.instance_states {
				...
				let view = match st {
					W::List { scroll_offset, selected_index, .. } => WidgetInstanceView {
						selected_index: Some(*selected_index),
						scroll_offset: Some(*scroll_offset),
						expanded_keys: Vec::new(),
					},
					W::Tree { scroll_offset, selected_index, expanded_keys } => WidgetInstanceView {
						selected_index: Some(*selected_index),
						scroll_offset: Some(*scroll_offset),
	...
	pub fn context_menu_view(&self) -> Option<ContextMenuView> {
		...
		if let Some(m) = &w.new_tab_menu {
			return Some(ContextMenuView {
				...
				y: m.position.1,
				highlighted: m.highlighted,
				items: crate::app::types::NewTabMenuItem::all().iter().map(|i| i.label()).collect(),
			});
		}
		if let Some(m) = &w.tab_context_menu {
			return Some(ContextMenuView {
				...
				y: m.position.1,
				highlighted: m.highlighted,
				items: crate::app::types::TabContextMenuItem::all().iter().map(|i| i.label()).collect(),
			});
		}
		None
	}
	...
	pub fn aux_modals_view(&self) -> Option<AuxModalView> {
		...
		if let Some(ed) = &w.event_debug {
			let mut lines: Vec<AuxLine> = ed
				.history
				.iter()
				.map(|r| AuxLine { text: r.description.clone(), selected: false })
				.collect();
			if lines.is_empty() {
				lines.push(AuxLine {
		...
		if let Some(ti) = &w.theme_info_popup {
			...
			let info = &ti.info;
			let mut lines = vec![AuxLine { text: format!("Region: {}", info.region), selected: false }];
			if let Some(k) = &info.fg_key {
				let c = info.fg_color.map(|c| format!("  {}", color_str(c))).unwrap_or_default();
				lines.push(AuxLine { text: format!("Foreground: {k}{c}"), selected: false });
			}
			if let Some(k) = &info.bg_key {
				let c = info.bg_color.map(|c| format!("  {}", color_str(c))).unwrap_or_default();
				lines.push(AuxLine { text: format!("Background: {k}{c}"), selected: false });
			}
			if let Some(cat) = &info.syntax_category {
				lines.push(AuxLine { text: format!("Category: {cat}"), selected: false });
			}
			return Some(AuxModalView {
				kind: "themeInfo",
				title: "Theme".to_string(),
				rect: Some(RectView { x: ti.position.0, y: ti.position.1, w: 0, h: 0 }),
				lines,
				footer: None,
			});
		}
	...
	pub fn keybinding_editor_view(&self) -> Option<KeybindingEditorView> {
		...
		let rows = kb
			...
			.map(|(i, dr)| {
				...
				match dr {
					DisplayRow::SectionHeader { plugin_name, collapsed, binding_count } => KbRow::Section {
...
fn setting_control_view(c: &crate::view::settings::items::SettingControl) -> SettingControlView {
	...
	match c {
		...
		C::Number(s) => SettingControlView::Number { value: s.value, min: s.min, max: s.max },
		...
		C::TextList(s) => {
			SettingControlView::TextList { items: s.items.clone(), focused: s.focused_item }
		}
		C::DualList(s) => SettingControlView::DualList {
			...
			included: s.included_items().iter().map(|(_, n)| n.to_string()).collect(),
		...
		C::Map(s) => SettingControlView::Map {
			entries: s
				...
				.map(|(k, v)| MapEntryView {
					key: k.clone(),
					display: v.as_str().map(|x| x.to_string()).unwrap_or_else(|| v.to_string()),
				})
				.collect(),
		},
		...
		C::Complex { type_name } => SettingControlView::Complex { type_name: type_name.clone() },
	}
}
