...
impl FileExplorerSlotOverrideCache {
	...
	where
		I: IntoIterator<Item = dx_core::file_explorer::FileExplorerSlotEntry>,
	{
		...
		for slot in slots {
			...
			if slot.trailing.is_some() || slot.suppress_trailing {
				let cached = CachedTrailingOverride {
					slot: slot.trailing.as_ref().map(|trailing| CachedTrailingSlot {
						...
						tooltip: trailing.tooltip.as_ref().map(|tooltip| ExplorerTooltipSummary {
			...
			if slot.name_color.is_some() || slot.suppress_name_color {
				let cached =
					CachedNameColorOverride { color: slot.name_color.clone(), priority: slot.priority };
				insert_with_aliases(
		...
		Self { direct_leading, direct_trailing, direct_name_color }
	}
...
impl ExplorerLeadingSlotProvider for DefaultLeadingSlotProvider {
	fn resolve(&self, context: &ExplorerSlotContext<'_>) -> Option<ExplorerLeadingSlotPayload> {
		context.slot_overrides.leading_override_for_path(context.path).and_then(|override_entry| {
			override_entry.slot.as_ref().map(|slot| ExplorerLeadingSlotPayload {
...
impl ExplorerTrailingSlotProvider for DefaultTrailingSlotProvider {
	fn resolve(&self, context: &ExplorerSlotContext<'_>) -> ExplorerTrailingSlotResolution {
		let compatibility = super::decorations::COMPATIBILITY_TRAILING_SLOT_PROVIDER.resolve(context);
		let override_trailing = context.slot_overrides.trailing_override_for_path(context.path);
		let override_name_color = context.slot_overrides.name_color_override_for_path(context.path);

		ExplorerTrailingSlotResolution {
			payload: match override_trailing {
				Some(override_entry) => {
					override_entry.slot.as_ref().map(|slot| ExplorerTrailingSlotPayload {
						text: slot.text.clone(),
						fg: resolve_overlay_color(&slot.color, context.theme, context.neutral_fg),
...
mod tests {
	...
	fn suppressed_trailing_and_name_color_block_compatibility_fallback() {
		...
		let decorations = FileExplorerDecorationCache::rebuild(
			vec![dx_core::file_explorer::FileExplorerDecoration {
				path: path.clone(),
				symbol: "M".to_string(),
