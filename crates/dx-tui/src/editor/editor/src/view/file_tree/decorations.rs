...
use super::slots::{
	COMPATIBILITY_TRAILING_SLOT_HIT_WIDTH, ExplorerSlotContext, ExplorerTooltipSummary,
	ExplorerTrailingSlotPayload, ExplorerTrailingSlotProvider, ExplorerTrailingSlotResolution,
};
use crate::view::theme::Theme;
use ratatui::style::Color;

// Re-export from fresh-core for shared type usage
pub use dx_core::file_explorer::FileExplorerDecoration;
...
impl<'a> ExplorerRowStatus<'a> {
	pub fn resolve(
		...
		Self { resolved: resolve_explorer_status(path, is_dir, has_unsaved, decorations) }
	}
	...
	pub fn compatibility_trailing_slot(
		...
		let (text, fg) = match self.resolved {
			...
			Some(ResolvedExplorerStatus::Decoration(decoration)) => {
				(decoration_symbol(&decoration.symbol), compatibility_decoration_color(decoration, theme))
			}
			Some(ResolvedExplorerStatus::BubbledDecoration(decoration)) => {
				("●".to_string(), compatibility_decoration_color(decoration, theme))
			}
			None => return None,
		};

		Some(ExplorerTrailingSlotPayload { text, fg, tooltip: self.tooltip_summary(is_dir) })
	}
	...
	pub fn tooltip_summary(&self, is_dir: bool) -> Option<ExplorerTooltipSummary> {
		...
		Some(ExplorerTooltipSummary { title: "Git Status".to_string(), lines })
	}
}
...
impl FileExplorerDecorationCache {
	...
	pub fn direct_paths_under(&self, dir_path: &Path) -> Vec<PathBuf> {
		let mut paths: Vec<PathBuf> =
			self.direct.keys().filter(|path| path_is_strict_child_of(path, dir_path)).cloned().collect();
		paths.sort();
		paths
	}
}
...
pub fn compatibility_decoration_color(decoration: &FileExplorerDecoration, theme: &Theme) -> Color {
	match &decoration.color {
		dx_core::api::OverlayColorSpec::Rgb(r, g, b) => Color::Rgb(*r, *g, *b),
...
pub fn decoration_symbol(symbol: &str) -> String {
	symbol.chars().next().map(|c| c.to_string()).unwrap_or_else(|| " ".to_string())
}
...
mod tests {
	...
	fn resolves_direct_decoration() {
		...
		let decorations = FileExplorerDecorationCache::rebuild(
			vec![FileExplorerDecoration {
				path: path.clone(),
				symbol: "P".to_string(),
				color: dx_core::api::OverlayColorSpec::ThemeKey("ui.file_status_added_fg".into()),
				priority: 99,
			}],
			Path::new("/repo"),
			&HashMap::new(),
	...
	fn lists_direct_paths_under_directory_in_sorted_order() {
		...
		assert_eq!(
			...
			vec![PathBuf::from("/repo/src/nested/alpha.ts"), PathBuf::from("/repo/src/zeta.ts"),]
		);
	}
	...
	fn decoration_tooltip_treats_git_explorer_staged_modified_as_staged() {
		...
		assert_eq!(decoration_tooltip(&decoration), "Modified - File has staged changes");
	}
}
