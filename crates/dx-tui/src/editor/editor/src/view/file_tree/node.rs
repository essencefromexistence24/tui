...
impl TreeNode {
	...
	pub fn new(id: NodeId, entry: DirEntry, parent: Option<NodeId>) -> Self {
		let state = if entry.is_dir() { NodeState::Collapsed } else { NodeState::Leaf };

		Self { id, entry, parent, children: Vec::new(), state }
	}
...
mod tests {
	...
	fn test_node_creation() {
		let entry =
			DirEntry::new(PathBuf::from("/test/file.txt"), "file.txt".to_string(), EntryType::File);
	...
	fn test_directory_node() {
		let entry = DirEntry::new(PathBuf::from("/test/dir"), "dir".to_string(), EntryType::Directory);
	...
	fn test_node_depth() {
		...
		let child1 = TreeNode::new(
			NodeId(1),
			DirEntry::new(PathBuf::from("/dir1"), "dir1".to_string(), EntryType::Directory),
			Some(NodeId(0)),
		);

		let child2 = TreeNode::new(
			NodeId(2),
			DirEntry::new(PathBuf::from("/dir1/dir2"), "dir2".to_string(), EntryType::Directory),
			Some(NodeId(1)),
		);
