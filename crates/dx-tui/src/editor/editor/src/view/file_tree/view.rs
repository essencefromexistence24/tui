...
impl FileTreeView {
	...
	fn is_absorbed(&self, node_id: NodeId) -> bool {
		...
		let mut visible_iter = node.children.iter().copied().filter(|&c| self.is_node_visible(c));
	...
	pub async fn expand_with_chain(&mut self, node_id: NodeId) -> std::io::Result<()> {
		...
		let needs_initial_expand =
			self.tree.get_node(node_id).map(|n| n.is_dir() && !n.is_expanded()).unwrap_or(false);
		...
		loop {
			...
			let next = {
				...
				let mut visible = node.children.iter().copied().filter(|&c| self.is_node_visible(c));
			...
			let already_expanded = self.tree.get_node(next).map(|n| n.is_expanded()).unwrap_or(false);
			if !already_expanded {
	...
	pub async fn toggle_with_chain(&mut self, node_id: NodeId) -> std::io::Result<()> {
		let was_expanded = self.tree.get_node(node_id).map(|n| n.is_expanded()).unwrap_or(false);
	...
	fn promote_to_anchor(&self, node_id: NodeId) -> NodeId {
		...
		while self.is_absorbed(cur) {
			let next = self
				.tree
				.get_node(cur)
				.and_then(|node| node.children.iter().copied().find(|&c| self.is_node_visible(c)));
			match next {
				Some(c) => cur = c,
				None => break,
			}
	...
	pub fn get_selected_entry(&self) -> Option<&DirEntry> {
		self.selected_node.and_then(|id| self.tree.get_node(id)).map(|node| &node.entry)
	}
	...
	pub fn load_gitignore_from_bytes(
		...
		self.ignore_patterns.load_gitignore_from_bytes(dir_path, contents, mtime);
	}
	...
	pub fn select_prev_match(&mut self) {
		...
		if let Some(current) = self.selected_node {
			if let Some(pos) = matching.iter().position(|&id| id == current) {
				...
				let prev_pos = if pos == 0 { matching.len() - 1 } else { pos - 1 };
	...
	pub fn get_match_for_node(&self, node_id: NodeId) -> Option<FuzzyMatch> {
		...
		self.tree.get_node(node_id).and_then(|node| self.search.match_name(&node.entry.name))
	}
	...
	pub fn node_matches_search(&self, node_id: NodeId) -> bool {
		...
		self.tree.get_node(node_id).map(|node| self.search.matches(&node.entry.name)).unwrap_or(false)
	}
}
...
mod tests {
	...
	async fn test_expanded_dir_with_all_children_filtered_stays_visible() {
		...
		let build_id = view
			...
			.iter()
			.copied()
			.find(|&id| view.tree().get_node(id).map(|n| n.entry.name == "build").unwrap_or(false))
			.expect("build/ child not found");

		view.tree_mut().expand_node(build_id).await.unwrap();
		view.load_gitignore_from_bytes(&build_dir, b"*\n", None);

		let visible: Vec<NodeId> = view.get_display_nodes().into_iter().map(|(id, _)| id).collect();
		assert!(
	...
	fn id_for(view: &FileTreeView, rel: &str) -> NodeId {
		...
		view.tree().get_node_by_path(&path).unwrap_or_else(|| panic!("expected node at {:?}", path)).id
	}
	...
	async fn test_compact_chain_collapses_single_child_dirs() {
		...
		view.expand_with_chain(id_for(&view, "chain")).await.unwrap();
	...
	async fn test_compact_display_skips_absorbed_nodes() {
		...
		let visible_names: Vec<String> =
			view.get_display_nodes().into_iter().map(|(id, _)| name_of(&view, id)).collect();
	...
	async fn test_compact_disabled_renders_each_node_as_row() {
		...
		let display = view.get_display_nodes();
		let visible_names: Vec<String> = display.iter().map(|(id, _)| name_of(&view, *id)).collect();
		assert!(visible_names.contains(&"chain".to_string()));
		assert!(visible_names.contains(&"a".to_string()));
		assert!(visible_names.contains(&"b".to_string()));
		assert!(visible_names.contains(&"c".to_string()));
		...
		let indent_for = |target: NodeId| {
			display.iter().find(|(id, _)| *id == target).map(|(_, indent)| *indent).unwrap()
		};
		...
		assert!(view.compact_chain_for_anchor(id_for(&view, "chain/a/b/c")).is_empty());
	}
	...
	async fn test_toggle_with_chain_keeps_cursor_on_visible_row() {
		...
		let visible: Vec<_> = view.get_display_nodes().into_iter().map(|(id, _)| id).collect();
