...
impl MapState {
	...
	pub fn get_display_value(&self, value: &serde_json::Value) -> String {
		...
		match value {
			serde_json::Value::Object(obj) => {
				let n = obj.len();
				if n == 1 { "1 field".to_string() } else { format!("{n} fields") }
			}
	...
	pub fn with_value_schema(mut self, schema: crate::view::settings::schema::SettingSchema) -> Self {
	...
	pub fn remove_entry(&mut self, index: usize) {
		...
		if let Some(focused) = self.focused_entry {
			if focused >= self.entries.len() {
				self.focused_entry =
					if self.entries.is_empty() { None } else { Some(self.entries.len() - 1) };
			}
		}
		...
		self.expanded =
			self.expanded.iter().map(|&idx| if idx > index { idx - 1 } else { idx }).collect();
	}
