...
impl DualListState {
	...
	pub fn included_items(&self) -> Vec<(&String, &String)> {
		self
			...
			.iter()
			.filter_map(|value| {
				self.all_options.iter().find(|(v, _)| v == value).map(|(v, name)| (v, name))
			})
			.collect()
	}
...
mod tests {
	...
	fn test_shift_arrows_focus_follows_item() {
		...
		let avail_before: Vec<String> =
			state.available_items().iter().map(|(v, _)| v.clone()).collect();
		...
		let avail_after: Vec<String> = state.available_items().iter().map(|(v, _)| v.clone()).collect();
		assert_eq!(avail_before, avail_after, "Available order should not change");
	}
