...
impl NumberInputState {
	...
	pub fn display_text(&self) -> String {
		if let Some(editor) = &self.editor { editor.value() } else { self.value.to_string() }
	}
	...
	pub fn has_selection(&self) -> bool {
		self.editor.as_ref().map(|e| e.has_selection()).unwrap_or(false)
	}
	...
	pub fn selection_range(&self) -> Option<(usize, usize)> {
		self
			.editor
			.as_ref()
			.and_then(|e| e.selection_range().map(|((_, start_col), (_, end_col))| (start_col, end_col)))
	}
}
