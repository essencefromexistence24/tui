...
use super::types::{BUILTIN_THEMES, Theme, ThemeFile, ThemeInfo};
...
/// shared dotfiles repo can write `file://${HOME}/.config/dx/themes/x.json`
...
impl ThemeRegistry {
	...
	pub fn get(&self, key_or_name: &str) -> Option<&Theme> {
		self.resolve_key(key_or_name).and_then(|key| self.themes.get(&key))
	}
	...
	/// | `file://PATH` (env-expanded) | `file://${HOME}/.config/dx/themes/x.json` | exact user-theme key |
...
impl ThemeLoader {
	...
	pub fn new(user_themes_dir: PathBuf) -> Self {
		Self { user_themes_dir: Some(user_themes_dir) }
	}
	...
	pub fn load_all(&self, bundle_theme_dirs: &[PathBuf]) -> ThemeRegistry {
							...
							if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
								if !name.starts_with('.') {
									...
									if manifest_path.exists() {
										self.load_package_themes(&path, name, &mut themes, &mut theme_list);
									} else {
										let pack_name = format!("pkg/{}", name);
										self.scan_directory(&path, &pack_name, None, &mut themes, &mut theme_list);
									}
								}
							}
						}
		...
		ThemeRegistry { themes, theme_list, themes_dir: self.user_themes_dir.clone() }
	}

	/// Read the `repository` field from a package.json manifest value.
	fn read_repository(manifest: &serde_json::Value) -> Option<String> {
		manifest.get("repository").and_then(|v| v.as_str()).map(|s| s.to_string())
	}
	...
	fn load_package_themes(
		...
		if let Some(fresh) = manifest.get("dx") {
			if let Some(theme_entries) = fresh.get("themes").and_then(|t| t.as_array()) {
				for entry in theme_entries {
					if let (Some(file), Some(name)) =
						(entry.get("file").and_then(|f| f.as_str()), entry.get("name").and_then(|n| n.as_str()))
					{
		...
		self.scan_directory(pkg_dir, &pack_name, repository.as_deref(), themes, theme_list);
	}
...
impl Theme {
	...
	pub fn set_terminal_cursor_color(&self) {
		...
		if let Some((r, g, b)) = color_to_rgb(self.cursor) {
			...
			let _ = write!(std::io::stdout(), "\x1b]12;#{:02x}{:02x}{:02x}\x07", r, g, b);
			#[allow(clippy::let_underscore_must_use)]
...
mod tests {
	...
	/// This is a regression test for the macOS bug where themes in ~/.config/dx/themes/
	...
	fn test_custom_theme_loading_from_user_dir() {
		...
		assert!(registry.get("my-custom-theme").is_some(), "Custom theme should be retrievable");
		...
		let theme_info = theme_list.iter().find(|t| t.name == "my-custom-theme").unwrap();
		assert_eq!(theme_info.pack, "user", "Custom theme should have 'user' pack");
			...
			let theme_keys: Vec<_> = menu_items
				...
				.filter_map(|item| match item {
					crate::config::MenuItem::Action { args, .. } => {
						args.get("theme").map(|v| v.as_str().unwrap_or_default().to_string())
					}
					_ => None,
				})
				.collect();
	...
	fn test_custom_theme_package_loading() {
		...
		std::fs::write(pkg_dir.join("package.json"), manifest).expect("Failed to write package.json");
		...
		assert!(registry.contains("packaged-theme"), "Packaged theme should be loaded");
	...
	fn test_theme_name_mismatch_json_vs_filename() {
		...
		assert!(registry.contains("catppuccin-mocha"), "Theme should be found by normalized filename");
		...
		assert!(registry.contains("CATPPUCCIN-MOCHA"), "Theme should be found regardless of casing");
	...
	fn test_custom_theme_in_subdirectory() {
		...
		assert!(registry.contains("nested-theme"), "Theme in subdirectory should be loaded");
	...
	fn test_resolve_key_portable_config_forms_for_user_themes() {
		...
		//    not the built-in.
		let user_key = registry.resolve_key("dark.json").expect("`dark.json` should resolve");
		assert!(
			user_key.starts_with("file://") && user_key.ends_with("dark.json"),
			"`dark.json` must resolve to the user theme file, got: {}",
			user_key
		);
		let theme = registry.get("dark.json").expect("theme should be retrievable by relative path");
		...
		std::env::set_var("DX_TEST_THEMES_ROOT", themes_dir.to_string_lossy().to_string());
		let uri = "file://${DX_TEST_THEMES_ROOT}/dark.json";
		let resolved = registry.resolve_key(uri).expect("env-var-expanded file:// URI should resolve");
		assert_eq!(resolved, user_key, "env-expanded URI should match user theme");
		std::env::remove_var("DX_TEST_THEMES_ROOT");
	...
	fn test_portable_form_round_trip() {
		...
		let builtin_portable =
			registry.portable_form("dark").expect("built-in should have a portable form");
		assert_eq!(builtin_portable, "builtin://dark");
		assert_eq!(registry.resolve_key(&builtin_portable).as_deref(), Some("dark"));
		...
		let user_info = registry
			...
			.clone();
		let user_portable =
			registry.portable_form(&user_info.key).expect("user theme should have a portable form");
		assert_eq!(
	...
	fn test_expand_env_vars() {
		std::env::set_var("DX_TEST_VAR_A", "/foo/bar");
		std::env::set_var("DX_TEST_VAR_B", "baz");
		assert_eq!(expand_env_vars("${DX_TEST_VAR_A}/x"), "/foo/bar/x");
		assert_eq!(expand_env_vars("$DX_TEST_VAR_A/x"), "/foo/bar/x");
		assert_eq!(expand_env_vars("a/${DX_TEST_VAR_B}/c"), "a/baz/c");
		// Unknown vars remain literal.
		assert_eq!(expand_env_vars("${DX_NO_SUCH_VAR_XYZ}/x"), "${DX_NO_SUCH_VAR_XYZ}/x");
		...
		std::env::remove_var("DX_TEST_VAR_A");
		std::env::remove_var("DX_TEST_VAR_B");
	}
	...
	fn test_to_json_map() {
		...
		let dark = json_map.get("dark").expect("dark theme should be in json map");
		...
		assert!(dark.get("_pack").is_some(), "theme JSON should have _pack metadata");
	}
}
