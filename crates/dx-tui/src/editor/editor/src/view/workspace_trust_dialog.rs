...
use crate::primitives::display_width::{DisplayWidth, str_width};
...
pub fn render_workspace_trust_dialog(
	...
	let TrustDialogParams { area, selected, path, triggers, secondary_label, scroll, theme, draw } =
		params;
	...
	let mut layout = TrustDialogLayout { dialog, ..Default::default() };
	if inner.width == 0 || inner.height == 0 {
		return layout;
	}
	let iw = inner.width;
	let row_rect = |r: u16| Rect { x: inner.x, y: inner.y + r, width: iw, height: 1 };
	let put = |frame: &mut Frame, r: u16, line: Line| {
		if draw && r < inner.height {
			frame.render_widget(Paragraph::new(line).style(Style::default().bg(bg)), row_rect(r));
		}
	};
		...
		match seg {
			Seg::Header => put(
				...
				Line::from(Span::styled(
					...
					Style::default().fg(theme.popup_border_fg).bg(bg).add_modifier(Modifier::BOLD),
				)),
			),
			Seg::Sep => put(
				frame,
			...
			Seg::Plain(text) => {
				put(frame, r, Line::from(Span::styled(text, Style::default().fg(fg).bg(bg))))
			}
			Seg::Dim(text) => put(
				frame,
				r,
				Line::from(Span::styled(text, Style::default().fg(fg).bg(bg).add_modifier(Modifier::DIM))),
			),
			Seg::Path(p) => put(
				frame,
				r,
				Line::from(vec![
					...
					Span::styled(p, Style::default().fg(fg).bg(bg).add_modifier(Modifier::BOLD)),
				]),
			),
			Seg::Radio(i) => {
			...
			Seg::Blank => {}
			Seg::Buttons => {
				let (ok_rect, sec_rect) = render_buttons(frame, row_rect(r), secondary_label, bg, fg, draw);
				layout.ok = ok_rect;
	...
	if max_scroll > 0 {
		let track = Rect { x: inner.x + iw.saturating_sub(1), y: inner.y, width: 1, height: visible };
...
fn render_buttons(
	...
	let ok_rect = Rect { x: ok_x, y: row.y, width: ok_w, height: 1 };
	let quit_rect = Rect { x: quit_x, y: row.y, width: quit_w, height: 1 };
	if draw {
		frame.render_widget(
...
mod tests {
	...
	fn truncate_middle_keeps_ends() {
		assert_eq!(truncate_middle("/home/noam/chunky/fresh", 100), "/home/noam/chunky/fresh");
