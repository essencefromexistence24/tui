...
fn process_limits_schema_default() -> ProcessLimits {
	ProcessLimits { max_memory_percent: Some(50), max_cpu_percent: Some(90), enabled: true }
}
...
impl ProcessLimits {
	...
	pub fn unlimited() -> Self {
		Self { max_memory_percent: None, max_cpu_percent: None, enabled: false }
	}
...
impl LspServerConfig {
	...
	pub fn merge_with_defaults(self, defaults: &LspServerConfig) -> LspServerConfig {
		LspServerConfig {
			...
			command: if self.command.is_empty() { defaults.command.clone() } else { self.command },
			...
			only_features: self.only_features.or_else(|| defaults.only_features.clone()),
			except_features: self.except_features.or_else(|| defaults.except_features.clone()),
...
mod tests {
	...
	fn test_merge_omitted_args_inherits_default() {
		...
		let user =
			LspServerConfig { command: "marksman".to_string(), args: None, ..Default::default() };
	...
	fn test_args_deserializes_missing_as_none_and_empty_as_some() {
		...
		let omitted: LspServerConfig = serde_json::from_str(r#"{"command":"markdown-oxide"}"#).unwrap();
