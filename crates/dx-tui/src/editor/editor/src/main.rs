...
use dx::input::key_translator::KeyTranslator;
#[cfg(target_os = "linux")]
use dx::services::gpm::{gpm_to_crossterm, GpmClient};
use dx::services::terminal_modes::{self, KeyboardConfig, TerminalModes};
use dx::services::tracing_setup;
use dx::{
...
struct Cli {
    ...
    /// Skip `~/.config/dx/init.ts` for this launch
    #[arg(long)]
    no_init: bool,
    ...
    fn from(cli: Cli) -> Self {
        ...
        let (
            ...
            match cmd_args.as_slice() {
                ...
                // Open file in a daemon: dx --cmd daemon open-file <name> <files...> [--wait]
...
struct SetupState {
    ...
    authority: dx::services::authority::Authority,
...
fn start_stdin_streaming() -> AnyhowResult<StdinStreamState> {
    ...
    let temp_path = temp_dir.join(format!("dx-stdin-{}.tmp", std::process::id()));
...
fn start_stdin_streaming() -> AnyhowResult<StdinStreamState> {
    ...
    let temp_dir = std::env::temp_dir();
    let temp_path = temp_dir.join(format!("dx-stdin-{}.txt", std::process::id()));
...
) -> Vec<dx::server::protocol::FileRequest> {
    use dx::server::protocol::FileRequest;
...
fn create_startup_authority(
    ...
    trust: &std::sync::Arc<dx::services::workspace_trust::WorkspaceTrust>,
    env: &std::sync::Arc<dx::services::env_provider::EnvProvider>,
) -> AnyhowResult<StartupAuthority> {
    if let Some(remote) = remote_info {
        connect_remote(remote, trust, env)
    } else {
        Ok(StartupAuthority {
            authority: dx::services::authority::Authority::local(trust.clone(), env.clone()),
            remote_session: None,
        })
    }
}
...
fn connect_remote(
    ...
    Ok(StartupAuthority {
        authority: dx::services::authority::Authority::ssh(
            filesystem,
            process_spawner,
...
fn initialize_app(args: &Args) -> AnyhowResult<SetupState> {
    ...
    // below.
    let workspace_trust =
        std::sync::Arc::new(dx::services::workspace_trust::WorkspaceTrust::new(
            None,
            dx::services::workspace_trust::TrustLevel::Restricted,
        ));
    ...
    let env_provider = std::sync::Arc::new(dx::services::env_provider::EnvProvider::inactive());
    ...
    let dir_context = dx::config_io::DirectoryContext::from_system()?;
    ...
    workspace_trust.set_store(Some(
        dx::services::workspace_trust::TrustStore::for_project_dir(
    ...
    env_provider.set_store(
        Some(dx::services::env_provider::EnvStore::for_project_dir(
            &dir_context.project_state_dir(&effective_working_dir),
        )),
        workspace_trust.level() == dx::services::workspace_trust::TrustLevel::Trusted,
    );

    let mut config = if let Some(config_path) = &args.config {
        // Explicit config file overrides layered system
        match dx::config::Config::load_from_file(config_path) {
            Ok(cfg) => cfg,
            Err(e) => {
                eprintln!(
        ...
        dx::config::Config::load_with_layers(&dir_context, &effective_working_dir)
    };
    ...
    dx::i18n::init_with_config(locale_override);
...
fn check_plugin_bundle(plugin_path: &std::path::Path) -> AnyhowResult<()> {
    use dx_parser_js::{bundle_module, has_es_module_syntax, transpile_typescript};
...
/// `dx --cmd init check` — syntax-check ~/.config/dx/init.ts via oxc.
/// Exits 0 if the file is absent or parses cleanly, 1 on any parse error.
fn init_check_command() -> AnyhowResult<()> {
    let dir_context = dx::config_io::DirectoryContext::from_system()
        .context("failed to resolve config directory")?;
    let report = dx::init_script::check(&dir_context.config_dir);
    ...
    for d in &report.diagnostics {
        let tag = match d.severity {
            dx::init_script::CheckSeverity::Error => "error",
            dx::init_script::CheckSeverity::Warning => "warning",
        };
        eprintln!(
            "{path_display}:{}:{}  {tag}  {}",
            d.line, d.column, d.message
    ...
    let errors = report
        .diagnostics
        .iter()
        .filter(|d| d.severity == dx::init_script::CheckSeverity::Error)
        .count();
    eprintln!(
...
/// Initialize a new Dx package (plugin, theme, or language pack)
fn init_package_command(package_type: Option<String>) -> AnyhowResult<()> {
    ...
    println!("Dx Package Initializer");
    println!("=========================\n");
    ...
    let pkg_type = match package_type.as_deref() {
        ...
        None => {
            ...
            println!("  1. plugin   - Extend Dx with custom commands and functionality");
    ...
    // Get package name
    let default_name = format!("my-dx-{}", pkg_type);
    let name = loop {
    ...
    match pkg_type {
        "plugin" => {
            ...
            println!("  3. Test locally: dx --check-plugin .");
        ...
        "language" => {
            ...
            println!("  4. Test by copying to ~/.config/dx/grammars/");
    ...
    println!("  2. Submit a PR to: https://github.com/anomalyco/dx-plugins-registry");
    ...
    println!("\nDocumentation: https://github.com/anomalyco/dx-plugins-registry#readme");
...
# Validate package.json against the official Dx package schema
#
...
schema_url = 'https://raw.githubusercontent.com/sinelaw/fresh/main/crates/dx-editor/plugins/schemas/theme.schema.json'
try:
...
fn write_package_json(
    ...
    let content = format!(
        r#"{{
  "$schema": "https://raw.githubusercontent.com/sinelaw/fresh/main/crates/dx-editor/plugins/schemas/package.schema.json",
  "name": "{name}",
  "version": "0.1.0",
  "description": "{desc}",
  "type": "{pkg_type}",
...
// Example: Add a keybinding in your Dx config:
// {
//   "keyBindings": {
...
Install via Dx's package manager:
```
:pkg install {}
```
...
3. Test by copying to `~/.config/dx/grammars/` and restarting Dx
...
fn list_grammars_command() -> AnyhowResult<()> {
    ...
    let registry = dx::primitives::grammar::GrammarRegistry::for_editor(config_dir);
...
fn list_sessions_command() -> AnyhowResult<()> {
    ...
    for entry in std::fs::read_dir(&socket_dir)? {
        ...
        if let Some(name) = filename.strip_suffix(".ctrl.sock") {
            ...
            let display_name = if let Some(decoded_path) = dx::workspace::decode_filename_to_path(name)
            {
    ...
    if sessions.is_empty() {
        ...
        if sessions.len() == 1 {
            ...
            if display != id {
                println!("Attach with: dx -a  (from that directory)");
                println!("         or: dx -a {}", id);
            } else {
                println!("Attach with: dx -a {}", id);
            }
        } else {
            println!("Attach with: dx -a [NAME]");
        }
    }
...
fn kill_session_command(session: Option<&str>, args: &Args) -> AnyhowResult<()> {
    use dx::server::ipc::ClientConnection;
    use dx::server::protocol::ClientControl;
    ...
    use dx::server::protocol::{ClientHello, TermSize};
...
fn run_server_command(args: &Args) -> AnyhowResult<()> {
    use dx::server::{EditorServer, EditorServerConfig};
    ...
    let editor_config = if let Some(config_path) = &args.config {
        dx::config::Config::load_from_file(config_path)?
    } else {
        dx::config::Config::load_with_layers(&dir_context, &config_dir)
    };
...
/// target sessions that were started with `dx -a` in that directory.
...
fn run_open_files_command(
    ...
    use dx::server::daemon::is_process_running;
    use dx::server::protocol::{ClientControl, ServerControl};
    use dx::server::spawn_server_detached;
    ...
    // Connect to server
    let conn = dx::server::ipc::ClientConnection::connect(&socket_paths)?;
    ...
    if server_was_started {
        ...
        if std::io::IsTerminal::is_terminal(&std::io::stdin()) {
            ...
            eprintln!(
                "Started a new daemon and opened {} file(s). Attach with: dx -a{}",
                file_requests.len(),
...
fn client_handshake(conn: &dx::server::ipc::ClientConnection) -> AnyhowResult<bool> {
    use dx::server::protocol::{
...
/// When launched from inside Dx's own embedded terminal, forward the
/// file/dir arguments to the parent editor (identified by `DX_SESSION`)
...
fn try_forward_nested(args: &Args) -> Option<AnyhowResult<()>> {
    ...
    let session = std::env::var("DX_SESSION").ok()?;
...
fn forward_to_session(session: &str, files: &[String]) -> AnyhowResult<bool> {
    ...
    let conn = match dx::server::ipc::ClientConnection::connect(&socket_paths) {
        Ok(c) => c,
...
fn run_attach(session_name: Option<&str>, files: &[String]) -> AnyhowResult<()> {
    ...
    use tracing_subscriber::{fmt, EnvFilter};
    let log_path = dx::services::log_dirs::log_dir()
        .join(format!("dx-client-{}.log", std::process::id()));
    ...
    #[cfg(windows)]
    let original_console_mode = dx_winterm::save_console_mode();
    ...
    match result {
        ...
        Ok(client::ClientExitReason::Detached) => {
            ...
            eprintln!("Reattach with: dx -a  or  dx --cmd daemon attach");
        }
...
fn print_deprecation_warnings(cli: &Cli) {
    ...
    if cli.dump_config {
        eprintln!("warning: --dump-config is deprecated, use `dx --cmd config show` instead");
    }
    if cli.show_paths {
        eprintln!("warning: --show-paths is deprecated, use `dx --cmd config paths` instead");
    }
    if cli.init.is_some() {
        eprintln!("warning: --init is deprecated, use `dx --cmd init` instead");
    }
}
...
fn show_paths_command() -> AnyhowResult<()> {
    let dir_context = dx::config_io::DirectoryContext::from_system()?;
    dx::services::log_dirs::print_all_paths(&dir_context);
    Ok(())
}
...
fn dump_config_command(args: &Args) -> AnyhowResult<()> {
    ...
    let config = if let Some(config_path) = &args.config {
        ...
        dx::config::Config::load_with_layers(&dir_context, &working_dir)
    };
    println!(
        "{}",
...
///
/// Section headers and prose are translated via `dx::i18n::t`; literal
/// CLI invocations (e.g. `dx -a mysession`) are kept verbatim
...
fn build_localized_after_help() -> String {
    ...
    out.push_str(&format!(
        "  dx --cmd daemon new proj                  {}\n",
        t("cli.example.new_session")
    ));
    out.push_str(&format!(
        "  dx --cmd daemon open-file . main.rs      {}\n",
        t("cli.example.open_in_dir")
    ));
    out.push_str(&format!(
        "  dx --cmd daemon open-file proj a.rs      {}\n",
    ...
    out.push_str("    dx --cmd daemon open-file . 'src/main.rs:10-25@\"msg\"' --wait\n\n");
    out.push_str(&format!("  {}\n", t("cli.guided.markdown")));
    out.push_str(
        "    dx --cmd daemon open-file . \\\n      $'src/main.rs:10-25@\"**Title**\\nBody text here\"' --wait\n\n",
    );
    out.push_str(&format!("  {}\n", t("cli.guided.walkthrough")));
    out.push_str("    dx --cmd daemon open-file . 'a.rs:1-10@\"Step 1\"' --wait\n");
    out.push_str("    dx --cmd daemon open-file . 'b.rs:5-20@\"Step 2\"' --wait\n");
    out.push_str("    dx --cmd daemon open-file . 'c.rs:30@\"Step 3\"'   --wait\n\n");
    out.push_str(&format!("  {}\n", t("cli.guided.git_editor")));
    out.push_str("    git config core.editor 'dx --cmd daemon open-file . --wait'\n\n");
...
fn real_main() -> AnyhowResult<()> {
    ...
    dx::i18n::init_with_config(pre_clap_locale_override().as_deref());
    ...
    // Expose `DX_INTERACTIVE=1` on the editor's process env when Dx
    ...
    if is_interactive_launch(&args) {
        std::env::set_var("DX_INTERACTIVE", "1");
    }
    ...
    // If launched from inside Dx's own embedded terminal (DX_SESSION
    ...
    // `try_forward_nested` / `dx::server::local_control`). Best-effort: if it
    // fails, the editor runs normally and nested launches just open inline.
    if let Err(e) = dx::server::local_control::start() {
    ...
    let (result, last_update_result) = loop {
        ...
        let color_capability = dx::view::color_support::ColorCapability::detect();
        ...
        let boot_authority = {
            let placeholder_root = current_working_dir
                ...
                .unwrap_or_default();
            let placeholder = dx::services::authority::Authority::local_scoped(
                dx::services::authority::SessionScope::for_root(
                    &placeholder_root,
        ...
        // User init.ts: auto-load from ~/.config/dx/init.ts through the
        ...
        if first_run {
            ...
            editor.set_status_message(dx::i18n::switched_to_project_message(&path));
        }
    ...
    if let Some(update_result) = last_update_result {
        if update_result.update_available {
            eprintln!();
            eprintln!(
                "A new version of Dx is available: {} -> {}",
            ...
            if let Some(cmd) = update_result.install_method.update_command() {
                ...
                eprintln!(
                    "Download from: https://github.com/anomalyco/dx/releases/tag/v{}",
                    update_result.latest_version
                );
            }
            eprintln!();
...
fn handle_suspend_request(
    ...
    #[cfg(unix)]
    {
        ...
        editor.set_status_message(dx::i18n::resumed_after_suspend_message());
    }
    #[cfg(not(unix))]
    {
        let _ = terminal_modes;
        editor.set_status_message(dx::i18n::suspend_unsupported_message());
    }
    Ok(())
}
...
fn run_event_loop(
    ...
    use dx::server::input_parser::InputParser;
    use dx_winterm::{VtInputEvent, VtInputReader};

    let old_console_mode = dx_winterm::enable_vt_input()?;
    ...
    dx_winterm::enable_mouse_tracking(mouse_mode)?;
    ...
    let _ = dx_winterm::disable_mouse_tracking();
    let _ = dx_winterm::restore_console_mode(old_console_mode);
    ...
    F: FnMut(Duration) -> AnyhowResult<Option<CrosstermEvent>>,
{
    ...
    loop {
        ...
        if dx::server::local_control::pump(editor) {
            needs_render = true;
        }
