...
use dx_core::api::{TreeNode, WidgetSpec};
...
pub fn set_toggle_checked_in_spec(
	...
	spec.children_mut().any(|c| set_toggle_checked_in_spec(c, widget_key, new_checked))
}
...
pub fn set_list_items_in_spec(
	...
	if let WidgetSpec::List { items, item_keys, key, .. } = spec {
		if key.as_deref() == Some(widget_key) {
			*items = new_items;
...
pub fn set_tree_nodes_in_spec(
	...
	if let WidgetSpec::Tree { nodes, item_keys, key, .. } = spec {
		if key.as_deref() == Some(widget_key) {
			*nodes = new_nodes;
			*item_keys = new_item_keys;
			return true;
...
pub fn set_tree_checked_keys_in_spec(
	...
	spec.children_mut().any(|c| set_tree_checked_keys_in_spec(c, widget_key, checked, keys))
}
...
pub fn tree_parent_index(nodes: &[TreeNode], child_idx: usize) -> Option<usize> {
	...
	nodes[..child_idx].iter().enumerate().rev().find(|(_, n)| n.depth == target_depth).map(|(i, _)| i)
}
...
mod tests {
	...
	fn tree_parent_index_finds_immediate_parent() {
		let nodes = vec![node("root", 0, true), node("child", 1, false), node("child2", 1, false)];
	...
	fn tree_parent_index_skips_intermediate_siblings() {
		...
		let nodes = vec![node("root", 0, true), node("child", 1, true), node("grand", 2, false)];
	...
	fn tree_parent_index_finds_parent_across_unrelated_subtree() {
		...
		// not root_a.
		let nodes =
			vec![node("a", 0, true), node("a.0", 1, false), node("b", 0, true), node("b.0", 1, false)];
	...
	fn set_tree_nodes_in_spec_replaces_nodes() {
		...
		assert!(ok);
		match &spec {
			WidgetSpec::Tree { nodes, item_keys, .. } => {
	...
	// `crates/dx-editor/src/primitives/text_edit.rs`.
	...
	fn set_tree_checked_keys_in_spec_flips_only_named_keys() {
		...
		let ok =
			set_tree_checked_keys_in_spec(&mut spec, "t", false, &["k_a".to_string(), "k_c".to_string()]);
		assert!(ok);
		match &spec {
	...
	fn set_tree_checked_keys_in_spec_skips_nodes_without_checkbox() {
		...
		let _ok =
			set_tree_checked_keys_in_spec(&mut spec, "t", false, &["k0".to_string(), "k1".to_string()]);
		match &spec {
			WidgetSpec::Tree { nodes, .. } => {
				assert_eq!(nodes[0].checked, Some(false));
