...
pub fn get_socket_dir() -> io::Result<PathBuf> {
	let local_app_data =
		std::env::var("LOCALAPPDATA").unwrap_or_else(|_| std::env::temp_dir().display().to_string());
	let socket_dir = PathBuf::from(local_app_data).join("dx").join("sockets");
...
pub fn socket_name_for_path(path: &Path) -> io::Result<interprocess::local_socket::Name<'static>> {
	...
	let ns_name = format!("dx-{}", name.replace(".sock", ""));
	ns_name
...
pub fn try_read_nonblocking(stream: &mut LocalStream, buf: &mut [u8]) -> io::Result<usize> {
	...
	let handle = match stream {
		LocalStream::NamedPipe(pipe) => pipe.as_handle().as_raw_handle(),
	};
