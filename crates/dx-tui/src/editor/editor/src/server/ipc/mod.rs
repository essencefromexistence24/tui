...
impl ServerListener {
	...
	pub fn bind(paths: SocketPaths) -> io::Result<Self> {
		...
		let data_listener = ListenerOptions::new().name(data_name).create_sync().map_err(|e| {
		...
		let control_listener =
			ListenerOptions::new().name(control_name).create_sync().map_err(|e| {
		...
		Ok(Self { data_listener, control_listener, paths })
	}
	...
	pub fn accept(&mut self) -> io::Result<Option<ServerConnection>> {
		...
		self.control_listener.set_nonblocking(ListenerNonblockingMode::Accept)?;
		...
		self.data_listener.set_nonblocking(ListenerNonblockingMode::Neither)?;
...
impl StreamWrapper {
	...
	pub fn set_nonblocking(&self, nonblocking: bool) -> io::Result<()> {
		self.0.lock().map_err(|_| io::Error::other("mutex poisoned"))?.set_nonblocking(nonblocking)
	}

	/// Write all bytes (takes &self for thread sharing)
	pub fn write_all(&self, buf: &[u8]) -> io::Result<()> {
		let mut guard = self.0.lock().map_err(|_| io::Error::other("mutex poisoned"))?;
	...
	pub fn try_read(&self, buf: &mut [u8]) -> io::Result<usize> {
		let mut guard = match self.0.try_lock() {
			...
			Err(std::sync::TryLockError::WouldBlock) => {
				return Err(io::Error::new(io::ErrorKind::WouldBlock, "stream busy (mutex contended)"));
			}
...
impl Read for StreamWrapper {
	fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
		let result = self.0.lock().map_err(|_| io::Error::other("mutex poisoned"))?.read(buf);
...
impl ClientConnection {
	...
	pub fn connect(paths: &SocketPaths) -> io::Result<Self> {
		...
		Ok(Self { data: StreamWrapper::new(data), control: StreamWrapper::new(control) })
	}
...
mod tests {
	...
	fn test_named_session_uses_name_directly() {
		...
		assert!(paths.data.to_string_lossy().contains("my-session.data.sock"));
		assert!(paths.control.to_string_lossy().contains("my-session.ctrl.sock"));
	}
