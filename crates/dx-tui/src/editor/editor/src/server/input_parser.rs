...
impl InputParser {
	pub fn new() -> Self {
		Self { buffer: Vec::with_capacity(32), max_buffer_size: 256, paste_buffer: None }
	}
		...
		for &byte in bytes {
			...
			match self.try_parse() {
				...
				ParseResult::Invalid => {
					...
					tracing::trace!("InputParser: Invalid sequence, buffer={:02x?}", &self.buffer,);
		...
		match std::str::from_utf8(&bytes[..needed]) {
			Ok(s) => {
				if let Some(c) = s.chars().next() {
					ParseResult::Complete(Event::Key(KeyEvent::new(KeyCode::Char(c), KeyModifiers::empty())))
				} else {
					ParseResult::Invalid
				}
			}
	...
	fn parse_csi_sequence(&self) -> ParseResult {
		...
		match final_idx {
			None => {
				...
				if all_valid { ParseResult::Incomplete } else { ParseResult::Invalid }
			}
			Some(idx) => {
	...
	fn parse_csi_final(&self, params: &[u8], final_byte: u8) -> ParseResult {
		match final_byte {
			// Cursor keys
			b'A' => {
				ParseResult::Complete(Event::Key(KeyEvent::new(KeyCode::Up, self.parse_modifiers(params))))
			}
			...
			b'F' => {
				ParseResult::Complete(Event::Key(KeyEvent::new(KeyCode::End, self.parse_modifiers(params))))
			}
			...
			b'Z' => {
				ParseResult::Complete(Event::Key(KeyEvent::new(KeyCode::BackTab, KeyModifiers::SHIFT)))
			}
	...
	fn parse_sgr_mouse(&self, params: &[u8], pressed: bool) -> ParseResult {
		...
		let modifiers = KeyModifiers::from_bits_truncate(
			if cb & 4 != 0 { KeyModifiers::SHIFT.bits() } else { 0 }
				| if cb & 8 != 0 { KeyModifiers::ALT.bits() } else { 0 }
				| if cb & 16 != 0 { KeyModifiers::CONTROL.bits() } else { 0 },
		);
...
fn modifiers_from_param(param: u8) -> KeyModifiers {
	...
	KeyModifiers::from_bits_truncate(
		if param & 1 != 0 { KeyModifiers::SHIFT.bits() } else { 0 }
			| if param & 2 != 0 { KeyModifiers::ALT.bits() } else { 0 }
			| if param & 4 != 0 { KeyModifiers::CONTROL.bits() } else { 0 },
	)
}
...
mod tests {
	...
	fn test_bracketed_paste_followed_by_keypress() {
		...
		assert_eq!(events.len(), 2, "Expected paste + key event, got: {:?}", events);
		match &events[0] {
	...
	fn test_utf8_incomplete_sequence_returns_no_events() {
		...
		assert!(events.is_empty(), "Incomplete UTF-8 sequence should produce no events");
	}
	...
	fn test_utf8_invalid_continuation_byte_does_not_panic() {
		...
		assert!(!events.is_empty(), "Invalid sequence should emit events (graceful recovery)");
	}
