...
//!
//! When Dx runs as a plain in-process editor (no detached daemon
...
//!   terminal can advertise it to children via `DX_SESSION`.
//! - A `fresh` launched with `DX_SESSION` set connects as a thin
...
use crate::server::protocol::{
	ClientControl, FileRequest, PROTOCOL_VERSION, ServerControl, ServerHello, VersionMismatch,
};
...
enum LocalControlRequest {
	...
	OpenFiles { files: Vec<FileRequest>, wait_id: Option<u64> },
...
/// to embedded terminals via `DX_SESSION`.
static SESSION_ID: OnceLock<String> = OnceLock::new();

/// The session id of this process's local control socket, if active.
///
/// The embedded-terminal spawner reads this to set `DX_SESSION` on
...
pub fn start() -> std::io::Result<&'static str> {
	...
	let _ = GLOBAL.set(Shared { req_rx: Mutex::new(bound.req_rx), waiters: bound.waiters });
...
fn bind_and_spawn(paths: SocketPaths) -> std::io::Result<BoundControl> {
	...
	std::thread::Builder::new().name("dx-local-control".to_string()).spawn(move || {
		accept_loop(listener, req_tx, accept_waiters, accept_shutdown);
	})?;

	Ok(BoundControl { req_rx, waiters, shutdown })
}
	...
	while !shutdown.load(Ordering::SeqCst) {
		match listener.accept() {
			Ok(Some(conn)) => {
				...
				#[allow(clippy::let_underscore_must_use)]
				let _ =
					std::thread::Builder::new().name("dx-local-control-conn".to_string()).spawn(move || {
	...
	loop {
		...
		match msg {
			ClientControl::OpenWindow { path } => {
				#[allow(clippy::let_underscore_must_use)]
				let _ = req_tx.send(LocalControlRequest::OpenWindow { path: PathBuf::from(path) });
			}
			ClientControl::OpenFiles { files, wait } => {
				...
				if let Some((id, rx)) = wait_slot {
					...
					#[allow(clippy::let_underscore_must_use)]
					let _ = rx.recv();
					let done = serde_json::to_string(&ServerControl::WaitComplete).unwrap_or_default();
					#[allow(clippy::let_underscore_must_use)]
...
mod tests {
	...
	fn connect_client(paths: &SocketPaths) -> ClientConnection {
		...
		conn
			...
			.expect("write hello");
		let resp = conn.read_control().expect("read server hello").expect("server hello present");
	...
	fn open_window_request_is_received() {
		...
		let msg = ClientControl::OpenWindow { path: "/abs/project".to_string() };
		conn.write_control(&serde_json::to_string(&msg).unwrap()).unwrap();
	...
	fn waited_open_files_completes_after_signal() {
		...
		let conn = connect_client(&paths);
		let msg = ClientControl::OpenFiles { files: vec![file_req("/abs/COMMIT_EDITMSG")], wait: true };
		conn.write_control(&serde_json::to_string(&msg).unwrap()).unwrap();
		...
		let line = conn.read_control().expect("read wait complete").expect("wait complete present");
	...
	fn unwaited_open_files_has_no_wait_id() {
		...
		let conn = connect_client(&paths);
		let msg = ClientControl::OpenFiles { files: vec![file_req("/abs/file.txt")], wait: false };
		conn.write_control(&serde_json::to_string(&msg).unwrap()).unwrap();
	...
	fn multiple_commands_on_one_connection_all_received() {
		...
		let conn = connect_client(&paths);
		let msgs = [
			ClientControl::OpenWindow { path: "/abs/a".to_string() },
			ClientControl::OpenWindow { path: "/abs/b".to_string() },
			ClientControl::OpenFiles { files: vec![file_req("/abs/file.txt")], wait: false },
		];
		for m in &msgs {
			conn.write_control(&serde_json::to_string(m).unwrap()).unwrap();
		}
	...
	fn command_sent_after_a_delay_is_still_received() {
		...
		let msg = ClientControl::OpenWindow { path: "/abs/delayed".to_string() };
		conn.write_control(&serde_json::to_string(&msg).unwrap()).unwrap();
