...
// Re-export Action so semantic tests can `use dx::test_api::Action`
// without reaching into `input::keybindings` directly. Keeping
...
impl Caret {
	...
	pub fn selection_range(&self) -> Option<std::ops::Range<usize>> {
		self.anchor.map(|a| if a <= self.position { a..self.position } else { self.position..a })
	}
}
...
impl EditorTestApi for crate::app::Editor {
	...
	fn buffer_text(&self) -> String {
		self.active_state().buffer.to_string().expect(
			"buffer_text(): buffer has unloaded regions; semantic tests do not support large-file mode",
		)
	}

	fn primary_caret(&self) -> Caret {
		let c = self.active_cursors().primary();
		Caret { position: c.position, anchor: c.anchor }
	}
	...
	fn carets(&self) -> Vec<Caret> {
		let mut out: Vec<Caret> = self
			.active_cursors()
			.iter()
			.map(|(_, c)| Caret { position: c.position, anchor: c.anchor })
			.collect();
	...
	fn selection_text(&mut self) -> String {
		...
		let parts: Vec<String> =
			ranges.into_iter().map(|r| state.get_text_range(r.start, r.end)).collect();
		parts.join("\n")
	}
	...
	fn hardware_cursor_position(&mut self) -> Option<(u16, u16)> {
		...
		if row >= viewport_height || col >= viewport_width { None } else { Some((col, row)) }
	}
	...
	fn modal_snapshot(&self) -> ModalSnapshot {
		...
		fn project(p: &Popup) -> PopupView {
			let (items, selected_index) = match &p.content {
				PopupContent::List { items, selected } => {
					(items.iter().map(|i| i.text.clone()).collect(), Some(*selected))
				}
				_ => (Vec::new(), None),
			};
			PopupView {
		...
		let top = self.global_popups.top().or_else(|| local.top()).map(project);
		...
		ModalSnapshot { top_popup: top, depth, prompt }
	}
	...
	fn buffer_paths(&self) -> Vec<String> {
		self
			...
			.map(|id| {
				let name = self.get_buffer_display_name(id);
				if name.is_empty() { format!("<unnamed:{}>", id.0) } else { name }
			})
			.collect()
	}
	...
	fn seed_marker(&mut self, byte_offset: usize, symbol: &str, color: &str) {
		...
		let state = self.active_state_mut();
		let _ = state.margins.set_line_indicator(byte_offset, symbol.to_string(), indicator);
	}
	...
	fn marker_positions(&self, symbol: &str) -> Vec<usize> {
		...
		let max = self.active_state().buffer.to_string().map(|s| s.len()).unwrap_or(usize::MAX);
	...
	fn create_side_by_side_diff(
		...
		let old_buffer_id =
			self.active_window_mut().create_virtual_buffer("OLD".to_string(), "text".to_string(), true);
		self
			.set_virtual_buffer_content(old_buffer_id, vec![TextPropertyEntry::text(old_content)])
			.expect("seed OLD virtual buffer");

		let new_buffer_id =
			self.active_window_mut().create_virtual_buffer("NEW".to_string(), "text".to_string(), true);
		self
		...
		let layout = CompositeLayout::SideBySide { ratios: vec![0.5, 0.5], show_separator: true };
		let composite_id =
			self.create_composite_buffer(name.to_string(), mode.to_string(), layout, sources);

		let hunk_vec: Vec<DiffHunk> =
			hunks.iter().map(|(os, oc, ns, nc)| DiffHunk::new(*os, *oc, *ns, *nc)).collect();
		...
		self.active_window_mut().set_composite_alignment(composite_id, alignment);
	...
	fn seed_virtual_line(&mut self, spec: VirtualLineSpec) {
		let VirtualLineSpec { byte_offset, text, fg, bg, placement, namespace, priority } = spec;
		...
		let pos = match placement {
			...
			other => {
				panic!("seed_virtual_line: unsupported placement {other:?}; want 'above' or 'below'")
			}
		};
	...
	fn add_margin_annotation(
		...
		let event = Event::AddMarginAnnotation {
			line,
			position: pos,
			content: MarginContentData::Symbol { text: symbol.to_string(), color },
	...
	fn remove_margin_annotation(&mut self, annotation_id: &str) {
		...
		let event = Event::RemoveMarginAnnotation { annotation_id: annotation_id.to_string() };
		self.apply_event_to_active_buffer(&event);
	}
	...
	fn primary_scrollbar_geometry(&self) -> Option<(usize, usize, u16, u16)> {
		...
		Some((*thumb_start, *thumb_end, scrollbar_rect.height, scrollbar_rect.y))
	}
}
