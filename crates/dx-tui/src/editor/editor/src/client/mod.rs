...
use crate::server::protocol::{
	ClientControl, ClientHello, PROTOCOL_VERSION, ServerControl, TermSize,
};
...
pub fn run_client_with_connection(
	...
	match server_msg {
		...
		ServerControl::VersionMismatch(mismatch) => {
			return Ok(ClientExitReason::VersionMismatch { server_version: mismatch.server_version });
		}
...
pub fn run_client_relay(
	...
	#[cfg(windows)]
	{
		let result = dx_winterm::relay_loop(&mut conn)?;
		Ok(match result {
			dx_winterm::RelayExitReason::ServerQuit => ClientExitReason::ServerQuit,
			dx_winterm::RelayExitReason::Detached => ClientExitReason::Detached,
		})
	}
}
...
pub fn get_terminal_size() -> io::Result<TermSize> {
	...
	#[cfg(windows)]
	{
		let size = dx_winterm::get_terminal_size()?;
...
#[cfg(windows)]
impl dx_winterm::RelayConnection for ClientConnection {
	...
	fn handle_server_control(&mut self, msg: &str) -> Option<dx_winterm::RelayExitReason> {
		if let Ok(ctrl) = serde_json::from_str::<ServerControl>(msg) {
			match ctrl {
				ServerControl::Quit { .. } => Some(dx_winterm::RelayExitReason::ServerQuit),
