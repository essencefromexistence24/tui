...
//!   Dx's existing `@dedent_after`, issue #2192).
...
impl IndentRules {
	...
	pub fn calculate_indent<F: Fn(usize) -> bool>(
		...
		let reference = if cur_has_content {
			Some(LineSpan { start: cur.start, end: position })
		} else {
...
pub fn set_user_rule(
	...
	USER_RULES.write().unwrap().insert(id.to_string(), Arc::new(rules));
}
...
mod tests {
	...
	fn indent(id: &str, content: &str, tab: usize) -> usize {
		rules_for_id(id).unwrap().calculate_indent(&buf(content), content.len(), tab, |_| true)
	}
	...
	fn indent_masked(id: &str, content: &str, tab: usize, masked: &[(usize, usize)]) -> usize {
		...
		rules_for_id(id).unwrap().calculate_indent(&b, content.len(), tab, is_code)
	}
	...
	fn curly_dedents_before_close_brace() {
		...
		let b = buf(content);
		let got = rules_for_id("rust").unwrap().calculate_indent(&b, pos, 4, |_| true);
		assert_eq!(got, 0);
	}
	...
	fn fish_indents_after_block_openers() {
		...
		assert_eq!(indent("fish", "function greet --argument-names name\n", 4), 4);
	...
	fn fish_one_liner_with_end_does_not_indent() {
		assert_eq!(indent("fish", "if test -n \"$name\"; echo $name; end\n", 4), 0);
	}
	...
	fn smali_indents_after_method_directive() {
		assert_eq!(indent("smali", ".method public onCreate(Landroid/os/Bundle;)V\n", 4), 4);
	}
	...
	fn smali_dedents_before_end_directive() {
		...
		let got = rules_for_id("smali").unwrap().calculate_indent(&buf(content), pos, 4, |_| true);
		assert_eq!(got, 0);
	}
	...
	fn user_overrides_register_and_merge() {
		...
		// indentation for a language Dx otherwise doesn't know.
...
mod parity {
	...
	fn rules_match_tree_sitter_on_corpus() {
		...
		for (lang, id, code) in cases {
			...
			compared += 1;
			let rules = rules_for_id(id).unwrap_or_else(|| panic!("no rules for {id}")).calculate_indent(
				&buf(code),
				code.len(),
				tab,
				|_| true,
