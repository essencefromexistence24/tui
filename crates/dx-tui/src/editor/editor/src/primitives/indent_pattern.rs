...
impl PatternIndentCalculator {
	...
	fn calculate_dedent_pattern(buffer: &Buffer, position: usize, tab_size: usize) -> Option<usize> {
		...
		while search_pos > 0 {
			...
			if let Some(&last_char) = last_non_ws {
				...
				let line_indent = Self::count_leading_indent(buffer, line_start, search_pos, tab_size);
	...
	fn find_reference_line_indent(buffer: &Buffer, line_start: usize, tab_size: usize) -> usize {
		...
		while search_pos > 0 {
			...
			let ref_last_non_ws =
				ref_line_bytes.iter().rev().find(|&&b| b != b' ' && b != b'\t' && b != b'\r' && b != b'\n');

			if let Some(&last_char) = ref_last_non_ws {
				// Found a non-empty reference line
				let line_indent = Self::count_leading_indent(buffer, ref_line_start, search_pos, tab_size);
