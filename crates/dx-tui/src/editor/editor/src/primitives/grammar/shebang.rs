...
//! regex (sh/bash, python, ruby, perl, php); many languages Dx bundles
...
/// [`dx_languages::Language::id`] so the two stay in sync; the syntect-only
/// grammars below have no [`Language`] variant and are named explicitly.
type LanguageId = &'static str;

// Syntect-only grammar ids — no `dx_languages::Language` variant exists for
...
/// whose interpreter maps to a grammar Dx ships. Handles:
...
///
/// Returns `None` for non-shebangs and for interpreters with no Dx grammar
...
mod tests {
	...
	fn parses_interpreters_and_indirection() {
		...
		assert_eq!(language_for_shebang("#!/usr/bin/pwsh\n"), Some("powershell"));
		// `env` indirection, options, version suffixes, and assignments.
		assert_eq!(language_for_shebang("#!/usr/bin/env python3\n"), Some("python"));
		assert_eq!(language_for_shebang("#!/usr/bin/env -S deno run\n"), Some("typescript"));
		assert_eq!(language_for_shebang("#!/usr/bin/env FOO=bar ruby\n"), Some("ruby"));
		assert_eq!(language_for_shebang("#!/usr/bin/python3.11\n"), Some("python"));
		assert_eq!(language_for_shebang("#!/usr/bin/env Rscript\n"), Some("r"));
		assert_eq!(language_for_shebang("#!/usr/bin/env node\n"), Some("javascript"));
		assert_eq!(language_for_shebang("#!/usr/bin/env elixir\n"), Some("elixir"));
