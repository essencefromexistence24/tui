...
pub enum GrammarSource {
	/// Built-in to Dx (pre-compiled syntect defaults + embedded grammars)
	#[serde(rename = "built-in")]
	BuiltIn,
	/// Installed from a user grammar directory (~/.config/dx/grammars/)
	#[serde(rename = "user")]
	User { path: PathBuf },
	/// From a language pack (~/.config/dx/languages/packages/)
	#[serde(rename = "language-pack")]
	LanguagePack { name: String, path: PathBuf },
	/// From a bundle package (~/.config/dx/bundles/packages/)
	#[serde(rename = "bundle")]
...
/// Bridge between syntect display names and `dx_languages::Language`.
///
...
const SYNTECT_TO_TREE_SITTER_ALIASES: &[(&str, dx_languages::Language)] =
	&[("Bourne Again Shell (bash)", dx_languages::Language::Bash)];
...
fn tree_sitter_for_syntect_name(display_name: &str) -> Option<dx_languages::Language> {
	for (syntect_name, lang) in SYNTECT_TO_TREE_SITTER_ALIASES {
		if *syntect_name == display_name {
			return Some(*lang);
		}
	}
	dx_languages::Language::all().iter().find(|l| l.display_name() == display_name).copied()
}

/// Which highlighters can serve a given `GrammarEntry`.
///
/// A catalog entry may come from syntect (a TextMate grammar indexed into
/// `SyntaxSet`), tree-sitter (a `dx_languages::Language`), or both.
...
pub struct GrammarEngines {
	...
	pub tree_sitter: Option<dx_languages::Language>,
}
...
impl GrammarRegistry {
	...
	pub fn defaults_only() -> Arc<Self> {
		...
		let syntax_set: SyntaxSet = syntect::dumps::from_uncompressed_data(include_bytes!(concat!(
			env!("OUT_DIR"),
			"/default_syntaxes.packdump"
		)))
		.expect("Failed to load pre-compiled syntax packdump");
		tracing::info!("defaults_only: loaded ({} syntaxes)", syntax_set.syntaxes().len());
	...
	pub fn find_syntax_for_file(&self, path: &Path) -> Option<&SyntaxReference> {
		...
		entry.engines.syntect.map(|i| &self.syntax_set.syntaxes()[i])
	}
	...
	pub(crate) fn register_alias(&mut self, short_name: &str, full_name: &str) -> bool {
		...
		if let Some(&idx) = self.catalog_by_name.get(&full_lower) {
			self.catalog_by_name.entry(short_lower.clone()).or_insert(idx);
	...
	fn register_alias_inner(&mut self, short_name: &str, full_name: &str, is_built_in: bool) -> bool {
		let short_lower = short_name.to_lowercase();

		// Validate: target grammar must exist in the syntax set
		let target_exists =
			self.syntax_set.syntaxes().iter().any(|s| s.name.eq_ignore_ascii_case(full_name));
		if !target_exists {
		...
		let collides_with_full_name =
			self.syntax_set.syntaxes().iter().any(|s| s.name.eq_ignore_ascii_case(&short_lower));
	...
	/// 2. Every `dx_languages::Language` not already covered by a syntect entry
	...
	pub(crate) fn rebuild_catalog(&mut self) {
		...
		let derive_language_id = |display_name: &str| -> (String, Option<dx_languages::Language>) {
			let ts = tree_sitter_for_syntect_name(display_name);
			let id = ts.map(|l| l.id().to_string()).unwrap_or_else(|| display_name.to_lowercase());
			(id, ts)
		};
		...
		for (idx, syntax) in self.syntax_set.syntaxes().iter().enumerate() {
			...
			// Only Dx's dedicated Fish grammar may own `.fish`. Syntect's
			...
			catalog.push(GrammarEntry {
				...
				source,
				engines: GrammarEngines { syntect: Some(idx), tree_sitter },
			});
		}
		...
		let mut ts_covered: std::collections::HashSet<dx_languages::Language> =
		...
		for lang in dx_languages::Language::all() {
			...
			catalog.push(GrammarEntry {
				...
				engines: GrammarEngines { syntect: None, tree_sitter: Some(*lang) },
			});
		}
	...
	pub fn find_by_name(&self, name: &str) -> Option<&GrammarEntry> {
		self.catalog_by_name.get(&name.to_lowercase()).map(|&idx| &self.catalog[idx])
	}
	...
	pub fn find_by_extension(&self, ext: &str) -> Option<&GrammarEntry> {
		self.catalog_by_extension.get(&ext.to_lowercase()).map(|&idx| &self.catalog[idx])
	}
	...
	fn apply_language_config_inner(
		...
		for (lang_id, lang_cfg) in languages {
			let grammar_name =
				if lang_cfg.grammar.is_empty() { lang_id.as_str() } else { lang_cfg.grammar.as_str() };
			...
			self.catalog_by_name.entry(lang_id.to_lowercase()).or_insert(idx);
			...
			for filename in &lang_cfg.filenames {
				if is_glob_pattern(filename) {
					if !self.catalog[idx].filename_globs.iter().any(|f| f == filename) {
	...
	pub(crate) fn load_grammar_file(path: &Path) -> Result<SyntaxDefinition, String> {
		...
		match ext {
			"sublime-syntax" => {
				...
				SyntaxDefinition::load_from_str(&content, true, path.file_stem().and_then(|s| s.to_str()))
					.map_err(|e| format!("Failed to parse sublime-syntax: {}", e))
			}
			_ => Err(format!("Unsupported grammar format: .{}. Only .sublime-syntax is supported.", ext)),
		}
	}
}
...
mod tests {
	...
	fn test_find_syntax_for_common_extensions() {
		...
		for (filename, should_exist) in test_cases {
			...
			assert_eq!(result.is_some(), should_exist, "Expected {:?} for {}", should_exist, filename);
		}
	}
	...
	fn test_racket_grammar_loaded() {
		...
		for filename in ["main.rkt", "data.rktd", "info.rktl", "doc.scrbl"] {
			...
			assert!(result.is_some(), "Racket grammar should be available for {}", filename);
	...
	fn test_shell_dotfiles_detection() {
		...
		for filename in shell_files {
			let path = Path::new(filename);
			let result = registry.find_syntax_for_file(path);
			assert!(result.is_some(), "{} should be detected as a syntax", filename);
			let syntax = result.unwrap();
			// Should be detected as Bash/Shell
			assert!(
				syntax.name.to_lowercase().contains("bash") || syntax.name.to_lowercase().contains("shell"),
	...
	fn test_find_by_path_extension_still_wins_over_shebang() {
		...
		let registry = GrammarRegistry::default();
		let entry = registry.find_by_path(Path::new("script.py"), Some("#!/bin/sh\n")).unwrap();
	...
	fn test_find_syntax_with_glob_filenames() {
		...
		assert!(
			registry.find_by_path(Path::new("nftables.conf"), None).is_some(),
			"*.conf should match nftables.conf"
		);
		assert!(registry.find_by_path(Path::new("lfrc"), None).is_some(), "*rc should match lfrc");
	...
	fn test_find_syntax_with_path_glob_filenames() {
		...
		assert!(
			registry.find_by_path(Path::new("/etc/rc.conf"), None).is_some(),
			"/etc/**/rc.* should match /etc/rc.conf"
		);
		assert!(
			registry.find_by_path(Path::new("/etc/init/rc.local"), None).is_some(),
	...
	fn test_available_grammar_info_includes_short_names() {
		...
		assert!(bash_info.short_name.is_some(), "bash grammar should have a short_name");
	...
	fn test_catalog_contains_each_language_once() {
		...
		let ts = registry.find_by_name("TypeScript").expect("TypeScript must be in the catalog");
		assert!(ts.engines.syntect.is_none());
		assert_eq!(ts.engines.tree_sitter, Some(dx_languages::Language::TypeScript));
		...
		for name in ["Rust", "Python"] {
			let entry =
				registry.find_by_name(name).unwrap_or_else(|| panic!("{} must be in the catalog", name));
			assert!(entry.engines.syntect.is_some(), "{} should have a syntect index", name);
			assert!(
			...
			let by_id = registry.find_by_name(&entry.language_id).expect("language_id should resolve");
		...
		let js = registry.find_by_name("JavaScript").expect("JavaScript must be in the catalog");
		assert!(
		...
		assert_eq!(
			...
			Some(dx_languages::Language::JavaScript),
		...
		assert!(gdscript.engines.syntect.is_some(), "GDScript should use the embedded Syntect grammar");
		assert!(gdscript.engines.tree_sitter.is_none(), "GDScript must not carry a tree-sitter parser");
	}

	#[test]
	fn test_catalog_find_by_path_and_extension() {
		let registry = GrammarRegistry::default();
		let ts = registry.find_by_path(Path::new("foo.ts"), None).expect("foo.ts should resolve");
	...
	fn test_user_alias_resolves_via_find_by_name() {
		...
		let entry = registry.find_by_name("mylang").expect("user-declared alias 'mylang' must resolve");
	...
	fn test_register_alias_preserves_applied_language_config() {
		let mut registry = GrammarRegistry::default();
		let mut languages = std::collections::HashMap::new();
		languages.insert("shell-configs".to_string(), lang_cfg("bash", &["myconf"], &["*.myconf"]));
		registry.apply_language_config(&languages);

		// Sanity: config applied.
		assert!(registry.find_by_extension("myconf").is_some());
		assert!(
			registry.find_by_path(Path::new("foo.myconf"), None).is_some(),
	...
	fn test_fish_extension_resolves_to_fish_grammar_not_bash() {
		...
		// strips it so only Dx's dedicated Fish grammar owns the extension.
		let registry = GrammarRegistry::default();
		let entry =
			registry.find_by_extension("fish").expect(".fish should resolve to a grammar entry");
	...
	fn test_config_extension_overrides_builtin() {
		...
		assert_eq!(registry.find_by_extension("js").unwrap().display_name, "JavaScript");

		let mut languages = std::collections::HashMap::new();
		languages.insert("ts-overlay".to_string(), lang_cfg("TypeScript", &["js"], &[]));
		registry.apply_language_config(&languages);
	...
	fn test_jsx_resolves_to_javascript() {
		let registry = GrammarRegistry::default();
		let entry = registry.find_by_path(Path::new("foo.jsx"), None).expect("foo.jsx must resolve");
	...
	fn test_rebuild_catalog_replays_language_config() {
		let mut registry = GrammarRegistry::default();
		let mut languages = std::collections::HashMap::new();
		languages.insert("myshell".to_string(), lang_cfg("bash", &["myext"], &["*.myglob"]));
		registry.apply_language_config(&languages);
		assert!(registry.find_by_extension("myext").is_some());
		assert!(registry.find_by_path(Path::new("foo.myglob"), None).is_some());
		...
		assert!(
			registry.find_by_path(Path::new("foo.myglob"), None).is_some(),
	...
	fn test_apply_language_config_idempotent() {
		let mut registry = GrammarRegistry::default();
		let mut languages = std::collections::HashMap::new();
		languages.insert("shell-cfg".to_string(), lang_cfg("bash", &["myconf"], &["*.myconf"]));

		registry.apply_language_config(&languages);
		let first_extensions =
			registry.find_by_name("bash").unwrap().extensions.iter().filter(|e| e == &"myconf").count();
	...
	fn test_julia_adjoint_does_not_start_string() {
		...
		let syntax = registry.find_syntax_by_name("Julia").expect("Julia grammar must be loaded");
		...
		for line in &lines {
			...
			for (byte_idx, _) in line.char_indices() {
				...
				let in_string = scopes.as_slice().iter().any(|s| s.build_string().starts_with("string."));
		...
		assert!(found_function_keyword, "test harness must have reached the `function` keyword");
	}
	...
	fn test_tree_sitter_bridge() {
		assert_eq!(
			...
			Some(dx_languages::Language::Bash)
		);
		assert_eq!(tree_sitter_for_syntect_name("Rust"), Some(dx_languages::Language::Rust));
