...
use crate::primitives::highlighter::{
	HighlightCategory, HighlightSpan, Highlighter, Language, highlight_bg, highlight_color,
};
...
fn prepare_line_at(content_bytes: &[u8], pos: usize) -> (usize, usize, Option<PreparedLine>) {
	...
	let prepared = std::str::from_utf8(line_bytes).ok().map(|line_str| {
		...
		let line_for_syntect =
			if is_streaming_tail { line_content.to_string() } else { format!("{}\n", line_content) };
		PreparedLine { line_for_syntect, line_content_len: line_content.len(), ends_with_newline }
	});
...
impl TextMateEngine {
	...
	fn maybe_create_checkpoint(
		...
		if nearby.is_empty() {
			...
			self.checkpoint_states.insert(marker_id, (state.clone(), current_scopes.clone()));
		}
	}
	...
	fn parse_line_into_spans(
		...
		for (op_offset, op) in ops {
			...
			if clamped_op_offset > syntect_offset {
				if let Some(category) = self.scope_stack_to_category(current_scopes) {
					on_span(current_offset + syntect_offset, current_offset + clamped_op_offset, category);
				}
			}
		...
		if syntect_offset < line_content_len {
			if let Some(category) = self.scope_stack_to_category(current_scopes) {
				on_span(current_offset + syntect_offset, current_offset + line_content_len, category);
			}
		}
		true
	}
	...
	pub fn highlight_viewport(
		...
		let cache_covers_viewport = self
			.cache
			.as_ref()
			.is_some_and(|c| c.range.start <= desired_parse_start && c.range.end >= desired_parse_start);
		let exact_cache_hit = cache_covers_viewport
			&& dirty.is_none()
			&& self.last_buffer_len == buffer.len()
			&& self.cache.as_ref().is_some_and(|c| c.range.end >= parse_end);
		...
		if dirty.is_none()
			...
			&& self.last_buffer_len == buffer.len()
			&& self.cache.as_ref().is_some_and(|c| c.range.end < parse_end && c.tail_state.is_some())
		{
	...
	fn try_partial_update(
		...
		while pos < content_bytes.len() {
			...
			let collect_spans = current_offset + line_byte_len > desired_parse_start.max(actual_start);
				...
				let _ = self.parse_line_into_spans(
					...
					|byte_start, byte_end, category| {
						...
						if clamped_start < byte_end {
							new_spans.push(CachedSpan { range: clamped_start..byte_end, category });
						}
					},
				);
			}
		...
		if let Some(cache) = &mut self.cache {
			...
			cache.spans.retain(|span| span.range.end <= splice_start || span.range.start >= splice_end);
	...
	fn extend_cache_forward(
		...
		let (extension_start, mut state, mut current_scopes) = {
			let cache = self.cache.as_ref().expect("extend_cache_forward: cache must exist");
			let (s, sc) =
				cache.tail_state.as_ref().expect("extend_cache_forward: tail_state must exist").clone();
		...
		while pos < content_bytes.len() {
			...
			if let Some(prepared) = prepared {
				let parse_ok = self.parse_line_into_spans(
					...
					current_offset,
					|byte_start, byte_end, category| {
						new_spans.push(CachedSpan { range: byte_start..byte_end, category });
					},
				);
				if parse_ok {
		...
		let (safe_spans, unsafe_spans): (Vec<_>, Vec<_>) =
			new_spans.into_iter().partition(|s| s.range.end <= safe_offset);

		let cache = self.cache.as_mut().expect("extend_cache_forward: cache must still exist");
	...
	fn full_parse(
			...
			if let Some(prepared) = prepared {
				let parse_ok = self.parse_line_into_spans(
					...
					|byte_start, byte_end, category| {
						...
						if clamped_start < byte_end {
							spans.push(CachedSpan { range: clamped_start..byte_end, category });
						}
					},
				);
				if parse_ok {
		...
		let cached_spans: Vec<CachedSpan> =
			spans.iter().filter(|s| s.range.end <= cache_range_end).cloned().collect();
...
impl HighlightEngine {
	...
	pub fn highlight_stats(&self) -> Option<&HighlightStats> {
		if let Self::TextMate(h) = self { Some(h.stats()) } else { None }
	}
...
pub fn highlight_string(
	...
	for line in code.split_inclusive('\n') {
		...
		let line_for_syntect =
			if line.ends_with('\n') { format!("{}\n", line_content) } else { line_content.to_string() };
...
mod tests {
	...
	fn test_fish_indented_control_keywords_stay_keywords() {
		...
		assert_eq!(engine.category_at_position(indented_if), Some(HighlightCategory::Keyword));
		assert_eq!(engine.category_at_position(indented_end), Some(HighlightCategory::Keyword));
		assert_eq!(engine.category_at_position(column_zero_end), Some(HighlightCategory::Keyword));
	}
	...
	fn test_diff_inserted_line_is_fully_covered() {
		...
		let content = "diff --git a/file.ts b/file.ts\n\
			...
			while line_start < bytes.len() {
				...
				if bytes[line_start] == b'+' && !content[line_start..line_end].starts_with("+++") {
					for byte_pos in line_start..line_end {
						let span = spans.iter().find(|s| s.range.start <= byte_pos && s.range.end > byte_pos);
	...
	fn test_storage_type_keyword() {
		assert_eq!(scope_to_category("storage.type"), Some(HighlightCategory::Keyword));
		assert_eq!(scope_to_category("storage.type.class"), Some(HighlightCategory::Keyword));
		assert_ne!(scope_to_category("storage.type"), Some(HighlightCategory::Type));
	}
	...
	fn test_small_file_scroll_is_cache_hit() {
		...
		assert_eq!(stats_after_first.cache_misses, 1, "first call must be a miss");
		...
		assert_eq!(stats_after_scroll.cache_hits, 3, "all three scroll probes must hit the cache");
		assert_eq!(
	...
	fn test_partial_update_budget_caps_work() {
		...
		assert!(tm.dirty_from.is_some(), "budget exit must keep dirty_from set");
	}
	...
	fn test_javascript_template_substitution_closing_tokens_are_string() {
		...
		let close_brace = source.find("}`").expect("locate substitution closing brace");
