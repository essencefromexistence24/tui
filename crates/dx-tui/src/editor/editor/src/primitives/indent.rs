...
use dx_languages::tree_sitter::{Parser, Query, QueryCursor, StreamingIterator};
...
impl IndentCalculator {
	...
	fn get_config(&mut self, language: &Language) -> Option<(&mut Parser, &Query)> {
		...
		#[cfg(feature = "tree-sitter")]
		{
			...
			// fallback for them. See dx_languages::Language::ts_language.
			let ts_language: dx_languages::tree_sitter::Language = language.ts_language()?;
	...
	fn uses_keyword_delimited_blocks(language: &Language) -> bool {
		matches!(language, Language::Lua | Language::Ruby | Language::Bash | Language::Pascal)
	}
	...
	pub fn calculate_indent_no_language(buffer: &Buffer, position: usize, tab_size: usize) -> usize {
	...
	pub fn calculate_dedent_for_delimiter(
		...
		let Some((parser, query)) = self.get_config(language) else {
			if let Some(rules) = crate::primitives::indent_rules::rules_for_id(language.id()) {
				if let Some(indent) =
					rules.calculate_dedent_for_delimiter(buffer, position, _delimiter, tab_size, |_| true)
				{
					return Some(indent);
				}
			}
	...
	fn calculate_dedent_pattern(buffer: &Buffer, position: usize, tab_size: usize) -> Option<usize> {
		let mut depth = 0;
		let mut search_pos = position;
		...
		while search_pos > 0 {
			...
			let last_non_ws =
				line_bytes.iter().rev().find(|&&b| b != b' ' && b != b'\t' && b != b'\r' && b != b'\n');
	...
	fn calculate_indent_pattern(buffer: &Buffer, position: usize, tab_size: usize) -> Option<usize> {
		if position == 0 {
			return None;
		}
		...
		let last_non_whitespace =
			line_bytes.iter().rev().find(|&&b| b != b' ' && b != b'\t' && b != b'\r');
		...
		while let Some((match_result, _)) = captures.next() {
			for capture in match_result.captures {
				...
				if let Some(idx) = dedent_after_capture_idx {
					if capture.index == idx as u32 {
						if let Some(last_pos) = last_nonws_offset {
							if node_start >= line_start_offset && node_start <= last_pos && last_pos < node_end {
...
mod tests {
	...
	fn test_current_and_previous_line_indent() {
		...
		let current_indent = IndentCalculator::get_current_line_indent(&buffer, buffer.len(), tab_size);
		assert_eq!(current_indent, 4, "Current line (line 2) has 4 spaces");

		let prev_indent = IndentCalculator::get_previous_line_indent(&buffer, buffer.len(), tab_size);
	...
	fn test_python_enter_after_return_dedents() {
		...
		let buffer = Buffer::from_str_test("def foo():\n\treturn 42");
		let indent = calc.calculate_indent(&buffer, buffer.len(), &Language::Python, 4);
		assert_eq!(indent, Some(0), "Enter after `return` should dedent one level (got {:?})", indent);
	}
	...
	fn test_tree_sitter_used_for_complete_block() {
		...
		let ts_result = calc.calculate_indent_tree_sitter(&buffer, position, &Language::TypeScript, 4);
	...
	fn test_tree_sitter_auto_dedent_on_close_brace() {
		...
		let nested_indent =
			calc.calculate_dedent_for_delimiter(&nested_buffer, nested_pos, '}', &Language::Rust, 4);
	...
	fn test_tree_sitter_handles_multiple_languages() {
		...
		let cpp_dedent =
			calc.calculate_dedent_for_delimiter(&cpp_buffer, cpp_buffer.len(), '}', &Language::Cpp, 4);
	...
	fn test_enter_at_start_of_indented_line_does_not_displace_content() {
		...
		let position =
			buffer.to_string().unwrap().find("    target").expect("test fixture should contain marker");
	...
	fn test_enter_at_start_of_unindented_line_python() {
		...
		let buffer = Buffer::from_str_test("def foo():\n    pass\n\nunindented");
		let position =
			buffer.to_string().unwrap().find("unindented").expect("test fixture should contain marker");
	...
	fn test_enter_in_middle_of_leading_ws_preserves_content_column() {
		...
		let buffer = Buffer::from_str_test("    line1\n    indented_target");
		let target = buffer.to_string().unwrap().find("    indented_target").unwrap();
	...
	fn test_lua_open_paren_on_function_call_does_not_trigger_indent() {
		...
		assert_eq!(indent, Some(0), "Lua: open paren on a function call must not deepen indent");
	}
	...
	fn test_ruby_open_paren_on_function_call_does_not_trigger_indent() {
		...
		assert_eq!(indent, Some(0), "Ruby: open paren on a method call must not deepen indent");
	}
	...
	fn test_bash_open_paren_on_subshell_does_not_trigger_indent() {
		...
		assert_eq!(indent, Some(0), "Bash: open paren on a subshell must not deepen indent");
	}
	...
	fn test_ruby_def_opens_block() {
		...
		let buffer = Buffer::from_str_test("def foo");
		let indent = calc.calculate_indent(&buffer, buffer.len(), &Language::Ruby, 4);
		assert_eq!(indent, Some(4), "Ruby: `def foo` opens a method block — should indent +4");
	}
