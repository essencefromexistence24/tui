...
impl AnsiParser {
	pub fn new() -> Self {
		Self { current_style: Style::default(), escape_buffer: String::new(), in_escape: false }
	}
	...
	fn parse_sgr_params(&mut self, params_str: &str) {
		...
		let params: Vec<u8> = params_str.split(';').filter_map(|s| s.parse().ok()).collect();
		...
		while i < params.len() {
			...
			match code {
				...
				22 => {
					self.current_style =
						self.current_style.remove_modifier(Modifier::BOLD).remove_modifier(Modifier::DIM)
				}
				...
				30..=37 => {
					self.current_style = self.current_style.fg(STANDARD_COLORS[(code - 30) as usize])
				}
				...
				40..=47 => {
					self.current_style = self.current_style.bg(STANDARD_COLORS[(code - 40) as usize])
				}
				...
				90..=97 => self.current_style = self.current_style.fg(BRIGHT_COLORS[(code - 90) as usize]),
