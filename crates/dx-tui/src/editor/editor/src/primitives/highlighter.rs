...
use dx_languages::tree_sitter_highlight::{
	HighlightConfiguration, HighlightEvent, Highlighter as TSHighlighter,
};
pub use dx_languages::{HighlightCategory, Language};
...
impl Highlighter {
	...
	pub fn highlight_viewport(
					...
					match event {
						Ok(HighlightEvent::Source { start, end }) => {
							...
							if let Some(&highlight_idx) = highlight_stack.last() {
								if let Some(category) = self.language.highlight_category(highlight_idx) {
									cached_spans.push(CachedSpan { range: span_start..span_end, category });
								}
							}
						}
		...
		// Update cache
		self.cache = Some(HighlightCache { range: parse_range, spans: cached_spans.clone() });
		self.last_buffer_len = buffer.len();
...
mod tests {
	...
	fn test_highlighter_templ() {
		...
		assert!(!spans.is_empty(), "Templ file should produce highlighted spans");
	...
	fn test_theme_affects_colors() {
		...
		let dark_spans = highlighter.highlight_viewport(&buffer, 0, buffer.len(), &dark_theme, 100_000);
