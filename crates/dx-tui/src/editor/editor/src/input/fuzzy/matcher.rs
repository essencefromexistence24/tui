...
use super::pattern::{PreparedPattern, PreparedTerm, is_subsequence_prepared};
use super::{FuzzyMatch, score};
...
impl FuzzyMatcher {
	...
	pub fn from_pattern(pattern: PreparedPattern) -> Self {
		Self { pattern, target_chars: Vec::new(), target_lower: Vec::new() }
	}
	...
	pub fn match_target(&mut self, target: &str) -> FuzzyMatch {
		if self.pattern.is_empty() {
			return FuzzyMatch { matched: true, score: 0, match_positions: Vec::new() };
		}
...
fn score_single_term(
	...
	FuzzyMatch { matched: true, score: final_score, match_positions: positions }
}
...
fn score_multi_term(
	...
	if let Some((span_start, span_end)) = find_sequential_term_span(target_lower, &pattern.terms) {
		...
		if gap_excess <= max_gap_excess {
			...
			all_positions = (span_start..span_end).filter(|&i| i < target_char_count).collect();
		}
	}

	FuzzyMatch { matched: true, score: total_score, match_positions: all_positions }
}
	...
	for ti in 0..n {
		...
		for qi in (0..m).rev() {
			...
			if should_update {
				...
				arena.push(ChainNode { ti, prev: best_node_for_qi[qi] });
