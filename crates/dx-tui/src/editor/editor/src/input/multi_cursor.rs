...
fn get_cursor_line_info(state: &mut EditorState, position: usize) -> Option<CursorLineInfo> {
	...
	Some(CursorLineInfo { line_start, col_offset: position.saturating_sub(line_start) })
}
...
fn success_result(cursor: Cursor, cursors: &Cursors) -> AddCursorResult {
	AddCursorResult::Success { cursor, total_cursors: cursors.iter().count() + 1 }
}
	...
	let selection_range = match primary.selection_range() {
		...
		None => {
			...
			if word_start == word_end {
				return AddCursorResult::Failed { message: "No word at cursor position".to_string() };
			}

			// Return WordSelected so caller can update the cursor's selection
			return AddCursorResult::WordSelected { word_start, word_end };
		}
	};
	...
	loop {
		let match_pos = match state.buffer.find_next(&pattern, search_start) {
			...
			None => {
				...
				return AddCursorResult::Failed { message: "No more matches".to_string() };
			}
		};
		...
		let is_occupied = cursors
			.iter()
			.any(|(_, c)| if let Some(r) = c.selection_range() { r == match_range } else { false });
		...
		if match_pos == selection_range.start {
			...
			return AddCursorResult::Failed { message: "All matches are already selected".to_string() };
		}
...
pub fn add_cursor_above(state: &mut EditorState, cursors: &Cursors) -> AddCursorResult {
	...
	let Some(info) = get_cursor_line_info(state, adjusted_position) else {
		return AddCursorResult::Failed { message: "Unable to find current line".to_string() };
	};

	// Check if we're on the first line
	if info.line_start == 0 {
		return AddCursorResult::Failed { message: "Already at first line".to_string() };
	}
	...
	if let Some((prev_line_start, prev_line_content)) = iter.prev() {
		...
		AddCursorResult::Failed { message: "Already at first line".to_string() }
	}
}
...
pub fn add_cursor_below(state: &mut EditorState, cursors: &Cursors) -> AddCursorResult {
	...
	if let Some((next_line_start, next_line_content)) = iter.next_line() {
		...
		AddCursorResult::Failed { message: "Already at last line".to_string() }
	}
}
