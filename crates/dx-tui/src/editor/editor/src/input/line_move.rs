...
fn line_bounds_at(
	...
	iter
		.next_line()
		.map(|(_start, content)| LineByteRange { start: line_start, end: line_start + content.len() })
}
...
fn prev_line_bounds(
	...
	let mut iter = buffer.line_iterator(pos, estimated_line_length);
	iter.prev().map(|(start, content)| LineByteRange { start, end: start + content.len() })
}
...
fn next_line_bounds(
	...
	iter.next_line().map(|(start, content)| LineByteRange { start, end: start + content.len() })
}
...
fn selection_line_range(
	...
	let (start_pos, end_pos) = if let Some(range) = cursor.selection_range() {
		if range.start == range.end {
			...
			let end_for_line = if start < end { end.saturating_sub(1) } else { start };
			(start, end_for_line)
		}
	} else {
	...
	Some(LineByteRange { start: start_line.start, end: end_line.end })
}
...
fn map_position_in_region(
	...
	if pos == block_end && treat_block_end_as_block {
		if let Some(mapping) = region.mappings.iter().find(|mapping| mapping.old_end == block_end) {
	...
	if let Some(mapping) = region.mappings.iter().find(|mapping| {
		...
		let max_offset =
			if mapping.new_has_newline { mapping.new_len.saturating_sub(1) } else { mapping.new_len };
...
pub(crate) fn move_lines(
	...
	let cursor_snapshots: Vec<(
		...
		.map(|(cursor_id, cursor)| {
			(cursor_id, cursor.selection_range(), cursor.position, cursor.anchor, cursor.sticky_column)
		})
		.collect();
	...
	for region in &move_regions {
		...
		let mut iter = state.buffer.line_iterator(region.start, estimated_line_length);
		...
		for (idx, line) in ordered.iter().enumerate() {
			...
			let mut append_line_ending = if region_includes_last {
				if has_trailing_newline { true } else { idx + 1 < ordered.len() }
			} else {
				true
			};
			...
			let new_len = line.content.len() + if append_line_ending { line_ending_len } else { 0 };
		...
		applied_regions.push(AppliedRegion { region: *region, mappings });
