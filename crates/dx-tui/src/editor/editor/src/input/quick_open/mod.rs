...
pub enum QuickOpenResult {
	...
	OpenFile { path: String, line: Option<usize>, column: Option<usize> },
	/// Show a buffer by ID
	ShowBuffer(usize),
...
pub fn parse_path_line_col(input: &str) -> (String, Option<usize>, Option<usize>) {
	...
	let has_drive =
		Path::new(trimmed).components().next().is_some_and(|c| matches!(c, Component::Prefix(_)));
	let search_start = if has_drive { trimmed.find(':').map(|i| i + 1).unwrap_or(0) } else { 0 };
	...
	let rebuild_path = |rest: &str| {
		if has_drive { format!("{}{}", &trimmed[..search_start], rest) } else { rest.to_string() }
	};
...
mod tests {
	...
	fn parse_goto_line_input_absolute() {
		assert_eq!(parse_goto_line_input("500"), Some(GotoLineTarget::Absolute(500)));
		assert_eq!(parse_goto_line_input("  42 "), Some(GotoLineTarget::Absolute(42)));
	}

	#[test]
	fn parse_goto_line_input_relative() {
		assert_eq!(parse_goto_line_input("+3"), Some(GotoLineTarget::Relative(3)));
		assert_eq!(parse_goto_line_input("-3"), Some(GotoLineTarget::Relative(-3)));
		assert_eq!(parse_goto_line_input(" -10 "), Some(GotoLineTarget::Relative(-10)));
	}
	...
	fn test_provider_routing() {
		...
		registry.register(Box::new(TestProvider { prefix: "".to_string() }));
		registry.register(Box::new(TestProvider { prefix: ">".to_string() }));
		registry.register(Box::new(TestProvider { prefix: "#".to_string() }));
