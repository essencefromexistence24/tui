...
use super::{
	GotoLineTarget, QuickOpenContext, QuickOpenProvider, QuickOpenResult, parse_goto_line_input,
};
...
impl CommandProvider {
	pub fn new(
		...
		Self { command_registry, keybinding_resolver }
	}
}
...
impl QuickOpenProvider for CommandProvider {
	...
	fn on_select(
		...
		let suggestion = match suggestion {
			...
			Some(_) => return QuickOpenResult::Error(t!("status.command_not_available").to_string()),
			None => return QuickOpenResult::None,
		};

		let registry = self.command_registry.read().unwrap();
		let cmd = registry.get_all().into_iter().find(|c| c.get_localized_name() == suggestion.text);
...
impl FileProvider {
	...
	pub fn record_access(&self, path: &str) {
		if let Ok(mut frecency) = self.frecency.write() {
			let entry = frecency
				...
				.or_insert(FrecencyData { access_count: 0, last_access: std::time::Instant::now() });
	...
	fn get_frecency_score(&self, path: &str) -> f64 {
		self.frecency.read().ok().and_then(|m| m.get(path).map(frecency_score)).unwrap_or(0.0)
	}
		...
		if let Ok(entries) = self.filesystem.read_dir(&abs_path) {
			...
			for entry in entries {
				if entry.is_file() && !entry.name.starts_with('.') {
					...
					results
						.push(FileEntry { frecency_score: self.get_frecency_score(&rel), relative_path: rel });
				}
			}
			results.truncate(50);
			return results;
		...
		handle.spawn_blocking(move || {
			...
			if let Some(files) = try_git_files_blocking(&process_spawner, &cwd) {
				...
				let entries: Vec<FileEntry> = files
					...
					.map(|path| {
						let score =
							frecency_map.as_ref().and_then(|m| m.get(&path)).map(frecency_score).unwrap_or(0.0);
						FileEntry { relative_path: path, frecency_score: score }
					})
					.collect();
				// Send failure means the receiver has been dropped (editor
				// shutting down); nothing more to do since we return below.
				drop(sender.send(crate::services::async_bridge::AsyncMessage::QuickOpenFilesLoaded {
					cwd: cwd.clone(),
	...
	fn load_files_sync(&self, cwd: &str) -> Option<std::sync::Arc<Vec<FileEntry>>> {
		let files = self.try_git_files(cwd).or_else(|| self.try_walk_dir(cwd)).unwrap_or_default();

		let entries: Vec<FileEntry> = files
			.into_iter()
			.map(|path| FileEntry { frecency_score: self.get_frecency_score(&path), relative_path: path })
			.collect();
...
fn try_walk_dir_blocking(
	...
	drop(fs.walk_files(base, IGNORED_DIRS, cancel, &mut |_path, rel| {
...
fn walk_dir_with_updates(
	...
	if let Err(e) = fs.walk_files(base, IGNORED_DIRS, cancel, &mut |_path, rel| {
		...
		if last_send.elapsed() >= WALK_UPDATE_INTERVAL {
			...
			let entries: Vec<FileEntry> = paths
				...
				.collect();
			if sender
				.send(crate::services::async_bridge::AsyncMessage::QuickOpenFilesLoaded {
					cwd: cwd.to_string(),
					files: std::sync::Arc::new(entries),
					complete: false,
				})
	...
	let entries: Vec<FileEntry> = paths
		.into_iter()
		.map(|p| {
			let score = frecency_map.as_ref().and_then(|m| m.get(&p).map(frecency_score)).unwrap_or(0.0);
			FileEntry { relative_path: p, frecency_score: score }
		})
		.collect();
	drop(sender.send(crate::services::async_bridge::AsyncMessage::QuickOpenFilesLoaded {
		cwd: cwd.to_string(),
...
impl QuickOpenProvider for FileProvider {
	...
	fn suggestions(&self, query: &str, context: &QuickOpenContext) -> Vec<Suggestion> {
		...
		if !self.filesystem.is_remote_connected() {
			return vec![Suggestion::disabled("Remote connection lost — cannot list files".to_string())];
		}
		...
		let prefix_entries =
			if !search_query.is_empty() { self.probe_prefix(&context.cwd, search_query) } else { vec![] };
		...
		let prefix_set: std::collections::HashSet<&str> =
			prefix_entries.iter().map(|e| e.relative_path.as_str()).collect();
		...
		for entry in &prefix_entries {
			...
			scored.push((entry.relative_path.clone(), base_score + frecency_boost + PREFIX_PROBE_BOOST));
		}
		...
		if let Some(files) = &files {
			if search_query.is_empty() {
				...
				entries.sort_by(|a, b| {
					b.0.frecency_score.partial_cmp(&a.0.frecency_score).unwrap_or(std::cmp::Ordering::Equal)
				});
		...
		let mut suggestions: Vec<Suggestion> =
			scored.into_iter().map(|(path, _)| Suggestion::new(path.clone()).with_value(path)).collect();

		if still_loading {
			let msg =
				if suggestions.is_empty() { "Loading files…" } else { "Scanning for more files…" };
	...
	fn on_select(
		...
		if let Some(path) = suggestion.and_then(|s| s.value.as_deref()) {
			...
			return QuickOpenResult::OpenFile { path: path.to_string(), line, column };
		}

		// Fallback: direct path input with :line:col
		if line.is_some() && !path_part.is_empty() {
			self.record_access(&path_part);
			return QuickOpenResult::OpenFile { path: path_part, line, column };
		}
...
mod tests {
	...
	fn test_buffer_provider_suggestions() {
		...
		let lib_suggestion = suggestions.iter().find(|s| s.text.contains("lib.rs")).unwrap();
	...
	impl crate::services::remote::ProcessSpawner for FailingSpawner {
		async fn spawn(
			...
			Err(crate::services::remote::SpawnError::Process("no git in test".to_string()))
		}
	}
	...
	impl crate::services::remote::ProcessSpawner for OtherSpawner {
		async fn spawn(
			...
			Err(crate::services::remote::SpawnError::Process("other".to_string()))
		}
	}
	...
	fn set_backends_repoints_spawner_and_invalidates_cache() {
		...
		fp.set_backends(std::sync::Arc::clone(&new_fs), std::sync::Arc::clone(&new_spawner));
	...
	fn test_file_provider_discovers_files_via_walk() {
		...
		let paths: Vec<&str> = suggestions.iter().filter_map(|s| s.value.as_deref()).collect();
	...
	fn test_probe_prefix_all_shapes() {
		...
		assert!(paths.contains(&"etc/hosts"), "missing etc/hosts in {paths:?}");
		assert!(paths.contains(&"etc/hosts.allow"), "missing etc/hosts.allow in {paths:?}");
		assert!(paths.contains(&"etc/hosts.deny"), "missing etc/hosts.deny in {paths:?}");
		assert!(!paths.contains(&"etc/passwd"), "passwd shouldn't match prefix 'hosts': {paths:?}");

		// 2. Directory query — the query is itself a directory, so we list it.
		let r = provider.probe_prefix(&cwd, "src");
		let paths: Vec<&str> = r.iter().map(|e| e.relative_path.as_str()).collect();
		assert!(paths.contains(&"src/main.rs"), "missing src/main.rs in {paths:?}");
		assert!(paths.contains(&"src/lib.rs"), "missing src/lib.rs in {paths:?}");
		...
		assert!(paths.contains(&"Makefile.bak"), "missing Makefile.bak in {paths:?}");
		assert!(
