...
impl ClipboardSync {
	...
	fn new(editor: &Editor) -> Self {
		Self { seq: 0, last_hash: Self::hash(editor.clipboard_text()) }
	}
...
pub fn run(addr: &str, files: &[PathBuf]) -> Result<()> {
	...
	eprintln!("fresh web bridge on http://{addr}  (real render pipeline, no mocks; WS push on /ws)");
	...
	loop {
		...
		if let Some(client) = ws.as_mut() {
			match client.drain_messages() {
				Ok(msgs) => {
					inputs =
						msgs.iter().map(|m| serde_json::from_str(m).unwrap_or_else(|_| json!({}))).collect()
				}
				Err(e) => {
		...
		loop {
			match listener.accept() {
				Ok((stream, _)) => {
					...
					pending.push(PendingConn { stream, buf: Vec::new(), since: Instant::now() });
				}
...
fn serve_request(
	...
	let wants_ws = req.method == "GET"
		&& req.path == "/ws"
		&& req.header("upgrade").is_some_and(|u| u.to_ascii_lowercase().contains("websocket"));
	if wants_ws {
...
fn apply_widget(editor: &mut Editor, v: &Value) {
	match v.get("surface").and_then(|s| s.as_str()) {
		...
		Some("panel") => {
			...
			let hit_index = v.get("hitIndex").and_then(|i| i.as_u64()).map(|i| i as usize);
...
fn upgrade_ws(
	...
	let Some(key) = req.header("sec-websocket-key") else {
		respond(&mut stream, "400 Bad Request", "text/plain", b"missing Sec-WebSocket-Key")?;
		return Ok(None);
	};
	let resp = format!(
...
fn ws_parse_frame(buf: &[u8]) -> Result<Option<(WsFrame, usize)>> {
	...
	Ok(Some((WsFrame { fin, opcode, payload }, off + len)))
}
...
impl WsSession {
	...
	fn push_diff(&mut self, scene: &Value) -> std::io::Result<()> {
		...
		self.seq += 1;
		let msg =
			json!({ "type": "frame", "seq": self.seq, "changed": Value::Object(changed) }).to_string();
		self.send_text(&msg)
	}
	...
	fn diff(&mut self, scene: &Value) -> serde_json::Map<String, Value> {
		...
		let regions = scene.get("regions").and_then(|r| r.as_object()).unwrap_or(&empty);
		for (k, v) in regions {
			if k == "panes" {
		...
		let panes: &[Value] =
			regions.get("panes").and_then(|p| p.as_array()).map(|v| v.as_slice()).unwrap_or(&[]);
		...
		loop {
			...
			match frame.opcode {
				...
				0x8 => {
					...
					drop(write_all_nb(&mut self.stream, &ws_encode(0x8, &frame.payload)));
					return Err(std::io::Error::new(ErrorKind::ConnectionAborted, "client sent close"));
				}
...
fn render_to_buffer(editor: &mut Editor, cols: u16, rows: u16) -> (Buffer, Option<(u16, u16)>) {
	...
	let cursor = terminal.backend_mut().get_cursor_position().ok().map(|p| (p.x, p.y));
	(buf, cursor)
}
...
fn scene_json(editor: &mut Editor, cols: u16, rows: u16) -> Value {
	...
	// crates/dx-editor/src/view/scene.rs.
	...
	let panes: Vec<Value> = layout
		...
		.map(|(leaf, bufid, content_rect, scrollbar_rect, thumb_s, thumb_e)| {
			...
			let gw = editor.leaf_gutter_width(*leaf, *bufid).min(content_rect.width);
			let gutter_rect = Rect::new(content_rect.x, content_rect.y, gw, content_rect.height);
	...
	// serializes them. See crates/dx-editor/src/view/scene.rs.
...
fn apply_mouse(editor: &mut Editor, v: &Value) {
	...
	for _ in 0..n {
		let ev = MouseEvent { kind, column: col, row, modifiers: mods };
