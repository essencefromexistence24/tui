...
impl Merge for PartialConfig {
	fn merge_from(&mut self, other: &Self) {
		...
		self.active_keybinding_map.merge_from(&other.active_keybinding_map);
	}
}
...
impl Merge for PartialEditorConfig {
	fn merge_from(&mut self, other: &Self) {
		...
		self.cursor_jump_animation.merge_from(&other.cursor_jump_animation);
		self.line_numbers.merge_from(&other.line_numbers);
		self.relative_line_numbers.merge_from(&other.relative_line_numbers);
		self.scroll_offset.merge_from(&other.scroll_offset);
		self.syntax_highlighting.merge_from(&other.syntax_highlighting);
		...
		self.highlight_timeout_ms.merge_from(&other.highlight_timeout_ms);
		self.snapshot_interval.merge_from(&other.snapshot_interval);
		self.large_file_threshold_bytes.merge_from(&other.large_file_threshold_bytes);
		self.estimated_line_length.merge_from(&other.estimated_line_length);
		self.enable_inlay_hints.merge_from(&other.enable_inlay_hints);
		self.enable_semantic_tokens_full.merge_from(&other.enable_semantic_tokens_full);
		self.diagnostics_inline_text.merge_from(&other.diagnostics_inline_text);
		self.recovery_enabled.merge_from(&other.recovery_enabled);
		self.auto_recovery_save_interval_secs.merge_from(&other.auto_recovery_save_interval_secs);
		self.auto_save_enabled.merge_from(&other.auto_save_enabled);
		self.auto_save_interval_secs.merge_from(&other.auto_save_interval_secs);
		self.hot_exit.merge_from(&other.hot_exit);
		self.confirm_quit.merge_from(&other.confirm_quit);
		self.restore_previous_session.merge_from(&other.restore_previous_session);
		self
		...
		self.highlight_context_bytes.merge_from(&other.highlight_context_bytes);
		self.mouse_hover_enabled.merge_from(&other.mouse_hover_enabled);
		self.mouse_hover_delay_ms.merge_from(&other.mouse_hover_delay_ms);
		self.double_click_time_ms.merge_from(&other.double_click_time_ms);
		self.auto_revert_poll_interval_ms.merge_from(&other.auto_revert_poll_interval_ms);
		self.read_concurrency.merge_from(&other.read_concurrency);
		self.file_tree_poll_interval_ms.merge_from(&other.file_tree_poll_interval_ms);
		self.default_line_ending.merge_from(&other.default_line_ending);
		self.trim_trailing_whitespace_on_save.merge_from(&other.trim_trailing_whitespace_on_save);
		self.ensure_final_newline_on_save.merge_from(&other.ensure_final_newline_on_save);
		self.auto_read_only.merge_from(&other.auto_read_only);
		self.highlight_matching_brackets.merge_from(&other.highlight_matching_brackets);
		self.rainbow_brackets.merge_from(&other.rainbow_brackets);
		self.cursor_style.merge_from(&other.cursor_style);
		self.keyboard_disambiguate_escape_codes.merge_from(&other.keyboard_disambiguate_escape_codes);
		self.keyboard_report_event_types.merge_from(&other.keyboard_report_event_types);
		self.keyboard_report_alternate_keys.merge_from(&other.keyboard_report_alternate_keys);
		self
			.keyboard_report_all_keys_as_escape_codes
			.merge_from(&other.keyboard_report_all_keys_as_escape_codes);
		self.completion_popup_auto_show.merge_from(&other.completion_popup_auto_show);
		self.quick_suggestions.merge_from(&other.quick_suggestions);
		self.quick_suggestions_delay_ms.merge_from(&other.quick_suggestions_delay_ms);
		self.suggest_on_trigger_characters.merge_from(&other.suggest_on_trigger_characters);
		self.show_menu_bar.merge_from(&other.show_menu_bar);
		self.screensaver_enabled.merge_from(&other.screensaver_enabled);
		self.screensaver_idle_minutes.merge_from(&other.screensaver_idle_minutes);
		self.menu_bar_mnemonics.merge_from(&other.menu_bar_mnemonics);
		...
		self.show_vertical_scrollbar.merge_from(&other.show_vertical_scrollbar);
		self.show_horizontal_scrollbar.merge_from(&other.show_horizontal_scrollbar);
		...
		self.terminal_auto_title.merge_from(&other.terminal_auto_title);
		self.rulers.merge_from(&other.rulers);
		self.indentation_guide.merge_from(&other.indentation_guide);
		self.indentation_guide_glyph.merge_from(&other.indentation_guide_glyph);
		self.whitespace_show.merge_from(&other.whitespace_show);
		self.whitespace_spaces_leading.merge_from(&other.whitespace_spaces_leading);
		self.whitespace_spaces_inner.merge_from(&other.whitespace_spaces_inner);
		self.whitespace_spaces_trailing.merge_from(&other.whitespace_spaces_trailing);
		self.whitespace_tabs_leading.merge_from(&other.whitespace_tabs_leading);
		self.whitespace_tabs_inner.merge_from(&other.whitespace_tabs_inner);
		self.whitespace_tabs_trailing.merge_from(&other.whitespace_tabs_trailing);
	}
}
...
pub struct PartialFileExplorerConfig {
	...
	#[serde(default, deserialize_with = "crate::config::explorer_width::deserialize_optional")]
...
impl Merge for PartialFileExplorerConfig {
	fn merge_from(&mut self, other: &Self) {
		...
		self.custom_ignore_patterns.merge_from(&other.custom_ignore_patterns);
		self.width.merge_from(&other.width);
		self.preview_tabs.merge_from(&other.preview_tabs);
		self.side.merge_from(&other.side);
		self.auto_open_on_last_buffer_close.merge_from(&other.auto_open_on_last_buffer_close);
		self.follow_active_buffer.merge_from(&other.follow_active_buffer);
		self.compact_directories.merge_from(&other.compact_directories);
		self.tree_indicator_collapsed.merge_from(&other.tree_indicator_collapsed);
		self.tree_indicator_expanded.merge_from(&other.tree_indicator_expanded);
	}
}
...
impl Merge for PartialClipboardConfig {
	fn merge_from(&mut self, other: &Self) {
		...
		self.use_system_clipboard.merge_from(&other.use_system_clipboard);
	}
}
...
impl Merge for PartialTerminalConfig {
	fn merge_from(&mut self, other: &Self) {
		self.jump_to_end_on_output.merge_from(&other.jump_to_end_on_output);
		self.shell.merge_from(&other.shell);
		self.skip_app_execution_alias.merge_from(&other.skip_app_execution_alias);
...
impl Merge for PartialWarningsConfig {
	fn merge_from(&mut self, other: &Self) {
		self.show_status_indicator.merge_from(&other.show_status_indicator);
	}
}
...
impl Merge for PartialLanguageConfig {
	fn merge_from(&mut self, other: &Self) {
		...
		self.show_whitespace_tabs.merge_from(&other.show_whitespace_tabs);
		self.line_wrap.merge_from(&other.line_wrap);
		self.wrap_column.merge_from(&other.wrap_column);
...
impl From<&crate::config::EditorConfig> for PartialEditorConfig {
	fn from(cfg: &crate::config::EditorConfig) -> Self {
		Self {
			...
			skip_session_restore_when_files_passed: Some(cfg.skip_session_restore_when_files_passed),
			...
			keyboard_report_all_keys_as_escape_codes: Some(cfg.keyboard_report_all_keys_as_escape_codes),
...
impl PartialEditorConfig {
	...
	pub fn resolve(self, defaults: &crate::config::EditorConfig) -> crate::config::EditorConfig {
		crate::config::EditorConfig {
			...
			cursor_jump_animation: self.cursor_jump_animation.unwrap_or(defaults.cursor_jump_animation),
			line_numbers: self.line_numbers.unwrap_or(defaults.line_numbers),
			relative_line_numbers: self.relative_line_numbers.unwrap_or(defaults.relative_line_numbers),
			scroll_offset: self.scroll_offset.unwrap_or(defaults.scroll_offset),
			syntax_highlighting: self.syntax_highlighting.unwrap_or(defaults.syntax_highlighting),
			highlight_current_line: self
				.highlight_current_line
				.unwrap_or(defaults.highlight_current_line),
			highlight_occurrences: self.highlight_occurrences.unwrap_or(defaults.highlight_occurrences),
			...
			highlight_timeout_ms: self.highlight_timeout_ms.unwrap_or(defaults.highlight_timeout_ms),
			...
			estimated_line_length: self.estimated_line_length.unwrap_or(defaults.estimated_line_length),
			enable_inlay_hints: self.enable_inlay_hints.unwrap_or(defaults.enable_inlay_hints),
			...
			mouse_hover_enabled: self.mouse_hover_enabled.unwrap_or(defaults.mouse_hover_enabled),
			mouse_hover_delay_ms: self.mouse_hover_delay_ms.unwrap_or(defaults.mouse_hover_delay_ms),
			double_click_time_ms: self.double_click_time_ms.unwrap_or(defaults.double_click_time_ms),
			...
			default_line_ending: self.default_line_ending.unwrap_or(defaults.default_line_ending.clone()),
			...
			screensaver_enabled: self.screensaver_enabled.unwrap_or(defaults.screensaver_enabled),
			screensaver_idle_minutes: self
				.screensaver_idle_minutes
				.unwrap_or(defaults.screensaver_idle_minutes),
			menu_bar_mnemonics: self.menu_bar_mnemonics.unwrap_or(defaults.menu_bar_mnemonics),
			...
			terminal_auto_title: self.terminal_auto_title.unwrap_or(defaults.terminal_auto_title),
			...
			whitespace_tabs_inner: self.whitespace_tabs_inner.unwrap_or(defaults.whitespace_tabs_inner),
...
impl PartialFileExplorerConfig {
	pub fn resolve(self, defaults: &FileExplorerConfig) -> FileExplorerConfig {
		FileExplorerConfig {
			...
			follow_active_buffer: self.follow_active_buffer.unwrap_or(defaults.follow_active_buffer),
			compact_directories: self.compact_directories.unwrap_or(defaults.compact_directories),
...
impl From<&FileBrowserConfig> for PartialFileBrowserConfig {
	fn from(cfg: &FileBrowserConfig) -> Self {
		Self { show_hidden: Some(cfg.show_hidden) }
	}
}

impl PartialFileBrowserConfig {
	pub fn resolve(self, defaults: &FileBrowserConfig) -> FileBrowserConfig {
		FileBrowserConfig { show_hidden: self.show_hidden.unwrap_or(defaults.show_hidden) }
	}
}

impl From<&ClipboardConfig> for PartialClipboardConfig {
	fn from(cfg: &ClipboardConfig) -> Self {
		Self { use_osc52: Some(cfg.use_osc52), use_system_clipboard: Some(cfg.use_system_clipboard) }
	}
}
...
impl PartialClipboardConfig {
	pub fn resolve(self, defaults: &ClipboardConfig) -> ClipboardConfig {
		ClipboardConfig {
			...
			use_system_clipboard: self.use_system_clipboard.unwrap_or(defaults.use_system_clipboard),
		}
	}
}
...
impl PartialTerminalConfig {
	pub fn resolve(self, defaults: &TerminalConfig) -> TerminalConfig {
		TerminalConfig {
			jump_to_end_on_output: self.jump_to_end_on_output.unwrap_or(defaults.jump_to_end_on_output),
...
impl From<&WarningsConfig> for PartialWarningsConfig {
	fn from(cfg: &WarningsConfig) -> Self {
		Self { show_status_indicator: Some(cfg.show_status_indicator) }
	}
}
...
impl PartialWarningsConfig {
	pub fn resolve(self, defaults: &WarningsConfig) -> WarningsConfig {
		WarningsConfig {
			show_status_indicator: self.show_status_indicator.unwrap_or(defaults.show_status_indicator),
		}
	}
}
...
impl From<&crate::config::PackagesConfig> for PartialPackagesConfig {
	fn from(cfg: &crate::config::PackagesConfig) -> Self {
		Self { sources: Some(cfg.sources.clone()) }
	}
}

impl PartialPackagesConfig {
	pub fn resolve(self, defaults: &crate::config::PackagesConfig) -> crate::config::PackagesConfig {
...
impl From<&PluginConfig> for PartialPluginConfig {
	fn from(cfg: &PluginConfig) -> Self {
		Self { enabled: Some(cfg.enabled), path: cfg.path.clone(), settings: cfg.settings.clone() }
	}
}
...
impl PartialLanguageConfig {
	pub fn resolve(self, defaults: &LanguageConfig) -> LanguageConfig {
		LanguageConfig {
			extensions: self.extensions.unwrap_or_else(|| defaults.extensions.clone()),
			...
			comment_prefix: self.comment_prefix.or_else(|| defaults.comment_prefix.clone()),
			auto_indent: self.auto_indent.unwrap_or(defaults.auto_indent),
			auto_close: self.auto_close.or(defaults.auto_close),
			auto_surround: self.auto_surround.or(defaults.auto_surround),
			textmate_grammar: self.textmate_grammar.or_else(|| defaults.textmate_grammar.clone()),
			show_whitespace_tabs: self.show_whitespace_tabs.unwrap_or(defaults.show_whitespace_tabs),
			...
			word_characters: self.word_characters.unwrap_or_else(|| defaults.word_characters.clone()),
...
impl From<&crate::config::Config> for PartialConfig {
	fn from(cfg: &crate::config::Config) -> Self {
		Self {
			...
			languages: Some(
				cfg.languages.iter().map(|(k, v)| (k.clone(), PartialLanguageConfig::from(v))).collect(),
			),
			...
			plugins: {
				...
				let non_default_plugins: HashMap<String, PartialPluginConfig> = cfg
					...
					.collect();
				if non_default_plugins.is_empty() { None } else { Some(non_default_plugins) }
			},
...
impl PartialConfig {
	...
	pub fn resolve_with_defaults(self, defaults: &crate::config::Config) -> crate::config::Config {
				...
				for (key, lang_config) in partial_lsp {
					...
					if let Some(default_configs) = result.get(&key) {
						...
						if user_configs.len() == 1 && default_slice.len() == 1 {
							let merged =
								user_configs.into_iter().next().unwrap().merge_with_defaults(&default_slice[0]);
		...
		let mut config = crate::config::Config {
			...
			locale: crate::config::LocaleName::from(self.locale.or_else(|| defaults.locale.0.clone())),
			...
			keybindings: self.keybindings.unwrap_or_else(|| defaults.keybindings.clone()),
			keybinding_maps,
			active_keybinding_map: self
				.active_keybinding_map
				.unwrap_or_else(|| defaults.active_keybinding_map.clone()),
			languages,
			default_language: self.default_language.or_else(|| defaults.default_language.clone()),
...
impl SessionConfig {
	...
	pub fn to_partial_config(&self) -> PartialConfig {
		PartialConfig { theme: self.theme.clone(), editor: self.editor.clone(), ..Default::default() }
	}
...
impl From<PartialConfig> for SessionConfig {
	fn from(partial: PartialConfig) -> Self {
		Self { theme: partial.theme, editor: partial.editor, buffer_overrides: HashMap::new() }
	}
}
...
mod tests {
	...
	fn merge_editor_config_recursive() {
		let mut higher = PartialEditorConfig { tab_size: Some(2), ..Default::default() };
		let lower =
			PartialEditorConfig { tab_size: Some(4), line_numbers: Some(true), ..Default::default() };
	...
	fn merge_partial_config_combines_languages() {
		let mut higher = PartialConfig {
			languages: Some(HashMap::from([(
				"rust".to_string(),
				PartialLanguageConfig { tab_size: Some(4), ..Default::default() },
			)])),
			..Default::default()
		};
	...
	fn merge_languages_same_key_higher_wins() {
		let mut higher = PartialConfig {
			languages: Some(HashMap::from([(
				"rust".to_string(),
				PartialLanguageConfig { tab_size: Some(2), use_tabs: Some(true), ..Default::default() },
			)])),
			..Default::default()
		};
		let lower = PartialConfig {
			languages: Some(HashMap::from([(
				"rust".to_string(),
				PartialLanguageConfig { tab_size: Some(4), auto_indent: Some(false), ..Default::default() },
			)])),
			..Default::default()
		};
	...
	fn resolve_fills_defaults() {
		let partial = PartialConfig { theme: Some(ThemeName::from("dark")), ..Default::default() };
	...
	fn session_config_buffer_overrides() {
		...
		let config = PartialEditorConfig { tab_size: Some(8), ..Default::default() };

		session.set_buffer_override(path.clone(), config);
		assert!(session.get_buffer_override(&path).is_some());
		assert_eq!(session.get_buffer_override(&path).unwrap().tab_size, Some(8));
	...
	fn plugins_with_disabled_are_serialized() {
		...
		assert_eq!(plugins.len(), 1, "Only disabled plugins should be serialized");
	...
	fn plugin_path_never_serialized() {
		...
		assert!(plugin.path.is_none(), "Path is runtime-discovered and should never be serialized");
	}
	...
	fn resolving_partial_with_disabled_plugin_preserves_state() {
		...
		let partial = PartialConfig {
			plugins: Some(HashMap::from([(
				"my_plugin".to_string(),
				PartialPluginConfig { enabled: Some(false), path: None, settings: serde_json::Value::Null },
			)])),
			..Default::default()
		};
		...
		assert!(plugin.is_some(), "Disabled plugin should be in resolved config");
		assert!(!plugin.unwrap().enabled, "Plugin should remain disabled after resolve");
	}
	...
	fn merge_plugins_preserves_higher_precedence_disabled_state() {
		...
		let plugin = plugins.get("my_plugin").unwrap();
		assert_eq!(plugin.enabled, Some(false), "Higher precedence disabled state should win");
	}
	...
	fn roundtrip_disabled_plugin_only_saves_delta() {
		...
		assert!(json.contains("plugin_b"), "Disabled plugin should be in serialized JSON");
		assert!(!json.contains("plugin_a"), "Enabled plugin_a should not be in serialized JSON");
		assert!(!json.contains("plugin_c"), "Enabled plugin_c should not be in serialized JSON");
