...
use dx_core::config::PluginConfig;
...
#[cfg(feature = "plugins")]
use dx_plugin_runtime::PluginThreadHandle;
...
impl PluginManager {
	...
	pub fn new(
		...
		#[cfg(feature = "plugins")]
		{
			if enable {
				...
				match PluginThreadHandle::spawn(services) {
					Ok(handle) => return Self { inner: Some(handle), pending_injected_commands: Vec::new() },
					Err(e) => {
			...
			Self { inner: None, pending_injected_commands: Vec::new() }
		}
		...
		#[cfg(not(feature = "plugins"))]
		{
			...
			Self { _phantom: std::marker::PhantomData, pending_injected_commands: Vec::new() }
		}
	}
	...
	#[cfg(feature = "plugins")]
	pub fn search_handles_handle(&self) -> Option<dx_core::api::SearchHandleRegistry> {
	...
	pub fn execute_action_async(
		&self,
		action_name: &str,
	) -> Option<anyhow::Result<dx_plugin_runtime::thread::oneshot::Receiver<anyhow::Result<()>>>> {
		self.inner.as_ref().map(|m| m.execute_action_async(action_name))
	}

	/// List all loaded plugins.
	#[cfg(feature = "plugins")]
	pub fn list_plugins(&self) -> Vec<dx_plugin_runtime::backend::quickjs_backend::TsPluginInfo> {
		self.inner.as_ref().map(|m| m.list_plugins()).unwrap_or_default()
	}
	...
	pub fn load_plugins_from_dir_with_config_request(
		...
		dx_plugin_runtime::thread::oneshot::Receiver<dx_plugin_runtime::thread::PluginsDirLoadResult>,
	> {
		self
			.inner
			.as_ref()
			.and_then(|m| m.load_plugins_from_dir_with_config_request(dir, plugin_configs).ok())
	}
	...
	pub fn load_plugin_from_source_request(
		...
		name: &str,
		is_typescript: bool,
	) -> Option<dx_plugin_runtime::thread::oneshot::Receiver<anyhow::Result<()>>> {
		self
			.inner
			.as_ref()
			.and_then(|m| m.load_plugin_from_source_request(source, name, is_typescript).ok())
	}
	...
	pub fn list_plugins_request(
		...
		dx_plugin_runtime::thread::oneshot::Receiver<
			Vec<dx_plugin_runtime::backend::quickjs_backend::TsPluginInfo>,
		>,
	> {
		self.inner.as_ref().and_then(|m| m.list_plugins_request().ok())
	}
	...
	pub fn has_hook_handlers(&self, hook_name: &str) -> bool {
		#[cfg(feature = "plugins")]
		{
			self.inner.as_ref().map(|m| m.has_hook_handlers(hook_name)).unwrap_or(false)
		}
		#[cfg(not(feature = "plugins"))]
		{
			let _ = hook_name;
	...
	pub fn has_subscribers(&self, hook_name: &str) -> bool {
		#[cfg(feature = "plugins")]
		{
			self.inner.as_ref().map(|m| m.has_subscribers(hook_name)).unwrap_or(false)
		}
		#[cfg(not(feature = "plugins"))]
		{
			let _ = hook_name;
