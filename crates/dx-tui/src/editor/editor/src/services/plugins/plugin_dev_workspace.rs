...
//! - A copy of `dx.d.ts` for type definitions
...
/// - `skipLibCheck: true` avoids checking dx.d.ts itself
...
impl PluginDevWorkspace {
	...
	///
	/// This creates a temp directory, copies `dx.d.ts` from the source,
	...
	/// * `fresh_dts_source` - Path to the `dx.d.ts` file to copy
	pub fn create(buffer_id: usize, content: &str, fresh_dts_source: &Path) -> Result<Self, String> {
		let cache_dir =
			dirs::cache_dir().ok_or_else(|| "Could not determine cache directory".to_string())?;
		let dir = cache_dir.join("dx").join("plugin-dev").join(buffer_id.to_string());
		...
		// Copy dx.d.ts
		let dest_dts = dir.join("dx.d.ts");
		std::fs::copy(fresh_dts_source, &dest_dts)
			.map_err(|e| format!("Failed to copy dx.d.ts: {}", e))?;
		...
		tracing::info!("Created plugin dev workspace at {:?} for buffer {}", dir, buffer_id);
	...
	pub fn cleanup(&self) {
		if self.dir.exists() {
			if let Err(e) = std::fs::remove_dir_all(&self.dir) {
				tracing::warn!("Failed to clean up plugin dev workspace {:?}: {}", self.dir, e);
			} else {
