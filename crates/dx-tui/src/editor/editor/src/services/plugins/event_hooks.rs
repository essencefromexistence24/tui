...
impl EventHooks for Event {
	fn before_hook(&self, buffer_id: BufferId) -> Option<HookArgs> {
		match self {
			Self::Insert { position, text, cursor_id: _ } => {
				Some(HookArgs::BeforeInsert { buffer_id, position: *position, text: text.clone() })
			}
			Self::Delete { range, .. } => {
				Some(HookArgs::BeforeDelete { buffer_id, start: range.start, end: range.end })
			}
	...
	fn after_hook(&self, buffer_id: BufferId) -> Option<HookArgs> {
		match self {
			Self::Insert { position, text, cursor_id: _ } => Some(HookArgs::AfterInsert {
				buffer_id,
				position: *position,
				text: text.clone(),
			...
			Self::Delete { range, deleted_text, .. } => Some(HookArgs::AfterDelete {
				buffer_id,
				start: range.start,
				end: range.end,
			...
			Self::MoveCursor { cursor_id, old_position, new_position, .. } => {
...
pub fn apply_event_with_hooks(
	...
	if let Some(mut after_args) = event.after_hook(buffer_id) {
		...
		if let HookArgs::CursorMoved { new_position, ref mut line, ref mut text_properties, .. } =
			after_args
		{
...
mod tests {
	...
	use dx_core::CursorId;

	#[test]
	fn test_insert_event_has_hooks() {
		let event = Event::Insert { position: 0, text: "test".to_string(), cursor_id: CursorId(0) };
	...
	fn test_delete_event_has_hooks() {
		let event =
			Event::Delete { range: 0..5, deleted_text: "test".to_string(), cursor_id: CursorId(0) };
