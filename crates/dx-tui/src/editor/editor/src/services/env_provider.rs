...
const ENV_INPUT_FILES: &[&str] =
	&[".envrc", "mise.toml", ".mise.toml", ".tool-versions", "pyvenv.cfg", ".venv/pyvenv.cfg"];
...
impl EnvProvider {
	...
	pub fn is_active(&self) -> bool {
		self.state.read().map(|s| !s.snippet.trim().is_empty()).unwrap_or(false)
	}
		...
		Fut: Future<Output = Option<String>>,
	{
		...
		if let Ok(mut s) = self.state.write() {
			s.cache = Some(Cached { inputs_hash: hash, vars: vars.clone() });
		}
		vars
	}
	...
	pub fn current_local_delta_blocking(&self) -> EnvDelta {
		self.capture_delta_blocking(|script| {
			...
			let output =
				std::process::Command::new(&shell).arg("-lc").arg(&script).hide_window().output().ok()?;
	...
	fn capture_delta_blocking(&self, run: impl FnOnce(String) -> Option<String>) -> EnvDelta {
		...
		if let Ok(mut s) = self.state.write() {
			s.delta_cache = Some(CachedDelta { inputs_hash: hash, delta: delta.clone() });
		}
		delta
	}
}
...
pub(crate) const DELTA_SENTINEL: &str = "__DX_ENV_DELTA_SENTINEL_b3f1c2__";
...
fn parse_delta(stdout: &str) -> EnvDelta {
	...
	let baseline_lookup: std::collections::HashMap<&str, &str> =
...
impl EnvStore {
	...
	pub fn for_project_dir(project_state_dir: &Path) -> Self {
		Self { path: project_state_dir.join("env.json") }
	}
...
mod tests {
	...
	fn for_session_restores_a_persisted_recipe_when_trusted() {
		...
		first.set("eval \"$(direnv export bash)\"".into(), Some(PathBuf::from("/proj")));
