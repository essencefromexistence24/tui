...
/// Top-level package.json manifest for Dx packages.
///
...
#[schemars(
	title = "Dx Package Manifest",
	description = "Schema for Dx plugin and theme package.json files"
)]
...
fn process_language_pack(
	...
	if let Some(lang_config) = &fresh.language {
		result.language_configs.push((lang_id.clone(), lang_config.to_language_config()));
	}

	// LSP config
	if let Some(lsp_config) = &fresh.lsp {
		result.lsp_configs.push((lang_id.clone(), lsp_config.to_lsp_config()));
	}
}
...
fn process_bundle(pkg_dir: &Path, manifest: &PackageManifest, result: &mut PackageScanResult) {
	...
	for lang in &fresh.languages {
		...
		if let Some(lang_config) = &lang.language {
			result.language_configs.push((lang.id.clone(), lang_config.to_language_config()));
		}

		// LSP config
		if let Some(lsp_config) = &lang.lsp {
			result.lsp_configs.push((lang.id.clone(), lsp_config.to_lsp_config()));
		}
	}
...
mod tests {
	...
	fn test_parse_bundle_manifest() {
		...
		assert_eq!(elixir.grammar.as_ref().unwrap().extensions, vec!["ex", "exs"]);
	...
	fn test_scan_language_pack() {
		...
		assert_eq!(result.language_configs[0].1.comment_prefix, Some("//".to_string()));
	...
	fn test_scan_bundle() {
		...
		std::fs::write(grammars_dir.join("Elixir.sublime-syntax"), "# dummy grammar").unwrap();
