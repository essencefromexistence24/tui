...
mod imp {
	...
	pub fn get_release_json(url: &str) -> Result<String, String> {
		...
		let response = agent
			.get(url)
			.header("User-Agent", "dx-editor-update-checker")
	...
	pub fn download_to_file(url: &str, target: &Path) -> Result<u16, String> {
		...
		let mut reader = response.into_body().into_with_config().limit(HTTP_FETCH_MAX_BYTES).reader();
