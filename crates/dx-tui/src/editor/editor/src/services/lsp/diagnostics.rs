...
pub fn invalidate_cache_for_file(file_path: &str) {
	if let Ok(mut cache) = DIAGNOSTIC_CACHE.lock() {
		if cache.remove(file_path).is_some() {
			tracing::debug!("DIAG CACHE: invalidated cache for {} (buffer edited)", file_path);
		}
	}
}
...
pub fn diagnostic_to_overlay(
	...
	let (face, priority, theme_key) = match diagnostic.severity {
		Some(DiagnosticSeverity::ERROR) => (
			OverlayFace::Background { color: theme.diagnostic_error_bg },
			100, // Highest priority
			"diagnostic.error_bg",
		),
		Some(DiagnosticSeverity::WARNING) => (
			OverlayFace::Background { color: theme.diagnostic_warning_bg },
			50, // Medium priority
			"diagnostic.warning_bg",
		),
		Some(DiagnosticSeverity::INFORMATION) => (
			OverlayFace::Background { color: theme.diagnostic_info_bg },
			30, // Lower priority
			"diagnostic.info_bg",
		),
		Some(DiagnosticSeverity::HINT) | None => (
			OverlayFace::Background { color: theme.diagnostic_hint_bg },
			10, // Lowest priority
			"diagnostic.hint_bg",
		),
