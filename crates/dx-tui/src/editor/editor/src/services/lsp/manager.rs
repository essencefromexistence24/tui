...
fn path_to_uri(path: &Path) -> Option<Uri> {
	let abs =
		if path.is_absolute() { path.to_path_buf() } else { std::env::current_dir().ok()?.join(path) };
			...
			"textDocument/completion" => {
				...
				if register {
					if let Some(opts) = register_options {
						if let Some(chars) = opts.get("triggerCharacters").and_then(|v| v.as_array()) {
							self.completion_trigger_characters =
								chars.iter().filter_map(|v| v.as_str().map(str::to_string)).collect();
						}
						if let Some(resolve) = opts.get("resolveProvider").and_then(serde_json::Value::as_bool)
						{
			...
			"textDocument/semanticTokens" => {
				if register {
					...
					match register_options.and_then(|opts| {
						serde_json::from_value::<lsp_types::SemanticTokensOptions>(opts.clone()).ok()
					}) {
						Some(opts) => {
...
impl LspManager {
	...
	pub fn window_id(&self) -> dx_core::WindowId {
		self.window_id
	}

	/// Create a new LSP manager owned by the given window.
	pub fn new(window_id: dx_core::WindowId, root_uri: Option<Uri>) -> Self {
		Self {
			window_id,
			handles: Vec::new(),
			config: HashMap::new(),
	...
	fn lsp_autostart_allowed(&self) -> bool {
		...
		self.workspace_trust.as_ref().map(|t| t.level() == TrustLevel::Trusted).unwrap_or(true)
	}
	...
	pub fn apply_dynamic_capabilities(
		...
		for (method, options) in registrations {
			if sh.capabilities.apply_dynamic_registration(method, options.as_ref(), register) {
				changed = true;
			}
		}
		changed
	...
	pub fn semantic_tokens_full_supported(&self, language: &str) -> bool {
		self.get_handles(language).iter().any(|sh| {
			sh.feature_filter.allows(LspFeature::SemanticTokens) && sh.capabilities.semantic_tokens_full
		})
	}
	...
	pub fn semantic_tokens_range_supported(&self, language: &str) -> bool {
		self.get_handles(language).iter().any(|sh| {
			sh.feature_filter.allows(LspFeature::SemanticTokens) && sh.capabilities.semantic_tokens_range
		})
	}
	...
	pub fn is_completion_trigger_char(&self, ch: char, language: &str) -> bool {
		...
		self.get_handles(language).iter().any(|sh| {
			sh.feature_filter.allows(LspFeature::Completion)
				&& sh.capabilities.completion_trigger_characters.contains(&ch_str)
		})
	}
	...
	pub fn try_spawn(&mut self, language: &str, file_path: Option<&Path>) -> LspSpawnResult {
		...
		if self.handles.iter().any(|sh| sh.handle.scope().accepts(language)) {
		...
		let configs = match self.config.get(language) {
			...
			_ => {
				...
				if self.handles.iter().any(|sh| sh.handle.scope().is_universal()) {
					return LspSpawnResult::Spawned;
				}
		...
		if spawned || self.handles.iter().any(|sh| sh.handle.scope().is_universal()) {
	...
	pub fn set_language_root_uri(&mut self, language: &str, uri: Uri) -> bool {
		...
		self.per_language_root_uris.insert(language.to_string(), uri.clone());

		// If there's an existing server for this language, restart it with the new root
		if self.handles.iter().any(|sh| sh.handle.scope().accepts(language)) {
			tracing::info!("Restarting {} LSP server with new root: {}", language, uri.as_str());
	...
	pub fn resolve_root_uri(&self, language: &str, file_path: Option<&Path>) -> Option<Uri> {
		...
		if let Some(path) = file_path {
			...
			let mapped =
				self.path_translation.as_ref().and_then(|t| t.host_to_remote(&root)).unwrap_or(root);
	...
	pub fn get_handle(&self, language: &str) -> Option<&LspHandle> {
		self.handles.iter().find(|sh| sh.handle.scope().accepts(language)).map(|sh| &sh.handle)
	}
	...
	pub fn get_handle_mut(&mut self, language: &str) -> Option<&mut LspHandle> {
		self.handles.iter_mut().find(|sh| sh.handle.scope().accepts(language)).map(|sh| &mut sh.handle)
	}
	...
	pub fn get_handles(&self, language: &str) -> Vec<&ServerHandle> {
		self.handles.iter().filter(|sh| sh.handle.scope().accepts(language)).collect()
	}
	...
	pub fn get_handles_mut(&mut self, language: &str) -> Vec<&mut ServerHandle> {
		self.handles.iter_mut().filter(|sh| sh.handle.scope().accepts(language)).collect()
	}
	...
	pub fn force_spawn(
		...
		for config in &configs {
			...
			tracing::info!("Spawning LSP server '{}' for language: {}", server_name, language);
			...
			match LspHandle::spawn(
				...
				Ok(handle) => {
					...
					if let Err(e) = handle.initialize(effective_root, config.initialization_options.clone()) {
						tracing::error!(
				...
				Err(e) => {
					tracing::error!("Failed to spawn LSP handle for {} ({}): {}", language, server_name, e);
				}
			}
		}
	...
	fn ensure_universal_servers_running(&mut self, file_path: Option<&Path>) {
		...
		let long_running_spawner = self.long_running_spawner.as_ref().cloned().unwrap_or_else(|| {
			std::sync::Arc::new(crate::services::remote::LocalLongRunningSpawner::new(
				std::sync::Arc::new(crate::services::env_provider::EnvProvider::inactive()),
				std::sync::Arc::new(crate::services::workspace_trust::WorkspaceTrust::permissive()),
			))
		...
		for config in &self.universal_configs {
			...
			match LspHandle::spawn(
				...
				Ok(handle) => {
					...
					if let Err(e) = handle.initialize(effective_root, config.initialization_options.clone()) {
						tracing::error!("Failed to initialize universal LSP server '{}': {}", server_name, e);
						continue;
					}
					tracing::info!("Universal LSP server '{}' initialization started", server_name);
				...
				Err(e) => {
					tracing::error!("Failed to spawn universal LSP server '{}': {}", server_name, e);
				}
			}
		}
	...
	pub fn handle_server_crash(&mut self, language: &str, server_name: &str) -> String {
		...
		if self.handles.iter().any(|sh| sh.name == server_name && sh.handle.scope().is_universal()) {
		...
		let now = Instant::now();
		let attempt_number = self.restart_attempts.get(language).map(|v| v.len()).unwrap_or(0);
	...
	pub fn manual_restart_server(
		...
		let is_universal = self.universal_configs.iter().any(|c| c.display_name() == server_name);
		let config = if is_universal {
			self.universal_configs.iter().find(|c| c.display_name() == server_name).cloned()
		} else {
			self
				.config
				.get(language)
		...
		let Some(config) = config else {
			let message = format!("No config found for server '{}' ({})", server_name, language);
			tracing::error!("{}", message);
			return (false, message);
		};

		if config.command.is_empty() {
			let message = format!("LSP command is empty for {} server '{}'", language, server_name);
			tracing::error!("{}", message);
			return (false, message);
		}
		...
		let scope = if is_universal { LanguageScope::all() } else { LanguageScope::single(language) };
		...
		match LspHandle::spawn(
			...
			Ok(handle) => {
				...
				if let Err(e) = handle.initialize(effective_root, config.initialization_options.clone()) {
					let message =
						format!("Failed to initialize LSP server '{}' for {}: {}", server_name, language, e);
					tracing::error!("{}", message);
					return (false, message);
				}
			...
			Err(e) => {
				let message =
					format!("Failed to start LSP server '{}' for {}: {}", server_name, language, e);
				tracing::error!("{}", message);
				(false, message)
			}
		}
	...
	pub fn running_servers(&self) -> Vec<String> {
		let mut labels: Vec<String> =
			self.handles.iter().map(|sh| sh.handle.scope().label().to_string()).collect();
		labels.sort();
		labels.dedup();
		labels
	}
	...
	pub fn shutdown_server_by_name(&mut self, language: &str, server_name: &str) -> bool {
		let Some(idx) = self.handles.iter().position(|sh| sh.name == server_name) else {
			tracing::warn!("No running LSP server named '{}' found for {}", server_name, language);
			return false;
		};
	...
	pub fn shutdown_all(&mut self) {
		for sh in &self.handles {
			tracing::info!("Shutting down LSP server '{}' ({})", sh.name, sh.handle.scope().label());
			fire_and_forget(sh.handle.shutdown());
		}
		self.handles.clear();
	}
...
fn detect_language_by_config(
	...
	if let Some(filename) = path.file_name().and_then(|f| f.to_str()) {
		...
		for (language_name, lang_config) in languages {
			if lang_config.filenames.iter().any(|f| !is_glob_pattern(f) && f == filename) {
...
fn header_in_cpp_tree(path: &std::path::Path) -> bool {
	...
	if let Ok(entries) = std::fs::read_dir(start_dir) {
		for entry in entries.flatten() {
			...
			if matches!(ext, "cc" | "cpp" | "cxx" | "C" | "c++" | "hpp" | "hh" | "hxx") {
				return true;
			}
		}
	}
...
mod tests {
	...
	fn test_lsp_manager_new() {
		...
		let manager = LspManager::new(dx_core::WindowId(1), root_uri.clone());
	...
	fn test_lsp_manager_set_language_config() {
		let mut manager = LspManager::new(dx_core::WindowId(1), None);
	...
	fn test_detect_language_from_config() {
		...
		assert_eq!(detect_language(Path::new("main.rs"), &languages), Some("rust".to_string()));
		assert_eq!(detect_language(Path::new("index.js"), &languages), Some("javascript".to_string()));
		assert_eq!(detect_language(Path::new("App.jsx"), &languages), Some("javascript".to_string()));
		assert_eq!(detect_language(Path::new("Program.cs"), &languages), Some("csharp".to_string()));
	...
	fn test_detect_language_path_glob() {
		...
		assert_eq!(detect_language(Path::new("/etc/rc.conf"), &languages), Some("shell".to_string()));
		assert_eq!(
		...
		assert_eq!(detect_language(Path::new("lfrc"), &languages), Some("shell".to_string()));
	}
	...
	fn test_detect_language_h_stays_c_without_cpp_signals() {
		...
		assert_eq!(detect_language(Path::new("foo.h"), &languages), Some("c".to_string()));
	}
	...
	fn dynamic_registration_enables_then_disables_inlay_hints() {
		...
		assert!(recognized, "inlayHint must be a recognized capability method");
		assert!(caps.inlay_hints, "dynamic registration must enable inlay hints");
	...
	fn dynamic_registration_ignores_unknown_methods() {
		...
		let mut caps = ServerCapabilitySummary::default();
		let recognized = caps.apply_dynamic_registration("workspace/didChangeWatchedFiles", None, true);
		assert!(!recognized);
	}
	...
	fn dynamic_registration_parses_completion_options() {
		...
		let recognized = caps.apply_dynamic_registration("textDocument/completion", Some(&opts), true);
		assert!(recognized);
