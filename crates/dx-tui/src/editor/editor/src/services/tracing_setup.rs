...
pub fn init_global(log_file_path: &Path) -> Option<TracingHandles> {
	...
	Some(TracingHandles { warning: warning_handle, status: status_handle })
}
...
pub fn build_subscriber(
	...
	let span_events = if std::env::var("DX_LOG_SPANS").is_ok() {
		fmt::format::FmtSpan::CLOSE
	} else {
		fmt::format::FmtSpan::NONE
	};

	let fmt_layer = fmt::layer().with_writer(Arc::new(log_file)).with_span_events(span_events);
...
mod tests {
	...
	fn create_test_subscriber() -> TestSubscriber {
		...
		let subscriber =
			build_subscriber(log_file.reopen().unwrap(), Some(warning_layer), Some(status_layer));
	...
	fn test_warning_log_captures_warn_level() {
		...
		assert!(contents.contains("Test warning message"), "Log should contain message");
	}
	...
	fn test_warning_log_captures_error_level() {
		...
		assert!(contents.contains("Test error message"), "Log should contain message");
	}
	...
	fn test_warning_log_ignores_info_level() {
		...
		assert!(!contents.contains("Test info message"), "Log should NOT contain INFO message");
	}
	...
	fn test_warning_log_multiple_warnings() {
		...
		let contents = std::fs::read_to_string(&path).expect("Failed to read log");
		assert!(contents.contains("First warning"), "Log should contain first warning");
		assert!(contents.contains("An error"), "Log should contain error");
		assert!(contents.contains("Second warning"), "Log should contain second warning");
	}
}
