...
fn stamp_file_path(data_dir: &std::path::Path) -> PathBuf {
	data_dir.join("dx").join(STAMP_FILE_NAME)
}
...
fn parse_stamp_content(content: &str) -> Option<StampData> {
	...
	Some(StampData { unique_id, last_date })
}
...
mod tests {
	...
	fn parse_stamp_content_valid() {
		for content in ["abc123\n2025-01-15\n", "abc123\n2025-01-15"] {
			...
			assert_eq!(result.last_date, NaiveDate::from_ymd_opt(2025, 1, 15).unwrap());
		}
	}
	...
	fn parse_stamp_content_invalid() {
		for content in [
			...
			assert!(parse_stamp_content(content).is_none(), "expected None for {:?}", content);
		}
	}
