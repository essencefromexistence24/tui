...
use crate::services::remote::{
	ConnectionParams, KubeConnection, KubeTarget, LocalLongRunningSpawner, LocalProcessSpawner,
	LongRunningSpawner, ProcessSpawner, RemoteFileSystem, RemoteLongRunningSpawner,
	RemoteProcessSpawner, SshConnection, SshError, TransportError, build_kube_agent_terminal_args,
	build_kube_terminal_args, build_ssh_agent_terminal_args, build_ssh_terminal_args,
	spawn_kube_reconnect_task, spawn_reconnect_task,
};
...
pub enum CommandWrap {
	...
	Ssh { params: ConnectionParams, remote_dir: Option<String> },
	...
	Kube { target: KubeTarget, base_env: Vec<(String, String)> },
}
...
impl Authority {
			...
			CommandWrap::Ssh { params, remote_dir } => {
				SessionAuthoritySpec::RemoteAgent(RemoteAgentSpec {
					transport: RemoteTransportSpec::Ssh {
						...
						port: params.port,
						identity_file: params.identity_file.as_ref().map(|p| p.to_string_lossy().into_owned()),
	...
	pub fn terminal_command(&self, argv: &[String]) -> TerminalWrapper {
		...
		match &self.command_wrap {
			CommandWrap::Direct => {
				...
				TerminalWrapper { command: cmd.clone(), args: rest.to_vec(), manages_cwd: false }
			}
			CommandWrap::Prefix(prefix) => {
				...
				TerminalWrapper { command: prefix[0].clone(), args, manages_cwd: true }
			}
			CommandWrap::Ssh { params, remote_dir } => {
	...
	pub fn local(
		...
		Self {
			...
			process_spawner: Arc::new(LocalProcessSpawner::new(Arc::clone(&env), Arc::clone(&trust))),
	...
	pub fn kube(
		...
		Self {
			...
			command_wrap: CommandWrap::Kube { target: target.clone(), base_env: base_env.to_vec() },
		}
	}
	...
	pub fn kube_from_connection(
		...
		let filesystem: Arc<dyn FileSystem + Send + Sync> =
			Arc::new(RemoteFileSystem::new(channel.clone(), connection.connection_string().to_string()));
		let process_spawner: Arc<dyn ProcessSpawner> =
			Arc::new(RemoteProcessSpawner::new(channel, Arc::clone(&env), Arc::clone(&trust)));
		...
		Self::kube(filesystem, process_spawner, long_running_spawner, &target, &base_env, trust, env)
	}
	...
	pub fn from_plugin_payload(
		...
		let (process_spawner, long_running_spawner, command_wrap): (
			...
			SpawnerSpec::DockerExec { container_id, user, workspace, env: docker_env } => {
				...
				(
					Arc::new(crate::services::authority::docker_spawner::DockerExecSpawner::with_env(
						container_id.clone(),
						user.clone(),
						workspace.clone(),
						docker_env.clone(),
					...
					Arc::new(crate::services::authority::docker_spawner::DockerLongRunningSpawner::with_env(
						container_id,
						user,
						workspace,
						docker_env,
		...
		let terminal_wrapper = match payload.terminal_wrapper {
			...
			TerminalWrapperSpec::Explicit { command, args, manages_cwd } => {
				TerminalWrapper { command, args, manages_cwd }
			}
		};
...
impl SessionAuthoritySpec {
	...
	/// Reduce this backend spec to the [`dx_core::api::RemoteBackendInfo`]
	...
	pub fn remote_backend_info(&self, connected: bool) -> Option<dx_core::api::RemoteBackendInfo> {
		...
		Some(dx_core::api::RemoteBackendInfo { kind: kind.to_string(), detail, connected })
	}
}
...
impl RemoteAgentSpec {
	...
	pub fn into_kube_target(self) -> (KubeTarget, Vec<(String, String)>) {
		match self.transport {
			RemoteTransportSpec::KubectlExec { context, namespace, pod, container, workspace } => {
				(KubeTarget { context, namespace, pod, container, workspace }, self.base_env)
			}
...
pub async fn connect_kube_authority(
	...
	type Built =
		Result<(KubeConnection, tokio::task::JoinHandle<()>, tokio::runtime::Runtime), TransportError>;
	...
	Ok((authority, KubeKeepalive { reconnect, _connection: connection, _runtime: runtime }))
}
...
pub async fn connect_ssh_authority(
	...
	type Built =
		Result<(SshConnection, tokio::task::JoinHandle<()>, tokio::runtime::Runtime), SshError>;
		...
		.spawn(move || {
			let built: Built = (|| {
				...
				let (connection, reconnect) = runtime.block_on(async {
					...
					let reconnect = spawn_reconnect_task(connection.channel(), connection.params().clone());
	...
	let (connection, reconnect, runtime) =
		rx.await.map_err(|_| SshError::AgentStartFailed("connect thread vanished".to_string()))??;
	...
	let process_spawner: Arc<dyn ProcessSpawner> =
		Arc::new(RemoteProcessSpawner::new(channel.clone(), Arc::clone(&env), Arc::clone(&trust)));
	let long_running_spawner: Arc<dyn LongRunningSpawner> = Arc::new(RemoteLongRunningSpawner::new(
	...
	Ok((authority, SshKeepalive { reconnect, _connection: connection, _runtime: runtime }))
}
...
mod tests {
	...
	fn kube_terminal_wrapper_reparents_into_pod() {
		...
		let override_shell =
			crate::config::TerminalShellConfig { command: "/usr/local/bin/fish".into(), args: vec![] };
		let after = wrapper.clone().with_user_shell_override(Some(&override_shell));
	...
	fn payload_accepts_docker_exec_env_pairs() {
		...
		let payload: AuthorityPayload = serde_json::from_value(json).expect("env field is accepted");
		...
		let payload2: AuthorityPayload = serde_json::from_value(json_no_env).expect("env is optional");
	...
	fn payload_defaults_manages_cwd_to_true_for_explicit_wrapper() {
		...
		let payload: AuthorityPayload = serde_json::from_value(json).expect("manages_cwd is optional");
		let auth = Authority::from_plugin_payload(
			payload,
			Arc::new(WorkspaceTrust::permissive()),
			Arc::new(crate::services::env_provider::EnvProvider::inactive()),
	...
	fn user_shell_override_is_noop_when_wrapper_manages_cwd() {
		...
		let override_shell =
			crate::config::TerminalShellConfig { command: "/usr/local/bin/fish".into(), args: vec![] };
		let wrapper = docker.clone().with_user_shell_override(Some(&override_shell));
	...
	fn ssh_terminal_command_runs_agent_on_remote_in_workspace() {
		...
		let w = auth.terminal_command(&["claude".into(), "--resume".into(), "u-1".into()]);
		assert_eq!(w.command, "ssh", "agent must launch through ssh, not locally");
		assert!(w.manages_cwd, "ssh re-parents the agent, so it manages its own cwd");
	...
	fn path_translation_round_trips_under_workspace() {
		...
		assert_eq!(pt.remote_to_host(&remote).expect("remote under remote_root"), host.to_path_buf(),);
	}
	...
	fn path_translation_returns_none_outside_root() {
		...
		assert!(pt.host_to_remote(Path::new("/usr/include/stdio.h")).is_none());
		assert!(pt.remote_to_host(Path::new("/usr/include/stdio.h")).is_none());
	}
	...
	fn from_plugin_payload_with_path_translation_round_trips() {
		...
		let pt = auth.path_translation.expect("authority carries the translation");
