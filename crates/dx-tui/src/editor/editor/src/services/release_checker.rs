...
impl InstallMethod {
	...
	pub fn update_command(&self) -> Option<&'static str> {
		Some(match self {
			Self::Homebrew => "brew upgrade dx-editor",
			Self::Cargo => "cargo install --locked dx-editor",
			Self::Npm => "npm update -g @dx-editor/dx-editor",
			Self::Aur => "yay -Syu dx-editor  # or use your AUR helper",
...
impl UpdateChecker {
	...
	pub fn is_update_available(&self) -> bool {
		self.last_result.as_ref().map(|r| r.update_available).unwrap_or(false)
	}
	...
	pub fn latest_version(&self) -> Option<&str> {
		self
			.last_result
			.as_ref()
			.and_then(|r| if r.update_available { Some(r.latest_version.as_str()) } else { None })
	}
}
...
pub fn start_periodic_update_check(
	...
	UpdateChecker { receiver: rx, thread: handle, last_result: None }
}
...
pub fn start_update_check(
	...
	UpdateCheckHandle { receiver: rx, thread: handle }
}
...
fn parse_version_from_json(json: &str) -> Result<String, String> {
	...
	let start =
		json.find(tag_name_key).ok_or_else(|| "tag_name not found in response".to_string())?;

	let after_key = &json[start + tag_name_key.len()..];

	let value_start =
		after_key.find('"').ok_or_else(|| "Invalid JSON: missing quote after tag_name".to_string())?;

	let value_content = &after_key[value_start + 1..];
	let value_end =
		value_content.find('"').ok_or_else(|| "Invalid JSON: unclosed quote".to_string())?;
...
pub fn check_for_update(releases_url: &str) -> Result<ReleaseCheckResult, String> {
	...
	Ok(ReleaseCheckResult { latest_version, update_available, install_method })
}
...
mod tests {
	...
	fn test_detect_install_method() {
		let cases = [
			("/opt/homebrew/Cellar/fresh/0.1.26/bin/fresh", InstallMethod::Homebrew),
			("/usr/local/Cellar/fresh/0.1.26/bin/fresh", InstallMethod::Homebrew),
			("/home/linuxbrew/.linuxbrew/bin/fresh", InstallMethod::Homebrew),
			("/home/user/.cargo/bin/fresh", InstallMethod::Cargo),
			("C:\\Users\\user\\.cargo\\bin\\fresh.exe", InstallMethod::Cargo),
			("/usr/local/lib/node_modules/dx-editor/bin/fresh", InstallMethod::Npm),
	...
	fn test_parse_version_from_json() {
		...
		let cases = [
			...
			(r#"{"tag_name": "v0.2.0", "name": "v0.2.0", "draft": false}"#, "0.2.0"),
		];
	...
	fn test_update_checker_detects_new_version() {
		...
		let mut checker = start_periodic_update_check(&url, time_source, temp_dir.path().to_path_buf());
