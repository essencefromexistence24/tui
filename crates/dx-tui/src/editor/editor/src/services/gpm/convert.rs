...
pub fn gpm_to_crossterm(event: &GpmEvent) -> Option<crossterm::event::MouseEvent> {
	...
	Some(MouseEvent { kind, column: event.x.max(0) as u16, row: event.y.max(0) as u16, modifiers })
}
...
mod tests {
	...
	fn test_gpm_to_crossterm_left_click() {
		...
		assert!(matches!(crossterm_event.kind, MouseEventKind::Down(MouseButton::Left)));
	...
	fn test_gpm_to_crossterm_right_click() {
		...
		let crossterm_event = gpm_to_crossterm(&gpm_event).unwrap();
		assert!(matches!(crossterm_event.kind, MouseEventKind::Down(MouseButton::Right)));
	}
	...
	fn test_gpm_to_crossterm_drag() {
		...
		let crossterm_event = gpm_to_crossterm(&gpm_event).unwrap();
		assert!(matches!(crossterm_event.kind, MouseEventKind::Drag(MouseButton::Left)));
	}
