...
use crate::services::workspace_trust::{WorkspaceTrust, gate};
...
pub trait ProcessSpawner: Send + Sync {
	...
	async fn spawn_to_file(
		...
		Ok(SpawnResult { stdout: String::new(), stderr: result.stderr, exit_code: result.exit_code })
	}
...
impl ProcessSpawner for LocalProcessSpawner {
	async fn spawn(
		...
		let output = cmd.output().await.map_err(|e| SpawnError::Process(e.to_string()))?;
	...
	async fn spawn_cancellable(
			...
			if let Some(parent) = path.parent() {
				if !parent.as_os_str().is_empty() {
					tokio::fs::create_dir_all(parent)
						.await
						.map_err(|e| SpawnError::Process(format!("create_dir_all {:?}: {}", parent, e)))?;
				}
			}
		}

		let mut child = cmd.spawn().map_err(|e| SpawnError::Process(e.to_string()))?;

		let mut child_stdout =
			child.stdout.take().ok_or_else(|| SpawnError::Process("child stdout missing".to_string()))?;
		let mut child_stderr =
			child.stderr.take().ok_or_else(|| SpawnError::Process("child stderr missing".to_string()))?;
		...
		let stderr_task: tokio::task::JoinHandle<std::io::Result<Vec<u8>>> = tokio::spawn(async move {
			let mut buf = Vec::new();
	...
	async fn spawn_to_file(
		...
		let status = child.wait().await.map_err(|e| SpawnError::Process(format!("wait: {}", e)))?;
	...
	async fn captured_env(&self) -> Vec<(String, String)> {
		...
		self
			...
			.current(move |script| async move {
				...
				let (mut data_rx, _result) = channel.request_streaming("exec", params).await.ok()?;
...
impl ProcessSpawner for RemoteProcessSpawner {
	async fn spawn(
		...
		let exit_code = result.get("code").and_then(|v| v.as_i64()).map(|c| c as i32).unwrap_or(-1);
	...
	async fn spawn_to_file(
		...
		Err(SpawnError::Process("stdoutTo is not supported for remote processes".to_string()))
	}
}
...
impl StdioChild {
	...
	pub fn from_tokio_child(mut child: tokio::process::Child, spawned_locally: bool) -> Self {
		...
		Self { inner: child, stdin, stdout, stderr, spawned_locally }
	}
	...
	pub fn from_local_tokio_child(child: tokio::process::Child, post_spawn: PostSpawnAction) -> Self {
...
impl LongRunningSpawner for LocalLongRunningSpawner {
	async fn spawn_stdio(
		...
		gate(&self.trust, command, cwd.map(|p| p.to_string_lossy()).as_deref())?;
		...
		let child = cmd.spawn().map_err(|e| SpawnError::Process(e.to_string()))?;
...
fn build_ssh_remote_args(
	...
	let mut a =
		vec!["-t".to_string(), "-o".to_string(), "StrictHostKeyChecking=accept-new".to_string()];
	if let Some(port) = params.port {
		a.push("-p".to_string());
		a.push(port.to_string());
	}
...
fn agent_login_exec_tail(argv: &[String]) -> String {
	...
	format!("exec ${{SHELL:-/bin/sh}} -lc {}", shell_quote(&format!("exec {joined}")))
}
	...
	async fn captured_env(&self) -> Vec<(String, String)> {
		...
		self
			...
			.current(move |script| async move {
				...
				let output =
					tokio::process::Command::new("ssh").args(&ssh_args).hide_window().output().await.ok()?;
				Some(String::from_utf8_lossy(&output.stdout).into_owned())
			})
			.await
	}
...
impl LongRunningSpawner for RemoteLongRunningSpawner {
	...
	async fn command_exists(&self, command: &str) -> bool {
		...
		let ssh_args = build_ssh_args(&self.params, &remote);
		match tokio::process::Command::new("ssh").args(&ssh_args).hide_window().output().await {
...
mod tests {
	...
	fn resolve_program_is_passthrough_on_unix() {
		assert_eq!(resolve_program("typescript-language-server"), "typescript-language-server");
	...
	fn resolve_program_falls_back_and_resolves_on_windows() {
		assert_eq!(resolve_program("dx-unlikely-binary-name-ygzu9"), "dx-unlikely-binary-name-ygzu9");
	...
	async fn test_local_spawner() {
		...
		let result = spawner.spawn("echo".to_string(), vec!["hello".to_string()], None).await.unwrap();
	...
	async fn test_local_spawner_stdout_to_file() {
		...
		let tmp = std::env::temp_dir().join(format!("dx-spawner-test-{}.out", std::process::id()));
		...
		let result = spawner
			.spawn_to_file("echo".to_string(), vec!["hello-from-disk".to_string()], None, tmp.clone())
			.await
			.unwrap();
	...
	async fn test_local_spawner_cancellable_kill() {
		...
		let task = tokio::spawn(async move {
			spawner
				.spawn_cancellable("sleep".to_string(), vec!["30".to_string()], None, None, kill_rx)
				.await
		});
	...
	async fn local_long_running_spawn_stdio_pipes_output() {
		...
		let mut child = spawner
			.spawn_stdio("sh", &["-c".into(), "echo hi".into()], Vec::new(), None, None)
			.await
	...
	async fn local_long_running_command_exists_for_sh() {
		...
		assert!(!spawner.command_exists("dx-unlikely-binary-name-ygzu9").await);
	}
	...
	async fn local_spawner_applies_active_env_provider() {
		...
		env.set("export DX_ENV_TEST=hi-from-provider".into(), None);
		let spawner = LocalProcessSpawner::new(env, Arc::new(WorkspaceTrust::permissive()));
		let result = spawner
			.spawn("sh".into(), vec!["-c".into(), "printf %s \"$DX_ENV_TEST\"".into()], None)
			.await
			.unwrap();
		assert_eq!(result.exit_code, 0);
	...
	async fn local_spawner_inactive_provider_injects_nothing() {
		...
		let result = spawner
			.spawn("sh".into(), vec!["-c".into(), "printf %s \"${DX_ENV_TEST:-unset}\"".into()], None)
			.await
			.unwrap();
	...
	fn build_remote_exec_with_cwd_and_env() {
		...
		assert_eq!(s, "cd '/proj dir' && exec env VIRTUAL_ENV='/proj/.venv' 'python' 'x.py'");
	}
	...
	fn ssh_remote_env_launcher_is_a_safe_single_quoted_python_oneliner() {
		...
		let inner = launcher.trim_start_matches("exec python3 -c '").trim_end_matches('\'');
	...
	fn build_ssh_terminal_args_without_dir_skips_cd() {
		...
		assert_eq!(
			a,
			vec!["-t", "-o", "StrictHostKeyChecking=accept-new", "u@h", "exec ${SHELL:-/bin/sh} -l",]
		);
	...
	fn build_ssh_agent_terminal_args_runs_agent_in_remote_workspace() {
		...
		let argv = vec!["claude".to_string(), "--resume".to_string(), "u-1".to_string()];
		...
		assert_eq!(
			&a[..8],
			&["-t", "-o", "StrictHostKeyChecking=accept-new", "-p", "2222", "-i", "/k", "u@h",]
		);
	...
	fn build_ssh_agent_terminal_args_quotes_args_with_spaces() {
		...
		let remote_cmd = build_ssh_agent_terminal_args(&params, None, &argv).pop().unwrap();
	...
	fn build_kube_terminal_args_without_workspace_skips_cd() {
		...
		assert_eq!(
			a,
			vec!["exec", "-it", "-n", "dev", "pod-1", "--", "sh", "-lc", "exec ${SHELL:-/bin/sh} -l",]
		);
	}
}
