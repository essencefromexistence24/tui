...
#[test]
fn test_exec_params() {
	let params = exec_params("rg", &["pattern".to_string(), ".".to_string()], Some("/home"));
...
mod agent_integration {
	...
	fn spawn_agent() -> Option<(std::process::ChildStdin, BufReader<std::process::ChildStdout>)> {
	...
	fn test_agent_exists_command() {
		...
		assert!(result.get("exists").and_then(|v| v.as_bool()).unwrap_or(false));
		...
		assert!(resp.is_some());
		let result = resp.unwrap().result.unwrap();
		assert!(!result.get("exists").and_then(|v| v.as_bool()).unwrap_or(true));
	}
	...
	fn test_agent_unknown_method() {
		...
		// Send unknown method
		let resp = send_request(&mut stdin, &mut stdout, "unknown_method", serde_json::json!({}));
		assert!(resp.is_some());
		let resp = resp.unwrap();
