	...
	pub fn apply_to_child(&self, _child_pid: u32) {
		...
		if let Some(ref cgroup_dir) = self.cgroup_dir {
			...
			if let Err(e) = fs::write(&procs_file, format!("{}", _child_pid)) {
				tracing::info!("Failed to move child {} into cgroup {:?}: {}", _child_pid, cgroup_dir, e);
			}
		}
	}
}
		...
		if let Some(ref cgroup_base) = cgroup_path {
			...
			if cgroup_usable {
				...
				tracing::info!(
					...
					self.max_memory_percent.map(|p| format!("{}%", p)).unwrap_or("unlimited".to_string()),
					memory_method,
...
impl SystemResources {
	...
	fn linux_total_memory_mb() -> io::Result<u64> {
		...
		Err(io::Error::new(io::ErrorKind::InvalidData, "Could not parse MemTotal from /proc/meminfo"))
	}
...
mod tests {
	...
	fn test_process_limits_serialization() {
		let limits =
			ProcessLimits { max_memory_percent: Some(50), max_cpu_percent: Some(80), enabled: true };
