...
use super::types::{
	...
	SessionInfo, generate_buffer_id, path_hash,
};
...
pub enum RecoveryScope {
	...
	/// Named daemon (`dx -a NAME`): recovery scoped by daemon name.
...
impl RecoveryStorage {
	...
	fn chunk_path(&self, id: &str, chunk_index: usize) -> PathBuf {
		self.recovery_dir.join(format!("{id}.chunk.{}", chunk_index))
	}
	...
	pub fn save_recovery(
		...
		let meta_file = ChunkedMetadataFile { metadata: metadata.clone(), chunked_index: index };
	...
	pub fn read_chunked_content(&self, id: &str) -> io::Result<Option<ChunkedRecoveryData>> {
		...
		Ok(Some(ChunkedRecoveryData::new(index.original_size, index.final_size, chunks)))
	}
	...
	pub fn reconstruct_from_chunks(&self, id: &str, original_file: &Path) -> io::Result<Vec<u8>> {
		let chunked_data = self
			.read_chunked_content(id)?
			.ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, "Chunked recovery data not found"))?;
	...
	pub fn load_entry(&self, id: &str) -> io::Result<Option<RecoveryEntry>> {
		...
		Ok(Some(RecoveryEntry { id: id.to_string(), metadata, content_path, metadata_path: meta_path }))
	}
			...
			if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
				...
				if !is_valid {
					if let Err(e) = fs::remove_file(&meta_path) {
						tracing::warn!("Failed to remove orphan metadata {}: {}", meta_path.display(), e);
					}
					...
					if let Ok(content) = fs::read_to_string(&path) {
							...
							serde_json::from_str::<super::types::InplaceWriteRecovery>(&content)
						{
							...
							if !recovery.is_in_progress() && recovery.temp_path.exists() {
								...
								if let Err(e) = fs::remove_file(&path) {
									tracing::warn!(
										"Failed to remove orphaned inplace recovery metadata {}: {}",
										path.display(),
										e
									);
								}
...
impl Default for RecoveryStorage {
	fn default() -> Self {
		Self::new().unwrap_or_else(|_| Self { recovery_dir: std::env::temp_dir().join("dx-recovery") })
	}
}
...
mod tests {
	...
	fn create_test_storage() -> (RecoveryStorage, TempDir) {
		...
		let storage = RecoveryStorage { recovery_dir: temp_dir.path().to_path_buf() };
		(storage, temp_dir)
	}
	...
	fn test_save_and_load_recovery() {
		...
		let metadata =
			storage.save_recovery(&id, chunks, Some(path), None, Some(1), 0, content.len()).unwrap();
	...
	fn test_list_entries() {
		...
		storage.save_recovery("id1", chunks1, None, Some("Buffer 1"), None, 0, 8).unwrap();
		let chunks2 = vec![RecoveryChunk::new(0, 0, b"content2".to_vec())];
		storage.save_recovery("id2", chunks2, None, Some("Buffer 2"), None, 0, 8).unwrap();
	...
	fn test_delete_recovery() {
		...
		storage.save_recovery(id, chunks, None, Some("Test"), None, 0, 7).unwrap();
	...
	fn test_cleanup_orphans() {
		...
		let chunks = vec![RecoveryChunk::new(0, 0, b"content".to_vec())];
		storage.save_recovery("complete", chunks, None, Some("Test"), None, 0, 7).unwrap();
	...
	fn test_chunked_recovery_read_content() {
		...
		storage.save_recovery(id, chunks, None, None, None, 200, 206).unwrap();
	...
	fn test_chunked_recovery_delete() {
		...
		storage.save_recovery(id, chunks, None, None, None, 100, 95).unwrap();
	...
	fn test_chunked_recovery_cleanup_orphan_chunks() {
		...
		storage.save_recovery("valid", chunks, None, None, None, 100, 105).unwrap();
	...
	fn test_multiple_entries() {
		...
		storage.save_recovery("new-buffer", chunks1, None, Some("New"), None, 0, 18).unwrap();

		// Second: large file (original_file_size > 0)
		let chunks2 = vec![RecoveryChunk::new(0, 0, b"chunk".to_vec())];
		storage.save_recovery("large-file", chunks2, None, Some("Large"), None, 100, 105).unwrap();
	...
	mod proptests {
		...
		enum Modification {
			...
			Replace { offset: usize, original_len: usize, content: Vec<u8> },
		...
		fn apply_modification(content: &[u8], modification: &Modification) -> (Vec<u8>, RecoveryChunk) {
			match modification {
				...
				Modification::Replace { offset, original_len, content: new_bytes } => {
					let offset = (*offset).min(content.len());
		...
		fn position_targeted_modification(
			...
			if buffer_size == 0 {
				Just((
					"beginning".to_string(),
					Modification::Insert { offset: 0, content: b"inserted".to_vec() },
				))
				.boxed()
			} else {
				...
				prop_oneof![
					...
					prop_oneof![
						...
						prop::collection::vec(any::<u8>(), 0..16).prop_map(move |content| {
							Modification::Replace { offset: 0, original_len: 1.min(buffer_size), content }
						}),
						Just(Modification::Delete { offset: 0, len: 1.min(buffer_size) }),
					]
					.prop_map(|m| ("beginning".to_string(), m)),
					// Middle: insert/replace/delete around middle
					prop_oneof![
						prop::collection::vec(any::<u8>(), 1..16)
							.prop_map(move |content| { Modification::Insert { offset: middle, content } }),
						prop::collection::vec(any::<u8>(), 0..16).prop_map(move |content| {
						...
						Just(Modification::Delete { offset: middle, len: 1.min(buffer_size - middle) }),
					]
					...
					prop_oneof![
						...
						Just(Modification::Delete { offset: end.saturating_sub(1), len: 1.min(buffer_size) }),
					]
		...
		fn test_insert_at_every_position() {
			...
			for insert_pos in 0..=original.len() {
				...
				let reconstructed = storage.reconstruct_from_chunks(&id, &original_path).unwrap();
				assert_eq!(reconstructed, expected, "Insert at position {} failed", insert_pos);
			}
		}
		...
		fn test_delete_at_every_position() {
			...
			for delete_pos in 0..original.len() {
				...
				let reconstructed = storage.reconstruct_from_chunks(&id, &original_path).unwrap();
				assert_eq!(reconstructed, expected, "Delete at position {} failed", delete_pos);
			}
		}
		...
		fn test_replace_at_every_position() {
			...
			for replace_pos in 0..original.len() {
				...
				let reconstructed = storage.reconstruct_from_chunks(&id, &original_path).unwrap();
				assert_eq!(reconstructed, expected, "Replace at position {} failed", replace_pos);
			}
		}
		...
		fn test_combined_beginning_middle_end_modifications() {
			...
			let id = "combined";
			storage
				.save_recovery(id, chunks, Some(&original_path), None, None, original.len(), expected.len())
				.unwrap();
		...
		fn test_single_byte_operations() {
			...
			// Insert before single byte
			{
				...
				storage.save_recovery(id, vec![chunk], Some(&original_path), None, None, 1, 2).unwrap();
				let reconstructed = storage.reconstruct_from_chunks(id, &original_path).unwrap();
			...
			// Replace single byte
			{
				...
				storage.save_recovery(id, vec![chunk], Some(&original_path), None, None, 1, 1).unwrap();
				let reconstructed = storage.reconstruct_from_chunks(id, &original_path).unwrap();
			...
			// Delete single byte
			{
				...
				storage.save_recovery(id, vec![chunk], Some(&original_path), None, None, 1, 0).unwrap();
				let reconstructed = storage.reconstruct_from_chunks(id, &original_path).unwrap();
	...
	fn test_migrate_flat_layout_to_standalone_scope() {
		...
		let scope = RecoveryScope::Standalone { working_dir: PathBuf::from("/home/user/project") };
		...
		assert_eq!(fs::read(scoped_dir.join("abc123.chunk.0")).unwrap(), b"chunk data");
	}
	...
	fn test_migrate_flat_layout_to_session_scope() {
		...
		let scope = RecoveryScope::Session { name: "my-session".into() };
	...
	fn test_migrate_skips_when_no_flat_files() {
		...
		let scope = RecoveryScope::Standalone { working_dir: PathBuf::from("/tmp") };
	...
	fn test_with_scope_creates_correct_directory() {
		...
		// Session scope
		let scope = RecoveryScope::Session { name: "test-session".into() };
		let storage = RecoveryStorage::with_scope(&base_dir, &scope);
		assert_eq!(storage.recovery_dir, base_dir.join("sessions").join("test-session"));
	}
}
