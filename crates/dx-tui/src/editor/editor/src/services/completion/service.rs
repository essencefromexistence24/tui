...
impl CompletionService {
	...
	pub fn new() -> Self {
		let mut svc = Self { providers: Vec::new(), pending_async: Vec::new() };
	...
	pub fn supply_async_results(
		...
		let pos = self.pending_async.iter().position(|(id, _)| *id == request_id)?;
	...
	fn rank(candidates: &mut Vec<CompletionCandidate>) {
		...
		candidates.sort_by(|a, b| {
			b.score.cmp(&a.score).then_with(|| a.label.to_lowercase().cmp(&b.label.to_lowercase()))
		});
		...
		candidates.retain(|c| {
			let key = (c.label.to_lowercase(), c.insert_text.clone().unwrap_or_default());
			seen.insert(key)
		});
	}
...
mod tests {
	...
	fn merges_multiple_providers() {
		let mut svc = CompletionService { providers: Vec::new(), pending_async: Vec::new() };
