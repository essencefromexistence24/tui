...
impl FsManager {
	...
	pub fn new(fs: Arc<dyn FileSystem + Send + Sync>) -> Self {
		Self { fs, pending_dir_requests: Arc::new(Mutex::new(HashMap::new())) }
	}
		...
		if should_execute {
			...
			if let Some(senders) = pending.remove(&path) {
				for sender in senders {
					...
					let _ = sender.send(
						result.as_ref().map(|v| v.clone()).map_err(|e| io::Error::new(e.kind(), e.to_string())),
					);
				}
			}
			...
			rx.await.unwrap_or_else(|_| Err(io::Error::other("Request cancelled")))
		}
	}
	...
	pub async fn exists(&self, path: &Path) -> bool {
		let fs = Arc::clone(&self.fs);
		let path = path.to_path_buf();
		tokio::task::spawn_blocking(move || fs.exists(&path)).await.unwrap_or(false)
	}
...
impl Clone for FsManager {
	fn clone(&self) -> Self {
		Self { fs: Arc::clone(&self.fs), pending_dir_requests: Arc::clone(&self.pending_dir_requests) }
	}
}
...
mod tests {
	...
	async fn test_request_deduplication() {
		...
		// Create test files
		for i in 0..10 {
			std_fs::write(temp_path.join(format!("file{}.txt", i)), format!("content{}", i)).unwrap();
		}
	...
	async fn test_list_dir_with_metadata() {
		...
		let entries = manager.list_dir_with_metadata(temp_path.to_path_buf()).await.unwrap();
	...
	async fn test_concurrent_different_dirs() {
		...
		for i in 0..5 {
			...
			std_fs::create_dir(&dir_path).unwrap();
			for j in 0..3 {
				std_fs::write(dir_path.join(format!("file{}.txt", j)), format!("content{}", j)).unwrap();
			}
		}
