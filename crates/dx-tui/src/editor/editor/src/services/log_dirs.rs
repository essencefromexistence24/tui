...
//! - `$XDG_STATE_HOME/dx/logs/` (typically `~/.local/state/dx/logs/`)
//!
//! Each Dx instance uses PID-based log files to support concurrent runs.
...
/// Get the base log directory for Dx, creating it if necessary.
///
/// Returns `$XDG_STATE_HOME/dx/logs/` (typically `~/.local/state/dx/logs/`).
/// Falls back to `~/.local/state/dx/logs/` if XDG_STATE_HOME is not set.
/// As a last resort, falls back to the system temp directory.
pub fn log_dir() -> &'static PathBuf {
	LOG_DIR.get_or_init(|| {
		let dir = get_xdg_log_dir().unwrap_or_else(|| std::env::temp_dir().join("dx-logs"));
		...
		if let Err(e) = fs::create_dir_all(&dir) {
			...
			return std::env::temp_dir().join("dx-logs");
		}
...
fn get_xdg_log_dir() -> Option<PathBuf> {
	...
	if let Ok(state_home) = std::env::var("XDG_STATE_HOME") {
		...
		if path.is_absolute() {
			return Some(path.join("dx").join("logs"));
		}
	}

	// Fall back to ~/.local/state
	if let Some(home) = home_dir() {
		return Some(home.join(".local").join("state").join("dx").join("logs"));
	}
...
/// Get the path for the main Dx log file for this process.
///
/// Returns `{log_dir}/fresh-{PID}.log`
pub fn main_log_path() -> PathBuf {
	log_dir().join(format!("dx-{}.log", std::process::id()))
}
...
fn cleanup_legacy_tmp_logs() {
	...
	if let Ok(entries) = fs::read_dir(&tmp_dir) {
		for entry in entries.flatten() {
			...
			// Check if this is an old Dx log file
			let should_cleanup = cleanup_patterns.iter().any(|pattern| name.starts_with(pattern));
...
fn is_file_older_than(path: &std::path::Path, age: Duration) -> bool {
	...
	SystemTime::now().duration_since(modified).map(|elapsed| elapsed > age).unwrap_or(false)
}

/// Extract PID from a filename like "dx-12345.log" or "rust-12345.log"
...
/// Print all directories used by Dx to stdout
pub fn print_all_paths(dir_context: &crate::config_io::DirectoryContext) {
	...
	writeln!(handle, "  workspaces/:  {}", data_dir.join("workspaces").display()).ok();
...
mod tests {
	...
	fn test_main_log_path_contains_pid() {
		...
		assert!(name.starts_with("dx-"), "Should start with fresh-");
		assert!(name.ends_with(".log"), "Should end with .log");
		assert!(name.contains(&std::process::id().to_string()), "Should contain PID");
	}
	...
	fn test_extract_pid_from_filename() {
		assert_eq!(extract_pid_from_filename("dx-12345.log"), Some(12345));
