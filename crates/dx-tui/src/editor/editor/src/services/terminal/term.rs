...
fn parse_osc7_path(payload: &str) -> Option<PathBuf> {
	...
	// cfg) because Dx may run on Linux while editing a Windows host, or vice
	// versa — the OSC 7 payload's flavour follows the *shell's* OS, not ours.
	let raw = {
		let bytes = raw.as_bytes();
		if bytes.len() >= 3 && bytes[0] == b'/' && bytes[2] == b':' { raw[1..].to_owned() } else { raw }
	};
	...
	if is_osc7_absolute(&raw) { Some(PathBuf::from(raw)) } else { None }
}
...
impl TerminalState {
	...
	pub fn get_line(&self, row: u16) -> Vec<TerminalCell> {
		...
		for col in 0..self.cols as usize {
			...
			cells.push(TerminalCell { c, fg, bg, bold, italic, underline, inverse });
		}
	...
	fn row_wraps(&self, line: Line) -> bool {
		...
		let grid = self.term.grid();
		grid[line][Column(self.cols as usize - 1)].flags.contains(Flags::WRAPLINE)
	}
	...
	match color {
		...
		Color::Named(named) => {
			...
			let rgb = match named {
				...
				alacritty_terminal::vte::ansi::NamedColor::Red => (255, 91, 91),
				alacritty_terminal::vte::ansi::NamedColor::Green => (80, 227, 194),
				alacritty_terminal::vte::ansi::NamedColor::Yellow => (255, 174, 4),
				alacritty_terminal::vte::ansi::NamedColor::Blue => (38, 113, 244),
				alacritty_terminal::vte::ansi::NamedColor::Magenta => (214, 112, 214),
				alacritty_terminal::vte::ansi::NamedColor::Cyan => (121, 192, 255),
				alacritty_terminal::vte::ansi::NamedColor::White => (228, 228, 228),
				alacritty_terminal::vte::ansi::NamedColor::BrightBlack => (82, 82, 82),
				alacritty_terminal::vte::ansi::NamedColor::BrightRed => (255, 123, 123),
				alacritty_terminal::vte::ansi::NamedColor::BrightGreen => (126, 237, 200),
				alacritty_terminal::vte::ansi::NamedColor::BrightYellow => (255, 197, 61),
				alacritty_terminal::vte::ansi::NamedColor::BrightBlue => (77, 142, 247),
				alacritty_terminal::vte::ansi::NamedColor::BrightMagenta => (232, 160, 232),
				alacritty_terminal::vte::ansi::NamedColor::BrightCyan => (160, 216, 240),
		...
		Color::Indexed(idx) => {
			...
			if idx < 16 {
				...
				let colors = [
					...
					(121, 192, 255), // Cyan
					(228, 228, 228), // White
					(82, 82, 82),    // Bright Black
					(255, 123, 123), // Bright Red
					(126, 237, 200), // Bright Green
					(255, 197, 61),  // Bright Yellow
					(77, 142, 247),  // Bright Blue
					(232, 160, 232), // Bright Magenta
					(160, 216, 240), // Bright Cyan
...
mod tests {
	...
	fn test_height_shrink_streams_spilled_rows() {
		...
		state.resize(80, 10);
		assert!(state.history_size() > before, "shrink should push rows into history");
	...
	fn test_no_scrollback_lost_across_many_mixed_resizes() {
		...
		let sizes = [(120u16, 24u16), (60, 30), (200, 18), (90, 40), (50, 22), (160, 50), (70, 20)];
		for b in 0..n / 20 {
			for i in 0..20 {
				let idx = b * 20 + i;
	...
	fn test_resize_during_alt_screen_then_capture() {
		...
		for i in 0..150 {
			assert!(text.contains(&format!("POST{i:04}#")), "post-alt output lost POST{i:04}");
		}
	...
	fn test_flush_new_scrollback_after_scroll() {
		...
		assert!(output.contains("Line 1"), "Scrollback should contain Line 1");
	}
	...
	fn test_append_visible_screen() {
		...
		let output = String::from_utf8_lossy(&buffer);
		assert!(output.contains("Line A"), "Visible screen should contain Line A");
		assert!(output.contains("Line B"), "Visible screen should contain Line B");
		assert!(output.contains("Line C"), "Visible screen should contain Line C");
	}
	...
	fn test_scrollback_then_visible_no_duplication() {
		...
		for i in 1..=15 {
			...
			assert!(count >= 1, "Line {} should appear at least once, but found {} times", i, count);
			// Allow for some overlap at boundaries, but not excessive duplication
			assert!(count <= 2, "Line {} appears {} times - too much duplication", i, count);
		}
	}
	...
	fn test_dsr_cursor_position_response() {
		...
		assert!(state.drain_pty_write_queue().is_empty(), "Write queue should be empty initially");
		...
		assert!(response.starts_with("\x1b["), "Response should start with ESC[");
		assert!(response.ends_with("R"), "Response should end with R");
		eprintln!("DSR response: {:?}", response);

		// Draining again should return empty
		assert!(state.drain_pty_write_queue().is_empty(), "Write queue should be empty after draining");
	}
	...
	fn test_osc7_sets_cwd_bel_terminated() {
		...
		assert_eq!(state.cwd(), Some(std::path::Path::new("/home/user/project")));
	...
	/// remote Windows shell can report this while Dx runs on Linux), so this
