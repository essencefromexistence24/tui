...
//! This powers Ctrl+Click / Ctrl+hover "open path in Dx" from the embedded
//! terminal.
//!
...
//! — relative to the terminal's OSC 7 cwd or Dx's working directory — is the
...
pub fn detect_link_at(line: &str, col: usize) -> Option<DetectedLink> {
	...
	let link_end = if line_no.is_some() { start + suffix_end } else { path_end };

	Some(DetectedLink { range: start..link_end, path, line: line_no, column: col_no })
}
...
mod tests {
	...
	#[test]
	fn path_line_col() {
		let line = "crates/dx-editor/src/app.rs:128:5: error";
		let got = detect_link_at(line, 10).unwrap();
		assert_eq!(got.path, "crates/dx-editor/src/app.rs");
		assert_eq!(got.line, Some(128));
		assert_eq!(got.column, Some(5));
		// Range underlines the path AND the :line:col suffix.
		assert_eq!(&line[got.range], "crates/dx-editor/src/app.rs:128:5");
	}
