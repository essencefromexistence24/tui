...
pub fn select_windows_shell() -> String {
	pick_shell(skip_app_execution_alias(), &default_candidates(), resolve_shell_candidate)
...
pub(crate) fn resolve_shell_candidate(cmd: &str) -> Option<PathBuf> {
	...
	path_var.split(';').filter(|d| !d.is_empty()).find_map(|dir| {
...
mod tests {
	...
	fn resolve_candidate_honors_explicit_path_and_misses() {
		...
		assert!(resolve_shell_candidate(&dir.path().join("missing.exe").to_string_lossy()).is_none());
	}
	...
	fn pick_shell_skips_alias_stub_only_when_flag_set() {
		...
		let candidates =
			vec![alias.to_string_lossy().into_owned(), real.to_string_lossy().into_owned()];
