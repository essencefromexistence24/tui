...
impl TextProperty {
	...
	pub fn new(start: usize, end: usize) -> Self {
		Self { start, end, properties: HashMap::new() }
	}
...
impl TextPropertyEntry {
	...
	pub fn normalize_widths(&mut self) {
		...
		if let Some(max_chars) = self.truncate_to_chars {
			...
			if cur > max {
				if max <= 3 {
					let cut_byte =
						self.text.char_indices().nth(max).map(|(b, _)| b).unwrap_or(self.text.len());
					self.text.truncate(cut_byte);
				} else {
					let keep = max - 3;
					let cut_byte =
						self.text.char_indices().nth(keep).map(|(b, _)| b).unwrap_or(self.text.len());
					self.text.truncate(cut_byte);
					self.text.push_str("...");
				}
			}
		...
		let needs_conversion = self.inline_overlays.iter().any(|o| matches!(o.unit, OffsetUnit::Char));
		if needs_conversion {
	...
	pub fn with_segment(mut self, text: impl Into<String>, style: Option<OverlayOptions>) -> Self {
		self.segments.push(StyledSegment { text: text.into(), style, overlays: Vec::new() });
		self
	}
}
...
mod normalize_tests {
	...
	fn styled(text: &str, fg_marker_bold: bool) -> StyledSegment {
		StyledSegment {
			...
			style: if fg_marker_bold {
				Some(OverlayOptions { bold: true, ..Default::default() })
			} else {
				None
			},
			overlays: Vec::new(),
	...
	fn segments_concatenate_into_text() {
		...
		e.segments = vec![styled("hello", false), styled(" ", false), styled("world", false)];
		e.normalize_widths();
	...
	fn styled_segments_with_multibyte_text_emit_correct_byte_overlays() {
		...
		let bold = e.inline_overlays.iter().find(|o| o.style.bold).expect("styled middle segment");
	...
	fn segment_nested_overlays_shift_by_segment_position_in_their_unit() {
		let mut e = entry("");
		e.segments = vec![
			StyledSegment { text: "abc".to_string(), style: None, overlays: vec![] },
			StyledSegment {
				text: "éé".to_string(),
				style: None,
		...
		let bold = e.inline_overlays.iter().find(|o| o.style.bold).expect("nested overlay");
	...
	fn segments_then_pad_works() {
		...
		e.normalize_widths();
		assert_eq!(e.text, "ab   ");
		let bold = e.inline_overlays.iter().find(|o| o.style.bold).expect("segment overlay");
