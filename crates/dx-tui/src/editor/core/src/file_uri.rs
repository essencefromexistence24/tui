...
mod tests {
	...
	fn brackets() {
		...
		let uri = path_to_file_uri(&path).unwrap();
		assert!(uri.contains("%5Btemp%5D"), "Brackets should be encoded: {uri}");
		assert_eq!(file_uri_to_path(&uri).unwrap(), path);
	}
	...
	fn hash_in_path() {
		...
		let uri = path_to_file_uri(&path).unwrap();
		assert!(uri.contains("C%23%20Projects"), "# and space should be encoded: {uri}");
		assert_eq!(file_uri_to_path(&uri).unwrap(), path);
	}
	...
	fn ampersand_and_equals_preserved() {
		...
		let uri = path_to_file_uri(&path).unwrap();
		assert!(uri.contains("a&b=c"), "sub-delimiters should be preserved: {uri}");
		assert_eq!(file_uri_to_path(&uri).unwrap(), path);
	}
	...
	fn verbatim_disk_path_produces_valid_uri() {
		...
		assert_eq!(verbatim_uri, plain_uri, "verbatim and plain paths must produce identical URIs");
	...
	fn verbatim_disk_path_with_special_chars() {
		...
		assert!(uri.starts_with("file:///D:/"), "URI should start with file:///D:/, got: {}", uri);
		assert!(!uri.contains(r"\\?\"), "URI must not contain verbatim prefix, got: {}", uri);
		assert!(uri.contains("%5Btemp%5D"), "brackets should be percent-encoded: {}", uri);
	}
	...
	fn verbatim_unc_matches_plain_unc() {
		...
		assert_eq!(unc_uri, verbatim_uri, "UNC and VerbatimUNC must produce identical URIs");
	}
	...
	fn device_namespace_returns_none() {
		...
		assert!(path_to_file_uri(&path).is_none(), "DeviceNS paths cannot be represented as file URIs");
