...
pub enum HookArgs {
	...
	BeforeDelete { buffer_id: BufferId, start: usize, end: usize },
	...
	PromptConfirmed { prompt_type: String, input: String, selected_index: Option<usize> },
	...
	PromptSelectionChanged { prompt_type: String, selected_index: usize },
...
mod tests {
	...
	fn hook_args_to_json_serializes_payload_fields() {
		let json =
			hook_args_to_json(&HookArgs::DiagnosticsUpdated { uri: "file:///x.rs".into(), count: 7 })
				.unwrap();
	...
	fn hook_args_to_json_empty_variants_produce_empty_object() {
		for args in [
			...
			assert_eq!(json, serde_json::json!({}), "variant should serialize as {{}}");
		}
	}
	...
	fn hook_args_to_json_terminal_output_fields_are_flat() {
		...
		assert_eq!(json["last_line"], "Do you want me to attempt a fix? (Y/n): ");
	}
