...
pub fn render(
	...
	if all_fonts.is_empty() {
		Paragraph::new(Line::from(Span::styled("DX", Style::default().fg(theme.accent))))
	...
	let max_title_rows = area.height.saturating_sub(SPLASH_TAIL_ROWS).clamp(1, MAX_FIGLET_ROWS);

	let figlet_lines = render_figlet_title(current_font, area.width, max_title_rows, rainbow, theme);
	...
	splash_lines.push(Line::from(Span::styled(desc_trimmed, Style::default().fg(theme.muted_fg))));
	splash_lines.push(Line::from(""));
	let hints = "↑ Menu  ↓ Animations  ← File Browser  → Editor  /home splash  type to chat";
	let hints_trimmed: String = hints.chars().take(area.width as usize).collect();
	splash_lines.push(Line::from(Span::styled(hints_trimmed, Style::default().fg(theme.muted_fg))));
...
fn render_figlet_title(
	...
	let mut lines: Vec<String> = figure
		...
		.map(|s| {
			...
			s.chars().map(|c| if c == hardblank { ' ' } else { c }).collect::<String>()
		})
		.collect();
...
fn compact_dx_title(rainbow: &RainbowEffect, theme: &ChatTheme) -> Vec<Line<'static>> {
	...
	for row in BLOCK {
		...
		for (i, ch) in row.chars().enumerate() {
			let color = if ch == ' ' { theme.muted_fg } else { rainbow.color_at(i) };
			spans.push(Span::styled(ch.to_string(), Style::default().fg(color)));
		}
...
fn get_valid_fonts() -> &'static [String] {
	static FONTS: Lazy<Vec<String>> = Lazy::new(|| {
		...
		names.sort_by(|a, b| {
			let ai = preferred.iter().position(|p| p.eq_ignore_ascii_case(a)).unwrap_or(999);
			let bi = preferred.iter().position(|p| p.eq_ignore_ascii_case(b)).unwrap_or(999);
