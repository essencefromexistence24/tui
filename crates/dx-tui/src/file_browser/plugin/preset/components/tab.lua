...
function Tab:scroll(event, step)
	local c = ya.child_at(ui.Rect { x = event.x, y = event.y }, self:reflow())
	if c and c ~= self then
		return c:scroll(event, step)
	end
	-- Fallback: scroll the current file list when hit-test misses a child.
	ya.emit("arrow", { step })
end
