...
pub(crate) fn action_for_menu_item(
    ...
    let entry_index = if opened_directly {
        selected_item
    } else {
        selected_item.checked_sub(1)?
    };
    let entries = entries();
...
mod tests {
    ...
    fn action_mapping_accounts_for_back_row() {
        assert_eq!(
            action_for_menu_item(0, true)
                .expect("direct first action")
                .command,
            "dx py status"
        );
        assert!(action_for_menu_item(0, false).is_none());
        assert_eq!(
            action_for_menu_item(1, false)
                .expect("submenu first action")
                .command,
            "dx py status"
        );
    }
