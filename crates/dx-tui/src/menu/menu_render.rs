    ...
    pub fn render_in_area(
        ...
        let menu_title = if let Some(ref custom) = self.custom_title {
            ...
            let item_count = self
                .menu_items
                .len()
                .saturating_sub(if self.opened_directly { 0 } else { 1 });
        ...
        ratatui::widgets::Clear.render(content_area, buf);
        buf.set_style(
            content_area,
        ...
        for (idx, (title, description)) in self.menu_items[self.scroll_offset..end_idx]
            .iter()
            .enumerate()
        {
            ...
            let item_text = if description == "TOGGLE_MODE" {
                ...
                let mode_indicator = if self.recording_mode {
                    "(Recording)"
                } else {
                    "(Viewing)"
                };
                ...
                if right_part.is_empty() {
                    ...
                    if char_count(left_part) > available_width {
                        format!(
                            "{}  {}",
                            truncate_with_ellipsis(left_part, available_width),
                            right_part
                        )
                    } else {
            ...
            let padded_line = format!(
                " {:<width$}",
                line_text,
                width = exact_width.saturating_sub(1)
            );
