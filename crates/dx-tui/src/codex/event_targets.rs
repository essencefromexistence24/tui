...
pub(crate) fn server_request_thread_id(request: &ServerRequest) -> Option<ThreadId> {
	match request {
		...
		ServerRequest::DynamicToolCall { params, .. } => ThreadId::from_string(&params.thread_id).ok(),
		ServerRequest::CurrentTimeRead { params, .. } => ThreadId::from_string(&params.thread_id).ok(),
...
pub(crate) enum ServerNotificationThreadTarget {
	Thread(ThreadId),
...
pub(crate) fn server_notification_thread_target(
