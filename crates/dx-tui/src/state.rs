...
pub struct UiState {
	...
	pub hovered_message_since: Option<Instant>,
	...
	/// Pointer is over the file browser scrollbar track.
	pub fb_scrollbar_hovered: bool,
	/// Exact file-browser scrollbar geometry from the latest rendered frame.
	pub fb_scrollbar_area: ratatui::layout::Rect,
...
impl UiState {
	pub fn new() -> Self {
		Self {
			...
			fb_scrollbar_area: ratatui::layout::Rect::default(),
...
impl ProviderState {
	pub fn new() -> Self {
		Self {
			...
			models_catalog: crate::providers::load_cached_catalog().unwrap_or_default(),
...
pub struct ChatState {
	...
	pub codex_connection:
		Option<tokio::sync::oneshot::Receiver<anyhow::Result<crate::codex_bridge::CodexBridge>>>,
	/// Text queued for submission to codex on next poll cycle.
	pub codex_pending_submit: Option<String>,
	/// Pending approval or user-input request from codex, waiting for user's y/n answer.
	pub codex_pending_request: Option<crate::codex_bridge::BridgeEvent>,
	/// Pending resume/fork operation result.
	pub codex_pending_operation:
		Option<tokio::sync::oneshot::Receiver<anyhow::Result<crate::codex_bridge::ResumeForkResult>>>,
}
...
impl ChatState {
	pub fn new() -> Self {
		...
		let mut state = Self {
			...
			codex_pending_submit: None,
			codex_pending_request: None,
			codex_pending_operation: None,
		};
	...
	pub fn add_user_message(&mut self, content: String) {
		...
		// For Codex mode, check for pending approval or enqueue text to submit
		if agent_mode == crate::modes::AgentMode::Codex {
			// If there's a pending approval request, resolve it with user's answer
			let pending = self.codex_pending_request.take();
			if let Some(crate::codex_bridge::BridgeEvent::RequestApproval(request)) = pending {
				let answer = user_for_model.trim().to_lowercase();
				let is_approve = matches!(answer.as_str(), "y" | "yes" | "approve" | "accept" | "allow");
				let is_deny = matches!(answer.as_str(), "n" | "no" | "deny" | "decline" | "never");

				if is_approve || is_deny {
					let decision_label = if is_approve { "Approved" } else { "Declined" };
					if let Some(ref mut bridge) = self.codex_bridge {
						let handle = bridge.request_handle();
						let resolution = bridge.pending_requests_mut().resolve_approval(&request, is_approve);
						if let Some(resolution) = resolution {
							tokio::spawn(async move {
								let _ =
									handle.resolve_server_request(resolution.request_id, resolution.result).await;
							});
						} else {
							tracing::warn!("no pending approval found for request");
						}
					}
					self.append_codex_delta(&format!("\n> _{decision_label}_\n"));
					self.show_toast(decision_label.into());
					return;
				}
				// User typed something other than y/n — re-queue and let them try again
				self.codex_pending_request =
					Some(crate::codex_bridge::BridgeEvent::RequestApproval(request));
				self.show_toast("Type y to approve or n to deny the request".into());
				return;
			}

			if self.codex_bridge.is_some() {
				self.codex_pending_submit = Some(user_for_model.clone());
			}
			return;
		}
		...
		tokio::spawn(async move {
			let result = match runtime {
				RuntimeMode::Remote => {
					...
					let run_agent = async {
						agent_backend
							.generate_stream_for_session(Some(&session_id), &agent_prompt, &history, agent_cb)
							.await
					};
						...
						match run_agent.await {
							...
							Err(agent_err) => {
								...
								match run_tool_loop.await {
									Ok(()) => Ok(()),
									Err(loop_err) => match run_plain.await {
										Ok(()) => Ok(()),
										Err(plain_err) => {
											Err(format!("agent: {agent_err}; tool-loop: {loop_err}; plain: {plain_err}"))
										}
									},
								}
							}
	...
	pub fn disconnect_codex(&mut self) {
		...
		self.codex_pending_operation = None;
...
impl ChatState {
	...
	pub fn interrupt_generation(&mut self) {
		...
		// Codex mode interrupt: send turn/interrupt RPC
		if self.agent_mode == AgentMode::Codex
			&& let Some(ref bridge) = self.codex_bridge
		{
			let handle = bridge.request_handle();
			let tid = bridge.thread_id().to_string();
			let turn_id =
				bridge.event_store().active_turn_id().map(|id| id.to_string()).unwrap_or_default();
			tokio::spawn(async move {
				let request_id = codex_app_server_protocol::RequestId::Integer(0);
				let _ = handle
					.request_typed::<codex_app_server_protocol::TurnInterruptResponse>(
						codex_app_server_protocol::ClientRequest::TurnInterrupt {
							request_id,
							params: codex_app_server_protocol::TurnInterruptParams { thread_id: tid, turn_id },
						},
					)
					.await;
			});
	...
	/// True when `(x, y)` is inside the painted rounded user bubble for `msg_idx`.
	pub fn pointer_inside_user_bubble(&self, x: u16, y: u16, msg_idx: usize) -> bool {
		use unicode_width::UnicodeWidthStr;
		let area = self.ui.chat_list_area;
		...
		if msg.role != crate::components::MessageRole::User {
			return false;
		}
		let content_w = area
			.width
		...
		let msg_width = (max_line + 2 + (H_PAD as usize) * 2)
			...
			.max(4) as u16;
		let msg_x = area.x + content_w.saturating_sub(msg_width).saturating_sub(RIGHT_EDGE_GAP);
		x >= msg_x && x < msg_x.saturating_add(msg_width)
	}
	...
	fn drain_agent_response_chunks(&mut self) {
		self.sync_pty_parts_into_messages();
		// Drain fast enough to keep up with the model, but bound work per frame so
		// keyboard input stays responsive while streaming.
		...
		while n < MAX_CHUNKS && Instant::now() < deadline {
			match self.agent_rx.try_recv() {
				...
				Ok(chunk) => {
					n += 1;
					if Self::is_stream_control_chunk(&chunk) {
						if !text_batch.is_empty() {
							self.append_agent_chunk(&std::mem::take(&mut text_batch));
						}
						self.append_agent_chunk(&chunk);
					} else {
						text_batch.push_str(&chunk);
						if text_batch.len() >= FLUSH_CHARS {
							self.append_agent_chunk(&std::mem::take(&mut text_batch));
						}
					}
				}
		...
		if !text_batch.is_empty() {
			self.append_agent_chunk(&text_batch);
		}
		if end_of_response {
			self.is_loading = false;
	...
	/// Chunks that must not be coalesced with plain token text (cards / IPC / fences).
	fn is_stream_control_chunk(chunk: &str) -> bool {
		use crate::permission_hub::{
			COMPACTION_MARKER, ERROR_CARD_PREFIX, INTERRUPTED_MARKER, PERM_REQ_PREFIX,
			QUESTION_REQ_PREFIX, RETRY_HINT_PREFIX,
		};
		chunk.starts_with(crate::stream_events::STREAM_EVENT_PREFIX)
			|| chunk.starts_with(PERM_REQ_PREFIX)
			|| chunk.starts_with(ERROR_CARD_PREFIX)
			|| chunk.starts_with(RETRY_HINT_PREFIX)
			|| chunk.starts_with(COMPACTION_MARKER)
			|| chunk.starts_with("\n__UPDATE_STATUS__\n")
			|| chunk.starts_with("\n__VOICE_")
			|| chunk.contains(QUESTION_REQ_PREFIX)
			|| chunk.contains(INTERRUPTED_MARKER)
			|| chunk.contains("```command")
			|| chunk.contains("<tool")
			|| chunk.contains("<subagent")
	}
	...
	pub fn poll_codex_events(&mut self) {
		...
		// Check for pending resume/fork operation result
		if let Some(rx) = &mut self.codex_pending_operation {
			match rx.try_recv() {
				Ok(Ok(result)) => {
					if let Some(ref mut bridge) = self.codex_bridge {
						bridge.apply_resume_fork(result);
					}
					self.codex_pending_operation = None;
					self.show_toast("Codex thread resumed".into());
				}
				Ok(Err(e)) => {
					self.codex_pending_operation = None;
					self.show_toast(format!("Resume/fork failed: {e}"));
				}
				Err(_) => {} // Not ready yet
			}
		}
		...
		// Drain enough events per frame to keep stream snappy, but leave headroom
		// for input/redraw (same budget idea as agent_rx drain).
		let deadline = Instant::now() + Duration::from_millis(8);
		let mut events = Vec::with_capacity(32);
		while events.len() < 64 && Instant::now() < deadline {
			let Some(event) = bridge.try_recv_event() else {
				break;
			};
			events.push(event);
		}
		...
		// Submit any pending text via the bridge
		if let Some(text) = self.codex_pending_submit.take()
			&& let Some(ref bridge) = self.codex_bridge
		{
			let handle = bridge.request_handle();
			let params = bridge.build_turn_start_params(&text);
			let request_id = bridge.next_request_id();
			let id = codex_app_server_protocol::RequestId::Integer(request_id);
			tokio::spawn(async move {
				let _ = handle
					.request_typed::<codex_app_server_protocol::TurnStartResponse>(
						codex_app_server_protocol::ClientRequest::TurnStart { request_id: id, params },
					)
					.await;
			});
		}
		...
		for event in events {
			match event {
				...
				crate::codex_bridge::BridgeEvent::ReasoningSummaryTextDelta { delta } => {
					self.append_codex_delta(&format!("<think>{delta}</think>"));
				}
				crate::codex_bridge::BridgeEvent::PlanDelta { delta } => {
					self.append_codex_delta(&format!("\n**[Plan]** {delta}\n"));
				}
				crate::codex_bridge::BridgeEvent::TurnStarted { .. } => {}
				crate::codex_bridge::BridgeEvent::TurnCompleted { .. } => {
					self.finish_codex_message();
				}
				crate::codex_bridge::BridgeEvent::ItemStarted { .. } => {}
				crate::codex_bridge::BridgeEvent::ItemCompleted { .. } => {}
				crate::codex_bridge::BridgeEvent::CommandExecutionOutputDelta { delta } => {
					self.append_codex_delta(&format!("```sh\n{delta}\n```"));
				}
				crate::codex_bridge::BridgeEvent::FileChangeOutputDelta { delta } => {
					self.append_codex_delta(&format!("```diff\n{delta}\n```"));
				}
				crate::codex_bridge::BridgeEvent::TerminalInteraction { .. } => {}
				crate::codex_bridge::BridgeEvent::ThreadNameUpdated { name } => {
					if let Some(name) = name {
						self.show_toast(format!("Thread renamed: {name}"));
					}
				}
				crate::codex_bridge::BridgeEvent::ThreadTokenUsageUpdated { .. } => {}
				crate::codex_bridge::BridgeEvent::ThreadSettingsUpdated { .. } => {}
				crate::codex_bridge::BridgeEvent::RequestApproval(req) => {
					let summary = crate::codex::pending_requests::extract_approval_summary(&req);
					self.append_codex_delta(&format!(
						"\n> **Approval requested**: {summary}\n> Type `y` to approve or `n` to deny.\n"
					));
					self.codex_pending_request = Some(crate::codex_bridge::BridgeEvent::RequestApproval(req));
					self.show_toast("Approval requested — type y/n".into());
				}
				crate::codex_bridge::BridgeEvent::HookStarted => {}
				crate::codex_bridge::BridgeEvent::HookCompleted => {}
				crate::codex_bridge::BridgeEvent::ServerError { message, .. } => {
					self.append_codex_delta(&format!("\n> Error: {message}"));
					self.show_toast(format!("Codex error: {message}"));
				}
				crate::codex_bridge::BridgeEvent::Warning { message } => {
					self.append_codex_delta(&format!("\n> Warning: {message}"));
				}
				...
				crate::codex_bridge::BridgeEvent::Lagged { skipped } => {
					self.show_toast(format!("Codex event lag, skipped {skipped}"));
				}
				crate::codex_bridge::BridgeEvent::Ignored => {}
			}
		}
		if disconnected {
			self.codex_bridge = None;
	...
	fn append_codex_delta(&mut self, delta: &str) {
		let last = self.messages.last_mut();
		match last {
			Some(msg) if msg.role == crate::components::MessageRole::Assistant => {
				msg.append_content(delta);
			}
			_ => {
				self.messages.push(crate::components::Message::assistant(delta.to_string()));
			}
		}
	}

	fn finish_codex_message(&mut self) {
		self.is_loading = false;
		if let Some(msg) = self.messages.last_mut()
			&& msg.role == crate::components::MessageRole::Assistant
...
mod tests {
	...
	use super::{AnimationType, ChatState, CommandDialog};
	...
	#[test]
	fn theme_mode_toggle_changes_variant() {
		let mut state = ChatState::new();
		let initial = state.theme_mode;

		state.toggle_theme_mode();

		assert_eq!(state.theme_mode, initial.toggle());
	}

	#[test]
	fn note_dialog_saves_sidebar_note() {
		let mut state = ChatState::new();
		state.ui.dialog = CommandDialog::Note;
		state.ui.dialog_input = "Review release checklist".into();

		state.activate_dialog_selection();

		assert_eq!(state.sidebar.snapshot().note, "Review release checklist");
		assert_eq!(state.ui.dialog, CommandDialog::None);
	}
	...
	#[test]
	fn codex_deltas_update_live_message_parts() {
		let mut state = ChatState::new();
		state.messages.push(Message::assistant(String::new()));

		state.append_codex_delta("streaming");
		state.append_codex_delta(" response");
	...
	fn animation_carousel_has_multiple_entries() {
		...
		assert!(
			carousel.len() >= 5,
			"carousel should have at least 5 animations, got {}",
			carousel.len()
		);
	}
