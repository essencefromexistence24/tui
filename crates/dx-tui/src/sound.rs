...
impl AudioBackend {
	fn new() -> Option<Self> {
		...
		Some(Self { device, loop_player: None })
	}
	...
	fn play_sequence(&self, sequence: SoundSequence) {
		...
		for asset in sequence {
			match rodio::Decoder::try_from(Cursor::new(asset.bytes())) {
				...
				Err(error) => {
					debug!("failed to decode bundled DX TUI sound {}: {error}", asset.file_name())
				}
			}
		}
	...
	fn start_loop(&mut self, asset: SoundAsset) {
		...
		match rodio::Decoder::try_from(Cursor::new(asset.bytes())) {
			...
			Err(error) => {
				debug!("failed to decode bundled DX TUI ambience {}: {error}", asset.file_name());
			}
		}
	}
...
mod tests {
	...
	fn train_ambience_trims_detected_leading_silence() {
		assert_eq!(SoundAsset::TrainRunning.loop_start_trim(), Duration::from_millis(25));
	...
	fn bundled_audio_manifest_contains_dx_tui_assets() {
		...
		let names = ALL_SOUND_ASSETS.iter().map(|asset| asset.file_name()).collect::<Vec<_>>();
	...
	fn bundled_audio_assets_are_embedded() {
		for asset in ALL_SOUND_ASSETS {
			assert!(!asset.bytes().is_empty(), "empty bundled audio asset: {}", asset.file_name());
		}
	}
	...
	fn exit_cue_uses_train_whistle_then_running_audio() {
		let names =
			SoundCue::Exit.configured_sounds().iter().map(|sound| sound.file_name()).collect::<Vec<_>>();
	...
	fn interaction_cues_use_dx_code_audio() {
		let cue_names: &[(SoundCue, &[&str])] = &[
			...
			(SoundCue::MenuOpen, &["dx_panel_open.wav", "dx_menu_snap.wav"]),
		...
		for (cue, expected) in cue_names.iter().copied() {
			let names = cue.configured_sounds().iter().map(|sound| sound.file_name()).collect::<Vec<_>>();
