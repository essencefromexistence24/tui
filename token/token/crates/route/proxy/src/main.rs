...
fn build_mode_pipeline(body: &str, mode: &str) -> Result<String, dx_route_core::CoreError> {
    match mode {
        "off" => Ok(body.to_string()),
        "lite" => dx_route_lite::compress(body, "full")
            .map(|r| r.text)
            .map_err(|e| dx_route_core::CoreError::EngineFailed("lite".into(), e.into())),
        "caveman" => dx_route_caveman::compress(body, "full")
            .map(|r| r.text)
            .map_err(|e| dx_route_core::CoreError::EngineFailed("caveman".into(), e.into())),
        "rtk" => dx_route_rtk::compress(body, None)
            .map(|r| r.text)
            .map_err(|e| dx_route_core::CoreError::EngineFailed("rtk".into(), e.into())),
        "ultra" => dx_route_ultra::compress(body, "full")
            .map(|r| r.text)
            .map_err(|e| dx_route_core::CoreError::EngineFailed("ultra".into(), e.into())),
        "aggressive" => dx_route_aggressive::compress(body, "full")
            .map(|r| r.text)
        ...
        _ => {
            ...
            dx_route_lite::compress(body, "full")
                .map(|r| r.text)
                .map_err(|e| dx_route_core::CoreError::EngineFailed("lite".into(), e.into()))
        }
    }
    ...
    if is_streaming {
        ...
        match upstream_req.body(upstream_body).send().await {
            Ok(up_resp) => {
                ...
                let resp_headers: Vec<(HeaderName, HeaderValue)> = up_resp
                    .headers()
                    .iter()
