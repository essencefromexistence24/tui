...
fn detect_command(command: Option<&str>) -> String {
    command
        ...
        .map(|lower| {
            {
                    ...
                    || lower.contains("git show")
                {
                    ...
                    "test-cargo"
                } else if lower.contains("npm test") || lower.contains("jest") {
                    "test-jest"
                } else if lower.contains("pytest") || lower.contains("python -m pytest") {
                    "test-pytest"
                } else if lower.contains("go test") {
                    "test-go"
                } else if lower.contains("docker build") || lower.contains("docker compose") {
                    "docker-build"
                } else if lower.contains("kubectl") {
                    "kubectl"
                } else if lower.contains("terraform") || lower.contains("tofu") {
                    "terraform-plan"
                ...
                {
                    "npm-install"
                } else if lower.contains("npm run build") || lower.contains("tsc") {
                    "build-typescript"
                    ...
                    "curl"
                } else if lower.contains("gh ") {
                    "gh"
                } else if lower.contains("aws ") {
                    "aws"
                } else if lower.contains("gcloud") {
                    "gcloud"
                } else if lower.contains("ssh") {
                    "ssh"
                } else if lower.contains("pip install") || lower.contains("poetry") {
                    "pip"
                } else if lower.contains("make ") {
                    "make"
                } else if lower.contains("systemctl") {
                    "systemctl-status"
                } else if lower.contains("grep ") || lower.contains("rg ") {
                    "shell-grep"
                } else if lower.contains("ls ") || lower.contains("ll ") {
                    "shell-ls"
                } else if lower.contains("ps ") || lower.contains("top ") {
                    "ps"
                } else if lower.contains("df ") || lower.contains("du ") {
                    "df"
        ...
        .unwrap_or_else(|| "generic-output".to_string())
}
...
fn load_filters(command_type: &str) -> Vec<Filter> {
    match command_type {
        ...
        _ => vec![Filter {
            ...
            preserve_re: compile_patterns(&[
                r"error", r"Error", r"ERROR", r"failed", r"panic", r"Panic", r"warning", r"Warning",
            ]),
            collapse_empty: true,
        }],
    }
...
mod tests {
    ...
    fn detects_command() {
        ...
        assert_eq!(
            detect_command(Some("docker build -t foo .")),
            "docker-build"
        );
