...
pub fn compress(body: &str, intensity: &str) -> LiteResult<LiteOutput> {
  {
    ...
    tracing::debug!(
      ...
      techniques.len(), intensity, original_len, compressed_len
    );

    Ok(LiteOutput { text, techniques: techniques.into_iter().map(String::from).collect(), original_len, compressed_len })
  }
}
...
mod tests {
  ...
  fn json_null_stripped() {
    ...
    assert!(!result.contains("null"), "null should be stripped, got: {}", result);
    assert!(result.contains("keep"), "keep should remain, got: {}", result);
  }
  ...
  fn intensity_standard_skips_json() {
    ...
    assert!(result.text.contains("null"), "standard should not strip nulls");
  }
