...
impl Default for SessionState {
  fn default() -> Self {
    Self { exact_cache: HashSet::new(), fuzzy_cache: Vec::new(), refs: HashMap::new(), threshold: 0.85 }
  }
}

impl SessionState {
  pub fn new(threshold: f64) -> Self {
    Self { exact_cache: HashSet::new(), fuzzy_cache: Vec::new(), refs: HashMap::new(), threshold }
  }
  ...
  pub fn compress(&mut self, body: &str) -> DedupOutput {
    ...
    DedupOutput {
      text: out, exact_deduped, fuzzy_deduped, refs: self.refs.clone(),
  ...
  fn is_fuzzy_duplicate(&self, line: &str) -> bool {
    ...
    self.fuzzy_cache.iter().any(|(cached, cached_hash)| {
      hamming_distance(hash, *cached_hash) <= max_distance || line.to_lowercase() == cached.to_lowercase()
    })
  }
}
...
impl CrossTurnCache {
  ...
  pub fn compress(&mut self, session_id: &str, body: &str) -> DedupOutput {
    ...
    self.states.entry(session_id.to_string())
...
fn simhash(text: &str) -> u64 {
  ...
  for word in text.split_whitespace() {
    let hash = u64::from_le_bytes(
      Sha256::digest(word.as_bytes())[..8].try_into()
        .expect("SHA-256 output is 64 bytes")
    );
    for (i, elem) in v.iter_mut().enumerate() {
      if (hash >> i) & 1 == 1 { *elem += 1; } else { *elem -= 1; }
    }
  }

  let mut fingerprint = 0u64;
  for (i, val) in v.iter().enumerate() {
    if *val > 0 { fingerprint |= 1 << i; }
  }
  fingerprint
}
