...
fn main() -> anyhow::Result<()> {
  ...
  match cli.command {
    ...
    Commands::Caveman { intensity } => {
      let r = dx_route_caveman::compress(&input, &intensity.unwrap_or_else(|| "full".into()))?;
      output(&r.text, &cli.format);
    }
    ...
    Commands::Aggressive { intensity } => {
      let r = dx_route_aggressive::compress(&input, &intensity.unwrap_or_else(|| "full".into()))?;
      output(&r.text, &cli.format);
    }
    Commands::Headroom => {
    ...
    Commands::Preview { mode } => {
      ...
      let savings = if before > 0 { (before - after) as f64 / before as f64 * 100.0 } else { 0.0 };
      eprintln!("mode={} original={}B compressed={}B savings={:.1}%", mode, before, after, savings);
    ...
    Commands::Stats { db } => {
      ...
      match dx_route_storage::Store::open(&path) {
        Ok(store) => {
          ...
          println!("requests={} tokens_saved={} avg_savings={:.1}%", s.total_requests, s.total_tokens_saved, s.avg_savings_pct);
          for e in &s.engine_breakdown {
            println!("  {:20} {} runs, {} tokens saved", e.engine, e.count, e.tokens_saved);
          }
        }
...
impl dx_route_core::Engine for LiteEngine {
  fn name(&self) -> &'static str { "lite" }
  fn apply(&self, body: &str, intensity: &str) -> dx_route_core::CoreResult<dx_route_core::EngineOutput> {
    dx_route_lite::compress(body, intensity).map(|r| dx_route_core::EngineOutput::new(r.text)).map_err(|e| wrap(e, "lite"))
  }
}
impl dx_route_core::Engine for CavemanEngine {
  fn name(&self) -> &'static str { "caveman" }
  fn apply(&self, body: &str, intensity: &str) -> dx_route_core::CoreResult<dx_route_core::EngineOutput> {
    dx_route_caveman::compress(body, intensity).map(|r| dx_route_core::EngineOutput::new(r.text)).map_err(|e| wrap(e, "caveman"))
  }
}
impl dx_route_core::Engine for RtkEngine {
  fn name(&self) -> &'static str { "rtk" }
  fn apply(&self, body: &str, _intensity: &str) -> dx_route_core::CoreResult<dx_route_core::EngineOutput> {
    dx_route_rtk::compress(body, None).map(|r| dx_route_core::EngineOutput::new(r.text)).map_err(|e| wrap(e, "rtk"))
  }
}
impl dx_route_core::Engine for UltraEngine {
  fn name(&self) -> &'static str { "ultra" }
  fn apply(&self, body: &str, intensity: &str) -> dx_route_core::CoreResult<dx_route_core::EngineOutput> {
    dx_route_ultra::compress(body, intensity).map(|r| dx_route_core::EngineOutput::new(r.text)).map_err(|e| wrap(e, "ultra"))
  }
}
impl dx_route_core::Engine for AggressiveEngine {
  fn name(&self) -> &'static str { "aggressive" }
  fn apply(&self, body: &str, intensity: &str) -> dx_route_core::CoreResult<dx_route_core::EngineOutput> {
    dx_route_aggressive::compress(body, intensity).map(|r| dx_route_core::EngineOutput::new(r.text)).map_err(|e| wrap(e, "aggressive"))
  }
}
impl dx_route_core::Engine for HeadroomEngine {
  fn name(&self) -> &'static str { "headroom" }
  fn apply(&self, body: &str, _intensity: &str) -> dx_route_core::CoreResult<dx_route_core::EngineOutput> {
    dx_route_headroom::compress(body, "full").map(|r| dx_route_core::EngineOutput::new(r.text)).map_err(|e| wrap(e, "headroom"))
  }
}
...
fn run_mode(input: &str, mode: &str) -> Result<String, dx_route_core::CoreError> {
  match mode {
    "lite" => dx_route_lite::compress(input, "full").map(|r| r.text).map_err(|e| dx_route_core::CoreError::EngineFailed("lite".into(), e.into())),
    "caveman" => dx_route_caveman::compress(input, "full").map(|r| r.text).map_err(|e| dx_route_core::CoreError::EngineFailed("caveman".into(), e.into())),
    "rtk" => dx_route_rtk::compress(input, None).map(|r| r.text).map_err(|e| dx_route_core::CoreError::EngineFailed("rtk".into(), e.into())),
    "ultra" => dx_route_ultra::compress(input, "full").map(|r| r.text).map_err(|e| dx_route_core::CoreError::EngineFailed("ultra".into(), e.into())),
    "aggressive" => dx_route_aggressive::compress(input, "full").map(|r| r.text).map_err(|e| dx_route_core::CoreError::EngineFailed("aggressive".into(), e.into())),
    "headroom" => dx_route_headroom::compress(input, "full").map(|r| r.text).map_err(|e| dx_route_core::CoreError::EngineFailed("headroom".into(), e.into())),
