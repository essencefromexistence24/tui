...
impl ToolRegister {
  fn tool_names(&self) -> Vec<&'static str> {
    vec![
      "compress_lite", "compress_caveman", "compress_rtk",
      "compress_ultra", "compress_aggressive", "compress_stacked",
      "compress_preview", "stats",
    ]
  }
}
...
async fn handle_mcp(
  ...
  let (is_error, body) = match method.as_str() {
    "initialize" => {
      // MCP initialize — check protocol version
      let proto = req.params.as_ref()
...
fn tool_definition(name: &str) -> (&'static str, Value) {
  match name {
    "compress_lite" => ("Lite — whitespace, ANSI, comments", serde_json::json!({
      "type": "object", "properties": {
        "text": {"type": "string"},
        "intensity": {"type": "string", "enum": ["standard", "full", "aggressive"]}
      }, "required": ["text"]
    })),
    "compress_caveman" => ("Caveman — rule-based prose condensation", serde_json::json!({
      "type": "object", "properties": {
        "text": {"type": "string"},
        "intensity": {"type": "string", "enum": ["lite", "full", "ultra"]}
      }, "required": ["text"]
    })),
    "compress_rtk" => ("RTK — command-aware output compression", serde_json::json!({
      "type": "object", "properties": {
        "text": {"type": "string"},
        "command": {"type": "string", "description": "command that generated the output"}
      }, "required": ["text"]
    })),
    "compress_ultra" => ("Ultra — token-scoring heuristic", serde_json::json!({
      "type": "object", "properties": {
        "text": {"type": "string"},
        "intensity": {"type": "string", "enum": ["standard", "full", "aggressive"]}
      }, "required": ["text"]
    })),
    "compress_aggressive" => ("Aggressive — 3-stage compressor", serde_json::json!({
      "type": "object", "properties": {
        "text": {"type": "string"},
        "intensity": {"type": "string", "enum": ["standard", "full", "aggressive"]}
      }, "required": ["text"]
    })),
    "compress_stacked" => ("Stacked — run multiple engines", serde_json::json!({
      "type": "object", "properties": {
        "text": {"type": "string"},
        "steps": {"type": "string", "description": "comma-separated engine names"}
      }, "required": ["text"]
    })),
    "compress_preview" => ("Preview — stats + diff", serde_json::json!({
      "type": "object", "properties": {
        "text": {"type": "string"},
        "mode": {"type": "string", "enum": ["lite", "caveman", "rtk", "ultra", "aggressive"]}
      }, "required": ["text"]
    })),
    "stats" => ("Compression statistics", serde_json::json!({
      "type": "object", "properties": {
        "db_path": {"type": "string"}
      }, "required": []
    })),
...
fn execute_tool(name: &str, args: &Value, state: &AppState) -> Result<Value, Value> {
  ...
  match name {
    "compress_lite" => {
      ...
      dx_route_lite::compress(text, intensity).map(|r| serde_json::json!({
    ...
    "compress_caveman" => {
      let intensity = args["intensity"].as_str().unwrap_or("full");
      dx_route_caveman::compress(text, intensity).map(|r| serde_json::json!({
        "text": r.text, "savings_pct": format!("{:.1}%", r.savings_pct()),
    ...
    "compress_rtk" => {
      ...
      dx_route_rtk::compress(text, cmd).map(|r| serde_json::json!({
    ...
    "compress_ultra" => {
      let intensity = args["intensity"].as_str().unwrap_or("full");
      dx_route_ultra::compress(text, intensity).map(|r| serde_json::json!({
    ...
    "compress_aggressive" => {
      let intensity = args["intensity"].as_str().unwrap_or("full");
      dx_route_aggressive::compress(text, intensity).map(|r| serde_json::json!({
    ...
    "compress_stacked" => {
      ...
      for step in steps.split(',') {
        current = match step.trim() {
          "lite" => dx_route_lite::compress(&current, "full").map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
          "caveman" => dx_route_caveman::compress(&current, "full").map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
          "rtk" => dx_route_rtk::compress(&current, None).map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
          "ultra" => dx_route_ultra::compress(&current, "full").map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
          "aggressive" => dx_route_aggressive::compress(&current, "full").map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
          _ => current,
        };
      }
    ...
    "compress_preview" => {
      ...
      let compressed = match mode {
        "lite" => dx_route_lite::compress(text, "full").map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
        "caveman" => dx_route_caveman::compress(text, "full").map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
        "rtk" => dx_route_rtk::compress(text, None).map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
        "ultra" => dx_route_ultra::compress(text, "full").map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
        "aggressive" => dx_route_aggressive::compress(text, "full").map_err(|e| serde_json::json!({"code": -32603, "message": e.to_string()}))?.text,
        _ => text.to_string(),
      };
      Ok(serde_json::json!({
    ...
    "stats" => {
      ...
      match dx_route_storage::Store::open(db) {
        Ok(store) => store.get_dashboard_stats()
