...
fn score_tokens(tokens: &[&str]) -> Vec<f64> {
    tokens
        ...
        .map(|token| {
            ...
            if is_url_or_path(clean) || clean.bytes().any(|b| b.is_ascii_digit()) {
                return 1.0;
            }
            if is_capitalized(clean) {
                return 0.8;
            }
            if clean.len() >= 8 {
                return 0.75;
            ...
            if is_stop_word(&clean.to_lowercase()) {
                return 0.1;
            }
            if clean.len() <= 2 {
                return 0.2;
...
mod tests {
    ...
    fn retains_errors_and_urls() {
        let input =
            "the quick brown fox jumped over the lazy dog ERROR: https://example.com/crash failed";
        let r = compress(input, "full").unwrap();
        assert!(r.text.contains("ERROR"), "errors should be kept");
        assert!(
            r.text.contains("https://example.com/crash"),
            "URLs should be kept"
        );
    }
    ...
    fn aggressive_more_removal_than_standard() {
        let input =
            "foo bar baz qux quux corge grault garply waldo fred plugh xyzzy thud".repeat(5);
