...
pub fn estimate_tokens(text: &str) -> CoreResult<usize> {
  match tiktoken_rs::cl100k_base() {
    ...
    Err(_e) => {
      ...
      let cjk_chars = text.chars().filter(|&c| c as u32 >= 0x4E00 && c as u32 <= 0x9FFF
        || c as u32 >= 0x3040 && c as u32 <= 0x30FF
        || c as u32 >= 0xAC00 && c as u32 <= 0xD7AF).count();
      let fallback = words + cjk_chars + text.len() / 4;
      debug!("tiktoken unavailable, using fallback estimate: {}", fallback);
      Ok(fallback)
    }
  }
}
...
pub fn enforce_budget(text: &str, max_tokens: u32) -> CoreResult<String> {
  ...
  bpe.decode(truncated).map_err(|e| CoreError::Tokenizer(e.to_string()))
}
...
pub fn apply_stacked(
  ...
  for step in plan {
    ...
    let result = engine.apply(&current, intensity).map_err(|e| {
      CoreError::EngineFailed(step.engine.clone(), e.into())
    })?;
    ...
    if let Some(budget) = step.target_budget {
      ...
      if tokens > budget as usize {
        ...
        debug!(budget, "hard budget enforced after engine '{}'", step.engine);
      }
    }
  }
  ...
  Ok(CompressedBody { text: current, stats, refs: all_refs })
}
...
pub async fn apply_stacked_async(
  ...
  for step in plan {
    ...
    let intensity = step.intensity.as_deref().unwrap_or("full");
    let result = engine.apply_async(&current, intensity).await.map_err(|e| {
      CoreError::EngineFailed(step.engine.clone(), e.into())
    })?;
...
mod tests {
  ...
  fn apply_stacked_two_engines() {
    let engines: Vec<Arc<dyn Engine>> = vec![
      Arc::new(FakeEngine("a")),
      Arc::new(FakeEngine("b")),
    ];
    let plan = vec![
      EngineStep { engine: "a".into(), intensity: Some("full".into()), target_budget: None },
      EngineStep { engine: "b".into(), intensity: Some("full".into()), target_budget: None },
    ];
